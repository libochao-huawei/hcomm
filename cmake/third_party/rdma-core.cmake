...
if(EXISTS ${RDMA_CORE_SEARCH_PATHS})
    ...
    if(NOT EXISTS "${RDMA_CORE_SRC_DIR}/CMakeLists.txt")
        ...
        if(EXISTS ${RDMA_CORE_ARCHIVE_PATH})
            message(STATUS "${RDMA_CORE_ARCHIVE_PATH} exist")
        endif()
        # -------------------------- Extracting --------------------------
    ...
    # -------------------------- 诊断信息打印开始 --------------------------
    message(STATUS "--------- [DEBUG] rdma-core Archive Diagnostics ---------")
    
    # 1. 打印压缩包的详细信息 (大小、权限、修改时间)
    execute_process(
        COMMAND ls -lh "${RDMA_CORE_ARCHIVE_PATH}"
        OUTPUT_VARIABLE LS_OUTPUT
        ERROR_VARIABLE LS_ERROR
    )
    message(STATUS "Archive ls -lh:\n${LS_OUTPUT}")
    if(LS_ERROR)
        message(STATUS "Archive ls error: ${LS_ERROR}")
    endif()

    # 2. 验证压缩包是否损坏 (尝试列出内容)
    message(STATUS "Testing archive integrity (tar -ztf)...")
    execute_process(
        COMMAND tar -ztf "${RDMA_CORE_ARCHIVE_PATH}"
        OUTPUT_VARIABLE TAR_LIST_OUTPUT
        ERROR_VARIABLE TAR_LIST_ERROR
        RESULT_VARIABLE TAR_LIST_RESULT
        OUTPUT_QUIET # 不列所有文件，太长，只看是否成功
    )
    if(NOT TAR_LIST_RESULT EQUAL 0)
        message(STATUS "Archive is CORRUPT or NOT a gzip tar! Error: ${TAR_LIST_ERROR}")
    else()
        message(STATUS "Archive integrity check: PASSED")
    endif()

    # 3. 检查目标目录状态
    message(STATUS "Target directory: ${ROOT_BUILD_PATH}")
    execute_process(
        COMMAND mkdir -p "${ROOT_BUILD_PATH}" # 确保目录存在
        COMMAND ls -ld "${ROOT_BUILD_PATH}"
        OUTPUT_VARIABLE DIR_LS_OUTPUT
    )
    message(STATUS "Target dir info: ${DIR_LS_OUTPUT}")

    # 4. 检查磁盘剩余空间
    execute_process(
        COMMAND df -h "${ROOT_BUILD_PATH}"
        OUTPUT_VARIABLE DF_OUTPUT
    )
    message(STATUS "Disk space:\n${DF_OUTPUT}")
    message(STATUS "---------------------------------------------------------")
    # -------------------------- 诊断信息打印结束 --------------------------
endif()
        execute_process(
            COMMAND tar -zxvf "${RDMA_CORE_ARCHIVE_PATH}" -C "${ROOT_BUILD_PATH}"
            WORKING_DIRECTORY "${ROOT_BUILD_PATH}"
            RESULT_VARIABLE TAR_RESULT
            OUTPUT_VARIABLE TAR_OUTPUT
            ERROR_VARIABLE TAR_ERROR  # 捕获错误输出
        )
        if(TAR_OUTPUT)
    message(STATUS "Tar extraction log:\n${TAR_OUTPUT}")
endif()
        if(NOT TAR_RESULT EQUAL 0)
            message(FATAL_ERROR "Failed to extract rdma-core archive: ${TAR_ERROR}")
        endif()
