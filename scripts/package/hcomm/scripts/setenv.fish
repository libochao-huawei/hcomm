#!/usr/bin/env fish
# Perform setenv for hcomm package
...
function get_install_param
    ...
    set -l install_info_key_array Hcomm_Install_Type Hcomm_Feature_Type Hcomm_UserName Hcomm_UserGroup Hcomm_Install_Path_Param Hcomm_Arch_Linux_Path Hcomm_Hetero_Arch_Flag
...
function get_install_dir
    ...
    set -l hetero_arch (get_install_param "Hcomm_Hetero_Arch_Flag" "$install_info")
    if test "$param_mult_ver" = "multi_version"
        if test "$hetero_arch" = "y"
            echo (realpath $curpath/../../../../../latest)/hcomm
        else
            echo (realpath $curpath/../../../latest)/hcomm
        end
    else
