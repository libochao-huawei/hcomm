...
#define UT_ENTRY_INIT(k, v) { \
		.key		= (void *)(k), \
		.data		= (void *)(v)}

#define UT_MAP_VAR(func,cls) __ut_map_##func##cls
...
#define UT_MAP_CLR(func,cls) ut_map_clear(&UT_MAP_VAR(func,cls))

#define UT_MAP_INSERT(func,cls,k,v) ({ \
	struct ut_entry e = UT_ENTRY_INIT(k,v); \
	struct ut_entry *ptr_e; \
	ptr_e = ut_map_insert(&UT_MAP_VAR(func,cls), &e);\
	ptr_e ? ptr_e->data : NULL;})
	
#define UT_MAP_ERASE(func,cls,k) ({ \
	struct ut_entry e = UT_ENTRY_INIT(k, NULL); \
	struct ut_entry *ptr_e;\
	ptr_e = ut_map_erase(&UT_MAP_VAR(func,cls), &e); \
	ptr_e ? ptr_e->data : NULL;})

#define UT_MAP_REPLACE(func,cls,k,v) ({ \
	void *data = UT_MAP_ERASE(func,cls,k); \
	UT_MAP_INSERT(func,cls,k,v);\
	data;})

#define UT_MAP_FIND(func,cls,k) ({ \
	struct ut_entry e = UT_ENTRY_INIT(k, NULL); \
	struct ut_entry *ptr_e;\
	ptr_e = ut_map_find(&UT_MAP_VAR(func,cls), &e); \
	ptr_e ? ptr_e->data : NULL;})
...
int _ut_cnt_range_check(struct ut_cnt_range *range_handle, int step) ;

#define UT_CNT_RANGE_VAR(func,cls) __ut_range_##func##cls
...
#define UT_CNT_RANGE_CHECK(func, cls, step) \
	_ut_cnt_range_check(&UT_CNT_RANGE_VAR(func, cls), step)
...
#define ut_free(p) 		ut_free_trace(p, __FILE__, __LINE__, 0)
...
#define UT_ENTRY_STUB_INIT(fn) {		 \
		.type		= UT_STUB_TYPE_ENTRY, \
		.name		= ut_stringify(fn), \
		.val.func	= (ut_stub_fn_t)fn }

/* init function return stub */
#define UT_RETUEN_STUB_INIT(value) {		 \
		.type		= UT_STUB_TYPE_RETURN, \
		.name		= ut_stringify(value), \
		.val.ret	= (int)value }


/* init stub for replace function entry to new */
#define UT_FUNC_ENTRY_STUB_INIT(oldfunc, newfunc) {		 \
		.state	= 0,	\
		.name	= ut_stringify(oldfunc), \
		.func	= (ut_stub_fn_t)oldfunc,	\
		.stub	= UT_ENTRY_STUB_INIT(newfunc) }

/* init stub for replace function return value to other */
#define UT_FUNC_RETUEN_STUB_INIT(oldfunc, retval) {		 \
		.state			= 0,	\
		.name			= ut_stringify(oldfunc), \
		.func			= (ut_stub_fn_t)oldfunc,	\
		.stub			= UT_RETUEN_STUB_INIT(retval) }
...
#define UT_RETURN_STUB_DEFINE(name, func, retvalue)		\
	UT_ENTRY_STUB_HANDLE_DECLAER(name) = UT_FUNC_RETUEN_STUB_INIT(func, retvalue)

		
#define ut_stub_active_entry(handle, func) ({ \
	struct ut_stub_data stub_data = UT_ENTRY_STUB_INIT(func); \
	_ut_stub_register(handle, &stub_data, __FILE__, __LINE__);\
})

#define ut_stub_active_return(handle, value) ({ \
	struct ut_stub_data stub_data = UT_RETUEN_STUB_INIT(value); \
	_ut_stub_register(handle, &stub_data, __FILE__, __LINE__);\
})
...
#define UT_RETURN_STUB(stub_handler, retvalue)		\
	UT_RETURN_STUB_DEFINE(_UT_RETURN_STUB_VAR(stub_handler),stub_handler,retvalue)
...
#define _UT_CK_TYPE_STUB_ACTIVE		(2)
#define _UT_CK_TYPE_STUB_DEACTIVE	(3)
#define _UT_CK_TYPE_STUB_UPDATE		(4)
...
#define _ut_check(ck_type, X, fmt,...) ({ \
		int x = (X); \
...
#define _ut_check_str_eq(ck_type, X, Y, fmt,...) ({ \
		...
		ut_fail_unless_msg(strcmp(x,y) == 0, ck_type, \
			"'"#X"=="#Y"' : "#X"==\"%s\", "#Y"==\"%s\" " fmt, \
			x, y, ## __VA_ARGS__);\
	})
...
#define _ut_check_mem_eq(ck_type, X, Y, size, fmt,...) ({ \
		...
		ut_fail_unless_msg(memcmp(x,y,s) == 0, ck_type, \
			"'"#X"=="#Y"' :\n== mem of '"#X"'\n%s\n== mem of '"#Y"'\n%s\n==\n" fmt, \
			_ut_mem_to_str(x,s,x_str,sizeof(x_str)), \
			_ut_mem_to_str(y,s,y_str,sizeof(y_str)), \
			## __VA_ARGS__);\
	})
...
#define UT_ASSERT_TRUE(C) \
	_ut_check(UT_CK_FLAG_ASSERT, (C), "\n")

#define UT_ASSERT_TRUE_MSG(C, fmt,...) \
	_ut_check(UT_CK_FLAG_ASSERT, (C), fmt, ##__VA_ARGS__)
...
#define UT_EXPECT_TRUE(C) \
	_ut_check(UT_CK_FLAG_EXPECT, (C), "\n")
#define UT_EXPECT_TRUE_MSG(C, fmt,...) \
	_ut_check(UT_CK_FLAG_EXPECT, (C), fmt, ##__VA_ARGS__)
