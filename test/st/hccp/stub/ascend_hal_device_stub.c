        ...
        if (pSession->sessionId == g_hdcMgr[i]->sessionId) {
            switch(attr) {
                case HDC_SESSION_ATTR_PEER_CREATE_PID:
                    *value = pSession->hostPid; // 模拟返回对端创建进程ID
                    break;
                default:
                    return DRV_ERROR_INVALID_VALUE;
            }
...
retry:
    ...
    if (ret < 0) {
        printf("[WARNING][DEVICE] connect failed,IP: %s, Port: %d , ret=%d\n", "127.0.0.1", listen_port, ret);
        usleep(10000); //100ms
        goto retry;
    }
    ...
    pSessionNode->hostPhyId          = peerDevid;           // 本端绑定设备ID
    ...
    *pSession = pSessionNode;
    ...
    printf("[DEVICE]connect success sessionId=%d, conn_fd=%d, local PID=%d, remote devID=%d, peerPid=%d, peerNode=%d\n",
...
hdcError_t drvHdcServerCreate(int devid, int serviceType, HDC_SERVER *pServer)
{
    ...
    printf("[DEVICE] Server created and bound to port %d\n", listen_port);
    // 启动监听（驱动创建即监听）
    ...
    printf("[DEVICE] Server is listening on port %d, listen fd=%d\n", listen_port, listen_fd);
    // ==================== 填充对外Server句柄 ====================
...
hdcError_t drvHdcSessionAccept(HDC_SERVER server, HDC_SESSION *session)
{
    ...
    if (conn_fd < 0) {
        perror("[ERROR][DEVICE] accept failed");
        free(pSession);
        return DRV_ERROR_INVALID_VALUE;
    }
    ...
    *session = pSession;
    ...
    printf("[DEVICE] accept success: phyId=%u, sessionId=%d, conn_fd=%d\n", pSession->hostPhyId, pSession->sessionId, pSession->conn_fd);
...
hdcError_t drvHdcGetMsgBuffer(struct drvHdcMsg *msg, int index, char **pBuf, int *pLen)
{
    ...
    printf("[DEVICE] GetMsgBuffer count=%d, index=%d, buf=%p, len=%d\n", msg->count, index, msg->bufList[index].pBuf, msg->bufList[index].len);
...
hdcError_t drvHdcAllocMsg(HDC_SESSION session, struct drvHdcMsg **ppMsg, int count)
{
    *ppMsg = calloc(1, sizeof(struct drvHdcMsg) + count * sizeof(struct drvHdcMsgBuf));
    ...
    (*ppMsg)->count = count;
    printf("[DEVICE] AllocMsg success: session=%d, count=%d\n", ((HdcSessionT *)session)->sessionId, count);
    return DRV_ERROR_NONE;
}
