...
static struct RsPingCtxCb g_tmp_ping_cb_ab;
static struct RsPingTargetInfo  g_ping_target_node_ab;
static struct RsPingTargetInfo  g_ping_target_node1_ab;

int RsDev2rscb_stub0_ab(uint32_t dev_id, struct rs_cb **rs_cb, bool init_flag)
{
...
int RsDev2rscb_stub1_ab(uint32_t dev_id, struct rs_cb **rs_cb, bool init_flag)
{
    pthread_mutex_init(&g_tmp_rs_cb1_ab.pingCb.pingMutex, NULL);
    g_tmp_rs_cb1_ab.pingCb.threadStatus = RS_PING_THREAD_RUNNING;
...
int rs_get_ping_cb_stub_ab(struct RaRsDevInfo *rdev, struct RsPingCtxCb **pingCb)
{
    *pingCb = &g_tmp_ping_cb_ab;
    (*pingCb)->pingPongOps = RsPingRoceGetOps();
    (*pingCb)->pingPongDfx = RsPingRoceGetDfx();
...
int rs_get_rs_cb_stub_ab(unsigned int phyId, struct rs_cb **rs_cb)
{
...
int rs_ping_roce_find_target_node_stub_ab(struct RsPingCtxCb *pingCb, struct PingQpInfo *target,
    struct RsPingTargetInfo  **node)
{
...
int rs_ping_roce_find_target_node_stub1_ab(struct RsPingCtxCb *pingCb, struct PingQpInfo *target,
    struct RsPingTargetInfo  **node)
{
...
void tc_rs_abnormal_ping_handle_init()
{
    ...
    mocker_invoke(RsDev2rscb, RsDev2rscb_stub0_ab, 1);
...
void tc_rs_abnormal_ping_handle_deinit()
{
    ...
    mocker_invoke(RsDev2rscb, RsDev2rscb_stub1_ab, 1);
    ret = RsPingHandleDeinit(chipId);
    EXPECT_INT_EQ(ret, 0);
    mocker_clean();

    pthread_mutex_destroy(&g_tmp_rs_cb1_ab.pingCb.pingMutex);
}
...
void tc_rs_abnormal_ping_init()
{
    ...
    mocker_invoke(RsDev2rscb, RsDev2rscb_stub0_ab, 1);
    g_tmp_rs_cb0_ab.pingCb.initCnt = 1;
    ret = RsPingInit(&attr, &info, &rdevIndex);
    EXPECT_INT_NE(ret, 0);
    mocker_clean();

    mocker_invoke(RsGetRsCb, rs_get_rs_cb_stub_ab, 1);
...
void tc_rs_abnormal_ping_target_add()
{
    ...
    g_tmp_ping_cb_ab.taskStatus = RS_PING_TASK_RUNNING;
    mocker_invoke(RsGetPingCb, rs_get_ping_cb_stub_ab, 1);
    ret = RsPingTargetAdd(&rdev, &target);
    EXPECT_INT_NE(ret, 0);
    mocker_clean();

    g_tmp_ping_cb_ab.taskStatus = RS_PING_TASK_RESET;
    mocker_invoke(RsGetPingCb, rs_get_ping_cb_stub_ab, 1);
    mocker_invoke(RsPingRoceFindTargetNode, rs_ping_roce_find_target_node_stub_ab, 1);
    ret = RsPingTargetAdd(&rdev, &target);
    EXPECT_INT_NE(ret, 0);
    mocker_clean();

    mocker_invoke(RsGetPingCb, rs_get_ping_cb_stub_ab, 1);
    mocker_invoke(RsPingRoceFindTargetNode, rs_ping_roce_find_target_node_stub1_ab, 1);
...
void tc_rs_abnormal_ping_task_start()
{
    ...
    pthread_mutex_init(&g_tmp_ping_cb_ab.pongQp.recvMrCb.mutex, NULL);
    pthread_mutex_init(&g_tmp_ping_cb_ab.pingQp.recvMrCb.mutex, NULL);
    pthread_mutex_init(&g_tmp_ping_cb_ab.pingMutex, NULL);
    RS_INIT_LIST_HEAD(&g_tmp_ping_cb_ab.pingList);
    RsListAddTail(&tmp_target.list, &g_tmp_ping_cb_ab.pingList);
    mocker_invoke(RsGetPingCb, rs_get_ping_cb_stub_ab, 1);
    ret = RsPingTaskStart(&rdev, &attr);
    EXPECT_INT_EQ(ret, 0);
    RsListDel(&tmp_target.list);
    mocker_clean();
    pthread_mutex_destroy(&g_tmp_ping_cb_ab.pingMutex);
    pthread_mutex_destroy(&g_tmp_ping_cb_ab.pingQp.recvMrCb.mutex);
    pthread_mutex_destroy(&g_tmp_ping_cb_ab.pongQp.recvMrCb.mutex);
}
