...
extern int ra_hdc_get_eid_by_ip_async(struct ra_ctx_handle *ctx_handle, struct IpInfo ip[], union hccp_eid eid[],
...
void tc_ra_hdc_session_accept()
{
    ...
    mocker(DlDrvHdcSessionAccept, 1 , 0);
    mocker(DlDrvHdcSetSessionReference, 1 , 0);
    mocker(DlHalHdcGetSessionAttr, 1 , 0);
    ret = RaHdcSessionAccept(0, &session, &host_tgid);
    EXPECT_INT_EQ(ret, 0);
    mocker_clean();
}
