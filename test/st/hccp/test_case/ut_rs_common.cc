...
protected:
    ...
    void TearDown() override {
        ...
        // if (device_pid_ != -1) {
        //     kill(device_pid_, SIGTERM);
        //     waitpid(device_pid_, nullptr, 0);
        // }
        if (device_pid_ != -1) {
        int status = 0;
        // 等待子进程退出，获取退出状态
        waitpid(device_pid_, &status, 0);

        // ============== 核心DEBUG打印 ==============
        cout << "[HOST 诊断] 子进程退出状态：" << endl;
        if (WIFEXITED(status)) {
            // 子进程自己调用 exit 退出了
            ...
            cout << "❌ 子进程崩溃！被信号杀死: " << sig << " (信号" << sig << ")" << endl;
            if (sig == 11) cout << "   → 11 = SIGSEGV 段错误（空指针/非法内存）" << endl;
            if (sig == 6)  cout << "   → 6 = SIGABRT 断言失败/abort()" << endl;
        }

        kill(device_pid_, SIGKILL); // 保险清理
    }
    }
};
