...
TEST_F(AllGatherTest, ag_pipeline_DR_2pod_2server_int32_4blk_equal)
{
    ...
    checkerOpParam.DataDes.dataType = CheckerDataType::DATA_TYPE_INT32;
    checkerOpParam.algName = "AlignedAllGatherDoublePipelineFor91093Executor";
...
{
    setenv("HCCL_BUFFERSIZE", "1", 1);
    for (int superpod = 2; superpod <= 3; superpod++) {
        for (int server = 1; server <= 2; server++) {
            for (int device = 7; device <= 8; device++) {
                RankTable_For_LLT gen;
                TopoMeta topoMeta;
                gen.GenTopoMeta(topoMeta, superpod, server, device);
                ...
                checkerOpParam.opMode = CheckerOpMode::OPBASE;
                checkerOpParam.devtype = CheckerDevType::DEV_TYPE_910_93;
                checkerOpParam.DataDes.count = 1000000; // 数据量
                checkerOpParam.DataDes.dataType = CheckerDataType::DATA_TYPE_INT32;
                checkerOpParam.algName = "AlignedAllGatherDoublePipelineFor91093Executor";
