 ...
 * Description: orion adapter hccp
 * Author: zhangqingwei
 * Create: 2024-02-04
 */
...
void* HrtRaTlvInit(HRaTlvInitConfig  &cfg) 
{
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[Init][RaTlv]errNo[0x%016llx] ra tlv init fail. params: mode=%u, device id=%u, version=%d, tlv_handle=%p, return: ret[%d]",
...
{
    CHECK_NULLPTR(tlv_handle, "[HrtRaTlvRequest] tlv_handle is nullptr!");
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[Request][RaTlv]errNo[0x%016llx] ra tlv request fail. params: tlv_handle=%p, tlv_module_type=%u, tlv_ccu_msg_type=%u, return: ret=%d",
...
void HrtRaTlvDeInit(void* tlv_handle)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[DeInit][RaTlv]errNo[0x%016llx] ra tlv deinit fail. params: tlv_handle=%p, return: ret=%d",
...
void HrtRaInit(HRaInitConfig &cfg)
{
    ...
    while (true) {
        ...
        if (!ret) {
            ...
            if (bTimeout) {
                THROW<NetworkApiException>(StringFormat("[Init][Ra]errNo[0x%016llx], ra init timeout[%lld], phy_id=%u, nic_position=%u, ret=%d",
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), timeout,config.phyId, config.nicPosition, ret));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
            // 非ra限速场景错误，不轮询。直接退出
            THROW<NetworkApiException>(StringFormat("[Init][Ra]errNo[0x%016llx] ra init fail, phy_id=%u, nic_position=%u, ret=%d",
...
void HrtRaDeInit(HRaInitConfig &cfg)
{
    ...
    while (true) {
        ...
        if (!ret) {
            ...
            bool bTimeout = ((std::chrono::steady_clock::now() - startTime) >= timeout);
            if (bTimeout) {
                THROW<NetworkApiException>(StringFormat("[DeInit][Ra]errNo[0x%016llx] ra deinit timeout[%lld], phy_id=%u, nic_position=%u, ret=%d",
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), timeout,config.phyId, config.nicPosition, ret));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
            // 非ra限速场景错误，不轮询。直接退出
            THROW<NetworkApiException>(StringFormat("[DeInit][Ra]errNo[0x%016llx] ra deinit fail, phy_id=%u, nic_position=%u, ret=%d",
                HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), config.phyId, config.nicPosition, ret));
        }
    }
}
...
static void SocketBatchConnect(SocketConnectInfoT conn[], u32 num)
{
    ...
    while (true) {
        ...
        if (!ret) {
            ...
            bool bTimeout = ((std::chrono::steady_clock::now() - startTime) >= timeout);
            if (bTimeout) {
                THROW<NetworkApiException>(StringFormat("[BatchConnect][RaSocket]errNo[0x%016llx] ra socket batch connect, timeout[%lld]. return[%d], params: num[%u]", 
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), timeout, ret, num));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
            THROW<NetworkApiException>(StringFormat("[BatchConnect][RaSocket]errNo[0x%016llx] ra socket batch connect fail, return[%d], params: num[%u]", 
...
static void HRaSocketBatchClose(struct SocketCloseInfoT conn[], u32 num)
{
    ...
    while (true) {
        ...
        if (!ret) {
            ...
            bool bTimeout = ((std::chrono::steady_clock::now() - startTime) >= timeout);
            if (bTimeout) {
                THROW<NetworkApiException>(StringFormat("[BatchClose][RaSocket]errNo[0x%016llx]  ra socket batch close, timeout[%d], return[%d], params: num[%u]", 
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), timeout, ret, num));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
            // 非ra限速场景错误，不轮询，直接退出
            THROW<NetworkApiException>(StringFormat("[BatchClose][RaSocket]errNo[0x%016llx] ra socket batch close fail, return[%d], params: num[%u]", 
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), ret, num));
        }
    }
}
...
static void HRaSocketListenStart(struct SocketListenInfoT conn[], u32 num)
{
    ...
    while (true) {
        ...
        if (ret == 0) {
            ...
            bool bTimeout = ((std::chrono::steady_clock::now() - startTime) >= timeout);
            if (bTimeout) {
                THROW<NetworkApiException>(StringFormat("[ListenStart][RaSocket]errNo[0x%016llx] ra socket listen start, timeout[%d], return[%d], params: num[%u]", 
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), EnvLinkTimeoutGet(), ret, num));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
            // 非ra限速场景错误，不轮询，直接退出
            THROW<NetworkApiException>(StringFormat("[ListenStart][RaSocket]errNo[0x%016llx] ra socket listen start fail, return[%d], params: num[%u]", 
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), EnvLinkTimeoutGet(), ret, num));
        }
    }
}
...
static bool RaSocketTryListenStart(struct SocketListenInfoT conn[], u32 num)
{
    ...
    if (ret == 0) {
        ...
        // 非ra限速场景错误，不轮询，直接退出
        THROW<NetworkApiException>(StringFormat("[TryListenStart][RaSocket]errNo[0x%016llx] ra socket listen start fail, return[%d], params: num[%u]", 
            HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), ret, num));
    }
}
...
static void HRaSocketListenStop(struct SocketListenInfoT conn[], u32 num)
{
    ...
    while (true) {
        ...
        if (!ret || ret == 228202) { // 待修改: 同步版本后 228202 修改为 SOCK_ENODEV
            ...
            bool bTimeout = ((std::chrono::steady_clock::now() - startTime) >= timeout);
            if (bTimeout) {
                THROW<NetworkApiException>(StringFormat("[ListenStop][RaSocket]errNo[0x%016llx]  ra socket listen stop fail, timeout[%d], return[%d], params: num[%u]", 
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), timeout, ret, num));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
            // 非ra限速场景错误，不轮询，直接退出
            THROW<NetworkApiException>(StringFormat("[ListenStop][RaSocket]errNo[0x%016llx] ra socket listen stop fail, return[%d], params: num[%u]", 
                    HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), ret, num));
        }
    }
}
...
void RaBlockGetSockets(u32 role, SocketInfoT conn[], u32 num) // 修改为内部函数，不对外
{
    ...
    while (true) {
        if ((std::chrono::steady_clock::now() - startTime) >= timeout) {
            THROW<NetworkApiException>(StringFormat("[HrtRaBlockGetSockets] get rasocket timeout role[%u], num[%u], goten[%u], timeout[%lld]s, the HCCL_CONNECT_TIMEOUT may be insufficient",
        ...
        if ((connectedNum == 0 && sockRet == 0) || (sockRet == SOCK_EAGAIN)) {
            ...
            THROW<NetworkApiException>(StringFormat("[Get][RaSocket]get rasocket error. role[%u], num[%u] sockRet[%d] connectednum[%u]", role, num,sockRet, connectedNum));
        } else {
            ...
            if (gotSocketsCnt == num) {
                ...
                THROW<NetworkApiException>(StringFormat("[Get][RaSocket]total Sockets[%u], more than needed num[%u]!", gotSocketsCnt, num));
            } else {
                SaluSleep(ONE_MILLISECOND_OF_USLEEP);
            }
        }
...
RaSocketFdHandleParam HrtRaBlockGetOneSocket(u32 role, RaSocketGetParam &param)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[HrtRaBlockGetOneSocket] copy tag[%s] to hccp failed, ret=%d, role=%u,socketHandle=%p, fdHandle=%p, remoteIp=%s",
    ...
    while (true) {
        ...
        if (ret == 0) {
            ...
            if (totalSentSize > sendSize) {
                THROW<NetworkApiException>(StringFormat("[Send][RaSocket]errNo[0x%016llx] ra socket send failed, fdHandle=%p data=%p size=%u retSize=%u", 
            ...
            THROW<NetworkApiException>(StringFormat("[Send][RaSocket]errNo[0x%016llx] ra socket send failed, fdHandle=%p data=%p size=%u retSize=%u ret=%d", 
        ...
        if (elapsed > timeout) {
            THROW<NetworkApiException>(StringFormat("[Send][RaSocket]errNo[0x%016llx] Wait timeout for sockets send, fdHandle[%p], data[%p], size[%u], retsize[%u], ret[%d]",
                       HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), fdHandle, data, sendSize, sentSize, ret));
        }
    }
...
bool HrtRaSocketNonBlockSend(const FdHandle fdHandle, void *data, u64 size, u64 *sentSize)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[hrtRaSocketNonBlockSend]errNo[0x%016llx] ra socket send size is too large, "
            "data[%p], size[%llu], fdHandle[%p], send size[%llu]", HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), data, size, fdHandle, *sentSize),
        size > SOCKET_SEND_MAX_SIZE);
...
void HrtRaSocketBlockRecv(const FdHandle fdHandle, void *data, u32 size)
{
    ...
    while (true) {
        if ((std::chrono::steady_clock::now() - startTime) >= timeout) {
            THROW<NetworkApiException>(StringFormat("[Recv][RaSocket]errNo[0x%016llx] Wait timeout for sockets recv, data[%p], "
        ...
        if ((rtRet == 0) && (recvSize > 0)) { // 接收完成，也有可能要多次接收
            getedLen += recvSize;
            if (getedLen > size) {
                THROW<NetworkApiException>(StringFormat("[Recv][RaSocket]errNo[0x%016llx] socket receive call RaSocketRecv failed,"
            ...
            THROW<NetworkApiException>(StringFormat("[Recv][RaSocket]recv fail, fdHandle=%p data=%p bufLen=%u, recLen=%lld, ret=%d",
                fdHandle,data, size, recvSize, rtRet));
        } else if (rtRet == SOCK_ESOCKCLOSED || rtRet == SOCK_CLOSE) { // 连接关闭，出错
            THROW<NetworkApiException>(StringFormat("[Recv][RaSocket]errNo[0x%016llx] recv fail, call RaSocketRecv failed, sock_esockclosed, fdhandle=%p data=%p bufLen=%u, recLen=%lld, ret=%d",
...
SocketHandle HrtRaSocketInit(HrtNetworkMode  netMode, RaInterface &in)
{
    ...
    if (ret != 0 || (socketHandle == nullptr)) {
        THROW<NetworkApiException>(StringFormat("[Init][RaSock]errNo[0x%016llx] ra socket init fail, call RaSocketInit failed, params: mode=%u, ip=%u device id=%u, family=%u. return: ret=%d",
...
void HrtRaSocketDeInit(SocketHandle socketHandle)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[DeInit][RaSocket]errNo[0x%016llx] rt socket deinit fail. call RaSocketDeinit failed, params: socketHandle[%p], return: ret[%d]",
...
void HrtRaSocketSetWhiteListStatus(u32 enable)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[Set][WhiteListStatus]errNo[0x%016llx] ra socekt set whilte list fail, call RaSocketSetWhiteListStatus failed, params: enable[%u], return: ret[%d]",
...
u32 HrtRaSocketGetWhiteListStatus()
{
    ...
    CHK_RET_THROW(NetworkApiException, 
        StringFormat("[Get][WhiteListStatus]errNo[0x%016llx] ra socekt get whilte list fail, call RaSocketGetWhiteListStatus failed, return: ret[%d]",
...
void HrtRaSocketWhiteListAdd(SocketHandle socketHandle, vector<RaSocketWhitelist> &wlists)
{
    ...
    CHECK_NULLPTR(wlists, "[HrtRaSocketWhiteListAdd] wlists is nullptr!");
    ...
    while (wlistNum > 0) {
        ...
        for (size_t idx = startIdx; idx < addListNum + startIdx; idx++) {
            ...
            CHK_RET_THROW(InternalException, 
                StringFormat("[Add][RaSocketWhiteList]errNo[0x%016llx]errName[HCCL_E_MEMORY] memory copy failed. params: socketHandle[%p], return: ret[%d]",
                    HCOM_ERROR_CODE(HcclResult::HCCL_E_MEMORY), socketHandle, sret),
                sret != EOK);
        ...
        CHK_RET_THROW(NetworkApiException, 
                StringFormat("[Add][RaSocketWhiteList]errNo[0x%016llx]errName[HCCL_E_TCP_CONNECT] ra white list add fail, call RaSocketWhiteListAdd failed, socketHandle[%p], num=%llu, return[%d].",
...
void HrtRaSocketWhiteListDel(SocketHandle socketHandle, vector<RaSocketWhitelist> &wlists)
{
    ...
    CHECK_NULLPTR(wlists, "[HrtRaSocketWhiteListDel] wlists is nullptr!");
    ...
    for (size_t idx = 0; idx < wlists.size(); idx++) {
        ...
        int sret = strcpy_s(wlistInfo.tag, sizeof(wlistInfo.tag), wlists[idx].tag.c_str());
        CHK_RET_THROW(InternalException, 
                StringFormat("[Del][RaSocketWhiteList]errNo[0x%016llx] memory copy failed. params: socketHandle[%p], return: ret[%d]",
                                    HCOM_ERROR_CODE(HcclResult::HCCL_E_MEMORY), socketHandle, sret),
                sret != EOK);
        wlistInfoVec.push_back(wlistInfo);
    }
    ...
    CHK_RET_THROW(NetworkApiException, 
                StringFormat("[Del][RaSocketWhiteList]errNo[0x%016llx] ra white list del fail, socketHandle[%p], num=%llu, return[%d].",
                   HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), socketHandle, wlists.size(), ret),
                ret != ACL_SUCCESS);
}
...
static u32 HrtGetIfNum(struct RaGetIfattr &config)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
                StringFormat("[Get][IfNum]errNo[0x%016llx] ra get if num fail. call RaGetIfnum failed, Input params: phyId=%u, nicPosistion=%u, return: ret[%d], num[%u]",
...
static void HrtGetIfAddress(struct RaGetIfattr &config, InterfaceInfo ifaddrInfos[], u32 &num)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
                StringFormat("[Get][IfAddress]errNo[0x%016llx] ra get if address fail. call RaGetIfaddrs failed, Input params: phyId=%u, nicPosistion=%u, return: ret[%d], num[%u]",
                   HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), config.phyId, config.nicPosition, ret, num),
                ret != ACL_SUCCESS);
}
...
vector<IpAddress> HrtGetDeviceIp(u32 devicePhyId)
{
    ...
    config.nicPosition          = static_cast<u32>(NetworkMode::NETWORK_OFFLINE);
...
RdmaHandle HrtRaRdmaInit(HrtNetworkMode netMode, RaInterface &in)
{
    ...
    rdevInfo.family   = in.address.GetFamily();
    rdevInfo.localIp = IpAddressToHccpIpAddr(in.address);
    s32 ret           = RaRdevInit(mode, NOTIFY, rdevInfo, &rdmaHandle);

    CHK_RET_THROW(NetworkApiException, 
                StringFormat("[Init][RaRdma]errNo[0x%016llx] rdma init fail. call RaRdevInit failed, Input params: phyId=%u, mode=%u, return: ret[%d]",
                   HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), in.phyId, mode, ret),
                ret != ACL_SUCCESS || (rdmaHandle == nullptr));
    return rdmaHandle;
}
...
void HrtRaRdmaDeInit(RdmaHandle rdmaHandle)
{
    ...
    HCCL_INFO("[HrtRaRdmaDeInit] Input params: rdmaHandle=%p", rdmaHandle);
    s32 ret = RaRdevDeinit(rdmaHandle, NOTIFY);
    CHK_RET_THROW(NetworkApiException, 
                StringFormat("[DeInit][RaRdma]errNo[0x%016llx] rt rdev deinit fail. call RaRdevDeinit failed, rdmaHandle=%p, return[%d].",
    ...
    while (true) {
        ...
        if (ret == 0) {
            ...
            *size = notifySize;
            ...
            THROW<NetworkApiException>(StringFormat("[Get][RaNotifyBaseAddr]errNo[0x%016llx] ra get notify base addr fail, call RaGetNotifyBaseAddr failed,"
...
QpHandle HrtRaQpCreate(RdmaHandle rdmaHandle, int flag, int qpMode)
{
    ...
    CHK_RET_THROW(NetworkApiException, 
                StringFormat("[Create][RaQp]errNo[0x%016llx] ra qp create fail. call RaGetNotifyBaseAddr, params: rdmaHandle[%p], flag[%d], qpMode[%d]. return: ret[%d]",
                   HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), rdmaHandle, flag, qpMode, ret),
                ret != ACL_SUCCESS || (connHandle == nullptr));
    return connHandle;
}
...
void HrtRaQpDestroy(QpHandle qpHandle)
{
    ...
    while (true) {
        ...
        if (ret == 0) {
            ...
            THROW<NetworkApiException>(StringFormat("[Destroy][RaQp]errNo[0x%016llx] ra qp destroy fail. call RaQpDestroy failed, qpHandle[%p], return[%d]."
...
void HrtRaQpConnectAsync(QpHandle qpHandle, FdHandle fdHandle)
{
    ...
    while (true) {
        ...
        if (ret == 0) {
            ...
            THROW<NetworkApiException>(StringFormat("[ConnectAsync][RaQp]errNo[0x%016llx] ra qp connect async fail. call RaQpConnectAsync failed, qpHandle=%p, fdHandle=%p, return[%d]"
