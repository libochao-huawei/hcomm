...
void* inter_all_reduce_task_for_hostnic(void* parg)
{
    ...
    ret = hcom_info.pComm->SetWorkspaceResource("tag_inter_all_reduce_task_for_hostnic", memptr, memSize, streamList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret =  hcom_info.pComm->AllReduce("tag_inter_all_reduce_task_for_hostnic",
                               para_info->sendbuff,
                               para_info->recvbuff,
                               para_info->count,
                               para_info->datatype,
...
#if 1
TEST_F(MPI_HostNic_Test, st_hrtGetHostIf)
{
    HcclResult ret = HCCL_SUCCESS;
    u32 ifAddrNum = 3;
...
#if 1
TEST_F(MPI_HostNic_Test, st_hrtGetIfNum)
{
    HcclResult ret = HCCL_SUCCESS;
...
#if 1
TEST_F(MPI_HostNic_Test, st_hrtRaGetDeviceIP)
{
    HcclResult ret = HCCL_SUCCESS;
    u32 ifAddrNum = 0;
...
#if 1
TEST_F(MPI_HostNic_Test, st_mpi_910_93_hcclMasterInfo_single_server_2p_success_normal)
{
    s32 portNum = 7;
    MOCKER(hrtGetHccsPortNum)
        .stubs()
