...
void* reduce_8p_task(void* parg)
{
    ...
    hcom_info.pComm->init(hcom_info.params, commConfig, hcom_info.rankTable);
    if (ret != HCCL_SUCCESS)
    {
...
#if 1
TEST_F(MPI_ReduceInter_Test, st_mpi_reduce_16ranks_2server_float_sum_1024count_root8)
{
...
TEST_F(MPI_ReduceInter_Test, st_mpi_reduce_16ranks_2server_float_sum_1024count_root0)
{
    nlohmann::json g_rank_table_16p = rank_table_910_2server_8rank;
       char file_name_t[] = "./st_mpi_reduce_inter_sum_16plus.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
