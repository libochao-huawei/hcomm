...
TEST_F(HcomTest, st_hcom_cltm_bcast)
{
...
TEST_F(HcomTest, st_hcom_gradient_segment_no_model)
{
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    bool isConfig = true;
    ret = HcomGetSplitStrategy(group, &feature, segment_index, isConfig);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(HcomTest, st_hcom_gradient_segment_more_than_8_segment)
{
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    bool isConfig = true;
    ret = HcomGetSplitStrategy(getGroup, &feature, segment_index, isConfig);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(HcomTest, st_hcom_gradient_segment_force_size_mode)
{
    ...
    bool isConfig = true;
    ret = HcomGetSplitStrategy(getGroup, &feature, segment_index, isConfig, FORCE_SIZE);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    EXPECT_EQ(segment_index[0], 18);
    EXPECT_EQ(segment_index[1], 19);
...
TEST_F(HcomTest, st_HcclCommGraphSendRecv)
{
    ...
    rtStream_t stream2;
    rt_ret = rtStreamCreate(&stream2, 5);
    EXPECT_EQ(rt_ret, RT_ERROR_NONE);
    ...
    rt_ret = rtStreamDestroy(stream);
    rt_ret = rtStreamDestroy(stream2);
...
TEST_F(HcomTest, st_checkSetGroupTopoInfo)
{
    nlohmann::json rank_table = rank_table_910_1server_1rank;
    char rank_table_file[] = "./st_checkSetGroupTopoInfo.json";
    std::ofstream outfile(rank_table_file, std::ios::out | std::ios::trunc | std::ios::binary);

    if (outfile.is_open())
    {
        outfile << std::setw(1) << rank_table << std::endl;
        HCCL_INFO("open %s success", rank_table_file);
    }
    else
    {
        HCCL_ERROR("open %s failed", rank_table_file);
    }
    ...
    options.insert(pair<string,string> (ge::OPTION_EXEC_RANK_TABLE_FILE,"./st_checkSetGroupTopoInfo.json"));
    ...
    int64_t rankSize = 0;
    //校验在进行GetGroupRankSize前是否进行了SetGroupTopoInfo
    ge::Status ret2 = ge::HcomTopoInfo::Instance().GetGroupRankSize(groupName.c_str(), rankSize);
    EXPECT_EQ(ret2, ge::GRAPH_SUCCESS);
    HcomDestroy();
    remove(rank_table_file);
}
...
TEST_F(HcomTest, st_setAttachedStream)
{
    ...
    rtStream_t rtstream1;
    rtStreamCreate(&rtstream1, 5);
    std::vector<void *> emptyStreams = {rtstream, rtstream1};
    s64 base = reinterpret_cast<s64>(&h);
    u32 graphId = 0;
    EXPECT_EQ(HcclCommSetAttachedStream(base, graphId, emptyStreams), HCCL_SUCCESS);

    MOCKER(&HcomGetCommByGroup).stubs().will(returnValue(HCCL_E_INTERNAL));
    EXPECT_EQ(HcomSetAttachedStream(nullptr, graphId, emptyStreams), HCCL_SUCCESS);

    EXPECT_EQ(0, rtStreamDestroy(rtstream));
    EXPECT_EQ(0, rtStreamDestroy(rtstream1));
    GlobalMockObject::verify();
}
...
TEST_F(HcomTest, SetAivCoreLimit)
{
...
TEST_F(HcomTest, st_HcclCommCalcTaskNum_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
...
#undef OPEN_BUILD_PROJECT
    ...
    HcomOpParam opParam;
    u32 taskNum = 0;
    HcclResult result = HcclCommCalcTaskNum(&opParam, taskNum);
    EXPECT_EQ(result, HCCL_SUCCESS);

    comm.userRankSize_ = 8;
    comm.moduleNum_ = 2;
    result = HcomCalcTaskNum(&opParam, taskNum);
    EXPECT_EQ(result, HCCL_SUCCESS);
    GlobalMockObject::verify();
#define OPEN_BUILD_PROJECT
}

TEST_F(HcomTest, st_HcclCommGlobalWorkSpace_When_Normal_Expect_ReturnlsDEV_TYPE_910_95)
{
    hccl::hcclComm *comm = new hccl::hcclComm(1, 1, "123");
    s64 opBaseHcom = (s64)comm;
    std::vector<void *> globalWorkSpaceAddr;
...
TEST_F(HcomTest, st_MakeEnumToDevType_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif    
    int make = 0;
    DevType devType = MakeEnumToDevType(make);
    EXPECT_EQ(devType, DevType::DEV_TYPE_910);
    GlobalMockObject::verify();
#define OPEN_BUILD_PROJECT
}

TEST_F(HcomTest, st_MakeEnumToDevType_not_find_When_Normal_Expect_ReturnlsDEV_TYPE_NOSOC)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif 
    int make = 7;
    DevType devType = MakeEnumToDevType(make);
    EXPECT_EQ(devType, DevType::DEV_TYPE_NOSOC);
    GlobalMockObject::verify();
#define OPEN_BUILD_PROJECT
}
 
TEST_F(HcomTest, st_HcomCheckCommValidity)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif
    const char* group = "hccl_world_group";
    HcclResult result = HcomCheckCommValidity(group);
    EXPECT_EQ(result, HCCL_E_PTR);
    GlobalMockObject::verify();
#define OPEN_BUILD_PROJECT
}
 
TEST_F(HcomTest, st_HcomSetQosCfg_When_Abnormal_Expect_ReturnlsHCCL_E_PTR)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif
    const char* group = "hccl_world_group";
    const u32 qosCfg = 0;
    HcclResult result = HcomSetQosCfg(group, qosCfg);
    EXPECT_EQ(result, HCCL_E_PTR);
    GlobalMockObject::verify();
#define OPEN_BUILD_PROJECT
}
...
TEST_F(HcomTest, st_HcomGetandClearOverFlowTasks_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif
    ...
    DevType deviceType = DevType::DEV_TYPE_910_95;
    MOCKER(hrtGetDeviceType).stubs().with(outBound(deviceType)).will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HcomGetandClearOverFlowTasks(group, hcclDumpInfo);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();
#define OPEN_BUILD_PROJECT
}
 
TEST_F(HcomTest, st_HcclCommGetandClearOverFlowTasks_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif
    ...
    DevType deviceType = DevType::DEV_TYPE_910_95;
    MOCKER(hrtGetDeviceType).stubs().with(outBound(deviceType)).will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HcclCommGetandClearOverFlowTasks(opBaseHcom, hcclDumpInfo);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();
#define OPEN_BUILD_PROJECT
}

TEST_F(HcomTest, st_HcclCommGraphSetAivCoreLimit_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif
    ...
    u32 blockDim = 4;
    s64 base = reinterpret_cast<s64>(h.get());
    DevType deviceType = DevType::DEV_TYPE_910_95;
    MOCKER(hrtGetDeviceType).stubs().with(outBound(deviceType)).will(returnValue(HCCL_SUCCESS));
    EXPECT_EQ(HcclCommGraphSetAivCoreLimit(base, blockDim), HCCL_SUCCESS);
