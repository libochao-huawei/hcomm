...
static atomic<u32> g_opFileNum(0);
...
TEST_F(HcomKernelBuilderTest, st_CalcOpRunningParam_common)
{
    ...
    char* rank_table_file = "./st_CalcOpRunningParam_common.json";
    char* rank_ID = "0";
...
TEST_F(HcomKernelBuilderTest, st_CalcOpRunningParam_common_51)
{
    ...
    char *rank_table_file = "./st_CalcOpRunningParam_common_51.json";
    char *rank_ID = "0";
...
HcclResult MockGetOffDeviceTypeWithoutDev1(DevType &devType)
{
...
TEST_F(HcomKernelBuilderTest, st_CalcOpRunningParam_unknown)
{
    ...
    .stubs()
    .will(invoke(MockGetOffDeviceTypeWithoutDev1));
...
TEST_F(HcomKernelBuilderTest, st_CalcOpRunningParam_V51)
{
    ...
    char* rank_table_file = "./st_CalcOpRunningParam_V51.json";
    char* rank_ID = "0";
...
TEST_F(HcomKernelBuilderTest, st_GetDevAndSerNumFromRankTable)
{
    HcclResult ret;
...
TEST_F(HcomKernelBuilderTest, st_GetCombineComTaskNum)
{
    s32 serverNum = 2;
    s32 deviceNumPerServer = 15;
    u32 intraTaskNum = 0;
...
TEST_F(HcomKernelBuilderTest, st_getAlltoAllvStagedScratchMemSize)
{
    ge::NodePtr nodeptr(new NodeTest);
    HcomOpsKernelBuilder hcomKernelInfo;
...
TEST_F(HcomKernelBuilderTest, st_getAlltoAllCountsDispl)
{
    ge::NodePtr nodeptr(new NodeTest);
    HcomOpsKernelBuilder hcomKernelInfo;
...
TEST_F(HcomKernelBuilderTest, st_getReduceScatterVCountsDispl)
{
    ge::NodePtr nodeptr(new NodeTest);
    HcomOpsKernelBuilder hcomKernelInfo;
...
TEST_F(HcomKernelBuilderTest, st_getOpWorkspaceMemSize)
{
    ge::NodePtr nodeptr(new NodeTest);
    HcomOpsKernelBuilder hcomKernelInfo;
...
ge::graphStatus FakeGetOption4(ge::GEThreadLocalContext *that, const std::string &optionExec, std::string &dumpDebugValue)
{
    nlohmann::json group_list =
    {
        {
    ...
    if (optionExec == ge::OPTION_EXEC_HCOM_GROUPLIST) {
        ...
        dumpDebugValue = R"([{"rank_id": "0","device_index": [0,0,0]},{"rank_id": "1","device_index": [0,0,1]}])";
    } else if (optionExec == ge::OPTION_EXEC_RANK_TABLE) {
        dumpDebugValue = R"({"status": "completed","version": "1.1","node_list":[{"node_id": "0","rank_list":[
        {"rank_id": "0","item_id": "0","rank_ip":"192.168.2.10"},
...
ge::graphStatus TaskNumGetOption(ge::GEThreadLocalContext *that, const std::string &optionExec, std::string &dumpDebugValue)
{
    ...
    if (optionExec == ge::OPTION_EXEC_HCOM_GROUPLIST) {
        ...
        dumpDebugValue = "./ut_task_num_one_server_hcom_test_" + to_string(g_opFileNum.load()) + ".json";
        g_opFileNum++;
...
TEST_F(HcomKernelBuilderTest, st_CalcOpTaskNum_1server_stream)
{
    ...
    MOCKER_CPP(&hcclComm::GetDevType)
    .stubs()
    .with(outBound(deviceType))
    .will(returnValue(HCCL_SUCCESS));
    ...
    ret = comm->init(hcom_info.params, commConfig, hcom_info.rankTable);
...
TEST_F(HcomKernelBuilderTest, st_GetAlgoLevel1)
{
...
TEST_F(HcomKernelBuilderTest, st_Hcom_SplitHcclOpType)
{
    int ret = HCCL_SUCCESS;
...
TEST_F(HcomKernelBuilderTest, st_CalAndSetOpWorkerSpaceForKnowShape)
{
...
TEST_F(HcomKernelBuilderTest, st_TestAttachStream)
{
    ge::NodePtr nodeptr(new NodeTest);
    HcomOpUtils hcomKernelInfo;
    HcomOpsKernelBuilder KernelInfo;
...
TEST_F(HcomKernelBuilderTest, st_offlinebuild_calcSubStreamNum_v2_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
...
TEST_F(HcomKernelBuilderTest, st_GetAndSetTaskNum_A5_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
#ifdef OPEN_BUILD_PROJECT
#undef OPEN_BUILD_PROJECT
#endif
    ...
    nodeptr->GetOpDesc()->SetType(type);
    nodeptr->GetOpDesc()->SetName(nodeName);
    ge::AttrUtils::HasAttr(nodeptr->GetOpDesc(), "DUMMY_SET_TRUE_MAXNUM");
    ge::AttrUtils::HasAttr(nodeptr->GetOpDesc(), "DUMMY_SET_TRUE_EMBEDDINGDIM");
    ge::AttrUtils::HasAttr(nodeptr->GetOpDesc(), "DUMMY_SET_TRUE_FLAGS");
    ge::AttrUtils::SetInt(nodeptr->GetOpDesc(), "flags", 1);
    ...
    MOCKER(GetOffDeviceTypeWithoutDev).stubs().with(outBound(devType)).will(returnValue(HCCL_SUCCESS));
    MOCKER(HcomOpUtils::IsNeedCalTaskNum).stubs().with(any()).will(returnValue(true));
    auto ret = hcomOpsKernelInfoStore_.GetAndSetTaskNum(*nodeptr, sCollectiveType, stremNum);
    EXPECT_EQ(ret, HCCL_SUCCESS);
#define OPEN_BUILD_PROJECT
}
