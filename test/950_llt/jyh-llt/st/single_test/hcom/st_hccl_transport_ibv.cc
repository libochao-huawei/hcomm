...
protected:
    ...
    virtual void SetUp()
    {
        ...
        InitEnvParam();
        std::string value;
        MOCKER(HrtRaGetHccnCfg).stubs().with(any(), any(), any(), outBound(value)).will(returnValue(HCCL_SUCCESS));
...
TEST_F(LinkIbvExpTest, st_link_base_test)
{
    s32 ret;
...
TEST_F(LinkIbvExpTest, st_link_direct_test)
{
    s32 ret;
    Stream stream(StreamType::STREAM_TYPE_OFFLINE);
    s32 mem_size = 256;
...
TEST_F(LinkIbvExpTest, st_transport_for_batchsendrecv_muti)
{
    s32 ret;
    Stream stream(StreamType::STREAM_TYPE_OFFLINE);
    s32 mem_size = 256;
...
TEST_F(LinkIbvExpTest, st_function_for_onedoorbell_ibv)
{
    s32 ret;
    Stream stream(StreamType::STREAM_TYPE_OFFLINE);
...
TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath)
{
    ...
    TransportManager transManager(cclBufferManager,
        ...
        localVnicIp,
        netDevCtxMap);
    MulQpInfo mulQpInfoManager;
    EXPECT_EQ(mulQpInfoManager.Init(InitParams{
        NICDeployment::NIC_DEPLOYMENT_HOST,
        static_cast<std::int32_t>(0),
        DevType::DEV_TYPE_910_93}), HCCL_SUCCESS);
    ...
    EXPECT_EQ(mulQpInfoManager.GetSpecialSourcePortsByIpPair(machinePara.srcPorts,
        KeyPair{machinePara.localIpAddr, machinePara.remoteIpAddr}), HCCL_SUCCESS);
...
TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath_patch1)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    HcclResult ret = HCCL_SUCCESS;
    setenv("HCCL_RDMA_QP_PORT_CONFIG_PATH", "/tmp/", 1);
...
TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath_patch2)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    HcclResult ret = HCCL_SUCCESS;
    setenv("HCCL_RDMA_QP_PORT_CONFIG_PATH", "/tmp/", 1);
    ...
    EXPECT_EQ(mulQpInfoManager.Init(InitParams{
        NICDeployment::NIC_DEPLOYMENT_HOST,
        static_cast<std::int32_t>(0),
        DevType::DEV_TYPE_910_93}), HCCL_E_PARA);

    unsetenv("HCCL_RDMA_QP_PORT_CONFIG_PATH");
    ResetInitState();
}

TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath_patch3)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    HcclResult ret = HCCL_SUCCESS;
    setenv("HCCL_RDMA_QP_PORT_CONFIG_PATH", "/tmp/", 1);
...
TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath_patch4)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    HcclResult ret = HCCL_SUCCESS;
    setenv("HCCL_RDMA_QP_PORT_CONFIG_PATH", "/tmp/", 1);
...
TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath_patch5)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    HcclResult ret = HCCL_SUCCESS;
    setenv("HCCL_RDMA_QP_PORT_CONFIG_PATH", "/tmp/", 1);
...
TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath_patch6)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    HcclResult ret = HCCL_SUCCESS;
    setenv("HCCL_RDMA_QP_PORT_CONFIG_PATH", "/tmp/", 1);
...
TEST_F(LinkIbvExpTest, st_transport_ibv_multi_qp_configpath_patch7)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(LinkIbvExpTest, st_transport_ipc_memory_error)
{
    s32 ret = 0;
    CCLBufferManager cclBufferManager;
    ...
    ret = transManager.CreateLink("tag", error_context, MachineType::MACHINE_SERVER_TYPE, "10.21.78.208", 0, true,
        ...
        false, link, false, retOut, netDevCtx, 1, false);
    EXPECT_EQ(ret, HCCL_E_TCP_CONNECT);
    GlobalMockObject::verify();
}

TEST_F(LinkIbvExpTest, st_BatchSendRecv_2rank_unflod)
{
...
u32 g_index = 0;
struct CqeErrInfo g_infolist[CQE_NUM] = {0};
const u32 CQE_ARRAY_SIZE = 128;
HcclResult stub_hrtRaGetCqeErrInfoList(RdmaHandle rdmaHandle, struct CqeErrInfo *infolist, u32 *num)
{
    u32 cqeNum = 0;
...
TEST_F(LinkIbvExpTest, st_error_cqe_test)
{
    s32 ret ;
    SetTcpMode(false);
    ...
    .stubs()
    .with(any(), outBoundP(&g_infolist[0], sizeof(struct CqeErrInfo)), any())
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(LinkIbvExpTest, st_direct_error_cqe_test)
{
    s32 ret ;
    SetTcpMode(false);
    ...
    for (u32 i = 0; i < CQE_NUM; i++) {
        if (i % 2 == 0) {
            g_infolist[i].qpn = ((StubQpInfo*)((reinterpret_cast<TransportDirectNpu*>(link->pimpl_))->qpHandles_[0]))->qpn;
            g_infolist[i].status = 12;
        } else {
            g_infolist[i].qpn = ((StubQpInfo*)((reinterpret_cast<TransportDirectNpu*>(link2->pimpl_))->qpHandles_[0]))->qpn;
            g_infolist[i].status = 12;
        }
    }
...
TEST_F(LinkIbvExpTest, st_aiv_get_RMA_queue)
{
    MOCKER_CPP(&TransportIbverbs::CreateOneQp)
    .stubs()
    .with(any())
