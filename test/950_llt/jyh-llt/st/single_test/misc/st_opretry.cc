...
public:
    ...
    {
        OpRetryBase::ClearStream(opStreamPtr_, clearStep);
        return HCCL_SUCCESS;
    }
    ...
    {
        OpRetryBase::ClearStreamWithOpId(opStreamPtr_, clearStep, opId, CuropId);
        return HCCL_SUCCESS;
    }
...
TEST_F(RetryTest, st_retry_Agent_processEvent)
{
    ...
    agentParam.isEnableBackupLink = true;
    agentParam.isEnableSdmaRetry = true;
    ...
    std::shared_ptr<OpRetryAgentRunning> opRetryAgentRunningSdma = std::make_shared<OpRetryAgentRunning>();
    opRetryAgentRunningSdma->lastPollAicpuTime_ = 
        std::chrono::steady_clock::now() - std::chrono::seconds(OP_RETRY_WAIT_AICPU_TIMEOUT);
    RetryContext contextsdma(agentParam, opRetryAgentRunningSdma);
    contextsdma.localRetryInfo_.opInfo.execStatus.kfcError = KfcError::kSdma;
    ret = opRetryAgentRunningSdma->ProcessEvent(&contextsdma);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();

    std::shared_ptr<OpRetryAgentRunning> agentRunningTemp = std::make_shared<OpRetryAgentRunning>();
    RetryContext context(agentParam, agentRunningTemp);
    ...
    ret = retryAgentWaitCmd->ClearStreamWithOpId(agentParam.opStreamPtr, rtClearStep_t::RT_STREAM_CLEAR, opId, curOpId);
    GlobalMockObject::verify();
...
TEST_F(RetryTest, st_retry_Server_processEvent)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(RetryTest, st_retry_Server_SetNeedRetryServerRank)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(RetryTest, st_retry_Server_handleErrTimeout)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    ...
    // 超时场景模拟 OP_RETRY_WAIT_CAN_RETRY_RANK超时时长60s影响线上执行时长 无法规避只能下掉
}
TEST_F(RetryTest, st_retry_base_function)
{
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Put).stubs().with(any()).will(returnValue(0));
    HcclResult ret = HCCL_SUCCESS;
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = retrySon->ClearStream(opStream, rtClearStep_t::RT_STREAM_STOP);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(RetryTest, st_retry_base_function_withLink)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_base_function_withIncreLink)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_base_function_LinkManager)
{
    s32 deviceLogicID = 0;
...
TEST_F(RetryTest, st_retry_base_function_withopid)
{
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Put).stubs().with(any()).will(returnValue(0));
    HcclResult ret = HCCL_SUCCESS;
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = retrySon->ClearStreamWithOpId(opStream, rtClearStep_t::RT_STREAM_STOP, Identify, Identify);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = retrySon->ProcessEvent(&context);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = retrySon->IssueResponse(ServerSocket1, retryinfo);
...
TEST_F(RetryTest, st_hrtNotifyReset)
{
    rtNotify_t notify;
...
TEST_F(RetryTest, st_Init_Agent)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_NotifyResetCallBack)
{
...
TEST_F(RetryTest, st_NotifyResetPool)
{
    HcclCommunicator communication;
...
TEST_F(RetryTest, st_PrintAgentInfoAfterFail)
{
...
TEST_F(RetryTest, st_retry_Server_SetNeedRetryServerRank_RDMA_Err) {
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(RetryTest, st_retry_Agent_ParseRdmaErr_sendRecv)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_Agent_ParseRdmaErr1)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_Agent_ParseRdmaErr2)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_Agent_ParseRdmaErr_bsr_SEND)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_Agent_ParseRdmaErr_bsr_RECV)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_Agent_ParseRdmaErr_bsr_remainSendErr)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_retry_Agent_ParseRdmaErr_bsr_remainRECVErr)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HDCommunicate::Get).stubs().with(any()).will(returnValue(0));
...
TEST_F(RetryTest, st_SetTransportStatus_bsr)
{
...
TEST_F(RetryTest, st_SetTransportStatus)
{
    MOCKER_CPP(&Transport::Stop)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(RetryTest, st_SetSignalTransport)
{
    MOCKER_CPP(&Transport::Stop)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(RetryTest, st_retry_Agent_WaitCmdCanRetryCase)
{
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HCCL_SUCCESS;
    u32 rankId = 0;
...
TEST_F(RetryTest, st_retry_Agent_WaitCmdCanRetryCase_BSR) 
{
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(RetryTest, st_InitOpRetry)
{
    HcclCommunicator communication;
...
TEST_F(RetryTest, st_retry_Agent_ProcessError)
{
...
TEST_F(RetryTest, st_retry_Server_processError)
{
    u32 rankId = 0;
    s32 deviceLogicId = 0;
    HcclIpAddress localIp = HcclIpAddress("192.168.100.110");
...
TEST_F(RetryTest, st_retry_Server_OpName_Inconsistent)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(RetryTest, st_retry_Agent_OpName_Inconsistent)
{
    u32 rankId = 0;
    HcclIpAddress localIp = HcclIpAddress("192.168.100.110");
...
TEST_F(RetryTest, st_retry_Agent_Inplace_Err)
{
    u32 rankId = 0;
    HcclIpAddress localIp = HcclIpAddress("192.168.100.110");
...
TEST_F(RetryTest, st_retry_Server_Inplace_Err)
{
    MOCKER_CPP(&OpRetryBase::Send).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(any()).will(returnValue(0));
    MOCKER(hrtCtxSetCurrent).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(RetryTest, st_retry_agent_wait_resume_processEvent)
{
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(RetryTest, ut_retry_server_wait_resume_processEvent)
{
    ...
    opRetryManager.ExitWaitResumeState("group1", false, isChangedLink);
    ...
    opRetryManager.ExitWaitResumeState("group2", true, isChangedLink);
    opRetryManager.serverOpRetry["group2"].retryCtx->isOpRetryQuit = true;
    opRetryManager.SetRetryStateToWaitResume("group2", true);
}

TEST_F(RetryTest, st_retry_opbase_switch_state)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclIpAddress localIp = HcclIpAddress("192.168.100.110");
    u32 rankId = 0;
...
TEST_F(RetryTest, st_retry_Agent_Resume_Check_Link)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclOpStreamRes myMap;
    OpRetryAgentParam agentParam;
...
TEST_F(RetryTest, st_retry_Server_Resume_Check_Link)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclIpAddress localIp = HcclIpAddress("192.168.100.110");
    u32 rankId = 0;
...
TEST_F(RetryTest, st_SetTransportResumeStatus)
{
    MOCKER_CPP(&Transport::Stop)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
