...
TEST_F(OpCounterTest, st_op_counter)
{
    ResetInitState();
    InitExternalInput();
    ...
    u32 qosCfg = 0;
    ret = dispatcher->SetQosCfg(qosCfg);
    ...
    rt_ret = rtStreamCreate(&rtStream, 5);
    ...
    // 销毁资源
    if (rtStream != NULL) {
        rt_ret = rtStreamDestroy(rtStream);
        EXPECT_EQ(rt_ret, RT_ERROR_NONE);
    }
...
TEST_F(OpCounterTest, st_clear_opCounter)
{
    ResetInitState();
