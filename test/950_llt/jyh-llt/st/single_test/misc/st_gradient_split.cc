...
TEST_F(HcomGradientSplitTuneTest, st_Gradient_AutoTuning_E2E)
{
    ...
    bbRet = ge::AttrUtils::SetStr(nodeGroup, "group", group);
    EXPECT_EQ(bbRet, true);
    bbRet = ge::AttrUtils::SetInt(nodeFusionId, "fusion_id", fusionid);
    EXPECT_EQ(bbRet, true);
...
TEST_F(HcomGradientSplitTuneTest, st_gradient_HcomOpsKernelInfoStore_InitAlltoAllHostMem)
{
    ...
    EXPECT_EQ(HCCL_SUCCESS, athcomOpsKernelInfo.InitAlltoAllHostMem(dateType, recvCount, hostMem));
    ...
    recvCount = dataBytes/unitSize;
    EXPECT_EQ(HCCL_E_NOT_SUPPORT, athcomOpsKernelInfo.InitAlltoAllHostMem(dateType, recvCount, hostMem));
}
...
TEST_F(HcomGradientSplitTuneTest, st_gradient_Manage_Split_API)
{
    HcomSetAutoTuneMode(true);
    int ret;

    u32 ranksize = 0;
    ret = HcomGetRankSize(NULL, &ranksize);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    u32 rankid = 0;
    ret = HcomGetRankId(NULL, &rankid);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    u32 groupRanksNum = 0;
    u32 groupRanks[1] = {0};
    ret = HcomCreateGroup(NULL, groupRanksNum, static_cast<u32 *>(groupRanks));
    EXPECT_EQ(ret, HCCL_SUCCESS);

    ret = HcomDestroyGroup(NULL);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    u32 groupRank = 0;
    u32 worldRank = 0;
    ret = HcomGetWorldRankFromGroupRank(NULL, groupRank, &worldRank);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = HcomGetGroupRankFromWorldRank(worldRank, NULL, &groupRank);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    u32 idxList[3] = {1, 2, 3};
    ret = HcomSetGradFusionByIndex(NULL, 0, idxList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    float sizeList[3] = {1, 2, 3};
    ret = HcomSetGradFusionBySize(NULL, 3, sizeList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}