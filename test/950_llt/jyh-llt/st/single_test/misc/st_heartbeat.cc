...
TEST_F(HeartBeatTest, st_IsKeyEvent_OpretryNotSupport)
{
    ...
    CommConfiger::GetInstance().SetCommConfig(commconfig, identifier);
    CommConfiger::GetInstance().SetCommConfigExecTimeOut(68, identifier);
    HcclUs curTime;
    EXPECT_EQ(Heartbeat::GetInstance(0).IsKeyEvent(bf, curTime, identifier), true);
    GlobalMockObject::verify();
}
