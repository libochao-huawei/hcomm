...
class HcclCommAicpuTest_ST : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        ...
        std::cout << "\033[36m--HcclCommAicpuTest_ST SetUP--\033[0m" << std::endl;
    }
    static void TearDownTestCase()
    {
        ...
        std::cout << "\033[36m--HcclCommAicpuTest_ST TearDown--\033[0m" << std::endl;
    }
    virtual void SetUp()
    {
        s32 portNum = -1;
...
HcclDispatcher HcclCommAicpuTest_ST::dispatcherPtr = nullptr;
DispatcherPub *HcclCommAicpuTest_ST::dispatcher = nullptr;
...
TEST_F(HcclCommAicpuTest_ST, BuildOpRetryParam)
{
...
TEST_F(HcclCommAicpuTest_ST, hcclImpl_BuildOpResParam_ok)
{
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(HcclCommAicpuTest_ST, hcclImpl_BuildOpRemoteLinkP2pResParam_ok)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclCommParams params;
    RankTable_t rankTable;
...
TEST_F(HcclCommAicpuTest_ST, AiCpuCreateAndGetNotify)
{
    HcclResult ret = HCCL_SUCCESS;
    std::unique_ptr<HcclCommunicator> implBase(new (std::nothrow) HcclCommunicator());
...
TEST_F(HcclCommAicpuTest_ST, CommunicatorCustomTest)
{
    char c = '1';
    MOCKER(realpath)
    .stubs()
    ...
    MOCKER_CPP(&HcclCommunicator::SetAicpuCommEngine).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
