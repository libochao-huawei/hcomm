...
TEST_F(GlobalMemMgrTest, st_global_mem_mgr_reg)
{
    GlobalMemRegMgr mgr;
...
TEST_F(GlobalMemMgrTest, st_global_mem_mgr_dereg)
{
    GlobalMemRegMgr mgr;
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(GlobalMemMgrTest, st_global_mem_mgr_reg_maximum)
{
    GlobalMemRegMgr mgr;
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(GlobalMemMgrTest, st_global_mem_mgr_backupInit)
{
...
TEST_F(GlobalMemMgrTest, St_CheckOneSidedBackupAndSetDevId_When_AllNicIpIsEmpty_Expect_ReturnSuccess)
{
    MOCKER(hrtGetDeviceType).stubs().will(invoke(hrtGetDeviceTypeForTest));
    MOCKER(hrtGetPairDevicePhyId).stubs().will(invoke(hrtGetPairDevicePhyIdForTest));
    MOCKER(hrtRaGetDeviceIP).stubs().will(invoke(hrtRaGetDeviceIPForTest));
    ...
    u32 backupDevPhyId = 0;
    u32 backupDevLogicId = 0;
    std::vector<HcclIpAddress> localIpList;
    bool isOneSidedTaskAndBackupInitA3 = false;
    HcclResult ret;
    ret = mgr.CheckOneSidedBackupAndSetDevId(ipAddr, backupDevPhyId, backupDevLogicId,
        localIpList, isOneSidedTaskAndBackupInitA3);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();
}