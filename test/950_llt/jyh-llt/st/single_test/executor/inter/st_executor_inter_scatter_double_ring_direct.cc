...
TEST_F(ScatterDoubleRingDirectTest, create_prepare_destructor_001)
{
    // 获取模板实例
    std::unique_ptr<AlgTemplateBase> tempAlg =
        AlgTemplateRegistry::Instance().GetAlgTemplate(TemplateType::TEMPLATE_SCATTER_DOUBLE_RING_DIRECT, dispatcher);
    EXPECT_NE(tempAlg, nullptr);
    // 调用新增的prepare
    s32 ret = tempAlg->Prepare(&opInfo, userRank, localRank, subStreams, mainSignals,
        subSignals, ringsOrder, multiRingSliceZeroVector, userMemInputSlicesVector);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    // 转换子类指针
    ScatterDoubleRingDirect *alg = dynamic_cast<ScatterDoubleRingDirect *>(tempAlg.get());
    // 校验成员变量
    EXPECT_EQ(alg->opInfo_, &opInfo);
    EXPECT_EQ(alg->userRank_, userRank);
    EXPECT_EQ(alg->subRingRank_, localRank);
    ...
    EXPECT_EQ(alg->ringsOrders_.size(), ringsOrder.size());
    for (u32 i = 0; i < ringsOrder.size(); ++i) {
        EXPECT_EQ(alg->ringsOrders_[i].size(), ringsOrder[i].size());
        for (u32 j = 0; j < ringsOrder[i].size(); ++j) {
            EXPECT_EQ(alg->ringsOrders_[i][j], ringsOrder[i][j]);
        }
    }

    EXPECT_EQ(alg->multiRingSlices_.size(), multiRingSliceZeroVector.size());
    for (u32 i = 0; i < multiRingSliceZeroVector.size(); ++i) {
        EXPECT_EQ(alg->multiRingSlices_[i].size(), multiRingSliceZeroVector[i].size());
        for (u32 j = 0; j < multiRingSliceZeroVector[i].size(); ++j) {
            EXPECT_EQ(alg->multiRingSlices_[i][j].offset, multiRingSliceZeroVector[i][j].offset);
        }
    }

    EXPECT_EQ(alg->userMemInputSlices_.size(), userMemInputSlicesVector.size());
    for (u32 i = 0; i < userMemInputSlicesVector.size(); ++i) {
        EXPECT_EQ(alg->userMemInputSlices_[i].size(), userMemInputSlicesVector[i].size());
        for (u32 j = 0; j < userMemInputSlicesVector[i].size(); ++j) {
            EXPECT_EQ(alg->userMemInputSlices_[i][j].offset, userMemInputSlicesVector[i][j].offset);
        }
    }
}