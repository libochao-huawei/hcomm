...
class ReduceScatterRingConcurrentDirectTest : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        ...
        std::cout << "\033[36m--ReduceScatterRingConcurrentDirectTest SetUP--\033[0m" << std::endl;
    }
    static void TearDownTestCase()
    {
        ...
        std::cout << "\033[36m--ReduceScatterRingConcurrentDirectTest TearDown--\033[0m" << std::endl;
    }
...
HcclDispatcher ReduceScatterRingConcurrentDirectTest::dispatcherPtr = nullptr;
DispatcherPub *ReduceScatterRingConcurrentDirectTest::dispatcher = nullptr;

TEST_F(ReduceScatterRingConcurrentDirectTest, destructor_D0)
{
    ...
    std::unique_ptr<AlgTemplateBase> tempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(
        TemplateType::TEMPLATE_REDUCESCATTER_RING_DIRECT, dispatcher);
    ret = tempAlg->Prepare(reduceAttrBitMap, &opInfo, userRank, subStreams,
        mainSignals, subSignals, ringsOrder, userMemInputSlices);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(ReduceScatterRingConcurrentDirectTest, run_async_00)
{
    s32 ret = HCCL_SUCCESS;
    ...
    std::unique_ptr<AlgTemplateBase> tempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(
        TemplateType::TEMPLATE_REDUCESCATTER_RING_DIRECT, dispatcher);
    EXPECT_EQ(HcclResult::HCCL_SUCCESS, tempAlg->Prepare(reduceAttrBitMap, &opInfo, userRank, subStreams,
        mainSignals, subSignals, tmpRingsOrder, tmpUserMemInputSlices));
...
TEST_F(ReduceScatterRingConcurrentDirectTest, run_async_01)
{
    s32 ret = HCCL_SUCCESS;
    ...
    ret = tempAlg->Prepare(input, output, output, 3, HCCL_DATA_TYPE_INT8, stream, HCCL_REDUCE_SUM, 0, cCLslices);
    EXPECT_EQ(ret, HCCL_SUCCESS);
