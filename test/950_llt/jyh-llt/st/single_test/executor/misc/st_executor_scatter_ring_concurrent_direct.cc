...
class ScatterRingConcurrentDirectTest : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        ...
        std::cout << "\033[36m--ScatterRingConcurrentDirectTest SetUP--\033[0m" << std::endl;
    }
    static void TearDownTestCase()
    {
        ...
        std::cout << "\033[36m--ScatterRingConcurrentDirectTest TearDown--\033[0m" << std::endl;
    }
...
HcclDispatcher ScatterRingConcurrentDirectTest::dispatcherPtr = nullptr;
DispatcherPub *ScatterRingConcurrentDirectTest::dispatcher = nullptr;

TEST_F(ScatterRingConcurrentDirectTest, destructor_D0)
{
    ...
    AlgTemplateBase *tempAlg = new ScatterRingConcurrentDirect(dispatcher);
...
TEST_F(ScatterRingConcurrentDirectTest, run_async_00)
{
    s32 ret = HCCL_SUCCESS;
    ...
    for (int i = 0; i < 1; i++) {
        link[i].reset(new (std::nothrow)
                          Transport(new (std::nothrow) TransportBase(reinterpret_cast<DispatcherPub*>(dispatcher), nullptr, machinePara, timeout)));
        link[i]->Init();
    }
    ...
    AlgTemplateBase *tempAlg = new ScatterRingConcurrentDirect(dispatcher);
    ret = tempAlg->Prepare(&opInfo, userRank, subStreams, mainSignals, subSignals, tmpRingsOrder, tmpUserMemInputSlices);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(ScatterRingConcurrentDirectTest, run_async_01)
{
    s32 ret = HCCL_SUCCESS;
...
TEST_F(ScatterRingConcurrentDirectTest, run_async_root)
{
    s32                              ret        = HCCL_SUCCESS;
...
TEST_F(ScatterRingConcurrentDirectTest, create_prepare_destructor_001)
{
    // 获取模板实例
    std::unique_ptr<AlgTemplateBase> tempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(
        TemplateType::TEMPLATE_SCATTER_RING_CONCURRENT_DIRECT, dispatcher);
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    // 转换子类指针
    ScatterRingConcurrentDirect *alg = dynamic_cast<ScatterRingConcurrentDirect *>(tempAlg.get());
    // 校验成员变量
    ...
    EXPECT_EQ(alg->userMemInputSlices_.size(), userMemInputSlices.size());
    for (u32 i = 0; i < userMemInputSlices.size(); ++i) {
        EXPECT_EQ(alg->userMemInputSlices_[i].offset, userMemInputSlices[i].offset);
    }
}