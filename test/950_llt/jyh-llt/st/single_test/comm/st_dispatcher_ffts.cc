...
TEST_F(DispatcherFFTSTest, st_launch_task_before_reset_context) {
...
TEST_F(DispatcherFFTSTest, st_signalRecord_should_related_with_a_signal_wait_given_in_chip) {
...
TEST_F(DispatcherFFTSTest, st_signalRecord_should_create_an_ctx_given_not_in_chip) {
    MOCKER(GetWorkflowMode)
    .stubs()
    .will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
...
TEST_F(DispatcherFFTSTest, st_signalWait_should_add_into_unassigned_successor_liut_given_in_chip) {
    MOCKER(GetWorkflowMode)
    .stubs()
    .will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
...
TEST_F(DispatcherFFTSTest, st_signalWait_should_create_an_ctx_given_not_in_chip) {
    MOCKER(GetWorkflowMode)
    .stubs()
    .will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
...
TEST_F(DispatcherFFTSTest, st_memcpyAsync_should_create_an_ctx_given_init_case) {
    MOCKER(GetWorkflowMode)
    .stubs()   
    .will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
...
TEST_F(DispatcherFFTSTest, should_return_success_given_init_aiv_reduce_context)
{
    MOCKER(GetWorkflowMode)
    .stubs()   
    .will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    ...
    MOCKER_CPP(&gert::TilingContext::GetRawTilingData)
    .stubs()
    .with(any())
    .will(invoke(stub_GetRawTilingData));

    auto meta = HcclOpMetaInfo::GetOneForAllGather();
    HcclResult retCode = fftsDispatcher->ResetGraphCtx(meta.isEnableCache, "init_aiv_reduce_context");
    EXPECT_EQ(HCCL_SUCCESS, retCode);
    DeviceMem dst = DeviceMem::alloc(80);
    DeviceMem src = DeviceMem::alloc(80);
    ...
    ret = fftsDispatcher->ReduceAsync(src.ptr(), dst.ptr(), dataCount, HcclDataType::HCCL_DATA_TYPE_INT64,
        HcclReduceOp::HCCL_REDUCE_SUM, stream, HcclReduceType::HCCL_TBE_REDUCE);

    EXPECT_EQ(HCCL_SUCCESS, ret);
}

TEST_F(DispatcherFFTSTest, ffts_tbe_profiling)
{
    MOCKER(GetWorkflowMode)
    .stubs()
    .will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    ...
    auto meta = HcclOpMetaInfo::GetOneForAllGather();
    HcclResult retCode = fftsDispatcher->ResetGraphCtx(meta.isEnableCache, "ffts_tbe_profiling");
    EXPECT_EQ(HCCL_SUCCESS, retCode);
    DeviceMem dst = DeviceMem::alloc(80);
    DeviceMem src = DeviceMem::alloc(80);
    ...
    HcclRtNotify signal;
    EXPECT_EQ(hrtNotifyCreateWithFlag(0, &signal), HCCL_SUCCESS);
    ret = fftsDispatcher->SignalRecord(signal, stream, (u32)0, (u64)0x0, (s32)0, false, (u64)0x0);
    EXPECT_EQ(HCCL_SUCCESS, ret);
    ret = fftsDispatcher->MemcpyAsync(src, dst, stream, (u32)1, hccl::LinkType::LINK_HCCS);
        EXPECT_EQ(HCCL_SUCCESS, ret);
    ret = fftsDispatcher->ReduceAsync(src.ptr(), dst.ptr(), dataCount, HcclDataType::HCCL_DATA_TYPE_INT64,
        HcclReduceOp::HCCL_REDUCE_SUM, stream, HcclReduceType::HCCL_TBE_REDUCE);
    EXPECT_EQ(HCCL_SUCCESS, ret);
    ret = fftsDispatcher->InlineReduceAsync(src.ptr(),dataCount, HcclDataType::HCCL_DATA_TYPE_FP32,
        HcclReduceOp::HCCL_REDUCE_SUM, stream,  dst.ptr(), (u32)1, hccl::LinkType::LINK_HCCS);
    EXPECT_EQ(HCCL_SUCCESS, ret);
    ret = fftsDispatcher->SetGraphTailVectorReduceDescSdma(src.ptr(), dst.ptr(), dataCount, HcclDataType::HCCL_DATA_TYPE_FP32,
...
TEST_F(DispatcherFFTSTest, st_init_dispatcher_failed) {
...
TEST_F(DispatcherFFTSTest, st_set_normalmode) {
    fftsDispatcher->SetNormalMode();
}

TEST_F(DispatcherFFTSTest, st_profiler_task_handler_when_param_null) {
...
TEST_F(DispatcherFFTSTest, st_ffts_reduce_datatype_not_support)
{
    Stream stream(StreamType::STREAM_TYPE_OFFLINE);
    GraphMgr::GraphCtxMgr graphCtxMgr;
    // 无效reduce类型
