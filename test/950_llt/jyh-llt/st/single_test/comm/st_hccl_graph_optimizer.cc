...
TEST_F(HcomGraphOptimizerTest, st_hcom_plugin_congif_deterministic_0)
{
    ge ::Status ret;
...
TEST_F(HcomGraphOptimizerTest, st_hcom_plugin_congif_deterministic_1)
{
    ge ::Status ret;
    std::map<std::string, std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_Initialize_to_Finalize)
{
    ...
    HCCL_INFO("start hccl OptimizeGraphPrepare");
    ge_ret = graphOptimizers.at(HCCL_GRAPH_OPTIMIZER_NAME)->OptimizeGraphPrepare(*graph);
...
TEST_F(HcomGraphOptimizerTest, st_FuseOps)
{
    ...
    bRet = ge::AttrUtils::SetInt(nodeGroup, "comm", hcomComm);
    EXPECT_EQ(bRet, true);
    ...
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
    ret = fusionHcomAllReduceOp.FuseOps(graph, nodeVec_0);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();

    ge::AttrUtils::HasAttr(nodeGroup, "DUMMY_SET_FALSE_COMM");
}
...
HcclResult GetOffDeviceTypeWithoutDevMock1(DevType &devType)
{
...
HcclResult GetOffDeviceTypeWithoutDevMock2(DevType &devType)
{
...
TEST_F(HcomGraphOptimizerTest, st_hcom_plugin_congif_deterministic_2)
{
    ge ::Status ret;
    std::map<std::string,std::string> options;
...
TEST_F(HcomGraphOptimizerTest, st_OptimizeFusedGraph_allreduce)
{
    ...
    .stubs()
    .will(invoke(GetOffDeviceTypeWithoutDevMock1));
    graphOptimizer.OptimizeFusedGraph(*graph);
    GlobalMockObject::verify();
}

TEST_F(HcomGraphOptimizerTest, st_OptimizeFusedGraph_GetDeterministic)
{
    MOCKER(GetOffDeviceTypeWithoutDev).stubs().will(invoke(GetOffDeviceTypeWithoutDevMock2));
...
TEST_F(HcomGraphOptimizerTest, st_GetFusionOpInfo_Bcast_by_comm_pytorch)
{
    HcclResult ret;
    ge::ComputeGraph graph("test_graph");
    HcomBroadcastFusion fusionHcomBroadcastOp;
...
TEST_F(HcomGraphOptimizerTest, st_GetCommFromOpDesc_by_comm_pytorch)
{
    HcclResult ret;
    ge::ComputeGraph graph("test_graph");
    HcomGraphOptimizer hcomGraphOptimizer;
...
TEST_F(HcomGraphOptimizerTest, st_HcomCalcOpRunningParam_by_comm_pytorch)
{
    ...
    MOCKER_CPP(&HcomGraphOptimizer::GetOpIntAttr)
    .stubs()
    .with(any())
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomGraphOptimizerTest, st_GetOpWorkspaceMemSize)
{
    ge::Node node;
    ...
    HcclResult ret = graphOptimizer.GetOpWorkspaceMemSize(node, HCCL_KERNEL_OP_TYPE_REDUCESCATTERV, opMemSize);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    GlobalMockObject::verify();
}

TEST_F(HcomGraphOptimizerTest, stCalculateSegmentIndex)
{
...
TEST_F(HcomGraphOptimizerTest, stCalculateSegmentIndexFromHomeExport)
{
    HcclResult ret;
    u64 tensorFusionLimit = std::numeric_limits<u64>::max();
...
TEST_F(HcomGraphOptimizerTest, st_getAlltoAllCountsDispl)
{
    ge::NodePtr nodeptr(new NodeTest);
...
TEST_F(HcomGraphOptimizerTest, st_CalAndSetOpWorkerSpaceForKnowShape)
{
 ...
 */
TEST_F(HcomGraphOptimizerTest, GetOpsKernelInfoPtr_ShouldReturnValidPtr_WhenPtrIsNotNull)
{
    HcomPlugin *plugin_;
 ...
 */
TEST_F(HcomGraphOptimizerTest, GetOpsKernelInfoPtr_ShouldReturnNull_WhenPtrIsNull)
{
    HcomPlugin *plugin_;
    plugin_ = &HcomPlugin::Instance();
    plugin_->hcomOpsKernelInfoStoreInfoPtr_ = std::make_shared<HcomOpsKernelInfoStore>();
...
TEST_F(HcomGraphOptimizerTest, st_GetStreamNumOfflineComp_ErrorTest)
{
    HcclResult ret;
...
TEST_F(HcomGraphOptimizerTest, st_CalTaskNum_When_Normal_Expect_ReturnlsHCCL_SUCCESS)
{
...
#undef OPEN_BUILD_PROJECT
    ...
    std::string sCollectiveType = ops[0].GetOpDesc()->GetType();
    u32 taskNum = 0;
    auto ret = hcomGraphOptimizer.CalTaskNum(ops[0], sCollectiveType,taskNum);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();
