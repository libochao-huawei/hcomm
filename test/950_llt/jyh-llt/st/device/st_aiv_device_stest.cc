...
class Aiv_Device_ST : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "Aiv_Device_ST SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "Aiv_Device_ST TearDown" << std::endl;
    }
...
TEST_F(Aiv_Device_ST, AivDeviceTest) {
...
TEST_F(Aiv_Device_ST, AlltoallExecutorDeviceTest) {
    MOCKER_CPP(&CollAlltoAllExecutor::SetParallelTaskLoader)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
}

TEST_F(Aiv_Device_ST, CommDeviceTest) {
    hcclComm comm;
...
TEST_F(Aiv_Device_ST, PluginRunnerDeviceTest) {
    u32 deviceLogicId = 0;
...
TEST_F(Aiv_Device_ST, AlgDeviceTest) {
