...
class Communicator_Device_ST : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "Communicator_Device_ST SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "Communicator_Device_ST TearDown" << std::endl;
    }
...
TEST_F(Communicator_Device_ST, CommunicatorTest) {
    ...
    hcclCommunicator.HcclSelectAlg(HcclCMDType::HCCL_CMD_ALL, 0, dataType, op, 48, findTag, tag);
    hcclCommunicator.HcclCalcBlockDim(HcclCMDType::HCCL_CMD_ALL, 0, dataType, 48, tag, buffer);
    ...
    hcclCommunicator.SetQosCfg(0);
    hcclCommunicator.ResetQosCfg();
    hcclCommunicator.GetQosCfg(buffer);
    ...
    hcclCommunicator.AicpuUnfoldKernelLaunchV2(nullptr, nullptr, stream_t, 0, nullptr, 0, tag, mode, tag, false);
    hcclCommunicator.AicpuUnfoldKernelLaunchV2(nullptr, nullptr, stream_t, 0, nullptr, 0, tag, mode, tag, true);
    hcclCommunicator.AicpuResourceInit(tag, algResource, tag, stream_t, HcclCMDType::HCCL_CMD_ALL);
    hcclCommunicator.LoadAICPUKernel();
    hcclCommunicator.UnloadAICPUKernel();
    ...
    hcclCommunicator.SetExecTimeOutConfig(1836);
...
TEST_F(Communicator_Device_ST, CommunicatorAttrsTest) {
    RankTable_t rankTable;
    HcclCommParams params;
    std::vector<RankInfo> rankList;
    WorldGroupInfo groupCommonData;
