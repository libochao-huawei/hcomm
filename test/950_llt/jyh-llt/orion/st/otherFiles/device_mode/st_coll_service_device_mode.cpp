...
protected:
    ...
    virtual void SetUp()
    {
        ...
        fakeComm.aivTagBuffer = DevBuffer::Create(0x100, 0x100);
...
TEST_F(CollServiceDeviceModeTest, test_init_LoadWithOpBasedMode_AIV_Cache_A2A)
{
    void *ptr1 = (void*)1;
    MOCKER(HrtStreamCreateWithFlags).stubs().with(any(), any()).will(returnValue(ptr1));
    MOCKER(HrtGetStreamId).stubs().will(returnValue(0));
    fakeComm.opExecuteConfig.accState = AcceleratorState::AIV;
    fakeComm.currentCollOperator->opType = OpType::ALLTOALL;
    fakeComm.currentCollOperator->all2AllDataDes = {DataType::INT32, DataType::INT32, 64, 64};
    auto service = dynamic_cast<CollServiceDeviceMode *>(this->fakeComm.collService);
    auto stream = std::make_unique<Stream>();
    EXPECT_NO_THROW(service->LoadWithOpBasedMode(*fakeComm.currentCollOperator, std::move(stream)));
    EXPECT_NO_THROW(fakeComm.GetCollAlgComponent());
...
TEST_F(CollServiceDeviceModeTest, test_init_LoadWithOpBasedMode_AIV_Cache_A2AV)
{
    void *ptr1 = (void*)1;
    MOCKER(HrtStreamCreateWithFlags).stubs().with(any(), any()).will(returnValue(ptr1));
    MOCKER(HrtGetStreamId).stubs().will(returnValue(0));
    fakeComm.opExecuteConfig.accState = AcceleratorState::AIV;
    fakeComm.currentCollOperator->opType = OpType::ALLTOALLV;
    int64_t sendCounts[2] = {3, 5};
    int64_t recvCounts[2] = {3, 5};
    int64_t sdispls[2] = {0, 3};
    int64_t rdispls[2] = {0, 5};
    fakeComm.currentCollOperator->all2AllVDataDes = {DataType::INT32, DataType::INT32, sendCounts, recvCounts, sdispls, rdispls};
    auto service = dynamic_cast<CollServiceDeviceMode *>(this->fakeComm.collService);
    auto stream = std::make_unique<Stream>();
    EXPECT_NO_THROW(service->LoadWithOpBasedMode(*fakeComm.currentCollOperator, std::move(stream)));
    EXPECT_NO_THROW(fakeComm.GetCollAlgComponent());
