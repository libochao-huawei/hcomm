...
{
    EnvHostNicConfig hostNicCfg{};
    MOCKER(SalGetEnv).stubs().will(returnValue(string("llt/ace/comop/hccl/orion/ut/framework/env_config/not_exist_file")));
    EXPECT_THROW(hostNicCfg.hcclWhiteListFile.Parse(), InvalidParamsException);
}
...
{
    EnvHostNicConfig hostNicCfg{};
    string filePath{"llt/ace/comop/hccl/orion/ut/framework/env_config/test_file.txt"};
