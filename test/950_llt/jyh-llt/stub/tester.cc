...
Description   : 集合通信测试类，提供集合通信进程内核进程间测试接口
...
HcclResult tester::init_comm_all()
{
    ...
    for (rank_it = rank_list.begin(); rank_it != rank_list.end(); rank_it++)
    {
        ...
        rt_ret = rtStreamCreate(&(rank_it->stream), 5);
        EXPECT_EQ(rt_ret, RT_ERROR_NONE);

        if (rt_ret != RT_ERROR_NONE)
        {
            STUB_ERROR("device[%d] rtStreamCreate fail return value[%d]", rank_it->devid, hccl_ret);
            return HCCL_E_INTERNAL;
        }
    }
...
HcclResult tester::excute_malloc_task_mem(test_task_t& task)
{
    ...
    for (s32 i = 0; i < device_count; ++i)
    {
        ...
        rt_ret = rtMalloc(&new_buf, task.sendbuf_num * length, RT_MEMORY_HBM, HCCL);
        EXPECT_EQ(rt_ret, RT_ERROR_NONE);

        if (rt_ret != RT_ERROR_NONE)
        {
            STUB_ERROR("device[%d] rtMalloc sendbuf size[%d] fail return value[%d]",
        ...
        rt_ret = rtMalloc(&new_buf, task.recvbuf_num * length, RT_MEMORY_HBM, HCCL);
        EXPECT_EQ(rt_ret, RT_ERROR_NONE);

        if (rt_ret != RT_ERROR_NONE)
        {
            STUB_ERROR("device[%d] rtMalloc recvbuf size[%d] fail return value[%d]",
                       i, task.sendbuf_num * length, rt_ret);
            return HCCL_E_MEMORY;
        }
...
HcclResult tester::init_sendbuf_value(test_task_t* task, T cpu_type)
{
    ...
    for (s32 rank = 0; rank < device_count; rank++)
    {
        ...
        rt_ret = rtMemcpyAsync(task->dev_sendbuf[rank], (u64)temp, task->host_sendbuf[rank], (u64)temp,
                               RT_MEMCPY_HOST_TO_DEVICE, rank_list[rank].stream);
...
HcclResult tester::dev_to_host_mem_synchronize()
{
    ...
    for (task_it = task_vec.begin(); task_it != task_vec.end(); task_it++)
    {
        ...
        if (task_it->excute_type == EXCUTE_TYPE_BROADCAST)
        {
            ...
            for (int rank = 0; rank < device_count; rank++)
            {
                ...
                rt_ret = rtMemcpyAsync(task_it->host_sendbuf[rank], (u64)sendbuf_len, task_it->dev_sendbuf[rank], (u64)sendbuf_len,
                                       RT_MEMCPY_DEVICE_TO_HOST, rank_list[rank].stream);
        ...
        for (int rank = 0; rank < device_count; rank++)
        {
            ...
            rt_ret = rtMemcpyAsync(task_it->host_recvbuf[rank], (u64)recvbuf_len, task_it->dev_recvbuf[rank], (u64)recvbuf_len,
                                   RT_MEMCPY_DEVICE_TO_HOST, rank_list[rank].stream);
    ...
    for (s32 rank = 0; rank < device_count; rank++)
    {
        ...
        rt_ret = rtStreamSynchronize(rank_list[rank].stream);
        EXPECT_EQ(RT_ERROR_NONE, rt_ret);

        if (RT_ERROR_NONE != rt_ret)
        {
            STUB_ERROR("rtStreamSynchronize failed [%d], rank[%d], device id[%d]", hccl_ret, rank, rank_list[rank].devid);
            return hccl_ret;
        }
    }
...
HcclResult tester::wait_all_task_finish()
{
    ...
    for (s32 j = 0; j < device_count; j++)
    {
        ...
        rt_ret = rtStreamSynchronize(rank_list[j].stream);
        EXPECT_EQ( RT_ERROR_NONE, rt_ret);

        if (RT_ERROR_NONE != rt_ret)
        {
            STUB_ERROR("wait_all_task_finish rtStreamSynchronize ,rank[%d], dev_id[%d], stream[%p], hccl_ret[%d]"
...
HcclResult tester::destroy_comms()
{
    ...
    for (s32 rank = 0; rank < device_count; ++rank)
    {
        ...
        rt_ret = rtStreamDestroy(rank_list[rank].stream);
        EXPECT_EQ(rt_ret, RT_ERROR_NONE);
