...
int RaSocketSend(const void* handle, const void* data, u64 size, u64 *sentSize)/*同步发送*/
{
    ...
    if (handle != NULL) {
        ...
        if (ret  == 0) {
            HCCL_DEBUG("RaSocketSend size is %d handle=%p, socketfd=%d \n",sendbuf, handle, socketfd);
        }
    ...
    p_cn->qp.local_qp_msg_ptr->msg.MrInfoT.len = size;
    HcclResult ret = sal_memcpy(&p_cn->qp.local_qp_msg_ptr->msg.MrInfoT.data[0], QP_MSG_MAX_SIZE, data, size);
    if (ret != HCCL_SUCCESS) {
...
{
    HCCL_INFO("RaSendWrlist send_num[%u]", send_num);
    ...
    for (u32 j = 0; j < send_num; j++) {
        ...
        HCCL_INFO("[cc] RaSendWrlist dst[%0x] local[%0x] sendNum[%u] len[%u]", wr_tmp.dstAddr, wr[j].memList.addr, send_num, wr[j].memList.len);
...
{
    HCCL_INFO("RaSendWrlistExt fake");
    return 0;
}
...
int RaIsLastUsed(int ins_id)
{
    ...
    pthread_mutex_lock(&g_ref_instances[ins_id].mutex);
    if (g_ref_instances[ins_id].ref_count == 0) {
        HCCL_ERROR("[RaIsLastUsed] is called on ins_id %d which has not been used", ins_id);
        pthread_mutex_unlock(&g_ref_instances[ins_id].mutex);
        return -22;
    }
...
int RaSocketInit(int mode, struct rdev rdev_info, void **socketHandle)
{
    ...
    if (rdev_info.phyId > DEV_MAX) {
        HCCL_ERROR("in RaSocketInit, error dev_id[%d]", rdev_info.phyId);
        return -1;
    }

    HCCL_INFO("RaSocketInit, rdev_info.dev_id[%d], rdev_info.localIp.addr.s_addr[%u]",
        rdev_info.phyId, rdev_info.localIp.addr.s_addr);

    std::unique_lock<std::mutex> lock(g_raNicMutex);
    // 通过IP 判断当前是nic 还是vic, 小于DEV_MAX则是Vnic
    if (rdev_info.localIp.addr.s_addr < DEV_MAX) {
        u32 dev_id = rdev_info.phyId;
        gRdevVnicInfo[dev_id].dev_id = rdev_info.phyId;
        gRdevVnicInfo[dev_id].local_ip = rdev_info.localIp.addr.s_addr;
        gRdevVnicInfo[dev_id].idx = rdev_info.phyId;
        *socketHandle = (void*)&gRdevVnicInfo[rdev_info.phyId];
        return 0;
    } else {
        // 0， 1 存储device 0 的网卡0，1信息; 2， 3 存储device 1 的网卡0，1信息; 以此类推
        u32 idx = rdev_info.phyId * 2;
        if (gRdevNicInfo[idx].local_ip != 0) {
            idx = rdev_info.phyId * 2 + 1;
            if (gRdevNicInfo[idx].local_ip != 0) {
                HCCL_ERROR("RaSocketInit fail, local_ip0[%u], local_ip1[%u]", gRdevNicInfo[rdev_info.phyId * 2],
                gRdevNicInfo[rdev_info.phyId * 2 + 1]);
                return -1;
            }
        }

        gRdevNicInfo[idx].dev_id = rdev_info.phyId;
        gRdevNicInfo[idx].local_ip = rdev_info.localIp.addr.s_addr;
...
int RaSocketInitV1(int mode, struct SocketInitInfoT socket_init, void **socketHandle)
{
    ...
    if (socket_init.rdevInfo.phyId > DEV_MAX) {
        HCCL_ERROR("in RaSocketInit, error dev_id[%d]", socket_init.rdevInfo.phyId);
        return -1;
    }
    ...
    if (socket_init.rdevInfo.localIp.addr.s_addr < DEV_MAX) {
        ...
        *socketHandle = (void*)&gRdevVnicInfo[socket_init.rdevInfo.phyId];
        ...
        if (gRdevNicInfo[idx].local_ip != 0) {
            ...
            if (gRdevNicInfo[idx].local_ip != 0) {
                HCCL_ERROR("RaSocketInit fail, local_ip0[%u], local_ip1[%u]", gRdevNicInfo[socket_init.rdevInfo.phyId * 2],
...
int RaSocketDeinit(void *socket_handle)
{
    // CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
    if (!socket_handle) {
        HCCL_WARNING("in RaSocketDeinit, rdma_handle is null");
        return -1;
    }
    ...
    // 虚拟网卡场景
    HCCL_INFO("RaSocketDeinit: %d", socketHandle->local_ip);
...
int RaRdevInit(int mode, unsigned int NotifyTypeT, struct rdev rdev_info, void **rdma_handle)
{
    ...
    if (rdev_info.phyId > DEV_MAX) {
        HCCL_ERROR("in ra_dev_init, error dev_id[%d]", rdev_info.phyId);
        return -1;
    }

    // 通过IP 判断当前是nic 还是vic, 小于DEV_MAX则是Vnic
    if (rdev_info.localIp.addr.s_addr < DEV_MAX) {
        HCCL_ERROR("unsupport rdev init by Vic, local_ip[%u]", rdev_info.localIp.addr.s_addr);
        return -1;
    } else {
        int idx = rdev_info.phyId * 2;
        if (gRdevNicInfo[idx].local_ip != rdev_info.localIp.addr.s_addr) {
            idx = rdev_info.phyId * 2 + 1;
            if (gRdevNicInfo[idx].local_ip != rdev_info.localIp.addr.s_addr) {
                HCCL_ERROR("RaRdevInit, should init ra rdev after ra socket");
                return -1;
            }

        }
        if ((void*)&gRdevNicInfo[idx] == nullptr) {
            HCCL_ERROR("RaRdevInit, rdma handle is null");
        }
...
int RaRdevInitWithBackup(struct RdevInitInfo *init_info, struct rdev *rdev_info,
    struct rdev *backup_rdev_info, void **rdma_handle)
{
    int mode = init_info->mode;
    unsigned int NotifyTypeT = init_info->notifyType;
    CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
    // rdev_init 只有rdma使用，因此采用pcie时，不初始化
    // 采用rdma时，初始化内容与socket相同，直接return socket的内容；
    // dev_id 最大为8，DEV_MAX 为 16
...
int RaRdevInitV2(struct RdevInitInfo init_info, struct rdev rdev_info, void **rdma_handle)
{
    int mode = init_info.mode;
    unsigned int NotifyTypeT = init_info.notifyType;
    CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
...
int RaRdevDeinit(void *rdma_handle, unsigned int NotifyTypeT)
{
    CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
    if (!rdma_handle) {
        HCCL_WARNING("in RaRdevDeinit, rdma_handle is null");
        return -1;
    }
    // RaRdevDeinit 只做参数较验，不做销毁,由socket 销毁
    ...
    if (rdmaHandle->dev_id > DEV_MAX) {
        HCCL_WARNING("in RaRdevDeinit, device id[%d] ERROR", rdmaHandle->dev_id);
    }
    ...
    if (gRdevNicInfo[idx].local_ip != rdmaHandle->local_ip) {
        HCCL_WARNING("in RaRdevDeinit, rdma_handle error, idx[%u], rdmaHandle_ip[%u], gRdevNicInfo[%u]",
...
{
    if (handle == nullptr) {
        HCCL_ERROR("in RaRdevDeinit, handle is null");
        return -1;
    }
...
int RaGetSockets(u32 role, struct SocketInfoT conn[], u32 num, u32 *connectedNum)
{
    ...
    HCCL_DEBUG("RaGetSockets, localIp[0x%08x], role[%d], num", localIp, role, num);
            ...
            if (qp->remote_qp_msg_ptr->cmd == QP_CMD_WRITE_MR)/*mr相关信息的交换*/
            {
                ...
                HCCL_INFO("start pos %d,size %u", cn->rev_buff.start_pos, cn->rev_buff.size);
                 ...
                 HcclResult ret = sal_memcpy(&cn->rev_buff.buff[cn->rev_buff.start_pos + cn->rev_buff.size],
                           cn->qp.remote_qp_msg_ptr->msg.MrInfoT.len,
                           &cn->qp.remote_qp_msg_ptr->msg.MrInfoT.data[0],
                           cn->qp.remote_qp_msg_ptr->msg.MrInfoT.len);
                if (ret != HCCL_SUCCESS) {
                    HCCL_ERROR("rdma send: sal memcpy error");
                    return NULL;
                ...
                cn->rev_buff.size += cn->qp.remote_qp_msg_ptr->msg.MrInfoT.len;
...
int RaCreateSrq(const void*, struct SrqAttr *)
{
    return 0;
}
...
int RaCtlEventHandle(int event_handle, const void *fd_handle, int opcode, enum RaEpollEvent event)
{
    ...
    if (event_handle < 0) {
        HCCL_ERROR("[RaCtlEventHandle]event_handle[%d] is invalid", event_handle);
        return -EINVAL;
    }
    RA_CHECK_POINTER_NULL_WITH_RET(fd_handle);
    if (opcode != EPOLL_CTL_ADD && opcode != EPOLL_CTL_DEL && opcode != EPOLL_CTL_MOD) {
        HCCL_ERROR("[RaCtlEventHandle]opcode[%d] invalid, valid opcode includes {%d, %d, %d}",
    ...
    if (event == RA_EPOLLONESHOT) {
        ...
        HCCL_ERROR("[RaCtlEventHandle]unkown event[%d]", event);
        return -EINVAL;
    }
    ...
    unsigned int *events_num)
{
    ...
    if (event_count < 0) {
        HCCL_ERROR("[RaWaitEventHandle]epoll_wait failed, strerror[%s]", strerror(errno));
        return -EIO;
    }
...
int RaRdevGetPortStatus(RdmaHandle rdmaHandle, enum PortStatus *status)
{
    return 0;
}
...
int RaGetHccnCfg(struct RaInfo *info, enum HccnCfgKey key, char *value, unsigned int *value_len)
{
    HCCL_INFO("call %s", __func__);
    return 0;
}