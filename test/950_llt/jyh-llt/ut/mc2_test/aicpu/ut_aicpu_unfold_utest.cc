...
class ExecutorTracer_UT : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "ExecutorTracer_UT SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "ExecutorTracer_UT TearDown" << std::endl;
    }
    // Some expensive resource shared by all tests.
    virtual void SetUp()
    {
        ...
        MOCKER(hrtGetHccsPortNum)
            ...
            .with(any(), outBound(portNum))
            .will(returnValue(HCCL_SUCCESS));
        std::cout << "ExecutorTracer_UT Test SetUP" << std::endl;
    }
    virtual void TearDown()
    {
        GlobalMockObject::verify();
        std::cout << "ExecutorTracer_UT Test TearDown" << std::endl;
    }
};
...
template <typename T>
HcclResult InitAndVerifySingleSignal(HcclCommAicpu *This, const HcclSignalInfo &signalInfo, std::shared_ptr<T> &notify)
{
    return HCCL_SUCCESS;
}

HcclResult GetOpExecCtrlTargetOpStub(AicpuHdc* aicpuHdc, std::shared_ptr<HDCommunicate> h2dTransfer,HcclOpIdentifier &opId)
{
    (void)h2dTransfer;
    opId.tag[0] = 't';
    opId.tag[1] = 'e';
...
TEST_F(AicpuUnfold_UT, InitSlaveStreamObjs)
{
    ...
    std::string hcomId = "hcom_aicpu_unfold20";
    SqCqeContext sqeCqeContext;
    ...
    paramTask.localRes.signalNum = 32;
    EXPECT_EQ(hcclCommAicpu->InitLocalNotifyObj(&paramTask), HCCL_SUCCESS);
    ...
    paramTask.topoInfo.complanRank = reinterpret_cast<u64>(dstTlvDeviceMem.ptr());
    paramTask.topoInfo.complanRankLength = tlvLen;
    EXPECT_EQ(hcclCommAicpu->ParseTlvToVector(paramTask.topoInfo.complanRank, paramTask.topoInfo.complanRankLength,
                                              commPlaneRanks),
              HCCL_E_PARA);
...
TEST_F(AicpuUnfold_UT, InitTopoInfo)
{
    hccl::HcclCommAicpu *hcclCommAicpu = new hccl::HcclCommAicpu;
    hcclCommAicpu->dumpDebug_ = false;
    hcclCommAicpu->dispatcher_ = nullptr;
...
TEST_F(AicpuUnfold_UT, InitRemoteTagRes)
{
    ...
    while (loopNum > 0) {
        ...
        strcpy(tagBufferPtr->tag, newTag.c_str());
        ListCommonInit(&tagBufferPtr->nextTagRes, &tagBufferPtr->nextTagRes);
        ListCommonAddHead(nullptr, &tagBufferPtr->nextTagRes, &head, &head);
        loopNum--;
    }
...
TEST_F(AicpuUnfold_UT, GetRdmaLinksByRankAndTagErr)
{
    u32 extraNotifyNum = 2;
    hccl::HcclCommAicpu *hcclCommAicpu = new hccl::HcclCommAicpu;
    std::string tag = "hcom_aicpu_unfold2";
    std::shared_ptr<Transport> link = make_shared<Transport>(nullptr);
    std::shared_ptr<Transport> link1 = make_shared<Transport>(nullptr);
    HcclOpResParam paramTask;
    memset(&paramTask, 0, sizeof(HcclOpResParam));
    MOCKER_CPP(&HcclCommAicpu::RefreshTransportsResForRank).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    hcclCommAicpu->linkRdmaRes_[0][tag].push_back(link1);
    EXPECT_EQ(
        hcclCommAicpu->GetRdmaLinksByRankAndTag(
            &paramTask, CommTransportsType::SPECIAL, 0, tag, link, false, extraNotifyNum, true),
        HCCL_E_INTERNAL);
    delete hcclCommAicpu;
}
...
TEST_F(AicpuUnfold_UT, HcclOpNotEnoughWorkSpace)
{
    hccl::HcclCommAicpu *hcclCommAicpu = new hccl::HcclCommAicpu;
    AicpuKfcRpcServerV2 rpc;
    HcclMsgAreaForTest tmp;
    EXPECT_NE(rpc.Init({reinterpret_cast<u64>(&tmp), sizeof(tmp)}), HCCL_SUCCESS);
    GlobalMockObject::verify();
    delete hcclCommAicpu;
}
...
TEST_F(AicpuUnfold_UT, WaitFinishWhileLoop)
{
    ...
    EXPECT_EQ(hcclCommAicpu->WaitFinishWhileLoop(mainStream,subStreams,tag, 0, param),HCCL_E_TIMEOUT);
    GlobalMockObject::verify();
    delete hcclCommAicpu;
}
...
TEST_F(AicpuUnfold_UT, ResetSqBuff)
{
    ...
    hcclCommAicpu->orderStream_ = mainStream;
...
TEST_F(AicpuUnfold_UT, init_aicpucom)
{
    ...
    memset(&paramTask, 0, sizeof(HcclOpResParam));
    std::vector<void *> bufferVec;
    std::string hcomId = "hcom_aicpu_unfold24";
    SqCqeContext sqeCqeContext;
    TestConstructHcclOpResParam(paramTask, bufferVec, hcomId, sqeCqeContext);
    paramTask.winSize = 4096;
    DeviceMem inputMem = DeviceMem::alloc(4096);
    ...
    MOCKER_CPP(&HcclCommAicpu::GetRdmaLinksByRankAndTag).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&HcclCommAicpu::GetSdmaLinksByRankAndTag).stubs().with(any()).will(returnValue(0));
...
TEST_F(AicpuUnfold_UT, ut_GetAlgResponseRes)
{
    DlTraceFunction::GetInstance().DlTraceFunctionInit();
    dlog_setlevel(HCCL, DLOG_DEBUG, 1);
    HcclResult ret = HCCL_SUCCESS;
...
TEST_F(AicpuUnfold_UT, test_OrchestrateAicpu_agv)
{
...
TEST_F(AicpuUnfold_UT, test_RunAicpuRpcSrvLaunchV2_allreduce)
{
    HcclResult ret = HCCL_SUCCESS;
    hccl::HcclCommAicpu* hcclCommAicpu = new hccl::HcclCommAicpu();
    ...
    opTilingData->ahcConfInfo[0] = 3;
    u32 confData = (static_cast<u32>(0) << TOP_HIERARCHICAL_CONF_LEVEL_SHIFT) |
                (static_cast<u32>(0) << TOP_HIERARCHICAL_CONF_CONC_TYPE_SHIFT) |
                (static_cast<u32>(2) << TOP_HIERARCHICAL_CONF_OP_TYPE_SHIFT) |
                (static_cast<u32>(21) << TOP_HIERARCHICAL_CONF_TEMPLATE_TYPE_SHIFT);
    opTilingData->ahcConfInfo[1] = confData;
    confData = (static_cast<u32>(0) << TOP_HIERARCHICAL_CONF_LEVEL_SHIFT) |
                (static_cast<u32>(0) << TOP_HIERARCHICAL_CONF_CONC_TYPE_SHIFT) |
                (static_cast<u32>(0) << TOP_HIERARCHICAL_CONF_OP_TYPE_SHIFT) |
                (static_cast<u32>(65) << TOP_HIERARCHICAL_CONF_TEMPLATE_TYPE_SHIFT);
    opTilingData->ahcConfInfo[2] = confData;
    confData = (static_cast<u32>(0) << TOP_HIERARCHICAL_CONF_LEVEL_SHIFT) |
                (static_cast<u32>(1) << TOP_HIERARCHICAL_CONF_CONC_TYPE_SHIFT) |
                (static_cast<u32>(1) << TOP_HIERARCHICAL_CONF_OP_TYPE_SHIFT) |
                (static_cast<u32>(52) << TOP_HIERARCHICAL_CONF_TEMPLATE_TYPE_SHIFT);
    opTilingData->ahcConfInfo[3] = confData;

    std::string tag = "hcom_allreduce_aicpu_unfold";
    memcpy_s(opTilingData->tag, sizeof(opTilingData->tag), tag.c_str(), tag.size() + 1);
    std::string algName = "AllReduce";
    tag = reinterpret_cast<char *>(opTilingData->tag);
    std::string newTag = tag + "_" + algName;
    memcpy_s(opTilingData->newTag, sizeof(opTilingData->newTag), newTag.c_str(), newTag.length() + 1);
    memcpy_s(opTilingData->algName, sizeof(opTilingData->algName), algName.c_str(),
...
TEST_F(AicpuUnfold_UT, test_hcclCommAicpu_alltoall)
{
    ...
    std::vector<u64> sendCountMatrix(2 * 2, 16);
...
TEST_F(AicpuUnfold_UT, ut_SetDynamicTilingDataAlltoallvc)
{
    HcclCommunicator impl;
    ...
    u64 dynamicDataSize = sizeof(struct OpTilingAlltoallvcDataDes) + rankSize * rankSize * sizeof(u64);

    HostMem dynamicDataMem = HostMem::alloc(dynamicDataSize);
    HcclResult ret = impl.SetDynamicTilingDataAlltoallvc(opParam, dynamicDataMem);
    EXPECT_EQ(HCCL_SUCCESS, ret);
}

#define DEV_NUM_8 8
TEST_F(AicpuUnfold_UT, ut_SetDynamicTilingDataAlltoallv)
{
    HcclCommunicator impl;
    impl.userRankSize_ = 8;
    OpParam opParam;

    // 生成 alltoallv 入参
    u64 count = 16;
    std::vector<u64> sendCounts(DEV_NUM_8 * sizeof(u64), 0);
    std::vector<u64> sdispls(DEV_NUM_8 * sizeof(u64), 0);
    std::vector<u64> recvCounts(DEV_NUM_8 * sizeof(u64), 0);
    std::vector<u64> rdispls(DEV_NUM_8 * sizeof(u64), 0);
    ...
    opParam.All2AllDataDes.sendType = sendDataType;
    opParam.All2AllDataDes.recvType = recvDataType;
    opParam.All2AllDataDes.sendCounts = static_cast<void *>(sendCounts.data());
    opParam.All2AllDataDes.recvCounts = static_cast<void *>(recvCounts.data());
    opParam.All2AllDataDes.sdispls = static_cast<void *>(recvCounts.data());
    opParam.All2AllDataDes.rdispls = static_cast<void *>(recvCounts.data());

    u32 rankSize = DEV_NUM_8;
    u32 dynamicDataSize = sizeof(struct OpTilingAlltoallvDataDes) + rankSize * 4 * sizeof(u64);

    HostMem dynamicDataMem = HostMem::alloc(dynamicDataSize);
    HcclResult ret = impl.SetDynamicTilingDataAlltoallv(opParam, dynamicDataMem);
    EXPECT_EQ(HCCL_SUCCESS, ret);
}
...
TEST_F(ExecutorTracer_UT, HandleCqeException)
{
...
TEST_F(ExecutorTracer_UT, ReportCqeExceptionSdma)
{
    HcclCommAicpu * hcclCommAicpu = new HcclCommAicpu;
    hcclCommAicpu->opNotifies_.push_back(std::make_shared<LocalNotify>());
    hcclCommAicpu->opNotifies_.push_back(std::make_shared<LocalNotify>());
...
TEST_F(AicpuUnfold_UT, PrintTaskException_NoTask)
{
    hccl::HcclCommAicpu *hcclCommAicpu = new hccl::HcclCommAicpu;
    uint32_t sqHead = 1;
    uint32_t sqTail = 1;
...
TEST_F(AicpuUnfold_UT, PrintTaskException_noActive)
{
    hccl::HcclCommAicpu *hcclCommAicpu = new hccl::HcclCommAicpu;
    uint32_t sqHead = 0;
    uint32_t sqTail = 100;
...
TEST_F(AicpuUnfold_UT, aicpu_prof_test_init_false)
{
    ...
    memset(&paramTask, 0, sizeof(HcclOpResParam));
    std::vector<void *> bufferVec;
    std::string hcomId = "hcom_aicpu_unfold15";
    SqCqeContext sqeCqeContext;
    TestConstructHcclOpResParam(paramTask, bufferVec, hcomId, sqeCqeContext);
    paramTask.winSize = 4096;
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(AicpuUnfold_UT, ut_ReportFlagTask_test)
{
...
TEST_F(AicpuUnfold_UT, InitBatchSendRecvOpId)
{
    ...
    recv_item.sendRecvType = recv_item1.sendRecvType = HcclSendRecvType::HCCL_SEND;
...
TEST_F(AicpuUnfold_UT, ut_GetBsrTransportQpn)
{
...
TEST_F(AicpuUnfold_UT, ut_GenTaskExceptionInfo)
{
    HcclResult ret = HCCL_SUCCESS;
    hccl::HcclCommAicpu *hcclCommAicpu = new hccl::HcclCommAicpu;
    ...
    sqeType = RT_STARS_SQE_TYPE_SDMA;
    ret = hcclCommAicpu->GenTaskExceptionInfo(sqeType, stream, 1);
    EXPECT_EQ(HCCL_SUCCESS, ret);

    sqeType = RT_STARS_SQE_TYPE_NOTIFY_RECORD;
    ret = hcclCommAicpu->GenTaskExceptionInfo(sqeType, stream, 1);
    EXPECT_EQ(HCCL_SUCCESS, ret);

    sqeType = RT_STARS_SQE_TYPE_WRITE_VALUE;
    ret = hcclCommAicpu->GenTaskExceptionInfo(sqeType, stream, 1);
    EXPECT_EQ(HCCL_SUCCESS, ret);

    sqeType = RT_STARS_SQE_TYPE_VPC;
    ret = hcclCommAicpu->GenTaskExceptionInfo(sqeType, stream, 1);
...
HcclResult GetNotifyMrInfo(u32 phy_id, RdmaHandle handle, struct MrInfoT *mrInfo)
{
    static int i = 0;
...
HcclResult TxSendWrlistExtStub(struct WrInfo wrList[], u32 sendNum, struct SendWrRsp opRsp[],
    ...
    u32 phy_id, RdmaHandle rdmaHandle, struct QpExtAttrs *attrs, struct AiQpInfo *info1, QpHandle &qpHandle)
{
    ...
    info->qpn = qpn++;
    qpHandle = (void *)info;
    info1->aiQpAddr = reinterpret_cast<u64>(info);
    info1->dbIndex = info1->aiQpAddr;
    info1->sqIndex = info1->aiQpAddr;
    return HCCL_SUCCESS;
}
...
TEST_F(AicpuUnfold_UT, ut_MultiQp_32Qp_3Notify)
{
    ...
    MOCKER(hrtMemAsyncCopyWithCfg).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    ...
    AiQpInfo aiQpInfo;
    char aiapaddr[4];
    aiQpInfo.aiQpAddr = reinterpret_cast<std::uintptr_t>(&aiapaddr);
    aiQpInfo.dbIndex = 1;
    aiQpInfo.sqIndex = 1;
    char aphandle[4];
    ...
    MOCKER(hcclStreamSynchronize).stubs().with(any(), any()).will(returnValue(HCCL_SUCCESS));
    ...
    char machineParaMem[4 * 1024 * 1024];
    ...
    auto makeAlgResourceResponse = [&](OpCommTransport &algResourceResponse) {
        ...
        machinePara_.inputMem.size_ = 4 * 1024 * 1024;
        machinePara_.inputMem.owner_ = false;
        machinePara_.outputMem.ptr_ = &machineParaMem;
        machinePara_.outputMem.size_ = 4 * 1024 * 1024;
    ...
    };
    {
       ...
       impl->hostDeviceLock_->devMem_.size_ = 4 * 1024 * 1024;
       ...
       std::string algName = "Dev_Aicpu_multiQp";
       {
            ...
            txMems.emplace_back(TxMemoryInfo{UserMemType::INPUT_MEM, 0, &machineParaMem, 4 * 1024 * 1024});
...
// 临时补充覆盖率
TEST_F(AicpuUnfold_UT, ut_OrchestrateAicpu_tmp)
{
    HcclCommunicator impl;
...
TEST_F(AicpuUnfold_UT, AclgraphLaunchInOrder_Success) {
    OrderLaunch& orderLaunch = OrderLaunch::GetInstance(0);
    std::string group = "test_group";
    hccl::Stream kernelStream(StreamType::STREAM_TYPE_OFFLINE);
    u32 modelId = 1;
    rtModel_t rtModel = nullptr;
    std::shared_ptr<LocalNotify> notify0 = nullptr;
    std::shared_ptr<LocalNotify> notify1= nullptr;
    u32 timeOut = 2;

    // 注册通信域
    EXPECT_EQ(orderLaunch.RegisterOrderLaunch(group), HCCL_SUCCESS);
    MOCKER_CPP(&OrderLaunch::LaunchInOrder).stubs().will(returnValue(HCCL_SUCCESS));

    // 调用AclgraphLaunchInOrder函数
    EXPECT_EQ(orderLaunch.AclgraphLaunchInOrder(group, kernelStream, modelId, rtModel, notify0, notify1, timeOut), HCCL_SUCCESS);
    GlobalMockObject::verify();
}

TEST_F(AicpuUnfold_UT, OpbaseLaunchInOrder_Success) {
    OrderLaunch& orderLaunch = OrderLaunch::GetInstance(0);
    std::string group = "test_group";
    hccl::Stream kernelStream(StreamType::STREAM_TYPE_OFFLINE);
    std::shared_ptr<LocalNotify> notify0 = nullptr;
    ...
    // 调用OpbaseLaunchInOrder函数
    EXPECT_EQ(orderLaunch.OpbaseLaunchInOrder(group, kernelStream, notify0, notify1, timeOut), HCCL_SUCCESS);
    GlobalMockObject::verify();
}

TEST_F(AicpuUnfold_UT, HcomLaunchInOrder_Success) {
    OrderLaunch& orderLaunch = OrderLaunch::GetInstance(0);
    hccl::Stream hcomStream(StreamType::STREAM_TYPE_ONLINE);
    orderLaunch.hcomStreamMap_.emplace(std::make_pair(1, hcomStream));
    std::string group = "test_group";
    hccl::Stream kernelStream(StreamType::STREAM_TYPE_OFFLINE);
    u32 graphId = 1;
    std::shared_ptr<LocalNotify> notify0 = nullptr;
    ...
    EXPECT_EQ(orderLaunch.RegisterOrderLaunch(group), HCCL_SUCCESS);
    MOCKER_CPP(&OrderLaunch::LaunchInOrder).stubs().will(returnValue(HCCL_SUCCESS));
    // 调用HcomLaunchInOrder函数
    EXPECT_EQ(orderLaunch.HcomLaunchInOrder(group, kernelStream, graphId, notify0, notify1, timeOut), HCCL_SUCCESS);
    GlobalMockObject::verify();
}
...
TEST_F(AicpuUnfold_UT, Ut_IsAclGraphZeroCopyAlgAvailable_When_IsNotReduceOp_Expect_ReturnTrue)
{
    ZeroCopyAclGraph aclgraphcpy;
    HcclCMDType opType = HcclCMDType::HCCL_CMD_BROADCAST;
    OpParam opParam;

    EXPECT_EQ(aclgraphcpy.IsAclGraphZeroCopyAlgAvailable(opType, opParam), true);
}

TEST_F(AicpuUnfold_UT, Ut_IsAclGraphZeroCopyAlgAvailable_When_IsReduceOpAndConfigIs0_Expect_ReturnFalse)
{
    ZeroCopyAclGraph aclgraphcpy;
    HcclCMDType opType = HcclCMDType::HCCL_CMD_REDUCE;
    OpParam opParam;
    opParam.aclGraphZeroCopyEnable = 0; // 模拟用户配置不使能aclGraph的情况

    EXPECT_EQ(aclgraphcpy.IsAclGraphZeroCopyAlgAvailable(opType, opParam), false);
}

TEST_F(AicpuUnfold_UT, Ut_IsAclGraphZeroCopyAlgAvailable_When_IsReduceOpAndConfigIs1_Expect_ReturnTrue)
{
    ZeroCopyAclGraph aclgraphcpy;
    HcclCMDType opType = HcclCMDType::HCCL_CMD_REDUCE_SCATTER;
    OpParam opParam;
    opParam.aclGraphZeroCopyEnable = 1; // 模拟用户配置使能aclGraph的情况

    EXPECT_EQ(aclgraphcpy.IsAclGraphZeroCopyAlgAvailable(opType, opParam), true);
}

TEST_F(AicpuUnfold_UT, Ut_SetAclGraphZeroCopyMode_When_IsReduceOpAndConfigIs1_Expect_ReturnTrue)
{
    ZeroCopyAclGraph aclgraphcpy;
    HcclCMDType opType = HcclCMDType::HCCL_CMD_REDUCE_SCATTER;
    OpParam opParam;
...
TEST_F(AicpuUnfold_UT, AicpuReportRetryErr)
{
    ...
    EXPECT_EQ(hcclCommAicpu->HcclOpExecFsmStoppedProcess(state, errorCode, retryCnt, algName, param, beginSqePos, endSqePos),
              HCCL_E_OPRETRY_FAIL);
    EXPECT_EQ(errorCode, KfcError::kExecConstraint);
    EXPECT_EQ(state, HcclOpExecFSM::HCCL_OP_EXEC_FSM_ERROR);
    GlobalMockObject::verify();
...
TEST_F(AicpuUnfold_UT, ut_aicpu_getcomm)
{
...
TEST_F(AicpuUnfold_UT, ut_log_aicpuCoreExhausted)
{
