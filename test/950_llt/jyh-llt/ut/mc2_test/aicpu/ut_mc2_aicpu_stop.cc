...
class NsRecovery_UT : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "NsRecovery_UT SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "NsRecovery_UT TearDown" << std::endl;
    }
    ...
    virtual void SetUp()
    {
        ...
        std::cout << "NsRecovery_UT Test SetUP" << std::endl;
    }
    virtual void TearDown()
    {
        ResetMC2Context();
        GlobalMockObject::verify();
        std::cout << "NsRecovery_UT Test TearDown" << std::endl;
    }
};
...
TEST_F(NsRecovery_UT, NsCommStop_NsCommClean_for_a3_mc2) {
...
TEST_F(NsRecovery_UT, NsCommStop_for_HcommApi){
    hccl::HcclCommAicpu *hcclCommAicpu = new hccl::HcclCommAicpu;
    hcclCommAicpu->devId_ = 0;
...
TEST_F(NsRecovery_UT, ut_NsCommClean_for_HcommApi){ //OK
...
TEST_F(NsRecovery_UT,ut_KfcCommandHandle_StopExec_clear){ //OK
    ...
    MOCKER_CPP(&HcclCommAicpu::InitZeroCopyExchanger).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(NsRecovery_UT,ut_KfcCommandHandle_StopExec_clear_ERROR){ //ok
    dlog_setlevel(HCCL, DLOG_DEBUG, 1);
    AicpuComContext* ctx = AicpuGetComContext();
    ctx->commOpenStatus = false;
...
TEST_F(NsRecovery_UT, ut_HandleHcclCommAicpu){ //ok
...
TEST_F(NsRecovery_UT, ut_StopBackGround){ //ok
    AicpuComContext* ctx = AicpuGetComContext();
...
TEST_F(NsRecovery_UT,ut_HandleBackGrond_ctx){ //ok
...
TEST_F(NsRecovery_UT, ut_HandleBackGrond_aicpu) //ok
{
    AicpuComContext* ctx = AicpuGetComContext();
    ctx->commOpenStatus = false;
    ctx->isRunning = true;
...
TEST_F(NsRecovery_UT,ut_StopLaunchCommandHandle_ctx){
...
TEST_F(NsRecovery_UT, ut_HostMC2EnvResume){ //ok
...
TEST_F(NsRecovery_UT,ut_StopLaunchCommandHandle_aicpu){
    AicpuComContext *ctx = AicpuGetComContext();
    ctx->commOpenStatus = false;
...
TEST_F(NsRecovery_UT, ut_CleanStreamTrue) {
