...
TEST_F(MC2AicpuRetry_UT, ut_aicpuHdc)
{
    AicpuHdc aicpuHdc;
...
TEST_F(MC2AicpuRetry_UT, ut_suspend_in_once_stopLaunch)
{
 ...
 TEST_F(MC2AicpuRetry_UT, ut_suspend_in_once_Error)
{
...
TEST_F(MC2AicpuRetry_UT, ut_stopExec_in_once_stopExec)
{
...
TEST_F(MC2AicpuRetry_UT, ut_stopExec_in_once_End)
{
    HcclCommunicator hcclCommunicator;
    // 这个里面就是需要有两个内容来保证device侧和host侧是一个共享内存
    StubHccCommRes commRes;
...
TEST_F(MC2AicpuRetry_UT, ut_stopExec_in_once_Error)
{
    HcclCommunicator hcclCommunicator;
    // 这个里面就是需要有两个内容来保证device侧和host侧是一个共享内存
    StubHccCommRes commRes;
...
TEST_F(MC2AicpuRetry_UT, ut_clean_in_once_clean)
{
    HcclCommunicator hcclCommunicator;
    // 这个里面就是需要有两个内容来保证device侧和host侧是一个共享内存
    StubHccCommRes commRes;
...
TEST_F(MC2AicpuRetry_UT, ut_clean_in_once_End)
{
    HcclCommunicator hcclCommunicator;
    // 这个里面就是需要有两个内容来保证device侧和host侧是一个共享内存
    StubHccCommRes commRes;
...
TEST_F(MC2AicpuRetry_UT, ut_clean_in_once_Error)
{
    HcclCommunicator hcclCommunicator;
    // 这个里面就是需要有两个内容来保证device侧和host侧是一个共享内存
    StubHccCommRes commRes;
...
TEST_F(MC2AicpuRetry_UT, ut_cleanEnd_in_once_End)
{
    HcclCommunicator hcclCommunicator;
    // 这个里面就是需要有两个内容来保证device侧和host侧是一个共享内存
    StubHccCommRes commRes;
...
TEST_F(MC2AicpuRetry_UT, ut_UnRegisterBackGroundThread_success){
    HcclCommunicator hcclCommunicator;
...
TEST_F(MC2AicpuRetry_UT, ut_UnRegisterBackGroundThread_fail){
...
TEST_F(MC2AicpuRetry_UT, ut_RecordReportStatus_hcclCommAicpu)
{
    HcclCommAicpu comm;

    for (int i = 0; i < 101; i++) {
        comm.RecordReportStatus(dfx::ReportStatus::kRetrySuccess);
    }
}
...
TEST_F(MC2AicpuRetry_UT, ut_destroy_aicpu_comm)
{
...
TEST_F(MC2AicpuRetry_UT, ut_Handle_Resume_ChangeLink)
{
    KFCResInitTask initTask;
    init_kfc_args(initTask);
    EXPECT_EQ(0, RunAicpuKfcResInit(&initTask));
