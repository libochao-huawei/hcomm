...
# 获取HCCL的根目录
hccl_root = os.path.realpath("../../../../../../../ace/comop/hccl/whole")
...
def check_source_file_match(defined_spec: FileSpec):
    ...
    校验源码是否符合规范定义
    """
    source_file_path = os.path.realpath(os.path.join(hccl_root, defined_spec.source_file_name))
...
def compare_class_spec(spec1: ClassIntfSpec, spec2: ClassIntfSpec, file_name1="", file_name2=""):
    ...
    for func_name in func_names_1:
        if func_name not in func_names_2:
            ...
            compare_matched = False
    print("[====] compare class: \"%s\" end, result: \"%s\"" % (spec1.class_name, "Success" if compare_matched else "Failed"))
...
def compare_file_spec(spec1: FileSpec, spec2: FileSpec):
    ...
    print("[====] compare result: \"%s\". interface define: \"%s\", source file: \"%s\"" % ("Success" if compare_matched else "Failed", spec1.spec_file_name, spec1.source_file_name))
    return compare_matched
