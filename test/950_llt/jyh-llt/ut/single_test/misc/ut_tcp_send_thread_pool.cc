...
HcclResult ut_hrtHalGetDeviceInfo(uint32_t devId, int32_t moduleType, int32_t infoType, int64_t *value)
{
    *value = 4;
    return HCCL_SUCCESS;
}

HcclResult ut_hrtDrvGetDevNum(uint32_t *num_dev)
{
    *num_dev = 0;
    return HCCL_SUCCESS;
}
...
TEST_F(MPI_SendThreadPool_Test, ut_bind_core_test)
{
    DlHalFunction::GetInstance().DlHalFunctionInit();
    ...
    .stubs()
    .will(invoke(ut_hrtHalGetDeviceInfo));

    MOCKER(SalGetTid)
    .stubs()
    .will(returnValue(0));
    MOCKER_CPP(&TcpSendThreadPool::SetAffinity)
    .stubs()
    .will(returnValue(0));

    u32 devId = 0;
    HcclResult ret = TcpSendThreadPool::GetSendPoolInstance()->BindDataCpu(devId);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    MOCKER(hrtDrvGetDevNum)
    .stubs()
    .will(invoke(ut_hrtDrvGetDevNum));
    ret = TcpSendThreadPool::GetSendPoolInstance()->BindDataCpu(devId);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret = hrtHalBindCgroup(BIND_DATACPU_CGROUP);
    DlHalFunction::GetInstance().dlHalBindCgroup = nullptr;
    ret = hrtHalBindCgroup(BIND_DATACPU_CGROUP);
    GlobalMockObject::verify();
}
