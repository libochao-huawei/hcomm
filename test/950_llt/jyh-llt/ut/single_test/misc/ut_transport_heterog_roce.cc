...
std::unique_ptr<LocklessRingMemoryAllocate<HcclMessageInfo>> MPI_TRANSPORT_HETEROG_ROCE_TEST::pMsgInfosMem_ = nullptr;
std::unique_ptr<LocklessRingMemoryAllocate<HcclRequestInfo>> MPI_TRANSPORT_HETEROG_ROCE_TEST::pReqInfosMem_ = nullptr;
std::unique_ptr<HeterogMemBlocksManager> MPI_TRANSPORT_HETEROG_ROCE_TEST::memBlocksManager_ = nullptr;
std::unique_ptr<LocklessRingMemoryAllocate<RecvWrInfo>> MPI_TRANSPORT_HETEROG_ROCE_TEST::pRecvWrInfosMem_ = nullptr;
...
TEST_F(MPI_TRANSPORT_HETEROG_ROCE_TEST, ut_TransportHeterogRoce_MrManager)
{
    MrManager *mrManager = new(nothrow) MrManager();
    map<MrMapKey, MrInfo> unRegMrMap;
    int ret = mrManager->InitUnRegMrMap(unRegMrMap);
    delete mrManager;
    EXPECT_EQ(ret, HCCL_SUCCESS);
}
...
TEST_F(MPI_TRANSPORT_HETEROG_ROCE_TEST, ut_TransportHeterogRoce_White_List)
{
    ...
    int ret = transport->PreQpConnect();
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(MPI_TRANSPORT_HETEROG_ROCE_TEST, ut_TransportHeterogRoce_PullSendStatus)
{
    ...
    TransportHeterogEventRoce transportEvent("test_ta", invalidIp, invalidIp, 18000, 0, transportResourceInfo);
    // TransportHeterogEventRoce transportEvent("test_ta", 0, 1, 18000, 0, transportResourceInfo);
    MOCKER_CPP_VIRTUAL(transportEvent, &TransportHeterogEventRoce::ParseErrorTagSqe)
    .stubs()
    .with(any())
    .will(returnValue(HCCL_SUCCESS));

    MOCKER_CPP_VIRTUAL(transportEvent, &TransportHeterogEventRoce::ParseDataRqes)
    .stubs()
    .with(any())
    .will(returnValue(HCCL_SUCCESS));
    ...
    MOCKER(hrtIbvPollCq)
    .stubs()
    .with(any(), any(), outBoundP(wc, sizeof(wc)), outBound(1))
    .will(returnValue(HCCL_SUCCESS));

    transportHandle->connState_.store(ConnState::CONN_STATE_COMPLETE);
    HcclRequestInfo request;
    request.transportRequest.requestType = HcclRequestType::HCCL_REQUEST_SEND;
    s32 flag = 0;
    HcclStatus compState;
    int ret = transportHandle->Test(request, flag, compState);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();
}

TEST_F(MPI_TRANSPORT_HETEROG_ROCE_TEST, ut_TransportHeterogRoce_PullSendStatus_1)
{
    HcclIpAddress invalidIp;
    TransportHeterogEventRoce transportEvent("test_ta", invalidIp, invalidIp, 18000, 0, transportResourceInfo);
    // TransportHeterogEventRoce transportEvent("test_ta", 0, 1, 18000, 0, transportResourceInfo);
...
TEST_F(MPI_TRANSPORT_HETEROG_ROCE_TEST, ut_TransportHeterog_SubRecvEnvelopNum)
{
    HcclIpAddress invalidIp;
    unique_ptr<TransportHeterogRoce> transport(new (nothrow) TransportHeterogRoce("test_collective", invalidIp, invalidIp, 18000, 0, transportResourceInfo));
    TransportHeterogRoce *transportHandle = transport.get();
    transportHandle->AddRecvEnvelopNum(); 
    transportHandle->SubRecvEnvelopNum();
    GlobalMockObject::verify();
}
