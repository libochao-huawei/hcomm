...
class UT_TRANSPORT_MEM_TEST : public testing::Test
{
protected:
    static void SetUpTestCase()
    {
        std::cout << "UT_TRANSPORT_MEM_TEST SetUP" << std::endl;
        pMsgInfosMem_.reset(new (std::nothrow) LocklessRingMemoryAllocate<HcclMessageInfo>(100));
        if (pMsgInfosMem_ == nullptr) return;
        pMsgInfosMem_->Init();

        pReqInfosMem_.reset(new (std::nothrow) LocklessRingMemoryAllocate<HcclRequestInfo>(100));
        if (pReqInfosMem_ == nullptr) return;
        pReqInfosMem_->Init();

        memBlocksManager_.reset(new (std::nothrow) HeterogMemBlocksManager());
        if (memBlocksManager_ == nullptr) return;
        memBlocksManager_->Init(MEM_BLOCK_NUM);

        pRecvWrInfosMem_.reset(new (std::nothrow) LocklessRingMemoryAllocate<RecvWrInfo>(100));
        if (pRecvWrInfosMem_ == nullptr) return;
        pRecvWrInfosMem_->Init();
    }
    static void TearDownTestCase()
    {
        std::cout << "UT_TRANSPORT_MEM_TEST TearDown" << std::endl;
    }
    virtual void SetUp()
    {
        hrtSetDevice(0);
    ...
    static std::unique_ptr<LocklessRingMemoryAllocate<HcclMessageInfo>> pMsgInfosMem_;
    static std::unique_ptr<LocklessRingMemoryAllocate<HcclRequestInfo>> pReqInfosMem_;
    static std::unique_ptr<HeterogMemBlocksManager> memBlocksManager_;
    static std::unique_ptr<LocklessRingMemoryAllocate<RecvWrInfo>> pRecvWrInfosMem_;
    TransportResourceInfo transportResourceInfo = TransportResourceInfo(nullptr, pMsgInfosMem_, pReqInfosMem_,
        memBlocksManager_, pRecvWrInfosMem_);
};
std::unique_ptr<LocklessRingMemoryAllocate<HcclMessageInfo>> UT_TRANSPORT_MEM_TEST::pMsgInfosMem_ = nullptr;
std::unique_ptr<LocklessRingMemoryAllocate<HcclRequestInfo>> UT_TRANSPORT_MEM_TEST::pReqInfosMem_ = nullptr;
std::unique_ptr<HeterogMemBlocksManager> UT_TRANSPORT_MEM_TEST::memBlocksManager_ = nullptr;
std::unique_ptr<LocklessRingMemoryAllocate<RecvWrInfo>> UT_TRANSPORT_MEM_TEST::pRecvWrInfosMem_ = nullptr;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_TransportMem_Init_Roce)
{
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_TransportMem_Init_Ipc)
{
    dlog_setlevel(HCCL, DLOG_DEBUG, 1);
    HcclResult ret = HCCL_SUCCESS;
    s32 rank = 0;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_ipc_mem_exchange)
{
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_ipc_mem_enable_disable_mem_access)
{
    std::unique_ptr<NotifyPool> notifyPool = std::make_unique<NotifyPool>();
    NetDevContext devContext;
    HcclNetDevCtx netDevCtx = &devContext;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_ipc_mem_read_write)
{
    HcclResult ret;
    rtStream_t stream;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_ipc_mem_get_trans_info)
{
    HcclResult ret;
    HcclQpInfoV2 qpInfo;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_ipc_mem_wait_op_frence)
{
    HcclResult ret;
    std::shared_ptr<TransportMem> transport = CreateIpcMemTransport();
    rtStream_t stream;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_roce_mem_exchange)
{
    std::unique_ptr<NotifyPool> notifyPool = std::make_unique<NotifyPool>();
    NetDevContext devContext;
    HcclNetDevCtx netDevCtx = &devContext;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_roce_mem_CheckRaSendNormalWrlistSupport)
{
    std::unique_ptr<NotifyPool> notifyPool = std::make_unique<NotifyPool>();
    NetDevContext devContext;
    HcclNetDevCtx netDevCtx = &devContext;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_roce_mem_CheckRaSendNormalWrlistSupport2)
{
    std::unique_ptr<NotifyPool> notifyPool = std::make_unique<NotifyPool>();
    NetDevContext devContext;
    HcclNetDevCtx netDevCtx = &devContext;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_roce_mem_enable_disable_mem_access)
{
    std::unique_ptr<NotifyPool> notifyPool = std::make_unique<NotifyPool>();
    NetDevContext devContext;
    HcclNetDevCtx netDevCtx = &devContext;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_roce_mem_read_write)
{
    HcclResult ret;
    rtStream_t stream;
    rtError_t rt_ret = rtStreamCreate(&stream, 0);
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_roce_mem_qp_info_get)
{
    HcclResult ret;
    HcclQpInfoV2 qpInfo;
...
TEST_F(UT_TRANSPORT_MEM_TEST, ut_transport_roce_mem_get_trans_info)
{
    HcclResult ret;
    HcclQpInfoV2 qpInfo;
    std::shared_ptr<TransportRoceMem> transport = CreateRoceMemTransport();
...
TEST_F(UT_TRANSPORT_MEM_TEST, st_transport_device_roce_mem_qp_info_get)
{
    HcclResult ret;
