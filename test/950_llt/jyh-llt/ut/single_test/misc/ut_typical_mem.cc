...
protected:
    ...
    void *ptr;
    struct MrInfoT fakeMrInfo;
};

TEST_F(TypicalWindowMemTest, ut_window_mem_alloc_free_test)
{
...
TEST_F(TypicalWindowMemTest, ut_free_all_window_mem_test)
{
    s32 ret = HCCL_SUCCESS;
...
TEST_F(TypicalSyncMemTest, ut_sync_mem_alloc_free_test)
{
    MOCKER(HrtRaGetNotifyBaseAddr).stubs().will(invoke(stub_HrtRaGetNotifyBaseAddr));
...
TEST_F(TypicalSyncMemTest, ut_free_all_sync_mem_test)
{
    MOCKER(HrtRaGetNotifyBaseAddr).stubs().will(invoke(stub_HrtRaGetNotifyBaseAddr));
    MOCKER_CPP(&TypicalSyncMem::InitNotifySrcMem).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(TypicalSyncMemTest, ut_get_noitfy_handle_test)
{
    s32 ret = HCCL_SUCCESS;
...
TEST_F(TypicalSyncMemTest, ut_get_noitfy_src_mem)
{
    s32 ret = HCCL_SUCCESS;
    MrInfoT srcMrInfoOut = {};
...
HcclResult stub_hrtRaRegGlobalMr(const RdmaHandle rdmaHandle, struct MrInfoT &mrInfo, MrHandle &mrHandle)
{
...
TEST_F(TypicalMrManagerTest, ut_mr_manager_register_deregister_test)
{
...
TEST_F(TypicalMrManagerTest, ut_mr_manager_release_mr_resource_test)
{
    MOCKER(hrtRaRegGlobalMr)
    .stubs()
    .will(invoke(stub_hrtRaRegGlobalMr));
