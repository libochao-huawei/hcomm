...
TEST_F(OpRetryConnTest, ut_opretry_connection_default_root)
{
    ...
    EXPECT_EQ(conn.Init(rankId, rankSize, hostIp, serverPort, 13, localIp), HCCL_SUCCESS);
...
TEST_F(OpRetryConnTest, ut_opretry_connection)
{
    ...
    EXPECT_EQ(conn.Init(rankId, rankSize, hostIp, serverPort, 14, localIp, rootRank), HCCL_SUCCESS);
    EXPECT_EQ(conn.DeInit(), HCCL_SUCCESS);
}
...
TEST_F(OpRetryConnTest, ut_opretry_connection_whitelist)
{
    ...
    EXPECT_EQ(conn.Init(rankId, rankSize, hostIp, serverPort, 15, localIp, rootRank), HCCL_SUCCESS);
    EXPECT_EQ(conn.DeInit(), HCCL_SUCCESS);
