...
HcclResult stub_GetRaResourceInfo_SocketManagerTest(NetworkManager* that, RaResourceInfo &raResourceInfo)
{
...
TEST_F(SocketManagerTest, ut_SocketManagerTest_LocalServer)
{
    ...
    ret = socketManager->ServerInit(portCtx, 17668);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    for (auto &item : socketsMap) {
        ...
        ret = tempSocket->Accept(commTag, newSocket);
        ...
        .with(any())
        .will(returnValue(100));
        ret = tempSocket->IRecv((void *)buff, sizeof(buff), compSize);
        EXPECT_EQ(ret, 0);
        ...
        tempSocket->GetTag();
        tempSocket->GetSocketType();
        tempSocket->GetNicSocketHandle();
        tempSocket->GetRemotePort();
        tempSocket->GetLocalPort();
        tempSocket->GetLocalRole();
...
TEST_F(SocketManagerTest, ut_SocketManagerTest_LocalListen)
{
    HcclResult ret;
    ...
    ret = tempSocket->Listen();  
    EXPECT_EQ(ret, HCCL_SUCCESS);

    std::shared_ptr<HcclSocket> connectSocket = std::make_shared<HcclSocket>(commTag,
            nicPortCtx, localIp, 0, HcclSocketRole::SOCKET_ROLE_CLIENT);
    ret = connectSocket->Init();
    EXPECT_EQ(ret, HCCL_SUCCESS);

    ret = connectSocket->Connect();
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(SocketManagerTest, ut_SocketManagerTest_GetStatus_SOCKET_ERROR)
{
    ...
    remoteRankInfo.userRank = dstRank2;
    ...
    ret = socketManager->WaitLinksEstablishCompleted(localRole, socketsMap, dstRankToUserRank, loaclRankInfo, remoteRankInfo, netDevCtx);
    EXPECT_EQ(ret, HCCL_E_TCP_CONNECT);
    GlobalMockObject::verify();
    ...
    std::shared_ptr<HcclSocket> tempSocket(new (std::nothrow)HcclSocket("my tag2", nullptr, ipAddr, 16666, HcclSocketRole::SOCKET_ROLE_SERVER));
    ret = socketManager->WaitLinkEstablish(tempSocket);
    EXPECT_EQ(ret, HCCL_E_TIMEOUT);
