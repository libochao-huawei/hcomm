...
TEST_F(TaskExceptionTest, ut_without_task_callback)
{
    u32 deviceLogicId = 0;
    TaskExceptionHandler taskExceptionHandler(deviceLogicId);
    ...
    ret = taskExceptionHandler.InsertTaskMap(streamID, taskInfo1);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    rtExceptionInfo rtExceptionInfo1{65535, 0, 0, 0, 0};
    taskExceptionHandler.Callback(&rtExceptionInfo1);
}

TEST_F(TaskExceptionTest, ut_task_exception_getparadma)
{
    u32 deviceLogicId = 0;
    TaskExceptionHandler taskExceptionHandler(deviceLogicId);
    ...
    str = taskInfo1.GetParaDMA();
}

TEST_F(TaskExceptionTest, ut_without_task_callback2)
{
    u32 deviceLogicId = 0;
    TaskExceptionHandler taskExceptionHandler(deviceLogicId);
...
TEST_F(TaskExceptionTest, ut_TaskInfo_GetOpDataAndRankInfo)
{
    ...
    ret = taskExceptionHandler.GetOpDataInfoByTag(tag, opDataInfo);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    std::string stageErrInfo = "";
    TaskExceptionHandler::PrintOpDataInfo(opDataInfo, true, stageErrInfo);
...
TEST_F(TaskExceptionTest, ut_TaskInfo_GetUserGroupName)
{
    u32 deviceLogicId = 0;
    TaskExceptionHandler taskExceptionHandler(deviceLogicId);
    ...
    std::string udi = "test_user_group_name";
    ret = taskExceptionHandler.AddGroupUdi(group, udi);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    std::string getUdi;
    ret = taskExceptionHandler.GetUdiByGroup(group, getUdi);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    auto isFound = groupRankContentInfo.find("user define information");
    EXPECT_NE(isFound, std::string::npos);
    ...
    ret = taskExceptionHandler.DelGroupUdi(group);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(TaskExceptionTest, Ut_PrintCommAivInfo_When_GetDevice_Fail_Expect_Ret_HCCL_E_PARA)
{
    MOCKER(hrtGetDevice)
        .stubs()
        .with(any())
...
TEST_F(TaskExceptionTest, Ut_PrintCommAivInfo_When_AivGroup_Count_Zero_Expect_Print_Group_No_Aiv)
{
    u32 deviceLogicId = 0;
...
TEST_F(TaskExceptionTest, Ut_PrintCommAivInfo_When_AivGroup_Size_Zero_Expect_Print_Aiv_Group_Not_Recode)
{
    u32 deviceLogicId = 0;
...
TEST_F(TaskExceptionTest, Ut_PrintCommAivInfo_When_AivGroup_Size_One_Expect_No_Print_Aiv_Group)
{
    u32 deviceLogicId = 0;
...
TEST_F(TaskExceptionTest, Ut_PrintCommAivInfo_When_AivGroup_Size_No_Zero_Expect_Print_Multi_Aiv_May_Execution_Stuck)
{
    u32 deviceLogicId = 0;
...
TEST_F(TaskExceptionTest, Ut_PrintCommAivInfo_When_AivGroup_Size_100_Expect_Print_Multi_Aiv_May_Execution_Stuck)
{
    u32 deviceLogicId = 0;
...
TEST_F(TaskExceptionTest, Ut_DealExceptionTask_When_Comm_Has_Multi_Aiv_Expect_Print_May_Execution_Stuck)
{
    u32 deviceLogicId = 0;
    TaskExceptionHandler taskExceptionHandler(deviceLogicId);
