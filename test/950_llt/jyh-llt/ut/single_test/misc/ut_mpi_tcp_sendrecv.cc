...
TEST_F(MPI_TCP_SENDRECV_TEST, ut_mpi_tcp_send_link)
{
    ...
    MOCKER_CPP(&TransportHeterogIbv::InitStateUpdate)
    .stubs()
    .with(any())
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(MPI_TCP_SENDRECV_TEST, ut_mpi_tcp_imrecv)
{
    ...
    enveLope.envelope.dataType = HCCL_DATA_TYPE_INT8;
    ...
    msgInfo->transportHandle = link.get();
    void* msg = msgInfo;
