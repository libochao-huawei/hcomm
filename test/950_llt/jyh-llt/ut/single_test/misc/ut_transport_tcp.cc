...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_EschedAckCallback)
{
    ...
    TransportHeterogEventTcp::gCompCounterEvent[0][HCCL_EVENT_RECV_REQUEST_MSG].counter = 1;
    TransportHeterogEventTcp::EschedAckCallback(0, HCCL_EVENT_RECV_REQUEST_MSG);
}
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_transportTcp_1951_infer_DelEpollEvents)
{
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_transportTcp_1951_infer_AddEpollEvents)
{
    MOCKER(hrtRaCtlEventHandle)
    .stubs()
    .with(any())
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_transportTcp_1951_infer_CreateEventHandle)
{
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_transportTcp_1951_infer_AddFdMapping)
{
    HcclIpAddress invalidIp;
    unique_ptr<TransportHeterogEventTcp> transportHeterogTcp(new (nothrow) TransportHeterogEventTcp("test_collective", invalidIp, invalidIp, 18000, 0, 0, transportResourceInfo));
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_transportTcp_1951_infer_DestroyEventHandle)
{
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_transportTcp_1951_infer_WaitEvents)
{
    HcclIpAddress invalidIp;
    unique_ptr<TransportHeterogEventTcp> transportHeterogTcp(new (nothrow) TransportHeterogEventTcp("test_collective", invalidIp, invalidIp, 18000, 0, 0, transportResourceInfo));
...
HcclRequestInfo* stub_HcclRequestInfo_tcp_Alloc()
{
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_transportTcp_BlockSend)
{
    ...
    .stubs()
    .with(any())
    .will(invoke(stub_HcclRequestInfo_tcp_Alloc));
    bool envCompleted = true;
    bool tranCompleted = true;
...
TEST_F(MPI_TRANSPORT_TCP_TEST, ut_TransportHeterogEventTcp_GetNetworkResource)
{
    MOCKER(hrtRaSocketBatchClose)
    .stubs()
    .with(any())
    ...
    HcclResult ret = transportHeterogTcp->GetNetworkResource();
    EXPECT_EQ(ret, HCCL_E_PARA);

    HcclRequestInfo request;
    HcclStatus compState;
    s32 flag = 2;
    ret = transportHeterogTcp->QueryRequestStatus(request, flag, compState);
    EXPECT_EQ(ret, HCCL_E_PARA);
