...
TEST_F(MemDeviceTest, ut_Alloc_When_Normal_Expect_ReturnIsHCCL_SUCCESS)
{
    DeviceMem mem;
    s32 ret = DeviceMem::alloc(mem, 8);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();
}

TEST_F(MemDeviceTest, ut_Alloc_When_OOM_Expect_ReturnIsHCCL_E_OOM)
{
    /*构造rt_malloc OOM异常*/
    MOCKER(hrtMalloc)
    .expects(atMost(1))
