...
TEST_F(AlltoallDirectFullmeshTest, run_async_00)
{
    ...
    tempAlg->algOpContext_.mc2Handler = mc2Handler;
    tempAlg->podStartRank_ = 0;
    tempAlg->devNumInlocalPod_ = 2;
    localSendRecvInfo.sendLength[0] = 0;
    localSendRecvInfo.sendLength[1] = 0;
    localSendRecvInfo.recvLength[0] = 0;
    localSendRecvInfo.recvLength[1] = 0;
    tempAlg->localSendRecvInfoPtr_ = &localSendRecvInfo;
    tempAlg->sdmaDataBlockSize_ = 2;

    ret = tempAlg->RunAsync();
    EXPECT_EQ(ret, HCCL_SUCCESS);

    localSendRecvInfo.sendLength[0] = 1;
    localSendRecvInfo.sendLength[1] = 1;
    localSendRecvInfo.recvLength[0] = 1;
    localSendRecvInfo.recvLength[1] = 1;
    tempAlg->sdmaConcurrentNum_ = 1;
    ret = tempAlg->RunAsync();
    EXPECT_EQ(ret, HCCL_SUCCESS);
