...
class AlignedReduceScatterDoubleRingWithSerialLocalCopyTest : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        ...
        std::cout << "\033[36m--AlignedReduceScatterDoubleRingWithSerialLocalCopyTest SetUP--\033[0m" << std::endl;
    }
    static void TearDownTestCase()
    {
        ...
        std::cout << "\033[36m--AlignedReduceScatterDoubleRingWithSerialLocalCopyTest TearDown--\033[0m" << std::endl;
    }
...
HcclDispatcher AlignedReduceScatterDoubleRingWithSerialLocalCopyTest::dispatcherPtr = nullptr;
DispatcherPub *AlignedReduceScatterDoubleRingWithSerialLocalCopyTest::dispatcher = nullptr;

TEST_F(AlignedReduceScatterDoubleRingWithSerialLocalCopyTest, destructor_D0)
{
    ...
    std::unique_ptr<AlgTemplateBase> tempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(
        TemplateType::TEMPLATE_REDUCESCATTER_DB_RING_SLC, dispatcher);
...
TEST_F(AlignedReduceScatterDoubleRingWithSerialLocalCopyTest, run_async_00)
{
    ...
    multRingsSlices.push_back(cCLslices);
    ret = tempAlg->Prepare(input, output, output, 1, HCCL_DATA_TYPE_INT8, stream, multRingsSlices, HCCL_REDUCE_SUM,
                           0, 0, false, reduceAttrBitMap, &opInfo, userRank, subStreams, mainSignals, subSignals,
...
TEST_F(AlignedReduceScatterDoubleRingWithSerialLocalCopyTest, run_async_01)
{
    s32 ret = HCCL_SUCCESS;
    ...
    multRingsSlices.push_back(cCLslices);
    multRingsSlices.push_back(cCLslices);
    ret = tempAlg->Prepare(input, output, output, 3, HCCL_DATA_TYPE_INT8, stream, multRingsSlices, HCCL_REDUCE_SUM,
                           0, 0, false, reduceAttrBitMap, &opInfo, userRank, subStreams, mainSignals, subSignals,
                           tmpRingsOrders, userMemInputSlicesOfDoubleRing);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(AlignedReduceScatterDoubleRingWithSerialLocalCopyTest, GetActiveSubstreamNum)
{
    s32 ret = HCCL_SUCCESS;
    ...
    multRingsSlices.push_back(cCLslices);
    multRingsSlices.push_back(cCLslices);
    std::unique_ptr<AlignedReduceScatterDoubleRingWithSerialLocalCopy> tempAlg =
        std::unique_ptr<AlignedReduceScatterDoubleRingWithSerialLocalCopy>(
            dynamic_cast<AlignedReduceScatterDoubleRingWithSerialLocalCopy*>(basePtr.release()));
    ret = tempAlg->Prepare(input, output, output, 3, HCCL_DATA_TYPE_INT8, stream, multRingsSlices, HCCL_REDUCE_SUM,
                           0, 0, false, reduceAttrBitMap, &opInfo, userRank, subStreams, mainSignals, subSignals,
                           tmpRingsOrders, userMemInputSlicesOfDoubleRing);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret = tempAlg->GetActiveSubstreamNum(activeSubstreamNum);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    tempAlg->subStreams_.pop_back();
    ret = tempAlg->GetActiveSubstreamNum(activeSubstreamNum);
