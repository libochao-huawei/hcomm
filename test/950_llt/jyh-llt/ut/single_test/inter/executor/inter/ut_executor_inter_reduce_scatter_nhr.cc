...
void run_async_test_reducescatter_nhr(DispatcherPub *dispatcher, u32 rank, u32 rankSize, u32 root = -1)
{
    ...
    ret = tempAlg->Prepare(input, output, scratch, 128, HCCL_DATA_TYPE_INT16, stream,
...
{
    run_async_test_reducescatter_nhr(dispatcher, /*rank=*/2, /*rankSize=*/8);
}
#endif

TEST_F(ReduceScatterNHRInterTest, test_RunAsyncWithReorder)
{
    s32 ret = HCCL_SUCCESS;
    u32 rank = 1;
    u32 rankSize = 8;
    ...
    std::unique_ptr<AlgTemplateBase> tempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(
        TemplateType::TEMPLATE_REDUCESCATTER_NHR, dispatcher);
    EXPECT_EQ(HcclResult::HCCL_SUCCESS, tempAlg->Prepare(reduceAttrBitMap, true));
