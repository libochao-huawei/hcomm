...
HcclDispatcher get_inner_dispatcher(s32 devid, std::shared_ptr<hccl::ProfilerManager> &profilerManager)
{
...
void* comm_inner_task_handle(void* para)
{
    ...
    para_info->comm_inner.reset(new CommBase(para_info->collectiveId,
                                para_info->userRank,
...
TEST_F(CommInnerTest, destructor_D0)
{
...
TEST_F(CommInnerTest, gget_transport_by_rank_err)
{
    std::string rootInfo = "test_collective";
    IntraExchanger exchanger{};
    std::vector<RankInfo> para_vector(1);
...
TEST_F(CommInnerTest, get_rank_by_userrank)
{
    s32 ret = HCCL_SUCCESS;
...
TEST_F(CommInnerTest, get_userrank_by_rank)
{
    s32 ret = HCCL_SUCCESS;
...
TEST_F(CommInnerTest, get_userrank_by_rank_with_err)
{
    s32 ret = HCCL_SUCCESS;
...
TEST_F(CommInnerTest, ut_TransportInit)
{
    ...
    std::map<HcclIpAddress, HcclNetDevCtx> netDevCtxMap;
    ret = ConstructNetDevCtx(netDevCtxMap, NICDeployment::NIC_DEPLOYMENT_DEVICE, devicePhyId, devicePhyId, NicType::DEVICE_NIC_TYPE, tmp_para.nicIp[0]);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    CommBase* comm_inner = new CommBase(collective_id_tmp, userRank, user_rank_size, userRank, 1,  para_vector, topoFlag, dispatcher, nullptr,
    ...
    comm_star->interSocketManager_->ServerInit(netDevCtxMap[tmp_para.nicIp[0]], 0);
    ...
    comm_star->interSocketManager_->ServerDeInit(netDevCtxMap[tmp_para.nicIp[0]], 0);
