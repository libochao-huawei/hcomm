...
HcclDispatcher CommP2PTest::dispatcherPtr = nullptr;
DispatcherPub *CommP2PTest::dispatcher = nullptr;
...
TEST_F(CommP2PTest, destructor_D0)
{
    ...
    TopoType topoFlag = TopoType::TOPO_TYPE_8P_RING;
    std::map<HcclIpAddress, HcclNetDevCtx> netDevCtxMap;
    CommP2P* comm_p2p = new CommP2P(rootInfo, 0, 1, 0, 1, topoFlag, dispatcher, nullptr, netDevCtxMap, exchanger, para_vector, DeviceMem(), DeviceMem(), true, nullptr, 0);

    delete comm_p2p;
}

TEST_F(CommP2PTest, CommP2P_init)
{
    s32 ret = HCCL_SUCCESS;
    ...
    TopoType topoFlag = TopoType::TOPO_TYPE_8P_RING;
    std::map<HcclIpAddress, HcclNetDevCtx> netDevCtxMap;
    CommP2P* comm_p2p = new CommP2P(collective_id_tmp, userRank, user_rank_size, 0, 1, topoFlag, dispatcher, nullptr,
        netDevCtxMap, exchanger, para_vector, DeviceMem::alloc(1024), DeviceMem::alloc(1024), true, nullptr, 0);
...
TEST_F(CommP2PTest, ut_CommP2P_init_2_thread)
{
    s32 ret = HCCL_SUCCESS;
    ...
    for (s32 i = 0; i < ndev; i++)
    {
        ...
        para_info[i].tag = "ut_CommP2P_init_2_thread";
    ...
    tid[0] = sal_thread_create("CommP2P rank0 thread", comm_p2p_task_handle, (void*)&para_info[0]);

    tid[1] = sal_thread_create("CommP2P rank1 thread", comm_p2p_task_handle, (void*)&para_info[1]);
