...
#if 1
TEST_F(CollAllReduceInterTest, reduce_plus_bcast)
{
...
#if 1
TEST_F(CollAllReduceInterTest, 310P_doubling_direct)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclCommParams params;
    RankTable_t rankTable;
    ...
    std::unique_ptr<TopoMatcher> &topoMatcher = implBase->implAlg_->topoMatcher_;
    CollAllReduceFor310PDoublingDirectExecutor* executor = new CollAllReduceFor310PDoublingDirectExecutor(impl->dispatcher_, topoMatcher);
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    implBase->AllocAlgResource(opParam.tag, HcclCMDType::HCCL_CMD_ALLREDUCE, opParam, resourceRequest, resourceResponse);
    resourceResponse.cclInputMem = DeviceMem::alloc(4096);
    resourceResponse.cclOutputMem = DeviceMem::alloc(4096);
    ret = executor->Orchestrate(opParam, resourceResponse);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
#if 1
TEST_F(CollAllReduceInterTest, ut_allreduce_mesh_opbase_bigcount_aiv)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclCommParams params;
    RankTable_t rankTable;
