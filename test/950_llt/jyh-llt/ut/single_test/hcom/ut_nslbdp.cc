...
class UTnslbdpTest : public testing::TestWithParam<bool>
{
protected:
...
TEST_F(UTnslbdpTest, hccl_prepare_ut_env)
{
    ...
    hcclNslbDp::GetInstance().SendGlobalRankTable(0);
    hcclNslbDp::GetInstance().SendGlobalRankTable(11);
...
TEST_F(UTnslbdpTest, Ut_HcclNslb_When_Ahc_Expect_ReturnIsTRUE)
{
    MOCKER(H2DTlvRequest)
    .stubs()
    .with(any())
    ...
    u32 srcLocalRankId = 0;
    u8 algType = 0;
    string identifier = "192.168.0.101-8000-8001";
    u32 nRanks = 8;
    u32 rank = 1;
    u32 rankSize = 8;
    u64 count = 1000000;
    string algName = "ReduceScatterVMeshOpbaseExecutor";
    hcclNslbDp::GetInstance().SetGlobalCommNodeId(1);
    hcclNslbDp::GetInstance().SetGlobalCommLocalRankNum(localRankSize);
    hcclNslbDp::GetInstance().SetGlobalCommTaskId(25771649526027);
    hcclNslbDp::GetInstance().SetGlobalCommRankTotalNum(totalRankSize);
...
TEST_F(UTnslbdpTest, Ut_HcclNslb_When_Noranktable_Expect_ReturnIsError)
{
    MOCKER(H2DTlvRequest)
    .stubs()
    .with(any())
    ...
    u32 srcLocalRankId = 0;
    u8 algType = 0;
    std::string identifier = "192.168.0.101_8000_8001";
    u32 nRanks = 8;
    u32 rank = 1;
    u32 rankSize = 8;
    u64 count = 1000000;
    ...
    NslbDpOperatorInfo OperatorInfo = { 0 };
    OperatorInfo.taskId = 0xC0A800011;
    OperatorInfo.oper = 1;
    hcclNslbDp::GetInstance().fullcommDescInitTime(identifier, OperatorInfo);
    hcclNslbDp::GetInstance().CheckSameOperatorVal(0, OperatorInfo, 0);
    ...
    hcclNslbDp::GetInstance().InitCmmDesc(identifier);
    identifier = hcclNslbDp::GetInstance().GetCmmDesc();
...
TEST_F(UTnslbdpTest, ut_sendtable1)
{
    MOCKER(H2DTlvInit)
    .stubs()
    .with(any())
...
TEST_F(UTnslbdpTest, ut_init_hccp)
{
    MOCKER(H2DTlvInit)
    .stubs()
    .with(any())
    ...
    OperatorInfo.oper = 1;
    std::string identifier = "192.168.0.101_8000_8001";
    (void)memcpy_s(OperatorInfo.commDesc, sizeof(OperatorInfo.commDesc), identifier.c_str(), identifier.size());

    OperatorInfo.algorithm = 2;
    OperatorInfo.trafficCnt = 100000;

    u32 buffer_size = 4096;
    void *tlv_handle = nullptr;
    hcclNslbDp::GetInstance().setHccpInfo(buffer_size, tlv_handle);
    hcclNslbDp::GetInstance().SendRankTableOpAndAdj(OperatorInfo);
...
TEST_F(UTnslbdpTest, ut_inithccp_nslbfail)
{
    int ret = HCCL_SUCCESS;
    uint32_t ndev = 1;
    int32_t devices[ndev] = {0};
...
TEST_F(UTnslbdpTest, ut_checklevel1_910b)
{
    u8 typel1 = 0;
...
TEST_F(UTnslbdpTest, ut_checklevel2_910c)
{
    u8 typel2 = 0;
    ...
    algValue = AlgTypeLevel2::ALG_LEVEL2_PIPELINE;
    typel2 = hcclNslbDp::GetInstance().GetNslbLevel2AlgType(algValue);
    EXPECT_EQ(typel2, NSLB_ALGO_TYPE_PIPELINE);    
}

TEST_F(UTnslbdpTest, ut_checkNslbOpType)
{
    u8 type1 = 0;
...
TEST_F(UTnslbdpTest, ut_checkfirst4bit)
{
...
TEST_F(UTnslbdpTest, ut_checkalgo_match)
{
...
TEST_F(UTnslbdpTest, ut_getnslb_l4portid)
{
    u32 rankSize = 8;
    u8 algType = 1;
    u16 l4SPortId = 0;
...
TEST_F(UTnslbdpTest, ut_collectsend_table4)
{
    MOCKER(H2DTlvRequest)
    .stubs()
    .with(any())
...
TEST_F(UTnslbdpTest, ut_collect_table2_910b)
{
    MOCKER(H2DTlvRequest)
    .stubs()
    .with(any())
    ...
    std::string identifier_ = "192.168.0.101_8000_8001";
...
TEST_F(UTnslbdpTest, ut_collect_table3_910b)
{
    MOCKER(H2DTlvRequest)
    .stubs()
    .with(any())
...
TEST_F(UTnslbdpTest, ut_Initconfig_nslb)
{
    MOCKER(H2DTlvInit)
    .stubs()
    .with(any())
...
TEST_F(UTnslbdpTest, ut_Initconfig_NoNslb)
{
    MOCKER(H2DTlvInit)
    .stubs()
    .with(any())
...
TEST_F(UTnslbdpTest, ut_collect_ahc_table2_910C)
{
    MOCKER(H2DTlvRequest)
    .stubs()
    .with(any())
    ...
    DevType deviceType_ = DevType::DEV_TYPE_910_93;
    u32 serverNum_ = 8;
...
TEST_F(UTnslbdpTest, ut_collect_ahc_table3_910C)
{
    MOCKER(H2DTlvRequest)
    .stubs()
    .with(any())
...
TEST_F(UTnslbdpTest, Ut_HcclNslb_When_SendTable1_Expect_ReturnIsSUCCESS)
{
    ...
    HcclResult result = hcclNslbDp::GetInstance().SendTableProc(0, 2, globalTable1);
    EXPECT_EQ(result, HCCL_SUCCESS);
    result = hcclNslbDp::GetInstance().SendTableProc(1, 2, globalTable1);
    EXPECT_EQ(result, HCCL_SUCCESS);
    result = hcclNslbDp::GetInstance().SendTableProc(11, 2, globalTable1);
    EXPECT_EQ(result, HCCL_SUCCESS);
}