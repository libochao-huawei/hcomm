...
void UpdateIsInterRdmaStub(TransportManager *, const u32 remoteRank, bool &isInterRdma, bool forceRdma)
{
    isInterRdma = true;
}
...
TEST_F(BackupLinkTest, ut_SetMachinePara)
{
    ...
    MOCKER_CPP(&TransportManager::UpdateIsInterRdma).stubs().will(invoke(UpdateIsInterRdmaStub));
    MOCKER_CPP(&TransportManager::GetSocketsPerLink).stubs().with(any()).will(returnValue(0));
    MOCKER_CPP(&TransportManager::MakeRemoteLinkInfo).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    ...
    bool isBackup = true;
    communicator->transportManager_->CreateDestSockets("CreateDestSockets",
        remoteRank, taskNum, connectSockets, netDevCtx, isInterRdma, forceRdma, isBackup);
...
TEST_F(BackupLinkTest, ut_RefreshAlgResponseTransportRes)
{
    ...
    ret = hcclCommAicpu->RefreshAlgResponseTransportRes("test", algResResponse, remoteRankPortMap, isChangeLinkFlag, &commParam, opParam);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(BackupLinkTest, ut_CloseHccpProcess)
{
    ...
    MOCKER(hrtProcessCloseSubProcList)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));

    ProcOpenArgs hccpProcessInfo;
    pid_t pid = 1111;
    hccpProcessInfo.subPid = &pid;
    hccpProcessInfo.procType = SubProcType::TSD_SUB_PROC_HCCP;
    ret = NetworkManager::GetInstance(0).CloseHccpProcess(hccpProcessInfo);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
{
    HcclResult ret = HCCL_SUCCESS;
    MOCKER(hrtTsdProcessOpen).stubs().will(returnValue(HCCL_SUCCESS));
...
TEST_F(BackupLinkTest, ut_ReAllocTransportResource)
{
    ...
    */
    ...
    ret = hcclCommAicpu->CleanRoceResource("test", algResResponse, remoteRankPortMap, param);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    ret = hcclCommAicpu->ReAllocTransportResource("test", algResResponse, remoteRankPortMap, &commParam, param);
    EXPECT_EQ(ret, HCCL_SUCCESS);
