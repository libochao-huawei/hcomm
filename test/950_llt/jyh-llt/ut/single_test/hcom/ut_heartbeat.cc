...
TEST_F(HeartBeatTest, ut_HeartBeatTest5)
{
    ...
    MOCKER_CPP(&Heartbeat::DeInit).stubs().with(any()).will(returnValue(0));
...
TEST_F(HeartBeatTest, ut_DelErrorSocket)
{
    MOCKER_CPP(&HcclSocket::Close)
    .expects(once())
    .will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HCCL_SUCCESS;
    ...
    rankInfo.devicePhyId = 10;
    UIDType tmp = Heartbeat::GetInstance(0).GetUId(rankInfo);
    Heartbeat::GetInstance(0).errorSocket_.push_back(dst);
    Heartbeat::GetInstance(0).DelErrorSocket();
    EXPECT_EQ(0, Heartbeat::GetInstance(0).errorSocket_.size());
    EXPECT_EQ(0, Heartbeat::GetInstance(0).rankId2SocketMap_.has(dst));
    Heartbeat::GetInstance(0).errorSocket_.push_back(dst);
    Heartbeat::GetInstance(0).DelErrorSocket();
    EXPECT_EQ(0, Heartbeat::GetInstance(0).errorSocket_.size());
    EXPECT_EQ(0, Heartbeat::GetInstance(0).rankId2SocketMap_.has(dst));
...
TEST_F(HeartBeatTest, ut_ProcessCqeErrInfo_003)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, ut_ip_addr_check)
{
    MOCKER(inet_ntop)
    .stubs()
...
TEST_F(HeartBeatTest, ut_GetPort)
{
    bool isUseRankPort = false;
    std::vector<u32> nicRanksPorts;
    std::vector<u32> vnicRanksPorts;
    bool devPortSwitchOn = false;
    Heartbeat::GetInstance(15).SetRankPortInfo(isUseRankPort, nicRanksPorts, vnicRanksPorts, devPortSwitchOn);

    HcclSocketType type = HcclSocketType::SOCKET_NIC;
    u32 remoteUserRank = 1;
    u32 remoteDeviceId = 1;
    u32 port = Heartbeat::GetInstance(15).GetPort(type, remoteUserRank, remoteDeviceId);
    EXPECT_EQ(port, HETEROG_CCL_PORT);
}
...
TEST_F(HeartBeatTest, ut_SaveOpInfo)
{
    ...
    opInfoList[0] = opInfo;
    Heartbeat::GetInstance(0).SaveOpInfo(opInfoList, uid_);
    opInfo.isValid = true;
    opInfoList[0] = opInfo;
    Heartbeat::GetInstance(0).SaveOpInfo(opInfoList, uid_);
    ...
    bool checkRes = false;
    checkRes = Heartbeat::GetInstance(0).CheckIsSameOp(opInfo, remoteopInfo);
    ...
    Heartbeat::GetInstance(0).SaveOpInfo(opInfoList, uid_);
    Heartbeat::GetInstance(0).GetOneOpInfo();
