...
TEST_F(LoadBackupIpTest, ut_topo_exchange_verify_backup_ip)
{
    ...
    HcclResult ret = agent.SetIsInterSuperPodRetryEnable(true);
    ret = agent.VerifyClusterBackupDeviceIP(rankTable);
...
#if 1
TEST_F(LoadBackupIpTest, ut_GetSingleHostPort)
{
    nlohmann::json rank_table =
    {
        {"status", "completed"},
    ...
    topoRanktable.GetSingleServer(serverListObj, 0, rankinfo0);
    GlobalMockObject::verify();
}
#endif

#if 1
TEST_F(LoadBackupIpTest, ut_GetSingleBackupDevicePort)
{
    nlohmann::json rank_table =
    {
        {"status", "completed"},
    ...
    topoRanktable.GetSingleBackupDevicePort(deviceList, 0, rankinfo0);
    GlobalMockObject::verify();
}
#endif
