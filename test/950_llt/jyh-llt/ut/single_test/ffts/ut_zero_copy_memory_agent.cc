...
HcclResult stub_ZeroCopyMemoryAgentUt_Send(hccl::HcclSocket * socket, const void *data, u64 size)
{
...
HcclResult stub_ZeroCopyMemoryAgentUt_IRecv(hccl::HcclSocket *socket, void *recvBuf, u32 recvBufLen, u64 &compSize)
{
    std::unique_lock<std::mutex> lock(stub_ZeroCopyMemoryAgentUt_mutex);
...
TEST_F(ZeroCopyMemoryAgentUt, ut_agent_test)
{
    ...
    MOCKER_CPP(&HcclSocket::Send, HcclResult(HcclSocket::*)(const void *, u64))
        .stubs()
        .with(any())
        .will(invoke(stub_ZeroCopyMemoryAgentUt_Send));
    MOCKER_CPP(&HcclSocket::IRecv).stubs().will(invoke(stub_ZeroCopyMemoryAgentUt_IRecv));
