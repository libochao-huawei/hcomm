...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_CollAllReduceDoubleRingConcurrentExecutor_Ring)
{
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_CollAllReduceDoubleRingConcurrentExecutor_Ring_Largesize)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    u64 count = 67108872;
    ...
    std::printf("[ut_CollAllReduceDoubleRingConcurrentExecutor_Ring_Largesize]");
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_CollAllReduceDoubleRingConcurrentExecutor_superpod_Ring)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_CollAllReduceDoubleRingConcurrentExecutor_Ring_ffts)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_CollAllReduceDoubleRingConcurrentExecutor_Ring_1server)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_CollReduceScatterDoubleRingConcurrentExecutor_Ring)
{
    ...
    SubCommInfo mockCommInfo1 {11, 2, std::vector<LINK>()};
    MOCKER_CPP(&CollNativeExecutorBase::GetSubCommInfo)
    .stubs()
    .will(returnValue(mockCommInfo1));
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_GetRingsOrderByTopoType_DoubleRing_1)
{
    HcclResult ret = HCCL_SUCCESS;
    ret = InitEnvVarParam();
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_GetRingsOrderByTopoType_DoubleRing_2)
{
    HcclResult ret = HCCL_SUCCESS;
    ret = InitEnvVarParam();
    ...
    MOCKER(CheckSdmaWithRohTopo)
    .stubs()
    .will(returnValue(false));    

    std::vector<std::vector<u32>> res = GetRingsOrderForAnyPath(rankSize, topoType, nicList);
    EXPECT_EQ(res.size(), 2);
    std::vector<u32> target0 = {0, 1, 2, 3, 4, 5, 6, 7};
    bool isSame = (res[0] == target0);
    EXPECT_EQ(isSame, true);
    std::vector<u32> target1 = {0, 7, 6, 5, 4, 3, 2, 1};
    isSame = (res[1] == target1);
    EXPECT_EQ(isSame, true);
}

TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_GetRingsOrderByTopoType_SingleRing)
{
    HcclResult ret = HCCL_SUCCESS;
    ret = InitEnvVarParam();
...
TEST_F(HcclImplAlgTestDoubleRingConcurrent, ut_env_test)
{
    HcclResult ret = HCCL_SUCCESS;
