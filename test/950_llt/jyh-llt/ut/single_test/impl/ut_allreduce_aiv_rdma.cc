...
class AllReduceAivRdmaTest : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        DlRaFunction::GetInstance().DlRaFunctionInit();  
        std::cout << "AllReduceAivRdmaTest SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "AllReduceAivRdmaTest TearDown" << std::endl;
    }
...
TEST_F(AllReduceAivRdmaTest, ut_allreduce_aiv_rdma_mid_count)
{
    ...
    CollAllReduceMidCountAivRdmaExecutor * executor = new CollAllReduceMidCountAivRdmaExecutor(impl->dispatcher_, topoMatcher);
...
TEST_F(AllReduceAivRdmaTest, ut_allreduce_aiv_rdma_small_count)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclCommParams params;
    RankTable_t rankTable;
    ...
    implBase->InitCCLbuffer(200*1024*1024, 200*1024*1024);
    std::unique_ptr<TopoMatcher> &topoMatcher = implBase->implAlg_->topoMatcher_;
    CollAllReduceSmallCountAivRdmaExecutor * executor = new CollAllReduceSmallCountAivRdmaExecutor(impl->dispatcher_, topoMatcher);
    executor->workflowMode_ = HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE;
    ...
    MOCKER_CPP(&CollAllReduceSmallCountAivRdmaExecutor::InterServerHDOneshot)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
