...
TEST_F(ExternalInputTest, ut_external_input_env_variables_params)
{
    int timeout;
    HcclResult ret;


    timeout = GetExternalInputHcclLinkTimeOut();
    HCCL_INFO("the timeout is %d", timeout);    
    timeout == 120 ? ret = HCCL_SUCCESS : ret = HCCL_E_PARA;
    EXPECT_EQ(ret, HCCL_SUCCESS);    

    setenv("HCCL_CONNECT_TIMEOUT", "1800", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    
    timeout = GetExternalInputHcclLinkTimeOut();
    HCCL_INFO("the timeout is %d", timeout);    
    timeout == 1800 ? ret = HCCL_SUCCESS : ret = HCCL_E_PARA;   
    EXPECT_EQ(ret, HCCL_SUCCESS);

    setenv("HCCL_CONNECT_TIMEOUT", "0xffffffff", 1);
    ret = InitEnvVarParam();
    ...
    setenv("HCCL_CONNECT_TIMEOUT", "0x5a5a", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
    ...
    setenv("HCCL_CONNECT_TIMEOUT", "this is a test", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
    ...
    setenv("HCCL_CONNECT_TIMEOUT", "-2555", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
    ...
    setenv("HCCL_CONNECT_TIMEOUT", "3600", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_SUCCESS);

    timeout = GetExternalInputHcclLinkTimeOut();
    HCCL_INFO("the timeout is %d", timeout);    
    timeout == 3600 ? ret = HCCL_SUCCESS : ret = HCCL_E_PARA;  
    EXPECT_EQ(ret, HCCL_SUCCESS);

    setenv("HCCL_CONNECT_TIMEOUT", "4294967295", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
    ...
    setenv("HCCL_CONNECT_TIMEOUT", "0", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
...
TEST_F(ExternalInputTest, ut_external_input_env_variables_params_intra_comm_type)
{
    ...
    intraRoce = GetExternalInputIntraRoceSwitch();
    HCCL_INFO("the intraRoce is %u", intraPcie, intraRoce);
    intraRoce == 0 ? ret = HCCL_SUCCESS : ret = HCCL_E_PARA;
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
#if 1
TEST_F(ExternalInputTest, ut_ParseCannVersion)
{
...
TEST_F(ExternalInputTest, ut_external_hccl_qps_per_connection)
{
    u32 qpsPerConnection = 1;
    HcclResult ret;
    ResetInitState();

    setenv("HCCL_RDMA_QPS_PER_CONNECTION", "32", 1);
 
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    qpsPerConnection = GetExternalInputQpsPerConnection();
    EXPECT_EQ(qpsPerConnection, 32);

    setenv("HCCL_RDMA_QPS_PER_CONNECTION", "33", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
    qpsPerConnection = GetExternalInputQpsPerConnection();
    EXPECT_EQ(qpsPerConnection, 1);

    unsetenv("HCCL_RDMA_QPS_PER_CONNECTION");
    ResetInitState();
}

TEST_F(ExternalInputTest, ut_external_hccl_multi_qp_threshold)
{
    u32 multiQpThreshold = 1;
    HcclResult ret;
    ResetInitState();

    setenv("HCCL_MULTI_QP_THRESHOLD", "1024", 1);
 
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    multiQpThreshold = GetExternalInputMultiQpThreshold();
    EXPECT_EQ(multiQpThreshold, 1024);

    setenv("HCCL_MULTI_QP_THRESHOLD", "10000", 1);
    ret = InitEnvVarParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
    multiQpThreshold = GetExternalInputMultiQpThreshold();
    EXPECT_EQ(multiQpThreshold, HCCL_MULTI_QP_THRESHOLD_DEFAULT);
...
TEST_F(ExternalInputTest, ut_external_input_env_HCCL_DFS_CONFIG)
{
    HcclResult ret = HCCL_SUCCESS;

    // 配置为无效值

    setenv("HCCL_DFS_CONFIG", "", 1);
    ret = InitEnvParam();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    setenv("HCCL_DFS_CONFIG", "connection_fault_detection_time:0", 0);
    ret = InitEnvParam();
    EXPECT_EQ(ret, HCCL_SUCCESS);

    setenv("HCCL_DFS_CONFIG", "connection_fault_detection_time:7300", 1);
    ret = InitEnvParam();
    EXPECT_EQ(ret, HCCL_E_PARA);
    unsetenv("HCCL_DFS_CONFIG");
}
