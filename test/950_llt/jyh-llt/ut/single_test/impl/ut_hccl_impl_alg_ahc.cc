...
class HcclImplAlgTestAHC : public testing::Test {
public:
...
protected:
    static void SetUpTestCase()
    {
        ...
        std::cout << "HcclImplAlgTestAHC SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        ...
        std::cout << "HcclImplAlgTestAHC TearDown" << std::endl;
    }
...
HcclDispatcher HcclImplAlgTestAHC::dispatcherPtr = nullptr;
DispatcherPub *HcclImplAlgTestAHC::dispatcher = nullptr;
...
HcclResult FakeRunTemplateCoprimeCase(const std::unique_ptr<AlgTemplateBase> &tempAlg, const SubCommInfo &commInfo)
{
    ...
    for (int i = 0; i < localRankSize; i++)
    {
        links[i].reset(new(std::nothrow) Transport(new (std::nothrow) TransportBase(
            HcclImplAlgTestAHC::dispatcher, nullptr, machinePara, timeout)));
        links[i]->Init();
    }
...
static inline void ConstructCommTestCase91093(AHCEnvType ahcEnvType, AHCCommType ahcCommType)
{
    ...
    if (ahcEnvType == AHCEnvType::AHC) {
        ...
        MOCKER(ParserHcclAlgoLevel)
        .stubs()
        .will(invoke(FakeParserHcclAlgoLevel));
    }
    ResetInitState();
    ret = InitExternalInput();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    MOCKER_CPP_VIRTUAL(*HcclImplAlgTestAHC::dispatcher, &DispatcherPub::SignalRecord, HcclResult(DispatcherPub::*)(HcclRtNotify, hccl::Stream &, u32, u64,
        s32, bool, u64, u32)).stubs().will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP_VIRTUAL(*HcclImplAlgTestAHC::dispatcher, &DispatcherPub::SignalWait, HcclResult(DispatcherPub::*)(HcclRtNotify, hccl::Stream &, u32, u32,
...
// 910B AHC-Broke 执行流程
TEST_F(HcclImplAlgTestAHC, ut_AllReduceAHCBrokeExecute910B)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    u64 count = 1024;
...
// 91093 AHC 执行流程
TEST_F(HcclImplAlgTestAHC, ut_AllReduceAHCExecute91093)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(HcclImplAlgTestAHC, ut_AllReduceAHCBrokeExecute91093)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
    ...
    impl->topoAttr_.deviceLogicId = 0;
    impl->topoAttr_.devicePhyId = 0;
    algConfigurator->algType_[HcclCMDType::HCCL_CMD_ALLREDUCE].algoLevel0 = AlgTypeLevel0::ALG_LEVEL0_NP_SINGLE_RING;
...
// 91093 null-AHC 执行流程
TEST_F(HcclImplAlgTestAHC, ut_AllReduceNULLAHCExecute91093)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(HcclImplAlgTestAHC, ut_AICPU_AHC_Default_91093)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(HcclImplAlgTestAHC, ut_AllGatherAHCExecuteSingleBuffer910B)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    u64 count = 1024;
...
TEST_F(HcclImplAlgTestAHC, ut_AllGatherAHCBrokeExecuteSingleBuffer910B)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    u64 count = 1024;
...
TEST_F(HcclImplAlgTestAHC, ut_AllGatherAHCExecuteDoubleBuffer910B)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    u64 count = 1024;
...
TEST_F(HcclImplAlgTestAHC, ut_AllGatherAHCBrokeExecuteDoubleBuffer910B)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    u64 count = 1024;
...
TEST_F(HcclImplAlgTestAHC, ut_ReduceScatterAHCExecuteSingleBuffer910B)
{
    ...
    resourceResponse.cclInputMem = inputMem;
    resourceResponse.cclOutputMem = outputMem;
    executor->inCCLbufferSize_ = 6553500;
    ret = executor->Orchestrate(opParam, resourceResponse);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(HcclImplAlgTestAHC, ut_ReduceScatterAHCBrokeExecuteSingleBuffer910B)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    MOCKER(&HcclD2DMemcpyAsync)
        .stubs()
...
TEST_F(HcclImplAlgTestAHC, ut_ReduceScatterAHCExecuteDoubleBuffer910B)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    MOCKER(&HcclD2DMemcpyAsync)
        .stubs()
...
TEST_F(HcclImplAlgTestAHC, ut_ReduceScatterAHCBrokeExecuteDoubleBuffer910B)
{
    MOCKER(GetWorkflowMode).stubs().will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE));
    MOCKER(&HcclD2DMemcpyAsync)
        .stubs()
