...
static void TestConstructParam1(HcclCommParams &params, RankTable_t &rankTable)
{
    string commId = "comm ";
    memcpy_s(params.id.internal, HCCL_ROOT_INFO_BYTES, commId.c_str(), commId.length() + 1);
    params.rank = 0;
...
TEST_F(HcclImplAlgTestDoubleRing, ut_CollBroadcastRingFor91093Executor_NHR)
{
    ...
    TestConstructParam1(params, rankTable);
    params.deviceType = DevType::DEV_TYPE_910_93;
    ...
    std::printf("[ut_CollBroadcastRingFor91093Executor_NHR]");
...
TEST_F(HcclImplAlgTestDoubleRing, ut_CollAllReduceRingFor91093Executor_NHR)
{
    HcclResult ret = HCCL_SUCCESS;
    setenv("HCCL_ALGO", "level0:NA;level1:NHR", 1);
    ret = InitEnvVarParam();
    ...
    std::printf("[ut_CollAllReduceRingFor91093Level2Executor_NHR]");
...
TEST_F(HcclImplAlgTestDoubleRing, ut_CollAllReduceRingFor91093Level2Executor_Ring)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
    ...
    std::printf("[ut_CollAllReduceRingFor91093Level2Executor_Ring]");
    ret = implBase->AllReduce(tag, inputMem.ptr(), outputMem.ptr(), count, dataType, op, stream.ptr());
    implBase = nullptr;
...
TEST_F(HcclImplAlgTestDoubleRing, ut_CollReduceRingFor91093Executor_Ring)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
    ...
    std::printf("[ut_CollReduceRingFor91093Executor_Ring]");
...
TEST_F(HcclImplAlgTestDoubleRing, ut_CollReduceRingFor91093Executor_Ring_level2)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
    ...
    std::printf("[ut_CollReduceRingFor91093Executor_Ring_level2]");
    ret = implBase->Reduce(tag, inputMem.ptr(), outputMem.ptr(), count, dataType, op, 0, stream.ptr());
    implBase = nullptr;
...
TEST_F(HcclImplAlgTestDoubleRing, ut_CollReduceScatterDoubleRingExecutor_UpdateOffsetBasedOnStrideCount)
{
    ...
    HcomCollOpInfo opInfo = {
        "", opParam.inputPtr, opParam.outputPtr, opParam.DataDes.count, opParam.DataDes.dataType, 0, HCCL_REDUCE_RESERVED,
        opParam.DataDes.strideCount
    };
    HcomCollOpInfo *opInfoPtr = nullptr;
    opInfoPtr = &opInfo;
    std::vector<std::vector<u32>> multiRingsOrder;
    multiRingsOrder.push_back({0, 1, 2, 3});
    multiRingsOrder.push_back({0, 3, 2, 1});
    std::vector<Slice> userMemSlices;
    ret = executor->CalUserMemSlices(opParam.DataDes.dataType, opInfoPtr,
        firstRingsUserMemSlice, 0, multiRingsOrder, userMemSlices);
    EXPECT_EQ(ret, HCCL_SUCCESS);
