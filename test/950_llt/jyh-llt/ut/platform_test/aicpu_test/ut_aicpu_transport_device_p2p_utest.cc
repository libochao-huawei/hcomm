...
protected:
    ...
    virtual void SetUp()
    {
        ...
        u32 notifyNum = 8;
        transDevP2pData.userLocalNotify.resize(notifyNum);
        transDevP2pData.userRemoteNotify.resize(notifyNum);
        for (u32 i=0; i < notifyNum; i++) {
            transDevP2pData.userLocalNotify[i] = std::make_shared<LocalIpcNotify>();
            transDevP2pData.userLocalNotify[i]->Init(notifyInfo, NotifyLoadType::DEVICE_NOTIFY);
            transDevP2pData.userRemoteNotify[i] = std::make_shared<RemoteNotify>();
            transDevP2pData.userRemoteNotify[i]->Init(notifyInfo, NotifyLoadType::DEVICE_NOTIFY);
        }
