...
TEST_F(DispatcherAiCpu_UT, ut_DispatcherAiCpuFailed)
{
    HcclDispatcher dispatcher = nullptr;
    DispatcherAiCpu dispatcherAicpu(0);
    MOCKER_CPP_VIRTUAL(dispatcherAicpu, &DispatcherAiCpu::Init)
    .stubs()
    .with(any())
    .will(returnValue(HCCL_E_PARA));
    HcclResult ret = HcclDispatcherAicpuInit(&dispatcher, 0, DispatcherType::DISPATCHER_AICPU);
    EXPECT_EQ(HCCL_E_PARA, ret);
    EXPECT_EQ(dispatcher, nullptr);
    GlobalMockObject::verify();
}

TEST_F(DispatcherAiCpu_UT, aicpu_prof_taskInfoReport)
{
    MOCKER(dfx::ProfilingManager::IsProfOn).stubs().will(returnValue(true));
...
TEST_F(DispatcherAiCpu_UT, ut_DispatcherAiCpu_StreamSync)
{
    ...
    SqCqeContext sqeCqeCtx1;
    sqeCqeCtx1.sqContext.inited = false;
    ret = stream1.InitSqAndCqeContext(sqHead, sqTail, &sqeCqeCtx1);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
TEST_F(DispatcherAiCpu_UT, TaskInfo2Addition_ShouldSetCorrectFields_WhenMemcpySucceeds) {
    // Arrange
...
TEST_F(DispatcherAiCpu_UT, TaskInfo2Addition_ShouldReturnError_WhenMemcpyFails) {
    // Arrange
...
TEST_F(DispatcherAiCpu_UT, ut_data_plane_interface_aicpu)
{
    ...
    std::string mainStr = mainThread.GetUniqueId();
    std::string subStr = subThread.GetUniqueId();
    GlobalMockObject::verify();
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = CommLocalRecord(&mainDevThread, &subDevThread, 0);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = CommLocalWait(&subDevThread, 0, 1);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = CommLocalRecord(&subDevThread, &mainDevThread, 1);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = CommLocalWait(&mainDevThread, 1, 1);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    ThreadHandle threads[2];
    threads[0] = (ThreadHandle)&mainDevThread;
    threads[1] = (ThreadHandle)&subDevThread;
    ret = CommTaskLaunch(threads, 2);
    EXPECT_EQ(ret, HCCL_SUCCESS);
