...
class MPI_ReduceScatterAivRdma_910B_Test : public testing::Test
{
protected:
    static void SetUpTestCase()
    {
        std::cout << "MPI_ReduceScatterAivRdma_910B_Test SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "MPI_ReduceScatterAivRdma_910B_Test TearDown" << std::endl;
    }
    ...
    virtual void SetUp()
    {
        ...
        std::cout << "A MPI_ReduceScatterAivRdma_910B_Test SetUP" << std::endl;
    }
    virtual void TearDown()
    {
        MPI_Barrier(MPI_COMM_WORLD);
        std::cout << "A MPI_ReduceScatterAivRdma_910B_Test TearDown" << std::endl;
    }
};

#define GOUP_DEV_NUM 8
#define HCCL_REDUCE_SCATTER_DATA_SIZE 2
...
void* inter_aiv_rdma_reduce_scatter_task(void* parg)
{
    ...
    string strTag = "reduce_scatter_tag_magic996";
    ...
    hcom_info.pComm->communicator_->SetAivModeConfig(true);
    ret =  hcom_info.pComm->communicator_->ReduceScatterOutPlace(strTag,
                               para_info->sendbuff,
                               para_info->recvbuff,
                               para_info->count,
                               para_info->datatype,
...
TEST_F(MPI_ReduceScatterAivRdma_910B_Test, ut_mpi_aiv_rdma_reduce_scatter_4ranks_2server)
{
    ...
    char file_name_t[] = "./ut_mpi_aiv_rdma_reduce_scatter_4ranks_2server.json";
    ...
    s32 count = HCCL_REDUCE_SCATTER_DATA_SIZE;
    ...
    for (s32 i = 0; i < ndev; i++) {
        ...
        EXPECT_EQ(ret, HCCL_SUCCESS);
        sal_memset(sendbuf[i], count * sizeof(s8) * ndev, 0, count * sizeof(s8) * ndev);
    ...
    for (s32 i = 0; i < ndev; i++) {
        tid[i] = sal_thread_create("thread", inter_aiv_rdma_reduce_scatter_task, (void*)&para_info[i]);
...
TEST_F(MPI_ReduceScatterAivRdma_910B_Test, ut_mpi_aiv_rdma_reduce_scatter_4ranks_2server_prod)
{
    setenv("HCCL_OP_EXPANSION_MODE", "AIV", 1);
    ResetInitState();
    InitExternalInput();
    ...
    nlohmann::json rank_table = rank_table_910_2server_4rank;
    char file_name_t[] = "./ut_mpi_aiv_rdma_reduce_scatter_4ranks_2server_prod.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
