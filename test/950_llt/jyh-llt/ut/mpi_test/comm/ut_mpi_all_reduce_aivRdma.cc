...
class MPI_AllReduceAivRdma_910B_Test : public testing::Test
{
protected:
    static void SetUpTestCase()
    {
        std::cout << "MPI_AllReduceAivRdma_910B_Test SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "MPI_AllReduceAivRdma_910B_Test TearDown" << std::endl;
    }
    ...
    virtual void SetUp()
    {
        ...
        MOCKER_CPP(&MulQpInfo::Init).stubs().will(returnValue(HCCL_SUCCESS));
        bool isEnableMulQp = false;
        MOCKER_CPP(&MulQpInfo::IsEnableMulQp).stubs().with(outBound(isEnableMulQp)).will(returnValue(HCCL_SUCCESS));
        MOCKER_CPP(&MulQpInfo::GetPortsNumByIpPair).stubs().will(returnValue(HCCL_SUCCESS));
        MOCKER_CPP(&MulQpInfo::GetMulQpFromType).stubs().will(returnValue(HCCL_SUCCESS));
        std::cout << "A MPI_AllReduceAivRdma_910B_Test SetUP" << std::endl;
    }
    virtual void TearDown()
    {
        MPI_Barrier(MPI_COMM_WORLD);
        std::cout << "A MPI_AllReduceAivRdma_910B_Test TearDown" << std::endl;
    }
};

#define GOUP_DEV_NUM 8
#define HCCL_ALLREDUCE_DATA_SIZE 1024
#define HCCL_ALLREDUCE_MID_DATA_SIZE 4194304
...
#if 1
void* inter_aiv_rdma_all_reduce_task(void* parg)
{
    ...
    hcom_info.pComm.reset(new(std::nothrow) hccl::hcclComm(1024*1024*16, 1024*1024*16, HCCL_WORLD_GROUP));
    ...
    {
        HCCL_ERROR("rank[%d] task allreduce falis", hcom_info.params.rank);
    }
 
    return (nullptr);
}

TEST_F(MPI_AllReduceAivRdma_910B_Test, ut_mpi_aiv_rdma_small_all_reduce_4ranks_2server)
{
    s32 portNum = 7;
    MOCKER(hrtGetHccsPortNum)
        .stubs()
    ...
    char file_name_t[] = "./ut_mpi_aiv_rdma_small_all_reduce_4ranks_2server.json";
    ...
    for (s32 i = 0; i < ndev; i++) {
        tid[i] = sal_thread_create("thread", inter_aiv_rdma_all_reduce_task, (void*)&para_info[i]);
...
TEST_F(MPI_AllReduceAivRdma_910B_Test, ut_mpi_aiv_rdma_mid_all_reduce_4ranks_2server)
{
    s32 portNum = 7;
    MOCKER(hrtGetHccsPortNum)
        .stubs()
    ...
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_2server_4rank;
    char file_name_t[] = "./ut_mpi_aiv_rdma_mid_all_reduce_4ranks_2server.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
    ...
    HcclDataType datatype = HCCL_DATA_TYPE_INT8;
    s32 count = HCCL_ALLREDUCE_MID_DATA_SIZE;
    HCCL_ERROR("count : [%d]", count);
    s32 ndev = DEV_NUM_4;
