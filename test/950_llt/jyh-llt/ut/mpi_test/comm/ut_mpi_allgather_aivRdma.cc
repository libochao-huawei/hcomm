...
class MPI_AllGatherAivRdma_910B_Test : public testing::Test
{
protected:
    static void SetUpTestCase() {
        std::cout << "MPI_AllGatherAivRdma_910B_Test SetUP" << std::endl;
    }
    static void TearDownTestCase() {
        std::cout << "MPI_AllGatherAivRdma_910B_Test TearDown" << std::endl;
    }
    ...
    virtual void SetUp() {
        ...
        std::cout << "A MPI_AllGatherAivRdma_910B_Test SetUP" << std::endl;
    }
    virtual void TearDown() {
        MPI_Barrier(MPI_COMM_WORLD);
        std::cout << "A MPI_AllGatherAivRdma_910B_Test TearDown" << std::endl;
    }
};
...
void* inter_aiv_rdma_all_gather_task(void* parg)
{
    ...
    string strTag = "allgather_tag_magic996";
...
#if 1
TEST_F(MPI_AllGatherAivRdma_910B_Test, ut_mpi_aiv_rdma_allgather_4ranks_2server)
{
    nlohmann::json rank_table = rank_table_910_2server_4rank;
    char file_name_t[] = "./ut_mpi_aiv_rdma_allgather_4ranks_2server.json";
    ...
    for (s32 i = 0; i < ndev; i++) {
        tid[i] = sal_thread_create("thread", inter_aiv_rdma_all_gather_task, (void*)&para_info[i]);
    ...
    for (s32 i = 0; i < ndev; i++) {
        for (s32 j = 0; j < count; j++) {
            ...
            HCCL_ERROR("result_buff[%d][%d] = [%d], outputbuf[%d][%d] = [%d]", i, j, result_buff[i][j], i, j,
                outputbuf[i][j]);
