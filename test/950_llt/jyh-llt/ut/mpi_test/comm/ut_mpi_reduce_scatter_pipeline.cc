...
class MPI_ReduceScatterInter_910B_Test : public testing::Test
{
protected:
...
#if 1
void* inter_pipeline_reduce_scatter_task(void* parg)
{
    s32 portNum = 7;
    MOCKER(hrtGetHccsPortNum)
        .stubs()
    ...
    string strTag = "reduce_scatter_tag_magic9999997";
...
TEST_F(MPI_ReduceScatterInter_910B_Test, ut_mpi_pipeline_reduce_scatter_4ranks_2server)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_2server_4rank;
    char file_name_t[] = "./ut_mpi_pipeline_reduce_scatter_4ranks_2server.json";
    ...
    for (s32 i = 0; i < ndev; i++) {
        ...
        EXPECT_EQ(ret, HCCL_SUCCESS);
        sal_memset(sendbuf[i],count * sizeof(s8), 0, count * sizeof(s8) * ndev);
        ret = hrtMalloc((void **)&(recvbuf[i]), count * sizeof(s8));
        EXPECT_EQ(ret, HCCL_SUCCESS);
        sal_memset(recvbuf[i], count  * sizeof(s8) * ndev, 0,  count * sizeof(s8));
        ret = hrtMalloc((void **)&(result_buff[i]), count * sizeof(s8));
        EXPECT_EQ(ret, HCCL_SUCCESS);
        sal_memset(result_buff[i], count * sizeof(s8) * ndev, 0, count * sizeof(s8));
    ...
    for (s32 i = 0; i < ndev; i++) {
        tid[i] = sal_thread_create("thread", inter_pipeline_reduce_scatter_task, (void*)&para_info[i]);
...
TEST_F(MPI_ReduceScatterInter_910B_Test, ut_mpi_pipeline_reduce_scatter_4ranks_2server_prod)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_2server_4rank;
    char file_name_t[] = "./ut_mpi_pipeline_reduce_scatter_4ranks_2server_prod.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
