...
class MPI_UT_SAL_Test : public testing::Test
{
protected:
    static void SetUpTestCase()
    {
        std::cout << "MPI_UT_SAL_Test SetUP" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "MPI_UT_SAL_Test TearDown" << std::endl;
    }
...
#if 1
/* MPI机制测试用例 */
TEST_F(MPI_UT_SAL_Test, ut_mpi_sal_get_unique_id_test_01)
{
    s32 size, rank;
...
TEST_F(MPI_UT_SAL_Test, ut_mpi_ipc_memory_set_open_close_1)
{
    s32 ndev, rank;
    s32 err_cnt;
    void* addr;
    ...
    if (0 == rank)
    {
        ...
        h_ret = hrtIpcSetMemoryName(addr, reinterpret_cast<u8 *>(rootInfo.internal), size,sizeof(rootInfo.internal));
        EXPECT_EQ(h_ret, HCCL_SUCCESS);

        h_ret = hrtIpcSetMemoryPid(reinterpret_cast<u8 *>(rootInfo.internal), &pid, 1);
        EXPECT_EQ(h_ret, HCCL_SUCCESS);
    }
    ...
    if (0 != rank)
    {        
        h_ret = hrtIpcOpenMemory(&addr, (u8*)rootInfo.internal);
        EXPECT_EQ(h_ret, HCCL_SUCCESS);
...
void* rt_ipc_open_read(void* para)
{
    ...
    ret = hrtIpcCloseMemory(addr);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    return (NULL);
}

TEST_F(MPI_UT_SAL_Test, ut_mpi_ipc_memory_set_open_close_4)
{
    s32 ndev, rank;
    s32 err_cnt;
    ...
    if (0 != rank)
    {
        ...
        for (int i= 0; i < mem_num; i++) {
            hrtIpcCloseMemory((void*)new_addr[i]);
        }
    }
    if (rank == 0) {
        /* 等待其他进程读取完成 */
