...
HcclConn g_acceptConn = nullptr;

// pass
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclRawOpen_test)
{
...
// pass
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclRawBind_test)
{
    HcclAddr bindAddr;
...
static HcclRequestInfo g_Request;
HcclMessageInfo g_message;

// pass
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclRawGetCount_test)
{
    HcclStatus status;
    int count;
...
// pass
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclRegisterGlobalMemory_test)
{
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclCommConn_InitMemBlocksAndRecvWrMem_test)
{
...
#if 1
TEST_F(MPI_RAW_SENDRECV_TEST, ut_17d1p_test)
{
    HcclAddr acceptAddr;

    SocketInfoT tmp;
    tmp.status = CONNECT_OK;
    tmp.fdHandle = nullptr;
    ...
    MOCKER_CPP(&HcclCommConnMgr::IsExceedMaxLinkNum)
    .stubs()
    .will(returnValue(true));

    MOCKER_CPP(&HcclCommConn::SocketForceClose)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));

    HcclResult ret = HcclRawAccept(g_connPtr, &acceptAddr, &g_acceptConn);
    EXPECT_EQ(ret, HCCL_E_AGAIN);
    GlobalMockObject::verify();
}

TEST_F(MPI_RAW_SENDRECV_TEST, ut_513d1p_test)
{
    ...
    for (int i = 0; i < 16; i++) {
        conn.connHandleQueue_.push(acceptComConn);
    }
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_accept_error_test)
{
    MOCKER_CPP(&HcclCommConn::GetSocket)
    .stubs()
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_InitTransport_error_test)
{
    HcclCommConn::AcceptCommConn acceptComConn;
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_getsocket_error_test)
{
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_1d513p_test)
{
    HcclResult ret = HCCL_SUCCESS;
    HcclAddr connectAddr;
    connectAddr.type=HCCL_ADDR_TYPE_ROCE;//RDMA
    connectAddr.info.tcp.ipv4Addr = 2130706433;
    connectAddr.info.tcp.port = 60999;

    MOCKER_CPP(&HcclCommConn::Connect)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
    ...
    ret = HcclRawConnect(g_connPtr, &connectAddr);
    HCCL_ERROR("ret = %d", ret);
...
// pass
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclRawClose_test)
{
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_UninitRa_test)
{
    MOCKER(HrtRaDeInit)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclCommConn_connect_failed)
{
    ...
    MOCKER(RaSocketBatchAbort).stubs()
        .will(returnValue(0))
...
TEST_F(MPI_RAW_SENDRECV_TEST, ut_HcclCommConn_ResetConnection)
{
    HcclCommConn conn;
...
#if 1
TEST_F(MPI_RAW_SENDRECV_TEST, ut_pd_force_and_concurrency_link01)
{
