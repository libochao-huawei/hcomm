...
HcclResult Work(s32 deviceId, std::string ip, std::string dstIp, HcclResult &ret)
{
    ...
    std::shared_ptr<LocalIpcRmaBuffer> localRmaBuffer2 =
        std::make_shared<LocalIpcRmaBuffer>(netDevCtx, ptr, size, RmaMemType::HOST);
    ret = localRmaBuffer2->Init();
    EXPECT_EQ(ret, HCCL_SUCCESS);

    std::shared_ptr<LocalIpcRmaBuffer> localRmaBufferDev2 =
        std::make_shared<LocalIpcRmaBuffer>(netDevCtx, devicePtr.ptr(), size);
    ret = localRmaBufferDev2->Init();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    std::shared_ptr<RemoteIpcRmaBuffer> remoteRmaBuffer2 = std::make_shared<RemoteIpcRmaBuffer>(netDevCtx);
    std::shared_ptr<RemoteIpcRmaBuffer> remoteDevRmaBuffer2 = std::make_shared<RemoteIpcRmaBuffer>(netDevCtx);
#if 1
    if (deviceId == 0) {
        ...
        std::string data2 = localRmaBuffer2->Serialize();
        int length2 = 256 - data2.length();
        std::string padd2(length2, '0');
        data2 += padd2;
        ret = clientSocket.Send(&data2[0], data2.size());
        EXPECT_EQ(ret, HCCL_SUCCESS);

        std::string devData2 = localRmaBufferDev2->Serialize();
        length2 = 256 - devData2.length();
        std::string padd1_2(length2, '0');
        devData2 += padd1_2;
        ret = clientSocket.Send(&devData2[0], devData2.size());
        EXPECT_EQ(ret, HCCL_SUCCESS);

        printf("dev[%d]:host rdma ptr[%p], size[%llu], RmaType[%d], devPtr[%p], memType[%d], key[%u]\n", deviceId,
        ...
        printf("dev[%d]:device rdma ptr[%p], size[%llu], RmaType[%d], devPtr[%p], memType[%d], key[%u]\n", deviceId,
        ...
        printf("dev[%d]:host ipc ptr[%p], size[%llu], RmaType[%d], devPtr[%p], memType[%d]\n", deviceId,
            localRmaBuffer2->GetAddr(), localRmaBuffer2->GetSize(), localRmaBuffer2->GetRmaType(),
            localRmaBuffer2->GetDevAddr(), localRmaBuffer2->GetMemType());
        printf("dev[%d]:device ipc ptr[%p], size[%llu], RmaType[%d], devPtr[%p], memType[%d]\n", deviceId,
            localRmaBufferDev2->GetAddr(), localRmaBufferDev2->GetSize(), localRmaBufferDev2->GetRmaType(),
            localRmaBufferDev2->GetDevAddr(), localRmaBufferDev2->GetMemType());
    } else {
        ...
        std::string data2(256, '\0');
        ret = connectSocket->Recv(&data2[0], data2.size());
        EXPECT_EQ(ret, HCCL_SUCCESS);
        ret = remoteRmaBuffer2->Deserialize(data2);
        EXPECT_EQ(ret, HCCL_SUCCESS);

        std::string devData2(256, '\0');
        ret = connectSocket->Recv(&devData2[0], devData2.size());
        EXPECT_EQ(ret, HCCL_SUCCESS);
        ret = remoteDevRmaBuffer2->Deserialize(devData2);
        EXPECT_EQ(ret, HCCL_SUCCESS);
        ...
        printf("dev[%d]:host ipc ptr[%p], size[%llu], RmaType[%d], devPtr[%p], memType[%d]\n", deviceId,
            remoteRmaBuffer2->GetAddr(), remoteRmaBuffer2->GetSize(), remoteRmaBuffer2->GetRmaType(),
            remoteRmaBuffer2->GetDevAddr(), remoteRmaBuffer2->GetMemType());
        printf("dev[%d]:device ipc ptr[%p], size[%llu], RmaType[%d], devPtr[%p], memType[%d]\n", deviceId,
            remoteDevRmaBuffer2->GetAddr(), remoteDevRmaBuffer2->GetSize(), remoteDevRmaBuffer2->GetRmaType(),
            remoteDevRmaBuffer2->GetDevAddr(), remoteDevRmaBuffer2->GetMemType());
    }
    if (deviceId == 0) {
        std::string messageToSend = "hello, hcclsocket";
        ret = clientSocket.Send(messageToSend);
        EXPECT_EQ(ret, HCCL_SUCCESS);
    } else {
        std::string receivedMessage;
        ret = connectSocket->Recv(receivedMessage);
        EXPECT_EQ(ret, HCCL_SUCCESS);
    }
#endif
    ret = rt_stream_synchronize(stream.ptr());
    ...
    remoteRmaBuffer2 = nullptr;
    remoteDevRmaBuffer2 = nullptr;
