...
TEST_F(TransportShmEventTest, ut_TransportShmEvent_ExchangeIpcMesg)
{
    ...
    transport1.deviceLogicId_ = HOST_DEVICE_ID;
...
#if 1
TEST_F(TransportShmEventTest, ut_TransportShmEvent_ShmMemcpy)
{
    MachinePara machine_para;
    std::chrono::milliseconds timeout = std::chrono::milliseconds(10000);
    HcclIpAddress invaildIp;
    ...
    ret = transport.ShmMemcpy(dst, offset, src, count);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    u32 size = 2;
    ret = transport.RemoteEnvelopeQueSize(size);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    ret = transport.EnvelopeQueSize(size);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    TransportShmEventMember trans;
    ret = transport.GetTransportMember(trans);
    EXPECT_EQ(ret, HCCL_SUCCESS);

    ret = transport.PopEnvelope();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    pcie.tableId = 2;
    ret = transport.FrontEnvelope(pcie);
    EXPECT_EQ(ret, HCCL_E_INTERNAL);

    ret = transport.PushEnvelope(pcie);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ConnState nextState;
    ret = transport.EnterStateProcess(nextState);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
#endif
#if 1
TEST_F(TransportShmEventTest, ut_TransportShmEvent_Improbe)
{
    MachinePara machine_para;
    std::chrono::milliseconds timeout = std::chrono::milliseconds(10000);
    HcclIpAddress invaildIp;
    ...
    MOCKER_CPP(&LocalIpcNotify::Wait)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));

    ret = transport.Improbe(epParam, matched, msg, status);
    EXPECT_EQ(ret, HCCL_E_INTERNAL);
    GlobalMockObject::verify();
}
#endif
