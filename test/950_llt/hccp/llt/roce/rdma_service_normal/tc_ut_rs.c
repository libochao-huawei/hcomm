...
void tc_rs_socket_connect()
{
    ...
    white_list.connLimit = 1;
	int try_num = 10;
	//white_list[1].remoteIp.addr.s_addr = inet_addr("127.0.0.1");
    //white_list[1].connLimit = 1;
...
extern int RsCheckDstInterface(unsigned int phyId, char *ifa_name, enum RsHardwareType type, bool is_all);
...
int stub_getifaddrs(struct ifaddrs **ifap)
{
	*ifap = malloc(sizeof(struct ifaddrs));
	...
	(*ifap)->ifa_next = NULL;  // 这是链表的最后一个节点
	(*ifap)->ifa_name = "eth0";
