...
extern "C" int llt_testcase_main(int argc, char **argv)
{
...
#define TC_FAIL 0
extern "C" void llt_testcase_expect_result(int result)
{
	EXPECT_NE(TC_FAIL, result);
}
extern "C" void llt_testcase_assert_result(int result)
{
	ASSERT_NE(TC_FAIL, result);
}

extern "C" int llt_testcase_log(const char *szFormat, ...)
{
	int ret;
	va_list ap;
