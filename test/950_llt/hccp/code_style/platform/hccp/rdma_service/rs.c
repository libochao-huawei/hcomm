...
STATIC int RsInitRscbCfg(struct rs_cb *rscb)
{
    ...
    CHK_PRT_RETURN(ret != 0, hccp_err("RsGetChipProtocol failed, ret[%d]", ret), ret);
    ret = RsCtxApiInit(rscb->hccpMode, rscb->protocol);
    CHK_PRT_RETURN(ret != 0, hccp_err("RsCtxApiInit failed, ret[%d]", ret), ret);
    ret = RsEschedInit(rscb);
    if (ret != 0) {
        hccp_err("RsEschedInit chipId[%u] logic_devid[%u] failed, ret=%d", rscb->chipId, rscb->logicId, ret);
...
STATIC void RsFreeUdevList(struct rs_cb *rsCb)
{
    ...
    for (; (&udevCbCurr->list) != &rsCb->rdevList;
        ...
        if (ret != 0) {
            hccp_err("RsUbCtxDeinit failed, ret:%d", ret);
        }
    }
