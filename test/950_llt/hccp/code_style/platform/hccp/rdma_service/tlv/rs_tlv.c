...
STATIC int RsCcuRequest(struct TlvRequestMsgHead *head, char *data)
{
    ...
    switch (head->type) {
        case MSG_TYPE_CCU_INIT:
            ret = RsCcuInit();
            CHK_PRT_RETURN(ret != 0, hccp_err("RsCcuInit failed, ret(%d) module_type(%u) msg_type(%u) phyId(%u)",
                ret, head->moduleType, head->type, head->phyId), ret);
            break;
        case MSG_TYPE_CCU_UNINIT:
            ret = RsCcuUninit();
            CHK_PRT_RETURN(ret != 0, hccp_err("RsCcuUninit failed, ret(%d) module_type(%u) msg_type(%u) phyId(%u)",
                ret, head->moduleType, head->type, head->phyId), ret);
            break;
        default:
