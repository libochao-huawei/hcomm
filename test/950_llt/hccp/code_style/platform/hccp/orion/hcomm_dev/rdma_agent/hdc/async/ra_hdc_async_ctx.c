...
void RaHdcAsyncHandleGetEidByIp(struct RaRequestHandle *reqHandle)
{
    ...
    if (ret != 0) {
        hccp_err("[get][eid_by_ip]RaHdcGetEidResults failed ret[%d], phyId[%u] devIndex[0x%x]", ret,
...
void RaHdcAsyncHandleTpInfoList(struct RaRequestHandle *reqHandle)
{
    ...
    ret = memcpy_s(asyncRsp->infoList, (*asyncRsp->num) * sizeof(struct TpInfo),
        asyncData->rxData.infoList, asyncData->rxData.num * sizeof(struct TpInfo));
    if (ret != 0) {
        hccp_err("[get][ra_hdc_tp_info]memcpy_s tp_info failed ret[%d] *async_rsp->num[%u] rxData.num[%u], "
