...
STATIC int RsUbFillDevEidInfoList(struct DevEidInfo *totalList, unsigned int index, urma_device_t *device,
...
STATIC int RsUbFillInfoByEidList(urma_device_t *currDev, unsigned int *index, struct DevEidInfo *totalList,
...
int RsUbGetDevEidInfoList(unsigned int phyId, struct DevEidInfo infoList[], unsigned int startIndex,
    unsigned int count)
{
    struct DevEidInfo *totalList = NULL;
    urma_device_t **devList = NULL;
    unsigned int totalNum = 0;
    ...
    totalList = calloc(totalNum, sizeof(struct DevEidInfo));
    if (totalList == NULL) {
    ...
    (void)memcpy_s(infoList, sizeof(struct DevEidInfo) * count,
        totalList + startIndex, sizeof(struct DevEidInfo) * count);
...
STATIC int RsUbDeleteJfc(struct RsUbDevCb *devCb, struct RsCtxJfcCb *jfcCb)
{
    urma_user_ctl_out_t out = {0};
    urma_user_ctl_in_t in = {0};
    int outBuff = 0;
...
STATIC int RsUbFreeJfcCb(struct RsUbDevCb *devCb, struct RsCtxJfcCb *jfcCb)
{
    ...
    if (jfcCb->jfcType == JFC_MODE_STARS_POLL || jfcCb->jfcType == JFC_MODE_CCU_POLL) {
        (void)RsUbDeleteJfc(devCb, jfcCb);
...
#ifdef CUSTOM_INTERFACE
    (void)DlHalBuffFree((void *)(uintptr_t)jettyCb->ciAddr);
#endif
...
STATIC void RsUbCtxExtJettyDelete(struct RsCtxJettyCb *jettyCb)
{
    urma_user_ctl_out_t out = {0};
    urma_user_ctl_in_t in = {0};
...
STATIC void RsUbUnbindJettyCbList(struct RsUbDevCb *devCb, struct RsListHead *jettyList)
{
    ...
    if (!RsListEmpty(jettyList)) {
        ...
        for (; (&jettyCurr->list) != jettyList;
            ...
            if (ret != 0) {
                hccp_err("rs_urma_unbind_jetty failed, ret:%d errno:%d devIndex:0x%x jettyId:%u",
...
STATIC void RsUbFreeJettyCbBatch(struct JettyDestroyBatchInfo *batchInfo,
    ...
    unsigned int *num)
{
    ...
    udmaCfg.trans_mode = cfg->transMode;
...
STATIC int RsUbCtxJfcCreateExt(struct RsCtxJfcCb *ctxJfcCb, urma_jfc_cfg_t jfcCfg, urma_jfc_t **jfc)
{
...
STATIC int RsUbJettyCbBuffAlloc(struct RsUbDevCb *devCb, struct RsCtxJettyCb *jettyCb)
{
    ...
    ret = DlHalBuffAllocAlignEx(CI_ADDR_TWO_BYTES, CI_ADDR_BUFFER_ALIGN_4K_PAGE_SIZE, flag, (int)devCb->rscb->grpId,
        (void **)&jettyCb->ciAddr);
    if (ret != 0) {
        hccp_err("dl_hal_buff_alloc_align_ex failed, length:0x%llx, dev_id:0x%x, flag:0x%lx, grp_id:%u, ret:%d",
            CI_ADDR_TWO_BYTES, logicDevid, flag, devCb->rscb->grpId, ret);
    }
...
STATIC int RsUbCtxRegJettyDb(struct RsCtxJettyCb *jettyCb, struct udma_u_jetty_info *jettyInfo)
{
...
STATIC void RsUbCtxExtJettyCreate(struct RsCtxJettyCb *jettyCb, urma_jetty_cfg_t *jettyCfg)
{
...
STATIC int RsUbFillJettyInfo(struct RsCtxJettyCb *jettyCb, struct QpCreateInfo *jettyInfo)
{
    ...
    jettyInfo->ub.ciAddr = jettyCb->ciAddr;
...
STATIC int RsUbCtxDrvJettyImport(struct RsCtxRemJettyCb *rjettyCb)
{
    ...
    rjetty.policy = rjettyCb->policy;
    rjetty.type = rjettyCb->type;
...
STATIC int RsUbCtxGetCqeAuxInfo(struct RsUbDevCb *devCb, struct AuxInfoIn *infoIn,
    struct AuxInfoOut *infoOut)
{
...
STATIC int RsUbCtxGetAeAuxInfo(struct RsUbDevCb *devCb, struct AuxInfoIn *infoIn,
    struct AuxInfoOut *infoOut)
{
...
STATIC void RsJfcCallbackProcess(struct RsCtxJettyCb *jettyCb, urma_cr_t *cr, urma_jfc_t *jfc)
{
    if (cr->status != URMA_CR_SUCCESS) {
        ...
        RsUdmaRetryTimeoutExceptionCheck(&jettyCb->devCb->sensorNode, cr);
    }
}
...
STATIC int RsHandleEpollPollJfc(struct RsUbDevCb *devCb, urma_jfce_t *jfce)
{
    ...
    for (i = 0; i < polledCnt; i++) {
        ...
        (*(uint16_t *)(uintptr_t)jettyCb->ciAddr)++;
