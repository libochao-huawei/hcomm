...
STATIC int RaHdcGetDevEidSubInfoList(unsigned int phyId, struct DevEidInfo infoList[],
    unsigned int startIndex, unsigned int count)
{
    ...
    (void)memcpy_s(infoList, sizeof(struct DevEidInfo) * MAX_DEV_INFO_TRANS_NUM,
        opData.rxData.infoList, sizeof(struct DevEidInfo) * MAX_DEV_INFO_TRANS_NUM);
    return 0;
}

int RaHdcGetDevEidInfoList(unsigned int phyId, struct DevEidInfo infoList[], unsigned int *num)
{
    struct DevEidInfo subInfoList[MAX_DEV_INFO_TRANS_NUM] = {0};
    ...
    *num = 0;
    ...
    for (startIndex = 0; startIndex + MAX_DEV_INFO_TRANS_NUM <= infoSize; startIndex += MAX_DEV_INFO_TRANS_NUM) {
        ...
        *num += MAX_DEV_INFO_TRANS_NUM;
        (void)memcpy_s(infoList + startIndex, sizeof(struct DevEidInfo) * MAX_DEV_INFO_TRANS_NUM,
            subInfoList, sizeof(struct DevEidInfo) * MAX_DEV_INFO_TRANS_NUM);
    }
    ...
    *num += remainCount;
    (void)memcpy_s(infoList + startIndex, sizeof(struct DevEidInfo) * remainCount,
        subInfoList, sizeof(struct DevEidInfo) * remainCount);
    return ret;
}
    ...
    unsigned int *num)
{
    ...
    *num = 0;
    CHK_PRT_RETURN(opData->rxData.num > ipNum, hccp_err("[get][eid_by_ip]rxData.num:%u > ip_num:%u",
...
int RaHdcGetEidByIp(struct RaCtxHandle *ctxHandle, struct IpInfo ip[], union HccpEid eid[], unsigned int *num)
{
    ...
    CHK_PRT_RETURN(retTmp != 0, hccp_err("[get][eid_by_ip]RaHdcGetEidResults failed ret[%d], phyId[%u]"
    ...
    union OpLmemRegInfoData *opData)
{
    ...
    CHK_PRT_RETURN(ret != 0, hccp_err("[init][ra_hdc_lmem]RaCtxPrepareLmemRegister failed, ret[%d]", ret), ret);
    ...
    union OpCtxQpCreateData *opData)
{
    ...
    CHK_PRT_RETURN(ret != 0, hccp_err("[init][ra_hdc_qp]RaCtxPrepareQpCreate failed, ret[%d]", ret), ret);
    ...
    struct JettyAttr attr[], unsigned int *num)
{
    ...
    CHK_PRT_RETURN(opData.rxData.num > *num, hccp_err("[query][ra_qp]op_data.rxData.num[%u] is larger than num[%u], "
...
int RaHdcCtxGetAuxInfo(struct RaCtxHandle *ctxHandle, struct AuxInfoIn *in, struct AuxInfoOut *out)
{
    unsigned int phyId = ctxHandle->attr.phyId;
    ...
    opData.txData.phyId = phyId;
    opData.txData.devIndex = ctxHandle->devIndex;
    (void)memcpy_s(&(opData.txData.info), sizeof(struct AuxInfoIn), in, sizeof(struct AuxInfoIn));
...
int RaHdcCtxGetCrErrInfoList(struct RaCtxHandle *ctxHandle, struct CrErrInfo *infoList, unsigned int *num)
{
    ...
    if (opData.rxData.num > expectedNum) {
        hccp_err("[get][cr_err_info_list]rxData.num(%u) > expected_num(%u), phyId(%u) devIndex(0x%x)",
