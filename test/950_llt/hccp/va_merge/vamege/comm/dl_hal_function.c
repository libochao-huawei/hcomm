 ...
 * Description: 获取动态库中ascend_hal接口函数的适配接口私有头文件.
 * Create: 2022-05-20
 */
#include <pthread.h>
#include "errno.h"
...
#define DL_API_IS_NULL_CHECK(handle, ptr, str) do { \
    if ((handle) == NULL) { \
        net_err("g_hal_api_handle is NULL!"); \
        return (-EINVAL); \
    } \
    if ((ptr) == NULL) { \
        net_err("[%s] is NULL!", (str)); \
        return (-EINVAL); \
    } \
} while (0)
...
static void DlHalApiHdcInit(void)
{
    ...
    gHalOps.dlHalHdcRecv = (hdcError_t (*)(HDC_SESSION session, struct drvHdcMsg *pMsg, int bufLen,
        UINT64 flag, int *recvBufCount, UINT32 timeout))AscendHalDlsym(gHalApiHandle, "halHdcRecv");
...
static void DlHalApiMiscInit(void)
{
    gHalOps.halSetUserConfig = (int (*)(uint32_t devid, const char *name, uint8_t *buf, uint32_t bufSize))
        AscendHalDlsym(gHalApiHandle, "halSetUserConfig");

    gHalOps.halClearUserConfig = (int (*)(uint32_t devid, const char *name))
        AscendHalDlsym(gHalApiHandle, "halClearUserConfig");
}
...
void DlHalDeinit(void)
{
    ...
    if (gHalApiHandle != NULL) {
        ...
        if (gHalApiRefcnt > 0) {
            ...
            net_info("dl_hal_deinit success, no need to dlclose libascend_hal.so!");
            return;
        }
    ...
    pthread_mutex_unlock(&gHalApiLock);
    net_info("dl_hal_deinit success!");
    return;
}
...
int DlHalInit(void)
{
    ...
    if (gHalApiHandle != NULL) {
        gHalApiRefcnt++;
        pthread_mutex_unlock(&gHalApiLock);
        net_info("dl_hal_init success, no need to dlopen libascend_hal.so!");
        return 0;
    }
    ...
    if (gHalApiHandle == NULL) {
        pthread_mutex_unlock(&gHalApiLock);
        net_err("dlopen libascend_hal.so failed! error_no=[%d]", errno);
        return -EINVAL;
    }
...
int DlHalSetUserConfig(uint32_t devid, const char *name, uint8_t *buf, uint32_t bufSize)
{
    ...
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.halSetUserConfig, "dl_devdrv_set_user_config");

    ret = gHalOps.halSetUserConfig(devid, name, buf, bufSize);
    DlHalDeinit();
...
int DlHalClearUserConfig(uint32_t devid, const char *name)
{
    ...
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.halClearUserConfig, "dl_devdrv_clear_user_config");

    ret = gHalOps.halClearUserConfig(devid, name);
    DlHalDeinit();
...
{
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.dlHalBuffAllocAlignEx, "DlHalBuffAllocAlignEx");
...
{
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.dlHalBuffFree, "DlHalBuffFree");
...
int DlHalGetChipInfo(unsigned int devId, halChipInfo *chipInfo)
{
    ...
    if (gHalOps.dlHalGetChipInfo == NULL) {
        net_warn("DlHalGetChipInfo is NULL!");
        return -EINVAL;
    }
...
{
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.dlHalEschedAttachDevice, "halEschedAttachDevice");
...
{
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.dlHalEschedCreateGrp, "halEschedCreateGrp");
...
{
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.dlHalEschedSubscribeEvent, "halEschedSubscribeEvent");
...
{
    DL_API_IS_NULL_CHECK(gHalApiHandle, gHalOps.dlHalEschedWaitEvent, "dlHalEschedWaitEvent");
