...
{
	//((struct drvHdcMsgBuf *)(msg + 1))->pBuf = pBuf;
	...
	unsigned long long flag, int *recvBufCount, unsigned int timeout)
{
	if (session == tc_host_hdc_session) {
		while (!tc_host_recv_flag) {
			usleep(100000);
			//fprintf(stderr, "tc_host_hdc_session recv tc_host_recv_flag[%d]\n", tc_host_recv_flag);
		}
		...
		*recvBufCount = 1;
		...
		while (!tc_device_recv_flag) {
			usleep(100000);
			//fprintf(stderr, "tc_device_hdc_session recv tc_device_recv_flag[%d]\n", tc_device_recv_flag);
		}
...
struct {
	...
	{"halResAddrMapV2", halResAddrMapV2},
	{"halResAddrUnmapV2", halResAddrUnmapV2},
    {NULL, NULL}
};
