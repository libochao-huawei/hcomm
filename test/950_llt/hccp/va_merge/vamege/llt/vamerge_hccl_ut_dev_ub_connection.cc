...
// 将DevUbConnection类涉及到的一些通用底层/单例接口打桩
void MockDevUbConnResource()
{
    DevType fakeDevType = DevType::DEV_TYPE_910_95;
    MOCKER(HrtGetDeviceType).stubs().will(returnValue(fakeDevType));
    int32_t fakeDevLogicId = 10;
    MOCKER(HrtGetDevice).stubs().with(any()).will(returnValue(fakeDevLogicId)); // 测试发现好像有没有不太影响
    std::pair<uint32_t, uint32_t> fakeDieFuncPair = std::make_pair(1, 10);
    MOCKER_CPP(&RdmaHandleManager::GetDieAndFuncId).stubs().will(returnValue(fakeDieFuncPair)); // 同不太影响
    JfcHandle fakeJettyHandle = 10;
    MOCKER_CPP(&RdmaHandleManager::GetJfcHandle).stubs().will(returnValue(fakeJettyHandle)); // 同不太影响
    std::pair<TokenIdHandle, uint32_t> fakeTokenInfo = std::make_pair(0x12345678, 10);
    ...
    MOCKER_CPP(&TpManager::GetTpInfo).stubs()
            ...
            .then(returnValue(HcclResult::HCCL_SUCCESS));// 原先在SetUp里，一同挪过来
}
...
TEST_F(DevUbConnectionTest, Ut_CreateJetty_When_CorrectParams_ReturnIsOk)
{
    ...
    std::string tag = "test";
    // When
    MOCKER(ra_ctx_qp_create_async)
            .stubs().will(invoke(ra_ctx_qp_create_async_stub));
    DevUbCtpConnection devUbConnection(rdmaHandle, linkData.GetLocalAddr(), linkData.GetRemoteAddr(), OpMode::OPBASE);
    // Then
    devUbConnection.SetJettyInfo(); // 怎么保证执行到这里异步接口执行完？
    EXPECT_EQ(expectSqBuffVa, devUbConnection.sqBuffVa);
}
