...
{
    EnvHostNicConfig hostNicCfg{};
    MOCKER(SalGetEnv).stubs().will(returnValue(string(HCOMM_CODE_ROOT_DIR "/test/orion/ut/framework/env_config/not_exist_file")));
    EXPECT_THROW(hostNicCfg.hcclWhiteListFile.Parse(), InvalidParamsException);
}
...
{
    EnvHostNicConfig hostNicCfg{};
    string filePath{HCOMM_CODE_ROOT_DIR "/test/orion/ut/framework/env_config/test_file.txt"};
