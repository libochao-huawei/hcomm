...
{
  echo "Usage:"
  echo "sh build.sh [-h] [-n] [-a] [-k] [-e] [-f] [-c] [-p] [-v] [-g] [-o] [-j[n]]"
  echo ""
  echo "Options:"
  echo "    -h Print usage"
...
checkopts()
{
  ...
  # Process the options
  while getopts 'dnfakechj:vp:guswo' opt
  do
...
build_air()
{
  ...
  if [ "X$ENABLE_HCCE_LLT" = "Xon" ];then
    MOCKCPP_DIR="${BASEPATH}/tests/engines/hccleng/third_party"
  ...
  cmake -D BUILD_OPEN_PROJECT=True \
        ...
        ..
  if [ "X$ENABLE_AICPU_LLT" = "Xon" ] || [ "X$ENABLE_FE_LLT" = "Xon" ] || [ "X$ENABLE_FFTS_LLT" = "Xon" ] || [ "X$ENABLE_DVPP_LLT" = "Xon" ] || [ "X$ENABLE_HCCE_LLT" = "Xon" ];then
...
run_llt_with_cov()
{
  ...
  then
    cd "${BASEPATH}/build/tests/engines/nneng/depends/te_fusion/"
    ln -sf libte_fusion_stub.so libte_fusion.so
    cd "${BASEPATH}"
    if [ "X$ENABLE_UT" = "Xon" ]
    then
      echo "---------------- Begin to run fe ut ----------------"
      cd "${BASEPATH}/build/tests/engines/nneng/"
      ...
      cp "${BASEPATH}/compiler/graphcompiler/engines/nneng/optimizer/fe_config/fe.ini" "${BASEPATH}/build/tests/engines/nneng/ut/plugin/opskernel/fe_config"
      cd "${BASEPATH}/../"
      ...
      ./$AIRDIR/build/tests/engines/nneng/ut/fe_ut
      if [ "X$ENABLE_ASAN" = "Xtrue" ];then
        unset LD_PRELOAD
        unset ASAN_OPTIONS
      fi
      ...
      then
        ...
        lcov -c -d build/tests/engines/nneng/ut -o cov_fe_ut/tmp.info
    ...
    then
      ...
      cp "${BASEPATH}/compiler/graphcompiler/engines/nneng/optimizer/fe_config/fe.ini" "${BASEPATH}/build/tests/engines/nneng/st/plugin/opskernel/fe_config"
      cd "${BASEPATH}/../"
      if [ "X$ENABLE_ASAN" = "Xtrue" ];then
        USE_ASAN=$(gcc -print-file-name=libasan.so)
        export LD_PRELOAD=${USE_ASAN}:/usr/lib/x86_64-linux-gnu/libstdc++.so.6
      ...
      ./$AIRDIR/build/tests/engines/nneng/st/fe_st
      if [ "X$ENABLE_ASAN" = "Xtrue" ];then
        unset LD_PRELOAD
        unset ASAN_OPTIONS
      fi
      ...
      then
        ...
        lcov -c -d build/tests/engines/nneng/st -o cov_fe_st/tmp.info
    ...
    then
      ...
      cd "${BASEPATH}/../"
      date
      ./$AIRDIR/build/tests/engines/nneng/st_whole_process/fe_st_whole_process --gtest_filter=STestFeWholeProcess310P3.*
      ./$AIRDIR/build/tests/engines/nneng/st_whole_process/fe_st_whole_process --gtest_filter=STestFeWholeProcess910A.*
      ./$AIRDIR/build/tests/engines/nneng/st_whole_process/fe_st_whole_process --gtest_filter=STestFeWholeProcess910B.*
      ./$AIRDIR/build/tests/engines/nneng/st_whole_process/fe_st_whole_process --gtest_filter=STestFeOmConsistencyCheck.*
      ./$AIRDIR/build/tests/engines/nneng/st_whole_process/fe_st_whole_process --gtest_filter=STestFeWholeProcess310B.*
      ./$AIRDIR/build/tests/engines/nneng/st_whole_process/fe_st_whole_process --gtest_filter=STestFeWholeProcessNano.*
      date
      ...
      then
        ...
        lcov -c -d build/tests/engines/nneng/st -o cov_fe_st_whole_process/tmp.info
  ...
  then
    ...
    then
      ...
      cd "${BASEPATH}/../"
      ./$AIRDIR/build/tests/engines/fftseng/ut/ffts_ut
      ...
      then
        ...
        lcov -c -d build/tests/engines/fftseng/ut -o cov_ffts_ut/tmp.info
    ...
    then
      ...
      cd "${BASEPATH}/../"
      ./$AIRDIR/build/tests/engines/fftseng/st/ffts_st
      echo "---------------- Finish the ffts st ----------------"
      if [ "X$ENABLE_LLT_COV" = "Xon" ]
      then
        cd "${BASEPATH}"
        mk_dir "${BASEPATH}/cov_ffts_st/"
        lcov -c -d build/tests/engines/fftseng/st -o cov_ffts_st/tmp.info
  ...
  then
    ...
    then
      ...
      unset ASCEND_OPP_PATH
      cd "${BASEPATH}/build/tests/engines/hccleng/ut/"
      ./hccl_engine_utest
      export ASCEND_OPP_PATH=$ASCEND_CUSTOM_PATH/opp
      ...
      then
        ...
        lcov -c -d ${BASEPATH}/build/tests/engines/hccleng/ut -o ${BASEPATH}/cov_hcce_ut/tmp.info
    ...
    then
      ...
      unset ASCEND_OPP_PATH
      cd "${BASEPATH}/build/tests/engines/hccleng/st/"
      ./hccl_engine_stest
      export ASCEND_OPP_PATH=$ASCEND_CUSTOM_PATH/opp
      ...
      then
        ...
        lcov -c -d ${BASEPATH}/build/tests/engines/hccleng/st -o ${BASEPATH}/cov_hcce_st/tmp.info
...
generate_package()
{
  ...
  COMMON_LIB=("libaicore_utils.so" "libopskernel.so" "libaicpu_engine_common.so" "libop_compile_adapter.so")
  OPSKERNEL_LIB=("libfe.so" "libffts.so" "libaicpu_ascend_engine.so"
                 "libaicpu_tf_engine.so" "libdvpp_engine.so" "librts_engine.so" "libhost_cpu_opskernel_builder.so" "libhost_cpu_engine.so" "libhcce.so")
...
main() {
  ...
  if [[ "X$ENABLE_FE_LLT" = "Xoff" ]] && [[ "X$ENABLE_AICPU_LLT" = "Xoff" ]] && [[ "X$ENABLE_DVPP_LLT" = "Xoff" ]] && [[ "X$ENABLE_FFTS_LLT" = "Xoff" ]] && [[ "X$ENABLE_HCCE_LLT" = "Xoff" ]]; then
    generate_package
  else
