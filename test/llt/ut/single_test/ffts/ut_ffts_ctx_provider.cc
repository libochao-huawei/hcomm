...
TEST_F(FftsCtxProviderTest, should_get_an_new_valid_ctx_given_the_op_is_alltoallv_with_large_data_num) {
    HcclOpMetaInfo meta = HcclOpMetaInfo::GetOneForAllToAllV(CopyPattern::BCOPY, 0x8FFFFFFF);
    auto ctx = fftsCtxProvider->GetFftsCtx(meta.isEnableCache, meta.GetCacheKey());
    EXPECT_NE(ctx, nullptr);
}

TEST_F(FftsCtxProviderTest, should_get_an_valid_ctx_given_the_op_is_alltoallvc) {
    HcclOpMetaInfo meta = HcclOpMetaInfo::GetOneForAllToAllVC(CopyPattern::BCOPY, 0);
    auto ctx = fftsCtxProvider->GetFftsCtx(meta.isEnableCache, meta.GetCacheKey());
    EXPECT_NE(ctx, nullptr);
}
