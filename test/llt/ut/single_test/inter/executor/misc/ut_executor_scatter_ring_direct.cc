...
TEST_F(ScatterRingDirectTest, create_prepare_destructor_001)
{
    // 获取模板实例
    std::unique_ptr<AlgTemplateBase> tempAlg =
        AlgTemplateRegistry::Instance().GetAlgTemplate(TemplateType::TEMPLATE_SCATTER_RING_DIRECT, dispatcher);
    EXPECT_NE(tempAlg, nullptr);
    // 转换子类指针
    ScatterRingDirect *alg = dynamic_cast<ScatterRingDirect *>(tempAlg.get());
    // 调用新增的prepare
    s32 ret = tempAlg->Prepare(&opInfo, userRank, ringsOrder, userMemInputSlices);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    // 校验成员变量
