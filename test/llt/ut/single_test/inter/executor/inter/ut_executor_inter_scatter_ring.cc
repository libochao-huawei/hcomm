...
TEST_F(ScatterInterTest, create_prepare_destructor_001)
{
    // 获取模板实例
    std::unique_ptr<AlgTemplateBase> tempAlg =
        AlgTemplateRegistry::Instance().GetAlgTemplate(TemplateType::TEMPLATE_SCATTER_RING, dispatcher);
    EXPECT_NE(tempAlg, nullptr);
    // 转换子类指针
    ScatterRing *alg = dynamic_cast<ScatterRing *>(tempAlg.get());
