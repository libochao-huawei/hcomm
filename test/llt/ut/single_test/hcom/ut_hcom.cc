...
TEST_F(HcomTest, ut_rt_get_set_Device_v81)
{
...
TEST_F(HcomTest, ut_hcom_bcast_cloud)
{
...
TEST_F(HcomTest, ut_hcom_bcast_cloud_dev1)
{
    nlohmann::json rank_table =
    {
        {"status", "completed"},
...
TEST_F(HcomTest, ut_hcom_reduce_cloud)
{
    nlohmann::json rank_table =
    {
        {"status", "completed"},
...
TEST_F(HcomTest, ut_hcom_allgather_cloud)
{
    nlohmann::json rank_table =
    {
        {"status", "completed"},
...
TEST_F(HcomTest, ut_hcom_get_rank_info_boardType0_alg0)
{
    nlohmann::json rank_table = rank_table_910_1server_1rank;
...
TEST_F(HcomTest, ut_hcom_cltm_bcast)
{
    ...
    cltmResult_t c_ret = cltm_generate_ranktable(inputRes, rankTableBuf, maxBufSize, &usedBufSize);
    EXPECT_EQ(c_ret, CLTM_SUCCESS);
    if (c_ret== CLTM_SUCCESS) {
        HCCL_INFO("output json:        %s", rankTableBuf);
    }

    char file_name_t[] = "./st_hcom_cltm_test.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);

    if (outfile.is_open())
    {
        outfile << std::setw(1) << rankTableBuf << std::endl;
        HCCL_INFO("open %s success", file_name_t);
    }
    else
    {
    ...
    // 走1910 4pring
    char* rank_table_file = "./st_hcom_cltm_test.json";
    char* rank_ID = "0";
    ...
    ret = HcomBroadcast("testbcastCltm", sendbuf, count, HCCL_DATA_TYPE_INT8, 0, NULL, stream);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
#if 0 //coredump
TEST_F(HcomTest, ut_hcom_gradient_segment_no_model)
{
...
TEST_F(HcomTest, ut_hcom_gradient_segment_more_than_8_segment)
{
    struct model_feature feature;
    u32 segment_num = 10;
    std::vector<u32> segment_index;
...
TEST_F(HcomTest, ut_hcom_gradient_segment_get_and_global_set)
{
    struct model_feature feature;
    std::vector<u32> segment_index;
    HcclResult ret;
...
TEST_F(HcomTest, ut_hcom_gradient_segment_global_size_get_and_set)
{
    struct model_feature feature;
    std::vector<u32> segment_index;
    HcclResult ret;
...
TEST_F(HcomTest, ut_hcom_gradient_segment_force_size_mode)
{
    struct model_feature feature;
    u32 segment_num = 10;
    std::vector<u32> segment_index;
...
TEST_F(HcomTest, ut_setAttachedStream)
{
    hcclComm h;
...
TEST_F(HcomTest, ut_hcom_GetInstanceList_error)
{
    string rank_ID("0");
    std::string rankTableM;
...
TEST_F(HcomTest, ut_hcom_GetHcclComm)
{   
    int64_t comm = 0;
    std::string group;
    ...
    .stubs()
    .will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&hcclComm::GetIdentifier)
    .stubs()
    .will(returnValue(std::string("aa")));
    HcclResult ret = HcomGetHcclComm(comm, group);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    comm = 1;
    ret = HcomGetHcclComm(comm, group);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}

TEST_F(HcomTest, ut_hcom_CreateCommCclBuf)
{   
    std::string group = "aa";
    MOCKER(hrtGetDeviceType)
    .stubs()
    ...
    HcclResult ret = HcomCreateCommCclBuf(0LL, group.c_str());
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = HcomCreateCommCclBuf(1LL, group.c_str());
    EXPECT_EQ(ret, HCCL_SUCCESS);
}

TEST_F(HcomTest, ut_hcom_GetInCclBuf)
{   
    std::string group = "a";
    void* Ptr;
    u64 Size;

    HcclResult ret = HcomGetInCclBuf(0LL, group.c_str(), Ptr, Size);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = HcomGetInCclBuf(1LL, group.c_str(), Ptr, Size);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}

TEST_F(HcomTest, ut_hcom_GetOutCclBuf)
{   
    std::string group = "a";
    void* Ptr;
    u64 Size;

    HcclResult ret = HcomGetOutCclBuf(0LL, group.c_str(), Ptr, Size);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = HcomGetOutCclBuf(1LL, group.c_str(), Ptr, Size);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}

TEST_F(HcomTest, ut_hcom_GetdirectCclBuf)
{   
    int64_t hcomComm;
    const char *sGroup;
    ...
    HcclResult ret =  HcomGetIndirectInCclBuf(hcomComm, sGroup, indirectInCCLbufPtr, indirectCommInputSize);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = HcomGetIndirectOutCclBuf(hcomComm, sGroup, indirectInCCLbufPtr, indirectCommInputSize);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}

TEST_F(HcomTest, ut_hccl_CommCalcTaskNum)
{   
    u32 num;
    HcomOpParam OpParam{};
    ...
    HcclResult ret = HcclCommCalcTaskNum(&OpParam, num);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}

TEST_F(HcomTest, ut_get_GroupNameByOpBaseHcom)
{   
    s64 opBaseHcom;
    char *groupname;
    ...
    HcclResult ret = GetGroupNameByOpBaseHcom(opBaseHcom, &groupname);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}
...
TEST_F(HcomTest, ut_hcom_HcomTopoInfoRegCallback)
{
    // HcomInfo hcomInfo;
...
TEST_F(HcomTest, ut_hcom_HcomSetExecTimeOut)
{
...
TEST_F(HcomTest, ut_hcom_HcomSetAlgorithm)
{
    char *algo = nullptr;
...
TEST_F(HcomTest, ut_hcom_HcomSetDeterministic)
{
...
TEST_F(HcomTest, ut_hcom_SetAivCoreLimit)
{
...
TEST_F(HcomTest, ut_hcom_GetInitStatus)
{
    bool initiated = 0;
...
TEST_F(HcomTest, ut_hcom_GetServerNumAndDeviceNumPerServer)
{
    u32 serverNum;
...
TEST_F(HcomTest, ut_hcom_SetWorkflowMode)
{
    HcclResult ret;
...
TEST_F(HcomTest, ut_hcom_CreateComResourceByComm)
{
...
TEST_F(HcomTest, ut_hcom_GenerateCclOpTag)
{
    s64 hcomComm;
...
TEST_F(HcomTest, ut_hcom_GetOpScratchMemSize)
{
...
TEST_F(HcomTest, ut_hcom_GetOpWorkspaceMemSize)
{
    bool isOfflineCompilation;
    HcclCMDType hcclOpType;
    s32 serverNum;
...
TEST_F(HcomTest, ut_hcom_HcomGetMemType)
{
    const char *group;
    u32 memType = 0;
    bool isTsMen = false;
...
TEST_F(HcomTest, ut_hcom_GetAlltoAllvStagedScratchMemSize)
{
    HcomOpParam hcomOpParam;
    u64 getMemSize;
...
TEST_F(HcomTest, ut_hcom_GetAlltoAllvcStagedScratchMemSize)
{
    HcomOpParam hcomOpParam;
    u64 getMemSize;
...
TEST_F(HcomTest, ut_hcom_GetAllReduceScratchMemSize)
{
    HcomOpParam hcomOpParam;
    u64 getMemSize;
...
TEST_F(HcomTest, ut_hcom_GetAllReduceScratchSizeWithoutDev)
{
    HcomOpParam hcomOpParam;
    u64 scratchSize;
...
TEST_F(HcomTest, ut_hcom_GetRedcueScatterVScratchMemSize)
{
    HcomOpParam hcomOpParam;
    u64 scratchSize;
...
TEST_F(HcomTest, ut_hcom_SplitHcclOpTypeConfig)
{
...
TEST_F(HcomTest, ut_hcom_GetToSlaveStreamTaskNum)
{
    u32 taskNum;
...
TEST_F(HcomTest, ut_hcom_GetToMasterStreamTaskNum)
{
    u32 taskNum;
...
TEST_F(HcomTest, ut_hcom_GetCombineComTaskNum)
{
    u32 intraTaskNum;
    u32 interTaskNum;
...
TEST_F(HcomTest, ut_hcom_GetIntraComTaskNum)
{
    AlgType algType;
    u32 taskNum;
...
TEST_F(HcomTest, ut_hcom_GetInterComTaskNum)
{   
    DevType devType;
    u32 taskNum;
...
TEST_F(HcomTest, ut_hcom_CalcTaskNum)
{   
    HcomOpParam hcomOpParam;
    u64 streamNum;
...
TEST_F(HcomTest, ut_hcom_GetDeterministic)
{
...
TEST_F(HcomTest, ut_hcom_HcomCalcOpOnline)
{
...
TEST_F(HcomTest, ut_hcom_HcomCalcOpResOffline)
{
...
TEST_F(HcomTest, ut_hcom_GetClusterInfoAndDeviceNum)
{
...
TEST_F(HcomTest, ut_hcom_GetServerAndDevNumFromGroupList)
{
    std::string rankTableString = R"({"status": "completed","version": "1.1","node_list":[{"node_id": "0","rank_list":[
        {"rank_id": "0","item_id": "0","rank_ip":"192.168.2.10"},
        {"rank_id": "1","item_id": "0","rank_ip":"192.168.2.11"},
...
TEST_F(HcomTest, ut_hcom_GetStreamNumOfflineComp)
{
    HcclCMDType hcclOpType;
    s32 serverNum = 1;
