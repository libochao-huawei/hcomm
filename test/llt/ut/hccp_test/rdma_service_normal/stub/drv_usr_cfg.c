...
int read_file_to_buf(const char *path, char *content, int *len)
{
    return 0;
}

int net_comm_get_self_home(char *user_name_path, unsigned int path_len)
{
    memcpy(user_name_path, "/tmp", strlen("/tmp"));
    return 0;
}
...
int net_get_gateway_address(unsigned int chip_id, const char *inbuf, unsigned int size_in,
