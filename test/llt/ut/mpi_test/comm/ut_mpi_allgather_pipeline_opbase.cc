...
class MPI_AllGatherInter_910B_Test : public testing::Test
{
protected:
...
void* inter_pipeline_all_gather_task(void* parg)
{
 ...
 ret = hcom_info.pComm->init(hcom_info.params, commConfig, hcom_info.rankTable);
    ...
    string strTag = "allgather_tag_magic9999997";
    ...
    while (*(para_info->sync_addr) < para_info->ranks_local) { sched_yield(); }
...
static nlohmann::json rank_table_pipeline_allgather_4ranks_2server =
    {
        {"status", "completed"},
        {"deploy_mode", "lab"},
        {"group_count", "1"},
                    ...
                    {"instance_count", "8"},
                        {
                            "instance_list",
                            {
                                ...
                                {   {"rank_id", "1"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "1"}, {"device_ip", "192.168.0.13"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "2"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "2"}, {"device_ip", "192.168.0.15"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "3"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "3"}, {"device_ip", "192.168.0.17"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "4"}, {"server_id", "10.0.0.11"},
                                    {
                                        "devices", {{{"device_id", "0"}, {"device_ip", "192.168.0.29"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "5"}, {"server_id", "10.0.0.11"},
                                    {
                                        "devices", {{{"device_id", "1"}, {"device_ip", "192.168.0.27"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "6"}, {"server_id", "10.0.0.11"},
                                    {
                                        "devices", {{{"device_id", "2"}, {"device_ip", "192.168.0.25"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "7"}, {"server_id", "10.0.0.11"},
                                    {
                                        "devices", {{{"device_id", "3"}, {"device_ip", "192.168.0.23"}, {"device_port", "16666"}}}
                                    }
                                },
                            }
                        },
...
#if 1
TEST_F(MPI_AllGatherInter_910B_Test, ut_mpi_pipeline_allgather_4ranks_2server)
{
    ...
    nlohmann::json rank_table = rank_table_pipeline_allgather_4ranks_2server;
    char file_name_t[] = "./st_allgather_8p.json";
    ...
    for (s32 i = 0; i < ndev; i++) {
        tid[i] = sal_thread_create("thread", inter_pipeline_all_gather_task, (void*)&para_info[i]);
