...
// 补充覆盖率
#if 0
TEST_F(DispatcherAiCpu_ST, ut_DispatcherAiCpuLaunchTaskEx)
{
    u32 streamNum = 7;
...
TEST_F(DispatcherAiCpu_ST, ut_DispatcherProfilingRdmaSend)
{
...
TEST_F(DispatcherAiCpu_ST, ut_AdprofReportBatchAdditionalInfo)
{
    MOCKER(AdprofCheckFeatureIsOn).stubs().will(returnValue(1));
    bool isL1On = dfx::ProfilingManager::IsProfL1On();
    EXPECT_EQ(isL1On, true);
...
TEST_F(DispatcherAiCpu_ST, ut_launchTask_rtsqfull)
{
    uint32_t sqHead = 0;
    uint32_t sqTail = 100;
    Stream stream(streamInfo, true);
...
TEST_F(DispatcherAiCpu_ST, ut_DispatcherAiCpu_StreamSync)
{
    uint32_t sqHead = 0;
    uint32_t sqTail = 100;
    Stream stream(streamInfo, true);
...
TEST_F(DispatcherAiCpu_ST, ut_GetStreamSqeBufferAddr_2048)
{
...
TEST_F(DispatcherAiCpu_ST, ut_aicpu_fine_granularity)
{
    uint32_t sqHead = 0;
    uint32_t sqTail = 3;
    u64 addr = 0x12345678;
...
TEST_F(DispatcherAiCpu_ST, ut_DispatcherAiCpu_SignalRecord)
{
    uint32_t sqHead = 0;
    uint32_t sqTail = 100;
...
TEST_F(DispatcherAiCpu_ST, ut_data_plane_interface_aicpu)
{
