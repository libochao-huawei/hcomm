...
void *inter_all_reduce_task_3(void *parg)
{
    ...
    ret = DlRaFunction::GetInstance().DlRaFunctionInit();
    EXPECT_EQ(ret, HCCL_SUCCESS);
    /* 对 HcomLoadRanktableFile +  CfgGetClusterInfo 进行替换为使用 GenRankTable */
    TopoMeta topoMate;
    RankTable_For_LLT::GenTopoMeta(topoMate, 1, 1, 4);
    ...
    SalStrToULong(para_info->identify, HCCL_BASE_DECIMAL, rankId);
    s32 logicDevId_;
    DevType deviceType_;
    hrtGetDevice(&logicDevId_);
    hrtGetDeviceType(deviceType_);
    ...
    hcom_info.params.logicDevId = logicDevId_;
    hcom_info.params.deviceType = deviceType_;
...
// 71 4p 确定性 小数据量
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_deterministic_small_count)
{
    ...
    char file_name_t[] = "./st_allreduce_4p_ring.json";
...
// 71 4p 确定性 大数据量
#if 0 //执行失败偶现卡死
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_deterministic_big_count)
{
    setenv("HCCL_DETERMINISTIC", "true", 1);
    ResetInitState();
    InitExternalInput();
...
#endif
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_deterministic_big_count_opbase)
{
    setenv("HCCL_DETERMINISTIC", "true", 1);
    ResetInitState();
    InitExternalInput();
...
// 71 8p 确定性 小数据量
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_deterministic_small_count_opbase)
{
    setenv("HCCL_DETERMINISTIC", "true", 1);
    ResetInitState();
    InitExternalInput();
    ...
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_1server_4rank;
    char file_name_t[] = "./st_allreduce_8p.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
...
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_deterministic_small_count_graph)
{
    setenv("HCCL_DETERMINISTIC", "true", 1);
    ResetInitState();
    InitExternalInput();
...
#if 0
// 71 8p 确定性 中数据量
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_deterministic_big_count_graph)
{
    setenv("HCCL_DETERMINISTIC", "true", 1);
    ResetInitState();
    InitExternalInput();
...
#endif
#if 0 //卡住
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_deterministic_big_count_opbase)
{
    setenv("HCCL_DETERMINISTIC", "true", 1);
    ResetInitState();
    InitExternalInput();
...
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_deterministic_big_count_opbase_prod)
{
    setenv("HCCL_DETERMINISTIC", "true", 1);
    ResetInitState();
    InitExternalInput();
...
#endif
// 71 4p 不确定性 图模式
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_small_count)
{
    ResetInitState();
    InitExternalInput();
...
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_big_count)
{
    ResetInitState();
    InitExternalInput();
    setenv("HCCL_BUFFSIZE", "1", 1);
...
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_big_count_prod)
{
    ResetInitState();
    InitExternalInput();
    setenv("HCCL_BUFFSIZE", "1", 1);
...
#if 0
// 71 4p 不确定性 单算子模式 512k以上
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_big_count_80)
{
    ResetInitState();
    InitExternalInput();
    ...
    for (s32 i = 0; i < ndev; i++)
    {
        tid[i] = sal_thread_create("thread", inter_all_reduce_task_3, (void *)&para_info[i]);
        EXPECT_NE(tid[i], (sal_thread_t)NULL);
    }
...
// 80 8p  单算子模式 512k以上
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_big_count_80)
{
    ResetInitState();
    InitExternalInput();
    RankConsistent::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_1server_4rank;
    char file_name_t[] = "./st_allreduce_8p_ring.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
...
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_deterministic_big_count_hccl_algo)
{
...
// 71 8p 不确定性 单算子模式 512k以上
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_op_base)
{
    ResetInitState();
    InitExternalInput();
...
//71 8p AIV 单算子
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_aiv_opbase)
{
...
//71 8p AIV 图模式
TEST_F(HcclCommTest910BAllReduce, st_allreduce_4p_mesh_aiv_graph)
{
    setenv("HCCL_OP_EXPANSION_MODE", "AIV", 1);
    ResetInitState();
    InitExternalInput();
...
//71 8p AIV 单算子 大数据量
TEST_F(HcclCommTest910BAllReduce, st_allreduce_8p_mesh_aiv_opbase_bigdata)
{
    setenv("HCCL_OP_EXPANSION_MODE", "AIV", 1);
    ResetInitState();
    InitExternalInput();
