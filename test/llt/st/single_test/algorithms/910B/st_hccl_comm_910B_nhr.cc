...
void* inter_scatter_task_nhr(void* parg)
{
    HcclResult ret = HCCL_SUCCESS;
    para_t* para_info = (para_t*)parg;
    s32 rank_num_tmp;
...
void* inter_reduce_scatter_task_nhr(void* parg)
{
    ...
    ret = hcom_info.pComm->SetWorkspaceResource("tag_inter_reduce_scatter_task_nhr", memptr, memSize, streamList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret =  hcom_info.pComm->ReduceScatter("tag_inter_reduce_scatter_task_nhr",
                               para_info->sendbuff,
                               para_info->recvbuff,
                               para_info->count,
                               para_info->datatype,
...
TEST_F(HcclCommTest910BNHR, st_allreduce_8p_mesh)
{
    ...
    char file_name_t[] = "./st_allreduce_8p_nhr.json";
...
TEST_F(HcclCommTest910BNHR, st_scatter_8p_ring)
{
    EXPECT_EQ(SetHcclAlgoConfig("level0:NA;level1:NHR"), HCCL_SUCCESS);
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_1server_8rank;
    char file_name_t[] = "./st_allreduce_8p_nhr.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
    ...
    MOCKER_CPP(&HcclSocketManager::ServerInit).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    for (s32 i = 0; i < ndev; i++) {
        tid[i] = sal_thread_create("thread", inter_scatter_task_nhr, (void*)&para_info[i]);
        EXPECT_NE(tid[i], (sal_thread_t )NULL);
    }
...
TEST_F(HcclCommTest910BNHR, st_reducescatter_8p_mesh)
{
    EXPECT_EQ(SetHcclAlgoConfig("level0:null;level1:NHR"), HCCL_SUCCESS);
...
TEST_F(HcclCommTest910BNHR, st_allgather_8p_mesh)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_1server_8rank;
