...
void* inter_reduce_scatter_task_91093(void* parg)
{
 ...
 ret = hcom_info.pComm->init(hcom_info.params, commConfig, hcom_info.rankTable);
    ...
    ret = hcom_info.pComm->SetWorkspaceResource("tag_inter_reduce_scatter_task_91093", memptr, memSize, streamList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret =  hcom_info.pComm->ReduceScatter("tag_inter_reduce_scatter_task_91093",
                               para_info->sendbuff,
                               para_info->recvbuff,
                               para_info->count,
                               para_info->datatype,
...
#if 1
TEST_F(HcclCommTest91093, st_allreduce_4p_ring)
{
...
TEST_F(HcclCommTest91093, st_reducescatter_4p_ring)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_1server_4rank;
    char file_name_t[] = "./st_allreduce_4p_ring.json";
...
TEST_F(HcclCommTest91093, st_allgather_outplace_4p_ring)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_1server_4rank;
    char file_name_t[] = "./st_allgather_4p_ring.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
...
void* inter_reduce_scatter_ring_atomic_opbase_task_91093(void* parg)
{
 ...
 ret = hcom_info.pComm->init(hcom_info.params, commConfig, hcom_info.rankTable);
    ...
    ret = hcom_info.pComm->SetWorkspaceResource("tag_inter_reduce_scatter_ring_atomic_opbase_task_91093", memptr, memSize, streamList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    (void) SetWorkflowMode(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE);
    ret =  hcom_info.pComm->communicator_->ReduceScatterOutPlace("tag_inter_reduce_scatter_ring_atomic_opbase_task_91093",
                               para_info->sendbuff,
                               para_info->recvbuff,
                               para_info->count,
                               para_info->datatype,
...
TEST_F(HcclCommTest91093, st_reducescatter_4p_ring_atomic_opbase)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_1server_4rank;
    char file_name_t[] = "./st_allreduce_4p_ring.json";
...
TEST_F(HcclCommTest91093, st_reduce_4p_ring)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_910_1server_4rank;
    char file_name_t[] = "./st_reduce_4p_reduce.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
...
TEST_F(HcclCommTest91093, st_broadcast_4pring)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    ...
    char file_name_t[] = "./st_broadcast_4pring.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
    ...
    s32 count = HCCL_BROADCAST_DATA_SIZE;
    s32 ndev = DEV_NUM_4;
    HcclRootInfo rootInfo;
    set_board_id(0x0000);
    for (s32 i = 0; i < ndev; i++ )
    ...
    for (s32 i = 0; i < ndev; ++i)
    {
        tid[i] = sal_thread_create("thread", inter_broadcast_task_91093, (void*)&para_info[i]);
