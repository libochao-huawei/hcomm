...
void* inter_scatter_task_nb(void* parg)
{
    ...
    if (ret != HCCL_SUCCESS)
    {
        HCCL_ERROR("dev[%d] task HcclScatter falis", hcom_info.params.rank);
    }
    ...
    if ( rt_ret != RT_ERROR_NONE)
    {
        HCCL_ERROR("rank[%d] task HcclScatter falis", hcom_info.params.rank);
    }
...
void* inter_reduce_scatter_task_nb(void* parg)
{
    ...
    ret = hcom_info.pComm->SetWorkspaceResource("tag_inter_reduce_scatter_task_nb", memptr, memSize, streamList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret =  hcom_info.pComm->ReduceScatter("tag_inter_reduce_scatter_task_nb",
                               para_info->sendbuff,
                               para_info->recvbuff,
                               para_info->count,
                               para_info->datatype,
...
TEST_F(HcclCommTest910BNB, st_allreduce_8p_mesh)
{
    ...
    char file_name_t[] = "./st_allreduce_8p_nb.json";
...
TEST_F(HcclCommTest910BNB, st_scatter_8p_ring)
{
...
static nlohmann::json rank_table_reducescatter_8p_mesh_1server_8rank =
{
        {"status", "completed"},
        {"deploy_mode", "lab"},
        {"group_count", "1"},
                    ...
                    {"instance_count", "8"},
                        {
                            "instance_list",
                            {
                                {   {"rank_id", "0"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "0"}, {"device_ip", "192.168.1.11"}, {"device_port", "16666"}}}
                                    }
                                },

                                {   {"rank_id", "1"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "1"}, {"device_ip", "192.168.1.12"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "2"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "2"}, {"device_ip", "192.168.1.13"}, {"device_port", "16666"}}}
                                    }
                                },

                                {   {"rank_id", "3"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "3"}, {"device_ip", "192.168.1.14"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "4"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "4"}, {"device_ip", "192.168.1.15"}, {"device_port", "16666"}}}
                                    }
                                },

                                {   {"rank_id", "5"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "5"}, {"device_ip", "192.168.1.16"}, {"device_port", "16666"}}}
                                    }
                                },
                                {   {"rank_id", "6"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "6"}, {"device_ip", "192.168.1.17"}, {"device_port", "16666"}}}
                                    }
                                },

                                {   {"rank_id", "7"}, {"server_id", "10.0.0.10"},
                                    {
                                        "devices", {{{"device_id", "7"}, {"device_ip", "192.168.1.18"}, {"device_port", "16666"}}}
                                    }
                                }
                            }
                        },
...
TEST_F(HcclCommTest910BNB, st_reducescatter_8p_mesh)
{
    EXPECT_EQ(SetHcclAlgoConfig("level0:null;level1:NB"), HCCL_SUCCESS);

    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_reducescatter_8p_mesh_1server_8rank;
    char file_name_t[] = "./st_reducescatter_8p.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
...
TEST_F(HcclCommTest910BNB, st_allgather_8p_mesh)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_1server_8rank;
    char file_name_t[] = "./st_allgather_8p_mesh.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
