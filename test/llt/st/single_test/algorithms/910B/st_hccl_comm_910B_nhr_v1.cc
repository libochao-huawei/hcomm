...
class HcclCommTest910BNHRV1 : public testing::Test
{
protected:
    static void SetUpTestCase()
    {
        std::cout << "\033[36m--HcclCommTest910BNHRV1 SetUP--\033[0m" << std::endl;
    }
    static void TearDownTestCase()
    {
        std::cout << "\033[36m--HcclCommTest910BNHRV1 TearDown--\033[0m" << std::endl;
    }
...
void* inter_all_reduce_task_nhr_v1(void* parg)
{
...
void* inter_reduce_scatter_task_nhr_v1(void* parg)
{
    HcclResult ret = HCCL_SUCCESS;
    para_t* para_info = (para_t*)parg;
    s32 rank_num_tmp;
    ...
    ret = hcom_info.pComm->SetWorkspaceResource("tag_inter_reduce_scatter_task_nhr_v1", memptr, memSize, streamList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret =  hcom_info.pComm->ReduceScatter("tag_inter_reduce_scatter_task_nhr_v1",
                               para_info->sendbuff,
                               para_info->recvbuff,
                               para_info->count,
                               para_info->datatype,
...
#if 0 //卡住
TEST_F(HcclCommTest910BNHRV1, st_allreduce_8p_mesh)
{
    EXPECT_EQ(SetHcclAlgoConfig("level0:null;level1:NHR_V1"), HCCL_SUCCESS);
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_1server_8rank;
    char file_name_t[] = "./st_allreduce_8p_nhr_v1.json";
    ...
    for (s32 i = 0; i < ndev; i++) {
        tid[i] = sal_thread_create("thread", inter_all_reduce_task_nhr_v1, (void*)&para_info[i]);
...
TEST_F(HcclCommTest910BNHRV1, st_reducescatter_8p_mesh)
{
    EXPECT_EQ(SetHcclAlgoConfig("level0:null;level1:NHR_V1"), HCCL_SUCCESS);
    ...
    for (s32 i = 0; i < ndev; ++i)
    {
        tid[i] = sal_thread_create("thread", inter_reduce_scatter_task_nhr_v1, (void*)&para_info[i]);
        EXPECT_NE(tid[i], (sal_thread_t )NULL);
    }
...
#endif
void* inter_all_gather_mesh_nhr_v1(void* parg)
{
    HcclResult ret = HCCL_SUCCESS;
    para_t* para_info = (para_t*)parg;
    s32 rank_num_tmp;
...
#if 0 // 卡住
TEST_F(HcclCommTest910BNHRV1, st_allgather_8p_mesh)
{
    RankConsistentcyChecker::GetInstance().ClearCheckInfo();
    nlohmann::json rank_table = rank_table_1server_8rank;
    ...
    int ret = HCCL_SUCCESS;
    std::string algo = "level0:null;level1:NHR_V1";
    ...
    for (s32 i = 0; i < ndev; i++)
    {
        tid[i] = sal_thread_create("thread", inter_all_gather_mesh_nhr_v1, (void*)&para_info[i]);
        EXPECT_NE(tid[i], (sal_thread_t )NULL);
    }
