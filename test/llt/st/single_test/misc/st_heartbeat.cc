...
HcclResult stub_GetRaResourceInfo(NetworkManager* that, RaResourceInfo &raResourceInfo)
{
    ...
    if (!initialized) {
        ...
        for (int i = 0; i < 8; i++) {
            fake_raResourceInfo.nicSocketMap[HcclIpAddress(1684515008)] = tmpIpSocket;
        }
    }
...
HcclResult stub_GetRaResourceInfo_RdmaHandle(NetworkManager* that, RaResourceInfo &raResourceInfo)
{
    ...
    if (!initialized) {
        ...
        RdmaHandle nicRdmaHandle = &fake_handle;
        for (int i = 0; i < 8; i++) {
            fake_raResourceInfo.nicSocketMap[ipAddr] = tmpIpSocket;
            fake_raResourceInfo.nicSocketMap[ipAddr].nicRdmaHandle = nicRdmaHandle;
        }
    }
    raResourceInfo = fake_raResourceInfo;
    return HCCL_SUCCESS;
...
TEST_F(HeartBeatTest, st_HeartBeatTest1)
{
...
TEST_F(HeartBeatTest, st_HeartBeatTest2)
{
    std::vector<RankInfo> rankInfos1;
    for (size_t i = 0; i < 8; i++) {
        RankInfo rankInfo;
...
TEST_F(HeartBeatTest, st_HeartBeatTest3)
{
    std::vector<RankInfo> rankInfos1;
    for (size_t i = 0; i < 8; i++) {
        RankInfo rankInfo;
...
TEST_F(HeartBeatTest, st_HeartBeatTest5)
{
    struct ra_init_config config;
    ra_init(&config);
...
TEST_F(HeartBeatTest, st_SendFrame)
{
    ...
    ret = Heartbeat::GetInstance(0).SendFrame(dst, crimer, informer, HeartBeatStatus::HEARTBEAT_LOST, opInfoList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = Heartbeat::GetInstance(0).SendFrame(dst, crimer, informer, HeartBeatStatus::HEARTBEAT_LOST, opInfoList);
    EXPECT_EQ(ret, HCCL_SUCCESS);
...
HcclResult stub_hrtRaGetCqeErrInfo_value_status_0(unsigned int phy_id, struct cqe_err_info *info)
{
    info->qpn = 10;
    info->status = 0;
    struct timeval tv;
...
HcclResult stub_hrtRaGetCqeErrInfo_value(unsigned int phy_id, struct cqe_err_info *info)
{
    info->qpn = 1;
    info->status = 12;
    struct timeval tv;
...
HcclResult stub_hrtRaGetCqeErrInfoList_value(RdmaHandle handle, struct cqe_err_info *infolist, unsigned int *num)
{
    *num = 65;
    infolist[0].qpn=1;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_1)
{
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_001)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_002)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_003)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_004)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_005)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_006)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_007)
{
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_008)
{
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_009)
{
    // 未开启重执行分支 
...
TEST_F(HeartBeatTest, st_ProcessCqeErrInfo_010)
{
    // 开启重执行分支 && 注入 status = 0，提前退出
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(any()).will(returnValue(true));
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_SaveQpnForOpRetry_001)
{
    // 提前 return 覆盖
    ErrCqeInfo info;
...
TEST_F(HeartBeatTest, st_SaveQpnForOpRetry_002)
{
    std::map<u32, std::set<ErrQpnInfo>> infomap;
    ErrQpnInfo errqpn;
    errqpn.qpn = 54;
    ErrCqeInfo infoExt;
    CqeInfo info1;
...
TEST_F(HeartBeatTest, st_CheckErrorCqe)
{
...
TEST_F(HeartBeatTest, st_keyEvents)
{
    std::vector<RankInfo> rankInfos1;
    for (size_t i = 0; i < 8; i++) {
        RankInfo rankInfo;
...
TEST_F(HeartBeatTest, st_SutckDetection1)
{
...
TEST_F(HeartBeatTest, st_SutckDetection2)
...
TEST_F(HeartBeatTest, st_transport_ibv_stop_test)
{
...
TEST_F(HeartBeatTest, st_transport_ibv_Resume_test)
{
    DispatcherPub *dispatcher;
    const std::unique_ptr<NotifyPool> notifyPool;
    MachinePara machinePara;
...
TEST_F(HeartBeatTest, st_GetQpnErrForOpRetryAgent_Null)
{
...
TEST_F(HeartBeatTest, st_GetQpnErrForOpRetryAgent)
{
    // 开启重执行分支
...
TEST_F(HeartBeatTest, st_BroadcastCqeErr)
{
    // 开启重执行分支
    // qpmapForOpRetry 中没有指定通信域的 kv pair
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(any()).will(returnValue(true));
...
TEST_F(HeartBeatTest, st_ClearAllCqeErr)
{
    // 开启重执行分支
    // qpmapForOpRetry 中没有指定通信域的 kv pair
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(any()).will(returnValue(true));
...
TEST_F(HeartBeatTest, st_ClearCqeErr1)
{
    // 清除不存在的通信域中的数据
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(any()).will(returnValue(true));
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ClearCqeErr2)
{
    // 清除不存在的通信域中的dstRank
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(any()).will(returnValue(true));
    TransportIbverbs::g_flag = true;
...
TEST_F(HeartBeatTest, st_ClearCqeErr3)
{
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(any()).will(returnValue(true));
    TransportIbverbs::g_flag = true;
        MOCKER(hrtRaGetCqeErrInfoList)
...
TEST_F(HeartBeatTest, st_ClearCqeErr5)
{
    struct timeval time;
    uint32_t status = 0;
    ...
    rankExtendMap[5] = {ErrQpnInfo(cqeInfo, 12)};
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    rankExtendMap[6] = {ErrQpnInfo(cqeInfo, 14), ErrQpnInfo(cqeInfo, 13)};
...
TEST_F(HeartBeatTest, st_IsKeyEvent_OpretryNotSupport)
{
    // 构造异常心跳帧
    RankInfo rankInfo;
    ...
    MOCKER(GetExternalInputHcclExecTimeOut).stubs().will(returnValue(68));
    HcclUs curTime;
    EXPECT_EQ(Heartbeat::GetInstance(0).IsKeyEvent(bf, curTime), true);
    GlobalMockObject::verify();
}

TEST_F(HeartBeatTest, st_MakeErrMsg_OpretryNotSupport)
{
    // 构造异常心跳帧
    RankInfo rankInfo;
    rankInfo.serverId = "127.0.0.1";
...
TEST_F(HeartBeatTest, st_GetHostPort)
{
...
TEST_F(HeartBeatTest, st_cluster_heart_switch_off)
{
...
TEST_F(HeartBeatTest, st_cluster_heart_switch_on)
{
...
TEST_F(HeartBeatTest, st_cluster_heart_env_config)
{
    setenv("HCCL_DFS_CONFIG", "cluster_heartbeat:OFF", 1);
...
TEST_F(HeartBeatTest, st_stuck_detection_env_config)
{
...
TEST_F(HeartBeatTest, st_SaveOpInfo)
{
    // SetStatus 函数覆盖
    RankInfo localRank;
    localRank.serverId="1.1.1.0";
    localRank.devicePhyId=7;
    UIDType uid_ = Heartbeat::GetInstance(7).GetUId(localRank);
    Heartbeat::GetInstance(7).SetStatus(uid_,uid_,HeartBeatStatus::HEARTBEAT_INCONSISTENT, false);
    ...
    opInfoList[0] = opInfo;
    Heartbeat::GetInstance(7).SaveOpInfo(opInfoList, uid_);
    opInfo.isValid = true;
    opInfoList[0] = opInfo;
    Heartbeat::GetInstance(7).SaveOpInfo(opInfoList, uid_);
    ...
    bool checkRes = false;
    checkRes = Heartbeat::GetInstance(7).CheckIsSameOp(opInfo, remoteopInfo);
    ...
    memcpy_s(opInfo.identifier, identifier.length() + 1, identifier.c_str(), identifier.length() + 1);
    memcpy_s(remoteopInfo.identifier, identifier.length() + 1, identifier.c_str(), identifier.length() + 1);
    Heartbeat::GetInstance(7).AddOpInfo(identifier, opInfo, identifier);
    opInfoList[0] = remoteopInfo;
    Heartbeat::GetInstance(7).SaveOpInfo(opInfoList, uid_);
    Heartbeat::GetInstance(7).GetOneOpInfo();
    Heartbeat::GetInstance(7).CheckRecvOpInfoList();
}