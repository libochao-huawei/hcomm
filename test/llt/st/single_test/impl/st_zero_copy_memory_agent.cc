...
TEST_F(ZeroCopyMemoryAgentSt, st_agent_test)
{
...
TEST_F(ZeroCopyMemoryAgentSt, st_agent_test1)
{
    MOCKER(GetIsSupSockBatchCloseImmed).stubs().will(invoke(stub_SocketManagerTest_GetIsSupSockBatchCloseImmed));
    u32 interfaceVersion = 1;
    MOCKER(hrtRaGetInterfaceVersion)
    ...
    MOCKER_CPP(&HcclSocket::Send, HcclResult(HcclSocket::*)(const void *, u64))
        ...
        .with(any())
        .will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclSocket::IRecv).stubs().will(returnValue(HCCL_SUCCESS));
    MOCKER(aclrtMapMem).stubs().will(returnValue(ACL_SUCCESS));
    MOCKER(aclrtUnmapMem).stubs().will(returnValue(ACL_SUCCESS));
    std::unique_ptr<HcclSocketManager> socketManager;
    socketManager.reset(new (std::nothrow) HcclSocketManager(NICDeployment::NIC_DEPLOYMENT_DEVICE, 0, 0, 0));
...
TEST_F(ZeroCopyMemoryAgentSt, st_agent_wait_timeout)
{
    std::unique_ptr<HcclSocketManager> socketManager;
    socketManager.reset(new (std::nothrow) HcclSocketManager(NICDeployment::NIC_DEPLOYMENT_DEVICE, 0, 0, 0));
    HcclIpAddress localIPs(0x01);
...
TEST_F(ZeroCopyMemoryAgentSt, st_set_memory_failed)
{
    int dummy = 0;
    HcclCommunicator comm;

    comm.serverNum_ = 2;
    EXPECT_EQ(comm.SetMemoryRange(&dummy, sizeof(int), 0, 0), HCCL_E_NOT_SUPPORT);

    comm.serverNum_ = 1;
    comm.deviceType_ = DevType::DEV_TYPE_910;
    EXPECT_EQ(comm.SetMemoryRange(&dummy, sizeof(int), 0, 0), HCCL_E_NOT_SUPPORT);
}
...
TEST_F(ZeroCopyMemoryAgentSt, St_AgentFunc_When_UseAsyncSocketApi_ExpectNorm)
{
    MOCKER(GetIsSupSockBatchCloseImmed).stubs().will(invoke(stub_SocketManagerTest_GetIsSupSockBatchCloseImmed));
    u32 interfaceVersion = 1;
...
TEST_F(ZeroCopyMemoryAgentSt, St_RequestBatchSendAsync_When_CombineAckAndReq_ExpectNorm)
{
    MOCKER(GetIsSupSockBatchCloseImmed).stubs().will(invoke(stub_SocketManagerTest_GetIsSupSockBatchCloseImmed));
    u32 interfaceVersion = 1;
    MOCKER(hrtRaGetInterfaceVersion).expects(atMost(2))
...
TEST_F(ZeroCopyMemoryAgentSt, St_RequestBatchSendAsync_When_SocketSendFailed_ExpectTryMore)
{
    MOCKER(GetIsSupSockBatchCloseImmed).stubs().will(invoke(stub_SocketManagerTest_GetIsSupSockBatchCloseImmed));
    u32 interfaceVersion = 1;
    MOCKER(hrtRaGetInterfaceVersion).expects(atMost(2))
...
TEST_F(ZeroCopyMemoryAgentSt, St_RequestBatchRecvAsync_When_SocketRecvFailed_ExpectTryMore)
{
    MOCKER(GetIsSupSockBatchCloseImmed).stubs().will(invoke(stub_SocketManagerTest_GetIsSupSockBatchCloseImmed));
    u32 interfaceVersion = 1;
    MOCKER(hrtRaGetInterfaceVersion).expects(atMost(2))
