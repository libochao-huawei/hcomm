...
class CollBroadcastSTTest : public testing::Test
{
protected:
     static void SetUpTestCase()
    {
        ...
        std::cout << "\033[36m--CollBroadcastSTTest SetUP--\033[0m" << std::endl;
    }
    static void TearDownTestCase()
    {
        ...
        std::cout << "\033[36m--CollBroadcastSTTest TearDown--\033[0m" << std::endl;
    }
    virtual void SetUp()
    {
        HcclOpMetaInfo meta;
...
#if 1
TEST_F(CollBroadcastSTTest, broadcast_ring)
{
...
#if 1
TEST_F(CollBroadcastSTTest, broadcast_mesh)
{
    MOCKER(hrtCtxSetCurrent)
    .stubs()
    .with(any())
...
#if 1
TEST_F(CollBroadcastSTTest, broadcast_smallcount)
{
    MOCKER(hrtCtxSetCurrent)
    .stubs()
    .with(any())
...
#if 1
TEST_F(CollBroadcastSTTest, broadcast_common)
{
    MOCKER(hrtCtxSetCurrent)
    .stubs()
    .with(any())
...
HcclDispatcher CollBroadcastSTTest::dispatcherPtr = nullptr;
DispatcherPub *CollBroadcastSTTest::dispatcher = nullptr;

static void TestConstructParam_910_93(HcclCommParams &params, RankTable_t &rankTable)
{
    string commId = "comm ";
    memcpy_s(params.id.internal, HCCL_ROOT_INFO_BYTES, commId.c_str(), commId.length() + 1);
    params.rank = 0;
...
TEST_F(CollBroadcastSTTest, broadcast_superpod_Ring)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(CollBroadcastSTTest, broadcast_for_310P3_comm)
{
    MOCKER(hrtCtxSetCurrent)
    .stubs()
    .with(any())
...
TEST_F(CollBroadcastSTTest, st_bcast_plus_bcast_NB)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
...
TEST_F(CollBroadcastSTTest, broadcast_test_910B) {
    HcclResult ret = HCCL_SUCCESS;
    HcclCommParams params;
    RankTable_t rankTable;
    TestConstructParam(params, rankTable);
...
TEST_F(CollBroadcastSTTest, broadcast_test_310P) {
    HcclResult ret = HCCL_SUCCESS;
    HcclCommParams params;
    RankTable_t rankTable;
    TestConstructParam(params, rankTable);
...
TEST_F(CollBroadcastSTTest, broadcast_double_ring_1)
{
    HcclResult ret = HCCL_SUCCESS;
    std::string tag = "test";
    DeviceMem inputMem = DeviceMem::alloc(4096);
