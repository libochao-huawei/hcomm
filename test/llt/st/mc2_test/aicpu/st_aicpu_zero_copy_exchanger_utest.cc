...
class AicpuZeroCopyExchanger_ST : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "\033[36m--AicpuZeroCopyExchanger_ST SetUP--\033[0m" << std::endl;
        // 初始化notifyPool
    ...
    static void TearDownTestCase()
    {
        delete dispatcher;
        notifyPool = nullptr;
        std::cout << "\033[36m--AicpuZeroCopyExchanger_ST TearDown--\033[0m" << std::endl;
    }
...
DispatcherPub * AicpuZeroCopyExchanger_ST::dispatcher = nullptr;
std::unique_ptr<NotifyPool> AicpuZeroCopyExchanger_ST::notifyPool = nullptr;
MachinePara AicpuZeroCopyExchanger_ST::machinePara{};
        ...
        for (int levelIdx = 0; levelIdx < LEVEL_SUB_COMM_NUM; levelIdx++) {
            ...
            for (int i = 0; i < SINGAL_SUB_COMM_NUM; i++) {
                ...
                transDevP2p.reset(new (std::nothrow) Transport(TransportType::TRANS_TYPE_DEVICE_P2P,
                    para,
                    AicpuZeroCopyExchanger_ST::dispatcher,
                    AicpuZeroCopyExchanger_ST::notifyPool,
                    AicpuZeroCopyExchanger_ST::machinePara,
                    transDevP2pData));
...
TEST_F(AicpuZeroCopyExchanger_ST, ExchangeAddress)
{
