...
static struct rs_cb g_tmp_rs_cb0_ab;
static struct rs_cb g_tmp_rs_cb1_ab;
static struct rs_cb g_ping_rs_cb_ab;
static struct rs_ping_ctx_cb g_tmp_ping_cb_ab;
static struct rs_ping_target_info g_ping_target_node_ab;
static struct rs_ping_target_info g_ping_target_node1_ab;

int rs_dev2rscb_stub0_ab(uint32_t dev_id, struct rs_cb **rs_cb, bool init_flag)
{
    *rs_cb = &g_tmp_rs_cb0_ab;

    return 0;
}

int rs_dev2rscb_stub1_ab(uint32_t dev_id, struct rs_cb **rs_cb, bool init_flag)
{
    pthread_mutex_init(&g_tmp_rs_cb1_ab.ping_cb.ping_mutex, NULL);
    g_tmp_rs_cb1_ab.ping_cb.thread_status = RS_PING_THREAD_RUNNING;

    *rs_cb = &g_tmp_rs_cb1_ab;

    return 0;
}

int rs_get_ping_cb_stub_ab(struct ra_rs_dev_info *rdev, struct rs_ping_ctx_cb **ping_cb)
{
    *ping_cb = &g_tmp_ping_cb_ab;
    (*ping_cb)->ping_pong_ops = rs_ping_roce_get_ops();
    (*ping_cb)->ping_pong_dfx = rs_ping_roce_get_dfx();
...
int rs_get_rs_cb_stub_ab(unsigned int phy_id, struct rs_cb **rs_cb)
{
    *rs_cb = &g_ping_rs_cb_ab;
    (*rs_cb)->ping_cb.ping_pong_ops = rs_ping_roce_get_ops();
    (*rs_cb)->ping_cb.ping_pong_dfx = rs_ping_roce_get_dfx();
...
int rs_ping_roce_find_target_node_stub_ab(struct rs_ping_ctx_cb *ping_cb, struct ping_qp_info *target,
    struct rs_ping_target_info **node)
{
    *node = &g_ping_target_node_ab;

    return 0;
}

int rs_ping_roce_find_target_node_stub1_ab(struct rs_ping_ctx_cb *ping_cb, struct ping_qp_info *target,
    struct rs_ping_target_info **node)
{
    *node = &g_ping_target_node1_ab;
...
void tc_rs_abnormal_ping_handle_init()
{
    ...
    mocker_invoke(rs_dev2rscb, rs_dev2rscb_stub0_ab, 1);
...
void tc_rs_abnormal_ping_handle_deinit()
{
    ...
    mocker_invoke(rs_dev2rscb, rs_dev2rscb_stub1_ab, 1);
    ret = rs_ping_handle_deinit(chip_id);
    EXPECT_INT_EQ(ret, 0);
    mocker_clean();

    pthread_mutex_destroy(&g_tmp_rs_cb1_ab.ping_cb.ping_mutex);
}
...
void tc_rs_abnormal_ping_init()
{
    ...
    mocker_invoke(rs_dev2rscb, rs_dev2rscb_stub0_ab, 1);
    g_tmp_rs_cb0_ab.ping_cb.init_cnt = 1;
    ret = rs_ping_init(&attr, &info, &rdev_index);
    EXPECT_INT_NE(ret, 0);
    mocker_clean();

    mocker_invoke(rs_get_rs_cb, rs_get_rs_cb_stub_ab, 1);
...
void tc_rs_abnormal_ping_target_add()
{
    ...
    g_tmp_ping_cb_ab.task_status = RS_PING_TASK_RUNNING;
    mocker_invoke(rs_get_ping_cb, rs_get_ping_cb_stub_ab, 1);
    ret = rs_ping_target_add(&rdev, &target);
    EXPECT_INT_NE(ret, 0);
    mocker_clean();

    g_tmp_ping_cb_ab.task_status = RS_PING_TASK_RESET;
    mocker_invoke(rs_get_ping_cb, rs_get_ping_cb_stub_ab, 1);
    mocker_invoke(rs_ping_roce_find_target_node, rs_ping_roce_find_target_node_stub_ab, 1);
    ret = rs_ping_target_add(&rdev, &target);
    EXPECT_INT_NE(ret, 0);
    mocker_clean();

    mocker_invoke(rs_get_ping_cb, rs_get_ping_cb_stub_ab, 1);
    mocker_invoke(rs_ping_roce_find_target_node, rs_ping_roce_find_target_node_stub1_ab, 1);
...
void tc_rs_abnormal_ping_task_start()
{
    ...
    pthread_mutex_init(&g_tmp_ping_cb_ab.pong_qp.recv_mr_cb.mutex, NULL);
    pthread_mutex_init(&g_tmp_ping_cb_ab.ping_qp.recv_mr_cb.mutex, NULL);
    pthread_mutex_init(&g_tmp_ping_cb_ab.ping_mutex, NULL);
    RS_INIT_LIST_HEAD(&g_tmp_ping_cb_ab.ping_list);
    rs_list_add_tail(&tmp_target.list, &g_tmp_ping_cb_ab.ping_list);
    mocker_invoke(rs_get_ping_cb, rs_get_ping_cb_stub_ab, 1);
    ret = rs_ping_task_start(&rdev, &attr);
    EXPECT_INT_EQ(ret, 0);
    rs_list_del(&tmp_target.list);
    mocker_clean();
    pthread_mutex_destroy(&g_tmp_ping_cb_ab.ping_mutex);
    pthread_mutex_destroy(&g_tmp_ping_cb_ab.ping_qp.recv_mr_cb.mutex);
    pthread_mutex_destroy(&g_tmp_ping_cb_ab.pong_qp.recv_mr_cb.mutex);
}
