...
TEST_F(MC2AicpuInterface_UT, RunAicpuRpcSrvLaunch_RDMAFullmeshPostsend)
{
    ...
    ibv_qp qp_info[2];
    memset(&qp_info, 0, sizeof(qp_info)*2);
    ...
    transDevIbverbsData[0].qpMode = QPMode::INVALID;
    ...
    transDevIbverbsData[1].qpMode = QPMode::INVALID;
    ...
    HccCommResParamTask paramTask;
    memset(&paramTask, 0, sizeof(HccCommResParamTask));
    paramTask.rankNum = 2;
    paramTask.ibverbsData = reinterpret_cast<u64>(transDevIbverbsData.data());
    KFCResInitTask initTask;
    memset(&initTask, 0, sizeof(KFCResInitTask));
    initTask.context = uint64_t(&paramTask);
    // Size不一致，应报错返回
    ...
    std::vector<hccl::Transport::Buffer> remoteBuf{remoteBuf1, remoteBuf1};
    std::vector<hccl::Transport::Buffer> localBuf{localBuf1, localBuf1};
...
static u64 mockCpuTimestamp() {
    static u64 lastTime = 0;  
    const u64 timeOutTime = 300 * 1000000000U;
    lastTime += timeOutTime + 1;
    return lastTime;
}

TEST_F(MC2AicpuInterface_UT, RunAicpuKfcSrvLaunch_Mc2api_SlaveTimeOut)
{
    MOCKER(AdprofCheckFeatureIsOn).stubs().will(returnValue(1));
    StubHccCommRes commRes;
    HccCommResParamTask paramTask = commRes.StubHccCommResParamTask();
    ...
    MOCKER(HcclAicpuUtils::GetCpuId).stubs().will(returnValue(2));
    InitMultiThreadSharedCtx(0);
    MOCKER(GetCurCpuTimestamp).stubs().will(invoke(mockCpuTimestamp));
    ArgsInputApi curArgs = {inputDesc, (void *)&paramTask, (void *)&paramTask, nullptr, &tilingData};
    void *args[sizeof(curArgs)/ sizeof(void *)];
    
    memcpy(args, &curArgs, sizeof(curArgs));
    MOCKER(AicpuKfcBatchwriteProcess::GetVerifiedServerRole).stubs().will(returnValue(AicpuServerRole::SLAVE));

    EXPECT_EQ(9, RunAicpuKfcSrvLaunch(args));
