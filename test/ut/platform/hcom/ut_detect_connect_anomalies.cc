...
{
    MOCKER_CPP(&HcclSocket::GetStatus).stubs().with(_).will(returnValue(HcclSocketStatus::SOCKET_OK));
    ...
    .stubs()
    .will(returnValue(1));
    MOCKER_CPP(&HcclSocket::GetStatus).stubs().with(_).will(returnValue(HcclSocketStatus::SOCKET_ERROR));
...
{
    MOCKER_CPP(&HcclSocket::AddWhiteList).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    HcclIpAddress ipAddr("192.168.1.1");
    HcclIpAddress localIpAddr("192.168.1.2");
    NicType nicType = NicType::DEVICE_NIC_TYPE;
