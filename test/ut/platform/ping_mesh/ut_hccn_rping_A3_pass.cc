...
TEST_F(HccnRping_UT, ut_HccnRpingInit)
{
    ...
    strcpy(initAttr->ip, deviceIp_[devId]);
...
TEST_F(HccnRping_UT, ut_HccnRping)
{
    ...
    target[0].addrType = 0;
    target[0].srcAddr.ip = new char[ipLen_];
    target[0].dstAddr.ip = new char[ipLen_];
    char payload[12] = "hellotarget";
    strcpy(target[0].payload, payload);
    strcpy(target[0].srcAddr.ip, deviceIp_[devserverId]);
    strcpy(target[0].dstAddr.ip, deviceIp_[devId]);
    ...
    delete[] target[0].srcAddr.ip;
    delete[] target[0].dstAddr.ip;
    delete[] target;
}
