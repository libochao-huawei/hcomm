...
protected:
    ...
    virtual void SetUp()
    {
        ...
        .stubs()
        .with(_, _, outBoundP(&ptr, sizeof(ptr)))
        .will(returnValue(HCCL_SUCCESS));
...
TEST_F(PingMesh_UT, ut_PingMeshInit_950)
{
    ...
    u32 num = 1;
    MOCKER(hrtRaGetDevEidInfoNum).stubs().with(_, outBoundP(&num, sizeof(&num)))
        .will(returnValue(HCCL_E_NOT_SUPPORT))
        .then(returnValue(HCCL_SUCCESS));
    MOCKER(hrtRaGetSecRandom).stubs().with(_, outBoundP(&num, sizeof(&num))).will(returnValue(HCCL_SUCCESS));
