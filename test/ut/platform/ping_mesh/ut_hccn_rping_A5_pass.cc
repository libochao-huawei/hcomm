...
TEST_F(HccnRping_UT, ut_HccnRping)
{
    ...
    target[0].addrType = 1;
    target[0].srcAddr.eid = new char[ipLen_];
    target[0].dstAddr.eid = new char[ipLen_];
    char payload[12] = "hellotarget";
    strcpy(target[0].payload, payload);
    strcpy(target[0].srcAddr.eid, device_eid[devserverId]);
    strcpy(target[0].dstAddr.eid, device_eid[devId]);
    ...
    delete[] target[0].srcAddr.eid;
    delete[] target[0].dstAddr.eid;
    delete[] target;
}
...
TEST_F(HccnRping_UT, ut_HccnRping_RpingAddTargetV2)
{
    ...
    initAttr->tc = 0;
    initAttr->addrType = 1;
    initAttr->ip = new char[ipLen_];
    strcpy(initAttr->ip, device_eid[devId]);
    void *pingmesh = nullptr;
