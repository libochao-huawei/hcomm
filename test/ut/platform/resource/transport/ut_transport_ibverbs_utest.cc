...
TEST_F(TransportIbverbs_UT, test_TransportIbverb_Flush_will_return_SUCCESS)
{
    ...
    Stream stream;
    u32 flushTimeout = 120;
    HcclResult ret = ibverbsDev->Flush(stream, flushTimeout);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ...
    ret = ibverbs->Flush(stream, flushTimeout);
