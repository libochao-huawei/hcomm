...
TEST_F(TransportIbverbs_UT, BatchTransferAsync_WriteSuccess)
{
    ...
    transferDescs[0].write.dst = reinterpret_cast<void*>(0x1000);
    transferDescs[0].write.src = reinterpret_cast<void*>(0x2000);
...
TEST_F(TransportIbverbs_UT, BatchTransferAsync_ReadSuccess)
{
    ...
    transferDescs[0].read.dst = reinterpret_cast<void*>(0x2000);
    transferDescs[0].read.src = reinterpret_cast<void*>(0x1000);
    transferDescs[0].read.len = 1024;

    HcclResult ret = ibverbs->BatchTransferAsync(transferDescs, 1, stream);
    GlobalMockObject::verify();
}

TEST_F(TransportIbverbs_UT, BatchTransferAsync_EmptyDescs)
{
    MOCKER_CPP(&TransportDeviceIbverbs::UseMultiQp).stubs().will(returnValue(false));
