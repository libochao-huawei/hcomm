...
TEST_F(TransportP2pNullPtrCheckTest, RxAsync_GetRemoteMemReturnsNull_ReturnError)
{
    ...
    // Mock GetRemoteMem 返回错误（而不是让 remoteInputPtr_ = nullptr）
    MOCKER_CPP(static_cast<HcclResult (TransportP2p::*)(UserMemType, void**)>(&TransportP2p::GetRemoteMem))
        .stubs()
        .with(any(), any())
        .will(returnValue(HCCL_E_PTR));  // 返回指针空错误
    ...
    // 当 GetRemoteMem 返回错误时，RxAsync 应该返回错误
