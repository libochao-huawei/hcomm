...
TEST_F(TransportBoundaryCoverage_UT, ut_transport_ibverbs_tx_payload_boundary)
{
    ...
    u64 dstOffset = 0x1000;
    u64 len = 0x100;
    
    // 测试dstOffset越界：dstOffset > dstMemSize 是非法的
    bool isOffsetValid = (dstOffset > dstMemSize);
    EXPECT_TRUE(isOffsetValid);  // 0x1000 > 0x1000 是false，但这里我们测试越界条件
    
    // 实际检查：dstOffset <= dstMemSize 是合法的
    isOffsetValid = (dstOffset <= dstMemSize);
    EXPECT_TRUE(isOffsetValid);  // 0x1000 <= 0x1000 是true，合法
    
    // 测试dstOffset真正越界的情况
    dstOffset = 0x1001;
    isOffsetValid = (dstOffset <= dstMemSize);
    EXPECT_FALSE(isOffsetValid);  // 0x1001 <= 0x1000 是false，非法
    
    // 测试len越界
    dstOffset = 0x800;
    len = 0x801;
    bool isLenValid = (len <= (dstMemSize - dstOffset));
    EXPECT_FALSE(isLenValid);
    
    // 合法情况
    len = 0x800;
    isLenValid = (len <= (dstMemSize - dstOffset));
    EXPECT_TRUE(isLenValid);
}
