...
extern void *RsNdaDbMmapHostVa(struct doorbell_map_desc *desc);
...
{
    rs_nda_ut_msg("dstPtr :0x%llx\n", (uintptr_t)dstPtr);

    if (dstPtr == NULL) {
        return -1;
    }
    rs_nda_ut_msg("1 2 3..................\n");
    // void *tmp_ptr = (void *)malloc(32);
    // if (tmp_ptr == NULL) {
    //     return -1;
    // }
    rs_nda_ut_msg("1 2 ..................\n");
    *dstPtr = (uintptr_t)0x10;
    rs_nda_ut_msg("1 ..................\n");
    return 0;
}
...
void TcRsNdaDbUnmapHostVa()
{
    ...
    mocker_invoke(DlHalHostRegister, DlHalHostRegisterStub, 10);
    ptr = RsNdaDbMmapHostVa(&desc);
    EXPECT_ADDR_NE(ptr, NULL);
    ptr = NULL;
    ptr = RsNdaDbMmapHostVa(&desc);
    EXPECT_ADDR_NE(ptr, NULL);
    ...
    ret = RsNdaDbUnmapHostVa(&gDstPtr, &desc);
    EXPECT_INT_EQ(ret, 0);
    ret = RsNdaDbUnmapHostVa(&gDstPtr, &desc);
    EXPECT_INT_EQ(ret, 0);
    mocker_clean(); 
