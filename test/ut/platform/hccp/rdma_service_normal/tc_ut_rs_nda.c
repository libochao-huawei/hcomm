...
extern int RsGetNdaDbCb(uint64_t hva, struct NdaDbCb **ndaDbCb);
extern int RsGetNdaGuidCb(uint64_t guidL, uint64_t guidH, struct NdaGuidCb **ndaGuidCb);
...
int DlHalHostUnRegisterStub(void *srcPtr, UINT32 devId)
{
    uint64_t alignHva = AlignDown((uint64_t)1, RS_NDA_4K);
...
void TcRsNdaUbAllocFailed()
{
    ...
    gRsCb->ndaCb->ndaOps.alloc = malloc;
    ptr = RsNdaUbAlloc(size);
    EXPECT_ADDR_EQ(ptr, NULL);

    gRsCb->ndaCb->ndaOps.free = free;
...
void TcRsNdaDbMmapHostVa()
{
    ...
    RS_INIT_LIST_HEAD(&gRsCb->ndaCb->ndaDbHostList);
    mocker_clean();
    ...
    mocker_invoke_p5("DlHalHostRegister", "DlHalHostRegisterStub",
        (stub_fn_t)DlHalHostRegister, (stub_fn_t)DlHalHostRegisterStub, 10);
    mocker_invoke(DlHalHostUnRegister, DlHalHostUnRegisterStub, 10);
    ret = RsNdaDbUnmap(&gDstPtr, &desc);
    EXPECT_INT_EQ(ret, 0);
    ret = RsNdaDbUnmap(&gDstPtr, &desc);
    EXPECT_INT_EQ(ret, 0);
    mocker_clean();
...
void TcRsNdaDbMmapUbRes()
{
    ...
    RS_INIT_LIST_HEAD(&gRsCb->ndaCb->ndaDbGuidList);
    mocker_clean();
    mocker(DlDrvDeviceGetIndexByPhyId, 10, -1);
    ptr = RsNdaDbMmap(&desc);
    EXPECT_ADDR_EQ(ptr, NULL);
