...
{
    struct DevEidInfo infoList[5] = {0};
    unsigned int num = 0;
    int ret = 0;
...
{
    struct RaRsDevInfo devInfo = {0};
    struct AuxInfoIn infoIn;
    struct AuxInfoOut infoOut;
    int ret = 0;

    (void)memset_s(&infoOut, sizeof(struct AuxInfoOut), 0, sizeof(struct AuxInfoOut));
    (void)memset_s(&infoIn, sizeof(struct AuxInfoIn), 0, sizeof(struct AuxInfoIn));
