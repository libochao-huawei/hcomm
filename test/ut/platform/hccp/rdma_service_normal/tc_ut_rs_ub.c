...
void tc_rs_ub_ctx_jetty_destroy_batch()
{
    ...
    // rs_ub_calloc_jetty_batch_info fail
    num = 1;
    ...
    // rs_ub_get_jetty_destroy_batch_info fail
    num = 1;
    ...
    // rs_ub_destroy_jetty_cb_batch fail
    num = 1;
    ...
    // rs_urma_delete_jetty_batch fail
    num = 1;
    jetty_cb_stub.state = RS_JETTY_STATE_CREATED;
    RsListAddTail(&jetty_cb_stub.list, &dev_cb.jetty_list);
    dev_cb.jetty_cnt++;
...
void tc_rs_handle_epoll_poll_jfc()
{
    ...
    mocker_clean();
    ret = rs_handle_epoll_poll_jfc(&dev_cb, &jfce);
    EXPECT_INT_EQ(-EAGAIN, ret);

    mocker(rs_urma_wait_jfc, 1, 2);
    ret = rs_handle_epoll_poll_jfc(&dev_cb, &jfce);
    EXPECT_INT_EQ(-EOPENSRC, ret);
    mocker_clean();
    ...
    mocker(rs_urma_wait_jfc, 1, 1);
    mocker(rs_urma_poll_jfc, 1, 1);
    mocker(rs_ub_get_jetty_cb, 1, -ENODEV);
    mocker(rs_urma_rearm_jfc, 1, 0);
    ret = rs_handle_epoll_poll_jfc(&dev_cb, &jfce);
    EXPECT_INT_EQ(-ENODEV, ret);
    mocker_clean();

    jetty_cb.jetty->jetty_id.id = 0;
    jetty_cb.ci_addr = &ci_addr;
    RS_INIT_LIST_HEAD(&dev_cb.jetty_list);
