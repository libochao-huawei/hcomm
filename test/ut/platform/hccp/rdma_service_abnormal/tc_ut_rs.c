...
int tc_rs_abnormal()
{
	...
	fprintf(stderr, "\n+++++++++ABNORMAL TC Start++++++++\n");
	ret = RsDev2rscb(0, &rs_cb);
	ret = RsDev2rscb(2, &rs_cb);
	ret = rs_get_rscb(&rs_cb);

	ret = RsQpn2qpcb(5, &qp_cb);
...
void tc_rs_qp_create2()
{
	...
	// ret = RsQpCreate(dev_id, rdevIndex, qp_norm, &qpn);
	// EXPECT_INT_NE(ret, 0);
	// mocker_clean();
...
//!!!!!
	...
	qp_cb_t->connInfo->connfd = 99999; //close a non-exist fd
	mocker((stub_fn_t)RsSocketRecv, 1, 0);
...
void tc_rs_mr_abnormal2()
{
	...
	ret = RsQpDestroy(dev_id, rdevIndex, resp2.qpn);
	//ret = RsQpDestroy(dev_id, rdevIndex, qpn);
...
void tc_rs_socket_ops2()
{
	...
	//ret = RsFindSockets(&conn_tmp, &conn_server, 1, 0);
	//EXPECT_INT_EQ(ret, -22);
...
// RsNotifyMrListAdd begin
	...
	//EXPECT_INT_NE(ret, 0);
	mocker_clean();
// RsNotifyMrListAdd end


// RsEpollRecvHandle begin
	cmd = (uint32_t *)buf;
	*cmd = RS_CMD_QP_INFO;
	mocker((stub_fn_t)memcpy_s, 10, 1);
	...
	*cmd = RS_CMD_QP_INFO + 444;
	RsEpollRecvHandle(qp_cb, buf, 64);
// RsEpollRecvHandle end


//RsEpollRecvHandleRemain begin
	mocker((stub_fn_t)memcpy_s, 10, 1);
	RsEpollRecvHandleRemain(qp_cb, total_size, cur_size, flag, buf_tmp);
	mocker_clean();
//RsEpollRecvHandleRemain end
// rs_create_cq begin
