...
/* RaTypicalCqCreate/RaTypicalCqDestroy are defined in ra_host.c with HCCP_ATTRI_VISI_DEF
   but have no header declaration — declare them here for the test */
extern int RaTypicalCqCreate(void *rdevHandle, unsigned int cqDepth, unsigned int *cqn, void **cqHandle);
extern int RaTypicalCqDestroy(void *rdevHandle, unsigned int cqn, void *cqHandle);
