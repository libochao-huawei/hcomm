...
// {
//     tc_adp_env_init();
//     struct MsgHead* head = (struct MsgHead*)add_test_msg(RA_RS_GET_IFADDRS, sizeof(union OpIfaddrData));
//     head->msgDataLen = 0;
//     tc_common_test();
//     tc_adp_env_init();
//     head = (struct MsgHead*)add_test_msg(RA_RS_GET_IFADDRS, sizeof(union OpIfaddrData));
//     head->hostTgid = g_host_tgid + 1;
//     tc_common_test();
//     tc_adp_env_init();
//     head = (struct MsgHead*)add_test_msg(RA_RS_GET_IFADDRS, sizeof(union OpIfaddrData));
...
//     mocker_invoke((stub_fn_t)drvHdcSessionAccept, (stub_fn_t)stub_accept_session, 10);
//     ret = HccpInit(devid, g_host_tgid, HDC_SERVICE_TYPE_RDMA, WHITE_LIST_ENABLE);
//     ret = HccpDeinit(devid);
//     EXPECT_INT_EQ(ret, 0);
