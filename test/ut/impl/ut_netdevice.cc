...
TEST_F(NetDevUt, TestDeviceDeploymentNicAddr) {
    info.netdevDeployment = HcclNetDevDeployment::HCCL_NETDEV_DEPLOYMENT_DEVICE;
    info.isBackup = false;
    MOCKER(Is310PDevice).stubs().with(_).will(returnValue(true));
    MOCKER(GetExternalInputHcclIsTcpMode).stubs().with(_).will(returnValue(true));
    HcclAddress * addr;
    uint32_t addrNum = 0;
...
TEST_F(NetDevUt, TestHostDeploymentRoce) {
    ...
    GlobalMockObject::verify();
    MOCKER(Is310PDevice).stubs().with(_).will(returnValue(false));
    MOCKER(GetExternalInputHcclIsTcpMode).stubs().with(_).will(returnValue(false));
    EXPECT_EQ(HcclNetDevOpen(&info, &netDev), HCCL_SUCCESS);
