...
TEST_F(HcclImplTest, ut_AllocBatchSendRecvLinks_When_Normal_Return_HCCL_SUCCESS)
{
    ...
    MOCKER(hrtProfRegisterCtrlCallback).stubs().with(_).will(returnValue(HCCL_SUCCESS));
   ...
   // stubs in CreateLink
    ...
    MOCKER_CPP(&TransportManager::CheckBatchSendRecvLinkStatus).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclImplTest, ut_AllocBatchSendRecvLinks_When_GroupModeAndAllocSliceMemFailed_Return_HCCL_E_INTERNAL)
{
    ...
    MOCKER_CPP(&TransportManager::GetIOMem).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER(hrtSetDevice).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&TransportManager::AllocSliceMem).stubs().with(_).will(returnValue(HCCL_E_INTERNAL));
    MOCKER(hrtResetDevice).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclImplTest, ut_AllocBatchSendRecvLinks_When_hrtSetDevice_Failed_Return_HCCL_E_INTERNAL)
{
    ...
    MOCKER(hrtSetDevice).stubs().with(_).will(returnValue(HCCL_E_RUNTIME));
...
TEST_F(HcclImplTest, ut_AllocBatchSendRecvLinks_When_CreateSingleLinkSocket_Failed_Return_HCCL_E_INTERNAL)
{
   ...
   // stubs in CreateDestSockets
    ...
    MOCKER(Is310PDevice).stubs().will(returnValue(false));
    MOCKER_CPP(&HcclSocketManager::CreateSingleLinkSocket).stubs().with(_).will(returnValue(HCCL_E_INTERNAL));
