...
{
    MOCKER_CPP(&HcclCommunicator::SetMemoryRange).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::ActivateCommMemory).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::DeactivateCommMemory).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::UnsetMemoryRange).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::InitZeroCopyMemoryAgent).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::DeinitZeroCopyMemoryAgent).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclImplTest, ut_hcclimpl_AiCpuSetCommResource_MultiServer)
{
    ...
    MOCKER_CPP_VIRTUAL(transportBase, &TransportBase::GetRemoteMemSize)
        .stubs()
        .with(_, outBound(commBufferSize))
        .will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclImplTest, ut_AllocTransport910B)
{
    ...
    MOCKER_CPP(&NotifyPool::RegisterOp).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&NotifyPool::UnregisterOp).stubs().with(_).will(returnValue(HCCL_SUCCESS));
   ...
   // stubs in CreateDestSockets
    MOCKER_CPP(&TransportManager::UpdateIsInterRdma).stubs().with(_, outBound(false), _).will(ignoreReturnValue());
    MOCKER_CPP(&TransportManager::MakeRemoteLinkInfo).stubs().will(returnValue(HCCL_SUCCESS));

   // stubs in CreateLink
    MOCKER(hrtErrMSetErrorContextPub).stubs().with(_).will(ignoreReturnValue());
    MOCKER(hrtSetDevice).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&TransportManager::TransportInit).stubs().with(_).will(returnValue(HCCL_SUCCESS));

    MOCKER(hrtResetDevice).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclImplTest, ut_AllocTransport910C)
{
    ...
    MOCKER_CPP(&TransportManager::GetIOMem).stubs().with(_).will(returnValue(HCCL_SUCCESS));
   ...
   // stubs in CreateDestSockets
    ...
    MOCKER(hrtGetPairDeviceLinkType).stubs().with(_, _, outBound(LinkTypeInServer::HCCS_SW_TYPE)).will(returnValue(HCCL_SUCCESS));

    MOCKER(Is310PDevice).stubs().will(returnValue(false));
    MOCKER_CPP(&HcclSocketManager::CreateSingleLinkSocket).stubs().with(_).will(returnValue(HCCL_SUCCESS));
   ...
   // stubs in CreateLink
    ...
    MOCKER(hrtResetDevice).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&TransportManager::checkSubCommLinkThreadsStatus).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclSocketManager::DestroySockets, void(HcclSocketManager::*)(const std::string&))
    .stubs().with(_).will(ignoreReturnValue());
...
TEST_F(HcclImplTest, ut_TransportMgrConstructTransTag)
{
    ...
    .with(_, _, outBound(LinkTypeInServer::SIO_TYPE))
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclImplTest, ut_Mc2CreateAndLaunchContext_multi_server)
{
    ...
    MOCKER(hrtAicpuKernelLaunchExWithArgs).stubs().with(_).will(returnValue(HCCL_SUCCESS));
