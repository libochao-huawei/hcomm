...
TEST_F(LinkPcieTest, ut_SpecifyLink)
{
    ...
    MOCKER(hrtGetPairDeviceLinkType).stubs().with(_, _, outBound(linkType)).will(returnValue(HCCL_SUCCESS));
    ...
    MOCKER(aclrtIpcMemSetAttr).stubs().with(_).will(returnValue(1));
    ret = hrtIpcSetMemoryAttr(&name, ACL_RT_IPC_MEM_ATTR_ACCESS_LINK, 1);
