...
TEST_F(HeartBeatTest, ut_HeartBeatTest5)
{
    ...
    MOCKER(hrtGetDeviceIndexByPhyId).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&Heartbeat::IsEnableBackupLink).stubs().with(_).will(returnValue(true));
    MOCKER_CPP(&Heartbeat::GetConnectRank).stubs().with(_).will(returnValue(0));
    ...
    MOCKER_CPP(&Heartbeat::Init).stubs().with(_).will(returnValue(0));
...
TEST_F(HeartBeatTest, ut_ProcessCqeErrInfo_008)
{
    ...
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(_).will(returnValue(true));
    TransportIbverbs::g_flag = true;
...
{
    // 未开启重执行分支 
    MOCKER_CPP(&Heartbeat::GetRetryEnable).stubs().with(_).will(returnValue(false));
