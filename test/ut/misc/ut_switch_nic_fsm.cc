...
TEST_F(SwitchNicFsmTestCase, ut_Agent_SwitchNicSuc)
{
    ...
    opInfo.execStatus.kfcStatus = KfcStatus::kPlanSwitch;
    MOCKER_CPP(&OpRetryBase::GetOpExecInfo).stubs().with(_, outBound(opInfo)).will(returnValue(HCCL_SUCCESS));
    ...
    MOCKER_CPP(&OpRetryBase::GetSwitchRanks).stubs()
        .with(_, outBound(needCheckDefaultNic), outBound(needCheckBackupNic))
        .will(returnValue(HCCL_SUCCESS));
    RetryCommand command = RETRY_CMD_NOTIFY_SWITCH_SUC;
    MOCKER_CPP(&OpRetryBase::WaitCommand).stubs().with(_, outBound(command)).will(returnValue(HCCL_SUCCESS));
...
TEST_F(SwitchNicFsmTestCase, ut_Server_SwitchNicSuc)
{
    ...
    MOCKER_CPP(&OpRetryBase::WaitActiveSwitchInfo).stubs().with(_).will(invoke(WaitActiveSwitchInfo_Stub));
...
TEST_F(SwitchNicFsmTestCase, ut_server_CollectSingle)
{
    ...
    MOCKER_CPP(&OpRetryBase::WaitResponse).stubs().with(_, outBound(retryInfo)).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&OpRetryBase::IssueCommand).stubs().will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&OpRetryBase::Recv).stubs().with(_)
