...
public:
    static void SetUpTestCase()
    {
        ...
        MOCKER(hrtMemSyncCopy).stubs().with(_).will(invoke(stub_hrtMemSyncCopy));
        MOCKER(hrtMemSet).stubs().with(_).will(invoke(stub_hrtMemSet));
        MOCKER(hrtMalloc).stubs().with(_).will(invoke(stub_hrtMalloc));
        MOCKER(hrtFree).stubs().with(_).will(invoke(stub_hrtFree));
    }
