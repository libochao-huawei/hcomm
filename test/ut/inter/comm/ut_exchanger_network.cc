...
{
    MOCKER(HrtRaRdmaInitWithAttr).stubs().with(any()).will(invoke(stub_HrtRaRdmaInitWithAttr));

    MOCKER(HrtRaRdmaInit).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
...
TEST_F(ExchangerNetworkTest, ut_init_input_invalid)
{
    ...
    const std::string tag("default_tag");
    const std::string illegal_tag(
        "12345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890"
        "12345678901234567890_tag");
