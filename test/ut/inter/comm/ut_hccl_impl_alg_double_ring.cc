...
protected:
    ...
    virtual void SetUp()
    {
        ...
        MOCKER_CPP(&Stream::ptr).stubs().with(any()).will(returnValue((void*)0x12345678));
        MOCKER_CPP(&Stream::IsMainStream).stubs().with(any()).will(returnValue(false));
...
TEST_F(HcclImplAlgTestDoubleRing, ut_AllGatherDoubleRingExecutor_Ring_SuperPod)
{
    ...
    tmpComm.commLevel1[0].reset(new (std::nothrow) CommRing(
        ...
        paraVector, inputMem, outputMem, false, nullptr, 0));
    ...
    for (int i = 0; i < 2; i++) {
        tmpComm.commLevel0[i].reset(new (std::nothrow) CommRing(
            ...
            exchanger, paraVector, inputMem, outputMem, false, nullptr, 0));
