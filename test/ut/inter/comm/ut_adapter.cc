...
TEST_F(RuntimeTest, EventTest610)
{
    ...
    InitEnvParam();
    MOCKER(CompareDevType).stubs().with(any(), eq(DevType::DEV_TYPE_310P1)).will(returnValue(true));
...
{
    MOCKER(rtGetNotifyAddress).stubs().with(any(), any()).will(returnValue(RT_ERROR_NONE));
...
{
    MOCKER(aclrtGetNotifyId).stubs().with(any(), any()).will(returnValue(ACL_SUCCESS));
...
TEST_F(RuntimeTest, ut_hrtGetPairDevicePhyId_16)
{
    ...
    MOCKER(hrtGetPairDeviceLinkType).stubs().with(any(), any(), outBound(linkTypeHCCS)).will(returnValue(HCCL_SUCCESS));
