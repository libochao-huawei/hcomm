...
{
    MOCKER_CPP(&PreemptPortManager::Release).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
{
    MOCKER_CPP(&HcclCommunicatorAttrs::GetInlineReduceSwitchOn).stubs().with(_).will(returnValue(true));
    MOCKER_CPP(&HcclCommunicatorAttrs::GetMeshAggregationRankSize).stubs().with(_).will(returnValue(1));
    MOCKER_CPP(&HcclCommunicatorAttrs::GetUsedRdmaLevel0).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(&HcclCommunicator::SetWorldGroupInfo).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER(SetRetryEnable).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER(IsHostUseDevNic).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclCommTest, ut_InitRaResource_notSupportChangelink)
{
    ...
    MOCKER_CPP(&HcclCommunicator::InitNic).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER(hrtGetPairDevicePhyId).stubs().with(_).will(returnValue(HCCL_SUCCESS));

    // rts无法访问备用die（客户自定义） -> 不支持借轨
    MOCKER(hrtGetDeviceIndexByPhyId).stubs().with(_).will(returnValue(HCCL_E_RUNTIME));
    // 网络资源初始化失败（不初始化备用资源）
...
TEST_F(HcclCommTest, hcclComm_InitRaResource_devVnicSocket_branch)
{
    ...
    MOCKER(Is310PDevice).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(&HcclCommunicator::IsEnableBackupLink).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(& HcclCommunicator::InitSocketManager).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::IsEnableRoce).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(&HcclSocketManager::ServerInit).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclSocketManager::ServerDeInit, HcclResult(HcclSocketManager::*)(const HcclNetDevCtx, u32)).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::GenSupportRdmaLite).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::GetSupportRdmaLite).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(&HcclCommunicator::ReleasePreemptSocket).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclCommTest, hcclComm_InitNic_IsEnableBackupLink_branch1)
{
    ...
    MOCKER_CPP(&HcclCommunicator::IsEnableBackupLink).stubs().with(_).will(returnValue(true));
    MOCKER_CPP(&HcclSocketManager::ServerInit).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclSocketManager::ServerDeInit, HcclResult(HcclSocketManager::*)(const HcclNetDevCtx, u32)).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::SetNeedInitNicFlag).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER(Is310PDevice).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(&HcclCommunicator::ReleasePreemptSocket).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
{
    MOCKER_CPP(&HcclCommunicatorAttrs::SethbRankInfo).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::TransformRankList).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::SetServerNum).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::SetModuleInfo).stubs().with(_).will(returnValue(HCCL_SUCCESS));

    MOCKER_CPP(&HcclCommunicatorAttrs::SetSuperPodInfo).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::SetLocalRankInfoSubGroup).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::SetInterModeInSuperPod).stubs().with(_).will(returnValue(HCCL_SUCCESS));

    MOCKER_CPP(&HcclCommunicatorAttrs::UpdateNicList).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::CheckLocalRankInfo).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::CalAndSetMeshAggRankSize).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicatorAttrs::IsEnableRoce).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(&HcclCommunicatorAttrs::SetWorldGroupInfo).stubs().with(_).will(returnValue(HCCL_SUCCESS));

    MOCKER(HcclCheckLogLevel).stubs().with(_).will(returnValue(false));
    MOCKER(IsHostUseDevNic).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    ...
    MOCKER_CPP(&HcclCommunicatorAttrs::SetInnerServerAverageDevice,
        ...
        .stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcclCommTest, hcclComm_AllGatherV_call)
{
    ...
    MOCKER_CPP(&HcclCommunicator::ExecOp).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCallbackTask::CallbackRegStream).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::StreamIsCapture).stubs().with(_).will(returnValue(false));
