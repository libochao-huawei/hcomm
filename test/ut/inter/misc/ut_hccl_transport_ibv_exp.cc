...
TEST_F(LinkIbvExpTest, ut_function_for_batchsendrecv_ibv)
{
    ...
    MOCKER_CPP(
        &TransportIbverbs::RdmaSendAsyncHostNIC, HcclResult (TransportIbverbs::*)(std::vector<WqeInfo>&, Stream&))
        .stubs()
        .will(returnValue(HCCL_SUCCESS));
...
TEST_F(LinkIbvExpTest, ut_transport_ibv_multi_qp_configpath)
{
    ...
    TransportManager transManager(
        ...
        nicDeployment, isHaveCpuRank, transportResourceInfoAddr, transportResourceInfoSize, isUseRankPort,
        isUsedRdmaOuter, ranksPort, ranksVnicPort, useSuperPodMode, devIpAddr, hostIp, localVnicIp, netDevCtxMap);
    ...
    MOCKER_CPP(&TransportIbverbs::CreateOneQp).stubs().with(any()).will(returnValue(HCCL_SUCCESS));
    ...
    MOCKER(GetWorkflowMode).stubs().with(any()).will(returnValue(HcclWorkflowMode::HCCL_WORKFLOW_MODE_RESERVED));
...
TEST_F(LinkIbvExpTest, ut_transport_ipc_memory_error)
{
    ...
    MOCKER_CPP(&TransportManager::TransportInit).stubs().with(any()).will(returnValue(HCCL_E_MEMORY));
...
TEST_F(LinkIbvExpTest, ut_BatchSendRecv_2rank_unflod)
{
    ...
    TransportManager transManager(
        cclBufferManager, socketManager, dispatcher0, notifyPool0, rankInfoList, userRank, identifier, deviceLogicId,
        nicDeployment, isHaveCpuRank, transportResourceInfoAddr, transportResourceInfoSize, isUseRankPort,
        isUsedRdmaOuter, nicRanksPort, vnicRanksPort, useSuperPodMode, devIpAddr, hostIp, localVnicIp, netDevCtxMap);

    TransportType type = TransportType::TRANS_TYPE_IBV_EXP;
    MOCKER_CPP(&TransportManager::GetTransportType).stubs().with(any()).will(returnValue(type));
