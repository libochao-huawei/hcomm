...
TEST_F(HcclCommAicpuTest_UT, CommunicatorCustomTest)
{
    ...
    MOCKER_CPP(&HcclCommunicator::BuildOpResParam).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::BuildCustomOpResParam).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::SetMC2EnvFlag).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER(hcclStreamSynchronize).stubs().with(_).will(returnValue(HCCL_SUCCESS));
