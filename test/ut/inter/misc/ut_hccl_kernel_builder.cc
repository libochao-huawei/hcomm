...
TEST_F(HcomKernelBuilderTest, ut_CalcOpRunningParam_common_51)
{
    ...
    u32 numHccsLink = 0;
    MOCKER(HcomGetHccsLinkNum).stubs().with(_, outBound(numHccsLink)).will(returnValue(HCCL_SUCCESS));
    u32 rankSize = 2;
    MOCKER(HcomGetRankSize).stubs().with(_, outBoundP(&rankSize)).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelBuilderTest, ut_getOpWorkspaceMemSize)
{
    ...
    .stubs()
    .with(_, outBoundP(&rankSize, sizeof(rankSize)))
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelBuilderTest, ut_CalcOpTaskNum_1server_stream)
{
    ...
    .stubs()
    //.with(_)
    .with(_, outBound(deviceNumPerServer), outBound(serverNum), _)
    .will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomKernelBuilderTest, ut_GenerateTaskAivCoreLimit)
{
    ...
    MOCKER_CPP(&HcomOpsKernelBuilder::GenerateTaskPrivateDef).stubs().with(_, spy(privateDef), _, _).will(returnValue(HCCL_SUCCESS));
