...
{
    printf("ut_CleanCaptureRes_WhenExpect_Success\n");
    MOCKER_CPP(&HcclCommunicator::ClearOpResource)
    .stubs()
    .will(returnValue(HCCL_SUCCESS));

    printf("ut_CleanCaptureRes_WhenExpect_Success 2\n");
    // std::string newTag = "tag";
    // OpParam opParam;
    // HcclResult ret = AclgraphCallback::GetInstance().InsertNewTagToCaptureResMap(&communicator1_, newTag, opParam);
    // EXPECT_EQ(ret, HCCL_SUCCESS);
    printf("ut_CleanCaptureRes_WhenExpect_Success 3\n");
    u64 modelId = 1;
    ret = AclgraphCallback::GetInstance().CleanCaptureRes(modelId);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    GlobalMockObject::verify();
}
