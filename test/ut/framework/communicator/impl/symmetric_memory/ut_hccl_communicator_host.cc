...
TEST_F(HcclCommunicatorHostTest, Ut_InitSymmetricMemory_When_Normal_Expect_ReturnIsHCCL_SUCCESS) {
    ...
    EXPECT_NE(hcclCommunicator->allGatherManager_, nullptr);
...
TEST_F(HcclCommunicatorHostTest, Ut_InitSymmetricMemory_When_StrideIsValid_Expect_ReturnsIsHCCL_SUCCESS) {
    ...
    config.symmetricMemoryStride = 8;
    hcclCommunicator->commConfig_.SetConfigSymmetricMemoryStride(config);
...
TEST_F(HcclCommunicatorHostTest, Ut_InitSymmetricMemory_When_SuperPodNumGreaterThanOne_Expect_ReturnIsHCCL_SUCCESS) {
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    // assert not created
    EXPECT_EQ(hcclCommunicator->allGatherManager_, nullptr);
    EXPECT_EQ(hcclCommunicator->symmetricMemory_, nullptr);
}

TEST_F(HcclCommunicatorHostTest, Ut_InitSymmetricMemory_When_AclrtGetMemInfoFailure_Expect_ReturnsIsHCCL_E_PARA) {
    MOCKER(&aclrtGetMemInfo).stubs().will(returnValue(ACL_ERROR_INTERNAL_ERROR));
    std::unique_ptr<HcclCommunicator> hcclCommunicator(new (std::nothrow) HcclCommunicator());
    HcclResult ret = hcclCommunicator->InitSymmetricMemory();
...
TEST_F(HcclCommunicatorHostTest, Ut_InitSymmetricMemory_When_StrideTooLarge_Expect_ReturnsIsHCCL_E_PARA) {
    std::unique_ptr<HcclCommunicator> hcclCommunicator(new (std::nothrow) HcclCommunicator());
    CommConfigHandle config;
    config.symmetricMemoryStride = 1024;
    hcclCommunicator->commConfig_.SetConfigSymmetricMemoryStride(config);
    HcclResult ret = hcclCommunicator->InitSymmetricMemory();
    EXPECT_EQ(ret, HCCL_E_PARA);
    EXPECT_EQ(hcclCommunicator->allGatherManager_, nullptr);
...
TEST_F(HcclCommunicatorHostTest, Ut_IsSupportSymmetricMemory_When_Normal_Expect_ReturnIsTrue) {
    ...
    OpParam opParam;
    opParam.inputWindow = reinterpret_cast<void*>(0x1000);
    opParam.outputWindow = reinterpret_cast<void*>(0x2000);
    opParam.aicpuUnfoldMode = true;

    bool ret = hcclCommunicator->IsSupportSymmetricMemory(opParam);
    EXPECT_EQ(ret, true);
    GlobalMockObject::verify();
}
