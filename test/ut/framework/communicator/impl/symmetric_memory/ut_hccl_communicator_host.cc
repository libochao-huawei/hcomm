...
TEST_F(HcclCommunicatorHostTest, Ut_HcclGetAlgExecParam_When_Normal_Expect_ReturnHCCL_SUCCESS) {
    ...
    hcclCommunicator->pimpl = new CommunicatorImpl();
    hcclCommunicator->pimpl->myRank = 0;
    hcclCommunicator->pimpl->rankSize = 2;

    HcclResult ret = hcclCommunicator->InitSymmetricMemory();
    EXPECT_EQ(ret, HCCL_SUCCESS);

    MOCKER_CPP(&hrtMalloc).stubs().will(invoke(invocation {
        void** ptr, u64 size, rtMemType_t type) -> HcclResult {
        *ptr = malloc(size);
        return HCCL_SUCCESS;
    }));
    ...
    HcclResult initParaRet = hcclCommunicator->InitPara();
    if (initParaRet != HCCL_SUCCESS) {
        free(hcclCommunicator->notifyPool_.get());
        free(hcclCommunicator->workSpaceRes_.get());
        delete hcclCommunicator->pimpl;
        EXPECT_EQ(initParaRet, HCCL_SUCCESS);
        return;
    }
    ...
    u64 count = 1024;
    void* inputPtr = malloc(1024);
    void* outputPtr = malloc(1024);
    ...
    ret = hcclCommunicator->HcclGetAlgExecParam(tag, opType, count, inputPtr, outputPtr,
