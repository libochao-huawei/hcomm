...
TEST_F(AicpuCommunicatorTest, Ut_ExecOp_When_SymMemEnabled_Expect_ReturnIsHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&HcclCommAicpu::Orchestrate).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommAicpu::PrepareSymmetricMemory).stubs().with(_).will(returnValue(HCCL_E_INTERNAL));
    MOCKER_CPP(&DeviceMem::create).stubs().with(_).will(returnValue(DeviceMem()));
