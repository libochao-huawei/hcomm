...
{  
    ...
    MOCKER_CPP(&SymmetricMemory::RegisterSymmetricMem)
        .stubs()
        .with(_, _, outBoundP(&mockDevWin, sizeof(mockDevWin)))
        .will(returnValue(HCCL_SUCCESS));
    std::vector<RankInfo> rankInfoList(2);
    HcclIpAddress localVnicIp;
    std::shared_ptr<SymmetricMemoryAgent> symmetricMemoryAgent_ = std::make_shared<SymmetricMemoryAgent>(nullptr, 0,
...
{  
    ...
    MOCKER_CPP(aclrtMemGetAddressRange)
        .stubs()
        .with(_, outBoundP(&ptr, sizeof(ptr)), outBoundP(&bsize, sizeof(bsize)))
        .will(returnValue(ACL_SUCCESS));
    MOCKER_CPP(aclrtReserveMemAddressNoUCMemory)
        .stubs()
        .with(outBoundP(&heapBase_, sizeof(heapBase_)), _, _, _, _)
        .will(returnValue(ACL_SUCCESS));
    ...
    MOCKER_CPP(&SymmetricMemoryAgent::ExchangeInfo)
        .expects(once())
        .with(_, outBoundP((void*)remoteShareablePids, sizeof(remoteShareablePids)), eq((u64)sizeof(int32_t)))
        .will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&SymmetricMemoryAgent::ExchangeInfo)
        .expects(once())
        .with(_, outBoundP((void*)remoteShareableInfos, sizeof(remoteShareableInfos)), eq((u64)sizeof(ShareableInfo)))
        .will(returnValue(HCCL_SUCCESS));
...
{  
    ...
    MOCKER_CPP(aclrtMemGetAddressRange)
        .stubs()
        .with(eq(ptr+3), outBoundP(&ptr, sizeof(ptr)), _)
        .will(returnValue(ACL_SUCCESS));
