...
HcclResult stub_AllGatherManagerTest()
{
    return HCCL_SUCCESS;
}
...
{  
    ...
    std::shared_ptr<AllGatherManager> allGatherManager_ = std::make_shared<AllGatherManager>(nullptr, 0,
        0, localVnicIp, rankInfoList, 0, true, "1");
    SymmetricMemory symmetricMemory(0, 2, 4096, allGatherManager_);
    HcclResult ret = symmetricMemory.Init(); // 1MB
    EXPECT_EQ(ret, HCCL_E_PARA);
}
...
TEST_F(SymmetricMemoryTest, ut_Init_When_AllGatherManagerInitFails_Expect_ReturnHCCL_E_PARA)
{  
    MOCKER_CPP(&AllGatherManager::Init)
        .stubs()
        .with()
...
TEST_F(SymmetricMemoryTest, ut_Init_When_AllGatherFails_Expect_ReturnHCCL_E_INTERNAL)
{  
    MOCKER_CPP(&AllGatherManager::AllGather)
        .stubs()
        .with()
