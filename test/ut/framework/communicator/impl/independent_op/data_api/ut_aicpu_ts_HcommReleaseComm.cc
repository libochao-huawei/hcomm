...
/**
 * @test Ut_HcommReleaseComm_When_CommId_IsNull_Expect_ReturnHCCL_E_PARA
 * @brief 测试通信ID为空时的错误处理
 *
 * 测试场景：使用空指针作为通信ID调用HcommReleaseComm
 * 预期结果：返回HCCL_E_PARA，表示检测到无效参数
 */
TEST_F(UtAicpuTsHcommReleaseComm, Ut_HcommReleaseComm_When_CommId_IsNull_Expect_ReturnHCCL_E_PARA)
{
