...
class AllReduceCorrectnessExecutorProbe : public CollAllReduceRingLayeredExecutor {
public:
    explicit AllReduceCorrectnessExecutorProbe(const HcclDispatcher dispatcher, std::unique_ptr<TopoMatcher> &topoMatcher)
...
protected:
        ...
        const HcomCollOpInfo *) override
    {
        ...
        int values[8] = {101, 102, 201, 202, 301, 302, 401, 402};
        for (int i = 0; i < 8; ++i) {
            buffer[i] = values[i];
        }
        return HCCL_SUCCESS;
        ...
        const HcomCollOpInfo *) override
    {
        ...
        int *buffer = static_cast<int *>(outputMem.ptr());
        int values[8] = {1001,1002,1003,1004,1005,1006,1007,1008};
        for (int i = 0; i < 8; ++i) {
            buffer[i] = values[i];
        }
        return HCCL_SUCCESS;
...
class AllReduceLayeredCorrectnessApiTest : public BaseInit {
public:
    void SetUp() override
    {
        BaseInit::SetUp();
...
protected:
    ...
    {
        OpParam param;
        param.tag = "all_reduce_layered_correctness_ut";
...
TEST_F(AllReduceLayeredCorrectnessApiTest,
    Ut_KernelRun_When_CanonicalNonDegenerateCase_Expect_FinalOutputMem1001To1008)
{
    ...
    AllReduceCorrectnessExecutorProbe executor(reinterpret_cast<HcclDispatcher>(&dispatcher), topoMatcher);
