...
TEST_F(GetOpScratchMemSizeTest, Ut_GetOpScratchMemSize_When_ReduceScatterNormal_Expect_Success) {
    ...
    MOCKER_CPP(HcomGetCommByGroup).stubs().andReturn([](const char *group, std::shared_ptr<hccl::hcclComm> &hcclComm) {
        auto mockComm = std::make_shared<hccl::hcclComm>();
        MOCKER_CPP(&hccl::hcclComm::GetOnlyAivModeConfig).stubs().will(returnValue(HCCL_SUCCESS));
        hcclComm = mockComm;
        return HCCL_SUCCESS;
    });
...
TEST_F(GetOpScratchMemSizeTest, Ut_GetOpScratchMemSize_When_ReduceScatterDeterministic_Expect_DoubleMemSUCCESS) {
    // Mock HcomGetCommByGroup 并设置有效的 hcclComm
    MOCKER_CPP(HcomGetCommByGroup).stubs().andDo([](const char *group, std::shared_ptr<hccl::hcclComm> &hcclComm) {
        auto mockComm = std::make_shared<hccl::hcclComm>();
        MOCKER_CPP(&hccl::hcclComm::GetOnlyAivModeConfig).stubs().will(returnValue(HCCL_SUCCESS));
        hcclComm = mockComm;
        return HCCL_SUCCESS;
...
TEST_F(GetOpScratchMemSizeTest, Ut_GetOpScratchMemSize_When_AllToAllNormal_Expect_Success) {
    // Mock HcomGetCommByGroup 并设置有效的 hcclComm
    MOCKER_CPP(HcomGetCommByGroup).stubs().call([](const char *group, std::shared_ptr<hccl::hcclComm> &hcclComm) {
        auto mockComm = std::make_shared<hccl::hcclComm>();
        MOCKER_CPP(&hccl::hcclComm::GetOnlyAivModeConfig).stubs().will(returnValue(HCCL_SUCCESS));
        hcclComm = mockComm;
        return HCCL_SUCCESS;
...
TEST_F(GetOpScratchMemSizeTest, Ut_GetOpScratchMemSize_When_AllReduceSmallData_Expect_SUCCESS) {
    // Mock HcomGetCommByGroup 并设置有效的 hcclComm
    MOCKER_CPP(HcomGetCommByGroup).stubs().call([](const char *group, std::shared_ptr<hccl::hcclComm> &hcclComm) {
        // 创建一个 mock hcclComm 对象
        auto mockComm = std::make_shared<hccl::hcclComm>();
        // Mock GetOnlyAivModeConfig 方法
        MOCKER_CPP(&hccl::hcclComm::GetOnlyAivModeConfig).stubs().will(returnValue(HCCL_SUCCESS));
        hcclComm = mockComm;
        return HCCL_SUCCESS;
    });
