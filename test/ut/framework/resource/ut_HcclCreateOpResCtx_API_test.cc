...
TEST_F(HcclCreateOpResCtxTest, ut_HcclCreateOpResCtx_When_Normal_Expect_ReturnIsHCCL_SUCCESS)
{
    ...
    uint64_t count = 256;
    char algConfig[128] = "AllGather=level0:ring";
...
TEST_F(HcclCreateOpResCtxTest, ut_HcclCreateOpResCtx_When_ParamIsNullptr_Expect_ReturnIsHCCL_E_PTR)
{
    ...
    HcclResult result = HcclCreateOpResCtxInner(comm, opType, srcDataType, dstDataType, reduceType, count, nullptr, engine, &ctx);
    EXPECT_EQ(result, HCCL_E_PTR);

    HcclResult result = HcclCreateOpResCtxInner(comm, opType, srcDataType, dstDataType, reduceType, count, algConfig, engine, nullptr);
    EXPECT_EQ(result, HCCL_E_PTR);
