...
// Tripwire: 当前生产 ccumDfxInfoV2 与 RTS panicLog ABI 不一致（132B vs 128B），
// 已知存在 4 字节越界读，待 RTS 协议对齐后再启用强约束断言。
// 此处先固化当前现状，任一边变化都会触发编译期失败，提示重新评估 OOB 风险。
// TODO(ABI-align): 协议对齐后改为
//     static_assert(sizeof(ccumDfxInfoV2) <= MAX_CCU_EXCEPTION_INFO_SIZE, ...);
static_assert(sizeof(TestCcumDfxInfoV2) == 132,
              "TestCcumDfxInfoV2 size drifted; sync mirror with production ccumDfxInfoV2");
static_assert(MAX_CCU_EXCEPTION_INFO_SIZE == 128,
              "panicLog ABI changed; re-evaluate V2 OOB risk before relaxing this tripwire");
...
static_assert(sizeof(CcuLoopContext)      == CCU_CTX_RAW_CAPACITY,
              "CcuLoopContext size must match CCU raw ABI (32 x u16 = 64B)");
static_assert(sizeof(CcuLoopContextV2)    == CCU_CTX_RAW_CAPACITY,
              "CcuLoopContextV2 size must match CCU raw ABI (32 x u16 = 64B)");
...
TEST_F(CcuTaskExceptionDecodeTest, Ut_PrintCcumDfxInfoV1_WhenDeviceTypeIs950_ExpectReadableString)
{
    ...
    const std::string expected =
        " SQE_RECV_CNT[11] SQE_SEND_CNT[12] MISSION_DFX[13] "
...
TEST_F(CcuTaskExceptionDecodeTest, Ut_PrintCcumDfxInfoV2_WhenDeviceTypeIs910B_ExpectReadableString)
{
    ...
    static_assert(sizeof(TestCcumDfxInfoV2) >= MAX_CCU_EXCEPTION_INFO_SIZE,
                  "reinterpret_cast below requires the test mirror to cover panicLog");
    ...
    const std::string expected =
        " SQE_RECV_CNT[21] SQE_SEND_CNT[22] MISSION_DFX[23] "
...
TEST_F(CcuTaskExceptionDecodeTest, Ut_PrintCcumDfxInfoV2_WhenPanicLogAtPageBoundary_ExpectOOBReadDeath)
{
    ...
    uint8_t* panicLog =
        static_cast<uint8_t*>(base) + pageSize - MAX_CCU_EXCEPTION_INFO_SIZE;
    ...
    EXPECT_DEATH(
        {
            ...
            MOCKER(hrtGetDeviceType)
                .stubs()
                .with(outBound(DevType::DEV_TYPE_910B))
                .will(returnValue(HCCL_SUCCESS));
            const CcuVersionOps* ops = nullptr;
            (void)GetCcuOps(ops);
            std::ostringstream oss;
...
{
    size_t copiedLen = 0;
    EXPECT_EQ(CcuTaskException::GetCcuMissionContextRaw(0, 0, 0, nullptr,
                  CCU_CTX_RAW_CAPACITY, copiedLen),
              HCCL_E_PARA);
    EXPECT_EQ(copiedLen, 0U);
}
...
TEST_F(CcuTaskExceptionDecodeTest, Ut_GetCcuMissionContextRaw_WhenBufTooSmall_ExpectParaError)
{
    ...
    size_t copiedLen = 0;
    EXPECT_EQ(CcuTaskException::GetCcuMissionContextRaw(0, 0, 0, buf, sizeof(buf), copiedLen),
              HCCL_E_PARA);
    EXPECT_EQ(copiedLen, 0U);
}
...
TEST_F(CcuTaskExceptionDecodeTest, Ut_GetCcuLoopContextRaw_WhenBufTooSmall_ExpectParaError)
{
    uint8_t buf[CCU_CTX_RAW_CAPACITY - 1] = {0};
    size_t copiedLen = 0;
    EXPECT_EQ(CcuTaskException::GetCcuLoopContextRaw(0, 0, 0, buf, sizeof(buf), copiedLen),
              HCCL_E_PARA);
    EXPECT_EQ(copiedLen, 0U);
}
...
TEST_F(CcuTaskExceptionDecodeTest, Ut_GetCcuMissionContextRaw_WhenChannelFail_ExpectErrorAndZeroBuf)
{
    ...
    size_t copiedLen = 123;
    EXPECT_NE(CcuTaskException::GetCcuMissionContextRaw(0, 0, 0, buf, sizeof(buf), copiedLen),
              HCCL_SUCCESS);
    EXPECT_EQ(copiedLen, 0U);
    for (size_t i = 0; i < sizeof(buf); ++i) {
        EXPECT_EQ(buf[i], 0U);
...
TEST_F(CcuTaskExceptionDecodeTest, Ut_GetCcuLoopContextRaw_WhenChannelFail_ExpectErrorAndZeroBuf)
{
    ...
    size_t copiedLen = 123;
    EXPECT_NE(CcuTaskException::GetCcuLoopContextRaw(0, 0, 0, buf, sizeof(buf), copiedLen),
              HCCL_SUCCESS);
    EXPECT_EQ(copiedLen, 0U);
    for (size_t i = 0; i < sizeof(buf); ++i) {
        EXPECT_EQ(buf[i], 0U);
...
TEST(Ut_CcuLoopContextV1, WhenAddrStridePartsSet_ExpectAddrStrideCombinedCorrectly)
{
    ...
    uint32_t expected = (static_cast<uint32_t>(0x3FFU) << 22) |
        (static_cast<uint32_t>(0xFFFFU) << 6) |
        static_cast<uint32_t>(0x3FU);
...
TEST(Ut_CcuLoopContextV2, WhenAddrStridePartsSet_ExpectAddrStrideCombinedCorrectly)
{
    ...
    uint32_t expected = (static_cast<uint32_t>(0x7FU) << 25) |
        (static_cast<uint32_t>(0xFFFFU) << 9) |
        static_cast<uint32_t>(0x1FFU);
    EXPECT_EQ(ctx.GetAddrStride(), expected);
}
