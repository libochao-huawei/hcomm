...
TEST_F(TestClusterMonitor, CreateMonitorLinksAsync)
{
    ...
    std::unique_lock<std::mutex> linksLock(g_monitor.linksConnInfoMtx_);
    ...
    HcclResult retVal = g_monitor.CreateMonitorLinksAsync();
    EXPECT_EQ(retVal, HCCL_SUCCESS);
    ...
    while((std::chrono::steady_clock::now() - startTime) <= CREATE_LINK_TIMEOUT) {
        ...
        if (linkThreadIt->second == MonitorLinkStatus::MONITOR_LINK_COMPLETED) {
            uncompletedCount--;
        }
...
{
    for (auto iter = g_monitor.uid2ContextRefMap_.begin(); iter != g_monitor.uid2ContextRefMap_.end(); iter++) {
        ClusterUIDType rem = iter->first;
        g_monitor.uid2ContextRefMap_[rem].lostNum++;
        HcclResult ret = g_monitor.SendMonitorFrame(rem, g_monitor.uid_, g_monitor.uid_, ClusterMonitorStatus::CLUSTER_MONITOR_STUCK);
        EXPECT_EQ(ret, HCCL_SUCCESS);
        EXPECT_EQ(g_monitor.uid2ContextRefMap_[rem].sendBuffer.size(), 0);
    }
}

TEST_F(TestClusterMonitor, RecvMonitorFrame_test)
{
    MOCKER_CPP(&ClusterMonitor::ParseMonitorFrame)
    .stubs()
    ...
    for (auto iter = g_monitor.uid2ContextRefMap_.begin(); iter != g_monitor.uid2ContextRefMap_.end(); iter++) {
        ClusterUIDType rem = iter->first;
        HcclResult ret = g_monitor.RecvMonitorFrame(rem);
        EXPECT_EQ(ret, HCCL_SUCCESS);
    }
}