...
protected:
    ...
    void StubKfcControlPut(const HcclResult ret) {
        MOCKER_CPP(&hccl::HDCommunicate::Put).stubs().with(_, _, _).will(returnValue(ret));
    }
    void StubKfcStatusGet(const HcclResult ret, const Hccl::KfcStatus status) {
        MOCKER_CPP(&hccl::HDCommunicate::Get).stubs().with(_, _, _).will(returnValue(ret));
    }
    void StubChannelUpdateKernelLaunch(const HcclResult ret) {
        MOCKER_CPP(&ChannelProcess::ChannelUpdateKernelLaunch).stubs().with(_, _, _, _, _).will(returnValue(ret));
    }
