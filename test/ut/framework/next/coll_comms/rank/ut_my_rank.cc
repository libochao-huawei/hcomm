...
TEST_F(MyRankTest, Ut_SetKfcControlTransfer_Calls_NsRecoveryProcessor)
{
    ...
    MOCKER_CPP(&hccl::NsRecoveryProcessor::SetKfcControlTransfer)
        .stubs()
        .with(any(), any())
        .will(returnValue());
    ...
    auto tx = std::make_shared<HDCommunicate>();
    auto rx = std::make_shared<HDCommunicate>();
    // 不会抛异常，GlobalMockObject::verify 会验证调用情况
    myRank.SetKfcControlTransfer(tx, rx);
}

TEST_F(MyRankTest, Ut_StopLaunch_When_NsRecoveryProcessor_Succeeds_ReturnsSuccess)
{
    MOCKER_CPP(&hccl::NsRecoveryProcessor::StopLaunch)
        .stubs()
        .will(returnValue(HCCL_SUCCESS));
    ...
    EXPECT_EQ(myRank.StopLaunch(), HCCL_SUCCESS);
}

TEST_F(MyRankTest, Ut_StopLaunch_When_NsRecoveryProcessor_Fails_ReturnsError)
{
    MOCKER_CPP(&hccl::NsRecoveryProcessor::StopLaunch)
        .stubs()
        .will(returnValue(static_cast<HcclResult>(-1)));
    ...
    EXPECT_NE(myRank.StopLaunch(), HCCL_SUCCESS);
}

TEST_F(MyRankTest, Ut_Clean_When_NoChannels_ReturnsSuccess)
{
    // GetAllChannelList 走空表分支：rankPairMgr_->GetChannelTable 返回空
    ...
    EXPECT_EQ(myRank.Clean(), HCCL_SUCCESS);
}

TEST_F(MyRankTest, Ut_Resume_When_NoChannels_ReturnsSuccess)
{
    // GetAllChannelList 走空表分支：rankPairMgr_->GetChannelTable 返回空
    MOCKER_CPP(&hcomm::RankPairMgr::GetChannelTable).stubs().will(returnValue(ChannelTable{}));
    ...
    EXPECT_EQ(myRank.Resume(), HCCL_SUCCESS);
}