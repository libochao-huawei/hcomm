...
protected:
    ...
    void MockerFuncs()
    {
        MOCKER_CPP(&Hccl::SocketManager::GetConnectedSocket).stubs().with(_).will(returnValue((Hccl::Socket*)0xab));
        MOCKER_CPP(&hccl::CommMems::GetTagMemoryHandles).stubs().with(_).will(returnValue(HCCL_SUCCESS));
        MOCKER_CPP(&hcomm::EndpointMgr::RegisterMemory).stubs().with(_).will(returnValue(HCCL_SUCCESS));
        MOCKER_CPP(&hccl::CommMems::SetMemHandles).stubs().with(_).will(returnValue(HCCL_SUCCESS));
        MOCKER_CPP(&hcomm::CcuResContainer::Init).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(MyRankTest, Ut_When_QueryListenPort_Listen_Port_Expect_SUCCESS)
{
    ...
    MOCKER_CPP(&Hccl::IRankGraph::GetDevicePort).stubs().with(_, outBoundP(&devPort)).will(returnValue(HCCL_SUCCESS));
...
TEST_F(MyRankTest, Ut_When_BatchCreateChannels_Expect_SUCCESS)
{
    ...
    uint32_t devPort = 60001;
    MOCKER_CPP(&Hccl::IRankGraph::GetDevicePort).stubs().with(_, outBoundP(&devPort)).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&Hccl::IRankGraph::GetDeviceId).stubs().with(_).will(returnValue(static_cast<int>(HCCL_SUCCESS)));
    MockerFuncs();
    ...
    MOCKER_CPP(&hcomm::ChannelProcess::ChannelGetStatus).stubs().with(_).will(returnValue(HCCL_E_AGAIN));
    MOCKER_CPP(&Hccl::EnvSocketConfig::GetLinkTimeOut).stubs().with(_).will(returnValue((s32)(1)));
    EXPECT_EQ(myRank.BatchConnectChannels(channelDesc, hostChannelHandleList, 3), HCCL_E_TIMEOUT);
    MOCKER_CPP(&hcomm::ChannelProcess::ChannelGetStatus).stubs().with(_).will(returnValue(HCCL_E_TIMEOUT));
    EXPECT_EQ(myRank.BatchConnectChannels(channelDesc, hostChannelHandleList, 3), HCCL_E_TIMEOUT);
...
TEST_F(MyRankTest, St_BatchCreateChannels_When_Resource_fallback_Expect_Return_HCCL_E_UNAVAIL)
{
    uint32_t devPort = 60001;
    MOCKER_CPP(&Hccl::IRankGraph::GetDevicePort).stubs().with(_, outBoundP(&devPort)).will(returnValue(HCCL_SUCCESS));
    MOCKER(HcommEndpointStartListen).stubs().with(_).will(returnValue(static_cast<int>(HCCL_SUCCESS)));
    MOCKER(HcommChannelDestroy).stubs().with(_, _).will(returnValue(static_cast<int>(HCCL_SUCCESS)));
    MockerFuncs();
