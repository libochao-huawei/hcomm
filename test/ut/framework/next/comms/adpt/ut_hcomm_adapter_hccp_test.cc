...
TEST_F(HcommAdapterHccpTest, ut_HccpGetUboeFlagEnable_When_RaGetInterfaceVersion_SuccAndVersionEnough_Expect_SuccessAndFlagValid)
{
    u32 devPhyId = 1;
    ...
    HcclResult ret = HccpGetUboeFlagEnable(devPhyId, uboeFlagValid);
    
    EXPECT_EQ(ret, HCCL_SUCCESS);
    EXPECT_TRUE(uboeFlagValid);
}

TEST_F(HcommAdapterHccpTest, ut_HccpGetUboeFlagEnable_When_RaGetInterfaceVersionSucc_ButVersionNotEnough_Expect_SuccessButFlagInvalid)
{
    u32 devPhyId = 1;
...
TEST_F(HcommAdapterHccpTest, ut_HccpGetUboeFlagEnable_When_RaGetInterfaceVersionFailed_Expect_Error)
{
    u32 devPhyId = 1;
    bool uboeFlagValid = false;
