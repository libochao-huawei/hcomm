...
TEST_F(AicpuTsUboeChannelTest, Ut_GetUserRemoteMem_WithCacheValid_SkipsRebuild) {
    ...
    ch.remoteUserMems_.resize(1);
    ch.remoteUserMems_[0].type = COMM_MEM_TYPE_HOST;
    ch.remoteUserMems_[0].addr = reinterpret_cast<void*>(0x9999);
    ch.remoteUserMems_[0].size = 1234;
    std::array<char, HCCL_RES_TAG_MAX_LEN> tag = {};
    strcpy_s(tag.data(), tag.size(), "cached_user");
    ch.remoteUserMemTag_.push_back(tag);
    ch.tagCopies_.clear();
