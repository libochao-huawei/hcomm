...
#include "next/comms/endpoint_pairs/channels/aicpu/device/aicpu_ts_roce_channel_lite.h" 
...
TEST_F(AicpuTsRoceChannelLiteTest, Ut_When_GetRmaBufSlicelite_Expect_Correct)
{
    ...
    // 验证结果
    EXPECT_EQ(slice.GetAddr(), buffer.GetAddr());
    EXPECT_EQ(slice.GetSize(), buffer.GetSize());
    EXPECT_EQ(slice.GetLkey(), buffer.GetLkey());
    EXPECT_EQ(slice.GetTokenId(), 0); // tokenId 应该被设置为 0
...
TEST_F(AicpuTsRoceChannelLiteTest, Ut_When_GetRmtRmaBufSliceLite_Expect_Correct)
{
    ...
    EXPECT_EQ(slice.GetAddr(), buffer.GetAddr());
    EXPECT_EQ(slice.GetSize(), buffer.GetSize());
    EXPECT_EQ(slice.GetRkey(), buffer.GetRkey());
    EXPECT_EQ(slice.GetTokenId(), 0); // tokenId 应该被设置为 0
...
TEST_F(AicpuTsRoceChannelLiteTest, Ut_When_GetRmtNotifySliceLite_Expect_Correct)
{
    ...
    // 验证结果
    EXPECT_EQ(slice.GetAddr(), 0x2000);
    EXPECT_EQ(slice.GetSize(), 0x100);
    EXPECT_EQ(slice.GetRkey(), 0x200);
    EXPECT_EQ(slice.GetTokenId(), 0); // tokenId 应该被设置为 0
