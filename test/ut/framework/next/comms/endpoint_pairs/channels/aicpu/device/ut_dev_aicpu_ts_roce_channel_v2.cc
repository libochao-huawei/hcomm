...
TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_NotifyNum_Expect_Correct)
{
    std::cout << "Start Ut_When_NotifyNum_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.notifyNum_, NOTIFY_NUM);
    
    std::cout << "End Ut_When_NotifyNum_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_BufferNum_Expect_Correct)
{
    std::cout << "Start Ut_When_BufferNum_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.bufferNum_, BUFFER_NUM);
    
    std::cout << "End Ut_When_BufferNum_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_ConnNum_Expect_Correct)
{
    std::cout << "Start Ut_When_ConnNum_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.connNum_, CONN_NUM);
    
    std::cout << "End Ut_When_ConnNum_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_LocalNotifies_Expect_NotEmpty)
{
    std::cout << "Start Ut_When_LocalNotifies_Expect_NotEmpty" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.localNotifies_.size(), static_cast<size_t>(NOTIFY_NUM));
    
    std::cout << "End Ut_When_LocalNotifies_Expect_NotEmpty" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_RemoteNotifies_Expect_NotEmpty)
{
    std::cout << "Start Ut_When_RemoteNotifies_Expect_NotEmpty" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.remoteNotifies_.size(), static_cast<size_t>(NOTIFY_NUM));
    
    std::cout << "End Ut_When_RemoteNotifies_Expect_NotEmpty" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_LocBufferVec_Expect_NotEmpty)
{
    std::cout << "Start Ut_When_LocBufferVec_Expect_NotEmpty" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.locBufferVec_.size(), static_cast<size_t>(BUFFER_NUM));
    
    std::cout << "End Ut_When_LocBufferVec_Expect_NotEmpty" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_RmtBufferVec_Expect_NotEmpty)
{
    std::cout << "Start Ut_When_RmtBufferVec_Expect_NotEmpty" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.rmtBufferVec_.size(), static_cast<size_t>(BUFFER_NUM));
    
    std::cout << "End Ut_When_RmtBufferVec_Expect_NotEmpty" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_ConnVec_Expect_NotEmpty)
{
    std::cout << "Start Ut_When_ConnVec_Expect_NotEmpty" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.connVec_.size(), static_cast<size_t>(CONN_NUM));
    
    std::cout << "End Ut_When_ConnVec_Expect_NotEmpty" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_ConnUniqueIdVec_Expect_NotEmpty)
{
    std::cout << "Start Ut_When_ConnUniqueIdVec_Expect_NotEmpty" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_EQ(channel.connUniqueIdVec_.size(), static_cast<size_t>(CONN_NUM));
    
    std::cout << "End Ut_When_ConnUniqueIdVec_Expect_NotEmpty" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_NotifyValueBuffer_Expect_NotNull)
{
    std::cout << "Start Ut_When_NotifyValueBuffer_Expect_NotNull" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    EXPECT_NE(channel.notifyValueBuffer_, nullptr);
    
    std::cout << "End Ut_When_NotifyValueBuffer_Expect_NotNull" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_LocalNotifyValues_Expect_Correct)
{
    std::cout << "Start Ut_When_LocalNotifyValues_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    ASSERT_EQ(channel.localNotifies_.size(), static_cast<size_t>(NOTIFY_NUM));
    for (u32 i = 0; i < NOTIFY_NUM; i++) {
        EXPECT_EQ(channel.localNotifies_[i]->GetId(), i + 1);
        EXPECT_EQ(channel.localNotifies_[i]->GetDevPhyId(), i + 10);
    }
    
    std::cout << "End Ut_When_LocalNotifyValues_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_RemoteNotifyValues_Expect_Correct)
{
    std::cout << "Start Ut_When_RemoteNotifyValues_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    ASSERT_EQ(channel.remoteNotifies_.size(), static_cast<size_t>(NOTIFY_NUM));
    for (u32 i = 0; i < NOTIFY_NUM; i++) {
        EXPECT_EQ(channel.remoteNotifies_[i].GetAddr(), 0x2000 + i * 0x100);
        EXPECT_EQ(channel.remoteNotifies_[i].GetSize(), 0x100u);
        EXPECT_EQ(channel.remoteNotifies_[i].GetRkey(), 0x200 + i);
    }
    
    std::cout << "End Ut_When_RemoteNotifyValues_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_NotifyValueBufferValues_Expect_Correct)
{
    std::cout << "Start Ut_When_NotifyValueBufferValues_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    ASSERT_NE(channel.notifyValueBuffer_, nullptr);
    EXPECT_EQ(channel.notifyValueBuffer_->GetAddr(), 0x3000u);
    EXPECT_EQ(channel.notifyValueBuffer_->GetSize(), 0x200u);
    EXPECT_EQ(channel.notifyValueBuffer_->GetLkey(), 0x300u);
    
    std::cout << "End Ut_When_NotifyValueBufferValues_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_LocBufferValues_Expect_Correct)
{
    std::cout << "Start Ut_When_LocBufferValues_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    ASSERT_EQ(channel.locBufferVec_.size(), static_cast<size_t>(BUFFER_NUM));
    for (u32 i = 0; i < BUFFER_NUM; i++) {
        EXPECT_EQ(channel.locBufferVec_[i].GetAddr(), 0x4000 + i * 0x200);
        EXPECT_EQ(channel.locBufferVec_[i].GetSize(), 0x200u);
        EXPECT_EQ(channel.locBufferVec_[i].GetLkey(), 0x400 + i);
    }
    
    std::cout << "End Ut_When_LocBufferValues_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_RmtBufferValues_Expect_Correct)
{
    std::cout << "Start Ut_When_RmtBufferValues_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    ASSERT_EQ(channel.rmtBufferVec_.size(), static_cast<size_t>(BUFFER_NUM));
    for (u32 i = 0; i < BUFFER_NUM; i++) {
        EXPECT_EQ(channel.rmtBufferVec_[i].GetAddr(), 0x5000 + i * 0x200);
        EXPECT_EQ(channel.rmtBufferVec_[i].GetSize(), 0x200u);
        EXPECT_EQ(channel.rmtBufferVec_[i].GetRkey(), 0x600 + i);
    }
    
    std::cout << "End Ut_When_RmtBufferValues_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_ConnVecValues_Expect_Correct)
{
    std::cout << "Start Ut_When_ConnVecValues_Expect_Correct" << std::endl;
    
    DevAicpuTsRoceChannelV2 channel(uniqueId_);
    
    ASSERT_EQ(channel.connVec_.size(), static_cast<size_t>(CONN_NUM));
    for (u32 i = 0; i < CONN_NUM; i++) {
        EXPECT_NE(channel.connVec_[i], nullptr);
        EXPECT_EQ(channel.connVec_[i]->dmaMode_, 0u);
        EXPECT_EQ(channel.connVec_[i]->sqContext.qpn, 1u + i);
        EXPECT_EQ(channel.connVec_[i]->cqContext.cqn, 2u + i);
    }
    
    std::cout << "End Ut_When_ConnVecValues_Expect_Correct" << std::endl;
}

TEST_F(DevAicpuTsRoceChannelV2Test, Ut_When_Create_Expect_Success)
{
    std::cout << "Start Ut_When_Create_Expect_Success" << std::endl;
    
    DevAicpuTsRoceChannelV2 manager;
    HcommDeviceInfo deviceInfo{};
    ChannelHandle handle{};
    
    HcclResult ret = manager.Create(uniqueId_.data(), static_cast<u64>(uniqueId_.size()),
        deviceInfo, handle);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    EXPECT_NE(handle, reinterpret_cast<ChannelHandle>(nullptr));
    
    bool destroyed = manager.Destroy(handle);
    EXPECT_TRUE(destroyed);
    
    std::cout << "End Ut_When_Create_Expect_Success" << std::endl;
}
