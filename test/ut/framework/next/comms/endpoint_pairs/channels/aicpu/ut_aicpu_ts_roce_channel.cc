...
TEST_F(AicpuTsRoceChannelTest, Ut_When_Normal_Init_Expect_HCCL_SUCCESS)
{
    ...
    auto channel = std::make_unique<AicpuTsRoceChannel>(endpointHandle, channelDesc, CommEngine::AICPU);
...
TEST_F(AicpuTsRoceChannelTest, Ut_When_GetCommEngine_Expect_AICPU)
{
    std::cout << "Start Ut_When_GetCommEngine_Expect_AICPU" << std::endl;
    
    auto channel = std::make_unique<AicpuTsRoceChannel>(endpointHandle, channelDesc, CommEngine::AICPU);
    channel->Init();
    
    CommEngine engine = channel->GetCommEngine();
    EXPECT_EQ(engine, CommEngine::AICPU);
    
    std::cout << "End Ut_When_GetCommEngine_Expect_AICPU" << std::endl;
}

TEST_F(AicpuTsRoceChannelTest, Ut_When_GetCommProtocol_Expect_ROCE)
{
    std::cout << "Start Ut_When_GetCommProtocol_Expect_ROCE" << std::endl;
    
    auto channel = std::make_unique<AicpuTsRoceChannel>(endpointHandle, channelDesc, CommEngine::AICPU);
    channel->Init();
    
    CommProtocol protocol = channel->GetCommProtocol();
    EXPECT_EQ(protocol, COMM_PROTOCOL_ROCE);
    
    std::cout << "End Ut_When_GetCommProtocol_Expect_ROCE" << std::endl;
}
