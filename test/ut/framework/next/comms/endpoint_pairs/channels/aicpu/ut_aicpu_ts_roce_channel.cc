...
TEST_F(AicpuTsRoceChannelTest, Ut_PrepareAicpuKernelDeviceBlob_WithoutInit_Returns_INTERNAL) {
    HcommChannelDesc desc{};
    AicpuTsRoceChannel ch(reinterpret_cast<EndpointHandle>(0x1), desc);
    std::shared_ptr<hccl::DeviceMem> out;
    EXPECT_EQ(ch.PrepareAicpuKernelDeviceBlob(out), HCCL_E_INTERNAL);
