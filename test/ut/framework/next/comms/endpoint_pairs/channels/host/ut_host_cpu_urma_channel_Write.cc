/**
 * Copyright (c) 2025 Huawei Technologies Technologies, Ltd.
...
class UtHostCpuUrmaChannelWrite : public testing::Test
{
protected:
...
TEST_F(UtHostCpuUrmaChannelWrite, 
       UT_Write_When_Normal_Expect_ReturnHCCL_SUCCESS)
{
    MOCKER_CPP(&urma_post_jfs_wr).stubs().will(returnValue(0));
    
    HcclResult ret = channel->Write(dstAddr, srcAddr, len);
...
TEST_F(UtHostCpuUrmaChannelWrite, 
       UT_Write_When_FenceFlagTrue_Expect_ReturnHCCL_SUCCESS)
{
...
TEST_F(UtHostCpuUrmaChannelWrite, 
       UT_Write_When_LenZero_Expect_ReturnHCCL_SUCCESS)
{
    len = 0;
    MOCKER_CPP(&urma_post_jfs_wr).stubs().will(returnValue(0));
...
TEST_F(UtHostCpuUrmaChannelWrite, 
       UT_Write_When_UrmaPostJfsWr_Fail_Expect_ReturnHCCL_E_NETWORK)
{
