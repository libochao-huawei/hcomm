...
TEST_F(TestEndpointPair, Ut_EndpointPair_Construct_Expect_HCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&hcomm::EndpointPair::GetSocket).stubs().with(any(), any(), any()).will(returnValue(HCCL_SUCCESS));
    ...
    ret = endpointPair.GetSocket(0, 1, "Hccl_Test_Group", 60001, socket.get());
    EXPECT_EQ(ret, HCCL_SUCCESS);
    ret = endpointPair.GetSocket(1, 0, "Hccl_Test_Group", 60001, socket.get());
    EXPECT_EQ(ret, HCCL_SUCCESS);
}