...
TEST_F(AicpuTsRoceEndpointTest, Ut_TwoInitsSameDevicePhyId_ShareOneNetDevSlot) {
    MOCKER(hrtGetDevice).stubs().with(_).will(invoke(StubHrtGetDeviceWriteZeroEp));
    MOCKER(hrtGetDevicePhyIdByIndex).stubs().with(_, outBound(0U)).will(returnValue(HCCL_SUCCESS));
