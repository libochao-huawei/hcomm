...
protected:
    ...
    void SetEndpointDesc(uint32_t id, uint32 devPhyId, EndpointDesc &endpointDesc)
    {
    ...
    HcommResult CreateHccsEndpoint(uint32_t id, uint32 devPhyId, EndpointDesc &endpointDesc, void** endpointHandle)
    {
...
TEST_F(AiCpuTsHccsEndpointTest, Ut_When_Register_Memory_NORMAL_Expect_Return_SUCCESS)
{
    ...
    HcommResult ret = CreateHccsEndpoint(0, 0, endpointDesc, endpointDesc, &endpointHandle);
...
TEST_F(AiCpuTsHccsEndpointTest, Ut_When_export_import_Expect_Return_SUCCESS)
{
    ...
    remoteGrantInfo.sdid = static_cast<uint32>(-1);
