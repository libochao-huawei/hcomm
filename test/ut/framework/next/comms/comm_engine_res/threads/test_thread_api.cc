...
TEST_F(TestHcclThread, UT_TestHcommThreadAllocWithStream_When_ThreadNullptr_expect_return_HCCL_E_PARA)
{
    std::shared_ptr<Thread> Handle;
    bool isDeviceSide{false};
    MOCKER(GetRunSideIsDevice)
...
TEST_F(TestHcclThread, UT_TestHcommThreadAllocWithStream_When_WithInvalidEngine_expect_return_HCCL_E_NOT_SUPPORT)
{
    std::shared_ptr<Thread> Handle;
    bool isDeviceSide{false};
    MOCKER(GetRunSideIsDevice)
