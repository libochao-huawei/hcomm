...
TEST_F(TestUrmaMem, HcommMemReg_DuplicateReg_Expect_ResultIsHCCL_E_AGAIN)
{
    EndpointHandle endpointHandle = nullptr;
    HcclResult res = HcommEndpointCreate(&endpointDesc, &endpointHandle);
    ASSERT_EQ(res, HCCL_SUCCESS);
    ...
    void *memHandlecopy;
    res = HcommMemReg(endpointHandle, memTag, mem, &memHandlecopy);
    EXPECT_EQ(res, HCCL_E_AGAIN);
    EXPECT_NE(memHandlecopy, nullptr);
}

TEST_F(TestUrmaMem, HcommMemReg_CrossReg_Expect_ResultIsHCCL_E_INTERNAL)
{
    EndpointHandle endpointHandle = nullptr;
    HcclResult res = HcommEndpointCreate(&endpointDesc, &endpointHandle);
    ASSERT_EQ(res, HCCL_SUCCESS);
    ...
    HcommMem mem1 = {
        .type = HCCL_MEM_TYPE_DEVICE,
        .addr = reinterpret_cast<void *>(0x1118),
        .size = 100
    }
    res = HcommMemReg(endpointHandle, memTag, mem1, &memHandlecopy);
    EXPECT_EQ(res, HCCL_E_INTERNA);
    EXPECT_NE(memHandlecopy, nullptr);
}