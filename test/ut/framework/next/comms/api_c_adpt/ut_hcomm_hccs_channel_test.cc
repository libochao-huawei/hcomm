...
HcclResult StubLaunchKernelDeviceParam(const HcclChannelUrmaRes &channelParam, aclrtBinHandle binHandle,
    const std::string &kernelName);
{
    uint64_t context = reinterpret_cast<uint64_t>(&channelParam);
    int32_t ret = RunAicpuChannelInitV3(reinterpret_cast<void *>(&context));
...
public:
    void SetUp() override {
        ...
        MOCKER_CPP(&ChannelProcess::LaunchKernelDeviceParam).stubs().will(invoke(StubLaunchKernelDeviceParam));
        socket_data_len = 0;
    }
