...
HcclResult StubHcclSocketSend(const std::string &sendMsg) 
{
...
HcclResult StubHcclSocketRecv(std::string &recvMsg, u32 timeout)
{
    (void)timeout;
    recvMsg.reserve(socket_data_len);
    (void)memcpy_s(recvMsg.data(), socket_data_len, socket_data, socket_data_len);
    socket_data_len = 0;
    return HCCL_SUCCESS;
}
