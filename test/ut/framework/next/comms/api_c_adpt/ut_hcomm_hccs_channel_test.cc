...
HcclResult StubHcclSocketSend(const void *data, u64 size) 
{
...
HcclResult StubHcclSocketRecv(void *recvBuf, u32 recvBufLen, u32 timeout)
{
    (void)timeout;
    (void)memcpy_s(recvBuf, socket_data_len, socket_data, socket_data_len);
    recvBufLen = socket_data_len;
    socket_data_len = 0;
    return HCCL_SUCCESS;
}

HcclResult StubHcclSocketSend2(const std::string &sendMsg) 
{
    (void)memcpy_s(socket_data, TEST_HCOMM_HCCS_CHANNEL_BUF_LEN, sendMsg.data(), sendMsg.size());
    socket_data_len = sendMsg.size();
    return HCCL_SUCCESS;
}

HcclResult StubHcclSocketRecv2(std::string &recvMsg, u32 timeout)
{
    (void)timeout;
    recvMsg.reserve(socket_data_len);
    (void)memcpy_s(const_cast<char *>(recvMsg.data()), socket_data_len, socket_data, socket_data_len);
    socket_data_len = 0;
    return HCCL_SUCCESS;
}
...
public:
    void SetUp() override {
        ...
        MOCKER_CPP(&hccl::HcclSocket::Send, HcclResult(hccl::HcclSocket::*)(const std::string &)).stubs().will(invoke(StubHcclSocketSend2));
        MOCKER_CPP(&hccl::HcclSocket::Recv, HcclResult(hccl::HcclSocket::*)(std::string &, u32)).stubs().will(invoke(StubHcclSocketRecv2));
