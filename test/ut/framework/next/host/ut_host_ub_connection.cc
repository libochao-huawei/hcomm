...
TEST_F(HostUbConnectionTest, HostUbConnection_process_slices_with_notify)
{
    // Given: construct HostUbConnection
    HostUbConnection conn(rdmaHandle, localIp, remoteIp, OpMode::OPBASE);
    ...
    // Then: should be split into slices with one notify at the end
    EXPECT_GT(sliceCount, 0u);
    EXPECT_EQ(notifyCount, 1u);
}

TEST_F(HostUbConnectionTest, HostUbConnection_process_slices_with_notify_and_reduce)
{
    // Given: construct HostUbConnection
    HostUbConnection conn(rdmaHandle, localIp, remoteIp, OpMode::OPBASE);
    ...
    // Then: with INT8, slice size should be adjusted
    EXPECT_GT(sliceCount, 0u);
    EXPECT_EQ(notifyCount, 1u);
}
...
TEST_F(HostUbConnectionTest, HostUbConnection_host_ub_tp_connection_constructor)
{
    // Given: construct HostUbTpConnection
    HostUbTpConnection conn(rdmaHandle, localIp, remoteIp, OpMode::OPBASE);
