...
protected:
    ...
    virtual void SetUp()
    {
        ...
        MOCKER(RaGetLbMax).stubs().with(any(), any()).will(invoke([](void *rdmaHandle, int *lbMax) -> int {
            if (lbMax) *lbMax = 1;
            return 0;
        }));
...
TEST_F(HostCpuRoceChannelTest, Ut_When_HostCpuRoceChannel_Pack_And_Unpack_Expect_HCCL_SUCCESS)
{
    ...
    // 确保所有连接的状态都是QP_CREATED或QP_MODIFIED
    for (auto &conn : impl_->connections_) {
        conn->rdmaConnStatus_ = HostRdmaConnection::RdmaConnStatus::QP_CREATED;
    }
