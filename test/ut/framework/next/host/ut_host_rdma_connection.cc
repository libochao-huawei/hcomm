...
protected:
    ...
    virtual void SetUp()
    {
        ...
        MOCKER(Hccl::HrtGetRaQpStatus).stubs().with(_).will(returnValue(1));
        MOCKER(Hccl::HrtRaDestroyQpWithCq).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
        MOCKER(RaCreateCompChannel).stubs().with(_, _).will(returnValue(0));
        MOCKER(RaDestroyCompChannel).stubs().with(_, _).will(returnValue(0));
    }
...
TEST_F(HostRdmaConnectionTest, Ut_When_Normal_Call_Expect_Status_Consisitent)
{
    ...
    MOCKER(Hccl::HrtGetSocVer)
        .stubs()
        .with(outBoundP(&targetChipVer[0], sizeof(targetChipVer)), _)
        .will(returnValue(RT_ERROR_NONE));
    QpHandle fakeQpHandle = (void *)0x1000000;
    MOCKER(Hccl::HrtRaQpCreate, QpHandle(*)(RdmaHandle, int, int)).stubs().with(_, _, _).will(returnValue(fakeQpHandle));
