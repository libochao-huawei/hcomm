...
int RaMrReg(void* handle, struct MrInfo *mrInfo)
{
    CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
      return ((handle == NULL) || (mrInfo == NULL)) ? -1 :0;
}

int RaGetNotifyMrInfo(void* handle, struct MrInfo *mrInfo)
{
    CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
...
int RaMrDereg(void* handle, struct MrInfo *mrInfo)/*桩函数只负责将数据发送出去，不需要设置mr*/
{
    CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
    return ((handle == NULL) || (mrInfo == NULL)) ? -1 :0;
}

int RaRegisterMr(const void* handle, struct MrInfo *mrInfo, void **mrHandle)
{
    CHK_PRT_RET(hccpThreadStatus == 0, HCCL_ERROR("Hccp thread has not been started"), -1);
