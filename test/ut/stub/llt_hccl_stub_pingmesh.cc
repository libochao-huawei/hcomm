    ...
    extern HcclResult RaGetEidMap(std::map<hccl::Eid, uint32_t>& eidmap, const hccl::HRaInfo &raInfo)
{
    ...
    // 基础EID字符串（去除最后两位）
    const std::string base = "000000000000002000100000dfdf17";

    // 循环插入尾数00-09
    for (uint32_t i = 0; i < 10; ++i) {
        // 生成当前尾数（两位十六进制小写）
        char suffix[3];
        snprintf(suffix, sizeof(suffix), "%02x", i); // 格式化为两位小写十六进制
        
        // 构造完整EID字符串
        std::string fullEid = base + suffix;
        
        // 转换为Eid类型并插入map
        hccl::Eid eid = hccl::HcclIpAddress::StrToEID(fullEid);
