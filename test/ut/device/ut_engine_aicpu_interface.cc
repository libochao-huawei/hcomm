...
TEST_F(Test_Engine_Aicpu_Interface, test_RunAicpuThreadSupplementNotify) {
    ...
    EXPECT_EQ(memcpy_s(opParam.hcomId, sizeof(opParam.hcomId), hcomId.c_str(), hcomId.size()), 0);
    EXPECT_EQ(memcpy_s(opParam.threadParam[0], THREAD_UNIQUE_ID_MAX_SIZE, mainStr.c_str(), copyLen), 0);

    u64 addr = (u64)(&para);
    args = (void*)(&addr);
    EXPECT_EQ(RunAicpuIndOpThreadInit(args), 0);
    copyLen = std::min(supplementStr.size(), static_cast<size_t>(THREAD_UNIQUE_ID_MAX_SIZE));
    EXPECT_EQ(memcpy_s(opParam.threadParam[0], THREAD_UNIQUE_ID_MAX_SIZE, supplementStr.c_str(), copyLen), 0);
