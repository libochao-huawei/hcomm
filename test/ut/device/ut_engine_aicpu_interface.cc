...
TEST_F(Test_Engine_Aicpu_Interface, test_RunAicpuThreadSupplementNotify) {
    ...
    std::string hcommId = "abcd";
    EXPECT_EQ(memcpy_s(aicpuPara.hcommId, sizeof(aicpuPara.hcommId), hcommId.c_str(), hcommId.size()), 0);
    aicpuPara.deviceLogicId = 0;
    aicpuPara.devicePhyId = 0;
    aicpuPara.dataType = (u32)DevType::DEV_TYPE_950;
    MOCKER_CPP(&CollCommAicpuMgr::InitAicpuIndOp)
        .stubs()
        .will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&CollCommAicpuMgr::RegisterThreadAddDfxTaskInfo)
        .stubs()
        .will(returnValue(HCCL_SUCCESS));
    ...
    EXPECT_EQ(memcpy_s(opParam.hcommId, opParam.hcommId, hcommId.c_str(), hcommId.size), 0);
    EXPECT_EQ(memcpy_s(opParam.threadParam[i], THREAD_UNIQUE_ID_MAX_SIZE, mainStr.c_str(), copyLen), 0);

    u64 addr = (u64)(&para);
    args = (void*)(&addr);
    EXPECT_EQ(RunAicpuIndOpThreadInit(args), 0);
    size_t copyLen = std::min(supplementStr.size(), static_cast<size_t>(THREAD_UNIQUE_ID_MAX_SIZE));
    EXPECT_EQ(memcpy_s(opParam.threadParam[i], THREAD_UNIQUE_ID_MAX_SIZE, supplementStr.c_str(), copyLen), 0);
