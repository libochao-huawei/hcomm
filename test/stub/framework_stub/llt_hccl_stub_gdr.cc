...
int ra_socket_recv(const void* handle, void* __data, u64 size, u64 *recvSize) /*非阻塞接受*/
{
    ...
    if (handle != NULL)  {
        ...
        *recvSize = size_recieved;
        HCCL_DEBUG("[DEBUG-recv] dev[%u] server[%x] client[%x] role[%u] fd[%d] recv[%d Bytes] size [%lu]",
            connection->device_id, connection->server_ip, connection->client_ip, connection->role,
    ...
    while (1) {
        ...
        if (nfds == 0) {
            ...
            for (int i = 0; i < nfds; i++) {
                ...
                HCCL_INFO("listen receive info[0x%08x], tag[%s]", recv_info.client_ip, recv_info.tag);
...
{
    if (type == id_type::PHY_ID_VNIC_IP) {
...
drvError_t halSdmaBatchCopy(void *dst[], void *src[], size_t size[], int count)
{
    for (int i = 0; i < count; i++) {
        printf("dst[%p], srv[%p], len[%llu], i[%ld]", dst[i], src[i], static_cast<long long unsigned int>(size[i]), static_cast<long int>(i));
