...
constexpr uint64_t AIV_COMM_INFO_SIZE = 33 * 1024 * 1024; // aiv算子的通信信息区大小
constexpr uint64_t AIV_FLAG_BUFFER_SIZE = AIV_COMM_INFO_SIZE - FLAG_ADDR_OFFSET; // aiv算子的flag区域大小
...
{
    send_flag(targetRank, flag_offset, curTag);
    pipe_barrier(PIPE_ALL);
}
...
{
    recv_flag(targetRank, flag_offset, curTag);
    pipe_barrier(PIPE_ALL);
}
...
__aicore__ inline void AivCommBase::ClearGM() // todo
{
