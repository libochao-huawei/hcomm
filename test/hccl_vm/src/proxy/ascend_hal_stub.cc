...
#define HAL_STUB_ERROR(format, ...) HCCL_VM_ERROR("[HAL_STUB]" format, ##__VA_ARGS__)
#define HAL_STUB_DEBUG(format, ...) HCCL_VM_DEBUG("[HAL_STUB]" format, ##__VA_ARGS__)
#define HAL_STUB_INFO(format, ...)  HCCL_VM_INFO("[HAL_STUB]" format, ##__VA_ARGS__)
#define HAL_STUB_WARN(format, ...)  HCCL_VM_WARN("[HAL_STUB]" format, ##__VA_ARGS__)
#define HAL_STUB_TRACE(format, ...) HCCL_VM_TRACE("[HAL_STUB]" format, ##__VA_ARGS__)
...
drvError_t drvGetDevNum(uint32_t *num_dev)
{
    ...
    if (devs.empty()) {
        HAL_STUB_INFO("devices failed");
        return DRV_ERROR_NO_DEVICE;
    }
    *num_dev = devs.size();
    HAL_STUB_INFO("num_dev:{}", *num_dev);
    return DRV_ERROR_NONE;
}
...
{
    *devId = localDevId;
    HAL_STUB_INFO("devId:{} localDevId:{}", localDevId, *devId);
    return DRV_ERROR_NONE;
}
...
drvError_t drvGetLocalDevIDByHostDevID(uint32_t host_dev_id, uint32_t *local_dev_id)
{
    ...
    HAL_STUB_INFO("host_dev_id:{} local_dev_id:{}", host_dev_id, *local_dev_id);
    return DRV_ERROR_NONE;
}
...
drvError_t halGetSocVersion(uint32_t devId, char *soc_version, uint32_t len)
{
    ...
    if (sim::GetDeviceByPhysicalId(devId, locDevice) != ACL_SUCCESS) {
        HAL_STUB_ERROR("get device by physic id {} failed.", devId);
        return DRV_ERROR_NO_DEVICE;
    }
    strncpy((char *)soc_version, locDevice.soc_version, len);
    HAL_STUB_INFO("get soc version {}", soc_version);
    return DRV_ERROR_NONE;
}
...
drvError_t drvDeviceGetPhyIdByIndex(uint32_t devIndex, uint32_t *phyId)
{
    ...
    if (!ret.second) {
        HAL_STUB_ERROR("cannot find device by devIndex id {}.", devIndex);
        return DRV_ERROR_NO_DEVICE;
    }
    *phyId = (uint32_t)ret.first.physical_id;
    HAL_STUB_INFO("index:%u, phyId:%u", devIndex, *phyId);
    return DRV_ERROR_NONE;
}
...
drvError_t drvDeviceGetIndexByPhyId(uint32_t phyId, uint32_t *devIndex)
{
    ...
    if (!ret.second) {
        HAL_STUB_ERROR("cannot find device by phsical id {}.", phyId);
        return DRV_ERROR_NO_DEVICE;
    }
    *devIndex = (uint32_t)ret.first.logic_id;
    HAL_STUB_INFO("phyId:%u, devIndex:%u", phyId, *devIndex);
    return DRV_ERROR_NONE;
}
...
DVresult drvMemcpy(DVdeviceptr dst, size_t dest_max, DVdeviceptr src, size_t byte_count)
{
    ...
    if (isSrcHost && !isDstHost) {
        ...
        HAL_STUB_ERROR("drvMemcpy stub not support ACL_MEMCPY_DEVICE_TO_DEVICE");
        return DRV_ERROR_NONE;
    }
