...
drvError_t drvGetDevNum(uint32_t *num_dev)
{
    ...
    if (devs.empty()) {
        HCCL_VM_INFO("[STUB]devices failed\n");
        return DRV_ERROR_NO_DEVICE;
    }
    *num_dev = devs.size();
    HCCL_VM_INFO("[STUB]num_dev:{}\n", *num_dev);
    return DRV_ERROR_NONE;
}
...
{
    *devId = localDevId;
    HCCL_VM_INFO("[STUB] devId:{} localDevId:{}\n", localDevId, *devId);
    return DRV_ERROR_NONE;
}
...
drvError_t drvDeviceGetPhyIdByIndex(uint32_t devIndex, uint32_t *phyId)
{
    ...
    if (!ret.second) {
        HCCL_VM_ERROR("[STUB] cannot find device by devIndex id {}.", devIndex);
        return DRV_ERROR_NO_DEVICE;
    }
    *phyId = (uint32_t)ret.first.physical_id;
    HCCL_VM_INFO("[STUB]index:%u, phyId:%u\n", devIndex, *phyId);
    return DRV_ERROR_NONE;
}
...
drvError_t drvDeviceGetIndexByPhyId(uint32_t phyId, uint32_t *devIndex)
{
    ...
    if (!ret.second) {
        HCCL_VM_ERROR("[STUB] cannot find device by phsical id {}.", phyId);
        return DRV_ERROR_NO_DEVICE;
    }
    *devIndex = (uint32_t)ret.first.logic_id;
    HCCL_VM_INFO("[STUB] phyId:%u, devIndex:%u\n", phyId, *devIndex);
    return DRV_ERROR_NONE;
}
...
{
    pid_t pid = getpid();
    HCCL_VM_INFO("[HAL][STUB][drvDeviceGetBareTgid] pid:[{}]\n", pid);
    return pid;
}
...
DVresult drvMemcpy(DVdeviceptr dst, size_t dest_max, DVdeviceptr src, size_t byte_count)
{
    ...
    if (isSrcHost && !isDstHost) {
        ...
        HCCL_VM_ERROR("[ERROR] drvMemcpy stub not support ACL_MEMCPY_DEVICE_TO_DEVICE\n");
        return DRV_ERROR_NONE;
    }
