...
TEST_F(ProfilingHandlerTest, StartSubscribe_test){
    ...
    uint64_t profconfig = PROF_ACL_API_MASK;
    handler.StartSubscribe(profconfig);
    uint64_t profconfig = PROF_TASK_TIME_MASK;
    handler.StartSubscribe(profconfig);
    uint64_t profconfig = PROF_TASK_TIME_L1_MASK;
    handler.StartSubscribe(profconfig);
}
...
TEST_F(ProfilingHandlerTest, Ut_ReportCcuInfo_When_ReportAllType)
{
    ...
    handler.ReprotCcuInfo(fakeTaskInfo);
}