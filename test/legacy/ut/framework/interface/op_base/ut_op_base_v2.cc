...
TEST_F(OpbaseTestV2, HcclBatchSendRecvV2)
{
    ...
    MOCKER_CPP(&HcclCommunicator::LoadOpbasedCollOp).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(static_cast<HcclResult(*)(const char*, u64, HcclDataType, const void*)>(&HcomCheckOpParamV2))
        .stubs().with(_, _, _, _).will(returnValue(HCCL_SUCCESS));
    MOCKER(&HcomCheckUserRankV2).stubs().with(_,_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, HcclCommInitClusterInfoV2)
{
    ...
    MOCKER(HrtGetDevice).stubs().with(_).will(returnValue(0));
    ...
    MOCKER(HrtGetDeviceType).stubs().with(_).will(returnValue(devType));
    MOCKER_CPP(&HcclCommunicator::Init, HcclResult(HcclCommunicator::*)(const std::string &)).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, HcclGetRankIdV2)
{
    ...
    uint32_t rank = 0;
    MOCKER_CPP(&HcclCommunicator::GetRankId).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, HcclGetRankSizeV2)
{
    ...
    HcclComm comm = static_cast<HcclComm>(hcclComm.get());
    uint32_t rankSize;
    MOCKER_CPP(&HcclCommunicator::GetRankSize).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, HcclAllocComResourceByTilingV2)
{
    ...
    MOCKER_CPP(&HcclCommunicator::AllocCommResource).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, HcclGetCcuTaskInfoLegacy_OK)
{
    ...
    MOCKER_CPP(&HcclCommunicator::GetCcuTaskInfo).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, HcclGetCcuTaskInfoLegacy_ERR)
{
    ...
    void *ccuTaskGroup = static_cast<void*>(&a);
    void *fusionArgs = static_cast<void*>(&a);
    MOCKER_CPP(&HcclCommunicator::GetCcuTaskInfo).stubs().with(_, _).will(returnValue(HCCL_E_INTERNAL));
    HcclResult ret = HcclGetCcuTaskInfoLegacy(comm, fusionArgs, ccuTaskGroup);
...
TEST_F(OpbaseTestV2, HcclSnapshotRecoverAllComms_OK)
{
    ...
    MOCKER_CPP(&SnapShotParser::ParseSnapshotToLocalBuff).stubs().with(_, _, outBound(localBuff)).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::RecoverSubComm).stubs().with(_, _, _).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::RecoverComm).stubs().with(_, _, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, HcclCreateSubCommConfigV2)
{
    ...
    MOCKER(GetHcomRankListV2)
            .stubs()
            .with(_, _, outBound(groupParamsV2Tem))
...
TEST_F(OpbaseTestV2, HcclCommInitClusterInfoMemConfigV2_err_2)
{
    ...
    MOCKER_CPP(&HcclCommunicator::Init, HcclResult(HcclCommunicator::*)(const std::string &)).stubs().with(_).will(returnValue(HCCL_E_INTERNAL));
...
TEST_F(OpbaseTestV2, Ut_HcclCommInitClusterInfoConfigV2_When_InputInvalue_Expect_Return_HCCL_SUCCESS) 
{
    ...
    MOCKER_CPP(&HcclCommunicator::Init, HcclResult(HcclCommunicator::*)(const std::string &)).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&CommunicatorImpl::SetCommExecuteConfig).stubs().will(ignoreReturnValue());
    auto ret = HcclCommInitClusterInfoConfigV2(clusterInfo, rank, &config, &comm);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetRootInfoV2_When_NoNeedInput_Expect_Return_HCCL_SUCCESS) 
{
    ...
    MOCKER(HrtGetDeviceType).stubs().with(_).will(returnValue((DevType)DevType::DEV_TYPE_950));
...
{
    // when
    MOCKER_CPP(&RankInfoDetect::SetupServer).stubs().with(_, _).will(throws(InternalException("...")));
    MOCKER(HrtGetDeviceCount).stubs().will(returnValue(1));
    MOCKER(HrtGetDeviceType).stubs().with(_).will(returnValue((DevType)DevType::DEV_TYPE_950));
...
TEST_F(OpbaseTestV2, Ut_HcclCommInitRootInfoV2_When_InputValue_Expect_Return_HCCL_SUCCESS) 
{
    // when
    HcclRootHandleV2 rootHandle{};
    MOCKER_CPP(&RankInfoDetect::SetupAgent).stubs().with(_, _, _).will(ignoreReturnValue());
    MOCKER_CPP(&RankInfoDetect::WaitComplete).stubs().with(_).will(ignoreReturnValue());
    MOCKER(HrtGetDeviceType).stubs().with(_).will(returnValue((DevType)DevType::DEV_TYPE_950));
    MOCKER(HrtGetDeviceCount).stubs().will(returnValue(1));
    MOCKER_CPP(&HcclCommunicator::Init, HcclResult(HcclCommunicator::*)(const RankTableInfo &)).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&CommunicatorImpl::SetCommExecuteConfig).stubs().will(ignoreReturnValue());
...
{
    // when
    MOCKER_CPP(&RankInfoDetect::SetupAgent).stubs().with(_, _, _).will(throws(InternalException("...")));
    MOCKER_CPP(&RankInfoDetect::WaitComplete).stubs().with(_).will(ignoreReturnValue());
    MOCKER(HrtGetDeviceType).stubs().with(_).will(returnValue((DevType)DevType::DEV_TYPE_950));
    MOCKER(HrtGetDeviceCount).stubs().will(returnValue(1));
    MOCKER_CPP(&HcclCommunicator::Init, HcclResult(HcclCommunicator::*)(const RankTableInfo &)).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclBarrierV2_When_Creat_Memory_Stub_SUCCESS)
{
    ...
    HcclComm comm = static_cast<HcclComm>(hcclComm.get());
    aclrtStream stream = &count;
    MOCKER_CPP(&CommunicatorImpl::CreateBarrierMemory).stubs().with(_, _, _).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&HcclCommunicator::LoadOpbasedCollOp).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclBarrierV2_When_Creat_Memory_Fail_Return_HCCL_E_PTR)
{
    ...
    HcclComm comm = static_cast<HcclComm>(hcclComm.get());
    aclrtStream stream = &count;
    MOCKER_CPP(&CommunicatorImpl::CreateBarrierMemory).stubs().with(_, _, _).will(returnValue(HCCL_E_PTR));
    MOCKER_CPP(&HcclCommunicator::LoadOpbasedCollOp).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    HcclResult result = HcclBarrierV2(comm, stream);
    EXPECT_EQ(result, HCCL_E_PTR);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetInstRanksByNetLayerV2_When_InputValue_Expect_Return_HCCL_SUCCESS)
{
    ...
    uint32_t rankNum = 0;
    MOCKER_CPP(&CommunicatorImpl::GetInstRanksByNetLayer).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclGetInstRanksByNetLayerV2_When_InValid_Expect_ReturnHCCL_NOT_FOUND)
{
    ...
    uint32_t *ranks = nullptr;
    uint32_t rankNum = 0;
    MOCKER_CPP(&CommunicatorImpl::GetInstRanksByNetLayer).stubs().with(_, _).will(returnValue(HCCL_E_PTR));
    HcclResult ret = HcclGetInstRanksByNetLayerV2(comm, 0, &ranks, &rankNum);
...
TEST_F(OpbaseTestV2, Ut_HcclGetInstTopoTypeByNetLayerV2_When_InputValue_Expect_Return_HCCL_SUCCESS)
{
    ...
    HcclComm comm = static_cast<HcclComm>(hcclComm.get());
    uint32_t type = 0;
    MOCKER_CPP(&CommunicatorImpl::GetInstTopoTypeByNetLayer).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclGetInstTopoTypeByNetLayerV2_When_InValid_Expect_ReturnHCCL_NOT_FOUND)
{
    ...
    HcclComm comm = static_cast<HcclComm>(hcclComm.get());
    uint32_t type = 0;
    MOCKER_CPP(&CommunicatorImpl::GetInstTopoTypeByNetLayer).stubs().with(_, _).will(returnValue(HCCL_E_PTR));
    HcclResult ret = HcclGetInstTopoTypeByNetLayerV2(comm, 0, &type);
    EXPECT_EQ(ret, HCCL_E_NOT_FOUND);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetInstSizeListByNetLayerV2_When_InValid_Expect_ReturnHCCL_NOT_FOUND)
{
    ...
    uint32_t listSize = 0;
    MOCKER_CPP(&CommunicatorImpl::GetInstSizeListByNetLayer).stubs().with(_, _).will(returnValue(HCCL_E_NOT_FOUND));
...
TEST_F(OpbaseTestV2, Ut_HcclGetInstSizeListByNetLayerV2_When_Valid_Expect_ReturnHCCL_SUCCESS)
{
    ...
    uint32_t *instSizeList = nullptr;
    uint32_t listSize = 0;
    MOCKER_CPP(&CommunicatorImpl::GetInstSizeListByNetLayer).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HcclGetInstSizeListByNetLayerV2(comm, 0, &instSizeList, &listSize);
    EXPECT_EQ(ret, HCCL_SUCCESS);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetLinksV2_When_InputValue_Expect_Return_HCCL_SUCCESS)
{
    ...
    uint32_t listSize = 0;
    MOCKER_CPP(&CommunicatorImpl::GetLinks).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclGetLinksV2_When_InValid_Expect_ReturnHCCL_NOT_FOUND)
{
    ...
    CommLink *linkList = nullptr;
    uint32_t listSize = 0;
    MOCKER_CPP(&CommunicatorImpl::GetLinks).stubs().with(_, _).will(returnValue(HCCL_E_PTR));
    HcclResult ret = HcclGetLinksV2(comm, 0, 0, 1, &linkList, &listSize);
    EXPECT_EQ(ret, HCCL_E_NOT_FOUND);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetTopoInstsByLayer_When_InputValue_Expect_Return_HCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::GetTopoInstsByLayer).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclGetTopoInstsByLayer_When_inValid_Expect_ReturnHCCL_NOT_FOUND)
{
    ...
    uint32_t *topoInsts = nullptr;
    uint32_t topoInstNum = 0;
    MOCKER_CPP(&CommunicatorImpl::GetTopoInstsByLayer).stubs().with(_, _).will(returnValue(HCCL_E_PTR));
    HcclResult ret = HcclGetTopoInstsByLayerV2(comm, 0, &topoInsts, &topoInstNum);
    EXPECT_EQ(ret, HCCL_E_NOT_FOUND);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetTopoType_When_InputValue_Expect_Return_HCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::GetTopoType).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclGetTopoType_When_inValid_Expect_ReturnHCCL_NOT_FOUND)
{
    ...
    CommTopo topoType;
    uint32_t topoInstId = 0;
    MOCKER_CPP(&CommunicatorImpl::GetTopoType).stubs().with(_, _).will(returnValue(HCCL_E_PTR));
    HcclResult ret = HcclGetTopoTypeV2(comm, 0, topoInstId, &topoType);
    EXPECT_EQ(ret, HCCL_E_NOT_FOUND);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetRanksByTopoInst_When_InputValue_Expect_Return_HCCL_SUCCESS)
{
    ...
    uint32_t *ranks = nullptr;
    uint32_t rankNum = 0;
    MOCKER_CPP(&CommunicatorImpl::GetRanksByTopoInst).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclGetRanksByTopoInst_When_InValid_Expect_ReturnHCCL_NOT_FOUND)
{
    ...
    uint32_t *ranks = nullptr;
    uint32_t rankNum = 0;
    MOCKER_CPP(&CommunicatorImpl::GetRanksByTopoInst).stubs().with(_, _).will(returnValue(HCCL_E_PTR));
    HcclResult ret = HcclGetRanksByTopoInstV2(comm, 0, topoInstId, &ranks, &rankNum);
    EXPECT_EQ(ret, HCCL_E_NOT_FOUND);
}
...
TEST_F(OpbaseTestV2, Ut_HcclGetAicpuOpStreamAndNotifyV2_When_Normal_Expect_ReturnIsHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&HcclCommunicator::GetAicpuOpStreamNotify).stubs().with(_, _, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclRankGraphGetEndpointNumV2_When_Valid_Expect_ReturnHCCL_SUCCESS)
{
    ...
    uint32_t layer = 0;
    uint32_t topoInstId = 0;
    MOCKER_CPP(&CommunicatorImpl::GetEndpointNum).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclRankGraphGetEndpointNumV2_When_InValid_Expect_ReturnHCCL_E_NOT_FOUND)
{
    ...
    uint32_t layer = 0;
    uint32_t topoInstId = 0;
    MOCKER_CPP(&CommunicatorImpl::GetEndpointNum).stubs().with(_, _).will(returnValue(HCCL_E_NOT_FOUND));
    HcclResult ret = HcclRankGraphGetEndpointNumV2(comm, layer, topoInstId, &num);
    EXPECT_EQ(ret, HCCL_E_NOT_FOUND);
}
...
TEST_F(OpbaseTestV2, Ut_HcclRankGraphGetEndpointDescV2_When_Valid_Expect_ReturnHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::GetEndpointDesc).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(OpbaseTestV2, Ut_HcclRankGraphGetEndpointDescV2_When_InValid_Expect_ReturnHCCL_E_NOT_FOUND)
{
    ...
    uint32_t topoInstId = 0;
    EndpointDesc* endPointDesc = new EndpointDesc[descNum];
    MOCKER_CPP(&CommunicatorImpl::GetEndpointDesc).stubs().with(_, _).will(returnValue(HCCL_E_NOT_FOUND));
    HcclResult ret = HcclRankGraphGetEndpointDescV2(comm, layer, topoInstId, &descNum, endPointDesc);
    EXPECT_EQ(ret, HCCL_E_NOT_FOUND);
    delete[] endPointDesc;
}
...
TEST_F(OpbaseTestV2, Ut_HcclRankGraphGetEndpointInfoV2_When_Valid_Expect_ReturnHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::GetEndpointDesc).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    HcclResult ret = HcclRankGraphGetEndpointDescV2(comm, layer, topoInstId, &descNum, endPointDesc);
    MOCKER_CPP(&CommunicatorImpl::GetEndpointInfo).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
