...
{
    // when
    MOCKER_CPP(&PreemptPortManager::PreemptPortInRange).stubs().with(_).will(returnValue(HCCL_SUCCESS));
...
{
    // when
    MOCKER_CPP(&Socket::Listen, bool(Socket::*)(u32 &port)).stubs().with(_).will(returnValue(true));
    MOCKER_CPP(&IpAddress::InitBinaryAddr).stubs().with(_).will(ignoreReturnValue());
...
{
    // when
    MOCKER_CPP(&Socket::Listen, bool(Socket::*)(u32 &port)).stubs().with(_).will(returnValue(false));
    MOCKER_CPP(&IpAddress::InitBinaryAddr).stubs().with(_).will(ignoreReturnValue());
...
{
    // when
    MOCKER_CPP(&PreemptPortManager::IsAlreadyListening).stubs().with(_).will(returnValue(true));
    MOCKER_CPP(&IpAddress::InitBinaryAddr).stubs().with(_).will(ignoreReturnValue());
...
{
    // when
    MOCKER_CPP(&PreemptPortManager::ReleasePreempt).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&IpAddress::InitBinaryAddr).stubs().with(_).will(ignoreReturnValue());
