...
TEST_F(InsRulesTest, Interpret_post_fin_ack_p2p_0_task)
{
    ...
    MOCKER_CPP(&MemTransportManager::GetOpbasedTransport).stubs().with(_, _).will(returnValue(stubTransportPtr));
...
TEST_F(InsRulesTest, Interpret_read_p2p_rma_connection_is_null_ptr)
{
    ...
    MOCKER_CPP(&RmaConnManager::Get).stubs().with(_, _, _).will(returnValue(rmaConnection));
...
TEST_F(InsRulesTest, Interpret_read_p2p_data_buffer_is_null_ptr)
{
    ...
    MOCKER_CPP(&DataBufManager::Get).stubs().with(_, _).will(returnValue(nullBuffer));
