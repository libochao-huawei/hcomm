...
TEST_F(TaskServiceTest, Ut_TaskProfRegister_When_Normal_Expect_ReturnSuccess)
{
    ...
    // 创建 prof 回调
    TaskService::ProfCallbackTemplate profCallback = [](const Hccl::TaskParam& taskParam) -> HcclResult {
        return HCCL_SUCCESS;
    };
...
TEST_F(TaskServiceTest, Ut_TaskProfRegister_When_Registered_Expect_CallbackSet)
{
    ...
    TaskService::ProfCallbackTemplate profCallback = [&callbackInvoked](const Hccl::TaskParam& taskParam) -> HcclResult {
...
TEST_F(TaskServiceTest, Ut_TaskProfRegister_When_MultipleRegistrations_Expect_LastOneTakesEffect)
{
    ...
    // 第一次注册
    TaskService::ProfCallbackTemplate profCallback1 = [](const Hccl::TaskParam& taskParam) -> HcclResult {
        return HCCL_SUCCESS;
    };
    ...
    // 第二次注册(覆盖)
    TaskService::ProfCallbackTemplate profCallback2 = [](const Hccl::TaskParam& taskParam) -> HcclResult {
