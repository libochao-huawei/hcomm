...
TEST_F(TaskServiceTest, Ut_TaskRun_When_TerminateFlag_Expect_Success)
{
    ...
    TaskService taskService(deviceMemFake.data(), deviceMemSize, hostMem.data(), hostMemSize, false);
...
TEST_F(TaskServiceTest, Ut_WriteFlag_HBM_When_rtMemcpy_Fail_Expect_Failure) {
    ...
    TaskService taskService(deviceMem.data(), deviceMemSize, hostMem.data(), hostMemSize, true); // HBM mode
...
TEST_F(TaskServiceTest, Ut_ReadFlag_HBM_When_rtMemcpy_SUCC_Expect_Failure) {
    MOCKER_CPP(&TaskService::MemcpyDevice).stubs().will(returnValue(HCCL_SUCCESS));
    TaskService taskService(deviceMem.data(), 4096, hostMem.data(), 1024, true); // HBM mode
...
TEST_F(TaskServiceTest, Ut_TaskRun_HbmMode_When_aclrtMemcpy_Fail_Expect_Failure)
{
    ...
    HcclResult ret = taskService.MemcpyDevice(hostMem.data(), 512, deviceMem.data(), 512, ACL_MEMCPY_DEVICE_TO_HOST, errorMsg); // should succeed with memcpy_s fallback
    EXPECT_EQ(ret, HCCL_E_INTERNAL);
}
