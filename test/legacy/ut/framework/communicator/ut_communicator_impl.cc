...
TEST_F(CommunicatorImplTest, Ut_GetKFCWorkSpaceVA_When_PciePath_Expect_Success)
{
    ...
    MOCKER(HrtHalGetDeviceInfo).stubs().with(any(), any(), any(), outBoundP(&connType)).will(returnValue(HCCL_SUCCESS));
    MOCKER(HrtMalloc).stubs().with(any(), any()).will(returnValue(reinterpret_cast<void*>(0x2000)));
    MOCKER(halHostRegister)
        .stubs()
        .with(any(), any(), any(), any(), outBoundP(&stubRegAddr))
...
TEST_F(CommunicatorImplTest, Ut_GetKFCWorkSpaceVA_When_PcieHalHostRegisterFail_Expect_DrvError)
{
    ...
    void* stubRegAddr = reinterpret_cast<void*>(0xBEEF0000);
    MOCKER(HrtHalGetDeviceInfo).stubs().with(any(), any(), any(), outBoundP(&connType)).will(returnValue(HCCL_SUCCESS));
    MOCKER(halHostRegister).stubs().with(any(), any(), any(), any(), any()).will(returnValue(DRV_ERROR_INNER_ERR));
    MOCKER(HrtFree).stubs();
