...
protected:
    ...
    virtual void SetUp()
    {
        ...
        MOCKER_CPP(&TaskAbortHandler::Register).stubs().with(_).will(ignoreReturnValue());
        MOCKER_CPP(&TaskAbortHandler::UnRegister).stubs().with(_).will(ignoreReturnValue());
        ...
        MOCKER_CPP(&CollAlgComponent::ExecAlgSelect).defaults().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
...
void MockCommunicatorImpl()
{
    ...
    MOCKER(HrtGetDevice).stubs().will(returnValue(0));
    MOCKER(HrtOpenTsdProcess).stubs().with(_, _).will(returnValue(HcclResult::HCCL_SUCCESS));
    MOCKER(HrtRaTlvInit).stubs().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
    MOCKER(HrtGetDevicePhyIdByIndex).stubs().with(_).will(returnValue(static_cast<DevId>(1)));
    MOCKER(RaInit).stubs().with(_).will(returnValue(0));
    MOCKER(RaTlvInit).stubs().with(_).will(returnValue(0));
...
{
    MOCKER(getenv).stubs().with(_).will(invoke(commGetenv_stub));
    CommunicatorImpl comm;
...
TEST_F(CommunicatorImplTest, Ut_CommunicatorImpl_When_EnableSuperFastLoad_Expect_LoadOpbasedCollOp_ReturnIsHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&Hccl::CcuJettyMgr::GetRemoteRankIdByChannelId).stubs().with(_).will(returnValue(0x23));
    void* fakePtr = (void *)1;
    u32 fakeId = 1;
    s32 fakeDevLogId = 1;
    u32 fakeDevPhyId = 1;
    ...
    MOCKER_CPP(&CcuInsPreprocessor::Preprocess).stubs().with().will(ignoreReturnValue());
    MOCKER_CPP(&HcclCommunicator::RegistTaskAbortHandler  ).stubs().with(_).will(ignoreReturnValue());
    MOCKER_CPP(&HcclCommunicator::UnRegistTaskAbortHandler).stubs().with(_).will(ignoreReturnValue());
...
TEST_F(CommunicatorImplTest, ut_ReLoadOpbasedOp_When_HOSTCPU_TS_Expect_returnHCCL_E_NOT_SUPPORT)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::ExecAlgSelect).stubs().with(_, _).will(ignoreReturnValue());
...
TEST_F(CommunicatorImplTest, ut_ReLoadOpbasedOp_When_AICPU_TS_Expect_returnHCCL_SUCCESS)
{
    ...
    fakeComm.opExecuteConfig.accState = AcceleratorState::AICPU_TS;
    MOCKER_CPP(&CollServiceAiCpuImpl::LoadWithOpBasedModeNoRegister).stubs().with(_).will(ignoreReturnValue());
...
TEST_F(CommunicatorImplTest, ut_ReLoadOffloadOp_When_AICPU_TS_Expect_returnHCCL_SUCCESS)
{
    ...
    fakeComm.opExecuteConfig.accState = AcceleratorState::AICPU_TS;
    MOCKER_CPP(&CollServiceAiCpuImpl::LoadWithOffloadModeNoRegister).stubs().with(_).will(ignoreReturnValue());
...
TEST_F(CommunicatorImplTest, ut_GetAlgExecParam_When_Normal_Expect_ReturnHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&UbMemoryTransportMgr::BatchCreateTransport).stubs().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
    MOCKER_CPP(&UbMemoryTransportMgr::TransportsConnect).stubs().with(_).will(ignoreReturnValue());
    EXPECT_EQ(fakeComm.SetCollOffloadScratchBuf("test", (void *)0x100, 0x100), HcclResult::HCCL_SUCCESS);
    EXPECT_EQ(fakeComm.GetAlgExecParam(opParams, clearEnable, commContext, len, aivCoreLimit), HcclResult::HCCL_SUCCESS);

    MOCKER_CPP(&CommunicatorImpl::HcomSelectAlg).stubs().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
    fakeComm.commExecuteConfig.accState = AcceleratorState::CCU_MS;
    fakeComm.opExecuteConfig.accState = AcceleratorState::CCU_MS;
...
{
    MOCKER(aclrtGetCurrentContext).stubs().with(_).will(returnValue(ACL_SUCCESS));
    MOCKER(aclrtSetCurrentContext).stubs().with(_).will(returnValue(ACL_SUCCESS));
    MOCKER(aclrtDestroyStreamForce).stubs().with(_).will(returnValue(ACL_SUCCESS));
    MOCKER(HrtSetXpuDevice).stubs().with(_).will(returnValue(HCCL_SUCCESS));
    MOCKER(HrtResetXpuDevice).stubs().with(_).will(returnValue(HCCL_E_RUNTIME));
...
TEST_F(CommunicatorImplTest, Ut_When_DestroyDpuKernelResource_Expect_DpuStream_Uninit_Failed)
{
    ...
    comm.isDpuKernelLaunched = true;
    MOCKER(aclrtSetCurrentContext).stubs().with(_).will(returnValue(ACL_SUCCESS));
    MOCKER(aclrtDestroyStreamForce).stubs().with(_).will(returnValue(ACL_ERROR_INVALID_PARAM));
    MOCKER_CPP(&CommunicatorImpl::WaitDpuKernelThreadTerminate).stubs().will(returnValue(HCCL_SUCCESS));
...
TEST_F(CommunicatorImplTest, Ut_GetKFCWorkSpaceVA_When_PciePath_Expect_Success)
{
    ...
    MOCKER(HrtHalGetDeviceInfo).stubs().with(_, _, _, outBoundP(&connType))
        .will(returnValue(HCCL_SUCCESS));
    MOCKER(HrtMalloc).stubs().with(_, _)
        .will(returnValue(reinterpret_cast<void *>(0x2000)));
    MOCKER(halHostRegister).stubs().with(_, _, _, _, outBoundP(&stubRegAddr))
...
TEST_F(CommunicatorImplTest, Ut_GetKFCWorkSpaceVA_When_PcieHalHostRegisterFail_Expect_DrvError)
{
    ...
    MOCKER(HrtHalGetDeviceInfo).stubs().with(_, _, _, outBoundP(&connType))
        .will(returnValue(HCCL_SUCCESS));
    MOCKER(halHostRegister).stubs().with(_, _, _, _, _)
...
protected:
    void SetUp() override {
        ...
        MOCKER_CPP(&CommunicatorImpl::InitCcuSuperFastLoad).stubs().with(_).will(ignoreReturnValue());
        MOCKER_CPP(&CommunicatorImpl::ExecuteFastCcuLaunch).stubs().with(_).will(ignoreReturnValue());
        CcuTaskParam ccuTaskParam{};
