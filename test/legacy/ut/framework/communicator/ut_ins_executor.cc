...
void MockRtsqA5(CommunicatorImplLite &communicatorImplLite)
{
    ...
    rtsq->sqTail_ = 500;
    rtsq->sqDepth_ = 1000;
    MOCKER_CPP_VIRTUAL(*rtsq, &RtsqA5::LaunchTask).stubs().with(_);
    ...
    MOCKER_CPP_VIRTUAL(*rtsq, &RtsqA5::SdmaReduce).stubs().with(_);
    MOCKER_CPP_VIRTUAL(*rtsq, &RtsqA5::IsRtsqQueueSpaceSufficient).stubs().with(_).will(returnValue(true));
}
