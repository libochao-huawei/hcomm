...
TEST_F(NsRecoveryHandlerFuncTest, test_handle_clean)
{
    ...
    MOCKER_CPP(&NsRecoveryHandlerFunc::DeviceQuery).stubs().with(_, _, _).will(returnValue(HcclResult::HCCL_SUCCESS));
...
{
    MOCKER(halTsdrvCtl).stubs().with(_).will(returnValue(DRV_ERROR_NOT_SUPPORT));
    auto ret = NsRecoveryHandlerFunc::GetInstance().DeviceQuery(0, 0, 0);
    EXPECT_EQ(ret, HcclResult::HCCL_E_DRV);
    GlobalMockObject::verify();

    MOCKER(halTsdrvCtl).stubs().with(_).will(returnValue(DRV_ERROR_NONE));
