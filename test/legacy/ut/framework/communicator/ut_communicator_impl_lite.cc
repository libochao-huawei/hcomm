...
{
    MOCKER_CPP(&InsExecutor::Execute).stubs().with(_).will(throws(InternalException("")));
    MOCKER_CPP(&PrimTranslator::Translate).stubs().with(_).will(returnValue(std::make_shared<InsQueue>()));
    MOCKER_CPP(&InsExecutor::ExecuteV82).stubs().with(_).will(throws(InternalException("")));
...
{
    MOCKER_CPP(&InsExecutor::Execute).stubs().with(_).will(throws(1));
    MOCKER_CPP(&PrimTranslator::Translate).stubs().with(_).will(returnValue(std::make_shared<InsQueue>()));
    MOCKER_CPP(&CommunicatorImplLite::RegisterRtsqCallback).stubs();
    CommunicatorImplLite service(0);
    service.primTranslator = std::make_unique<PrimTranslator>();
...
TEST_F(CommunicatorImplLiteTest, test_update_comm_with_hccl_exception)
{
    ...
    MOCKER_CPP(&CommunicatorImplLite::UpdateTransports).stubs().with(_).will(throws(InternalException("")));
...
{
    MOCKER_CPP(&CommunicatorImplLite::UpdateTransports).stubs().with(_).will(throws(1));
...
TEST_F(CommunicatorImplLiteTest, test_update_comm_success)
{
    ...
    MOCKER_CPP_VIRTUAL(*rtsq, static_cast<void (RtsqA5::*)(u32)>(&RtsqA5::NotifyWait)).stubs().with(_);
    MOCKER_CPP_VIRTUAL(*rtsq, static_cast<void (RtsqA5::*)(u32, u32)>(&RtsqA5::NotifyWait)).stubs().with(_);
    ...
    MOCKER_CPP(&MemTransportLiteMgr::ParseAllPackedData).stubs().with(_);
