...
protected:
    ...
    MOCKER_CPP(&CollServiceAiCpuImpl::CheckNeedUpdateRes).defaults().will(returnValue(false));
...
TEST_F(CommunicatorImplLiteTest, test_update_comm_success)
{
    ...
    // 需要更新资源
    service.isSuspended = true;
    MOCKER_CPP(&CollServiceAiCpuImpl::CheckNeedUpdateRes).stubs().will(returnValue(true));
