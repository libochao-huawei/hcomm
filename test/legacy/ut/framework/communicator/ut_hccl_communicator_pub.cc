...
TEST_F(HcclCommunicatorTest, should_success_when_calling_init_with_valid_params)
{
    ...
    MOCKER(HrtSetDevice).stubs().with(_);
    MOCKER_CPP(&HccpHdcManager::Init).stubs().with(_);
...
TEST_F(HcclCommunicatorTest, should_success_when_calling_collop_with_valid_params)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::LoadOpbasedCollOp).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    CommParams commParams;
    auto comm = new HcclCommunicator(commParams);
...
{
    MOCKER_CPP(&HcclCommunicator::Init, HcclResult(HcclCommunicator::*)(const std::string &)).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
    MOCKER_CPP(&CommunicatorImpl::LoadOpbasedCollOp).stubs().with(_, _).will(returnValue(HCCL_E_PARA));
    CommParams commParams;
    auto comm = new HcclCommunicator(commParams);
    comm->Init("ranktable.json");
    CollOpParams opParams;
...
TEST_F(HcclCommunicatorTest, Ut_GetLocalCclBuffer_When_Normal_Expect_OK)
{
    ...
    MOCKER(HrtMalloc).stubs().with(_,_).will(returnValue(bufAddr));
