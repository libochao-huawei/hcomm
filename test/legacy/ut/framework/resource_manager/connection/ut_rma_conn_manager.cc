...
TEST_F(RmaConnManagerTest, should_return_normally_when_calling_addwhitelist_with_valid_param)
{
    ...
    MOCKER_CPP(&SocketHandleManager::Get).stubs().with(_, _).will(returnValue(hccpSocketHandle));

    MOCKER(HrtRaSocketWhiteListAdd).stubs().with(_, _, _);
...
TEST_F(RmaConnManagerTest, should_return_valid_ptr_when_calling_creatermadevnetconn_with_valid_param)
{
    ...
    MOCKER(HrtMalloc).stubs().with(_,_).will(returnValue(devMemPtr));
    MOCKER(HrtGetDeviceType).stubs().will(returnValue(commParams.devType));
    MOCKER(HrtMemcpy).stubs().with(_, _, _, _, _);
    MOCKER_CPP(&CommunicatorImpl::InitCollService).stubs().will(returnValue(HcclResult::HCCL_SUCCESS));
    MOCKER(HrtSetDevice).stubs().with(_).will(ignoreReturnValue());
    ...
    MOCKER_CPP(&RdmaHandleManager::Get).stubs().with(_, _).will(returnValue(rdmaHandle));

    QpHandle qpHandle;
    MOCKER(HrtRaQpCreate).stubs().with(_, _, _).will(returnValue(qpHandle));

    DevBuffer devBuffer(100, 100);
    Buffer *buffer = &devBuffer;
    MOCKER_CPP(&DataBufManager::Get).stubs().with(_, _).will(returnValue(buffer));

    MOCKER(HrtRaMrReg).stubs().with(_, _);
