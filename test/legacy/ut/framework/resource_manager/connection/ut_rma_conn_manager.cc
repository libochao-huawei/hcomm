...
int ra_ctx_qp_destroy_batch_async_no_delete(void *ctx_handle, void*qp_handle[], unsigned int *num, void **req_handle) {
    *num = 0;
    return 0;
}
...
TEST_F(RmaConnManagerTest, should_delete_part_when_calling_batchdeletejettys_with_valid_param)
{
    ...
    ReqHandleResult result = ReqHandleResult::COMPLETED;
    MOCKER(&HrtRaGetAsyncReqResult).stubs().with().will(returnValue(result));
    MOCKER(&ra_ctx_qp_destroy_batch_async).stubs().will(invoke(ra_ctx_qp_destroy_batch_async_no_delete));
    BatchDeleteJettys();
...
int ra_ctx_qp_destroy_batch_async_return_false(void *ctx_handle, void*qp_handle[], unsigned int *num, void **req_handle) {
    return 1;
}

TEST_F(RmaConnManagerTest, should_failed_when_calling_batchdeletejettys_with_valid_param)
{
    BasePortType     basePortType(PortDeploymentType::P2P, ConnectProtoType::UB);
    LinkData *linkData = new LinkData(basePortType, 0, 1, 0, 1);
...
int ra_ctx_qp_destroy_batch_async_num_false(void *ctx_handle, void*qp_handle[], unsigned int *num, void **req_handle) {
    *num = *num + 1;
    return 0;
}
...
TEST_F(RmaConnManagerTest, should_delete_part_when_calling_batchdeletejettys_with_valid_param)
{
    ...
    ReqHandleResult result = ReqHandleResult::COMPLETED;
    MOCKER(&HrtRaGetAsyncReqResult).stubs().with().will(returnValue(result));
    MOCKER(&ra_ctx_qp_destroy_batch_async).stubs().will(invoke(ra_ctx_qp_destroy_batch_async_num_false));
    BatchDeleteJettys();
