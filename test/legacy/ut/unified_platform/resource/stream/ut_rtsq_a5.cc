...
TEST_F(RtsqA5Test, Ut_TryLaunchTask_When_NoEnoughSpace_Expect_NoAction)
{
    RtsqA5 rtsq(fakedevPhyId, fakeStreamId, fakeSqId);
    ...
    MOCKER_CPP(&RtsqA5::QuerySqHead).stubs().will(returnValue(0));

    rtsq.TryLaunchTask();

    EXPECT_EQ(rtsq.pendingSqeCnt, 5);
}
