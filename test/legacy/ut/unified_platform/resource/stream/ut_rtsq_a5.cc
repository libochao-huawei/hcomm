...
TEST_F(RtsqA5Test, launch_task_with_loop_back)
{
    ...
    MOCKER_CPP(&RtsqBase::QuerySqHead).stubs().with(_).will(returnValue(oldHead));
    rtsq.RefreshInfo();
    rtsq.RefreshInfo();
    u32 newTail = (rtsq.sqTail_ + rtsq.pendingSqeCnt) % rtsq.sqDepth_;
    rtsq.LaunchTask();
...
TEST_F(RtsqA5Test, config_sq_status_by_type)
{
    ...
    GlobalMockObject::reset();
    MOCKER(halSqCqConfig).stubs().with(_, _).will(returnValue(1));
...
{
    RtsqA5 rtsq(fakedevPhyId, fakeStreamId, fakeSqId);
    MOCKER_CPP(&RtsqA5::QuerySqHead).stubs().with(_).will(returnValue(2));
    rtsq.sqTail_ = 6;
    rtsq.sqDepth_ = 16;
    rtsq.pendingSqeCnt = 8;
    MOCKER(memcpy_s).stubs().with(_).will(returnValue(1));
    ...
    MOCKER_CPP(&RtsqA5::QuerySqHead).stubs().with(_).will(returnValue(10));
    rtsq.sqTail_ = 1;
    rtsq.sqDepth_ = 16;
    rtsq.pendingSqeCnt = 8;
    MOCKER(memcpy_s).stubs().with(_).will(returnValue(1));
