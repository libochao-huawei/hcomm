...
TEST_F(AicpuUbConnLiteTest, test_UBConnLite_WriteReduceWithNotify)
{
    ...
    MOCKER(memcpy_s).stubs().with(_).will(returnValue(0));
    MOCKER_CPP(&RtsqBase::QuerySqHead).stubs().with(_).will(returnValue(0));
    MOCKER_CPP(&RtsqBase::QuerySqTail).stubs().with(_).will(returnValue(1));
...
TEST_F(AicpuUbConnLiteTest, test_UBConnLite_WriteWithNotify_Throw)
{
    ...
    StreamLite stream(uniqueId);
    ubConn.ci = 0;
    MOCKER(memset_s).stubs().with(_).will(returnValue(1));
    MOCKER(memcpy_s).stubs().with(_).will(returnValue(0));
