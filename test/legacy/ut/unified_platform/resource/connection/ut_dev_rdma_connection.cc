...
TEST_F(DevRdmaConnectionTest, rma_net_connection_get_status_return_ok)
{
    ...
    MOCKER(HrtRaQpConnectAsync).stubs().with(_, _).will(returnValue(0));
    // construct DevRdmaConnection
    DevRdmaConnection devRdmaConnection(fakeSocket, rdmaHandle, OpMode::OPBASE);

    //  When: qp 建链成功
    MOCKER(RaQpConnectAsync).stubs().with(_, _).will(returnValue(0));
    MOCKER(HrtGetRaQpStatus).stubs().with(_, _, _).will(returnValue(1));
...
TEST_F(DevRdmaConnectionTest, rma_net_connection_get_status_return_time_connecting)
{
    ...
    // When
    DevRdmaConnection devRdmaConnection(fakeSocket, rdmaHandle, OpMode::OPBASE);
    MOCKER(HrtGetRaQpStatus).stubs().with(_).will(returnValue(0)).then(returnValue(1));
