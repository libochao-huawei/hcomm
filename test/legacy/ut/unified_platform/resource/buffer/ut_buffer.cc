...
protected:
    ...
    virtual void SetUp()
    {
        MOCKER(HrtMallocHost).stubs().with(_).will(returnValue((void *)fakeAddr));
        MOCKER(HrtFreeHost).stubs().with(_);
        MOCKER(HrtMalloc).stubs().with(_,_).will(returnValue((void *)fakeAddr));
        MOCKER(HrtFree).stubs().with(_);
