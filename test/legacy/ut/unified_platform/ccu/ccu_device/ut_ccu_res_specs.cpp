...
void MockDoOnce() // 提供给其他用例使用的ccu资源打桩，使用后会导致单例存在内存中，本用例不需要
{
    ...
    MOCKER(HrtRaCustomChannel)
        .stubs()
        .with(_, outBoundP(reinterpret_cast<void *>(&inBuff), sizeof(inBuff)),
...
{
    MOCKER(HrtGetDevicePhyIdByIndex).stubs().with(_).will(returnValue(MAX_MODULE_DEVICE_NUM));
...
void MockRtGetDeviceInfo(int64_t outVal)
{
    MOCKER(aclrtGetDeviceInfo)
        .stubs()
        .with(_, _, outBoundP(&outVal, sizeof(outVal)))
