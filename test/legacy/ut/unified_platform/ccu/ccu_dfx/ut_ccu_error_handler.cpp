...
protected:
    ...
    unique_ptr<CcuConnection> MockCcuConnection(uint32_t channelId)
    {
        ...
        MOCKER_CPP(&RdmaHandleManager::GetJfcHandle).stubs().with(_, _).will(returnValue(jfcHandle));
...
TEST_F(CcuErrorHandlerTest, test_error_info_when_rep_type_is_loc_post_sem)
{
    ...
    MOCKER(CcuErrorHandler::GetCcuCKEValue).stubs().with(_, _, _).will(returnValue(static_cast<u16>(0xabcd)));
...
TEST_F(CcuErrorHandlerTest, test_error_info_when_rep_type_is_rem_wait_sem)
{
    ...
    MOCKER(CcuErrorHandler::GetCcuCKEValue).stubs().with(_, _, eq(101)).will(returnValue(static_cast<u16>(0xabcd)));
...
TEST_F(CcuErrorHandlerTest, test_error_info_when_rep_type_is_rem_post_var)
{
    ...
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, eq(1)).will(returnValue(0xa));
...
TEST_F(CcuErrorHandlerTest, test_error_info_when_rep_type_is_rem_wait_group)
{
    ...
    MOCKER_CPP(&CcuTransportGroup::CheckTransportCntCke).stubs().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
    ...
    MOCKER(CcuErrorHandler::GetCcuCKEValue).stubs().with(_, _, eq(100)).will(returnValue(static_cast<u16>(0xabcd)));
...
TEST_F(CcuErrorHandlerTest, test_error_info_when_rep_type_is_post_shared_var)
{
    ...
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, eq(2)).will(returnValue(0xb));
...
TEST_F(CcuErrorHandlerTest, test_error_info_when_rep_type_is_read)
{
    ...
    MOCKER(CcuErrorHandler::GetCcuGSAValue).stubs().with(_, _, eq(1)).will(returnValue(0xa));
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, eq(2)).will(returnValue(0xb));
    ...
    MOCKER(CcuErrorHandler::GetCcuGSAValue).stubs().with(_, _, eq(3)).will(returnValue(0xc));
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, eq(4)).will(returnValue(0xd));

    Variable len;
    len.Reset(5);
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, eq(5)).will(returnValue(0xe));
...
TEST_F(CcuErrorHandlerTest, test_loop_group_error_info)
{
    ...
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, _).will(returnValue(loopGroupXn.value));
...
TEST_F(CcuErrorHandlerTest, test_gen_error_info_loop_should_throw_exception)
{
    ...
    MockCcuContext mockCcuCtx{};
    MOCKER_CPP(&MockCcuContext::GetRepByInstrId).stubs().with(_)
    ...
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, _).will(returnValue(0x5));
    CcuLoopContext loopCtx{};
    MOCKER(CcuErrorHandler::GetCcuLoopContext).stubs().with(_, _, _).will(returnValue(loopCtx));
    MOCKER_CPP(&CcuRepBlock::GetRepByInstrId).stubs().with(_).will(returnValue(nullRep));
...
TEST_F(CcuErrorHandlerTest, test_gen_error_info_loop)
{
    ...
    uint16_t mask = 0x0010;  // mask
    MOCKER(CcuErrorHandler::GetCcuCKEValue).stubs().with(_, _, eq(0xa)).will(returnValue(static_cast<u16>(0xabcd)));
    ...
    MOCKER(CcuErrorHandler::GetCcuXnValue).stubs().with(_, _, _).will(returnValue(loopXm.value));
