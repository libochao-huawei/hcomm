...
{
    // when
    MOCKER_CPP(&CcuInsPreprocessor::PrepareCcuCtx).stubs().with(_, _).will(ignoreReturnValue());
...
TEST_F(CcuInsPreprocessorTest, should_no_throw_when_calling_transportsconnect)
{
    ...
    MOCKER_CPP(&RdmaHandleManager::GetJfcHandle).stubs().with(_, _).will(returnValue((JfcHandle)0));
    MOCKER_CPP(&RdmaHandleManager::GetDieAndFuncId).stubs().with(_).will(returnValue(std::pair<uint32_t, uint32_t>(0,0)));
    ...
    MOCKER_CPP(&CcuTransport::SetHandshakeMsg).stubs().with(_).will(ignoreReturnValue());
