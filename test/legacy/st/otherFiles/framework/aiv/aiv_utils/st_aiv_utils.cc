...
TEST_F(AivUtilsTest, aiv_utils_test)
{
    ...
    MOCKER(realpath).stubs().with(any(), outBound(path)).will(returnValue(returnValueChar));
    RegisterKernel();

    char libPath[] = "./hcomm/test/llt/stub/workspace/fwkacllib/lib64";
    char* libPathPtr = libPath;

    MOCKER(getenv).stubs().with(any()).will(returnValue(libPathPtr));

    MOCKER(ReadBinFile).stubs().with(any()).will(returnValue(0));

    MOCKER(realpath).stubs().with(any(), outBound(path)).will(returnValue(nullptr));
    RegisterKernel();
