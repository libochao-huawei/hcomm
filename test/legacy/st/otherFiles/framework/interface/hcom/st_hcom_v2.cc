...
TEST_F(HcomTest, St_HcomAlltoAllVCV2_When_Normal_Input_Expect_Success)
{
    ...
    MOCKER_CPP(&HcclCommunicator::LoadOffloadCollOp).stubs().with(_, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomTest, ut_HcomGetAlltoAllStagedWorkSpaceMemSizeV2)
{
    ...
    MOCKER_CPP(&HcclCommunicator::CalcCollOffloadOpRes).stubs().with(_, _, _).will(returnValue(HCCL_SUCCESS));
...
TEST_F(HcomTest, St_HcomAllReduceV2_When_HcomCheckReductionOpV2_fail_Expect_HCCL_E_NOT_SUPPORT)
{
    ...
    recvbuf= (s8*)sal_malloc(count * sizeof(s8));
    sal_memset(recvbuf, count * sizeof(s8), 0, count * sizeof(s8));
    MOCKER(HcomCheckReductionOpV2).stubs().with(_).will(returnValue(HCCL_E_NOT_SUPPORT));
...
TEST_F(HcomTest, st_HcomGraphSelectAlgV2_When_Normal_Expect_ReturnHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::ExecAlgSelect).defaults().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
    MOCKER_CPP(&CommunicatorImpl::ReportProfInfo).defaults().with(_).will(ignoreReturnValue());
...
TEST_F(HcomTest, st_HcclGetAlgExecParamV2_When_Normal_Expect_ReturnHCCL_SUCCESS)
{
    ...
    MOCKER_CPP(&CommunicatorImpl::ExecAlgSelect).defaults().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
    MOCKER_CPP(&CommunicatorImpl::ReportProfInfo).defaults().with(_).will(ignoreReturnValue());
    MOCKER_CPP(&CommunicatorImpl::GetAlgExecParam).defaults().with(_).will(returnValue(HcclResult::HCCL_SUCCESS));
