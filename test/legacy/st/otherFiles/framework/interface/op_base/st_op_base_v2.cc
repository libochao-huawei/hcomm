...
TEST_F(OpbaseTestV2, HcclSnapshotRecoverAllComms_OK)
{
    ...
    MOCKER_CPP(&HcclCommunicator::RecoverComm).stubs().with(_, _).will(returnValue(HCCL_E_PARA));
    ...
    MOCKER_CPP(&HcclCommunicator::RecoverSubComm).stubs().with(_, _, _).will(returnValue(HCCL_E_PARA));
