...
{
    shared_ptr<TaskInfo> taskInfo = InitTaskInfo();
    MOCKER(TaskExceptionHandler::GetRankIdByChannelId).stubs().with(eq(static_cast<uint16_t>(1)), _).will(returnValue(100));
...
{
    shared_ptr<TaskInfo> taskInfo = InitTaskInfo();
    MOCKER(TaskExceptionHandler::GetRankIdByChannelId).stubs().with(_, _)
