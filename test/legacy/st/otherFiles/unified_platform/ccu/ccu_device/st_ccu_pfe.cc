...
TEST_F(CcuPfeTest, GetDevEidInfoListPass)
{
    ...
    MOCKER(RaGetDevEidInfoNum)
        .stubs()
        .with(_, outBoundP(&num, sizeof(num)))
        .will(returnValue(0));

    MOCKER(RaGetDevEidInfoList)
        .stubs()
        .with(_, _, outBoundP(&num, sizeof(num)))
        .will(returnValue(0));
...
TEST_F(CcuPfeTest, GetPfeJettyStrategy)
{
    ...
    MOCKER(HrtRaCustomChannel)
        .stubs()
        .with(_, outBoundP(reinterpret_cast<void *>(&inBuff), sizeof(inBuff)), outBoundP(reinterpret_cast<void *>(&inBuff), sizeof(inBuff)));
    MOCKER(HrtGetDevicePhyIdByIndex)
        .stubs()
        .with(_)
        .will(returnValue(0));
