...
{
    // Given
    MOCKER(HrtRaSocketListenOneStart).stubs().with(_, _);
    MOCKER(HrtRaSocketListenOneStop).stubs().with(_);
    // when
    socketServer->Listen();
...
TEST_F(SocketTest, server_connect_ok_then_close)
{
    ...
    MOCKER(HrtRaBlockGetOneSocket).stubs().with(_, _).will(returnValue(fakeParam));
    ...
    MOCKER(HrtRaSocketCloseOne).stubs().with(_);
    socketServer->Close();
}
