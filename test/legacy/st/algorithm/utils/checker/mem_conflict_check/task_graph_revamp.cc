        ...
        if ((waitParent->rankIdx == peerRank) && (waitParent->task->GetType() == TaskTypeStub::POST)) {
            ...
            if (waitParent->queIdx != peerRankVirtQueTailNode->queIdx) {
                ...
                CHK_PRT_RET(
                    ...
                    HCCL_ERROR(
                        "[GraphRevampBilateralSemantics] Not able to insert localPostToShadow between Post(currRank) "
                        "and "
                        "original next node."),
                    HcclResult::HCCL_E_INTERNAL);
            ...
            for (; waitParentIter != postChild->parents.end(); waitParentIter++) {
                if ((*waitParentIter)->rankIdx == peerRank) {
                    CHK_PRT_RET(
                        ...
                        HCCL_ERROR(
                            "[GraphRevampBilateralSemantics] Not able to insert localWaitFreom between Wait(peerRank) "
                            "and "
    ...
    TaskNodePtr recvNode, TaskNodePtr sendNode, TaskNodePtr currNode, VirtAivBlockMgr& virtAivBlockManager)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[GraphRevampBilateralSemantics] Not able to insert SetFlagShadow between SendSync and "
            "original next node."),
        HcclResult::HCCL_E_INTERNAL);
    ...
    for (; recvParentsIter != recvNode->parents.end(); recvParentsIter++) {
        if ((*recvParentsIter)->rankIdx == peerRank && (*recvParentsIter)->rankPos == peerRankPos
            ...
            CHK_PRT_RET(
                ...
                HCCL_ERROR(
                    "[GraphRevampBilateralSemantics] Not able to insert WaitFlagShadow between recvSync and "
                    "original next node."),
                HcclResult::HCCL_E_INTERNAL);
            break;
        }
