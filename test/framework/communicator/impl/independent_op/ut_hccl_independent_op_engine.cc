...
TEST_F(HcclIndependentOpEngineTest, Ut_HcclThreadAcquire_When_Param_Is_Invalid_Expect_Para_Error)
{
    ...
    CreateCommByConfig(&comm, static_cast<int32_t>(CommEngine::COMM_ENGINE_CPU), 4, 2);
    ...
    ret = HcclThreadAcquire(comm, CommEngine::COMM_ENGINE_CPU, 5, 1, threads);
    EXPECT_EQ(ret, HCCL_E_PARA);
    ret = HcclThreadAcquire(comm, CommEngine::COMM_ENGINE_CPU, 2, 3, threads);
    EXPECT_EQ(ret, HCCL_E_PARA);
...
TEST_F(HcclIndependentOpEngineTest, Ut_HcclThreadAcquire_When_Alloced_Threads_Morethan_Quota_Expect_Unavailable)
{
    ...
    ret = HcclThreadAcquire(comm, g_hostEngine, 2, 1, thread3);
...
{
    CreateCommByConfig(&comm, static_cast<int32_t>(CommEngine::COMM_ENGINE_CPU), 0, 0);
    ThreadHandle thread1[2] = {0};
