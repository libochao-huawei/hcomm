...
TEST_F(AlgTemplateBaseTest, alg_template_base_prepare)
{
    ...
    HcclReduceOp::HCCL_REDUCE_SUM, HcclDataType::HCCL_DATA_TYPE_INT8, isUseCclIn, isLevel0LastRank));
    ...
    SendRecvInfo sendrecvInfoData;
    sendrecvInfoData.sendLength.assign(10, 1);
    sendrecvInfoData.sendOffset.assign(10, 1);
    sendrecvInfoData.recvLength.assign(10, 1);
    sendrecvInfoData.recvOffset.assign(10, 1);
    sendrecvInfoData.sendCounts.assign(10, 1);
    sendrecvInfoData.sendDispls.assign(10, 1);
    sendrecvInfoData.recvCounts.assign(10, 1);
    sendrecvInfoData.recvDispls.assign(10, 1);
    sendrecvInfo.push_back(sendrecvInfoData);
    EXPECT_EQ(2, GetGlobalMaxUserInSize(sendrecvInfo));
}
...
TEST_F(AlgTemplateBaseTest, alltoallv_template_instance_init)
{
    ...
    tempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(
        TemplateType::TEMPLATE_ALL_2_ALL_V_MESH_READ_ONLY, disp);
    EXPECT_EQ(HcclResult::HCCL_SUCCESS, tempAlg->Prepare(devMem, devMem, devMem, devMem, alltoallAddr, alltoallAddr,
        HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE, mainStream, subStreams,
        signals, signals, 0, 1, links, sendrecvInfo));
