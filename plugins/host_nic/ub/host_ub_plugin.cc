...
int32_t InitEndpoint(void *endpointCtx)
{
    ...
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->Init();
}
...
HcommResult CreateEndpoint(const EndpointDesc *endpoint, void **endpointCtx)
{
    ...
    CHK_PTR_NULL(endpointCtx);
    auto ep = std::unique_ptr<CpuUrmaEndpoint>(new (std::nothrow) CpuUrmaEndpoint(*endpoint));
...
{
    delete static_cast<CpuUrmaEndpoint *>(endpointCtx);
}
...
HcommResult StartListen(void *endpointCtx, uint32_t port, HcommEndpointListenConfig *config)
{
    (void)config;
    CHK_PTR_NULL(endpointCtx);
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->ServerSocketListen(port);
}
...
HcommResult MemReg(void *endpointCtx, const CommMem *mem, const char *memTag, void **memHandle)
{
    ...
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->RegisterMemory(*mem, memTag, memHandle);
}

HcommResult MemUnreg(void *endpointCtx, void *memHandle)
{
    CHK_PTR_NULL(endpointCtx);
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->UnregisterMemory(memHandle);
}

HcommResult MemExport(void *endpointCtx, void *memHandle, void **memDesc, uint32_t *memDescLen)
{
    CHK_PTR_NULL(endpointCtx);
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->MemoryExport(memHandle, memDesc, memDescLen);
}

HcommResult MemImport(void *endpointCtx, const void *memDesc, uint32_t descLen, CommMem *outMem)
{
    CHK_PTR_NULL(endpointCtx);
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->MemoryImport(memDesc, descLen, outMem);
}

HcommResult MemUnimport(void *endpointCtx, const void *memDesc, uint32_t descLen)
{
    CHK_PTR_NULL(endpointCtx);
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->MemoryUnimport(memDesc, descLen);
}

HcommResult MemGetAllHandles(void *endpointCtx, void **memHandles, uint32_t *memHandleNum)
{
    CHK_PTR_NULL(endpointCtx);
    return static_cast<CpuUrmaEndpoint *>(endpointCtx)->GetAllMemHandles(memHandles, memHandleNum);
}
...
int32_t InitChannel(void *channelCtx)
{
    ...
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->Init();
}
...
HcommResult CreateChannel(void *endpointCtx, const HcommChannelDesc *channelDesc, void **channelCtx)
{
    ...
    CHK_PTR_NULL(channelCtx);
    auto ch = std::unique_ptr<HostCpuUrmaChannel>(
        new (std::nothrow) HostCpuUrmaChannel(reinterpret_cast<EndpointHandle>(endpointCtx), *channelDesc));
...
{
    delete static_cast<HostCpuUrmaChannel *>(channelCtx);
}
...
{
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->GetNotifyNum(notifyNum);
}

HcommResult GetRemoteMem(void *channelCtx, CommMem **remoteMem, uint32_t *memNum, char **memTags)
{
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->GetRemoteMem(reinterpret_cast<HcclMem **>(remoteMem),
        memNum, memTags);
}
...
{
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->UpdateMemInfo(memHandles, memHandleNum);
}
...
HcommResult NotifyRecord(void *channelCtx, ThreadHandle thread, uint32_t remoteNotifyIdx)
{
    (void)thread;
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->NotifyRecord(remoteNotifyIdx);
}
...
HcommResult NotifyWait(void *channelCtx, ThreadHandle thread, uint32_t localNotifyIdx, uint32_t timeout)
{
    (void)thread;
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->NotifyWait(localNotifyIdx, timeout);
}
    ...
    uint32_t remoteNotifyIdx)
{
    (void)thread;
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->WriteWithNotify(dst, src, len, remoteNotifyIdx);
}
...
HcommResult Write(void *channelCtx, ThreadHandle thread, void *dst, const void *src, uint64_t len)
{
    (void)thread;
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->Write(dst, src, len);
}
...
HcommResult Read(void *channelCtx, ThreadHandle thread, void *dst, const void *src, uint64_t len)
{
    (void)thread;
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->Read(dst, src, len);
}
...
HcommResult Fence(void *channelCtx, ThreadHandle thread)
{
    (void)thread;
    CHK_PTR_NULL(channelCtx);
    return static_cast<HostCpuUrmaChannel *>(channelCtx)->ChannelFence();
}
    ...
    HcommNicEndpointOps **outOps)
{
    ...
    CHK_PTR_NULL(outCtx);
    CHK_PTR_NULL(outOps);
    CHK_PRT_RET(epDescLen < sizeof(EndpointDesc), HCCL_ERROR("[HostUbPlugin] invalid endpoint desc len[%u].",
    ...
    void **outCtx, HcommNicChannelOps **outOps)
{
    ...
    CHK_PTR_NULL(outCtx);
    CHK_PTR_NULL(outOps);
    CHK_PRT_RET(chDescLen < sizeof(HcommChannelDesc), HCCL_ERROR("[HostUbPlugin] invalid channel desc len[%u].",
