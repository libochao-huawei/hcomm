...
function download_mockcpp() {
    ...
    # 下载mockcpp
    log "Info: Downloading mockcpp... 本地编译暂时不能下载 需要预先放置"
...
function build_llt_test() {
    ...
    # fi
    UT_TARGES="hccl_ut_prepare_env"
    cmake --build . --target ${UT_TARGES[@]} -j $CORE_NUMS
    # if [[ "$cov" =~ "TRUE" ]]; then
        # cmake --build . --target generate_ops_cpp_cov -- -j $CORE_NUMS
    # fi

    if [ "${TEST_TASK_NAME}" == "hcomm_ut_test" ];then
        log "Info: build_hcomm_ut"

        TARGET_LIST="ut_hccl_ut_prepare_env"
                    #  ut_hccl_utest_platform_aicpu
                    #  ut_hccl_utest_misc
                    #  ut_hccl_utest_hcom
                    #  ut_hccl_utest_inter_misc
                    #  ut_hccl_utest_inter_comm
                    #  ut_hccl_utest_inter_executor_misc
                    #  ut_hccl_utest_inter_executor_inter
                    #  ut_hccl_utest_inter_executor_all
                    #  ut_hccl_utest_inter_hcom
                    #  ut_hccl_utest_ffts
                    #  ut_hccl_utest_mc2_misc
                    #  ut_hccl_utest_mc2_aicpu
                    #  ut_hccl_utest_impl
                    #  ut_hccl_utest_pingmesh
                    #  ut_hccl_utest_api_single_thread
                    #  ut_hccl_utest_device
                    #  ut_hccl_utest_topo_detect"

        # echo "TARGET_LIST=${TARGET_LIST}"
        # build ${TARGET_LIST}
    fi

    # if [ "${TEST_TASK_NAME}" == "hcomm_st_test" ];then
    #     log "Info: build_hcomm_st"

    #     TARGET_LIST="st_hccl_st_prepare_env
    #                 st_hccl_stest_misc
    #                 st_hccl_stest_comm
    #                 st_hccl_stest_coll
    #                 st_hccl_stest_impl
    #                 st_hccl_stest_executor_misc
    #                 st_hccl_stest_executor_inter
    #                 st_hccl_stest_algorithms_910B
    #                 st_hccl_stest_algorithms_8pring
    #                 st_hccl_stest_hcom
    #                 st_hccl_stest_platform_aicpu
    #                 st_hccl_stest_mc2_misc
    #                 st_hccl_stest_mc2_aicpu
    #                 st_hccl_stest_pingmesh
    #                 st_hccl_stest_device"
...
while [[ $# -gt 0 ]]; do
    ...
    --llt_test_ut)
        ...
        TEST_TASK_NAME="hcomm_ut_test"
        shift
        ;;
    --llt_test_st)
        LLT_TEST="partial"
        DOWNLOAD_MOCKCPP="true"
        TEST_TASK_NAME="hcomm_st_test"
        shift
        ;;
...
if [ -n "${LLT_TEST}" ];then
    CUSTOM_OPTION="${CUSTOM_OPTION} -DENABLE_TEST=ON -DENABLE_ASAN=ON"
    if [ "${TEST_TASK_NAME}" == "hcomm_ut_test" ];then
        CUSTOM_OPTION="${CUSTOM_OPTION} -DHCOMM_UT=ON"
    elif [ "${TEST_TASK_NAME}" == "hcomm_st_test" ];then
        CUSTOM_OPTION="${CUSTOM_OPTION} -DHCOMM_ST=ON"
    fi
fi
