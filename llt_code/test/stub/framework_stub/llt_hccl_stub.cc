...
rtError_t rtGetPairDevicesInfo(uint32_t devId, uint32_t otherDevId, int32_t infoType, int64_t *value)
{
    ...
    {
        // 0-1 2-3 4-5 6-7
        if ((abs(static_cast<int>(devId - otherDevId)) == 1) && ((devId + otherDevId) % 4 == 1)) {
            *value = 5;     // SIO
        } else {
            *value = 6;     // HCCS_SW
        }
...
rtError_t rtsMemcpyAsync(void *dst, uint64_t destMax, const void *src, uint64_t count, rtMemcpyKind kind,
    rtMemcpyConfig_t *config, rtStream_t stream)
{
    /*拷贝类型检测*/
    if ((kind < RT_MEMCPY_KIND_HOST_TO_HOST) || (kind > RT_MEMCPY_KIND_DEVICE_TO_DEVICE))
    {
        return ACL_ERROR_RT_PARAM_INVALID;
    }
...
rtError_t rtMemcpyAsyncWithOffset(void **dst, uint64_t destMax, uint64_t destOffset, const void **src,
    uint64_t count, uint64_t srcOffset, rtMemcpyKind kind, rtStream_t stream)
{

    std::cout << "rtMemcpyAsyncWithOffset rtStream_t: " << stream << std::endl;
    ...
    stream_task.task_type = TASK_TYPE_MEMCPY;
    stream_task.stream_para.memcpystruct.dst = *dst;
    stream_task.stream_para.memcpystruct.src = static_cast<void *>(*src);
    stream_task.stream_para.memcpystruct.count = count;
...
aclError aclrtFree( void* devPtr )
{
    ...
    void* name_map_ptr = (void*)shm_head->relate_ptr ; // name map的地址作为本块共享内存的关联指针

    HCCL_DEBUG("devPtr[%p] relate_ptr name_map_ptr[%p], ", devPtr, name_map_ptr);

    /* 关闭该段内存的name map */
    if (NULL != name_map_ptr)
    {
        (void)sal_close_name_map(name_map_ptr);
        shm_head->relate_ptr = NULL;
    }
...
aclError aclrtIpcMemGetExportKey(void *ptr, size_t byteCount, char *name, size_t nameLen, uint64_t flag)
{
    ...
    shm_head->relate_ptr = name_map_ptr; // 本块共享内存的关联指针记录name map的地址

    HCCL_DEBUG("set ptr[%p] name_map_ptr[%p]", ptr, name_map_ptr);
...
{
    HCCL_DEBUG("open memory name[%s] match name_map -> shm_real_name[%s], mem_size(%d)",
              name, name_map_ptr->shm_real_name, name_map_ptr->mem_size);
...
rtError_t rtIpcCloseMemory(const void* ptr)
{
    ...
    sal_share_memory_destroy((void*)ptr);
...
{
#ifndef HCCD
    std::string targetChipVer;
    CHK_RET(hrtGetSocVer(targetChipVer));
    auto iter = SOC_VER_CONVERT.find(targetChipVer);
