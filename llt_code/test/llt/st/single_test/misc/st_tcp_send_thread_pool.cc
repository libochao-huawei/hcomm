...
TEST_F(MPI_SendThreadPool_Test, st_SendThreadPool_Init_Bind_CPU)
{
    GlobalMockObject::verify();
...
TEST_F(MPI_SendThreadPool_Test, st_GetThreadNum)
{
...
TEST_F(MPI_SendThreadPool_Test, st_ThreadTaskQueueAddTask)
{
    u32 devId = 0;
    u32 tag = 0;
    u32 threadSerial = 0;
...
TEST_F(MPI_SendThreadPool_Test, st_LoadBalancing)
{
    u32 devId = 0;
    HcclResult ret = TcpSendThreadPool::GetSendPoolInstance()->Init(devId);
    EXPECT_EQ(ret, HCCL_SUCCESS);
