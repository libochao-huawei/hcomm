...
TEST_F(AllReduceNHRV1InterTest, test_CalcVSlicesAndLinks)
{
    ...
    ret = tempAlg->Prepare(input, output, scratch, 128, HCCL_DATA_TYPE_INT8, stream, HcclReduceOp::HCCL_REDUCE_SUM,
        -1, slices);
...
TEST_F(AllReduceNHRV1InterTest, AllReduceNHRV1_Constructor)
{
    u64 reduceAttrBitMap = 0x01;

    // 获取模板实例
    std::unique_ptr<AlgTemplateBase> tempAlg = AlgTemplateRegistry::Instance().GetAlgTemplate(
        TemplateType::TEMPLATE_ALL_REDUCE_NHR_V1, dispatcher);
	...
	// 转换子类指针
    AllReduceNHRV1 *alg = dynamic_cast<AllReduceNHRV1 *>(tempAlg.get());
	// 校验成员变量
