...
TEST_F(HcclCommTest, hcclComm_allgather)
{
    ...
    rt_ret = aclrtMallocWithCfg(&mem_dev_input, 1024, ACL_MEM_MALLOC_HUGE_FIRST, &cfg);
    EXPECT_EQ(rt_ret, RT_ERROR_NONE);

    void* mem_dev_output = NULL;
    rt_ret = aclrtMallocWithCfg(&mem_dev_output, 1024, ACL_MEM_MALLOC_HUGE_FIRST, &cfg);
    EXPECT_EQ(rt_ret, RT_ERROR_NONE);
...
TEST_F(HcclCommTest, hcclComm_broadcast)
{
    ...
    aclrtMallocConfig cfg{.attrs = &attrs, .numAttrs = 1};
    void* mem_dev = NULL;
    rt_ret = aclrtMallocWithCfg(&mem_dev, 1024, ACL_MEM_MALLOC_HUGE_FIRST, &cfg);
    EXPECT_EQ(rt_ret, RT_ERROR_NONE);
...
TEST_F(HcclCommTest, hcclComm_broadcast_mesh)
{
    ...
    aclrtMallocConfig cfg{.attrs = &attrs, .numAttrs = 1};
    void* mem_dev = NULL;
    rt_ret = aclrtMallocWithCfg(&mem_dev, size, ACL_MEM_MALLOC_HUGE_FIRST, &cfg);
    EXPECT_EQ(rt_ret, RT_ERROR_NONE);
...
TEST_F(HcclCommTest, hcclComm_reduce_scatter_ring)
{
    ...
    //-----------------Set Workspace Resource Start------------------//
    u64 stream_list_size = 0;
    ret = comm.GetWorkspaceSubStreamNum(stream_list_size);
    EXPECT_EQ(ret, HCCL_SUCCESS);
    HCCL_INFO("get stream_list_size[%d] success", stream_list_size);
    vector<HcclRtStream> streamList(stream_list_size);
    //从流bind到model
...
TEST_F(HcclCommTest, hcclImpl_check_params_err)
{
    ...
    ret = hcom_info.pComm->init(params, rankTable);
    EXPECT_EQ(ret, HCCL_E_PARA);

    std::vector<RankInfo> rankList;
    WorldGroupInfo groupCommonData;

    ret = hcom_info.pComm->init(params, rankList, groupCommonData);
    EXPECT_EQ(ret, HCCL_E_PARA);
...
TEST_F(HcclCommTest, ut_comm_allreduce_ring_V80_inline)
{
    nlohmann::json rank_table = rank_table_910_1server_4rank;
...
TEST_F(HcclCommTest, ut_comm_allreduce_4pmesh_V80_inline)
{
    nlohmann::json rank_table = rank_table_910_1server_4rank;
...
TEST_F(HcclCommTest, ut_hcclComm_allreduce_8pring_inline)
{

    nlohmann::json rank_table = rank_table_1server_8rank;

    char file_name_t[] = "./ut_hcclComm_allreduce_8pring_inline.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
...
TEST_F(HcclCommTest, hcclComm_test_ring_algo)
{
    HcclCommParams params;
    RankTable_t rankTable;
    TestConstructParam(params, rankTable);
...
TEST_F(HcclCommTest, hcclComm_test_nhr_algo)
{
    HcclCommParams params;
    RankTable_t rankTable;
    TestConstructParam(params, rankTable);
...
TEST_F(HcclCommTest, hcclComm_test_algo)
{
    HcclCommParams params;
    RankTable_t rankTable;
    TestConstructParam(params, rankTable);
...
TEST_F(HcclCommTest, ut_multiServerDiffDeviceNumMode_SetAlgType)
{
    HcclCommParams params;
    RankTable_t rankTable;
    TestConstructParam(params, rankTable);
    ...
    std::map<HcclCMDType, AlgType> algType;
    impl->multiModuleDiffDeviceNumMode_ = true;
...
TEST_F(HcclCommTest, ut_multiModuleDiffDeviceNumMode_CheckSingleServerComm)
{
    // module0:0,1,2,3  module1:8,9,10
    HcclCommParams params;
    RankTable_t rankTable;
    ...
    EXPECT_EQ(ret, HCCL_SUCCESS);
    impl->attrCollector_.serverNum_ = 1;
    impl->attrCollector_.moduleNum_ = 2;
    ret = impl->attrCollector_.CheckSingleServerComm(rankTable.rankList);
    EXPECT_EQ(ret, HCCL_E_NOT_SUPPORT);
...
TEST_F(HcclCommTest, ut_multiModuleDiffDeviceNumMode_CheckSingleServerComm_Comm)
{
    // module0:0,1,2,3  module1:8,9,10
    HcclCommParams params;
    RankTable_t rankTable;
    ...
    impl->serverNum_ = 1;
    impl->moduleNum_ = 2;
    ret = impl->CheckSingleServerComm(rankTable.rankList);
    EXPECT_EQ(ret, HCCL_E_NOT_SUPPORT);

    // module0:0,1,2,3  module1:8,9,10,13
    RankInfo_t rank;
    rank.rankId = 8;
    rank.serverIdx = 0;
    rank.serverId = "192.168.1.1";
    rank.deviceInfo.devicePhyId = 13;
    rank.deviceInfo.deviceIp.push_back(HcclIpAddress("172.17.10.1"));
    rankTable.rankList.push_back(rank);
    params.totalRanks = 8;
    rankTable.deviceNum = 8;
...
TEST_F(HcclCommTest, hcclComm_SetAttachedStream)
{
    hcclComm h;
    ...
    EXPECT_EQ(HcomSetAttachedStream(nullptr, emptyStreams), HCCL_SUCCESS);
