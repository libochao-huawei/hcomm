...
TEST_F(HcomLoweringTest, st_hcomLaunchKernel_allgatherv_count0)
{
    ...
    HcomAllGatherVKernel(launchArgs);
...
TEST_F(HcomLoweringTest, st_hcomLaunchKernel_reducescatterv_count0)
{
    ...
    .with(mockcpp::any())
    .will(returnValue(HCCL_SUCCESS));
    HcomReduceScatterVKernel(launchArgs);
...
TEST_F(HcomLoweringTest, st_hcomLaunchKernel_errortest3)
{
    nlohmann::json rank_table = rank_table_910_1server_1rank;
    char file_name_t[] = "./st_hcomLaunchKernel_errortest3.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
    ...
    gert::KernelContext *kernelContext = new gert::KernelContext();
    KernelRunContext context;
    AsyncAnyValue any_value;
    ...
    *context.values = &any_value;
    context.input_size = 1;
    *(kernelContext->GetContext()) = context;

    MOCKER(HcomSetWorkflowMode)
    .stubs()
    .with(mockcpp::any())
    .will(returnValue(HCCL_E_PARA));
    LaunchHcomKernel(kernelContext);
...
TEST_F(HcomLoweringTest, st_hcomLaunchKernel_errortest4)
{
    nlohmann::json rank_table = rank_table_910_1server_1rank;
    char file_name_t[] = "./st_hcomLaunchKernel_errortest4.json";
    std::ofstream outfile(file_name_t, std::ios::out | std::ios::trunc | std::ios::binary);
...
TEST_F(HcomLoweringTest, st_HcomReduceScatterKernelV2) 
{
    ...
    result = HcomAllToAllVCKernelV2(launchArgs);
    EXPECT_EQ(result, HCCL_SUCCESS);

    result = HcomGatherAllToAllVKernel(launchArgs);
    EXPECT_EQ(result, HCCL_E_NOT_SUPPORT);

    result = HcomReduceKernelV2(launchArgs);
    EXPECT_EQ(result, HCCL_SUCCESS);
}
...
TEST_F(HcomLoweringTest, st_HcomCopyInputsToCCLbuff) 
{
    ...
    HcclResult result = HcomCopyInputsToCCLbuff(launchArgs, inputsNum, inputsOffset, inputsCount, cclBuff, cclBuffSize, commCount);
    EXPECT_EQ(result, HCCL_SUCCESS);
    result = HcomCopyCCLbuffToOutnputs(launchArgs, inputsNum, inputsOffset, inputsCount, cclBuff);
    EXPECT_EQ(result, HCCL_SUCCESS);
