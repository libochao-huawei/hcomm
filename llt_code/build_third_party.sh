...
download_and_compile() {
    ...
    # Downloading mockcpp
    wget -O "${OUTPUT_PATH}/pkg/mockcpp-2.7.zip" https://gitee.com/sinojelly/mockcpp/repository/archive/v2.7.zip
    if [ $? -ne 0 ]; then
    ...
    # Downloading mpich
    wget -O "${OUTPUT_PATH}/pkg/mpich-4.1.3.tar.gz" https://www.mpich.org/static/downloads/4.1.3/mpich-4.1.3.tar.gz
    if [ $? -ne 0 ]; then
      echo "Failed to download mpich files"
      exit 1;
    fi

    tar -zxvf "${OUTPUT_PATH}/pkg/mpich-4.1.3.tar.gz" -C "${OUTPUT_PATH}/mpich_shared"
    if [ $? -ne 0 ]; then
      echo "Failed to extract mpich files"
      exit 1;
    fi
    cd "${OUTPUT_PATH}/mpich_shared/mpich-4.1.3/"
    ./configure --disable-fortran  --prefix=${OUTPUT_PATH}/mpich_shared
