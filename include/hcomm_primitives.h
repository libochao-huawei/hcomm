...
/**
 * @brief 传输类型枚举
 * @note 当前仅支持 HCOMM_TRANSFER_TYPE_WRITE 和 HCOMM_TRANSFER_TYPE_READ，
 *       HCOMM_TRANSFER_TYPE_WRITE_WITH_NOTIFY 暂不支持。
 */
typedef enum {
    HCOMM_TRANSFER_TYPE_INVALID = -1, ///< 无效的传输类型
    HCOMM_TRANSFER_TYPE_WRITE = 0,    ///< 写操作
    HCOMM_TRANSFER_TYPE_READ = 1,     ///< 读操作
    HCOMM_TRANSFER_TYPE_WRITE_WITH_NOTIFY = 2, ///< 带通知的写操作（暂不支持）
} HcommTransferType;
