 ...
 * @return 执行状态码 HcommResult
 */
extern HcommResult HcommTcpSocketAccept(HcommTcpSocket serverSocket, char *handShakeTag, uint32_t tagLen, HcommTcpSocket *socket);

struct HcommTcpSocketAcceptInfo {
    char *handShakeTag;
    uint32_t tagLen;
};
extern HcommResult HcommTcpSocketBatchAccept(HcommTcpSocket serverSocket, HcommTcpSocketAcceptInfo *acceptInfos, uint32_t acceptNum, HcommTcpSocket *socket);
...
extern HcommResult HcommTcpSocketBatchGetStatus(HcommTcpSocket *socket, int32_t *status, uint32_t socketNum);
