...
HcclResult AllGatherOperator::SelectAlgforMix(const OpParam& param, std::string& algName)
{
    ...
    if (gcdDeviceNumPerAggregation_ > 1) {
        ...
        HCCL_WARNING(
            "[AllGatherOperator][SelectAlgforMix]only support NHR in AlgoLevel1 yet, "
            "default is algType=NHR.");
        algName = "AllGatherMixExecutor";
    } else {
        algType_.algoLevel1 = AlgTypeLevel1::ALG_LEVEL1_RING;
        HCCL_WARNING(
            "[AllGatherOperator][SelectAlgforMix]only support ring in AlgoComm yet, "
...
HcclResult AllGatherOperator::SelectAlgfor91093(const OpParam& param, std::string& algName)
{
    ...
    if (isARSAlgo) {
        if (!(algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_NB
            ...
            algType_.algoLevel1 = AlgTypeLevel1::ALG_LEVEL1_NHR;
            HCCL_WARNING(
                "[AllGatherOperator][SelectAlgfor91093] ARS only support NHR or RING in AlgoLevel1 "
                "yet, default is NHR.");
        }
    }
    // AHC 算法选择逻辑
    ...
    if (isHccsPlusSio && isSupportHccsAndSio_) {
        ...
        superPodNum_ > 1 && maxCountPerLoop * unitSize > ALLGATHER_PIPELINE_THRESHOLD
        && bufferSliceNum > ALLGATHER_PIPELINE_SLICE_THRESHOLD && isOpbase && !isAHCAlgo
        ...
        if (!(algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_RING
            ...
            algType_.algoLevel1 = AlgTypeLevel1::ALG_LEVEL1_NHR;
            HCCL_WARNING(
                "[AllGatherOperator][SelectAlgfor91093] only support ring, NB AHC and NHR in AlgoLevel1 yet, "
                "default is algType=NHR.");
        }
