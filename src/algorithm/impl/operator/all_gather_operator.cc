...
HcclResult AllGatherOperator::SelectAlg(
...
HcclResult AllGatherOperator::SelectAlgfor910B(const OpParam &param, std::string &algName)
{
    ...
    if (isOnlyAiv && !isAivRdmaMode) {
        ...
        CHK_PRT_RET(!IsSupportAIVCopy(param.DataDes.dataType),
            HCCL_ERROR("current data type[%s] not supported, support range: "
        ...
        CHK_PRT_RET(!isCCLBufferGE16M,
            HCCL_ERROR("current isOpbase[%d] or commInputSize[%llu] or commOutputSize[%llu] not supported", isOpbase,
    ...
    HCCL_INFO("[SelectAlgfor910B] AllGather SelectAlgfor910B is algName [%s], current mode is [%u].", algName.c_str(),
        workflowMode_);
    return HCCL_SUCCESS;
}
...
HcclResult AllGatherOperator::SelectAlgfor91093(const OpParam &param, std::string &algName)
{
        ...
        = superPodNum_ == 1 && serverNum_ > 1 && !GetExternalInputInterHccsDisable()
          && (((userRankSize_ <= ONE_EIGHTH_MAX_NUM_BLOCKS && dataSize <= AIV_ALL_GATHER_A3_SMALL_RANKSIZE_ENTRY_SIZE)
                  || (userRankSize_ <= ONE_THIRD_MAX_NUM_BLOCKS
                      && dataSize <= AIV_ALL_GATHER_A3_MID_RANKSIZE_ENTRY_SIZE)
    ...
    bool midCountOptimMultiPod
        = (superPodNum_ > 1) && isOpbase && (param.DataDes.count * unitSize > HCCL_SMALL_COUNT_16_KB)
    ...
    constexpr u64 ALLGATHER_PIPELINE_THRESHOLD = 4 * 1024 * 1024; // 8MB，待校准
    ...
    if (maxCountPerLoop != 0) {
        bufferSliceNum = (param.DataDes.count + maxCountPerLoop - 1) / maxCountPerLoop / userRankSize_;
    }
    ...
    } else if (superPodNum_ > 1 && maxCountPerLoop > 0 && dataSize >= ALLGATHER_PIPELINE_THRESHOLD && bufferSliceNum > 1
    ...
    if (isOnlyAiv && !isAivMode) {
        ...
        CHK_PRT_RET(!isAivSingleNode && !isAivCrossNode,
            HCCL_ERROR("not is aiv single or cross node. serverNum_[%u] isOpbase[%d] superPodNum_[%u]", serverNum_,
                isOpbase, superPodNum_),
            HCCL_E_NOT_SUPPORT);
        CHK_PRT_RET(retryEnable_, HCCL_ERROR("retryEnable_[%d] is true.", retryEnable_), HCCL_E_NOT_SUPPORT);
        CHK_PRT_RET(multiModuleDiffDeviceNumMode_,
