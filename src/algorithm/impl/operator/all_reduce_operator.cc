...
HcclResult AllReduceOperator::SelectAlgforMix(const OpParam& param, std::string& algName)
{
    ...
    if (gcdDeviceNumPerAggregation_ > 1) {
        ...
        HCCL_WARNING(
            "[AllReduceOperator][SelectAlgforMix] only support NHR in AlgoLevel1 yet, "
        ...
        ;
        HCCL_WARNING(
            "[AllReduceOperator][SelectAlgforMix] only support ring in AlgoComm yet, "
...
HcclResult AllReduceOperator::SelectAlgfor91093(const OpParam& param, std::string& algName)
{
    ...
    if (isARSAlgo) {
        if (!(algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_NB
            ...
            algType_.algoLevel1 = AlgTypeLevel1::ALG_LEVEL1_NHR;
            HCCL_WARNING(
                "[AllReduceOperator][SelectAlgfor91093] ARS only support NHR or RING in AlgoLevel1 "
                "yet, default is NHR.");
        }
    }
    // AHC 算法选择逻辑
    ...
    bool midCountOptimMultiPod
        = (superPodNum_ > 1) && isOpbase && (param.DataDes.count * unitSize >= HCCL_SMALL_COUNT_GRAPH_64_KB)
    ...
    if (multiModuleDiffDeviceNumMode_ && multiSuperPodDiffDeviceNumMode_) {
        ...
        if (algType_.algoLevel1 == AlgTypeLevel1::ALG_LEVEL1_HD) {
            algType_.algoLevel1 = AlgTypeLevel1::ALG_LEVEL1_NHR;
            HCCL_WARNING(
                "[AllReduceOperator][SelectAlgfor91093] only support ring, NB and NHR in AlgoLevel1 yet, "
                "default is algType=NHR.");
        }
