...
HcclResult AllReduceOperator::SelectAlgfor91093(const OpParam& param, std::string& algName)
{
    ...
    if (isOnlyAiv && !isAivMode && !isSupportAivDeter) {
        ...
        CHK_PRT_RET(userRankSize_ == 1, HCCL_ERROR("[HcclCommunicator][ExecOp] opType[AllReduce] currently do not select aiv mode, "
            "aiv only not support, please ensure rankNum is greater than one"), HCCL_E_NOT_SUPPORT);
