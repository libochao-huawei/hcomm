    ...
    bool isWaitEstablished, bool isSupportReuse, s32 timeout, uint32_t connectMode)
{
    ...
    if (connectMode) { /* HostNic(CpuRoceChannel) -- Device(TransportIbv)对接模式下，兼容next架构，使用ip判断role */
        role = localIp < rmtRank.ip ? HcclSocketRole::SOCKET_ROLE_SERVER : HcclSocketRole::SOCKET_ROLE_CLIENT;
    } else {
        role = userRank_ < rmtRank.userRank ? HcclSocketRole::SOCKET_ROLE_SERVER : HcclSocketRole::SOCKET_ROLE_CLIENT;
    }
