    ...
    DevType deviceType, u32 rankSize, bool useSuperPodMode, std::vector<HcclAlgoType> algoConfig)
{
    ...
    CHK_PRT_CONT(
        ...
        HCCL_WARNING(
            "[FullmeshPairwiseSatisfyHighPerfAlltoallMeshCondition] AllToAll read only algorithm only "
            "support 910_93 device type, use default algorithm type"));
    CHK_PRT_CONT(
        !(twoLevelIntraUseMesh && !isHCCS),
        HCCL_WARNING(
            "[FullmeshPairwiseSatisfyHighPerfAlltoallMeshCondition] AllToAll read only algorithm depends "
