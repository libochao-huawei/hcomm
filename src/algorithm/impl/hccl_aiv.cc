...
// 同步
static std::vector<AivKernelInfo> g_syncAivKernelInfoList = {
    {"hccl_aiv_sync", HcclDataType::HCCL_DATA_TYPE_RESERVED},
    {"hccl_aiv_sync_rdma", HcclDataType::HCCL_DATA_TYPE_RESERVED, KernelArgsType::ARGS_TYPE_SUPERPOD},
};
static std::string g_syncAivBinaryName = "hccl_aiv_sync_op.o";
...
static std::map<HcclCMDType, std::pair<std::string, std::vector<AivKernelInfo>>> g_aivKernelInfoMap = {
    ...
    {HcclCMDType::HCCL_CMD_INVALID, {g_syncAivBinaryName, g_syncAivKernelInfoList}},
};
...
HcclResult BarrierForMulServer(
    const AivResourceArgs& resourceArgs, s32 step, const AivTopoArgs& topoArgs, void* args, u32 argsSize,
    s32 execTimeOut, bool execTimeOutSet)
{
    ...
    aclrtFuncHandle funcHandle;
    s8* stubFunc = GetStubFunc(
        HcclCMDType::HCCL_CMD_INVALID, HcclDataType::HCCL_DATA_TYPE_RESERVED, KernelArgsType::ARGS_TYPE_SUPERPOD);
    ret = GetKernelFunc(funcHandle, stubFunc);
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[AIV][BarrierForMulServer] errNo[0x%016llx] GetKernelFunc failed, "
            "return[%d]",
    ...
    if (aclRet == ACL_ERROR_RT_INVALID_HANDLE) {
        aclError aclGetRet = aclrtBinaryGetFunction(
            g_binHandleMap[HcclCMDType::HCCL_CMD_INVALID], g_aivNameMap[stubFunc].c_str(), &funcHandle);
        CHK_PRT_RET(
    ...
    TaskParaAiv taskParaAiv(
        HcclCMDType::HCCL_CMD_INVALID, step, 0, resourceArgs.numBlocks, topoArgs.rankSize, -1, flagAddr, rank);
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[AIV][BarrierForMulServer] errNo[0x%016llx] rtKernelLaunch aiv fail, "
            "return[%d]",
            HCCL_ERROR_CODE(HCCL_E_RUNTIME), ret),
        HCCL_E_RUNTIME);
    return HCCL_SUCCESS;
...
HcclResult LaunchSyncKernel(
    aclrtLaunchKernelCfg& cfg, AivKernelArgs& aivKernelArgs, rtStream_t stream, s32 step, const std::string& comm)
{
    uint64_t beginTime = 0;
    SetAivProfilingInfoBeginTime(beginTime);
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[AIV][Barrier] errNo[0x%016llx] GetKernelFunc failed, "
            "return[%d]",
            HCCL_ERROR_CODE(HCCL_E_RUNTIME), ret),
        HCCL_E_RUNTIME);
    ...
    aclError aclRet = aclrtLaunchKernelWithHostArgs(
        funcHandle, aivKernelArgs.rankSize, stream, &cfg, &aivKernelArgs, sizeof(aivKernelArgs), nullptr, 0);
    if (aclRet == ACL_ERROR_RT_INVALID_HANDLE) {
        aclError aclGetRet = aclrtBinaryGetFunction(
            g_binHandleMap[HcclCMDType::HCCL_CMD_INVALID], g_aivNameMap[stubFunc].c_str(), &funcHandle);
        CHK_PRT_RET(
    ...
    CHK_PRT_RET(
        aclRet != ACL_SUCCESS,
        HCCL_ERROR(
            "[RegisterBinaryKernel]errNo[0x%016llx] aclrtLaunchKernelWithHostArgs error[%d].",
            HCCL_ERROR_CODE(HCCL_E_RUNTIME), aclRet),
        HCCL_E_RUNTIME);
    ...
    TaskParaAiv taskParaAiv(
        HcclCMDType::HCCL_CMD_INVALID, step, 0, aivKernelArgs.rankSize, aivKernelArgs.rankSize, -1, flagAddr,
        aivKernelArgs.rank);
    taskParaGeneral.isMainStream = true;
    ...
    void** cclBuffersOut, const AivResourceArgs& resourceArgs, const AivTopoArgs& topoArgs, AivAlgArgs algArgs)
{
    if (algArgs.argsType == KernelArgsType::ARGS_TYPE_SERVER
        ...
        bool execTimeOutSet = algArgs.execTimeOutSet;

        CHK_PRT_RET(
            numBlocks < rankSize,
            HCCL_ERROR("[ClearAivSyncBuf]aivCore[%u] is invalid, at least need [%u].", numBlocks, rankSize),
            HCCL_E_PARA);
...
HcclResult ClearAivSyncBufForMulServer(
    const AivResourceArgs& resourceArgs, const AivTopoArgs& topoArgs, void* args, u32 argsSize, AivAlgArgs algArgs)
{
    CHK_RET(
        BarrierForMulServer(resourceArgs, 1, topoArgs, args, argsSize, algArgs.execTimeOut, algArgs.execTimeOutSet));
    ...
    CHK_RET(hrtMemAsyncCopy(
        flagMem.ptr(), AIV_FLAG_AREA_SIZE, zeroMem.ptr(), AIV_FLAG_AREA_SIZE,
        HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_DEVICE_TO_DEVICE, resourceArgs.stream));

    CHK_RET(BarrierForMulServer(
        resourceArgs, RESET_TAIL_SYNC_TAG, topoArgs, args, argsSize, algArgs.execTimeOut, algArgs.execTimeOutSet));
