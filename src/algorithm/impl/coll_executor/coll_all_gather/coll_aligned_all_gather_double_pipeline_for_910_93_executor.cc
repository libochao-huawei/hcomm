    ...
    OpParam &param, AlgResourceResponse &algRes)
{
    ...
    notifyRingMain_.assign(algResResp_->notifiesMain.begin(), algResResp_->notifiesMain.end() - 2);
    for (size_t i = 0; i < notifyRingMain_.size(); ++i) {
        HCCL_INFO("notifyRingMain_[%zu][%p]", i, notifyRingMain_[i].get());
    }
    notifyRingSub_.assign(algResResp_->notifiesAux.begin(), algResResp_->notifiesAux.end() - 2);
    for (size_t i = 0; i < notifyRingSub_.size(); ++i) {
        HCCL_INFO("notifyRingSub_[%zu][%p]", i, notifyRingSub_[i].get());
    }
    ringSubStreams_.assign(subStreams_.begin(), subStreams_.end() - 2);
    ...
    const OpParam &param, ExecMem &execMem, u64 loopIdx, u64 memIdx, u64 bufferSliceNum)
{
    ...
    if (DMAReduceFlag_) {
        ...
        if (level1CommInfo_.localRankSize > 1 || level2CommInfo_.localRankSize > 1) {
            CHK_RET(HcclD2DMemcpyAsync(dispatcher_, dmaDst, srcMem, const_cast<Stream &>(mainStreamL2_)));
        }
    }
...
HcclResult CollAlignedAllGatherDoublePipelineFor91093Executor::RunLoop(OpParam &param)
{
    ...
    if (bufferSliceNum >= 2) {
        ...
        // bufferSliceNum == 1: L1L0 只在 memIdx=0 时运行，只 Post 了 A
        CHK_RET(LocalNotify::Wait(mainStreamL2_, dispatcher_, notifyL1L0ToL2A_, INVALID_VALUE_STAGE));
    }
    ...
    const OpParam &param, ExecMem &execMem, u64 baseOffset)
{
    ...
    l0OpInfo_.inputAddr = execMem.inputPtr;
    ...
    if (!DMAReduceFlag_ && (topoType_ == TopoType::TOPO_TYPE_NP_DOUBLE_RING)) {
        l0OpInfo_.inputAddr = execMem.inputMem.ptr();
        l0OpInfo_.outputAddr = execMem.outputMem.ptr();
    }
