    ...
    : CollAllGatherRingFor91093Executor(dispatcher, topoMatcher)
{
    ...
    desc_.level1SupportedAlgos = {
        ...
        AlgTypeLevel1::ALG_LEVEL1_RING // 要填这么多吗？
    };
...
// level1 NHR
// HcclResult CollAlignedAllGatherDoublePipelineFor91093Executor::CalcLevel1CommInfo(TransportMemType inputType,
//     TransportMemType outputType,
//     std::vector<LevelNSubCommTransport>& opTransport)
// {
//     CommParaInfo commParaInfo(COMM_LEVEL1, CommType::COMM_TAG_WHOLE_NHR);
    ...
    OpParam &param, AlgResourceResponse &algRes)
{
    ...
    // 从流最后一个给了L2 NHR用，先从subStreams_里pop出来，后续GetSubStreamInfoOnOneRing里就不会拿到这个流了
...
{
    u8* userInputPtr = static_cast<u8 *>(param.inputPtr); // userin
    u8* userOutputPtr = static_cast<u8 *>(param.outputPtr); // userout
    ...
    u64 bufferSliceNum = (param.DataDes.count + maxCountPerLoop - 1) / maxCountPerLoop;// 计算userIn片数（分了多少片，包含尾块）
    u64 loopNum = bufferSliceNum + 1;
    u64 countLeft = param.DataDes.count; // 剩余的count数量，循环过程中递减

    memIdx_ = 0;
    const u64 cclInputSizeHalved = algResResp_->cclInputMem.size() / 2;
    DeviceMem cclInputAMem = algResResp_->cclInputMem.range(0, cclInputSizeHalved);
    DeviceMem cclInputBMem = algResResp_->cclInputMem.range(cclInputSizeHalved, cclInputSizeHalved);
    const u64 cclOutputSizeHalved = algResResp_->cclOutputMem.size() / 2;
    DeviceMem cclOutputAMem = algResResp_->cclOutputMem.range(0, cclOutputSizeHalved);
    DeviceMem cclOutputBMem = algResResp_->cclOutputMem.range(cclOutputSizeHalved, cclOutputSizeHalved);

    // 数据片
    ExecMem lastExecMem; // 上一轮循环的ExecMem，用于同步等待和传递参数
    u64 lastSize = 0; // 上一轮循环处理的数据量，用于同步等待
    for (u64 loopIdx = 0; loopIdx < loopNum; loopIdx++) {
        bool isLastLoop = (loopIdx == bufferSliceNum);
        u64 curCount = countLeft > maxCountPerLoop ? maxCountPerLoop : countLeft; // 当前循环处理的count数量，最后一轮可能不足maxCountPerLoop
        countLeft -= curCount;
        u64 curSize = curCount * unitSize_;

        // 构造ExecMem，传入当前循环处理的片的信息
        ExecMem execMem;
        execMem.count = curCount;
        execMem.inputMem = memIdx_ == 0 ? cclInputAMem : cclInputBMem;
        execMem.outputMem = memIdx_ == 0 ? cclOutputAMem : cclOutputBMem;
        ...
        if (level2CommInfo_.localRankSize > 1 && loopIdx < bufferSliceNum) {
            // 同步：等待loopIdx-2的L1+L0通信完成，释放DMA buffer后L2再复用(L2_N与L1+L0_{N-2}使用同一块ping-pong buffer)
            if (loopIdx >= 2) {
            ...
            u64 inputMemSize = curSize;
            u64 dstMemOffset = topoAttr_.userRank * inputMemSize;
            DeviceMem dstMem2 = execMem.outputMem.range(dstMemOffset, inputMemSize);

            if (DMAReduceFlag_) {
                if (logicalLevel1CommInfo_.localRankSize > 1 || level2CommInfo_.localRankSize > 1) {
                    CHK_RET(HcclD2DMemcpyAsync(dispatcher_, dstMem2, dstMem, const_cast<Stream &>(mainStreamL2_)));
                }
            }

            // L2 Rx -> DMA[0/1]
            CHK_RET(KernelRunInterSuperPod(param, execMem)); // 跨超节点通信

            CHK_RET(LocalNotify::Post(mainStreamL2_, dispatcher_, notifyL2ToL1L0_, INVALID_VALUE_STAGE));
        }

        // L1 + L0 (第一轮等待L2完成之后再执行, 最后一轮单走)
        if (loopIdx >= 1) {
            // 同步：等待loopIdx-1的L2通信完成，确保CCL buffer数据就绪后再执行L1+L0
            // CHK_RET(NotifyRdmaStreamStart());
            CHK_RET(LocalNotify::Wait(mainStreamL1L0_, dispatcher_, notifyL2ToL1L0_, INVALID_VALUE_STAGE));

            if (logicalLevel1CommInfo_.localRankSize > 1) {
                CHK_RET(KernelRunInterServer(param, lastExecMem)); // 超节点内的节点间通信
            }
            
            CHK_RET(KernelRunIntraServer(param, lastExecMem)); // 同一节点内的通信

            // 将CCL buffer out中的结果拷贝到user buffer out
            // NP_SINGLE_RING下，CONCURRENT_DIRECT中的LOCAL_COPY + DMAReduce已覆盖全部user output，无需重复拷贝
            if (topoType_ != TopoType::TOPO_TYPE_NP_SINGLE_RING) {
                for (u64 i = 0; i < topoAttr_.userRankSize; i++) {
                    ...
                    DeviceMem dstMem = DeviceMem::create(
                        static_cast<u8 *>(lastExecMem.outputPtr) + param.DataDes.count * unitSize_ * i, lastSize);
                    DeviceMem srcMem
                        = DeviceMem::create(static_cast<u8 *>(lastExecMem.outputMem.ptr()) + lastSize * i, lastSize);
                    CHK_RET(HcclD2DMemcpyAsync(dispatcher_, dstMem, srcMem, mainStreamL1L0_));
                }
            }
            CHK_RET(LocalNotify::Post(mainStreamL1L0_, dispatcher_, notifyL1L0ToL2_, INVALID_VALUE_STAGE));
        }
        // CHK_RET(LaunchTaskExtend(dispatcher_, mainStreamL2_, subStreams_));

        userInputPtr += curSize;
        userOutputPtr += curSize;
        memIdx_ = 1 - memIdx_; // 双缓冲交替使用
        lastExecMem = execMem; // 保存当前循环的ExecMem用于下一轮同步等待
        lastSize = curSize; // 保存当前循环处理的数据量用于下一轮同步等待
    }
...
// 跨超节点
HcclResult CollAlignedAllGatherDoublePipelineFor91093Executor::KernelRunInterSuperPod(const OpParam &param, ExecMem &execMem)
{
    ...
    printf("CollAlignedAllGatherDoublePipelineFor91093Executor::KernelRunInterSuperPod, algType_.algoLevel2[%u]\n", algType_.algoLevel2);
    ...
    std::vector<Slice> level2DataSegsSlice = PrepareSlicesL2(
        param, level2CommInfo_, logicalLevel1CommInfo_, logicalLevel0CommInfo_, unitSize_, curDataSegsSliceSize);
    CHK_RET(level2AGExecutor->Prepare(execMem.outputMem, execMem.outputMem, execMem.inputMem, execMem.count, param.DataDes.dataType,
        mainStreamL2_, HCCL_REDUCE_RESERVED, INVALID_VALUE_RANKID, level2DataSegsSlice, 0));
    ...
    const OpParam &param, ExecMem &execMem)
{
    ...
    CHK_RET(level1AGExecutor->Prepare(execMem.outputMem, execMem.outputMem, execMem.inputMem, execMem.count, param.DataDes.dataType,
        mainStreamL1L0_, HCCL_REDUCE_RESERVED, INVALID_VALUE_RANKID, level1DataSegsSlice, 0));
    ...
    const OpParam &param, ExecMem &execMem)
{
    ...
    CHK_RET(PrepareSlicesL0(multRingsSlice, param, level2CommInfo_, logicalLevel1CommInfo_, logicalLevel0CommInfo_,
        unitSize_, curDataSegsSliceSize));

    std::vector<std::vector<Slice>> multRingsUserMemSlice;
    CHK_RET(PrepareUserMemSlices(multRingsUserMemSlice, multRingsSlice, param, level2CommInfo_, logicalLevel1CommInfo_,
        logicalLevel0CommInfo_, unitSize_, curDataSegsSliceSize));
    ...
    if (DMAReduceFlag_ && (logicalLevel1CommInfo_.localRankSize > 1 || level2CommInfo_.localRankSize > 1)) {
        // allgather输入放在CCL buffer上，通过设置nullptr指示要从CCL buffer获取输入
        opInfo.inputAddr = nullptr;
    }

    printf("DMAReduceFlag_[%u], level1RankSize_[%u], level2RankSize_[%u]\n", DMAReduceFlag_, logicalLevel1CommInfo_.localRankSize, level2CommInfo_.localRankSize);

    if (topoType_ == TopoType::TOPO_TYPE_NP_DOUBLE_RING) {
        printf("CollAlignedAllGatherDoublePipelineFor91093Executor::KernelRunIntraServer, run DoubleRingAllGather, topoType_[%u], agv[%u]\n", topoType_, isAllGatherV_);
        ...
        printf("CollAlignedAllGatherDoublePipelineFor91093Executor::KernelRunIntraServer, run MultiRingAllGather, topoType_[%u], agv[%u]\n", topoType_, isAllGatherV_);
        CHK_RET(MultiRingAllGather(param.tag, execMem.inputMem, execMem.outputMem, execMem.count, param.DataDes.dataType,
        multRingsSlice, mainStreamL1L0_, PROF_STAGE_2, baseOffset, &opInfo, multRingsUserMemSlice, logicalLevel0plane_));
    } else {
        // ?
    }
    return HCCL_SUCCESS;
...
HcclResult CollAlignedAllGatherDoublePipelineFor91093Executor::WriteBackToUserOutput(
    DeviceMem &dmaBuffer, u32 bufferIndex, std::vector<Stream> &subStreams,
...
REGISTER_EXEC("CollAlignedAllGatherDoublePipelineFor91093Executor",
