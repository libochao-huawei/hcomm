...
HcclResult CollAlignedAllGatherDoublePipelineFor91093Executor::RunLoop(OpParam &param)
{
    ...
    for (u64 loopIdx = 0; loopIdx < loopNum; loopIdx++) {
        ...
        if (level2CommInfo_.localRankSize > 1 && loopIdx < bufferSliceNum) {
            ...
            // L2 Rx -> DMA[0/1]
            u64 baseOffset = memIdx_ == 0 ? 0 : cclInputSizeHalved;
            CHK_RET(KernelRunInterSuperPod(param, execMem, baseOffset)); // 跨超节点通信
        ...
        if (loopIdx >= 1) {
            ...
            u64 baseOffset = memIdx_ == 0 ? cclInputSizeHalved : 0;
            if (logicalLevel1CommInfo_.localRankSize > 1) {
                CHK_RET(KernelRunInterServer(param, lastExecMem, baseOffset)); // 超节点内的节点间通信
            }
            
            CHK_RET(KernelRunIntraServer(param, lastExecMem, baseOffset)); // 同一节点内的通信

            // loopIdx=bufferSliceNum-1 的Post没有L2消费者(L2在loopIdx=bufferSliceNum不运行)
            // 只有有消费者时才Post: L2内部消费(loopIdx<bufferSliceNum-1)或循环后Wait消费(loopIdx==bufferSliceNum)
