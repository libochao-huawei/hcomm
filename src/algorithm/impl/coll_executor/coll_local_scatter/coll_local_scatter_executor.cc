...
void CollLocalScatterExecutor::ParseParam(const OpParam& param)
{
...
{
    streamNum = MAX_LOCAL_SCATTER_STREAMS;
    if (streamNum == 0) {
        streamNum = 1;
    }
...
HcclResult CollLocalScatterExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    ...
    HCCL_INFO("[LocalScatter] algResResp_->slaveStreams.size()=[%d]",algResResp_->slaveStreams.size());
    for (u32 i = 0; i < algResResp_->slaveStreams.size(); i++){
        HCCL_INFO("[LocalScatter] stream %u .streamId=[%d]", i, algResResp_->slaveStreams[i].id());
    }
    for (u32 s = 0; s < splitNum; ++s) {
        ...
        for (u32 i = 0; i < numBufs; ++i) {
            ...
            HCCL_INFO("[LocalScatter]Inside Stream %u. StreamIdx=[%d]", streamIdx, targetStream.id());
            streamIdx = (streamIdx + 1) % algResResp_->slaveStreams.size();
