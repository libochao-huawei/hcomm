...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::GetLocalSendRecvInfoforAlltoallVC(const OpParam &param)
{
    ...
    for (u32 j = 0; j < topoAttr_.userRankSize; j++) {
        ...
        for (u32 i = 0; i < usrRank; i++) {
            ...
            u64 sendLength = curSendCounts * SIZE_TABLE[param.All2AllDataDes.recvType];
            sendRecvInfo_.remoteSendOffset[j] += curSendLength;
        }
        ...
        for (u32 i = 0; i < j; i++) {
            ...
            u64 recvLength = curRecvCounts * SIZE_TABLE[param.All2AllDataDes.sendType];
            sendRecvInfo_.localRecvOffset[j] += curRecvLength;
        }
        ...
        HCCL_DEBUG("GetLocalSendRecvInfoforAlltoallVC rank[%u], sendCounts[%llu], sendDispls[%llu] "\
            "recvCounts[%llu], recvDispls[%llu]", topoAttr_.userRank, sendRecvInfo_.sendCounts[j],
            sendRecvInfo_.sendDispls[j], sendRecvInfo_.recvCounts[j],
            sendRecvInfo_.recvDispls[j]);
    }
    return HCCL_SUCCESS;
}
