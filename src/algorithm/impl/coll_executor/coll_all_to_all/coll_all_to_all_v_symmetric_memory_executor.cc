...
#include "coll_all_to_all_v_symmetric_memory_executor.h"
...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::GetAdjInfo(AlgResourceResponse& algRes, AdjInfo& adjInfo)
{
...
HcclOpMetaInfo CollRunAlltoAllFullMeshSymmetricMemory::GetOpMeta(HcclCMDType opType, const u64 size)
{
    (void)opType;
...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::GetLocalSDMAGroupInfo(const u32 userRank,
...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::GetLocalSendRecvInfoforAlltoallV(const OpParam &param)
{
...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::GetAlltoAllvTmpRankSendRecvInfo(const OpParam &param)
{
...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::Getlevel1CommRank(SubCommInfo& level1CommInfo)
{
...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::SelectTempAlg(std::unique_ptr<AlgTemplateBase> &level1TempAlg, u32 level1RankSize)
{
    (void) level1RankSize;
    HCCL_INFO("[GetAdjInfo-nslbdp] SelectTempAlg.");
...
HcclResult CollRunAlltoAllFullMeshSymmetricMemory::GetDevNumInlocalPod(u32& devNumInlocalPod)
{
...
REGISTER_EXEC("RunAlltoAllFullMeshSymmetricMemory", AlltoAllVSymmetricMemory, CollRunAlltoAllFullMeshSymmetricMemory);
} // namespace hccl