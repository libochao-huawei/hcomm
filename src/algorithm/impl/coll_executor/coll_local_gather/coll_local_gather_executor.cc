...
void CollLocalGatherExecutor::ParseParam(const OpParam& param)
{
...
{
    streamNum = MAX_LOCAL_GATHER_STREAMS;
    if (streamNum == 0) {
        streamNum = 1;
    }
...
HcclResult CollLocalGatherExecutor::CalcNotifyNum(u32 streamNum, u32 &notifyNum)
{
    ...
    HCCL_INFO("[CollLocalGatherExecutor][CalcNotifyNum] tag[%s] streamNum[%u] notifyNum[%u].",
