...
HcclResult CollAllGatherVMeshAivFor91093Executor::CalBlockDim(u32& blockDim, u32 rankSize, u64 dataSize, HcclCMDType cmdType)
{
    // Step1. Calculate the best block dimension
    const u32 bestBlockDim = (rankSize + 1 < MAX_BLOCK_DIM ? rankSize + 1 : MAX_BLOCK_DIM);
    const u32 minBlockDim = (rankSize + MAX_TARGET_NUM) / MAX_TARGET_NUM;
    ...
    CHK_PRT_RET(originUserLimit < BLOCK_DIM_FACTOR_TWO,
        HCCL_ERROR("[CollAllGatherVMeshAivFor91093Executor][CalBlockDim]aivCore[%u] is invalid, at lest need 2.",
    ...
    if (blockDim_ < minBlockDim){
        HCCL_ERROR("[CollAllGatherVMeshAivFor91093Executor][CalBlockDim] limit[%u], at least[%u]", originUserLimit, minBlockDim);
        return HCCL_E_PARA;
    }
    blockDim = blockDim_;
    HCCL_INFO("[CollAllGatherVMeshAivFor91093Executor][CalBlockDim] blockDim is set to [%u], limit[%u], best[%u]",
...
HcclResult CollAllGatherVMeshAivFor91093Executor::Orchestrate(OpParam& param, AlgResourceResponse& algRes)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAllGatherVMeshAivFor91093Executor][Orchestrate]errNo[0x%016llx] tag[%s] excutor kernel run failed",
