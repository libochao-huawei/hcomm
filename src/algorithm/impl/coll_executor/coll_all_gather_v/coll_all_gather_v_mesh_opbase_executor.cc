...
HcclResult CollAllGatherVMeshOpbaseExecutor::RunLevel0(const OpParam &param, ExecMem &execMem,
                                                        SubCommInfo &level0CommInfo, SubCommInfo &level1CommInfo)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
                HCCL_ERROR("[CollAllGatherVMeshOpbaseExecutor][KernelRun]all gatherV mesh memcpy Failed, Offset[%llu], Size[%llu].",
    ...
    HCCL_INFO("[CollAllGatherVMeshOpbaseExecutor][RunLevel0] level 0 for A2 run success");
    return HCCL_SUCCESS;
}

HcclResult CollAllGatherVMeshOpbaseExecutor::RunLevel1(const OpParam &param, ExecMem &execMem,
                                                        SubCommInfo &level0CommInfo, SubCommInfo &level1CommInfo)
{
    ...
    HCCL_INFO("[CollAllGatherVMeshOpbaseExecutor][RunLevel1] level 1 for A2 run success");

    return HCCL_SUCCESS;
}

HcclResult CollAllGatherVMeshOpbaseExecutor::RunSingleMesh(const OpParam &param, ExecMem &execMem)
{
    ...
    HCCL_INFO("[CollAllGatherVMeshOpbaseExecutor][RunSingleMesh] single mesh for A2 run success");
    return HCCL_SUCCESS;
}
...
HcclResult CollAllGatherVMeshOpbaseExecutor::CalcCurCountsAndCurDisplsMultiModule(const u64 maxTotalCount,
...
HcclResult CollAllGatherVMeshOpbaseExecutor::CalcCurCountsAndCurDisplsSingleModule(const u64 maxTotalCount,
                                                                        std::vector<u64> &countsLeft, std::vector<u64> &displs, std::vector<u64> &curCounts, std::vector<u64> &curDispls,
                                                                        bool &finished)
{
    finished = true;
