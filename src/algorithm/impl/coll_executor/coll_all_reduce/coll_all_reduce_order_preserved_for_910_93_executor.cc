    ...
    SubCommInfo &level1CommInfo)
{
    ...
    CHK_RET(level1TempAlg->Prepare(execMem.inputPtr, execMem.inputMem, outputMem, param.stream,
        ...
        param.reduceType, all2allOffset_, param.DataDes.dataType, true, true));
    ...
    SubCommInfo &level1CommInfo)
{
    ...
    CHK_SMART_PTR_NULL(level1TempAlg);
    CHK_RET(level1TempAlg->Prepare(algResResp_->slaveStreams, algResResp_->notifiesMain, algResResp_->notifiesAux,
    ...
    CHK_RET(level1TempAlg->RegisterProfiler((level0RankSize << PROF_RANKSIZE_OFFSET_OF_PLANEID) + commIndex,
