...
HcclResult CollAllReduceRingFor91093Executor::PrepareLevel1CommInfo(u32 &segmentIdx, u32 &commIndex, u64 &hdSize,
                                                  ...
                                                  const std::string &tag, const std::vector<u32>& nicList)
{
    ...
    if (multRingsSliceZero.size() > 1) {
        ...
        if (iterNic != nicList.end()) {                          // 如果当前rank为通信网口
            ...
            HCCL_INFO("[%s] ringRanks[%u], nicIdx[%u], nicList.size()[%u]",__func__, ringRanks, nicIdx, nicList.size());
    ...
    HCCL_INFO("[%s] segmentIdx[%u], commIndex[%u]",__func__, segmentIdx, commIndex);
    return HCCL_SUCCESS;
}
...
HcclResult CollAllReduceRingFor91093Executor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    ...
    u32 segmentIdx;
    u32 commIndex;
    CHK_RET(PrepareLevel1CommInfo(segmentIdx, commIndex, hdSize, logicalLevel0CommInfo_, multRingsSliceZero, param.tag, mockNicList));
