...
HcclResult CollReduceScatterOrderPreservedFor91093Executor::RunReduceScatterLevel1HD(const OpParam &param, ExecMem &execMem,
    SubCommInfo &level1CommInfo)
{
    ...
    CHK_RET(level1TempAlg->Prepare(execMem.inputMem, execMem.scratchMem, execMem.outputMem, execMem.count,
    ...
    SubCommInfo &level1CommInfo)
{
    ...
    if (isUseHDAlg_) {
        CHK_RET(RunReduceScatterLevel1HD(param, execMem, level1CommInfo));
    } else {
        ...
        CHK_RET(level1TempAlg->Prepare(execMem.inputPtr, execMem.inputMem, execMem.scratchMem, param.stream, 
            ...
            groupSlicesInfoLevel0, param.reduceType, all2allOffset_, param.DataDes.dataType, false, true));
        CHK_RET(level1TempAlg->RegisterProfiler(
            (level1CommInfo.localRankSize << PROF_RANKSIZE_OFFSET_OF_PLANEID) + level1CommInfo.localRank,
            PROF_STAGE_2, HCCL_EXEC_STEP_NOT_SET, param.stream));
        CHK_RET(RunTemplate(level1TempAlg, level1CommInfo));
