...
void CollReduceScatterDeterExecutor::ParseParam(const OpParam& param)
{
    ...
    HCCL_INFO("[CollReduceScatterDeterExecutor][ParseParam] tag[%s] totalSize[%llu], topoAttr_.userRankSize[%u], count[%llu], dataType[%d]",
        tag_.c_str(), totalSize_, topoAttr_.userRankSize, param.DataDes.count, param.DataDes.dataType);
...
HcclResult CollReduceScatterDeterExecutor::CalcScratchMemSize(u64& scratchMemSize)
{
    if (scratchMemFlag_) { // 确定性计算只有图模式需要scratch memory
        HCCL_INFO("[CollReduceScatterDeterExecutor][CalcScratchMemSize] tag[%s] totalSize[%llu]",
            tag_.c_str(), totalSize_);
        if (symmetricMemory_) {
            scratchMemSize = HCCL_SMALL_COUNT_1_MB * topoAttr_.userRankSize;
        } else {
...
HcclResult CollReduceScatterDeterExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    ...
    bool isLocalReduce91073 = (
        (
        ((topoAttr_.userRankSize & (topoAttr_.userRankSize - 1)) != 0) ||
        aicpuUnfoldMode_ || 
        ((workflowMode_ == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OPS_KERNEL_INFO_LIB) || symmetricMemory_)
        ) 
        && (topoAttr_.deviceType == DevType::DEV_TYPE_910_93)) && (topoAttr_.serverNum == 1);
    ...
    HCCL_INFO("[CollReduceScatterDeterExecutor] level0TempAlg, scratchMemSize[%llu], scratchMemPtr[%p]", execMem.scratchMem.size(), execMem.scratchMem.ptr());
