...
CommFactory::CommFactory(
    ...
    bool isHeterogComm, const void* transportResourceInfoAddr, size_t transportResourceInfoSize,
    ...
    std::vector<std::unique_ptr<CommBase>>& commVec)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ++ringIndex) {
        ...
        commVec[ringIndex].reset(new (std::nothrow) CommRing(
            ...
            notifyPool_, netDevCtxMap_, exchangerNetwork, commPlaneVec[ringIndex], inputMem, outputMem, isUsedRdma,
            transportResourceInfoAddr_, transportResourceInfoSize_, tag, nicDeployInner_, false, false, isHaveCpuRank_,
            useSuperPodMode_));
    ...
    std::vector<std::unique_ptr<CommBase>>& commVec)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ++ringIndex) {
        ...
        commVec[ringIndex].reset(new (std::nothrow) CommHalvingDoubling(
            identifier_, userRank_, userRankSize_, rank, commPlaneVec[ringIndex].size(), topoFlag_, dispatcher_,
            notifyPool_, netDevCtxMap_, exchangerNetwork, commPlaneVec[ringIndex], inputMem, outputMem, isUsedRdma,
            transportResourceInfoAddr_, transportResourceInfoSize_, tag, nicDeployInner_, subUserRankRoot,
            HalvingDoublingType::RECURSIVE_HALVING_DOUBLING, isHaveCpuRank_, useSuperPodMode_));
    ...
    std::vector<std::unique_ptr<CommBase>>& commVec)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ++ringIndex) {
        ...
        commVec[ringIndex].reset(new (std::nothrow) CommStar(
            ...
            notifyPool_, netDevCtxMap_, exchangerNetwork, commPlaneVec[ringIndex], inputMem, outputMem, isUsedRdma,
            transportResourceInfoAddr_, transportResourceInfoSize_, tag, nicDeployInner_, commParaInfo.root,
            isHaveCpuRank_));
    ...
    std::vector<std::unique_ptr<CommBase>>& commVec, DeviceMem& expMem)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ++ringIndex) {
        ...
        commVec[ringIndex].reset(new (std::nothrow) CommMesh(
            ...
            transportResourceInfoAddr_, transportResourceInfoSize_, tag, isAlltoAllCommMesh, nicDeployInner_, false,
            commParaInfo.isAicpuModeEn, isHaveCpuRank_, useSuperPodMode_, expMem));
    ...
    std::vector<std::unique_ptr<CommBase>>& commVec)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ++ringIndex) {
        ...
        commVec[ringIndex].reset(new (std::nothrow) CommP2P(
            identifier_, userRank_, userRankSize_, rank, commPlaneVec[ringIndex].size(), topoFlag_, dispatcher_,
            notifyPool_, netDevCtxMap_, exchangerNetwork, commPlaneVec[ringIndex], inputMem, outputMem, isUsedRdma,
            transportResourceInfoAddr_, transportResourceInfoSize_, tag, commParaInfo.peerUserRank, nicDeployInner_,
    ...
    const std::string& tag, const DeviceMem& inputMem, const DeviceMem& outputMem, const u32 dstUserRank, u32& status)
{
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ringIndex++) {
        ...
        commP2PArray[ringIndex].reset(new (std::nothrow) CommP2P(
            ...
            CommPlaneVector_[COMM_COMBINE][ringIndex], inputMem, outputMem, isUsedRdmaLevel0_,
            transportResourceInfoAddr_, transportResourceInfoSize_, tag, dstUserRank, nicDeployInner_));
