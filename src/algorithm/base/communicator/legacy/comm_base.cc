    ...
    std::map<u32, HcclRankLinkInfo> &dstClientMap)
{
    ...
    for (auto rankIter = rankRole.begin(); rankIter != rankRole.end(); rankIter++) {
        ...
        if (!vnicRanksPort_.empty()) {
            linkInfo.port = GetNicPort(paraVector_[dstRank].devicePhyId, vnicRanksPort_, linkInfo.userRank, isUseRankPort_);
        } else {
    ...
    std::map<u32, HcclRankLinkInfo> &dstClientMap)
{
    for (u32 dstRank : dstIntraVec) {
        ...
        bool isP2PLink = (paraVector_[rank_].serverId == rankInfo.serverId) && !isUsedRdmaLevel0_;
        if (isP2PLink && !vnicRanksPort_.empty()) {
