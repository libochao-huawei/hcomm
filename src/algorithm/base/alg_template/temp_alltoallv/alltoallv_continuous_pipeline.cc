...
HcclResult AlltoallvContinuousPipeline::NotifySdmaSubStreamStart()
{
    for (u32 streamIndex = 0; streamIndex < sdmaSubStreams_.size(); ++streamIndex) {
        ...
        CHK_RET(
            LocalNotify::Wait(
                sdmaSubStreams_[streamIndex], dispatcher_, streamNotifySdmaSubToMain_[streamIndex],
                INVALID_VALUE_STAGE));
    }
    return HCCL_SUCCESS;
}
...
HcclResult AlltoallvContinuousPipeline::WaitSdmaSubStreamFinish()
{
    for (u32 streamIndex = 0; streamIndex < sdmaSubStreams_.size(); ++streamIndex) {
        CHK_RET(
            LocalNotify::Post(
                sdmaSubStreams_[streamIndex], dispatcher_, streamNotifyMainToSdmaSub_[streamIndex],
                INVALID_VALUE_STAGE));
        CHK_RET(
...
HcclResult AlltoallvContinuousPipeline::NotifyRdmaSubStreamStart()
{
    for (u32 streamIndex = 0; streamIndex < rdmaSubStreams_.size(); ++streamIndex) {
        ...
        CHK_RET(
            LocalNotify::Wait(
                rdmaSubStreams_[streamIndex], dispatcher_, streamNotifyRdmaSubToMain_[streamIndex],
                INVALID_VALUE_STAGE));
    }
    return HCCL_SUCCESS;
}
...
HcclResult AlltoallvContinuousPipeline::WaitRdmaSubStreamFinish()
{
    for (u32 streamIndex = 0; streamIndex < rdmaSubStreams_.size(); ++streamIndex) {
        CHK_RET(
            LocalNotify::Post(
                rdmaSubStreams_[streamIndex], dispatcher_, streamNotifyMainToRdmaSub_[streamIndex],
                INVALID_VALUE_STAGE));
        CHK_RET(
    ...
    const u32 sendRank, const u32 recvRank, Stream& stream, const u32 loopIdx)
{
    ...
    for (u32 rankOffset = 0; rankOffset < intraRankSize_; ++rankOffset) {
        ...
        if (sendSize > 0) {
            sendMems.emplace_back(
                TxMemoryInfo{
                    UserMemType::OUTPUT_MEM, sendDstOffset, static_cast<s8*>(inBuffer_.ptr()) + sendSrcOffset,
                    sendSize});
        }
        HCCL_DEBUG(
        ...
        if (recvCount > 0) {
            recvMems.emplace_back(
                RxMemoryInfo{
                    UserMemType::INPUT_MEM, recvSrcOffset, static_cast<s8*>(outBuffer_.ptr()) + recvDstOffset,
                    recvSize});
