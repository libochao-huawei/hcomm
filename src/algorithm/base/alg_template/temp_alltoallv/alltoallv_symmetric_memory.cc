...
HcclResult AlltoAllFullMeshSymmetricMemory::Prepare(PrepareData &param)
{
    ...
    sdmaDataBlockSize_= (maxSendLen / std::max(1u, sdmaConcurrentNum_));
    // 向下对齐到16k Byte
    ...
    CHK_PRT_RET(sdmaDataBlockSize_== 0, HCCL_ERROR("[AlltoAllFullMeshSymmetricMemory][Prepare]sdmaDataBlockSize_is zero."),
        HCCL_E_INTERNAL);
    HCCL_DEBUG("[AlltoAllFullMeshSymmetricMemory][Prepare] userRank [%u]," \
        "sdmaDataBlockSize_[%llu]", userRank_, sdmaDataBlockSize_);
...
u64 AlltoAllFullMeshSymmetricMemory::CalcMaxSendLen()
{
    u64 maxSendLen = 0;
    const ZCopySendRecvInfo& sendRecvInfo = *sendRecvInfoPtr_;

    for (u32 dstRank = 0; dstRank < sendRecvInfo.localRecvLength.size(); dstRank++) {
        maxSendLen = std::max(maxSendLen, sendRecvInfo.localRecvLength[dstRank]);
    }

    HCCL_DEBUG("[AlltoAllFullMeshSymmetricMemory][CalcMaxSendLen] maxSendLen[%llu]", maxSendLen);
    return maxSendLen;
}
...
{
    const SendRecvInfo& sendRecvInfo = *sendRecvInfoPtr_;
    ...
    readInfo.push_back({recvLen, remoteUserInOffset, userOutOffset});
}
...
HcclResult AlltoAllFullMeshSymmetricMemory::RunGroupFullMeshAlltoall(u32 roundIdx)
{
    ...
    CHK_RET(UpdateSendRecvInfo(roundIdx, subStreamReadInfo_, partialCommRankSet_));
    CHK_RET(NotifySubStreamStart());
