...
namespace hccl {
AlltoAllVFullMeshSymmetricMemory::AlltoAllVFullMeshSymmetricMemory(const HcclDispatcher dispatcher)
    : AlgTemplateBase(dispatcher)
{
}

AlltoAllVFullMeshSymmetricMemory::~AlltoAllVFullMeshSymmetricMemory() {}

HcclResult AlltoAllVFullMeshSymmetricMemory::GenerateSubStreamInfo(const std::vector<Stream> &subStreams,
    ...
    const std::vector<std::shared_ptr<LocalNotify>> &meshSignalSubToMain)
{
    ...
    if (subStreams.size() < totalSubstreamSize || meshSignalMainToSub.size() < totalSubstreamSize ||
        ...
        HCCL_ERROR("[AlltoAllVFullMeshSymmetricMemory][GenerateSubStreamInfo]subStreamsSize[%zu], meshSignalMainToSubSize[%zu]"\
    ...
    CHK_PRT_RET(links_.size() < userRankSize_, HCCL_ERROR("[AlltoAllVFullMeshSymmetricMemory][GenerateSubStreamInfo]"\
...
HcclResult AlltoAllVFullMeshSymmetricMemory::Prepare(PrepareData &param)
{
    ...
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory]devNumInlocalPod_[%u], userRankSize_[%u] podStartRank_[%u]" \
        "podEndRank_[%u], sdmaConcurrentNum_[%u]",
        devNumInlocalPod_, userRankSize_, podStartRank_, podEndRank_, sdmaConcurrentNum_);

    CHK_PRT_RET(userRankSize_ == 0, HCCL_ERROR("[AlltoAllVFullMeshSymmetricMemory][Prepare]userRankSize_ is zero."),
        HCCL_E_PARA);
    ...
    CHK_PRT_RET(sdmaDataBlockSize_== 0, HCCL_ERROR("[AlltoAllVFullMeshSymmetricMemory][Prepare]sdmaDataBlockSize_is zero."),
        HCCL_E_INTERNAL);
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][Prepare] userRank [%u] total cclsize[%llu]," \
        "sdmaDataBlockSize_[%llu], BigCountFlag[%d]", userRank_, cclInMem_.size(), sdmaDataBlockSize_, isBigCount_);

    return HCCL_SUCCESS;
}

std::string AlltoAllVFullMeshSymmetricMemory::GetStreamIndexString()
{
    std::string res = "";
...
u64 AlltoAllVFullMeshSymmetricMemory::CalcMaxSendLen()
{
    u64 maxSendLen = 0;
    ...
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][CalcMaxSendLen] maxSendLen[%llu]", maxSendLen);
    return maxSendLen;
}

void AlltoAllVFullMeshSymmetricMemory::UpdateCurrRankRecvInfo(u32 step, u32 roundIdx, u32 side, u32 destRank,
    std::vector<ReadDataBlock>& readInfo, std::unordered_map<u32, ReadDataBlock>& subStreamZcopyReadInfo, u32 maxRecvStep)
{
    ...
    while(recvStepIdx < maxRecvStep && remainRecvLen > 0) {
        ...
        HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][UpdateCurrRankRecvInfo] usrRank[%u] recv from destRank [%u]"
...
void AlltoAllVFullMeshSymmetricMemory::UpdateCurrRankSendInfo(u32 step, u32 roundIdx, u32 side, u32 destRank,
    std::vector<SendDataBlock>& sendInfo, std::unordered_map<u32, SendDataBlock>& subStreamZcopySendInfo, u32 maxSendStep)
{
    ...
    while (sendStepIdx < maxSendStep && remainSendLen > 0) {
        ...
        HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][UpdateCurrRankSendInfo] usrRank[%u] send to destRank [%u]"
...
void AlltoAllVFullMeshSymmetricMemory::UpdateSendRecvInfo(u32 step, u32 roundIdx,
...
void AlltoAllVFullMeshSymmetricMemory::UpdateOpBaseSubStreamInfo(u32 step, u32 roundIdx)
{
...
// HcclResult AlltoAllVFullMeshSymmetricMemory::PrepareIntraData(u32 step,
//     std::unordered_map<u32,std::vector<SendDataBlock>> &subStreamSendInfo)
// {
//     u32 sendDataIndex = 0;
...
//             HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][PrepareIntraData]userRank [%u] copy from userInOffset[%llu]"
...
void AlltoAllVFullMeshSymmetricMemory::UpdateRemoteRankSet(u32 roundIdx, u32 groupRankSize)
{
...
void AlltoAllVFullMeshSymmetricMemory::UpdatePartialCommunicationRankSetPairWise(u32 roundIdx, u32 groupRankSize)
{
    ...
    for (u32 i = roundIdx * sdmaConcurrentNum_; i < (roundIdx * sdmaConcurrentNum_ + groupRankSize); i++) {
        ...
        HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][UpdatePartialCommunicationRankSetPairWise] userRank [%u] i[%u]" \
            "readRemoteRank[%u] writeRemoteRank[%u]", userRank_, i, readRemoteRank, sendRemoteRank);
    }
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][UpdatePartialCommunicationRankSetPairWise] partialCommRankSet_ size[%zu]",
        partialCommRankSet_[0].size());
}

void AlltoAllVFullMeshSymmetricMemory::UpdatePartialCommunicationRankSet(u32 roundIdx, u32 groupRankSize,
    std::vector<std::vector<std::pair<u32,u32>>> &partialCommRankSet)
{
    ...
    for (u32 i = roundIdx * pairNumPerRound + 1;
        ...
        HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][UpdatePartialCommunicationRankSet] round[%u] userRank [%u] i[%u]" \
            "read/write leftRemoteRank[%u] rightRemoteRank[%u]", roundIdx, userRank_, i, leftRemoteRank, rightRemoteRank);
    }
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][UpdatePartialCommunicationRankSet] round[%u] partialCommRankSet_ total size[%zu]",
        roundIdx, partialCommRankSet[0].size() + partialCommRankSet[1].size() + partialCommRankSet[2].size());
}

// 主流只需要通知当前子步骤需要收发数据的 SDMA 流，减少同步开销
HcclResult AlltoAllVFullMeshSymmetricMemory::NotifySubStreamStart()
{
    ...
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][NotifySubStreamStart] userRank [%u] main stream notify sdma stream [%s]",
        userRank_, GetStreamIndexString().c_str());
    return HCCL_SUCCESS;
}

HcclResult AlltoAllVFullMeshSymmetricMemory::WaitSubStreamFinish()
{
    for (u32 streamIndex = 0; streamIndex < subStreamReadInfo_.size(); streamIndex++) {
    ...
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][WaitSubStreamFinish] userRank [%u] main stream wait sdma stream [%s]",
        userRank_, GetStreamIndexString().c_str());
    return HCCL_SUCCESS;
}

HcclResult AlltoAllVFullMeshSymmetricMemory::NotifyLocalSubStreamStart()
{
...
HcclResult AlltoAllVFullMeshSymmetricMemory::WaitLocalSubStreamFinish()
{
    for (u32 streamIndex = 0; streamIndex < subStreamSendInfo_.size(); streamIndex++) {
...
u32 AlltoAllVFullMeshSymmetricMemory::CalcNumSubStep()
{
    const SendRecvInfo& localSendRecvInfo = *localSendRecvInfoPtr_;
    ...
    for (u32 destRank = podStartRank_; destRank < podStartRank_ + devNumInlocalPod_; destRank++) {
        ...
        HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][CalcNumSubStep] userRank [%u] currRankSendSubStep[%u]" \
    ...
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][CalcNumSubStep] userRank [%u] max communication step[%u]",
        userRank_, numSubStep);
    return numSubStep;
}

HcclResult AlltoAllVFullMeshSymmetricMemory::NotifyRemoteRankStart(u32 step)
{
    u32 streamIndex = 0;
    ...
    HCCL_INFO("[AlltoAllVFullMeshSymmetricMemory][NotifyRemoteRankStart] done");
    return HCCL_SUCCESS;
}

bool AlltoAllVFullMeshSymmetricMemory::IsPostSyncEnable(u32 step, u32 roundIdx)
{
...
HcclResult AlltoAllVFullMeshSymmetricMemory::SdmaMainStreamWait(u32 step, u32 roundIdx)
{
    // SDMA wait
    u32 streamIndex = 0;
    for (auto& sendRecvSide : partialCommRankSet_) {
        for (auto& sendRecvPair : sendRecvSide) {
            ...
            const std::vector<ReadDataBlock>& readInfo = subStreamReadInfo_[recvRank];
            if (step < readInfo.size()) {
                HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][SdmaMainStreamWait] userRank [%u], recvRank[%u], "
    ...
    HCCL_INFO("[AlltoAllVFullMeshSymmetricMemory][SdmaMainStreamWait] done");
    return HCCL_SUCCESS;
}

HcclResult AlltoAllVFullMeshSymmetricMemory::SdmaMainStreamPost(u32 step, u32 roundIdx)
{
    // SDMA post
    u32 streamIndex = 0;
    for (auto& sendRecvSide : partialCommRankSet_) {
        for (auto& sendRecvPair : sendRecvSide) {
            ...
            const std::vector<ReadDataBlock>& readInfo = subStreamReadInfo_[recvRank];
            if (step < readInfo.size()) {
                HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][SdmaMainStreamPost] userRank [%u], recvRank[%u], "
                    "sendRank[%u], sdma stream [%u], "
    ...
    HCCL_INFO("[AlltoAllVFullMeshSymmetricMemory][SdmaMainStreamPost] done");
    return HCCL_SUCCESS;
}

HcclResult AlltoAllVFullMeshSymmetricMemory::SetPostSyncTasks(u32 step, u32 roundIdx)
{
    // SDMA wait
    CHK_RET(SdmaMainStreamWait(step, roundIdx));
    // SDMA post
    CHK_RET(SdmaMainStreamPost(step, roundIdx));
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][SetPostSyncTasks] done");
    return HCCL_SUCCESS;
}

HcclResult AlltoAllVFullMeshSymmetricMemory::SDMAwithRemoteRankAndNotifyEnd(u32 step, u32 roundIdx)
{
    ...
    for (auto& sendRecvSide : partialCommRankSet_) {
        for (auto& sendRecvPair : sendRecvSide) {
            ...
            if (step < readInfo.size()) {
                ...
                HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][SendRecvData] userRank [%u], recvRank[%u], sendRank[%u]," \
                ...
                if (isPostSyncEnable) {
                    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][SendRecvData] post sync begins");
    ...
    HCCL_INFO("[AlltoAllVFullMeshSymmetricMemory][SDMAwithRemoteRankAndNotifyEnd] done");
    return HCCL_SUCCESS;
}

HcclResult AlltoAllVFullMeshSymmetricMemory::SendRecvData(u32 step, u32 roundIdx)
{
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][SendRecvData] userRank [%u] sdma stream [%s] wait main stream",
        userRank_, GetStreamIndexString().c_str());
...
HcclResult AlltoAllVFullMeshSymmetricMemory::LocalCopy()
{
    const SendRecvInfo& localSendRecvInfo = *localSendRecvInfoPtr_;
    ...
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][LocalCopy]userRank [%u] copy from userInput [%llu] len [%llu]" \
...
HcclResult AlltoAllVFullMeshSymmetricMemory::RunGroupFullMeshAlltoall(u32 roundIdx, u32 step)
{
...
HcclResult AlltoAllVFullMeshSymmetricMemory::RunSDMATasks(u32 roundIdx, u32 step, u32 groupRankSize, u32 leftRankSize)
{
    UpdatePartialCommunicationRankSet(roundIdx, groupRankSize, partialCommRankSet_);
...
HcclResult AlltoAllVFullMeshSymmetricMemory::RunSDMA(HcclOpMetaInfoDef &opMeta)
{
    ...
    HCCL_DEBUG("[AlltoAllVFullMeshSymmetricMemory][RunSDMA] userRank [%u] communication rounds[%llu] totalStep [%u] "
    ...
    HCCL_INFO("[AlltoAllVFullMeshSymmetricMemory][RunSDMA] finished.");
    return HCCL_SUCCESS;
}
...
{   
    ...
    if (userRankSize_ == 1) {
        HCCL_INFO("[AlltoAllVFullMeshSymmetricMemory][RunAsync] do localcopy with 1 rank");
        CHK_RET(LocalCopy());
        return HCCL_SUCCESS;
    }
    ...
    HCCL_INFO("[AlltoAllVFullMeshSymmetricMemory][RunAsync] finished.");
    return HCCL_SUCCESS;
}

REGISTER_TEMPLATE(TemplateType::TEMPLATE_ALL_2_ALL_V_DIRECT_FULL_MESH, AlltoAllVFullMeshSymmetricMemory);
} // namespace hccl