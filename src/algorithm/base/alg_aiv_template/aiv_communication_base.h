...
uint32_t* param34 = static_cast<uint32_t*>(args + 34); uint32_t rank = param34[0]; \
uint32_t* param35 = static_cast<uint32_t*>(param34 + 1); uint32_t rankSize = param35[0]; \
uint64_t* param36 = static_cast<uint64_t*>(param35 + 1); uint64_t len = param36[0]; \
uint32_t* param37 = static_cast<uint32_t*>(param36 + 1); uint32_t dataType = param37[0]; \
uint32_t* param38 = static_cast<uint32_t*>(param37 + 1); uint32_t reduceOp = param38[0]; \
uint32_t* param39 = static_cast<uint32_t*>(param38 + 1); uint32_t root = param39[0]; \
int32_t* param40 = static_cast<int32_t*>(param39 + 1); int32_t tag = param40[0]; \
uint32_t* param41 = static_cast<uint32_t*>(param40 + 1); uint32_t numBlocks = param41[0]; \
bool* param42 = static_cast<bool*>(param41 + 1); bool isOpBase = param42[0]; \
uint64_t* param43 = static_cast<uint64_t*>(param42 + 1); uint64_t bufferSize = param43[0]; \
int32_t* param44 = static_cast<int32_t*>(param43 + 1); int32_t aivRdmaStep = param44[0]; \
bool* param45 = static_cast<bool*>(param44 + 1); bool useAivRdmaSmall = param45[0]; \
int32_t* param46 = static_cast<int32_t*>(param45 + 1); int32_t serverNum = param46[0]; \
uint32_t* param47 = static_cast<uint32_t*>(param46 + 1); uint32_t devType = param47[0]; \
GM_ADDR* param48 = static_cast<GM_ADDR*>(param47 + 1); GM_ADDR headCountMem = param48[0]; \
GM_ADDR* param49 = static_cast<GM_ADDR*>(param48 + 1); GM_ADDR tailCountMem = param49[0]; \
GM_ADDR* param50 = static_cast<GM_ADDR*>(param49 + 1); GM_ADDR addOneMem = param50[0]; \
uint32_t* param51 = static_cast<uint32_t*>(param50 + 1); uint32_t counterMemSize = param51[0]; \
bool* param52 = static_cast<bool*>(param51 + 1); bool isEnableCounter = param52[0]; \
uint32_t* param53 = static_cast<uint32_t*>(param52 + 1); uint32_t deterministic = param53[0]; \
uint64_t* param54 = static_cast<uint64_t*>(param53 + 1); uint64_t rmaInfo = param54[0]
