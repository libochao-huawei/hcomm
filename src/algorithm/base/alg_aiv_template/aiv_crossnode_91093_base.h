...
uint32_t dataType, uint32_t reduceOp, uint32_t root, int32_t tag, int32_t numBlocks, bool isOpBase, \
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize, int32_t tag,
        int32_t numBlocks, bool isOpBase, bool useDoubleBuffer); // ALL2ALL的init

    template<typename T>
    __aicore__ inline void InitDeter(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize,
        uint32_t reduceOp, int32_t tag, int32_t numBlocks,bool useDoubleBuffer); 
    ...
    __aicore__ inline void Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize,
        uint64_t perRankBufferCount, uint64_t len, uint32_t reduceOp, int32_t tag, int32_t step, int32_t numBlocks, bool useDoubleBuffer); // AG、RS单算子的init

    template<typename T>
    __aicore__ inline void Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize,
        uint64_t len, uint32_t reduceOp, int32_t tag, int32_t step, int32_t numBlocks, bool useDoubleBuffer); // AG、RS图模式的init
    ...
    __aicore__ inline void localMultiRecord(uint32_t tag, AivNotifyType notifyType);
...
__aicore__ inline void AivCrossNode91093Base::Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize, int32_t tag,
    int32_t numBlocks, bool isOpBase, bool useDoubleBuffer)
{
...
__aicore__ inline void AivCrossNode91093Base::Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize,
    uint64_t perRankBufferCount, uint64_t len, uint32_t reduceOp, int32_t tag, int32_t step, int32_t numBlocks, bool useDoubleBuffer)
{
    flagAddrSelf_ = buffOut0;
    blockGroup_ = step;
    rank_ = rank;
...
template<typename T>
__aicore__ inline void AivCrossNode91093Base::Init(GM_ADDR buffOut0, uint32_t rank, uint32_t rankSize,
    uint64_t len, uint32_t reduceOp, int32_t tag, int32_t step, int32_t numBlocks, bool useDoubleBuffer)
{
    flagAddrSelf_ = buffOut0;
    blockGroup_ = step;
    rank_ = rank;
...
__aicore__ inline void AivCrossNode91093Base::localMultiRecord(uint32_t tag, AivNotifyType notifyType)
{ 
    //todo
    localSetTensor.SetValue(0, tag);
    SyncFunc<HardEvent::S_MTE3>();
    for (int32_t i = 1; i < blockGroup_; i++) {
...
{ 
    ...
    if (!ifClear) {
        localMultiRecord(tag + 1, notifyType);
    } else {
        localMultiRecord(0, notifyType);
    }
    return;
}
