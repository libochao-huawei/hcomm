...
{ 
    ...
    while (true) {
        DataCopy(localTensor, globalTensor, UB_FLAG_PAD_COUNT * blockGroup);
        SyncFunc<HardEvent::MTE2_S>();
        int32_t sum = 0;
        for (int32_t i = 1; i < blockGroup_; i++) {
            sum += localTensor.GetValue(UB_FLAG_PAD_COUNT * i)
        } 
...
__aicore__ inline void AivCrossNode91093Base::SyncAllCycle(int32_t tag, NotifyType notifyType, int32_t blockGroup, bool ifSyncCore)
{
    //todo
...
__aicore__ inline void AivCrossNode91093Base::ClearCycle(AivNotifyType notifyType, int32_t blockGroup)
{
    //todo
