...
__aicore__ inline void aiv_reduce_scatter_v_910b_bigdata(EXTERN_KERNEL_ARGS_DEF)
{
    ...
    if (blockIdx_ >= rankSize) {
        countLeft = extraArgs.sendCounts[blockIdx_ - rankSize];
    } else {
        countLeft = extraArgs.sendCounts[blockIdx_];
    }
    ...
    while (countLeft > 0 || countSelf > 0) {
        if (blockIdx_ == rank ||(blockIdx_ >= rankSize && countSelf <= 0) ||
            (blockIdx_ < rankSize && countLeft <= 0)) {
            break;
        }
