...
HcclResult HcclCommInitClusterInfoV2(const char *clusterInfo, uint32_t rank, HcclComm *comm)
{
    ...
    CHK_PTR_NULL(comm);
    HCCL_RUN_INFO("Entry-HcclCommInitClusterInfo V910_95, commEngine[%u]", 0);
    ...
    uint32_t subCommRankId, HcclCommConfig *config, HcclComm *subComm)
{
    ...
    std::string rankIdsStr;
    for (size_t i = 0; i < rankNum; ++i) {
        std::string rankIdStr = std::to_string(rankIds[i]);
        rankIdsStr += rankIdStr;
        if (i != rankNum - 1) {
            rankIdsStr += " ";
        }
    }
    HCCL_RUN_INFO("Entry-HcclCreateSubCommConfig V910_95 rankIds[%s], subCommRankId, commEngine[%u], hcclBufferSize[%u] MB",
                rankIdsStr.c_str(), subCommRankId, config->hcclOpExpansionMode, config->hcclBufferSize);
    HcclCommInfoV2 &opbasedCommInfoV2 = GetCommInfoV2();
    if (opbasedCommInfoV2.status == DeviceStatus::DEVICE_RECOVERED) {
...
HcclResult HcclGetRankIdV2(HcclComm comm, uint32_t *rank)
{
    ...
    /* 关键状态记录 */
    HCCL_INFO("HcclGetRankIdV2 success, comm[%s], rankIdPtr[%p], rankId[%u]", communicator->GetId().c_str(), rank,
              *rank);
...
HcclResult HcclGetRankSizeV2(HcclComm comm, uint32_t *rankSize)
{
    ...
    /* 关键状态记录 */
    HCCL_INFO("HcclGetRankSizeV2 success, comm[%s], rankSizePtr[%p], rankSize[%u]", communicator->GetId().c_str(),
              rankSize, *rankSize);
