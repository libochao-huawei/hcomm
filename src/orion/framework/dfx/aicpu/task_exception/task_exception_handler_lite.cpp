...
HcclResult TasktypeTransferD2H(const uint8_t sqeType, TaskParamType &taskType)
{
    switch (sqeType) {
        case static_cast<uint8_t>(Rt91095StarsSqeType::RT_91095_SQE_TYPE_PLACE_HOLDER):
        case static_cast<uint8_t>(Rt91095StarsSqeType::RT_91095_SQE_TYPE_NOTIFY_WAIT):
            taskType = TaskParamType::TASK_NOTIFY_WAIT;
            break;
        case static_cast<uint8_t>(Rt91095StarsSqeType::RT_91095_SQE_TYPE_SDMA):
            taskType = TaskParamType::TASK_SDMA;
            break;
        case static_cast<uint8_t>(Rt91095StarsSqeType::RT_91095_SQE_TYPE_NOTIFY_RECORD):
            taskType = TaskParamType::TASK_NOTIFY_RECORD;
            break;
        case static_cast<uint8_t>(Rt91095StarsSqeType::RT_91095_SQE_TYPE_WRITE_VALUE):
            taskType = TaskParamType::TASK_NOTIFY_WAIT;
            break;
        case static_cast<uint8_t>(Rt91095StarsSqeType::RT_91095_SQE_TYPE_UBDMA):
            taskType = TaskParamType::TASK_UB_DMA;
            break;
        default:
...
HcclResult GenerateErrorMessageReport(CommunicatorImplLite *aicpuComm, std::shared_ptr<TaskInfo> taskInfo, ErrorMessageReport &errMsgInfo)
{
    ...
    CHK_RET(TasktypeTransferD2H(taskInfo->taskParam_.taskType, errMsgInfo.taskType));
    ...
    // TODO：如果不是Reduce类型的任务，填HcclReduceOp::HCCL_REDUCE_RESERVED
