...
constexpr size_t JETTY_ID_OFFSET = 8;
constexpr size_t DB_ADDR_OFFSET = 17;
constexpr size_t SQ_VA_OFFSET = 33;
constexpr size_t SQ_DEPTH_OFFSET = 41;
constexpr size_t TP_ID_OFFSET = 45;
constexpr size_t RMT_EID_OFFSET = 49;
constexpr size_t SQ_BUFF_VA_OFFSET = 65;
constexpr size_t MIN_CONN_ID_SIZE = 65;
...
HcclAiRMAWQ UrmaDirectTransport::GetAiRMAWQ()
{
    ...
    if (connUniqueIds.size() < MIN_CONN_ID_SIZE) {
        THROW<InvalidParamsException>("connUniqueIds size is less than %u, please check", MIN_CONN_ID_SIZE);
    }
    ...
    memcpy_s(&wq.sqVA, sizeof(wq.sqVA), &connUniqueIds[SQ_VA_OFFSET], sizeof(wq.sqVA));
    memcpy_s(&wq.sqDepth, sizeof(wq.sqDepth), &connUniqueIds[SQ_DEPTH_OFFSET], sizeof(wq.sqDepth));
    memcpy_s(&wq.tp_id, sizeof(wq.tp_id), &connUniqueIds[TP_ID_OFFSET], sizeof(wq.tp_id));
    memcpy_s(wq.rmtEid, sizeof(wq.rmtEid), &connUniqueIds[RMT_EID_OFFSET], sizeof(wq.rmtEid));
    memcpy_s(&wq.sqVA, sizeof(wq.sqVA), &connUniqueIds[SQ_BUFF_VA_OFFSET], sizeof(wq.sqVA));
...
HcclAiRMACQ UrmaDirectTransport::GetAiRMACQ()
{
    ...
    return conn->GetCqInfo();
}

std::vector<char> UrmaDirectTransport::GetConnUniqueIds()
{
    HCCL_INFO("start packing all conn uniqueIds");
    std::vector<char> result(0);
    for (auto &it : commonLocRes.connVec) {
        HCCL_INFO("UrmaDirectTransport %s", it->Describe().c_str());
