...
void CcuErrorHandler::GetCcuErrorMsg(int32_t deviceId, const ParaCcu &ccuTaskParam, vector<CcuErrorInfo> &errorInfo, uint16_t &ccuMissionStatus)
{
    // 检查mission status
    ...
    ccuMissionStatus = missionStatus;
...
inline uint16_t GetMSIdPerDie(uint16_t msId) {
    return msId & 0x7fff;  // 指令中的MSId，是两个Die统一编址的，显示时需要去除DieId
}
