...
HcclResult TaskExceptionHandler::Save(u32 captureStreamID, u32 streamID, u32 taskID, const void *descBuf, size_t descBufLen)
{
    ...
    CHK_RET(InsertOpFFTSCtxInfo(streamID, taskID, tag, descBuf, descBufLen));
    CHK_RET(InsertRankInfo(tag));
    CHK_RET(InsertOpData(tag));
    return HCCL_SUCCESS;
}
...
HcclResult TaskExceptionHandler::InsertOpFFTSCtxInfo(u32 &streamID, u32 &taskID, std::string &tag,
    const void *descBuf, size_t descBufLen) const
{
    FFTSCtxInfo tmpCTXInfo;
    std::shared_ptr<FFTSOpInfo> tmpCTXInfo = nullptr;
    EXECEPTION_CATCH((tmpCTXInfo = std::make_shared<FFTSCtxInfo>()), return HCCL_E_PTR);
    char *tmpAddr = new (std::nothrow) char[tag.size() + 1]();
    CHK_PTR_NULL(tmpAddr);
    tmpCTXInfo->tag.reset(tmpAddr, default_delete<char[]>());
    CHK_SAFETY_FUNC_RET(memcpy_sp(tmpCTXInfo->tag.get(), tag.size() + 1, tag.data(), tag.size()));
    ...
    if (tempDeque == opFFTSCtxInfo[deviceLogicId_].end()) {
        ...
        EXECEPTION_CATCH((tmpCTXInfoQue = std::make_shared<std::deque<std::shared_ptr<FFTSOpInfo>>>()), return HCCL_E_PTR);
        ...
        if (tempDeque->second->size() > maxTaskCount) {
            ...
            if (tmpCTXInfoItem->descBuf != nullprt) {
