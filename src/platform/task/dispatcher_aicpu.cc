...
HcclResult DispatcherAiCpu::LaunchNewTask(OpUnfoldCacheEntry *entryPtr, const std::vector<OpUnfoldMemRange>& userInputMemRanges, const std::vector<OpUnfoldMemRange>& userOutputMemRanges, Stream& mainStream, std::vector<Stream> &slaveStreams, const bool profL1Enable, const bool isAlltoallv, const AlltoallvMetadata& alltoallvMetadata, const AlltoallvSendRecvInfo& alltoallvSendRecvInfo)
{
    ...
    for (size_t arrayIdx = 0; arrayIdx < sqeArrayCount; ++arrayIdx) {
        ...
        for (size_t i = 0; i < flipInfos.size(); ++i) {
            ...
            CHK_PRT_NULL(updateFlipPlaceholderSqIdxCallback_);
            int64_t curFlipPlaceholderSqIdx = sqeContextBuffer->sqTail; // uint32_t一定不溢出
            HCCL_INFO("[DispatcherAicpu][LaunchNewTask] update flipPlaceholderSqIdx[%lld] for streamId[%d] if first",
                curFlipPlaceholderSqIdx, streamId);
            CHK_RET(updateFlipPlaceholderSqIdxCallback_(streamId, curFlipPlaceholderSqIdx));

#ifdef ENABLE_PARTIAL_OPRETRY_BREAKDOWN
            auto updateFlipEndTime = std::chrono::steady_clock::now();
            double updateFlipUs = std::chrono::duration<double, std::chrono::microseconds>(updateFlipEndTime - updateFlipStartTime).count();
            updateFlipTotalUs += updateFlipUs;
            updateFlipCount++;
            HCCL_WARNING("[DispatcherAiCpu][LaunchNewTask] avg updateFlipPlaceholderSqIdxUs[%f] w/ %u updates",
                updateFlipTotalUs / updateFlipCount, updateFlipCount);
#endif
        ...
        double updateSqeUs = std::chrono::duration<double, std::chrono::microseconds>(updateSqeEndTime - updateSqeStartTime).count();
        updateSqeTotalUs += updateSqeUs;
        updateSqeCount++;
        HCCL_WARNING("[DispatcherAiCpu][LaunchNewTask] avg updateTotalSqeCountUs[%f] w/ %u updates",
            updateSqeTotalUs / updateSqeCount, updateSqeCount);
#endif
...
HcclResult DispatcherAiCpu::LaunchTask(Stream &stream, bool isBlockLaunch)
{
    ...
    updateSqeTotalUs += updateSqeUs;
    updateSqeCount++;
    HCCL_WARNING("[DispatcherAiCpu][LaunchTask] avg updateTotalSqeCountUs[%f] w/ %u updates",
        updateSqeTotalUs / updateSqeCount, updateSqeCount);
#endif
...
HcclResult DispatcherAiCpu::AddFlipTask(Stream &stream)
{
    ...
    HCCL_INFO("[DispatcherAicpu][AddFlipTask] update flipPlaceholderSqIdx[%lld] for streamId[%d] if first",
        curFlipPlaceholderSqIdx, streamId);
    CHK_RET(updateFlipPlaceholderSqIdxCallback_(streamId, curFlipPlaceholderSqIdx));
    ...
    updateFlipTotalUs += updateFlipUs;
    updateFlipCount++;
    HCCL_WARNING("[DispatcherAiCpu][AddFlipTask] avg updateFlipPlaceholderSqIdxUs[%f] w/ %u updates",
        updateFlipTotalUs / updateFlipCount, updateFlipCount);
#endif
