...
HcclResult DispatcherAiCpu::SetLaunchAsyncContext(AsyncUnfoldCacheEntry* asyncUnfoldCacheEntryPtr,
    const AsyncUnfoldStage asyncUnfoldStage, const bool profL1Enable)
{
    ...
    CHK_PRT_RET(asyncUnfoldStage != AsyncUnfoldStage::ASYNC_UNFOLD_GENERATE,
        HCCL_ERROR("asyncUnfoldStage[%u] != ASYNC_UNFOLD_GENERATE", asyncUnfoldStage), HCCL_E_INTERNAL);
...
HcclResult DispatcherAiCpu::LaunchTask(Stream &stream, bool isBlockLaunch)
{
    ...
    if (useAsyncUnfold_) { // 异步展开生成阶段
        ...
        CHK_PTR_NULL(asyncUnfoldCacheEntryPtr_);
        CHK_PRT_RET(asyncUnfoldStage_ != AsyncUnfoldStage::ASYNC_UNFOLD_GENERATE,
            HCCL_ERROR("[DispatcherAiCpu][LaunchTask] asyncUnfoldStage[%u] != ASYNC_UNFOLD_GENERATE",
                asyncUnfoldStage_),
            HCCL_E_INTERNAL);
