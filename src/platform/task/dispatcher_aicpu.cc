...
{
    // 尝试应用局部重执行黑名单
    // 注意: 需要在分配SQE前, 避免过滤的SQE占用taskId; 但要在参数有效性校验后, 避免统计展开未下发的SQE (黑名单是基于首次展开且下发的SQE)
    bool isEnable = false; // 是否使能局部重执行
    bool isFilter = false; // 是否过滤
    CHK_PTR_NULL(applyBlocklistCallback_);
    CHK_RET(applyBlocklistCallback_(false, stream, false, 0, signalAddr, isEnable, isFilter));
    if (isEnable) { // 故障算子重执行, 且使能局部重执行
        // 注意: 故障算子重执行一定不走aicpu cache
        CHK_PRT_RET(isPlaceholder_,
            HCCL_ERROR("[DispatcherAiCpu][SignalRecord] invalid isPlaceholder_[%u] for partial op retry"),
            HCCL_E_INTERNAL);

        // 打印局部重执行信息 (当前为故障算子局部重执行, 非性能瓶颈, 直接用RUN_INFO打印)
        HCCL_RUN_INFO("[DispatcherAiCpu][SignalRecord] isFilter[%d] stream[%d] userRank[%u] signalAddr[0x%016llx] "
            "notifyId[%u]", isFilter, stream.GetHcclStreamInfo().actualStreamId, userRank, signalAddr, notifyId);
    ...
    // 分配SQE前备份sqeCnt
    const uint16_t prevSqeCnt = stream.GetSqeContextPtr()->buffer.sqeCnt;
    ...
    if (isPlaceholder_) {
        ...
        // 注意: 计算alltoallv placeholder sqIdx用于局部重执行 (计算原理参考AddFlipTask中的注释)
        CHK_PTR_NULL(updatePlaceholderSqIdxCallback_);
        const int32_t streamId = streamInfo.actualStreamId;
        int64_t curAlltoallvPlaceholderSqIdx = stream.GetSqeContextPtr()->buffer.sqTail + prevSqeCnt; // uint32_t + uint16_t一定不溢出
        HCCL_INFO("[DispatcherAicpu][SignalRecord] update alltoallvPlaceholderSqIdx[%lld] for streamId[%d]",
            curAlltoallvPlaceholderSqIdx, streamId);
        CHK_RET(updatePlaceholderSqIdxCallback_(streamId, curAlltoallvPlaceholderSqIdx, false));
    ...
    if (isEnable) { // 故障算子局部重执行, 非性能瓶颈. 直接使用RUN_INFO打印TASK信息
        HCCL_RUN_INFO("%s para: streamId[%d] remoteRank[%u] inchip[%d] devType[%d] notifyId[%u]",
            __func__, streamInfo.actualStreamId, userRank, inchip, aicpuInfo_.devType, notifyId);
    } else { // 算子正常展开, 使用DEBUG_CONFIG_TASK打印TASK信息, 避免性能开销
    ...
    u32 remoteUserRank, hccl::LinkType inLinkType, u32 notifyId)
{
    ...
    bool isFilter = false; // 是否过滤
    CHK_PTR_NULL(applyBlocklistCallback_);
    const uint64_t signalAddr = reinterpret_cast<uint64_t>(dst.ptr()); // 参考TransportDeviceP2p::SignalRecord
    CHK_RET(applyBlocklistCallback_(false, stream, false, 0, signalAddr, isEnable, isFilter));
    if (isEnable) { // 故障算子重执行, 且使能局部重执行
        // 注意: 故障算子重执行一定不走aicpu cache
        CHK_PRT_RET(isPlaceholder_,
        ...
        // 打印局部重执行信息 (当前为故障算子局部重执行, 非性能瓶颈, 直接用RUN_INFO打印)
        HCCL_RUN_INFO("[DispatcherAiCpu][SignalRecord] isFilter[%d] stream[%d] remoteUserRank[%u] signalAddr[0x%016llx] "
            "notifyId[%u]", isFilter, stream.GetHcclStreamInfo().actualStreamId, remoteUserRank, signalAddr, notifyId);
    ...
    if (isEnable) { // 故障算子局部重执行, 非性能瓶颈. 直接使用RUN_INFO打印TASK信息
        HCCL_RUN_INFO("%s para: linkType[%u] srcPtr[%p] srcSize[%llu] dstPtr[%p] taskId[%u] streamId[%u] "
            "remoteRank[%u] notifyId[%u] hcclQos[%u]",
            __func__, linkType, src.ptr(), src.size(), dst.ptr(), taskId, streamInfo.actualStreamId,
            remoteUserRank, notifyId, SDMA_QOS_DEFAULT);
    } else { // 算子正常展开, 使用DEBUG_CONFIG_TASK打印TASK信息, 避免性能开销
        PLF_CONFIG_INFO(PLF_TASK,
            "%s para: linkType[%u] srcPtr[%p] srcSize[%llu] dstPtr[%p] taskId[%u] streamId[%u] "
            "remoteRank[%u] notifyId[%u] hcclQos[%u]",
            __func__, linkType, src.ptr(), src.size(), dst.ptr(), taskId, streamInfo.actualStreamId,
            remoteUserRank, notifyId, SDMA_QOS_DEFAULT);
    }
    ...
    bool inchip, u32 notifyId, u32 timeOut)
{
    ...
    bool isFilter = false; // 是否过滤
    CHK_PTR_NULL(applyBlocklistCallback_);
    CHK_RET(applyBlocklistCallback_(false, stream, true, notifyId, 0, isEnable, isFilter));
    if (isEnable) { // 故障算子重执行, 且使能局部重执行
        // 注意: 故障算子重执行一定不走aicpu cache
        CHK_PRT_RET(isPlaceholder_,
            HCCL_ERROR("[DispatcherAiCpu][SignalWait] invalid isPlaceholder_[%u] for partial op retry"),
            HCCL_E_INTERNAL);

        // 打印局部重执行信息 (当前为故障算子局部重执行, 非性能瓶颈, 直接用RUN_INFO打印)
        HCCL_RUN_INFO("[DispatcherAiCpu][SignalWait] isFilter[%d] stream[%d] userRank[%u] remoteUserRank[%u] "
            "notifyId[%u]", isFilter, stream.GetHcclStreamInfo().actualStreamId, userRank, remoteUserRank, notifyId);
    ...
    if (isPlaceholder_) {
        ...
        const int32_t streamId = streamInfo.actualStreamId;
        int64_t curAlltoallvPlaceholderSqIdx = stream.GetSqeContextPtr()->buffer.sqTail + prevSqeCnt; // uint32_t + uint16_t一定不溢出
        HCCL_INFO("[DispatcherAicpu][SignalWait] update alltoallvPlaceholderSqIdx[%lld] for streamId[%d]",
            curAlltoallvPlaceholderSqIdx, streamId);
        CHK_RET(updatePlaceholderSqIdxCallback_(streamId, curAlltoallvPlaceholderSqIdx, false));
    ...
    if (isEnable) { // 故障算子局部重执行, 非性能瓶颈. 直接使用RUN_INFO打印TASK信息
        HCCL_RUN_INFO("%s para: streamId[%u] userRank[%u] remoteRank[%u] inchip[%d] devType[%d] notifyId[%u] "
            "sqeTimeOutTimeOut[%llu]",
    ...
    u32 remoteUserRank, hccl::LinkType inLinkType)
{
    ...
    if (isPlaceholder_) {
        ...
        int64_t curAlltoallvPlaceholderSqIdx = stream.GetSqeContextPtr()->buffer.sqTail +
            stream.GetSqeContextPtr()->buffer.sqeCnt; // uint32_t + uint16_t一定不溢出 (注意: 已经在分配SQE前, 无需备份sqeCnt)
        HCCL_INFO("[DispatcherAicpu][MemcpyAsync] update alltoallvPlaceholderSqIdx[%lld] for streamId[%d]",
            curAlltoallvPlaceholderSqIdx, streamId);
        CHK_RET(updatePlaceholderSqIdxCallback_(streamId, curAlltoallvPlaceholderSqIdx, false));
    ...
    bool isFilter = false; // 是否过滤
    CHK_PTR_NULL(applyBlocklistCallback_);
    CHK_RET(applyBlocklistCallback_(true, stream, false, 0, 0, isEnable, isFilter));
    if (isEnable) { // 故障算子重执行, 且使能局部重执行
        // 注意: 故障算子重执行一定不走aicpu cache
        CHK_PRT_RET(isPlaceholder_,
            HCCL_ERROR("[DispatcherAiCpu][MemcpyAsync] invalid isPlaceholder_[%u] for partial op retry"),
            HCCL_E_INTERNAL);

         // 打印局部重执行信息 (当前为故障算子局部重执行, 非性能瓶颈, 直接用RUN_INFO打印)
        HCCL_RUN_INFO("[DispatcherAiCpu][MemcpyAsync] isFilter[%d] stream[%d] remoteUserRank[%u] "
            "src.ptr[0x%016llx] src.size[%llu] dst.ptr[0x%016llx] dst.size[%llu] inLinkType[%u]",
            isFilter, streamInfo.actualStreamId, remoteUserRank, src.ptr(), src.size(), dst.ptr(), dst.size(), inLinkType);
    ...
    for (uint64_t index = 0; index <= spiltLoop; index++) {
        ...
        if (isEnable) { // 故障算子局部重执行, 非性能瓶颈. 直接使用RUN_INFO打印TASK信息
            HCCL_RUN_INFO("%s para: linkType[%u] srcSplit[%p] dstSplit[%p] countSplit[%llu] taskId[%u] streamId[%u] remoteRank[%u]",
...
HcclResult DispatcherAiCpu::LaunchNewTask(OpUnfoldCacheEntry *entryPtr, const std::vector<OpUnfoldMemRange>& userInputMemRanges, const std::vector<OpUnfoldMemRange>& userOutputMemRanges, Stream& mainStream, std::vector<Stream> &slaveStreams, const bool profL1Enable, const bool isAlltoallv, const AlltoallvMetadata& alltoallvMetadata, const AlltoallvSendRecvInfo& alltoallvSendRecvInfo)
{
    ...
    std::vector<uint32_t> alltoallvPlhOffsets; // alltoallv placeholder相对于刷新时sqTail的偏移量
    ...
    for (size_t arrayIdx = 0; arrayIdx < sqeArrayCount; ++arrayIdx) {
        ...
        CHK_RET(entryPtr->UpdateAndGetSqeArray(arrayIdx, userInputMemRanges, userOutputMemRanges, mainStream, slaveStreams,
            ...
            profL1Enable, profTimestamps, isAlltoallv, alltoallvMetadata, alltoallvSendRecvInfo, alltoallvPlhOffsets));
        
        // 针对A3 AICPU局部重执行, 更新alltoallv placeholder相关的局部重执行信息
        if (isAlltoallv && alltoallvPlhOffsets.size() > 0) {
            CHK_PTR_NULL(updatePlaceholderSqIdxCallback_);

            const uint32_t curSqTail = streamPtr->GetSqeContextPtr()->buffer.sqTail;
            const int32_t streamId = streamPtr->GetHcclStreamInfo().actualStreamId;
            for (size_t plhIdx = 0; plhIdx < alltoallvPlhOffsets.size(); ++plhIdx) {
                int64_t curSqIdx = alltoallvPlhOffsets[plhIdx] + curSqTail; // uint32_t + uint32_t一定不会溢出
                CHK_RET(updatePlaceholderSqIdxCallback_(streamId, curSqIdx, false));
            }
        }
...
HcclResult DispatcherAiCpu::AddFlipTask(Stream &stream)
{
    ...
    CHK_PTR_NULL(updatePlaceholderSqIdxCallback_);
    const int32_t streamId = streamInfo.actualStreamId;
    // uint32_t + uint16_t一定不溢出 (注意: 已经在分配SQE前, 无需备份sqeCnt)
    int64_t curFlipPlaceholderSqIdx = sqeContextBuffer->sqTail + sqeContextBuffer->sqeCnt;
