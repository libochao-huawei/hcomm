...
void AddOneCacheMemcpyPlaceHolderSqeV1(uint16_t streamId, uint16_t taskId, const void *src, const void *dst,
    uint8_t linkType, const uint8_t *sqeIn, uint8_t *sqeType)
{
    *sqeType = SqeType::CACHE_MEMCPY_PLACEHOLDER_SQE;
    ...
    HCCL_INFO("[SQE] cache-memcpy placeholder: streamId=%u, taskId=%u, srcAddr=0x%160llx, dstAddr=0x%160llx, linkType=%u",
        streamId, taskId, src, dst, linkType);
}
    ...
    uint32_t devId, u64 overflowAddr, uint8_t linkType, const uint8_t *sqeIn, uint8_t *sqeType)
{
    ...
    *sqeType = SqeType::CACHE_MEMCPY_RECORD_PLACEHOLDER_SQE;
    ...
    HCCL_INFO("[SQE] cache-write-memcpy placeholder: copyKind=%u,Opcode=0x%x, streamId=%u, len=%u, src:%p, dst:%p",
        static_cast<uint32_t>(rtReduceOp), static_cast<uint32_t>(sqe->u.cache_memcpy_record_task_info.opcode),
        streamId, length, src, dst);
