...
HcclResult HcclSetDispatcherAicpuCallback(HcclDispatcher dispatcherPtr,
    ...
    std::function<HcclResult(const int32_t, const uint64_t)> updateSqeCountsCallback)
{
    ...
    reinterpret_cast<DispatcherAiCpu*>(dispatcherPtr)->SetUpdateSqeCountsCallback(updateSqeCountsCallback);
    return HCCL_SUCCESS;
}
