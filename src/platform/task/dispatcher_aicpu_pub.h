...
public:
    ...
    // 用于A3局部重执行
    void SetUpdateTotalSqeCountCallback(
    ...
    void SetUpdatePlaceholderSqIdxCallback(
        std::function<HcclResult(const int32_t, const int64_t, const bool)> updatePlaceholderSqIdxCallback)
    {
        updatePlaceholderSqIdxCallback_ = updatePlaceholderSqIdxCallback;
        return;
    }
    void SetApplyBlocklistCallback(
        std::function<HcclResult(const bool, Stream&, const bool, const uint32_t, const uint64_t, bool&, bool&)> applyBlocklistCallback)
    {
        applyBlocklistCallback_ = applyBlocklistCallback;
        return;
    }
...
private:
    ...
    std::function<HcclResult(const int32_t, const int64_t, const bool)> updatePlaceholderSqIdxCallback_ = nullptr;
    std::function<HcclResult(const bool, Stream&, const bool, const uint32_t, const uint64_t, bool&, bool&)>
        applyBlocklistCallback_ = nullptr;
};
} // namespace hccl
