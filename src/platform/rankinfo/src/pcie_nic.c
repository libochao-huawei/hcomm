...
int get_abs_path(const char* path, char* abs_path, size_t abs_path_len)
{
...
int get_nics(nic *nics, size_t *nic_len)
{
    ...
    while ((entry=readdir(dir)) != NULL) {
        ...
        hal_sprintf_s(nics[loop].path, MAX_NIC_PATH, "/sys/class/net/%s", entry->d_name);
...
static int GetPcieNics(NPU* npu, size_t pos, size_t npu_count)
{
    ...
    hal_sprintf_s(dev_path, sizeof(dev_path), "/sys/bus/pci/devices/%s", npu[pos].bus_id);
