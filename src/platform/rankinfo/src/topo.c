...
int GetTopoFilePathFromFile(const char* filePath, char* topoFilePath, size_t bufSize)
{
    ...
    hal_memset_s(fileBuf, fileSize+1, 0, fileSize + 1);
...
int TopoGetClosPort(unsigned int mainboardId, int dieId, int *ports, int *portCnt)
{
    ...
    for (int i = 0; i < size; ++i) {
        if (mainboardId == closPortMap[i].mainboardId && dieId == closPortMap[i].dieId) {
            hal_memcpy_s(ports, (*portCnt), closPortMap[i].ports, closPortMap[i].portCnt);
...
int TopoGetFilePath(unsigned mainboard_id, char* buf_size, size_t buf_len)
{
    ...
    switch(mainboard_id)
    {
        case MAIN_BOARD_ID_CARD_NOMESH:
            ret = hal_sprintf_s(buf_size, buf_len, "%s/%s", driver_install_path, "driver/topo/950/atlas_350_1.json");
            break;
        case MAIN_BOARD_ID_CARD_2PMESH:
            ret = hal_sprintf_s(buf_size, buf_len, "%s/%s", driver_install_path, "driver/topo/950/atlas_350_2.json");
            break;
        case MAIN_BOARD_ID_CARD_4PMESH:
            ret = hal_sprintf_s(buf_size, buf_len, "%s/%s", driver_install_path, "driver/topo/950/atlas_350_3.json");
            break;
        case MAIN_BOARD_ID_POD:
        case MAIN_BOARD_ID_POD_2D:
            ret = hal_sprintf_s(buf_size, buf_len, "%s/%s", driver_install_path, "driver/topo/950/atlas_950_1.json");
            break;
        case MAIN_BOARD_ID_SERVER_TYPE1:
        case MAIN_BOARD_ID_SERVER_8PMESH:
            ret = hal_sprintf_s(buf_size, buf_len, "%s/%s", driver_install_path, "driver/topo/950/atlas_850_1.json");
            break;
        default:
            break;
    }
