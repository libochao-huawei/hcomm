...
typedef struct AscendQpWindowDataDef {
...
struct AscendWc {
    ...
    uint32_t srcQp; // source QP number (UD only)
    uint32_t wcFlags; // completion flags
};
...
*/
HcclResult hcclAscendPostSend(AscendQPInfo* qpInfo, struct AscendSendWr *sendWr, struct AscendSendWr **badWr);

/**
* @brief 异构场景PostRecv
* @param qpInfo (in): 本地QP信息。
* @param recvWr (in): 接收WR链表头指针。
* @param badWr (out): 失败的WR指针。
*/
HcclResult hcclAscendPostRecv(AscendQPInfo* qpInfo, struct AscendRecvWr *recvWr, struct AscendRecvWr **badWr);
