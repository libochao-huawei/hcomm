...
HcclResult RdmaResourceManager::Init()
{
    ...
    if (HCCL_SUCCESS == GetResvMemPoolIdByType(0, poolId)) {
        NetworkManager::GetInstance(deviceLogicId_).DeInit(nicDeploy_, devicePhyId_, true);
        NetworkManager::GetInstance(deviceLogicId_).Init(nicDeploy_, false, devicePhyId_, true, true, poolId);
    }
