...
HcclResult RdmaResourceManager::GetResvMemInfo(u32 type, u32& poolId)
{
    auto it = resvMemInfo_.find(type);
    if (it == resvMemInfo_.end()) {
        HCCL_ERROR("[RdmaResourceManager][GetResvMemInfo] can not find [%u].", type);
        return HCCL_E_NOT_FOUND;
    }

    poolId = resvMemInfo_[type];
    HCCL_RUN_INFO("[RdmaResourceManager][GetResvMemInfo] resvMemPoolId [%u].", poolId);
    return HCCL_SUCCESS;
}
