...
HcclResult hcclRdmaMemRegister(AscendMrAttr* memInfo)
{
    ...
    HCCL_RUN_INFO("[hcclRdmaMemRegister] Register WindowMem addr[%p], size[%llu], lkey[%u], rkey[%u].",
...
HcclResult hcclRdmaMemDeRegister(AscendMrAttr* memInfo)
{
    ...
    CHK_RET(TypicalMrManager::GetInstance().DeRegisterMem(mrInfo));
    HCCL_RUN_INFO("[hcclRdmaMemDeRegister] DeRegister WindowMem addr[%p], size[%llu], lkey[%u], rkey[%u].",
        mrInfo.addr, mrInfo.size, memInfo->lkey, memInfo->rkey);
    return HCCL_SUCCESS;
}
