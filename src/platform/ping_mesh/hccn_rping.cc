...
HccnResult HccnRpingBatchPingStart(HccnRpingCtx rpingCtx, uint32_t pktNum, uint32_t interval, uint32_t timeout)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[HccnRpingBatchPingStart]task start failed, devLogicId[%d], ret[%d], pktNum:%u, interval:%u, timeout:%u.",
            devLogicId, ret, pktNum, interval, timeout),
        HCCN_E_FAIL);

    HCCL_RUN_INFO(
        "[HccnRpingBatchPingStart]task start success, devLogicId:%d, pktNum:%u, interval:%u, timeout:%u.", devLogicId,
        pktNum, interval, timeout);
    return HCCN_SUCCESS;
}
