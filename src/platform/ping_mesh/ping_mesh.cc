...
inline HcclResult TsdProcessOpenInit(rtNetServiceOpenArgs &openArgs, rtProcExtParam *extParam)
{
    ...
    for (u32 i = 0; i < TSD_EXT_PARA_NUM; i++) {
        CHK_SAFETY_FUNC_RET(memcpy_s(extParam[i].paramInfo, sizeof(extParam[i].paramInfo), extPam[i].c_str(), extPam[i].size()));
...
HcclResult PingMesh::HccnRpingOpenTsd(u32 deviceId, u32 mode, u32 port, u32 nodeNum, u32 bufferSize, u32 sl, u32 tc)
{
    ...
    CHK_RET(TsdProcessOpenInit(openArgs, extParam));
