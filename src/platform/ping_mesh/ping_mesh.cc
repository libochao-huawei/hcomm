...
HcclResult PingMesh::HccnRpingOpenTsd(u32 deviceId, u32 mode, u32 port, u32 nodeNum, u32 bufferSize, u32 sl, u32 tc)
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[HCCN][HccnRpingOpenTsd]HccnSupportedAndGetphyid Failed, ret[%d].", deviceId, ret);
        return HCCL_E_NOT_SUPPORT;
    }
...
HcclResult PingMesh::HccnRpingOpenTsd(u32 deviceId, u32 mode, HcclIpAddress ipAddr, u32 port, u32 nodeNum, u32 bufferSize,
    u32 sl, u32 tc)
{
    CHK_RET(OpenTsd(deviceId, mode, port, nodeNum, bufferSize, sl, tc));
