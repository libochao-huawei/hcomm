...
{
    RPT_INNER_ERR_PRT(
        "remote op nic connect failed, please ensure that collective communication execution status "
    ...
    HCCL_ERROR(
        "   |  dest_ip(user_rank)  |   dest_port   |  src_ip(user_rank)   |   src_port   |   MyRole   "
        "|   Status   |");
    HCCL_ERROR(
        "   |--------------------|--------------------|----------|------------|-----------------"
        "|-----------------|");
    ...
    HCCL_ERROR(
        "3. the time difference between the execution of hcom on this device and the target device exceeds the "
        "timeout threshold, make sure this by keyword [Entry-].");
    HCCL_ERROR(
        "4. the behavior of executing the calculation graph on this device and the target device is "
        "inconsistent. ");
    HCCL_ERROR(
        "5. Now you can freely specify a port for listening and connecting. If an invalid port is chosen, "
