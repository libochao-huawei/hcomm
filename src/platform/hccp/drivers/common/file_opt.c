...
STATIC int ReadFileToBufCheckParam(const char *path, const char *content, const unsigned int *contentLen)
{
    if (path == NULL || content == NULL || contentLen == NULL) {
        net_err("path or conf_path or content_len is NULL, invalid");
        return -EINVAL;
    }

    if (strlen(path) > PATH_MAX) {
        net_err("path_len[%lu] > [%d](PATH_MAX)", strlen(path), PATH_MAX);
        return -EFAULT;
    }
...
int ReadFileToBuf(const char *path, char *content, unsigned int *contentLen)
{
    ...
    if (ret) {
        net_err("read_file_to_buf_check_param failed, ret[%d]", ret);
        return ret;
    }
    ...
    if (realpath(path, realConfPath) == NULL) {
        ret = -errno;
        if (ret != -ENOENT) {
            net_err("conf_path[%s] is invalid, err[%d]", path, ret);
        }
        return ret;
    }
    ...
    if (readFile == NULL) {
        net_err("read_file is NULL, invalid");
        return -EINVAL;
    }

    ret = fseek(readFile, 0, SEEK_END);
    if (ret < 0) {
        net_err("fseek failed with error:%d", errno);
        goto out;
    }
    ...
    if (tempLen <= 0 || tempLen > (int)*contentLen || tempLen > INT_MAX) {
        ret = -EINVAL;
        net_err("ftell failed with error:%d, tempLen=%ld, contentLen[%u]", errno, tempLen, *contentLen);
        goto out;
    }
    ...
    if (ret != 1) {
        net_err("fread failed ret:%d, error:%d, ferror(fp):%d", ret, errno, ferror(readFile));
        ret = -EINVAL;
        goto out;
    }
    ret = 0;
...
out:
    ...
    if (retVal) {
        net_warn("fclose fail, retVal:%d, errno:%d", retVal, errno);
    }
...
STATIC void CheckFile(const char *file)
{
    ...
    if (access(file, F_OK) == FILE_EXIST) {
        ret = remove(file);
        net_run_info("file %s exist, should remove it, ret:%d, errno:%d", file, ret, errno);
    } else {
        ret = errno;
        if (ret != ENOENT) {
            net_run_info("access file %s invalid, errno:%d", file, ret);
        }
    }
}
...
int WriteBufToFile(const unsigned char *buf, unsigned int bufLen, const char *file, mode_t mode)
{
    ...
    if (buf == NULL || file == NULL) {
        net_err("buf or file is NULL, invalid!");
        return -EINVAL;
    }
    ...
    if (fd < 0) {
        ret = -errno;
        net_err("create file %s failed errno %d", file, ret);
        return ret;
    }

    ret = (int)write(fd, buf, bufLen);
    if (ret != (int)bufLen) {
        net_err("write file %s failed %d, bufLen:%u", file, ret, bufLen);
        ret = -errno;
        do {
            retVal = close(fd);
    ...
    ret = chmod(file, mode);
    if (ret != 0) {
        net_err("file[%s] chmod fail, errno: %d.", file, ret);
        RemoveFile(file);
    }
    return ret;
}
...
void RemoveFile(const char *file)
{
    ...
    if (ret < 0) {
        err = -errno;
        if (err != -ENOENT) {
            net_err("remove end file failed %d", err);
            return;
        }
    }
    return;
...
int CheckFilePath(const char *path, mode_t mode)
{
    ...
    if (realpath(path, realConfPath) == NULL) {
        ret = -errno;
        if (ret == -ENOENT) {
            net_warn("path[%s] is not exist, real_path[%s]", path, realConfPath);
            ret = mkdir(realConfPath, mode);
            if (ret) {
                net_err("mkdir real_conf_path[%s] failed, ret[%d]", realConfPath, ret);
                return ret;
            }
            ret = chmod(realConfPath, mode);
            if (ret != 0) {
                net_err("file[%s] chmod fail, errno: %d.", realConfPath, ret);
            }
            return ret;
        }

        net_err("path[%s] is invalid, err[%d]", path, ret);
        return ret;
    }
...
static void CloseFdSecurity(int fd)
{
    ...
    do {
        ...
        if (ret < 0) {
            errNo = errno;
            if (errNo != EINTR) {
                net_err("close fd[%d] failed, ret:%d, errNo[%d]", fd, ret, errNo);
                return;
            }
        }
...
int GetFileLock(const char *path, int *lockFd)
{
    ...
    if (lockFd == NULL || path == NULL) {
        net_err("lock_fd is NULL or path is NULL.");
        return -EINVAL;
    }
    ...
    if (fd == -1) {
        ret = -errno;
        net_err("open real_path[%s] failed, ret[%d]", path, ret);
        return ret;
    }
    ...
    if (ret) {
        ...
        net_err("lock fd[%d] failed, err[%d]", fd, err);
        return err;
    }
...
int ReleaseFileLock(int lockFd, const char *lockFilePath)
{
    ...
    if (lockFd < 0) {
        net_err("invalid lock_fd[%d]", lockFd);
        return -EINVAL;
    }
    ...
    if (ret) {
        err = -errno;
        net_err("release lock fd[%d] failed, err[%d]", lockFd, err);
...
STATIC int CfgInnerReadConfByfd(FILE *fp, const char *confName, char *confValue, unsigned int len)
{
    ...
    if (lineBuf == NULL) {
        net_err("calloc line_buf failed");
    ...
    while (feof(fp) == 0 && fgets(lineBuf, CONLINE_LEN, fp) != NULL) {
        ...
        if ((strncmp(lineBuf, confName, strlen(confName)) == 0) && (lenTmp == strlen(confName))) {
            ...
            if (ret) {
                net_err("strcpy_s err[%d], len:%u", ret, len);
...
STATIC int CfgInnerReadConf(const char *confPath, const char *confName, char *confValue, unsigned int len)
{
    ...
    if ((strlen(confPath) > PATH_MAX) || (realpath(confPath, realConfPath) == NULL)) {
        net_warn("read path_len[%u] > PATH_MAX[%d] or conf_path is invalid, errno[%d]",
    ...
    if (fp == NULL) {
        net_err("Open configure file fail errno[%d] real_conf_path[%s]", errno, realConfPath);
        return FILE_OPT_SYS_READ_FILE_ERR;
    }

    ret = flock(fileno(fp), LOCK_EX);
    if (ret) {
        net_err("hccn.conf lock fd[%d] failed! ret[%d] errno[%d]", fileno(fp), ret, errno);
    ...
    if (ret) {
        net_err("hccn.conf fseek fd[%d] failed! ret[%d] errno[%d]", fileno(fp), ret, errno);
        ret = FILE_OPT_SYS_READ_FILE_ERR;
        goto out;
    }
...
int FileReadCfg(const char *filePath, int devId, const char *confName, char *confValue, unsigned int len)
{
    ...
    if (ret <= 0) {
        net_err("conf str op fail! ret[%d] conf_name[%s] dev_id[%d]", ret, confName, devId);
        return -EINVAL;
    }

    ret = CfgInnerReadConf(filePath, conf, confValue, len);
    if (ret == FILE_OPT_SYS_RD_FILE_NOT_FOUND) {
        net_info("the conf[%s] not found", conf);
        return ret;
    } else if (ret == FILE_OPT_INNER_PARAM_ERR) {
        net_warn("cfg_inner_read_conf unsuccessful ret[%d]", ret);
        return ret;
    } else if (ret != 0) {
        net_err("cfg_inner_read_conf fail! ret[%d] conf[%s] conf_value[%s]", ret, conf, confValue);
        return ret;
    }

    net_info("read conf[%s] realconf[%s] val[%s]", conf, confName, confValue);
