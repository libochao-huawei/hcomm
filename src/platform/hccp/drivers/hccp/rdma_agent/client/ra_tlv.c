...
HCCP_ATTRI_VISI_DEF int RaTlvInit(struct TlvInitInfo *initInfo, unsigned int *bufferSize, void **tlvHandle)
{
    ...
    hccp_run_info("Input parameters: phyId[%u], nicPosition[%u]", initInfo->phyId, initInfo->nicPosition);
...
HCCP_ATTRI_VISI_DEF int RaTlvDeinit(void *tlvHandle)
{
    ...
    hccp_run_info("Input parameters: phyId[%u], nic_position[%u]", tlvHandleTmp->initInfo.phyId,
