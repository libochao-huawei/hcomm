...
{
    BOOL argIsInvalid = FALSE;
    int32_t val;
    int32_t r;

    argIsInvalid = (numArray == VOS_NULL) || (numCnt <= 0) || (numIdx < 0) || (numIdx >= numCnt);
    if (argIsInvalid) {
        if (hasErrOrNull != VOS_NULL) {
            *hasErrOrNull = VOS_TRUE;
        }
        return -1;
    }

    if (hasErrOrNull != VOS_NULL) {
        *hasErrOrNull = VOS_FALSE;
    }
    val = numArray[numIdx];
    if (randMax != 0) {
        r = (int32_t)VOS_Rand((uint32_t)randMax + 1);
        return ((r % BKF_NUM2) != 0) ? (val + r) : (val - r);
    } else {
        return val;
    }
}
