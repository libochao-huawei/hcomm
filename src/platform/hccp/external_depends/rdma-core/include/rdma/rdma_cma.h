...
enum {
	...
	RDMA_OPTION_ID_BINDTODEVICE = 64 /* const char*, optlen include '\0' */
};
...
/**
 * rdma_get_verbs - Returns verbs context.
 * @id: Communication identifier.
 */
struct ibv_context *rdma_get_verbs(const struct rdma_cm_id *cm_id);

/**
 * rdma_put_verbs - Put verbs refcount.
 * @verbs: Verbs context pointer.
 */
void rdma_put_verbs(struct ibv_context *verbs);

/**
 * rdma_bind_device - Bind to device before rdma_bind_addr
 * @id: Communication identifier
 * @ndev_name: net device name
 */
int rdma_bind_device(struct rdma_cm_id *id, const char *ndev_name);
