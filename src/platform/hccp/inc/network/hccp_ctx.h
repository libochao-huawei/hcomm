...
struct ctx_init_cfg {
    int mode; /**< refer to enum network_mode */
    union {
        struct {
...
enum jfc_mode {
    JFC_MODE_NORMAL = 0,      /* Not supported currently */
...
union jfs_wr_flag {
    struct {
        ...
        uint32_t inline_flag : 1;       /* 0: No inline data.
                                           1: Inline data. */
...
struct send_wr_data {
    ...
    union {
        ...
        struct {
            ...
            struct wr_reduce_info reduce_info; /**< inline reduce is enabled when reduce_en is set to true */
        } ub;
    };
...
struct send_wr_resp {
    union {
        struct wqe_info wqe_tmp; /**< wqe template info used for V80 offload */
        struct db_info db; /**< doorbell info used for V71 and V80 opbase */
