...
#define HNS_ROCE_CMQ_SEND_RET_CHECK(ret) \
    if (ret) { \
        dev_err(hr_dev->dev, "%s(%d) hns_roce cmq_send failed, ret[%d]\n", __func__, __LINE__, ret); \
        return ret; \
    }

#define HNS_ROCE_ALLOC_CMD_MAILBOX_RET_CHECK(mailbox) \
    if (IS_ERR(mailbox)) { \
        dev_err(hr_dev->dev, "%s(%d) hns_roce malloc mailbox failed\n", __func__, __LINE__); \
        return PTR_ERR(mailbox); \
    }

#define PTR_RDMA_NL_PUT_DRIVER_U32_ERR(attr) \
    pr_err("hns3: rdma drivers fill out nldev driver attribute [%s] failed\n", attr)
