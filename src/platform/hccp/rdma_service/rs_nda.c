...
int is_address_valid(void *addr)
{
    ...
    // 检查该页是否可访问
    if (mincore(page_start, page_size, &flag) == 0) {
        return 1; // 合法
    }
...
STATIC void *RsNdaDbMmapHostVa(struct RsNdaCb *ndaCb, struct doorbell_map_desc *desc)
{
    ...
    hccp_err("visited hva1:0x%llx hva1_align:0x%llx", desc->hva, alignHva);
    ...
    hccp_err("visited hva2:0x%llx hva2_align:0x%llx", desc->hva, alignHva);
    ret = is_address_valid((void *)(uintptr_t)alignHva);
    hccp_err("invalid ret%d", ret);
    ...
    if (ret != 0) {
        ...
        hccp_err("visited hva3:0x%llx hva3_align:0x%llx", desc->hva, alignHva);
        free(ndaDbCb);
        ndaDbCb = NULL;
        return NULL;
    }
    ...
    struct NdaQpInfo *info, struct RsQpResp *qpResp)
{
    ...
    hccp_err("craete qp, qpn:%d", qpCb->qpInfoLo.qpn);
