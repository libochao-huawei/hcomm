...
STATIC void RsNdaGetDirectFlagByRdev(struct RsRdevCb *rdevCb, int *directFlag)
{
...
RS_ATTRI_VISI_DEF int RsNdaGetDirectFlag(unsigned int phyId, unsigned int rdevIndex, int *directFlag)
{
    ...
    RsNdaGetDirectFlagByRdev(rdevCb, directFlag);
    return ret;
}
...
STATIC int RsGetNdaPcieDbCb(struct RsNdaCb *ndaCb, uint64_t hva, struct NdaPcieDbCb **ndaDbCb)
{
    ...
    RS_PTHREAD_MUTEX_ULOCK(&ndaCb->ndaMutex);
    *ndaDbCb = NULL;
    hccp_info("ndaDbCb for hva:0x%llx do not exist", hva);
    return -ENODEV;
}
...
STATIC int RsGetNdaUbDbCb(struct RsNdaCb *ndaCb, uint64_t guidL, uint64_t guidH, struct NdaUbDbCb **ndaDbCb)
{
    ...
    RS_PTHREAD_MUTEX_ULOCK(&ndaCb->ndaMutex);
    *ndaDbCb = NULL;
    hccp_info("ndaDbCb for guidL:0x%llx guidH:0x%llx do not exist", guidL, guidH);
    return -ENODEV;
}
...
STATIC void *RsNdaPcieAlloc(size_t size)
{
    ...
    CHK_PRT_RETURN(ndaCb == NULL, hccp_err("gRsCb->ndaCb is NULL, chipId:%u", gRsCb->chipId), NULL);
    CHK_PRT_RETURN(ndaCb->ndaOps.alloc == NULL, hccp_err("ndaOps.alloc is NULL, chipId:%u", gRsCb->chipId), NULL);
...
STATIC void RsNdaPcieFree(void *ptr)
{
    ...
    if (gRsCb == NULL) {
        hccp_err("gRsCb is NULL");
        return;
    }
    ndaCb = (struct RsNdaCb *)gRsCb->ndaCb;
...
STATIC void *RsNdaUbAlloc(size_t size)
{
    ...
    CHK_PRT_RETURN(ndaCb == NULL, hccp_err("gRsCb->ndaCb is NULL, chipId:%u", gRsCb->chipId), NULL);
    CHK_PRT_RETURN(ndaCb->ndaOps.alloc == NULL || ndaCb->ndaOps.free == NULL,
        hccp_err("ndaOps alloc or free is NULL, chipId:%u", gRsCb->chipId), NULL);
...
STATIC void *RsNdaDbMmapHostVa(struct RsNdaCb *ndaCb, struct doorbell_map_desc *desc)
{
    ...
    ret = RsGetNdaPcieDbCb(ndaCb, desc->hva, &ndaDbCb);
    if (ret == 0) {
        goto return_dva;
    }
    ...
    ret = DlHalHostRegister((void *)(uintptr_t)alignHva, alignSize, HOST_IO_MAP_DEV, logicId,
        &dbDva);
    if (ret != 0) {
        hccp_err("register host failed, chipId:%u logicId:%u ret:%d", gRsCb->chipId, logicId, ret);
        free(ndaDbCb);
        ndaDbCb = NULL;
        return NULL;
    }
...
int RsInitNdaCb(struct RsRdevCb *rdevCb)
{
    ...
    count = __sync_fetch_and_add(&rdevCb->rsCb->ndaRefCnt, 1);
    if (count != 0) {
        return 0;
    }
...
mutex_err:
    ...
    ndaCb = NULL;
calloc_err:
    (void)__sync_fetch_and_sub(&rdevCb->rsCb->ndaRefCnt, 1);
...
void RsDeinitNdaCb(struct RsRdevCb *rdevCb)
{
    ...
    if (__sync_fetch_and_sub(&rdevCb->rsCb->ndaRefCnt, 1) > 1) {
        return;
    }

    (void)pthread_mutex_destroy(&ndaCb->ndaMutex);
    ...
    struct ibv_cq_init_attr_extend *cqInitAttrEx)
{
    ...
    RsNdaInitExOps(ndaCb, attr->dmaMode, &ndaCb->ndaOps, cqInitAttrEx->ops);
}
    ...
    struct ibv_qp_init_attr_extend *qpInitAttrEx)
{
    ...
    RsNdaInitExOps(ndaCb, attr->dmaMode, &ndaCb->ndaOps, qpInitAttrEx->ops);
}
    ...
    struct NdaQpInfo *info, struct RsQpResp *qpResp)
{
    ...
    RsNdaGetDirectFlagByRdev(rdevCb, &qpResp->directFlag);
    return 0;
