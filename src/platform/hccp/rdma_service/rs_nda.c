    ...
    struct NdaCqInfo *info, void *ibvCqExt)
{
    ...
    if (ret != 0) {
        hccp_err("RsNdaCqCreateEx failed, phyId:%u rdevIndex:%u ret:%d", phyId, rdevIndex, ret);
        return ret;
    }
...
RS_ATTRI_VISI_DEF int RsNdaCqDestroy(unsigned int phyId, unsigned int rdevIndex, void *ibvCqExt)
{
    ...
    if (ret != 0) {
        hccp_err("RsNdaIbvDestroyCqExtend failed, phyId:%u rdevIndex:%u ret:%d", phyId, rdevIndex, ret);
        return ret;
    }
