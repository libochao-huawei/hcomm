...
STATIC void RsNdaInitExOps(uint32_t dmaMode, struct NdaOps *ops)
{
    if (dmaMode == QBUF_DMA_MODE_DEFAULT) {
        gRsCb->ibvExOps.alloc = ops->alloc;
        gRsCb->ibvExOps.free = ops->free;
    } else if (dmaMode == QBUF_DMA_MODE_INDEP_UB) {
        gRsCb->ndaOps.alloc = ops->alloc;
        gRsCb->ndaOps.free = ops->free;
        gRsCb->ibvExOps.alloc = RsNdaUbAlloc;
        gRsCb->ibvExOps.free = RsNdaUbFree;
    }
    gRsCb->ibvExOps.memset_s = ops->memset_s;
    gRsCb->ibvExOps.memcpy_s = ops->memcpy_s;
}
...
{
    RsNdaInitExOps(attr->dmaMode, attr->ops);
