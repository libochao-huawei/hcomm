...
STATIC int RsOpenHrnSo(void)
{
    ...
    if (gHrnApiHandle == NULL) {
        ...
        hccp_run_info("Hrn_api dlopen again!");
    }
#endif
    return 0;
}
...
int RsRoceSetQpLbValue(struct ibv_qp *qp, int lb_value)
{
    if (gHrnOps.rsRoceSetQpLbValue == NULL) {
#ifndef CA_CONFIG_LLT
        hccp_run_info("RsRoceSetQpLbValue is NULL");
        return 0;
#endif
    }
    return gHrnOps.rsRoceSetQpLbValue(qp, lb_value);
}

int RsRoceGetQpLbValue(struct ibv_qp *qp, int *lb_value)
{
    CHK_PRT_RETURN(lb_value == NULL, hccp_err("param error, lbValue is NULL"), -EINVAL);

    if (gHrnOps.rsRoceGetQpLbValue == NULL) {
#ifndef CA_CONFIG_LLT
        hccp_run_info("RsRoceGetQpLbValue is NULL");
        *lb_value = 1;
        return 0;
#endif
    }
    return gHrnOps.rsRoceGetQpLbValue(qp, lb_value);
}

int RsRoceGetQpNum(struct ibv_context *context, int *qp_num)
{
    CHK_PRT_RETURN(qp_num == NULL, hccp_err("param error, qp_num is NULL"), -EINVAL);

    if (gHrnOps.rsRoceGetQpNum == NULL) {
#ifndef CA_CONFIG_LLT
        hccp_run_info("RsRoceGetQpNum is NULL");
        *qp_num = 1;
        return 0;
#endif
    }
    return gHrnOps.rsRoceGetQpNum(context, qp_num);
}