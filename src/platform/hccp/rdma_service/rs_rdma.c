...
STATIC int RsMemRegisterUbSegment(struct RsRdevCb *rdevCb, unsigned int chipId, uint64_t addr, uint64_t len)
{
    if (rdevCb->deviceAttr.vendor_id == RS_VENDOR_ID_19E5) {
...
STATIC int RsMemUnRegisterUbSegment(struct RsRdevCb *rdevCb, unsigned int chipId, uint64_t addr, uint64_t len)
{
    if (rdevCb->deviceAttr.vendor_id == RS_VENDOR_ID_19E5) {
...
RS_ATTRI_VISI_DEF int RsDeregisterMr(unsigned int phyId, unsigned int rdevIndex, void *mrHandle)
{
    ...
    ret = RsMemUnRegisterUbSegment(devCb, chipId, (uint64_t)(uintptr_t)rsMrHandle->addr, rsMrHandle->length);
