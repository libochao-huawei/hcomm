...
RS_ATTRI_VISI_DEF int RsDeregisterMr(unsigned int phyId, unsigned int rdevIndex, uint64_t addr,
    uint64_t va, uint64_t size)
{
    struct ibv_mr *rsMrHandle = (struct ibv_mr *)(uintptr_t)addr;
    struct RsRdevCb *devCb = NULL;
    unsigned int chipId;
    int ret = 0;
    ...
    ret = RsMemUnRegisterUbSegment(devCb, chipId, );
