...
STATIC int RsMemRegisterUbSegment(int directFlag, unsigned int chipId, uint64_t addr, uint64_t len)
{
...
STATIC int RsMemUnRegisterUbSegment(int directFlag, unsigned int chipId, uint64_t addr)
{
    // if (directFlag == DIRECT_FLAG_UB) {
    ...
    void **mrHandle)
{
    ...
    ret = RsMemRegisterUbSegment(rdevCb->directFlag, chipId, (uint64_t)(uintptr_t)mrRegInfo->addr, mrRegInfo->len);
    if (ret != 0) {
    ...
    *mrHandle = (void *)RsDrvMrReg(rdevCb->ibPd, mrRegInfo->addr, mrRegInfo->len, mrRegInfo->access);
    ...
    return ret;
mr_reg_err:
    (void)RsMemUnRegisterUbSegment(rdevCb->directFlag, chipId, (uint64_t)(uintptr_t)mrRegInfo->addr);
mem_reg_err:
    mrRegInfo->lkey = 0;
...
RS_ATTRI_VISI_DEF int RsDeregisterMr(unsigned int phyId, unsigned int rdevIndex, void *mrHandle)
{
    ...
    ret = RsMemUnRegisterUbSegment(devCb->directFlag, chipId, (uint64_t)(uintptr_t)rsMrHandle->addr);
