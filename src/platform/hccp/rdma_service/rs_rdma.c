...
STATIC int RsMemUnRegisterUbSegment(bool needUnRegUb, unsigned int chipId, uint64_t addr, uint64_t len)
{
    if (needUnRegUb) {
...
RS_ATTRI_VISI_DEF int RsDeregisterMr(unsigned int phyId, unsigned int rdevIndex, void *mrHandle)
{
    ...
    ret = RsMemUnRegisterUbSegment((rdevCb->deviceAttr.vendor_id == RS_VENDOR_ID_19E5), chipId,
        (uint64_t)(uintptr_t)rsMrHandle->addr, rsMrHandle->length);
