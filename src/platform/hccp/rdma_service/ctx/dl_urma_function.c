...
STATIC int RsUrmaDeviceApiInit(void)
{
    ...
    gUrmaOps.rsUrmaGetSmac = (urma_status_t (*rsUrmaGetSmac)(const urma_context_t *, uint8_t *))
        HccpDlsym(gUrmaApiHandle, "urma_get_smac");
    DL_API_RET_IS_NULL_CHECK(gUrmaOps.rsUrmaGetSmac, "urma_get_smac");

    gUrmaOps.rsUrmaGetDmac = (urma_status_t (*rsUrmaGetDmac)(const urma_context_t *ctx, const urma_net_addr_t *net_addr, uint8_t *mac))
...
int rsUrmaGetDmac(const urma_context_t *ctx, const urma_net_addr_t *net_addr, uint8_t *mac)
{
    ...
    return gUrmaOps.rsUrmaGetDmac(ctx, net_addr, mac);
}
