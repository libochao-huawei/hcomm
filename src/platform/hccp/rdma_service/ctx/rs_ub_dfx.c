    ...
    struct RsCtxAsyncEventCb *asyncEventCb)
{
    ...
    switch (event->event_type) {
        case URMA_EVENT_JFC_ERR:
            ...
            CHK_PRT_RETURN(ret != 0, hccp_err("RsUrmaGetJfcOpt failed, ret:%d, jfcId:%u", ret,
        ...
        case URMA_EVENT_JETTY_LIMIT:
            ...
            ret = RsUrmaGetJettyOpt(event->element.jetty, URMA_JETTY_FULL_CTX, asyncEventCb->context,
                JETTY_CONTEXT_MAX_LEN);
            CHK_PRT_RETURN(ret != 0, hccp_err("RsUrmaGetJettyOpt failed, ret:%d, jettyId:%u", ret,
        ...
        case URMA_EVENT_ELR_DONE:
            ...
            break;
        default:
            hccp_warn("invalid eventType:%d dev_index:0x%x", event->event_type, devCb->index);
            break;
    }
    return ret;
}
...
STATIC int RsUbGetSaveAsyncEvent(struct RsUbDevCb *devCb)
{
    ...
    hccp_run_info("get asyncEventType:%d resId:%u devIndex:0x%x", event->event_type, asyncEventCb->resId,
        devCb->index);
...
STATIC int RsUbCtxFillAsyncEvents(struct RsCtxAsyncEventCb *eventCb, struct AsyncEvent *asyncEvent)
{
    int ret = 0;
...
void RsUbCtxGetAsyncEvents(struct RsUbDevCb *devCb, struct AsyncEvent asyncEvents[], unsigned int *num)
{
    ...
    *num = 0;
    ...
    for (; (&eventCbCurr->list) != &devCb->asyncEventList; eventCbCurr = eventCbNext,
        ...
        ret = RsUbCtxFillAsyncEvents(eventCbCurr, &asyncEvents[*num]);
        if (ret != 0) {
            hccp_err("RsUbCtxFillAsyncEvents failed, ret:%d devIndex:0x%x", ret, devCb->index);
...
int RsUbGetJettyContext(struct RsUbDevCb *devCb, unsigned int id, char context[], unsigned int *len)
{
    ...
    int ret = 0;
    uint32_t uContext[64] = {0};
    unsigned int i;

    ret = RsUbGetJettyCb(devCb, id, &jettyCb);
    CHK_PRT_RETURN(ret != 0, hccp_err("get jettyCb failed, ret:%d, jettyId:%u", ret, id), ret);

    hccp_run_info("opcode:0x%x, jettyId:%u, len:%u", URMA_JFS_FULL_CTX, id, JETTY_CONTEXT_LEN);
    ret = RsUrmaGetJettyOpt(jettyCb->jetty, URMA_JFS_FULL_CTX, context, JETTY_CONTEXT_LEN);
    CHK_PRT_RETURN(ret != 0, hccp_err("RsUrmaGetJettyOpt failed, ret:%d, jettyId:%u", ret, id), -EOPENSRC);
    (void)memcpy_s(uContext, JETTY_CONTEXT_LEN, context, JETTY_CONTEXT_LEN);
    for (i = 0;i < 64;i++) {
        hccp_run_info("i:%u, context:0x%x", i, uContext[i]);
    }

    ret = RsUbGetJfcCb(devCb, jettyCb->scqIndex, &sendJfcCb);
    CHK_PRT_RETURN(ret != 0, hccp_err("RsUbGetJfcCb failed, ret:%d, jettyId:%u", ret, id), -EOPENSRC);

    outJfc = (urma_jfc_t *)(uintptr_t)sendJfcCb->jfcAddr;
    ret = RsUrmaGetJfcOpt(outJfc, URMA_JFC_FULL_CTX, context, JFC_CONTEXT_LEN);
    CHK_PRT_RETURN(ret != 0, hccp_err("RsUrmaGetJfcOpt failed, ret:%d, jettyId:%u", ret, outJfc->jfc_id.id), -EOPENSRC);
    (void)memcpy_s(uContext, JFC_CONTEXT_LEN, context, JFC_CONTEXT_LEN);
    for (i = 0;i < 32;i++) {
        hccp_run_info("i:%u, context:0x%x", i, uContext[i]);
    }
