...
{
    unsigned int jettycontext[64] = {0};
    struct RsCtxJettyCb *jettyCb = NULL;
    int ret = 0;
    unsigned int i;
    ...
    ret = RsUrmaGetJettyOpt(jettyCb->jetty, URMA_JFS_FULL_CTX, jettycontext, JETTY_CONTEXT_LEN);
    CHK_PRT_RETURN(ret != 0, hccp_err("RsUrmaGetJettyOpt failed, ret:%d, jettyId:%u", ret, id), -EOPENSRC);

    for (i = 0; i < 64; i++) {
        hccp_run_info("context: 0x%x", jettycontext[i]);
    }
