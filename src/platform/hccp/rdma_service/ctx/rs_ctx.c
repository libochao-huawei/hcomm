...
RS_ATTRI_VISI_DEF int RsCtxGetJettyContext(struct RaRsDevInfo *devInfo, unsigned int id, uint8_t context[],
    unsigned int *len)
{
    struct RsUbDevCb *devCb = NULL;
    struct rs_cb *rscb = NULL;
    ...
    CHK_PRT_RETURN(*len != CONTEXT_MAX_LEN, hccp_err("len not equal to %u", CONTEXT_MAX_LEN), -EINVAL);
