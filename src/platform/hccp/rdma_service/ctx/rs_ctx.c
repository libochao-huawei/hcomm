...
RS_ATTRI_VISI_DEF int RsCtxGetJettyContext(struct RaRsDevInfo *devInfo, unsigned int id, char context[],
    unsigned int *len)
{
    struct RsUbDevCb *devCb = NULL;
    struct rs_cb *rscb = NULL;
    ...
    ret = rs_ub_get_dev_cb(rscb, devInfo->devIndex, &devCb);
    CHK_PRT_RETURN(ret != 0, hccp_err("get dev_cb failed, ret:%d dev_index:0x%x", ret, devInfo->devIndex), ret);

    ret = rs_ub_get_jetty_context(devCb, id, context, len);
