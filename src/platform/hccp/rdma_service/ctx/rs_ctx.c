...
RS_ATTRI_VISI_DEF int RsCtxQpDestroyBatch(struct RaRsDevInfo *devInfo, unsigned int ids[], unsigned int *num)
{
    ...
    CHK_PRT_RETURN(*num == 0 || *num > HCCP_MAX_QP_DESTROY_BATCH_NUM,
        hccp_err("num[%d] shoud <= %d and not zero", HCCP_MAX_QP_DESTROY_BATCH_NUM), -EINVAL);
    ...
    struct JettyAttr attr[], unsigned int *num)
{
    ...
    RS_CHECK_POINTER_NULL_RETURN_INT(attr);
    RS_CHECK_POINTER_NULL_RETURN_INT(num);
    CHK_PRT_RETURN(*num == 0 || *num > HCCP_MAX_QP_QUERY_NUM, hccp_err("num[%d] shoud <= %d and not zero", HCCP_MAX_QP_QUERY_NUM),
        -EINVAL);
