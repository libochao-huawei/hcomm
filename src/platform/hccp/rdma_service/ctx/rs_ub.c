    ...
    switch (event->event_type) {
        ...
        case URMA_EVENT_ELR_DONE:
            *resId = devCb->index;
            break;
        default:
            hccp_err("no such event_type:%d dev_index:0x%x", event->event_type, devCb->index);
            break;
    }
}
...
STATIC int RsUbGetSaveAsyncEvent(struct RsUbDevCb *devCb)
{
    ...
    RsUbGetAsyncEventResId(event, devCb, &resId);
    hccp_run_info("get async_event_type:%d res_id:%u dev_index:0x%x", event->event_type, resId, devCb->index);
...
int RsEpollEventUrmaAsyncEventInHandle(struct rs_cb *rsCb, int fd)
{
    ...
    for (; (&devCbCurr->list) != &rsCb->rdevList;
        devCbCurr = devCbNext, devCbNext = list_entry(devCbNext->list.next, struct RsUbDevCb, list)) {
        if (devCbCurr->urmaCtx->async_fd == fd) {
            RS_PTHREAD_MUTEX_LOCK(&devCbCurr->mutex);
...
STATIC void RsUbCtxFillAsyncEvents(struct RsCtxAsyncEventCb *eventCb, struct AsyncEvent asyncEvents[],
    unsigned int *num)
{
    RsUbGetAsyncEventResId(&eventCb->asyncEvent, eventCb->devCb, &asyncEvents[*num].resId);
    asyncEvents[*num].eventType = eventCb->asyncEvent.event_type;
}
...
void RsUbCtxGetAsyncEvents(struct RsUbDevCb *devCb, struct AsyncEvent asyncEvents[], unsigned int *num)
{
    ...
    *num = 0;
    ...
    for (; (&eventCbCurr->list) != &devCb->asyncEventList; eventCbCurr = eventCbNext,
        ...
        RsUbCtxFillAsyncEvents(eventCbCurr, asyncEvents, num);
        (*num)++;
