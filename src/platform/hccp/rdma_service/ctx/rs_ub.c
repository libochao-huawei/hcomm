...
int RsUbCtxLmemUnreg(struct RsUbDevCb *devCb, unsigned long long addr)
{
    ...
    ret = RsUbQuerySegCb(devCb, addr, &lsegCb, &devCb->lsegList.list);
...
int RsUbCtxRmemUnimport(struct RsUbDevCb *devCb, unsigned long long addr)
{
    ...
    ret = RsUbQuerySegCb(devCb, addr, &remSegCb, &devCb->rsegList.list);
    if (ret != 0) {
    ...
    unsigned int *totalLen, bool isInline)
{
    ...
    if (isInline == false) {
        for (i = 0; i < wrData->numSge; i++) {
            ...
            ret = RsUbQuerySegCb(devCb, wrData->sges[i].devLmemHandle, &segCb, &devCb->lsegList.list);
            if (ret != 0) {
    ...
    unsigned int totalLen, urma_opcode_t opcode)
{
    ...
    rsge[0].len = totalLen;
    ret = RsUbQuerySegCb(devCb, wrData->devRmemHandle, &segCb, &devCb->rsegList.list);
    if (ret != 0) {
