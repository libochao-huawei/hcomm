...
int RsUbCtxLmemReg(struct RsUbDevCb *devCb, struct MemRegAttrT *memAttr, struct MemRegInfoT *memInfo)
{
    ...
    if (lsegCb->segment->token_id == NULL) {
        hccp_err("[init][rs_ctx_lmem] token_id is null");
        ret = -EINVAL;
        goto reg_err;
    }
...
STATIC int RsUbCtxDrvJettyImport(struct RsCtxRemJettyCb *rjettyCb)
{
    ...
    } else (rjettyCb->mode == JETTY_IMPORT_MODE_EXP) { // rjetty_cb->mode == JETTY_IMPORT_MODE_EXP
