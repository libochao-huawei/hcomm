    ...
    struct RsCtxAsyncEventCb *asyncEventCb)
{
    ...
    switch (event->event_type) {
        case URMA_EVENT_JFC_ERR:
            ...
            CHK_PRT_RETURN(ret != 0, hccp_err("RsUrmaGetJfcOpt failed, ret:%d, jfc_id:%u", ret,
        ...
        case URMA_EVENT_JETTY_LIMIT:
            ...
            ret = RsUrmaGetJettyOpt(event->element.jetty, URMA_JETTY_FULL_CTX, asyncEventCb->context,
                JETTY_CONTEXT_MAX_LEN);
            CHK_PRT_RETURN(ret != 0, hccp_err("RsUrmaGetJettyOpt failed, ret:%d, jetty_id:%u", ret,
...
STATIC int RsUbGetSaveAsyncEvent(struct RsUbDevCb *devCb)
{
    ...
    if (ret != 0) {
        hccp_err("rs_urma_ack_async_event failed, ret:%d dev_index:0x%x", ret, devCb->index);
        goto free_event_cb;
    }
