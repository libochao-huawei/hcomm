...
int rs_ub_get_jetty_context(struct rs_ub_dev_cb *dev_cb, unsigned int id, char context[], unsigned int *len)
{
    ...
    ret = rs_urma_get_jetty_opt(jetty_cb->jetty, URMA_JFS_URER_CTX, context, JETTY_CONTEXT_MAX_LEN);
    CHK_PRT_RETURN(ret != 0, hccp_err("rs_urma_get_jetty_opt failed, ret:%d, jetty_id:%u", ret, id), ret);
