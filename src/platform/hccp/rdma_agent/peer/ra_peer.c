...
STATIC void RaPeerLoopbackSingleQpDestroy(struct RaQpHandle *qpHandle)
{
    ...
    (void)RaPeerDestroyCompChannel(loopbackInfo->compChannel);
    ...
    struct ibv_qp **qp)
{
    ...
    ret = RaPeerCreateCompChannel(rdmaHandle, &loopbackInfo->compChannel);
    if (ret != 0) {
    ...
    cqAttr.sendChannel = (struct ibv_comp_channel*)loopbackInfo->compChannel;
    cqAttr.recvChannel = (struct ibv_comp_channel*)loopbackInfo->compChannel;
...
cq_create_err:
    (void)RsDestroyCompChannel(loopbackInfo->compChannel);
channel_create_err:
    free(loopbackInfo);
    loopbackInfo = NULL;
    return ret;
