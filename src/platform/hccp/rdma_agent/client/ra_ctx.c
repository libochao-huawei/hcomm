...
HCCP_ATTRI_VISI_DEF int RaCtxGetJettyContext(void *qpHandle, uint8_t context[], unsigned int *len)
{
    ...
    CHK_PRT_RETURN(*len < CONTEXT_MAX_LEN, hccp_err("[get][jettyContext]len:%u must less than %d", *len,
    ...
    hccp_run_info("Input parameters: phyId:%u, devIndex:0x%x len:%u jettyId:%u",
        qpHandleTmp->phyId, qpHandleTmp->devIndex, *len, qpHandleTmp->qpInfo.ub.id);
