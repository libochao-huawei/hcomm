...
HCCP_ATTRI_VISI_DEF int RaGetDevEidInfoList(struct RaInfo info, struct DevEidInfo infoList[],
    unsigned int *num)
{
    int ret;
...
HCCP_ATTRI_VISI_DEF int RaCtxGetAsyncEvents(void *ctxHandle, struct AsyncEvent events[], unsigned int *num)
{
    ...
    hccp_run_info("[get][async_events]phy_id(%u), devIndex(0x%x)", ctxHandleTmp->attr.phyId,
        ctxHandleTmp->devIndex);
    ret = ctxHandleTmp->ctxOps->raCtxGetAsyncEvents(ctxHandleTmp, events, num);
    CHK_PRT_RETURN(ret != 0, hccp_err("[get][async_events]ra_ctx_get_async_events failed, ret(%d) phyId(%u) devIndex"
        "(0x%x)", ret, ctxHandleTmp->attr.phyId, ctxHandleTmp->devIndex), ConverReturnCode(RDMA_OP, ret));
...
HCCP_ATTRI_VISI_DEF int RaCtxGetAuxInfo(void *ctxHandle, struct AuxInfoIn *in, struct AuxInfoOut *out)
{
    struct RaCtxHandle *ctxHandleTmp = NULL;
    int ret = 0;
