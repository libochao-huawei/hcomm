...
HCCP_ATTRI_VISI_DEF int RaCtxGetJettyContext(void *qpHandle, char context[], unsigned int *len)
{
    ...
    CHK_PRT_RETURN(*len != JETTY_CONTEXT_MAX_LEN, hccp_err("[get][jettyContext]len:%u must equal to %d", *len,
        JETTY_CONTEXT_MAX_LEN), ConverReturnCode(RDMA_OP, -EINVAL));
    ...
    hccp_run_info("Input parameters: phyId:%u, devIndex:0x%x len:%u",
