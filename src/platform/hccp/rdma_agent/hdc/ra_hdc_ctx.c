...
int RaHdcCtxGetAsyncEvents(struct RaCtxHandle *ctxHandle, struct AsyncEvent events[], unsigned int *num)
{
    ...
    CHK_PRT_RETURN(ret, hccp_err("[get][async_events]hdc message process failed ret[%d], phyId[%u] devIndex[0x%x]",
        ret, phyId, ctxHandle->devIndex), ret);

    if (opData.rxData.num > expectedNum) {
        hccp_err("[get][async_events]rx_data.num(%u) > expected_num(%u), phyId(%u) devIndex(0x%x)",
