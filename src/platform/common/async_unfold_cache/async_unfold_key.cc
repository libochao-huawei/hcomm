...
namespace hccl {
    ...
    HcclResult AsyncUnfoldKey::Verify(const uint64_t curOpBaseOpIdx, const HcclCMDType curOpType, const HcclDataType curDataType)
    {
        if (opBaseOpIdx == curOpBaseOpIdx) {
            CHK_PRT_RET((opType != curOpType) || (dataType != curDataType),
                HCCL_ERROR("[AsyncUnfoldKey][Verify] two keys with same opBaseOpIdx[%llu] have inconsistent fields: "
                    "opType[%u] curOpType[%u]; dataType[%u] curDataType[%u]",
                    opBaseOpIdx, opType, curOpType, dataType, curDataType),
                HCCL_E_INTERNAL);
        }
        return HCCL_SUCCESS;
    }
