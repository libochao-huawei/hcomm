...
namespace hccl {
    ...
    constexpr size_t ASYNC_UNFOLD_CACHE_CAPACITY = 1; // Cache entry数目的上限 (暂时只异步展开1个后续算子)
    ...
    bool AsyncUnfoldCache::IsCacheFull() const {
        const size_t curSize = asyncUnfoldCacheMap_.size();
        if (curSize >= ASYNC_UNFOLD_CACHE_CAPACITY) {
            HCCL_INFO("[AsyncUnfoldCache][IsCacheFull] asyncUnfoldCacheMap_.size[%u] >= capacity[%u]",
                asyncUnfoldCacheMap_.size(), ASYNC_UNFOLD_CACHE_CAPACITY);
            return true;
        }
        return false;
    }
