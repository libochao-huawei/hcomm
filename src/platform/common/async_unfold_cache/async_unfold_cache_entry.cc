...
namespace hccl {
    ...
    HcclResult AsyncUnfoldCacheEntry::MemcpyAsyncUnfoldSqeArray(const size_t arrayIdx, const size_t sqeCount,
        const uint8_t *sqeArray, const uint8_t *sqeTypeArray, const AicpuDfxInfo *sqeDfxInfoArray,
        const bool profL1Enable, const uint64_t *profTimestampArray) {
        // Copy sqeArray[0:sqeCount) -> asyncUnfoldSqeArrays_[arrayIdx][0:sqeCount)
        ...
        CHK_PRT_RET(sqeCount != totalSqeCount,
            HCCL_ERROR("[AsyncUnfoldCacheEntry][MemcpyAsyncUnfoldSqeArray] sqeCount[%u] != totalSqeCount[%u]",
                sqeCount, totalSqeCount),
            HCCL_E_INTERNAL);
        ...
        HCCL_INFO("[AsyncUnfoldCacheEntry][MemcpyAsyncUnfoldSqeArray] memcpy %uth sqe array[%u:%u]",
            arrayIdx, 0, sqeCount - 1);

        // Copy SQE content
        const size_t sqeBytes = sqeCount * HCCL_SQE_SIZE;
        uint8_t *dstSqeArray = asyncUnfoldSqeArrays_[arrayIdx];
        CHK_PTR_NULL(dstSqeArray);
        CHK_SAFETY_FUNC_RET(memcpy_s(dstSqeArray, sqeBytes, sqeArray, sqeBytes));

        // Copy SQE type
        const size_t sqeTypeBytes = sqeCount * sizeof(uint8_t);
        uint8_t *dstSqeTypeArray = asyncUnfoldSqeTypeArrays_[arrayIdx];
        CHK_PTR_NULL(dstSqeTypeArray);
        CHK_SAFETY_FUNC_RET(memcpy_s(dstSqeTypeArray, sqeTypeBytes, sqeTypeArray, sqeTypeBytes));

        // Copy SQE DFX info
        const size_t sqeDfxInfoBytes = sqeCount * sizeof(AicpuDfxInfo);
        AicpuDfxInfo *dstSqeDfxInfoArray = asyncUnfoldSqeDfxInfoArrays_[arrayIdx];
        CHK_PTR_NULL(dstSqeDfxInfoArray);
        CHK_SAFETY_FUNC_RET(memcpy_s(dstSqeDfxInfoArray, sqeDfxInfoBytes, sqeDfxInfoArray, sqeDfxInfoBytes));

        // Copy profTimestamp
        if (profL1Enable) {
            // 检验入参
            CHK_PTR_NULL(profTimestampArray);
            ...
            CHK_SAFETY_FUNC_RET(memcpy_s(dstProfTimestampArray, profTimestampBytes, profTimestampArray, profTimestampBytes));
        }
