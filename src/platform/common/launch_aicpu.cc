    ...
    u32 tilingDataSize, aclrtFuncHandle& funcHandle, aclrtArgsHandle& argsHandle)
{
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[TaskCommKernelArgsPrepare]param is invalid,tilingDataPtr[%p], tilingDataSize[%llu], kernelName:%s",
    ...
    CHK_PRT_RET(
        aclRet != ACL_SUCCESS,
        HCCL_ERROR("[aclrtKernelArgsInit]errNo[0x%016llx] args init failed, kernelName:%s", aclRet, kernelName.c_str()),
        HCCL_E_RUNTIME);
    ...
    bool isInitTask, u16 timeOut, void* tilingDataPtr, u32 tilingDataSize)
{
    ...
    CHK_PRT_RET(
        (addr == nullptr || size == 0),
        HCCL_ERROR(
            "[AicpuAclKernelLaunch]param is invalid,contextAddr[%p], size[%llu], kernelName:%s", addr, size,
            kernelName.c_str()),
        HCCL_E_PARA);
    ...
    if (isInitTask) {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[InitKernelArgsPrepare]errNo[0x%016llx]init args prepare failed, kernelName:%s", ret,
                kernelName.c_str()),
            HCCL_E_RUNTIME);
    } else {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[TaskCommKernelArgsPrepare]errNo[0x%016llx]taskCOmm args prepare failed, kernelName:%s", ret,
                kernelName.c_str()),
            HCCL_E_RUNTIME);
    }
