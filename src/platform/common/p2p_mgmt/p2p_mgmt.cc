...
HcclResult P2PMgmt::EnableP2P(std::vector<uint32_t> remoteDevices)
{
    for (u32 i = 0; i < remoteDevices.size(); i++){
        HCCL_INFO("TEST6 remoteDevices[%u] = %u, remoteDevices.size(%u)",
    ...
    HCCL_INFO("TEST13 initFlag_ %s", initFlag_ ? "true" : "false");
    if (initFlag_) {
        for (u32 i = 0; i < remoteDevices.size(); i++){
            HCCL_INFO("TEST7 remoteDevices[%u] = %u, remoteDevices.size(%u)",
                i, remoteDevices[i], static_cast<u32>(remoteDevices.size()));
        }
        isStandardCardFor910B_ = IsStandardCardFor910B(remoteDevices);
        for (u32 i = 0; i < remoteDevices.size(); i++){
            HCCL_INFO("TEST8 remoteDevices[%u] = %u, remoteDevices.size(%u)",
                i, remoteDevices[i], static_cast<u32>(remoteDevices.size()));
        }
