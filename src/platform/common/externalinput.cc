...
HcclResult ParseIntraLinkType()
{
    ...
    if (intraPcieEnv == "EmptyString" && intraRoceEnv != "EmptyString") {
        if (intraRoce == 0) { // roce环境变量值为0，报错
            HCCL_ERROR(
                "[Parse][IntraLinkType]only set HCCL_INTRA_ROCE_ENABLE, and the val is zero, pls set "
    ...
    if (intraPcieEnv != "EmptyString" && intraRoceEnv == "EmptyString") {
        if (intraPcie == 0) { // pcie环境变量值为0，报错
            HCCL_ERROR(
                "[Parse][IntraLinkType]only set HCCL_INTRA_PCIE_ENABLE, and the val is zero, pls set "
...
HcclResult ParseHcclWhitelistFilePath()
{
    ...
    if (filePath == "EmptyString") {
        ...
        HCCL_RUN_INFO(
            "[HCCL_ENV][Parse][HcclWhitelistFilePath]environmental variable HCCL_WHITELIST_DISABLE is [0],"
...
HcclResult ParseRetryEnable()
{
    ...
    if (hcclRetryEnable == "EmptyString") {
        HCCL_RUN_INFO(
            "[HCCL_ENV][ParseRetryEnable] HCCL_OP_RETRY_ENABLE is not set. The retryEnable of all levels is set to "
            "false.");
        return HCCL_SUCCESS;
    }
    // 去除空格
