...
HcclResult NetworkManager::TsdProcessOpen(bool hasBackup)
{
    ...
    if (halAPIVersion < BACKUP_DEVICE_LOG_DEV_VERSION) {
        HCCL_WARNING("[%s]this package does not support obtaining backUp HCCP Log in PLOG. halAPIVersion[%d]", __func__,
            halAPIVersion);
    }
...
HcclResult NetworkManager::GetConfigAndRaInit(struct RaInitConfig &config, bool isHdcV2, NICDeployment nicDeploy)
{
    ...
    RPT_CALL_ERR(ret != HCCL_SUCCESS, "ra init failed,return[%d] devicePhyId_[%u], nicPosition[%u]", ret, devicePhyId_,
        static_cast<u32>(nicDeploy));
...
HcclResult NetworkManager::GetTsdOpen(NICDeployment nicDeploy, bool hasBackup, bool &supportMultiProcHCCP)
{
    if (nicDeploy == NICDeployment::NIC_DEPLOYMENT_DEVICE && !Is310PDevice()) { // DC场景不需要拉起tsd
        ...
        if (supportMultiProcHCCP || hasBackup) {
            ...
            CHK_RET(hrtOpenTsd());
            HCCL_INFO("[%s]NetworkManager open tsd success, devicePhyId[%u], deviceLogicId_[%d]", __func__,
                devicePhyId_, deviceLogicId_);
        }
    }
    return HCCL_SUCCESS;
    ...
    NICDeployment nicDeploy, bool enableWhitelistFlag, u32 devicePhyId, bool isHostUseDevNic, bool hasBackup)
{
    ...
    HCCL_INFO("[%s]config.phyId[%u], config.nicPosition[%u], config.hdcType[%d], config.enableHdcAsync[%d]", __func__,
...
HcclResult NetworkManager::PrepareDeInit(s32 &ref, NICDeployment nicDeploy)
{
    ...
    if (ref > 0) {
        HCCL_INFO("NetworkManager: PrepareDeInit nic success, nicPosition[%u] ref[%u], skip.",
            static_cast<u32>(nicDeploy), ref);
        return HCCL_SUCCESS;
    } else if (ref < 0) {
        HCCL_ERROR(
    ...
    struct RaInitConfig &config, NICDeployment nicDeploy, bool &isMultiProc, bool hasBackup)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[NetworkManager][DeInit]ra deinit failed. para: nicdeploy[%u], phyId[%u], ret[%u]",
            config.nicPosition, config.phyId, ret),
        ret);
    if (IsGeneralServer()) {
        isRaDeInit_ = true;
    }
...
HcclResult NetworkManager::DeInit(NICDeployment nicDeploy, bool resetDeviceFlag, bool hasBackup)
{
    ...
    if (isMultiProc) {
        ...
        HCCL_INFO("[%s]finish CloseHccpProcess, phyId[%u], hdcType[%d], nicDeployment[%d], hasBackup[%d]", __func__,
...
HcclResult NetworkManager::DeInitV2(NICDeployment nicDeploy, bool isBackup, bool resetDeviceFlag)
{
    ...
    if (isMultiProc) {
        CHK_RET(CloseHccpProcess());
        HCCL_INFO("[%s]finish CloseHccpProcess, phyId[%u], hdcType[%d], nicDeployment[%d], isBackup[%d]", __func__,
...
HcclResult NetworkManager::StartVnic(HcclIpAddress localIp, u32 &port)
{
    ...
    HCCL_INFO("NetworkManager devicePhyId_[%u] ip[%s] port[%u] start vnic OK. refCount[%d]", devicePhyId_,
...
{
    CHK_PRT_RET(!deviceNicInitRef_.Count(),
...
HcclResult NetworkManager::StopVnicSocketHandle(const HcclIpAddress &localIp)
{
    ...
    HCCL_INFO("[NetworkManager][StopVnicSocketHandle] devid[%u] ip[%s] stop vnic socket success", devicePhyId_,
        localIp.GetReadableAddress());
    return HCCL_SUCCESS;
}
...
HcclResult NetworkManager::CreateNicSocketHandle(const HcclIpAddress &ipAddr)
{
    ...
    if (sock.listenedPort.size() == 0) {
        if (sock.nicSocketHandle == nullptr) {
            HcclResult ret = InitDeviceSocket(devicePhyId_, ipAddr, sock.nicSocketHandle);
            CHK_PRT_RET(ret != HCCL_SUCCESS,
                HCCL_ERROR("[NetworkManager][CreateNicSocketHandle]errNo[0x%016llx] ra nic init socket failed, "
                           "devid[%u], return[%d]",
                    HCCL_ERROR_CODE(HCCL_E_TCP_CONNECT), devicePhyId_, ret),
                ret);
        }
    } else {
        HCCL_INFO("[NetworkManager][CreateNicSocketHandle] socket has inited, ipAddr[%s], skip.",
...
HcclResult NetworkManager::CreateRdmaHandle(const HcclIpAddress &ipAddr, bool isBackup, NetworkMode netMode,
    NotifyTypeT notifyType, HcclNetDevDeployment netDevDeployment)
{
    ...
    CHK_PRT_RET(!(deviceNicInitRef_.Count() || hostNicInitRef_.Count()),
    ...
    switch (netDevDeployment) {
        case HcclNetDevDeployment::HCCL_NETDEV_DEPLOYMENT_DEVICE: {
            ...
            if (sock.nicRdmaHandle == nullptr) {
                ...
                CHK_PRT_RET(ret != HCCL_SUCCESS,
                    HCCL_ERROR("[NetworkManager][CreateRdmaHandle]errNo[0x%016llx] ra nic init rdma failed, devid[%u], "
                               "return[%d]",
                        HCCL_ERROR_CODE(HCCL_E_NETWORK), devicePhyId_, ret),
                    HCCL_E_NETWORK);
            } else {
        ...
        default: {
            HCCL_ERROR("[NetworkManager][CreateRdmaHandle]this Deployment [%u] is not supported, please check the "
                       "configuration.",
                netDevDeployment);
...
HcclResult NetworkManager::StopRdmaHandle(const HcclIpAddress &ipAddr, HcclNetDevDeployment netDevDeployment)
{
    ...
    switch (netDevDeployment) {
        ...
        default: {
            HCCL_ERROR("[NetworkManager][StopRdmaHandle]this Deployment [%u] is not supported, please check the "
                       "configuration.",
                netDevDeployment);
            return HCCL_E_NOT_SUPPORT;
        }
    ...
    HCCL_INFO("[NetworkManager] [StopRdmaHandle] devicePhyId_[%u] StopRdmaHandle success ip [%s]", devicePhyId_,
        ipAddr.GetReadableAddress());
...
HcclResult NetworkManager::StopNicSocketHandle(const HcclIpAddress &ipAddr)
{
    ...
    for (auto &port : ipSock.listenedPort) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[NetworkManager][StopNicSocketHandle]errNo[0x%016llx] stop nic listen failed,devid[%u], "
                       "ip[%s], port[%u], return[%d]",
                HCCL_ERROR_CODE(HCCL_E_TCP_CONNECT), devicePhyId_, ipAddr.GetReadableAddress(), port, ret),
            HCCL_E_INTERNAL);
        ipSock.listenedPort.erase(port);
        IPPortListenRefMapDevice_[ipAddr][port].Clear(); // port计数更新
        HCCL_WARNING("[NetworkManager][StopNicSocketHandle] ip [%s] stop listen port [%u] refcount is [%d]",
    ...
    HcclIpAddress ipAddrBackup)
{
    ...
    if (isRaInitRepeated_) {
        ...
        if (!ipAddrBackup.IsInvalid()) {
            ...
            HCCL_DEBUG("[%s]backup rdev info: ipAddr[%s], ipAddrBackup[%s]", __func__, ipAddr.GetReadableIP(),
    ...
    u32 devId, const HcclIpAddress &ipAddr, u32 port, bool isHostUseDevNic, bool remoteIsHdc, bool isBoardVersion)
{
    ...
    if (isHostUseDevNic_) {
        ...
        isTsdProcessOpen_ = true;
        CHK_RET(hrtOpenNetService(&openArgs));
        HCCL_INFO("NetworkManager open tsd success, devicePhyId[%u], deviceLogicId_[%d], subPid[%lld]", devicePhyId_,
...
HcclResult NetworkManager::StopHostSocketHandle(const HcclIpAddress &ipAddr)
{
    ...
    if (ipIt != IPPortListenRefMapHost_.end()) {
        for (auto &portIt : ipIt->second) {
            ...
            if (IPPortListenRefMapHost_[ipAddr][port].Count() > 0) {
                ...
                HCCL_WARNING("[NetworkManager][StopHostSocketHandle] ip [%s] stop listen port [%u]",
                    ipAddr.GetReadableAddress(), port);
                raResourceInfo_.hostNetSocketMap[ipAddr].listenedPort.erase(port);
    ...
    CHK_PRT_RET((raResourceInfo_.hostNetSocketMap[ipAddr].listenedPort.size() != 0),
        HCCL_ERROR("[NetworkManager][StopHostSocketHandle]IPPortListenRefMapHost_[%s] is unequal to "
...
HcclResult NetworkManager::StartListenSocket(const SocketHandle socketHandle, u32 &port) const
{
    ...
    if (isAutoPort) {
        ...
        CHK_PRT_RET(port == AUTO_LISTEN_PORT, HCCL_ERROR("start listen on a port selected by os automatically failed"),
            HCCL_E_NOT_SUPPORT);
...
HcclResult NetworkManager::OccupyIp(const HcclIpAddress &ipAddr, std::map<hccl::HcclIpAddress, IpSocket> &socketMap)
{
    ...
    if (sockInfo == socketMap.end()) {
        ...
        HCCL_INFO("[NetworkManager][OccupyIp] device[%u] ip[%s] has already occupied.", devicePhyId_,
            ipAddr.GetReadableAddress());
    }
    return HCCL_SUCCESS;
}
