...
HcclResult NetworkManager::TsdProcessOpen(bool hasBackup, bool useResvMem, u32 poolId)
{
    ...
    if (!hasBackup || halAPIVersion < BACKUP_DEVICE_LOG_DEV_VERSION) {
        std::string extPams[TSD_OPEN_EXT_PARA_NUM-1] =
            {std::string("--hdcType=" + std::to_string(HDC_SERVICE_TYPE_RDMA_V2)),
             std::string("--ResvMemPoolId=" +std::to_string(poolId))};
        rtProcExtParam extParams[TSD_OPEN_EXT_PARA_NUM-1] {};
        for (u32 i = 0; i < TSD_OPEN_EXT_PARA_NUM-1; i++) {
        ...
        openArgs.extParamCnt = useResvMem ? TSD_OPEN_EXT_PARA_NUM-1 : TSD_OPEN_EXT_PARA_NUM-2;
        // 根据pid粒度拉起hccp的rs进程
        ...
        HCCL_INFO("[%s]hrtOpenNetService success, subPid[%u], "
            "devicePhyId_[%u], deviceLogicId_[%d], deviceBackUpPhyId[%u], hasBackup[%u], useResvMem[%u], poolId[%u]",
            __func__, subPid_, devicePhyId_, deviceLogicId_, deviceBackUpPhyId, hasBackup, useResvMem, poolId);
    }
