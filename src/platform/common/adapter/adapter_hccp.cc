...
HcclResult hrtRaGetSecRandom(struct RaInfo *info, unsigned int* token)
{
    ...
    if (ret != 0) {
        HCCL_ERROR("[HrtRaGetSecRandom] ra_get_sec_random failed, call interface, ret[%d]",ret);
...
HcclResult hrtRaGetDevEidInfoNum(struct RaInfo *info, unsigned int* num)
{
    ...
    if (ret != 0) {
        HCCL_ERROR("[HrtRaGetSecRandom] ra_get_dev_eid_info_num failed, call interface, ret[%d]",ret);
        return HCCL_E_INTERNAL;
    }
    return HCCL_SUCCESS;
}
...
HcclResult hrtRaGetDevEidInfoList(struct RaInfo *info, struct dev_eid_info *eid_info, unsigned int* num)
{
    ...
    if (ret != 0) {
        HCCL_ERROR("[HrtRaGetSecRandom] ra_get_dev_eid_info_list failed, call interface, ret[%d]",ret);
        return HCCL_E_INTERNAL;
    }
    return HCCL_SUCCESS;
}