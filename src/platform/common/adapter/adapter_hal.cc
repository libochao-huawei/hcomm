...
HcclResult hrtHalSensorNodeRegister(uint32_t devId, uint64_t *handle)
{
    ...
    return HCCL_SUCCESS;
#else
    HCCL_ERROR("[halSensorNodeRegister]Does does not support this interface.");
...
HcclResult hrtHalSensorNodeUnregister(uint32_t devId, uint64_t handle)
{
    ...
    return HCCL_SUCCESS;
#else
    HCCL_ERROR("[halSensorNodeUnregister]Does does not support this interface.");
    return HCCL_E_NOT_SUPPORT;
#endif
}
...
HcclResult hrtHalSensorNodeUpdateState(uint32_t devId, uint64_t handle, int val, HcclGeneralEventType assertion)
{
    ...
    return HCCL_SUCCESS;
#else
    HCCL_ERROR("[halSensorNodeUpdateState]Does does not support this interface.");
    return HCCL_E_NOT_SUPPORT;
#endif
}
