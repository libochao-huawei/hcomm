...
bool EletwiseV2::DoBlockTiling()
{
    ...
    numBlocks_ = static_cast<int64_t>(std::ceil(outputShape_[0] * 1.0 / blockFactor_));
    return true;
}
...
{
    runInfo.numBlocks = static_cast<uint32_t>(numBlocks_);
    if (onlyConstTiling_) {
                  ...
                  int64_t& key, int64_t& numBlocks)
{
    ...
    if (ret) {
        try {
            ...
            const auto& constNumBlocks = opInfo["_const_block_dims"];
            CHECK_GT(constNumBlocks.size(), key_index,
                     "opType[%s] const_block_dims index out of range", opType.c_str());
            numBlocks = constNumBlocks[key_index].get<int64_t>();
...
bool WriteConstTiling(const std::string& opType, OpRunInfo& runInfo, const int64_t& key, const int64_t& numBlocks)
{
    runInfo.numBlocks = static_cast<uint32_t>(numBlocks);
    ...
    OpRunInfo& runInfo)
{
    ...
    if (is_const) {
        ...
        ret = ret && CalcConstKey(opType, opParas, opInfo, isSupportBroadcast, key_, numBlocks_);
        ret = ret && WriteConstTiling(opType, runInfo, key_, numBlocks_);
