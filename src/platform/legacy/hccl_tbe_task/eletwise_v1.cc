...
bool EletwiseV1::DoBlockTiling()
{
    ...
    if (len == 1) {
        ...
        numBlocks_ = static_cast<int32_t>(std::ceil(outputShape_[0] * 1.0 / blockFactor));
        ...
        for (size_t i = 0; i < outputShape_.size(); i++) {
            if (outputShape_[i] > curCore) {
                ...
                numBlocks_ *= static_cast<int32_t>(std::ceil(outputShape_[i] * 1.0 / blockFactor));
                varNames_[blockFactorKey + i] = blockFactor;
                outputShape_[i] = blockFactor;
                break;
            } else {
                curCore /= outputShape_[i];
                numBlocks_ *= outputShape_[i];
                outputShape_[i] = 1;
            }
        }
    }
