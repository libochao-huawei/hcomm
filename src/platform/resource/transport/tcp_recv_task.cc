...
TcpRecvTask::~TcpRecvTask() { (void)Deinit(); }
...
HcclResult TcpRecvTask::SetRecvTask(const FdHandle fdHandle, HcclRequestInfo* request)
{
    ...
    if (request->transportRequest.transData.count > 0) {
        ...
        recvRecord.size
            = request->transportRequest.transData.count * SIZE_TABLE[request->transportRequest.transData.dataType];
        ...
        // 接收的信封中count为0，则不需要接收数据，继续接收信封即可。因此需要将epoll修改为oneshot模式，并提交recv
        // complete事件
    ...
    HCCL_DEBUG(
        "[RecvTask][SetRecvTask] recv buffer[%p]", reinterpret_cast<void*>(request->transportRequest.transData.srcBuf));
    return HCCL_SUCCESS;
}
...
HcclResult TcpRecvTask::RecvData(const FdHandle fdHandle)
{
    ...
    if (!envelopMap_[fdHandle].flag) {
        ...
        HCCL_DEBUG(
            "envelop rank[%u] tag[%d] count[%d] size[%u]", envelopeSummary.envelope.epParam.src.rank,
            envelopeSummary.envelope.epParam.src.tag, envelopeSummary.envelope.transData.count,
            sizeof(envelopeSummary.envelope));
        ...
        if ((rtRet == 0) && (recvSize > 0)) {
            CHK_PRT_RET(
                ...
                HCCL_ERROR(
                    "[RecvTask][RecvBody]errNo[0x%016llx] socket receive rtSize[%llu Byte] bigger size[%zu]",
                    HCCL_ERROR_CODE(HCCL_E_TCP_TRANSFER), recvSize, recvData.size),
                HCCL_E_TCP_TRANSFER);
