    ...
    struct Transport::Buffer &remoteBuf, struct Transport::Buffer &localBuf, Stream &stream)
{
    ...
    if (isLocalHostAddr && isRemoteHostAddr) {
        HostMem dstHostMem(const_cast<void *>(newRemoteBuf.addr), newRemoteBuf.size);
        HostMem srcHostMem(const_cast<void *>(newLocalBuf.addr), newLocalBuf.size);
    ...
    struct Transport::Buffer &localBuf, struct Transport::Buffer &remoteBuf, Stream &stream)
{
    ...
    if (isLocalHostAddr && isRemoteHostAddr) {
        HostMem dstHostMem(const_cast<void *>(newLocalBuf.addr), newLocalBuf.size);
        HostMem srcHostMem(const_cast<void *>(newRemoteBuf.addr), newRemoteBuf.size);
        CHK_RET(reinterpret_cast<DispatcherPub*>(dispatcher_)->MemcpyAsync(dstHostMem, srcHostMem, stream));
    } else if (isLocalHostAddr) {
