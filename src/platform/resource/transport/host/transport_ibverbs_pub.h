...
inline void CopyAiWQInfo(struct HcclAiRMAWQ &dest, const struct AiDataPlaneWq &source, DBMode dbMode, u32 sl)
{
    ...
    dest.sl = sl;
    HCCL_INFO("CopyAiWQInfo: wqn[%u] bufAddr[%p] wqeSize[%u] depth[%u] headAddr[%p] tailAddr[%p] dbMode[%u]", dest.wqn,
...
inline void CopyAiCQInfo(struct HcclAiRMACQ &dest, const AiDataPlaneCq &source, DBMode dbMode)
{
    ...
    HCCL_INFO("CopyAiCQInfo: cqn[%u] bufAddr[%p] cqeSize[%u] depth[%u] headAddr[%p] tailAddr[%p] dbMode[%u]", dest.cqn,
...
public:
    explicit TransportIbverbs(DispatcherPub *dispatcher, const std::unique_ptr<NotifyPool> &notifyPool,
...
protected:
    ...
    HcclResult AddWqeList(void *dstMemPtr, const void *srcMemPtr, u64 srcMemSize, WqeType wqeType, WrAuxInfo &aux,
        std::vector<WqeInfo> &wqeInfoVec);
    virtual HcclResult GetMemInfo(UserMemType memType, void **dstMemPtr, u64 *dstMemSize);
    virtual HcclResult TxPayLoad(UserMemType dstMemType, u64 dstOffset, const void *src, u64 len, WqeType wqeType,
    ...
    virtual HcclResult RdmaSendAsync(std::vector<WqeInfo> &wqeInfoVec, Stream &stream, bool useOneDoorbell = false,
    ...
    HcclResult ConstructPayLoadWqe(void *dstMemPtr, const void *src, u64 len, WqeType wqeType, WrAuxInfo &aux,
        std::vector<WqeInfo> &wqeInfoVec, u32 txSendDataTimes);
