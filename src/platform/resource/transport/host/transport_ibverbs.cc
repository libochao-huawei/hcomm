...
HcclResult TransportIbverbs::RxAsync(UserMemType srcMemType, u64 srcOffset, void* dst, u64 len, Stream& stream)
{
    ...
    if (UseMultiQp() && actualMultiQpNum != 1 && actualMultiQpNum <= qpsPerConnection_ && len != 0) {
        ...
        CHK_RET(
            LocalIpcNotify::Wait(
                stream, dispatcher_, dataNotify_, INVALID_VALUE_STAGE, NOTIFY_INVALID_WAIT_TIME,
                machinePara_.localUserrank, machinePara_.remoteWorldRank));
    }
    return HCCL_SUCCESS;
}
...
HcclResult TransportIbverbs::TxDone(Stream& stream)
{
    ...
    CHK_RET(
        LocalIpcNotify::Wait(
            stream, dispatcher_, dataAckNotify_, INVALID_VALUE_STAGE, NOTIFY_INVALID_WAIT_TIME,
            machinePara_.localUserrank, machinePara_.remoteWorldRank));
    return HCCL_SUCCESS;
}
...
HcclResult TransportIbverbs::ExchangeCapabilityHybrid()
{
    ...
    if (!RoCECapability::CheckMagic(reinterpret_cast<uint8_t*>(&recvCap), sizeof(recvCap))) {
        HCCL_WARNING(
            "[Hybrid][TransportIbverbs] Magic mismatch, peer may be old version. "
