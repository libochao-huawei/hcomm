    ...
    Stream &stream)
{
    ...
    CHK_PRT_RET(remoteBufs.size() != localBufs.size(),
        HCCL_ERROR("[BatchWriteAsync]remoteBufs size[%u] != localBufs size[%u]",
    ...
    for (size_t i = 0; i < remoteBufs.size(); i++) {
        HCCL_DEBUG("[BatchWriteAsync] index[%u] localAddr[%p] remoteAddr[%p] len[%llu]",
        ...
        if (localBufs[i].addr != nullptr) {
            ...
            CHK_RET(ConstructPayLoadWqe(resolve.transRemoteAddr, resolve.dstKey,
                resolve.transLocalAddr, resolve.srcKey,
                length, WqeType::WQE_TYPE_DATA, aux, wrInfoVec, txSendDataTimes));
        }
    }
    ...
    Stream &stream)
{
    ...
    CHK_PRT_RET(localBufs.size() != remoteBufs.size(),
        HCCL_ERROR("[BatchReadAsync]localBufs size[%u] != remoteBufs size[%u]",
            localBufs.size(), remoteBufs.size()), HCCL_E_PARA);
    ...
    for (size_t i = 0; i < localBufs.size(); i++) {
        HCCL_DEBUG("[BatchReadAsync] index[%u] localAddr[%p] remoteAddr[%p] len[%llu]",
            i, localBufs[i].addr, remoteBufs[i].addr, remoteBufs[i].size);
        ...
        if (localBufs[i].addr != nullptr) {
            ...
            CHK_RET(ConstructPayLoadWqe(resolve.transRemoteAddr, resolve.dstKey,
                resolve.transLocalAddr, resolve.srcKey,
                length, WqeType::WQE_TYPE_READ_DATA, aux, wrInfoVec, txSendDataTimes));
        }
    }
