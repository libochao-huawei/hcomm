    ...
    WqeType wqeType, struct WrAuxInfo &aux)
{
    ...
    if (localAddr != nullptr) {
        ...
        u32 dstKey = 0;
        u32 srcKey = 0;
        void *transLocalAddr = const_cast<void *>(localAddr);
        void *transremoteAddr = const_cast<void *>(remoteAddr);
        if (useMemDetailsLookup_) {
            ...
            transLocalAddr = LogicalPtrToDevPtr(*lf.second, localAddr);
        } else {
            u64 dstAddr = static_cast<u64>(reinterpret_cast<uintptr_t>(remoteAddr));
            ...
            u64 srcAddr = static_cast<u64>(reinterpret_cast<uintptr_t>(localAddr));
            if (srcAddr >= localInputMem_.addr && srcAddr < localInputMem_.addr + localInputMem_.size) {
                srcKey = localInputMem_.key;
            } else if (srcAddr >= localOutputMem_.addr && srcAddr <= localOutputMem_.addr + localOutputMem_.size) {
                srcKey = localOutputMem_.key;
        ...
        CHK_RET(ConstructPayLoadWqe(transremoteAddr, dstKey,
            transLocalAddr, srcKey,
            length,
            wqeType,
            aux,
            wrInfoVec,
