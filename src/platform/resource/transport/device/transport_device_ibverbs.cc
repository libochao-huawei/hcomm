    ...
    HcommBatchTransferDesc *transferDescs, uint32_t descNum, Stream &stream)
{
    ...
    for (uint32_t i = 0; i < descNum; i++) {
        ...
        if (transferDescs[i].transType == HCOMM_TRANSFER_TYPE_WRITE) {
            ...
            remoteAddr = transferDescs[i].write.dst;
            localAddr = transferDescs[i].write.src;
            length = transferDescs[i].write.len;
            ...
            remoteAddr = transferDescs[i].read.src;
            localAddr = transferDescs[i].read.dst;
            length = transferDescs[i].read.len;
            wqeType = WqeType::WQE_TYPE_READ_DATA;
        } else {
            HCCL_ERROR("[BatchTransferImpl] Unsupported transType[%d] in desc[%u].",
