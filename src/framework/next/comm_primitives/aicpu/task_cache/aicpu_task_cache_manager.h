...
public:
    ...
    inline static HcclResult AddWqeArray(UbConnLite* ubConnLitePtr, const std::vector<WqeTask>& wqeTasks,
        const uint32_t dbSqeIdx)
    {
        CHK_PTR_NULL(ubConnLitePtr);

        if (isCacheMiss && cacheEntryPtr != nullptr) { // cache miss且cache容量未满
            CHK_RET(cacheEntryPtr->AddWqeArray(ubConnLitePtr, wqeTasks, dbSqeIdx));
        }
