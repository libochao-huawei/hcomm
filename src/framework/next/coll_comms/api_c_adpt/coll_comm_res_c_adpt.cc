...
constexpr uint32_t RETRY_INTERVAL_MIN = 5u; // retryInterval范围的最小值
constexpr uint32_t RETRY_INTERVAL_MAX = 24u;// retryInterval范围的最大值
constexpr uint32_t RETRY_CNT_MIN = 1u;      // retryCnt范围的最小值
constexpr uint32_t RETRY_CNT_MAX = 7u;      // retryCnt范围的最大值
...
HcclResult ProcessRoceChannelDesc(const HcclChannelDesc &channelDesc, HcclChannelDesc &channelDescFinal, hccl::hcclComm *hcclComm)
{
    if (hcclComm->IsCommunicatorV2()) { // A5
        ...
        CHK_PRT_RET((tc != INVALID_UINT) && (tc > TC_MAX || (tc % MULTIPLE != 0)),
        ...
        CHK_PRT_RET((sl != INVALID_UINT) && (sl > SL_MAX),
        ...
        CHK_PRT_RET((retryInterval != INVALID_UINT) && (retryInterval < RETRY_INTERVAL_MIN || retryInterval > RETRY_INTERVAL_MAX),
