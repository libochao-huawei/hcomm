    ...
    const HcclChannelDesc* channelDescs, uint32_t channelNum, ChannelHandle* channels)
{
    ...
    if (hcclComm->IsCommunicatorV2()) {  // A5
        ...
        if (engine == COMM_ENGINE_CPU) {
            ...
            for (uint32_t idx = 0; idx < channelNum; idx++) {
                ...
                CHK_PRT_RET(ret != HCCL_SUCCESS,
                    HCCL_ERROR("[HcclChannelAcquire] group[%s] Failed to register callback for channel[%u], ret[%d]", commTag.c_str(), idx, ret),
        ...
        if (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) {
            HCCL_INFO("[HcclChannelAcquire] group[%s] ReportChannelAicpuKernel start", commTag.c_str());
            HcclCommDfx* hcclCommDfx = collComm->GetHcclCommDfx();
            CHK_PTR_NULL(hcclCommDfx);
            std::string kernelName = "RunAicpuIndOpChannelInitV2";
            ret = hcclCommDfx->ReportKernel(beginTime, commTag, kernelName, SalGetTid());
            CHK_PRT_RET(ret != HCCL_SUCCESS,
                HCCL_ERROR("[HcclChannelAcquire] group[%s] Failed to report kernel for kernelName[%s], tid[%d], ret[%d]", commTag.c_str(), kernelName.c_str(), SalGetTid(), ret),
                static_cast<HcclResult>(ret));
        }
    } else {
    ...
    HCCL_RUN_INFO("[%s] acquire channel success, group[%s], engine[%d], channelNum[%llu], take time [%lld]us.", 
        __func__, hcclComm->GetIdentifier().c_str(), engine, channelNum, DURATION_US(TIME_NOW() - startut));
