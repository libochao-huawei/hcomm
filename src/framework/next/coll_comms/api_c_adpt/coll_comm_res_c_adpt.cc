...
#include "../../comms/endpoint_pairs/channels/channel_process.h"
#include "../../comms/endpoint_pairs/channels/aicpu/aicpu_ts_roce_channel_v2.h"
#include "../../comms/endpoint_pairs/channels/aiv/aiv_urma_channel.h"
    ...
    uint32_t channelNum, ChannelHandle *channels)
{
    ...
    std::vector<ChannelHandle> deviceChannels(channels, channels + channelNum);
    for (uint32_t idx = 0; idx < channelNum; ++idx) {
        ...
        CHK_RET(BuildAivDeviceChannelEntity(channelDescs[idx], channels[idx], deviceChannels[idx]));
        HCCL_INFO("[%s] convert AIV channel success, idx[%u], protocol[%d], hostHandle[0x%llx], devEntity[0x%llx]",
            __func__, idx, channelDescs[idx].channelProtocol, static_cast<unsigned long long>(channels[idx]),
