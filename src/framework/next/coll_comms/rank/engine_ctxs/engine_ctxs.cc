...
HcclResult EngineCtxs::DestroyCommEngineCtx(const std::string &tag, CommEngine engine)
{
    std::lock_guard<std::mutex> lock(mutex_); 
    // Ctx不存在返回错误
    if (contextMap_.find(tag) == contextMap_.end()) {
        HCCL_WARNING("[%s] not exist a context with tag[%s]", __func__, tag.c_str());
        return HCCL_E_PARA;
    }
    auto& engineCtxMap = contextMap_[tag];
    if (engineCtxMap.find(engine) == engineCtxMap.end()) {
        HCCL_WARNING("[%s] not exist a context with tag[%s], engine[%d]", __func__, tag.c_str(), engine);
        return HCCL_E_PARA;
    }
    // 获取内存信息
