        ...
        const std::string &commTag, std::vector<HcommChannelDesc> &hcommDescs)
{
    ...
    for (uint32_t i = 0; i < channelNum; ++i) {
        ...
        if (localIpAddr < remoteIpAddr) {
            ...
            hcommDescs[i].role = HcommSocketRole::SOCKET_ROLE_SERVER;
            hcommDescs[i].port = listenPort; // HcommChannelDesc中填监听端口号
        } else {
            listenPort = rmtPort;
            hcommDescs[i].role = HcommSocketRole::SOCKET_ROLE_CLIENT;
            hcommDescs[i].port = rmtPort; // HcommChannelDesc中填对端端口号(此时对端端口号也就是监听端口号)
        }
