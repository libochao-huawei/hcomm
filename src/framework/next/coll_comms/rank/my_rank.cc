...
HcclResult MyRank::TryInitCcuInstance()
{
    ...
    if (ccuInitRet == HcclResult::HCCL_E_UNAVAIL) {
        ...
        ccuResContainer_ = nullptr;
        HCCL_RUN_WARNING(
            "[MyRank][%s] ccu resouces are unavailable, "
            "fallback to aicpu, rankId[%u].",
            __func__, rankId_);
        return HcclResult::HCCL_SUCCESS;
    }
    ...
    std::vector<HcommChannelDesc>& hcommDescs)
{
    ...
    for (uint32_t i = 0; i < channelNum; ++i) {
        ...
        HCCL_INFO(
            "[%s][%u/%u] remoteRank[%u] localProtocol[%d] remoteProtocol[%d]", __func__, i + 1, channelNum, remoteRank,
            localEndpointDesc.protocol, remoteEndpointDesc.protocol);
        ...
        auto ret = endpointPair->GetSocket(
            rankId_, remoteRank, socketTag, reuseIdx, listenPort, socket, devicePhyId, remoteDevicePhyId);
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
    ...
    std::vector<HcommChannelDesc>& hcommDescs, ChannelHandle* channelHandles)
{
    ...
    for (uint32_t i = 0; i < channelNum; ++i) {
        ...
        HCCL_INFO(
            "[%s][%u/%u] remoteRank[%u] localProtocol[%d] remoteProtocol[%d] engine[%d]", __func__, i + 1, channelNum,
            remoteRank, localEndpointDesc.protocol, remoteEndpointDesc.protocol, engine);
        ...
        if (ret == HCCL_E_UNAVAIL) {
            ...
            HCCL_RUN_WARNING(
                "[%s] create channel failed, channelIndex[%u], remoteRank[%u], engine[%d], reuseIdx[%u], need clean "
                "new channels",
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[%s] failed to create channel, channelIndex[%u], remoteRank[%u], engine[%d], reuseIndex[%u]", __func__,
                i + 1, remoteRank, engine, reuseIdx),
            ret);
    ...
    if (!isAllSuccess) {
        HCCL_RUN_WARNING(
            "[%s] create channel failed, destroy new channels num[%u], engine[%d]", __func__, newChannels_.size(),
            engine);
    ...
    ChannelHandle* channelHandles)
{
    ...
    HCCL_INFO(
        "[CreateChannels][Enter] engine[%d] commTag[%s] channelNum[%u] rankId[%u]", engine, commTag.c_str(), channelNum,
        rankId_);
    ...
    if (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) {
        ...
        CHK_RET(
            ChannelProcess::ChannelKernelLaunchForComm(
    ...
    for (u32 i = 0; i < memNum; i++) {
        ...
        if (strcmp(memTags[i], "HcclBuffer") == 0) {
            ...
            *size = remoteMemList[i]->size;
            HCCL_INFO(
                "[%s] Found Hccl buffer memNum is %u at index %u: addr=%p, size=%llu", __func__, memNum, i,
