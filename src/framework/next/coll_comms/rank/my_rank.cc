        ...
        const std::string &commTag, std::vector<HcommChannelDesc> &hcommDescs)
{
    ...
    for (uint32_t i = 0; i < channelNum; ++i) {
        ...
        CHK_RET(QueryListenPort(localRank, remoteRank, localEndpointDesc, remoteEndpointDesc, listenPort, hcommDescs[i]));

        Hccl::Socket* socket = nullptr;
        auto ret = endpointPair->GetSocket(rankId_, remoteRank, commTag, socket, listenPort);
