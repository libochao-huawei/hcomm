...
constexpr uint16_t TS_UB_DDRC_FAILED = 1002; // TS类型错误码1002，表示UB DDRC异常
constexpr uint16_t TS_UB_POISON_FAILED = 1003; // TS类型错误码1003，表示UB Poison异常
constexpr uint16_t TS_UB_LINK_FAILED = 1004; // TS类型错误码1004，表示链路异常
...
uint16_t HcclCommTaskExceptionLite::SwitchUBCqeErrCodeToTsErrCode(u32 cqeErrCode) {
    switch (cqeErrCode) {
        case RT_UB_LOCAL_OPERATIOINERR:
            return TS_UB_DDRC_FAILED;
        case RT_UB_REMOTE_OPERATIOINERR:
            return TS_UB_POISON_FAILED;
        case RT_UB_LINK_FAILEDERR:
            return TS_UB_LINK_FAILED;
        default:
