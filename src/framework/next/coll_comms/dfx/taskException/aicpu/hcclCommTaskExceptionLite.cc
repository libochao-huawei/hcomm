...
HcclResult HcclCommTaskExceptionLite::ProcessCqe(CollCommAicpu* aicpuComm, const rtLogicCqReport_t& exceptionInfo)
{
    ...
    // 1. 打印task信息
    HCCL_ERROR(
        "[TaskException][AICPU]base information is %s, %s", curTask->GetIndopBaseInfo().c_str(),
...
HcclResult HcclCommTaskExceptionLite::PrintTaskContextInfo(CollCommAicpu* aicpuComm, u32 sqId, u32 taskId)
{
    ...
    auto func = [taskId](const std::shared_ptr<Hccl::TaskInfo>& task) {
    ...
    for (uint32_t i = 0; i < TASK_CONTEXT_SIZE && *taskIterPtr != *queue->Begin(); ++i, --(*taskIterPtr)) {
        if ((**taskIterPtr)->taskId_ > taskId) {
            HCCL_ERROR(
                "[%s]prev taskId[%u] is bigger than err taskId[%u], stop traversal", __func__, (**taskIterPtr)->taskId_,
                taskId);
            break;
        }
    ...
    HCCL_ERROR(
        "[TaskException][AICPU]context sequence before error task is "
