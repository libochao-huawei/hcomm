...
void PrintCcuUbRegisters(s32 devLogicId, const Hccl::ParaCcu &ccuTaskParam)
{
    ...
    Hccl::RaBatchQueryJettyStatus(jettyHandles, jettyStatusVec, jettyNum);
...
void TaskExceptionHost::Process(rtExceptionInfo_t* exceptionInfo)
{
    ...
    TRY_CATCH_PRINT_ERROR(
        ...
        // } else if(curTask->taskParam_.taskType == Hccl::TaskParamType::TASK_AIV){
...
/*
 @Desc: AIV 算子异常DFX
*/
void TaskExceptionHost::ProcessAivException(rtExceptionInfo_t* exceptionInfo, const TaskInfo& taskInfo){
    HCCL_ERROR("[TaskExceptionHost][%s]Task from HCCL run failed.", __func__);
    
    HCCL_ERROR("[TaskExceptionHost][AIV]Task run failed, para information is "
    ...
    if(aclRet != ACL_SUCCESS){
        HCCL_ERROR("[TaskExceptionHost] [%s] error[%d].", __func__, aclRet);
        return;
    }
    ...
    flagStr << "[TaskExceptionHost][AIV]Task run failed, para information is deviceId["
...
void TaskExceptionHost::PrintAivPreviousTaskException(rtExceptionInfo_t *exceptionInfo)
{
    ...
    for (uint32_t i = 0; i < TASK_CONTEXT_SIZE && *taskItorPtr != *queue->Begin(); --(*taskItorPtr)) {
        ...
        if ((**taskItorPtr)->taskId_ != taskId && (**taskItorPtr)->taskParam_.taskType == Hccl::TaskParamType::TASK_AIV) {
                HCCL_ERROR("[TaskExceptionHost][AIV] "
...
string TaskExceptionHost::GetGroupRankInfo(const TaskInfo& taskInfo)
{
    ...
    const Hccl::CommunicatorImpl* communicator = static_cast<Hccl::CommunicatorImpl*>(taskInfo.dfxOpInfo_->comm_);
...
void TaskExceptionHost::ProcessCcuMC2Exception(rtExceptionInfo_t* exceptionInfo)
{
    ...
    for (uint32_t i = 0; i < ccuExDetailInfo.ccuMissionNum; ++i) {
        ...
        Hccl::ParaCcu serverParam = serverTaskInfo->taskParam_.taskPara.Ccu;
        serverParam.execMissionId = missionInfo.missionId;
        vector<Hccl::CcuErrorInfo> serverErrorInfos {};
        if (Hccl::GetCcuErrorMsg(exceptionInfo->deviceid, status, serverParam, serverErrorInfos) != HcclResult::HCCL_SUCCESS) {
        ...
        vector<Hccl::CcuTaskParam> algoTaskParams = GetMC2AlgTaskParam(*serverTaskInfo);
        for (const auto& algoTaskParam : algoTaskParams) {
            ...
            auto algoTaskInfo = Hccl::MC2GlobalMirrorTasks::GetInstance().GetTaskInfo(
            ...
            Hccl::ParaCcu algoParam = algoTaskInfo->taskParam_.taskPara.Ccu;
            algoParam.execMissionId = missionInfo.missionId;
            vector<Hccl::CcuErrorInfo> algoErrorInfos {};
            if (Hccl::GetCcuErrorMsg(exceptionInfo->deviceid, status, algoParam, algoErrorInfos) != HcclResult::HCCL_SUCCESS) {
                HCCL_ERROR("Get CCU error info failed.");
                continue;
            }
...
vector<Hccl::CcuTaskParam> TaskExceptionHost::GetMC2AlgTaskParam(const Hccl::TaskInfo& taskInfo)
{
 ...
 void TaskExceptionHost::ProcessCcuException(rtExceptionInfo_t* exceptionInfo, const Hccl::TaskInfo& taskInfo)
{
...
void TaskExceptionHost::PrintOpDataErrorMessage(u32 deviceId, Hccl::ErrorMessageReport &errorMessage, string &stageErrInfo)
{
...
void ReportErrorMsg(const Hccl::TaskInfo &exceptionTaskInfo, const string &groupRankContent, const Hccl::ErrorMessageReport &errorMessage, const rtExceptionInfo_t *exceptionInfo)
{
...
void TaskExceptionHost::PrintCcuErrorInfo(uint32_t deviceId, uint16_t status, const Hccl::TaskInfo& taskInfo)
{
    ...
    HcclResult ret = Hccl::GetCcuErrorMsg(deviceId, status, ccuTaskParam, errorInfos);
...
string TaskExceptionHost::GetCcuErrorMsgByType(const Hccl::CcuErrorInfo& ccuErrorInfo, const Hccl::TaskInfo& taskInfo)
{
    ...
    static const map<Hccl::CcuRepType, GetCcuErrorMsgFunc> handlerMap {
...
{
    std::pair<Hccl::IpAddress, Hccl::IpAddress> dummy = {IpAddress(), IpAddress()};
    if (taskInfo.taskParam_.taskType != Hccl::TaskParamType::TASK_CCU) {
