    ...
    const Hccl::ErrorMessageReport &errorMessage, const rtExceptionInfo_t *exceptionInfo)
{
    ...
    if (exceptionTaskInfo.taskParam_.taskType == Hccl::TaskParamType::TASK_NOTIFY_WAIT) {
        ...
        TaskExceptionHost::GetAicpuCqeErrNetInstanceByRankId(collComm, errorMessage.rankId, localServerId);
        u32 localDeviceId = INVALID_VALUE_RANKID;
        TaskExceptionHost::GetAicpuCqeErrRemoteLocalIdByRankId(collComm, errorMessage.rankId, localDeviceId);
        std::string remoteServerId = "";
        TaskExceptionHost::GetAicpuCqeErrNetInstanceByRankId(collComm, errorMessage.remoteUserRank, remoteServerId);
        u32 remoteDeviceId = INVALID_VALUE_RANKID;
        TaskExceptionHost::GetAicpuCqeErrRemoteLocalIdByRankId(collComm, errorMessage.remoteUserRank, remoteDeviceId);
