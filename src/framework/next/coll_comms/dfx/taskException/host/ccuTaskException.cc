...
constexpr uint64_t CCU_MSG_256MB_LEN = 256ULL * 1024ULL * 1024ULL; // CCU消息长度不能大于256MB
...
struct ccumDfxInfoV2 {
    unsigned int query_result : 1; // 0:success, 1:fail
    union {
        struct {
            ...
            unsigned int cif_sqe_cnt : 1;
            unsigned int cif_cqe_cnt : 1;
            unsigned int mcm_dfx : 1;
            unsigned int resv : 20;
        } bs;
        unsigned int valid_bits : 31;
    };
...
/// @brief 版本 ops 表：dfx_info 走 print 下沉，mission/loop 走最小 info getter
...
static void PrintCcumDfxInfoV2(const void* rawData, std::ostringstream& oss)
{
    ...
    dump("SQE_RECV_CNT", info.ccum_sqe_recv_cnt, info.bs.sqe_recv_cnt != 0U);
    dump("SQE_SEND_CNT", info.ccum_sqe_send_cnt, info.bs.sqe_send_cnt != 0U);
    dump("MISSION_DFX", info.ccum_mission_dfx, info.bs.mission_dfx != 0U);
    dump("TIF_SQE_CNT", info.ccum_tif_sqe_cnt, info.bs.tif_sqe_cnt != 0U);
    dump("TIF_CQE_CNT", info.ccum_tif_cqe_cnt, info.bs.tif_cqe_cnt != 0U);
    dump("CIF_SQE_CNT", info.ccum_cif_sqe_cnt, info.bs.cif_sqe_cnt != 0U);
    dump("CIF_CQE_CNT", info.ccum_cif_cqe_cnt, info.bs.cif_cqe_cnt != 0U);
    dump("SQE_DROP_CNT", info.ccum_sqe_drop_cnt, info.bs.sqe_drop_cnt != 0U);
    dump("SQE_ADDR_LEN_ERR_DROP_CNT", info.ccum_sqe_addr_len_err_drop_cnt, info.bs.sqe_err_drop_cnt != 0U);
    dump("ccumIsEnable", info.lqc_ccu_sec_reg0 & 1U, info.bs.sec_reg0 != 0U);
    dump("MCM_DFX", info.ccum_mcm_dfx, info.bs.mcm_dfx != 0U);
}
...
static HcclResult GetCcuMissionInfoV1(const void* rawData, CcuMissionInfo* out)
{
    if (rawData == nullptr || out == nullptr) {
        ...
        return static_cast<HcclResult>(-1);
    }
...
void CcuTaskException::PrintPanicLogInfo(const uint8_t *panicLog)
{
    ...
    if (getOpsRet != HCCL_SUCCESS || ops == nullptr || ops->printCcumDfxInfo == nullptr) {
        HCCL_ERROR("[CcuTaskException][%s] Failed to get printer ops for ccum_dfx_info", __func__);
        return;
    }
