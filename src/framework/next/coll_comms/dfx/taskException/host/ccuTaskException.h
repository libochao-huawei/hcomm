...
private:
    ...
    static std::string GetCcuErrorMsgLoopGroup(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgLocPostSem(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgLocWaitEvent(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgLocWaitNotify(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgRemPostSem(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgRemWaitSem(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgRemPostVar(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgPostSharedSem(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgRead(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgWrite(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgLocalCpy(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgLocalReduce(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgBufRead(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgBufWrite(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgBufLocRead(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgBufLocWrite(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    static std::string GetCcuErrorMsgBufReduce(
        const CcuErrorInfo &ccuErrorInfo, const Hccl::TaskInfo &taskInfo, u32 deviceId);
    ...
    static std::pair<Hccl::IpAddress, Hccl::IpAddress> GetAddrPairByChannelId(
    ...
    static void GenErrorInfoLocRecordEvent(const ErrorInfoBase &baseInfo, std::shared_ptr<CcuRep::CcuRepBase> repBase,
        std::vector<CcuErrorInfo> &errorInfo);
    static void GenErrorInfoLocWaitNotify(const ErrorInfoBase &baseInfo, std::shared_ptr<CcuRep::CcuRepBase> repBase,
        std::vector<CcuErrorInfo> &errorInfo);
    static void GenErrorInfoLocWaitEvent(const ErrorInfoBase &baseInfo, std::shared_ptr<CcuRep::CcuRepBase> repBase,
        std::vector<CcuErrorInfo> &errorInfo);
    ...
    static HcclResult GenErrorInfoLoop(
        const ErrorInfoBase &baseInfo, CcuRep::CcuRepContext &ctx, std::vector<CcuErrorInfo> &errorInfo);
    ...
    static uint16_t GetMSIdPerDie(uint16_t msId)
    {
        return msId & 0x7fff;
    }
};
