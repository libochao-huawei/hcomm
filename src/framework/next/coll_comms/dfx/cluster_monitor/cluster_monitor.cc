...
void ClusterMonitor::MonitorThread()
{
    ...
    while (clusterMonitorThreadFlag_) {
        ...
        if (count >= hccl::HEARTBEAT_COUNT) {
            ...
            ProcessUbAsyncEvents(); //1s发生帧，安1s时间间隔查询异步事件????
        }
...
HcclResult ClusterMonitor::InitUbAsyncEventMonitor()
{
    ...
    // 是否支持RaCtxGetAsyncEvents接口的版本校验
    uint32_t interfaceVersion{0};
    if ((RaGetInterfaceVersion(devPhyId, RA_RS_CTX_GET_ASYNC_EVENTS, &interfaceVersion) == 0)
        && (interfaceVersion > 1)) {
        isProcessUbAsyncEvents_ = true;
    }
}

HcclResult ClusterMonitor::RegisterCtxHandleToClusterMonitor(u32 devPhyId, CtxHandle handle)
{
    CHK_PTR_NULL(handle);
    std::unique_lock<std::mutex> tableLock(ctxHandleTableMtx_);
    ctxHandleTable_[devPhyId].emplace(reinterpret_cast<u64>(handle));
    tableLock.unlock();
    return HCCL_SUCCESS;
}
...
void ClusterMonitor::PrintUbAsyncEventsContext(u32 devPhyId, const struct AsyncEvent &event)
{
    ...
    if (contextLen > CONTEXT_MAX_LEN) {
        HCCL_ERROR("[ClusterMonitor][%s] print deviceId[%u] failed for context len[%u] exceed max len[%u]", __func__,
            devPhyId, contextLen, CONTEXT_MAX_LEN);
        return;
    }
    
    HCCL_ERROR("************* deviceId[%u] ub async event info, resId[%u], eventType[%u], contextLen[%u]*************",
...
void ClusterMonitor::ProcessUbAsyncEvents()
{
    ...
    if ((RaGetInterfaceVersion(devPhyId, RA_RS_CTX_GET_ASYNC_EVENTS, &interfaceVersion) == 0)
        ...
        HCCL_WARNING("[ClusterMonitor][%s] deviceId[%u] no support version[%u]", __func__, devPhyId, interfaceVersion);
        return;
    }

    std::unique_lock<std::mutex> tableLock(ctxHandleTableMtx_);
    auto iter = ctxHandleTable_.find(devPhyId);
    if (iter != ctxHandleTable_.end()) {
        for (const auto &value : ctxHandleTable_[devPhyId]) {
            CtxHandle ctxHandle = reinterpret_cast<CtxHandle>(value);
            unsigned int num = ASYNC_EVENT_MAX_NUM;
            auto events
                = std::unique_ptr<struct AsyncEvent[]>(new (std::nothrow) struct AsyncEvent[ASYNC_EVENT_MAX_NUM]);
            CHK_SMART_PTR_RET_NULL(events);

            int ret = RaCtxGetAsyncEvents(ctxHandle, events.get(), &num);
            if (ret != 0) {
                HCCL_ERROR("[ClusterMonitor][%s] get async events failed, deviceId[%u] ctx[%p], ret[%d]", __func__, devPhyId,
                    (void *)ctxHandle_, ret);
                break; // 还是return???
            }
    ...
    tableLock.unlock();
    HCCL_DEBUG("[ClusterMonitor][%s] deviceId[%u] end", __func__, devPhyId);
}
