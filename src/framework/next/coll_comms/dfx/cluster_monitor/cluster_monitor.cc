...
std::string ClusterMonitor::FormatConnTag(HcommSocketRole role, std::pair<ClusterUIDType, ClusterUIDType> uidPair)
{
    std::string tag;
...
HcclResult ClusterMonitor::InsertClusterMonitorCxt(
    HcclComm comm, UIDContext remoteCtx, std::map<ClusterUIDType, ClusterMonitorSocketCtx>& needConnectRank)
{
    ...
    CHK_PRT_RET(
        (ret != EOK), HCCL_ERROR("[%s] memcpy_s failed, errno:%d, error:%s", __func__, errno, strerror(errno)),
        HCCL_E_SYSCALL);
    ...
    HCCL_DEBUG(
        "[%s] InsertClusterMonitorCxt for remoteUID[%s], role[%s], localEndpoint[commAddr:0x%llx], "
        "remoteEndpoint[commAddr:0x%llx], tag[%s], listenPort [%u], newConn[%d]",
        __func__, GetUID(remoteUID).c_str(),
    ...
    std::map<uint32_t, std::vector<UIDContext>> uidCtxs, std::vector<uint32_t>& netLayersVector)
{
    ...
    for (uint32_t netLayer : netLayersVector) {
        ...
        for (auto it = uidCtxs[netLayersVector[netLayer]].begin(); it != uidCtxs[netLayersVector[netLayer]].end();
             ++it) {
    ...
    if (uid2SocketRefMap_[dst].sendBuffer.size() > 0) {
        ...
        while (uid2SocketRefMap_[dst].sendBuffer.size() > 0) {
            ...
            HcclResult ret = SocketSendNb(
                uid2SocketRefMap_[dst].socketHandler, (reinterpret_cast<char*>(&hbf) + sendDis),
                uid2SocketRefMap_[dst].restSize, (reinterpret_cast<uint64_t*>(&compSize)));
            if (ret != HCCL_SUCCESS) {
    ...
    u32 remoteLocalId, uint16_t status, std::string localEid, std::string remoteEid, std::string remoteInsId)
{
    ...
    s32 stringRet = snprintf_s(
        ...
        "localInfo{local instanceId[%s], LocalId[%d], localEid[%s]}, remoteInfo{remote instanceId[%s], "
        "remoteLocalId[%d], remoteEid[%s]}",
    ...
    if (now == nullptr) {
        HCCL_ERROR(
            ...
            LOG_KEYWORDS_TASK_EXEC.c_str(), LOG_KEYWORDS_CQE_ERROR.c_str(), cqeErrInfo_.cqeStatus, errorLinkLogBuffer);
    } else {
        HCCL_ERROR(
            "[%s][%s][%s]cqe error status[%u], time:[%04u-%02d-%02d %02d:%0d:%02d.%06u], %s",
            LOG_KEYWORDS_TASK_EXEC.c_str(), LOG_KEYWORDS_TASK_EXEC.c_str(), LOG_KEYWORDS_CQE_ERROR.c_str(),
            cqeErrInfo_.cqeStatus, now->tm_year + 1900, now->tm_mon + 1, now->tm_mday, now->tm_hour, now->tm_min,
...
ClusterMonitor::PrintEvents(std::map<ClusterMonitorStatus, std::queue<ClusterMonitorFrame>>& keyEvents)
{
