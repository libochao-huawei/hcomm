...
struct UidContext {
    ...
    UidContext() {}
    UidContext(std::string &uid, uint32_t netLayer, uint32_t rankId, uint32_t localId)
...
struct ClusterUidCtxt {
    ...
    uint32_t        localId;
    ClusterUidCtxt() {}
    ClusterUidCtxt(uint32_t localId, std::string &netInstId)
        localId(localId), netInstId(netInstId)
    {}
};
