...
using ClusterUIDType = struct HcclClusterMonitorUid {
    char id[512] = {0}; // netLayerId + netInstanceId + rankId 最大不超过512字节
...
struct ClusterUidCtxt {
    ...
    RankId        rankId;
    ClusterUidCtxt() {}
    ClusterUidCtxt(uint32_t netLayer, std::string &netInstanceId, uint32_t rankId)
    : netLayer(netLayer), netInstanceId(netInstanceId), rankId(rankId)
    {}
};
