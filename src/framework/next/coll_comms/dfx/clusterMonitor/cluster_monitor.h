...
public:
    ...
    HcclResult GetSamePlaneRank(HcclComm comm, std::vector<UidContext> singlePlaneCtx, std::map<ClusterUIDType, ClusterMonitorSocketCtx> &need
ConnectRank);
    HcclResult GetConnectRank(HcclComm comm, std::map<ClusterUIDType, ClusterMonitorSocketCtx> &needConnectRank, std::map<uint32_t, std::vecto
r<UidContext>> uidctxs, uint32_t netLayerNum);
