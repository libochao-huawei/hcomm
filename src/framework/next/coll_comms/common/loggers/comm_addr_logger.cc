...
namespace logger {
    ...
    void CommAddrLogger::Print(uint32_t idx, const char *endpointName, const CommAddr &commAddr)
    {
        ...
        HCCL_INFO("[%s] channelDescs[%u] %s commAddr: type[%d], addr[%s]", __func__, idx, endpointName, commAddr.type,
            addrStr.c_str());
    }
    ...
    void CommAddrLogger::CommLinkPrint(const CommLink &commlink)
    {
        ...
        if (commlink.srcEndpointDesc.loc.locType == ENDPOINT_LOC_TYPE_DEVICE) {
            HCCL_INFO("[%s] src devPhyId[%u] %s link to dst devPhyId[%u] %s", __func__,
                commlink.srcEndpointDesc.loc.device.devPhyId, srcCommAddrStr.c_str(),
                commlink.dstEndpointDesc.loc.device.devPhyId, dstCommAddrStr.c_str());
        } else if (commlink.srcEndpointDesc.loc.locType == ENDPOINT_LOC_TYPE_HOST) {
            HCCL_INFO("[%s] src host id[%u] %s link to dst host id[%u] %s", __func__,
                commlink.srcEndpointDesc.loc.host.id, srcCommAddrStr.c_str(), commlink.dstEndpointDesc.loc.host.id,
                dstCommAddrStr.c_str());
        } else {
            HCCL_INFO("[%s] loc: locType[%d]", __func__, commlink.srcEndpointDesc.loc.locType);
        }
    }
