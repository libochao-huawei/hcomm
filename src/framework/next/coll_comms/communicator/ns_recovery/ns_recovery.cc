...
{
    HCCL_INFO(
        "[NsRecovery][AddData] AddNsRecoveryData for engine[%d], channelNum[%u], commTag[%s]", static_cast<int>(engine),
        channelNum, commTag.c_str());
...
HcclResult NsRecoveryProcessor::ListenBackGround(Hccl::KfcExecStatus& opInfo)
{
    ...
    while (true) {
        ...
        if (opInfo.kfcStatus == Hccl::KfcStatus::CLEAN_DONE) {
            ...
            if ((std::chrono::steady_clock::now() - startTime) >= timeout) {
                HCCL_ERROR(
                    "[NsRecovery][Clean] Wait clean response status timeout[%u ms] and get the opExecStatus is [%u].",
                    WAIT_CMD_TIMEOUT, opInfo.kfcStatus);
                return HcclResult::HCCL_E_TIMEOUT;
            }
            continue;
    ...
    for (auto& recoveryData : nsRecoveryDatas_) {
        if (recoveryData.first == COMM_ENGINE_AICPU || recoveryData.first == COMM_ENGINE_AICPU_TS) {
            for (auto& handleData : recoveryData.second) {
                CHK_RET(
                    hcomm::ChannelProcess::ChannelUpdateKernelLaunch(
                        handleData.channelHandles_.data(), handleData.hostChannelHandleList_.data(),
