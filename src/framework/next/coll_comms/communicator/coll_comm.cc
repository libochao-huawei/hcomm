...
HcclResult CollComm::Init(void * rankGraph, aclrtBinHandle binHandle, HcclMem cclBuffer, HcclCommConfig *config)
{
    ...
    EXECEPTION_CATCH(myRank_ = std::make_shared<MyRank>(binHandle, rankId_, config_, callbacks_, rankgraph_.get(), rankIpPortMap_), return HCCL_E_PTR);
...
HcclResult CollComm::GetRankIpPortMap()
{
    ...
    CHK_PTR_NULL(comV2);
    rankIpPortMap_ = comV2->GetRankIpPortMap();
