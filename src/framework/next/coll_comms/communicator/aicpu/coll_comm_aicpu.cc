...
HcclResult CollCommAicpu::ParsePackData(std::vector<char> &data, ChannelHandle &handle)
{
    ...
    Hccl::BinaryStream binaryStream(transpUniqueId);
    u32 transType;
    binaryStream >> transType;

    // TODO TransportType
    if (transType == TransportType::TRANS_TYPE_UB) {
    ...
    } else if (transType == TransportType::TRANS_TYPE_DEVICE_P2P) {
