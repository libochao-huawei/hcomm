...
namespace CcuRep {
    ...
    bool CcuRepRemWaitSem::Translate(CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        CHK_PRT_THROW(
            channelImpl->GetLocCkeByIndex(semIndex, signalId) != HcclResult::HCCL_SUCCESS,
            HCCL_ERROR("[CcuRepRemWaitSem][%s] failed to get to loc cke id.", __func__), Hccl::InternalException,
