...
CcuRepPostSharedVar::CcuRepPostSharedVar(const Variable &srcVar,
    const Variable &dstVar, const LocalNotify &notify, uint16_t mask)
    : srcVar(srcVar), dstVar(dstVar), notify_(notify), mask(mask)
{
...
bool CcuRepPostSharedVar::Translate(CcuInstr *&instr, uint16_t &instrId, const TransDep &dep)
{
    ...
    if (notify_.DieId() != dep.dieId) {
        SyncXnInstr(instr++, dstVar.Id(), srcVar.Id(), dep.reserveChannalId[1], notify_.Id(), mask, 0, 0, 0, 0, 1);
        LoadImdToXnInstr(instr++, dep.reserveXnId, 0);
    } else {
        LoadXXInstr(instr++, dstVar.Id(), srcVar.Id(), dep.reserveXnId);
        SetCKEInstr(instr++, notify_.Id(), mask, 0, 0, 1);
    }
