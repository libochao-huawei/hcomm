...
namespace CcuRep {
    ...
    CcuRepRemPostVar::CcuRepRemPostVar(
        Variable param, const ChannelHandle channel, uint16_t paramIndex, uint16_t semIndex, uint16_t mask)
        : param(param),
          channel(channel),
    ...
    bool CcuRepRemPostVar::Translate(CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        CHK_PRT_THROW(
            channelImpl->GetRmtXnByIndex(paramIndex, rmtXnId) != HcclResult::HCCL_SUCCESS,
            HCCL_ERROR("[CcuRepRemPostSem][%s] failed to get remote xn id, channelHandle[0x%llx].", __func__, channel),
            Hccl::InternalException, "failed to get remote xn id.");

        CHK_PRT_THROW(
            channelImpl->GetRmtCkeByIndex(semIndex, rmtCkeId) != HcclResult::HCCL_SUCCESS,
            HCCL_ERROR("[CcuRepRemPostSem][%s] failed to get remote cke id, channelHandle[0x%llx].", __func__, channel),
            Hccl::InternalException, "failed to get remote cke id.");

        SyncXnInstr(instr++, rmtXnId, param.Id(), channelImpl->GetChannelId(), rmtCkeId, mask, 0, 0, 0, 0, 1);
        CHK_PRT_THROW(
