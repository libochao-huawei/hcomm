...
namespace CcuRep {
    ...
    CcuRepBufRead::CcuRepBufRead(
        const ChannelHandle channel, RemoteAddr src, CcuBuf dst, Variable len, CompletedEvent sem, uint32_t mask)
        : channel(channel),
          src(src),
          dst(dst),
          len(len),
    ...
    bool CcuRepBufRead::Translate(CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        TransRmtMemToLocMSInstr(
            instr++, dst.Id(), src.addr.Id(), src.token.Id(), len.Id(), channelId, sem.Id(), mask, 0, 0, 1, 1);
