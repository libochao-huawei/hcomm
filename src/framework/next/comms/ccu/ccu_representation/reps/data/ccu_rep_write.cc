...
namespace CcuRep {
    ...
    CcuRepWrite::CcuRepWrite(
        const ChannelHandle channel, RemoteAddr rem, LocalAddr loc, Variable len, CompletedEvent sem, uint16_t mask)
        : channel(channel),
          rem(rem),
          loc(loc),
          len(len),
    ...
    CcuRepWrite::CcuRepWrite(
        const ChannelHandle channel, RemoteAddr rem, LocalAddr loc, Variable len, uint16_t dataType, uint16_t opType,
    ...
    bool CcuRepWrite::Translate(CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        TransLocMemToRmtMemInstr(
            instr++, rem.addr.Id(), rem.token.Id(), loc.addr.Id(), loc.token.Id(), len.Id(),
