...
namespace CcuRep {
    ...
    CcuRepRemMem::CcuRepRemMem(const ChannelHandle channel, RemoteAddr rem) : channel(channel), rem(rem)
    {
    ...
    bool CcuRepRemMem::Translate(CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        CHK_PRT_THROW(
            ...
            HCCL_ERROR("[CcuRepRemMem][%s] failed to get remote buffer, channelHandle[0x%llx].", __func__, channel),
