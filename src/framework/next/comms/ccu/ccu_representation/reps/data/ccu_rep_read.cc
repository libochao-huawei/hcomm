...
namespace CcuRep {
    ...
    CcuRepRead::CcuRepRead(
        const ChannelHandle channel, LocalAddr loc, RemoteAddr rem, Variable len, CompletedEvent sem, uint16_t mask)
        : channel(channel),
          loc(loc),
          rem(rem),
          len(len),
    ...
    CcuRepRead::CcuRepRead(
        const ChannelHandle channel, LocalAddr loc, RemoteAddr rem, Variable len, uint16_t dataType, uint16_t opType,
    ...
    bool CcuRepRead::Translate(CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        TransRmtMemToLocMemInstr(
            instr++, loc.addr.Id(), loc.token.Id(), rem.addr.Id(), rem.token.Id(), len.Id(),
