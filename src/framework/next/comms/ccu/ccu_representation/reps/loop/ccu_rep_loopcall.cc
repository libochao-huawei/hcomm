...
namespace CcuRep {
    ...
    bool CcuRepLoopCall::Translate(CcuInstr *&instr, uint16_t &instrId, const TransDep &dep)
    {
        ...
        for (uint32_t i = 0; i < inArgs.size(); i++) {
            ...
            } else if (inArgs[i].type == CcuArgType::LOCAL_ADDR
                       && loopBlock->GetArg(i).type == CcuArgType::LOCAL_ADDR) {
                LoadGSAGSAInstr(
                    instr++, loopBlock->GetArg(i).localAddr.addr.Id(), inArgs[i].localAddr.addr.Id(), dep.reserveGsaId);
                LoadXXInstr(instr++, loopBlock->GetArg(i).localAddr.token.Id(), inArgs[i].localAddr.token.Id(),
                    dep.reserveXnId);
            } else if (inArgs[i].type == CcuArgType::REMOTE_ADDR
                       && loopBlock->GetArg(i).type == CcuArgType::REMOTE_ADDR) {
                LoadGSAGSAInstr(instr++, loopBlock->GetArg(i).remoteAddr.addr.Id(), inArgs[i].remoteAddr.addr.Id(),
                    dep.reserveGsaId);
                LoadXXInstr(instr++, loopBlock->GetArg(i).remoteAddr.token.Id(), inArgs[i].remoteAddr.token.Id(),
                    dep.reserveXnId);
            } else if (inArgs[i].type == CcuArgType::MEMORY_LIST
                       && loopBlock->GetArg(i).type == CcuArgType::MEMORY_LIST) {
            ...
            } else if (inArgs[i].type == CcuArgType::LOCAL_ADDR_LIST
                       && loopBlock->GetArg(i).type == CcuArgType::LOCAL_ADDR_LIST) {
                if (inArgs[i].localAddrList.size() != loopBlock->GetArg(i).localAddrList.size()) {
                    Hccl::THROW<Hccl::CcuApiException>("Mismatched Arg Size");
                }
                for (uint32_t j = 0; j < inArgs[i].localAddrList.size(); j++) {
                    LoadGSAGSAInstr(instr++, loopBlock->GetArg(i).localAddrList[j].addr.Id(),
                        inArgs[i].localAddrList[j].addr.Id(), dep.reserveGsaId);
                    LoadXXInstr(instr++, loopBlock->GetArg(i).localAddrList[j].token.Id(),
                        inArgs[i].localAddrList[j].token.Id(), dep.reserveXnId);
                }
            } else if (inArgs[i].type == CcuArgType::REMOTE_ADDR_LIST
                       && loopBlock->GetArg(i).type == CcuArgType::REMOTE_ADDR_LIST) {
                if (inArgs[i].remoteAddrList.size() != loopBlock->GetArg(i).remoteAddrList.size()) {
                    Hccl::THROW<Hccl::CcuApiException>("Mismatched Arg Size");
                }
                for (uint32_t j = 0; j < inArgs[i].remoteAddrList.size(); j++) {
                    LoadGSAGSAInstr(instr++, loopBlock->GetArg(i).remoteAddrList[j].addr.Id(),
                        inArgs[i].remoteAddrList[j].addr.Id(), dep.reserveGsaId);
                    LoadXXInstr(instr++, loopBlock->GetArg(i).remoteAddrList[j].token.Id(),
                        inArgs[i].remoteAddrList[j].token.Id(), dep.reserveXnId);
                }
            } else {
