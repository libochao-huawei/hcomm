...
HcclResult CcuCompGetTpAttrAsync(RdmaHandle handle, uint64_t tpHandle, uint32_t& attrBitmap, TpAttr& attr, RequestHandle& reqHandle)
{
    HCCL_INFO("[CcuCompGetTpAttrAsync] begain, reqHandle[%llu]", reqHandle);

    void *raReqHandle = nullptr;
    int ret = RaGetTpAttrAsync(handle, tpHandle, &attrBitmap, &attr, &raReqHandle);
    if (ret != 0) {
        return HCCL_E_RUNTIME;
        // std::string msg = StringFormat("call RaGetTpAttrAsync failed, error code =%d.", ret);
        // THROW<NetworkApiException>(msg);
    }

    CHK_RET(Hccl::WaitRequestResult(raReqHandle, reqHandle));
    HCCL_INFO("[CcuCompGetTpAttrAsync] success, reqHandle[%llu]", reqHandle);
    return HCCL_SUCCESS;
}

 
HcclResult CcuCompCalcTotalTimeout(CtxHandle ctxHandle, TpHandle tpHandle, uint32_t &outTotalTimeoutMs)
{
    uint32_t attrBitmap = 0;
    struct TpAttr tpAttr = {0};
    RequestHandle reqHandle{0};
    CHK_RET(CcuCompGetTpAttrAsync(ctxHandle, tpHandle, attrBitmap, tpAttr, reqHandle));
    ...
    const CommAddr &commAddr, const std::vector<JettyInfo> &jettyInfos)
{
    ...
    for (const auto &jettyInfo : jettyInfos) {
        ...
        u8 errTimeout = 8;                                   // TODO: 读环境变量HCCL_UB_TIMEOUT，默认为8（对应1s）
        u8 envValue = EnvConfig::GetExternalInputUBTimeOut();
        uint32_t envTimeoutMs = CcuCompTaHwValueToMs(envValue);
        uint32_t tpTimeOutMs = 0;
        CHK_RET(CcuCompCalcTotalTimeout(ctxHandle, tpHandle, tpTimeOutMs));

        if (envTimeoutMs < tpTimeOutMs) {
            // 规则: 如果环境变量时间 < TP总超时，选择大于TP总超时的最小TA挡位
            errTimeout = CcuCompFindMinTaHwValueGreaterThan(tpTimeOutMs);
            HCCL_WARNING(
                "[TaTimeoutFinalDecider] Env timeout [%ums] < TP timeout [%ums]. Auto upgrade TA to hw_val[%u] (%ums).",
                envTimeoutMs, tpTimeOutMs, errTimeout, CcuCompTaHwValueToMs(errTimeout));
        } else {
