...
HcclResult GetTpAttrAsync(RdmaHandle handle, uint64_t tpHandle, uint32_t& attrBitmap, TpAttr& attr, RequestHandle& reqHandle)
{
    ...
    HcclResult ret = RaGetTpAttrAsync(handle, tpHandle, &attrBitmap, &attr, &raReqHandle);
    if (ret != 0) {
    ...
    const CommAddr &commAddr, const std::vector<JettyInfo> &jettyInfos)
{
    ...
    for (const auto &jettyInfo : jettyInfos) {
        ...
        struct TpAttr tpAttr = {0};
        RequestHandle reqHandle{0};
        CHK_RET(HrtRaGetTpAttrAsync(ctxHandle, tpHandle, attrBitmap, tpAttr, reqHandle));
        ...
        CHK_RET(CalcTotalTimeout(ctxHandle, tpHandle, tpTimeOutMs));

        if (envTimeoutMs < tpTimeOutMs) {
            // 规则: 如果环境变量时间 < TP总超时，选择大于TP总超时的最小TA挡位
            errTimeout = FindMinTaHwValueGreaterThan(tpTimeOutMs);
            HCCL_WARNING(
