    ...
    std::vector<LocalJettyCtxData> &jettyCtxData)
{
    ...
    HCCL_INFO("[CcuJettyCtxMgr][%s] iodie[%u], startJettyCtxId[%u], jettyCtxData.size[%u]", __func__, dieId,
        startJettyCtxId, jettyNum);

    for (size_t i = 0; i < jettyNum; i++) {
        DumpJettyCtxData(jettyCtxData[i]);

        (void)memcpy_s(&inBuff.data.dataInfo.dataArray[i], sizeof(struct LocalJettyCtxData), &jettyCtxData[i],
    ...
    if (ret != 0) {
        HCCL_ERROR("[CcuResSpecifications][%s] failed to call ccu driver, "
            ...
            __func__, devPhyId, dieId, "SET_JETTY_CTX");
...
HcclResult CcuJettyCtxMgr::TryAllocWqeBBResource(const uint32_t sqSize, const uint32_t jettyCtxStartId,
