...
{
    // 安全问题：禁止打印token相关信息
    HCCL_RUN_INFO("doorbellAddr: [3]0x%04x, [2]0x%04x, [1]0x%04x, [0]0x%04x, pfeIdx: 0x%04x, ioDieId: 0x%04x, "
        "doorbellAddrType: 0x%04x, tokenValueIsValid: 0x%04x, sqeBasicBlockLeftShifts: 0x%04x, pi: 0x%04x, "
        "ci: 0x%04x, maxCi: 0x%04x, oooCqeCnt: 0x%04x, startWqeBasicBlockIdxLow: 0x%04x, "
        ...
        tmp.doorbellAddr[1],
        tmp.doorbellAddr[0],
        tmp.pfeIdx, tmp.ioDieId, tmp.doorbellAddrType, tmp.tokenValueIsValid, tmp.sqeBasicBlockLeftShifts,
        tmp.pi, tmp.ci, tmp.maxCi, tmp.oooCqeCnt, tmp.startWqeBasicBlockIdxLow, tmp.startWqeBasicBlockIdxHigh,
        tmp.doorbellSendState);
}
