...
static ChannelCtxDataV1 BuildChannelCtxDataV1(
    const ChannelCfg &cfg, const uint32_t feId, const uint8_t dieId, const uint16_t startTaJettyId)
{
...
static HcclResult ConfigChannelCtxDataV1(
    const uint32_t devPhyId, const uint8_t dieId, const uint32_t channelId, const ChannelCtxDataV1 &channelCtxData)
{
    ...
    HCCL_INFO("[CcuChannelCtxMgrV1][%s] set data to ccu driver, devPhyId[%u], "
        ...
        __func__, devPhyId, dieId, channelId, sizeof(struct ChannelCtxDataV1));
    ...
    if (ret != 0) {
        HCCL_ERROR("[CcuResSpecifications][%s] failed to call ccu driver, "
            ...
            __func__, devPhyId, dieId, "SET_CHANNEL");
