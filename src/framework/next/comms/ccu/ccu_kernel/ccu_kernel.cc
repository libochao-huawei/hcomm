...
HcclResult CcuKernel::GeneTaskParam(const CcuTaskArg &arg, std::vector<CcuTaskParam> &taskParams)
{
    ...
    const uint64_t *argsData = args.data();
    for (uint32_t index = 0; index < seqNum; index++) {
        ...
        if (index == seqNum - 1) {
            ...
            const uint32_t preMissionInsCnt = argOffset;
            taskParam.instCnt = instrInfo_.missionInstrCount - preMissionInsCnt;
            std::copy_n(argsData + preMissionInsCnt, argsNum - preMissionInsCnt, std::begin(taskParam.args));
        } else {
            taskParam.instCnt = CCU_SQE_ARGS_LEN;
            std::copy_n(argsData + argOffset, CCU_SQE_ARGS_LEN, std::begin(taskParam.args));
        }
