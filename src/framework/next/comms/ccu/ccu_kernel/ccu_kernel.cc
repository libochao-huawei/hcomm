    ...
    const CcuRep::Variable& len, CcuRep::CompletedEvent event)
{
    ...
    Append(
        std::make_shared<CcuRep::CcuRepBufReduce>(
            ccuBufs, count, CcuRep::GetCcuDataType(dataType_, opType_),
            CcuRep::GetCcuDataType(outputDataType_, opType_), CcuRep::GetCcuReduceType(opType_), event, len,
            event.mask));
    return HCCL_SUCCESS;
}
    ...
    const CcuRep::Variable& len, HcclDataType dataType, HcclReduceOp opType, CcuRep::CompletedEvent event)
{
    ...
    Append(
        std::make_shared<CcuRep::CcuRepRead>(
            channel, loc, rem, len, CcuRep::GetUBDataType(dataType_), CcuRep::GetUBReduceType(opType_), event,
            event.mask));
    return HCCL_SUCCESS;
}
    ...
    const CcuRep::Variable& len, HcclDataType dataType, HcclReduceOp opType, CcuRep::CompletedEvent event)
{
    ...
    Append(
        std::make_shared<CcuRep::CcuRepWrite>(
            channel, rem, loc, len, CcuRep::GetUBDataType(dataType_), CcuRep::GetUBReduceType(opType_), event,
            event.mask));
    return HCCL_SUCCESS;
}
    ...
    HcclReduceOp opType, CcuRep::CompletedEvent event)
{
    ...
    Append(
        std::make_shared<CcuRep::CcuRepLocCpy>(
...
CcuRep::Variable CcuKernel::CreateVariable() { return CreateResAssist(res_.variable); }

CcuRep::Variable CcuKernel::CreateContinuousVariable() { return CreateResAssist(res_.continuousVariable); }

CcuRep::Address CcuKernel::CreateAddress() { return CreateResAssist(res_.address); }

CcuRep::LocalNotify CcuKernel::CreateLocalNotify() { return CreateResAssist(res_.localNotify); }

CcuRep::CompletedEvent CcuKernel::CreateCompletedEvent() { return CreateResAssist(res_.completedEvent); }

CcuRep::CcuBuf CcuKernel::CreateCcuBuf() { return CreateResAssist(res_.ccubufs); }

CcuRep::Executor CcuKernel::CreateExecutor() { return CreateResAssist(res_.executor); }
...
HcclResult CcuKernel::GetCcuProfilingInfo(const CcuTaskArg& arg, std::vector<CcuProfilingInfo>& allCcuProfilingInfo)
{
    ...
    for (uint32_t i = 0; i < lgProfInfo.lgProfilingReps.size();
        ...
        CHK_RET(GetArgIndex(
            varId2VarIdMap, varId2ArgIndexMap, taskArgs, groupOpSizeInfo_[i].parallelParamId, parallelParam));
        HCCL_INFO(
