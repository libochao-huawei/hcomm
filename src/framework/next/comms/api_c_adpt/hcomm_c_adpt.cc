...
{
    if(hcomm::g_ThreadMap.find(thread) == hcomm::g_ThreadMap.end()) {
        HCCL_ERROR("[%s] failed. thread not found.", __func__);
        return HCCL_E_NOT_FOUND;
    }
    std::shared_ptr<hccl::Thread> threadPtr = hcomm::g_ThreadMap[thread];
...
{
    if (hcomm::g_ThreadMap.find(thread) == hcomm::g_ThreadMap.end()) {
        HCCL_ERROR("[%s] failed. thread[%u] not found.", __func__, thread);
        return HCCL_E_NOT_FOUND;
    }
    handle = hcomm::g_ThreadMap[thread];
    CHK_PTR_NULL(handle);
    return HCCL_SUCCESS;
}