...
HcclResult HcommChannelGetEntity(const ChannelHandle *channelList, uint32_t listNum, ChannelEntity **channelEntityList)
{
    ...
    for (uint32_t i = 0; i < listNum; ++i) {
        ...
        void *channel;
        HcclResult ret = HcommChannelGet(inHandle, &channel);
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("%s HcommChannelGet failed, ret[%d]", __func__, ret),
            HCCL_E_NOT_FOUND);
        AicpuTsRoceChannel* aicpuTsRoceChannel = static_cast<AicpuTsRoceChannel*>(channel); // ljy:是否考虑拓展
        ChannelEntity *channelEntity = channelEntityList[i];
        // 初始化ABI头信息
