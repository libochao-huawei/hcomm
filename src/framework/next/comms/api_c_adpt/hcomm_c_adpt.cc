...
HcclResult HcommEngineCtxCopy(CommEngine engine, const void *dstCtx, const void *srcCtx, uint64_t size)
{
    if (engine == COMM_ENGINE_AICPU_TS || engine == COMM_ENGINE_AICPU) {
        ...
        (void)memcpy_s(reinterpret_cast<uint8_t*>(dstCtx), size, srcCtx, size);
    } else {
        HCCL_ERROR("[%s]copy engine ctx failed, Unsupported engine[%d], tag[%s]", __func__, engine);
        return HCCL_E_PARA;
    }
