    ...
    uint32_t channelNum, ChannelHandle *channels)
{
    ...
    while (true) {
        ...
        // 3. Others: 错误状态
        HcclResult ret = HcommChannelGetStatusInner(targetChannels, channelNum, statusList);
        if (ret == HCCL_E_AGAIN) {
            continue;
        }
    ...
    if (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) {
        CHK_RET(HcommChannelKernelLaunchInner(channels, targetChannels, channelNum));
    } else {
...
HcclResult HcommChannelKernelLaunchInner(ChannelHandle *channelHandles, ChannelHandle *hostChannelHandles, uint32_t listNum)
{
    ...
    // channelParam资源参数填充
    s32 sRet = strncpy_s(channelParam.hcomId, HCOMID_MAX_LENGTH, " ", HCOMID_MAX_LENGTH - 1);
...
HcclResult HcommThreadFree(const ThreadHandle *threads, uint32_t threadNum)
{
    ...
    for (uint32_t i = 0; i < threadNum; ++i) {
        const ThreadHandle inHandle = channels[i];
