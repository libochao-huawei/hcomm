    ...
    ThreadHandle* threads, CommEngine engine, aclrtBinHandle binHandle)
{
    ...
    if (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) {
        ...
        EXECEPTION_CATCH(aicpuHandle = std::make_unique<ThreadHandle[]>(newThreads.size()),
                         return HCCL_E_PTR);
        CHK_PTR_NULL(binHandle);
        HcclResult ret = hccl::AicpuLaunchMgr::ThreadKernelLaunchForBase(
...
HcclResult FreeThreads(const ThreadHandle *threads, uint32_t threadNum, aclrtBinHandle binHandle)
{
    ...
    std::vector<ThreadHandle> deviceHandles; // 存放device侧的handle

    CHK_RET(FreeThreadHandlesLocked(threads, threadNum, deviceHandles));

    // 如果有需要销毁的deviceThread，调用销毁kernel
    if (!deviceHandles.empty()) {
        CHK_RET(hccl::AicpuLaunchMgr::ThreadKernelLaunchDestroy(deviceHandles.data(), deviceHandles.size(), binHandle));
    }
