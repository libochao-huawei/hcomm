...
HcclResult ValidateThreadParams(uint32_t threadNum, uint32_t notifyNumPerThread)
{
    if (threadNum == 0 || threadNum > HCOMM_THREADNUM_MAX_NUM) {
        HCCL_ERROR("[%s] Validate thread params failed. ThreadNum %u, range (0, %u]", __func__, threadNum,
            HCOMM_THREADNUM_MAX_NUM);
        return HCCL_E_PARA;
    }
    if (notifyNumPerThread > HCOMM_NOTIFY_MAX_NUM) {
        HCCL_ERROR("[%s] Validate thread params failed. notifyNumPerThread %u, range [0, %u]", __func__,
            notifyNumPerThread, HCOMM_NOTIFY_MAX_NUM);
        return HCCL_E_PARA;
    }
    return HCCL_SUCCESS;
}
...
HcclResult CreateAndInitThreads(const ThreadCreateParams &params, vector<shared_ptr<Thread>> &outThreads)
{
    ...
    for (uint32_t i = 0; i < params.threadNum; ++i) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[%s] Failed to create thread at index %u, error: %d", __func__, i, ret), ret);
    ...
    const ThreadHandle *threads, uint32_t threadNum, vector<ThreadHandle> &deviceHandles)
{
    ...
    for (uint32_t i = 0; i < threadNum; ++i) {
        ...
        if (itC == g_ThreadMap.end()) {
            HCCL_ERROR("[%s] failed to find thread in g_ThreadMap, inHandle[0x%llx], mappedHandle[0x%llx].", __func__,
        ...
        HCCL_INFO("[%s] erase thread: inHandle[0x%llx], mappedHandle[0x%llx], ptr[%p]", __func__, inHandle,
...
HcclResult Thread::ReportAicpuNotifyWaitTask(u64 notifyId, u64 beginTime, u32 taskId, u32 sqId) const
{
    ...
    HCCL_INFO("[Thread][%s] sqId[%u], taskId[%u], notifyId[%llu], %s", __func__, sqId, taskId, notifyId,
...
HcclResult Thread::ReportHostNotifyWaitTask(u64 notifyId, u64 beginTime, bool isMaster) const
{
    ...
    HCCL_INFO("[Thread][%s] streamId[%u], taskId[%u], notifyId[%llu], %s", __func__, streamId, taskId, notifyId,
        taskParam.Describe().c_str());
#endif
    return HCCL_SUCCESS;
}
    ...
    void *dst, const void *src, uint64_t sizeByte, u64 beginTime, bool isMaster) const
{
    ...
    CHK_PTR_NULL(callback_);
    CHK_RET(callback_(streamId, taskId, taskParam, INVALID_U64));
    HCCL_INFO("[Thread][%s] streamId[%u], taskId[%u], src[%p], dst[%p], len[%llu] %s", __func__, streamId, taskId, src,
        dst, sizeByte, taskParam.Describe().c_str());
#endif
    return HCCL_SUCCESS;
}
    ...
    void *dst, const void *src, uint64_t sizeByte, u64 beginTime, u32 taskId, u32 sqId) const
{
    ...
    CHK_PTR_NULL(callback_);
    CHK_RET(callback_(sqId, taskId, taskParam, INVALID_U64));
    HCCL_INFO("[Thread][%s] sqId[%u], taskId[%u], src[%p], dst[%p], len[%llu] %s", __func__, sqId, taskId, src, dst,
