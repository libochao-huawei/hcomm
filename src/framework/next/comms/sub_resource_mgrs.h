...
private:
    std::unordered_map<EndpointDesc, EndpointHandle> endpointMap_; // 从EndpointDesc到EndpointHandle的映射
    std::unordered_map<EndpointHandle, std::unique_ptr<Endpoint>> endpointObjMap_; // 从EndpointHandle到Endpoint对象的映射
    std::unordered_map<EndpointHandle, std::vector<MemHandle>> endpointMemMap_; // 从EndpointHandle到MemHandle的映射
    mutable std::mutex mutex_;
    std::atomic<size_t> endpointCount_{0}; // 端点数量
    std::atomic<size_t> memHandleCount_{0}; // 内存句柄数量
};
...
private:
    std::unordered_map<ChannelHandle, std::unique_ptr<Channel>> channelMap_; // 从ChannelHandle到Channel对象的映射
    std::unordered_map<ChannelHandle, std::vector<MemHandle>> channelMemMap_; // 从ChannelHandle到MemHandle的映射
    mutable std::mutex mutex_;
...
private:
    std::unordered_map<int32_t,
        std::unique_ptr<void, std::function<void(void*)>>> ccuInstances_;
    mutable std::mutex mutex_;
