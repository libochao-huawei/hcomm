...
public:
    ...
    HcclResult Size(size_t& outSize) const;
    HcclResult GetEndpointCount(size_t& outCount) const;
    HcclResult GetMemHandleCount(size_t& outCount) const;
    HcclResult IsEmpty(bool& outIsEmpty) const;

    HcclResult FindEndpointByHandle(EndpointHandle handle, Endpoint*& outEndpoint);
    HcclResult RemoveOwnedEndpoint(EndpointHandle handle);
    HcclResult InstallOwnedEndpoint(EndpointHandle handle, std::unique_ptr<Endpoint> endpoint);

private:
    HcclResult ReleaseAllMemHandlesUnlocked();
    HcclResult ReleaseAllEndpointsUnlocked();
    HcclResult AddMemHandle(EndpointHandle endpointHandle, const std::vector<MemHandle>& memHandleVec);
    HcclResult IsMemExist(EndpointHandle epHandle, bool& outExist);
    HcclResult IsDescExist(EndpointDesc epDesc, bool& outExist);
...
public:
    ...
    HcclResult DestroyAll();
    HcclResult Get(ChannelHandle handle, Channel*& outChannel);
    HcclResult Size(size_t& outSize) const;
    HcclResult GetChannelCount(size_t& outCount) const;
    HcclResult IsEmpty(bool& outIsEmpty) const;

private:
    std::unordered_map<ChannelHandle, std::unique_ptr<Channel>> channelMap_{};
    std::unordered_map<ChannelDeviceKey, ChannelHandle, ChannelDeviceKeyHash> channelD2HMap_{};
...
public:
    ...
    HcclResult DestroyAll();
    HcclResult Size(size_t& outSize) const;
    HcclResult GetThreadCount(size_t& outCount) const;
    HcclResult IsEmpty(bool& outIsEmpty) const;

#ifndef CCL_KERNEL_AICPU
    HcclResult SaveThreadsBatch(const std::vector<std::shared_ptr<hccl::Thread>>& newThreads);
    HcclResult FillThreadD2HMapBatch(ThreadHandle* deviceThreadHandles, ThreadHandle* hostThreadHandles, uint32_t listNum);
...
#endif
    HcclResult RegisterStreamThread(ThreadHandle handle, std::shared_ptr<hccl::Thread> threadPtr);
...
public:
    ...
    HcclResult DestroyAll();
    HcclResult GetCcuInstanceCount(size_t& outCount) const;
    HcclResult IsEmpty(bool& outIsEmpty) const;
...
public:
    ...
    HcclResult IsInitialized(bool& outIsInit) const;
    HcclResult GetRdmaHandleCount(size_t& outCount) const;
