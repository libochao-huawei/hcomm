...
HcclResult BaseCommRes::DestroyAll()
{
    if (destroyed_.exchange(true)) {
        HCCL_INFO("[BaseCommRes][%s] already destroyed, devPhyId=%u", __func__, devPhyId_);
        return HCCL_SUCCESS;
    }

    State expected = State::kReady;
    if (!TransitionState(expected, State::kDestroying)) {
        HCCL_ERROR("[BaseCommRes][%s] failed to transition state from Ready to Destroying, devPhyId=%u",
            __func__, devPhyId_);
        return HCCL_E_BUSY;
    }

    auto ret = DestroyInOrder();

    TransitionState(State::kDestroying, State::kDestroyed);
