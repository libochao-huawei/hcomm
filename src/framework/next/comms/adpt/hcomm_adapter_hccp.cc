    ...
    const CtxHandle ctxHandle, const HrtRaUbCreateJettyParam &in, HrtRaUbJettyCreatedOutParam &out)
{
    ...
    */
    ...
    HCCL_INFO("Create jetty, input params: attr.ub.jettyId[%u], attr.rqDepth[%u], "
        ...
        attr.ub.extMode.sq.buffVa, attr.ub.extMode.sq.buffSize, attr.ub.extMode.piType);
    ...
    void *&jettyHandle, RequestHandle &reqHandle)
{
    ...
    */
    ...
    s32 ret = RaCtxQpCreateAsync(
        ctxhandle, &attr, reinterpret_cast<QpCreateInfo *>(out.data()), &jettyHandle, &raReqHandle);
...
static HcclResult ImportJetty(const CtxHandle ctxHandle, u8 *key, const u32 keyLen, const u32 tokenValue,
    const JettyImportExpCfg &cfg, const JettyImportMode mode, const TpProtocol protocol,
...
static HcclResult ImportJettyAsync(CtxHandle ctxHandle, const HccpUbJettyImportedInParam &in, std::vector<char> &out,
    void *&remQpHandle, const JettyImportExpCfg &cfg, JettyImportMode mode, TpProtocol protocol,
    RequestHandle &reqHandle)
{
    ...
    if (protocol != TpProtocol::RTP && protocol != TpProtocol::CTP) {
        HCCL_ERROR("[%s] failed, tp protocol[%s] is not expected, %s.", __func__, protocol.Describe().c_str());
...
{
    HCCL_INFO("[%s]mode[%s], phyId[%u], customIn[%p], customOut[%p]", __func__, mode.Describe().c_str(), phyId,
        customIn, customOut);
    ...
    CHK_PRT_RET(ret != 0,
        HCCL_ERROR("[%s]RaCustomChannel fail, mode[%s], phyId[%u], customIn[%p], customOut[%p]", __func__,
            mode.Describe().c_str(), phyId, customIn, customOut),
        HCCL_E_NETWORK);
    return HCCL_SUCCESS;
}
...
HcclResult HccpRaGetDevBaseAttr(void *ctxHandle, struct DevBaseAttr *attr)
{
    ...
    HCCL_INFO("HccpRaGetDevBaseAttr success, sqMaxDepth[%u], rqMaxDepth[%u], sqMaxSge[%u], rqMaxSge[%u], "
