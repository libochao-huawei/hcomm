...
{
    HCCL_INFO(
        "[%s]mode[%s], phyId[%u], customIn[%p], customOut[%p]", __func__, mode.Describe().c_str(), phyId, customIn,
        customOut);
    ...
    CHK_PRT_RET(
        ret != 0,
        HCCL_ERROR(
            "[%s]RaCustomChannel fail, mode[%s], phyId[%u], customIn[%p], customOut[%p]", __func__,
