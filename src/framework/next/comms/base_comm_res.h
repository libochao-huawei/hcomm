...
public:
    ...
    SocketProcess* GetSocketProcess();
    TpMgr* GetTpMgr();
    HccpTlvHdcMgr* GetHccpTlvHdcMgr();
    EndpointMonitor* GetEndpointMonitor();

    EndpointsMgr* TryGetEndpointsMgr() const { return endpointsMgr_.get(); }
    ChannelsMgr* TryGetChannelsMgr() const { return channelsMgr_.get(); }
    ThreadsMgr* TryGetThreadsMgr() const { return threadsMgr_.get(); }
...
private:
    ...
    HcclResult LazyInitSocketProcess();
    HcclResult LazyInitTpMgr();
    HcclResult LazyInitHccpTlvHdcMgr();
    HcclResult LazyInitEndpointMonitor();
...
private:
    ...
    std::unique_ptr<SocketProcess> socketProcess_;
    std::unique_ptr<TpMgr> tpMgr_;
    std::unique_ptr<HccpTlvHdcMgr> hccpTlvHdcMgr_;
    std::unique_ptr<EndpointMonitor> endpointMonitor_;
