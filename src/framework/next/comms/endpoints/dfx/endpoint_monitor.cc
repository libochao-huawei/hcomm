...
{
    HCCL_INFO("RegisterToEndpointMonitor begin.");
    ...
    HCCL_INFO("RegisterToEndpointMonitor Completed.");
    return HCCL_SUCCESS;
}

HcclResult EndpointMonitor::RunMonitorThread()
{
    HCCL_INFO("[%s] Start EndpointMonitorThread.", __func__);
...
{
    HCCL_INFO("UnRegisterToEndpointMonitor begin.");
    {
        std::lock_guard<std::mutex> lock(threadLock_);
    ...
    CHK_RET(DeInit());
    initialized_ = false;
    HCCL_RUN_INFO("UnRegisterToEndpointMonitor completed.");
    return HCCL_SUCCESS;
}
...
bool EndpointMonitor::ProcessUbAsyncEvents()
{
    ...
    for (const auto &epHandle : handles) {
        ...
        CHK_PRT_RET(events == nullptr, HCCL_ERROR("[EndpointMonitor][%s] events is null", __func__, devPhyId_), false);

        HcclResult ret = HcommGetAsyncEvents(devPhyId_, epHandle, events.get(), num);
        ...
        if (ret == HCCL_E_NOT_SUPPORT) {
            ...
            HCCL_ERROR(
                "[EndpointMonitor][%s] devPhyId[%u] HcommGetAsyncEvents failed, ret[%d]", __func__, devPhyId_, ret);
            return false;
        }
    }
    return true;
...
void EndpointMonitor::PrintUbAsyncEventsContext(u32 devPhyId, const struct AsyncEvent &event)
{
    ...
    for (u32 i = 0; i < contextLen; i += bytesPerLine) {
        ...
        HCCL_ERROR("context[byte %u]: %02x%02x%02x%02x", endIndex, (i + 3 < contextLen) ? event.context[i + 3] : 0,
            (i + 2 < contextLen) ? event.context[i + 2] : 0, (i + 1 < contextLen) ? event.context[i + 1] : 0,
            event.context[i]);
    }
    HCCL_ERROR("********************************************************************************");
}
