...
void EndpointMonitor::RemoveEpHandleToEndpointMonitor(EndpointHandle epHandle)
{
    CHK_PRT_RET_NULL(epHandle == nullptr, HCCL_ERROR("[EndpointMonitor][%s] epHandle is null", __func__));
...
{
    HCCL_INFO("[EndpointMonitor] deviceId[%d] deinit begin.", deviceLogicId);
    ...
    HCCL_INFO("[EndpointMonitor] deviceId[%d] deinit end.", deviceLogicId);
    return HCCL_SUCCESS;
}
...
void EndpointMonitor::ProcessUbAsyncEvents()
{
    ...
    for (const auto &epHandle : epHandleSet_) {
        ...
        if (ret != HCCL_SUCCESS) {
            ...
            HCCL_ERROR("[EndpointMonitor][%s] devPhyId[%u] HcommGetAsyncEvents failed ret[%d], "
                ...
                __func__, devPhyId_, ret, epHandle);
            return;
        }
