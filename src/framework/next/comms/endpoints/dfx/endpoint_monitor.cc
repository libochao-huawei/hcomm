...
HcclResult EndpointMonitor::RunMonitorThread()
{
    ...
    exitFuture_ = exitSignal.get_future();
    endpointMonitorThread_.reset(
        new (std::nothrow) std::thread(&EndpointMonitor::MonitorThread, this, std::move(exitSignal)));
    CHK_SMART_PTR_NULL(endpointMonitorThread_);
    initialized_ = true;
    return HCCL_SUCCESS;
}

void EndpointMonitor::MonitorThread(std::promise<void> exitSignal)
{
    ...
    try {
        ...
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[EndpointMonitor][%s] hrtResetDevice failed, deviceLogicId[%d], ret[%d]", __func__,
                deviceLogicId_, ret);
        }
    } catch (const std::exception &e) {
        HCCL_ERROR("[EndpointMonitor][%s] thread exception: %s", __func__, e.what());
    } catch (...) {
        HCCL_ERROR("[EndpointMonitor][%s] thread unknown exception", __func__);
    }
    exitSignal.set_value();
}
...
HcclResult EndpointMonitor::DeInit()
{
    ...
    if (endpointMonitorThread_ == nullptr) {
        return HCCL_SUCCESS;
    }

    if (endpointMonitorThread_->joinable()) {
        constexpr u32 JOIN_TIMEOUT_S = 5;
        if (exitFuture_.wait_for(std::chrono::seconds(JOIN_TIMEOUT_S)) == std::future_status::timeout) {
            HCCL_WARNING("[EndpointMonitor][%s] thread[%p] join timeout, detach", __func__, endpointMonitorThread_);
            try {
                endpointMonitorThread_->detach();
            } catch (const std::exception &e) {
                HCCL_ERROR("[EndpointMonitor][%s] detach failed: %s", __func__, e.what());
                return HCCL_E_INTERNAL;
            }
        } else {
            try {
                HCCL_INFO("[EndpointMonitor][%s] thread[%p] join", __func__, endpointMonitorThread_);
...
void EndpointMonitor::ProcessUbAsyncEvents()
{
    ...
    for (const auto &epHandle : handles) {
        ...
        HCCL_INFO("[EndpointMonitor][%s] devPhyId[%u] events[%p] num[%d] out ", __func__, devPhyId_, events_, num);
        if (ret != HCCL_SUCCESS) {
            {
                std::lock_guard<std::mutex> lock(threadLock_);
