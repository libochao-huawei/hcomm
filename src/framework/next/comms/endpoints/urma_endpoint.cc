...
HcclResult UrmaEndpoint::Init()
{
    ...
    if (endpointDesc_.loc.locType != ENDPOINT_LOC_TYPE_DEVICE) {
        HCCL_ERROR("[UrmaEndpoint][%s] endpointDesc.loc.locType[%d] only support "
            "ENDPOINT_LOC_TYPE_DEVICE", __func__, endpointDesc_.loc.locType);
        return HCCL_E_PARA;
    }
    ...
    if (endpointDesc_.loc.device.devPhyId != devPhyId) {
        HCCL_WARNING("[UrmaEndpoint][%s] endpointDesc.loc.device.devPhyId[%u] incorrect", __func__,
    ...
    EXECEPTION_CATCH(ctxHandle_ = static_cast<void *>(rdmaHandleMgr.GetByIp(endpointDesc_.loc.device.devPhyId, ipAddr)),
        return HCCL_E_PARA);
    CHK_PTR_NULL(ctxHandle_);
    HCCL_INFO("[UrmaEndpoint][%s] success, devId[%u], ipAddr[%s], ctxHandle[%p]", __func__, devPhyId,
    ...
    // ccu模式专用的资源分配器
    ccuChannelCtxPool_.reset(new (std::nothrow) CcuChannelCtxPool(deviceLogicId, endpointDesc_.commAddr));
...
{
    if (endpointDesc_.loc.locType != ENDPOINT_LOC_TYPE_DEVICE) {
        HCCL_INFO("[UrmaEndpoint][%s] endpointDesc.loc.locType[%d] skip create ServerSocket", __func__,
    ...
    HCCL_INFO("[UrmaEndpoint][%s] devicePhyId[%u] localPort[%s]", __func__, endpointDesc_.loc.device.devPhyId,
...
HcclResult UrmaEndpoint::ServerSocketGetListenPort(uint32_t *port)
{
    ...
    HCCL_INFO("[UrmaEndpoint][%s] devicePhyId[%u] localPort[%s]",
        __func__, endpointDesc_.loc.device.devPhyId,
        localPort.Describe().c_str());
