...
HcclResult UrmaEndpoint::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if(this->endpointDesc_.loc.locType == ENDPOINT_LOC_TYPE_HOST) {
        ...
        int ret = halMemRegUbSegment(devLogicId, static_cast<uint64_t>(mem.addr), mem.size);
        if(ret) {
...
HcclResult UrmaEndpoint::UnregisterMemory(void* memHandle)
{
    ...
    if(this->endpointDesc_.loc.locType == ENDPOINT_LOC_TYPE_HOST) {
        // 获取设备逻辑id
        uint32_t devLogicId = HcclGetThreadDeviceId();
        int ret = halMemUnRegUbSegment(devLogicId, static_cast<uint64_t>(bufferInfo.first), bufferInfo.second);
        if(ret) {
