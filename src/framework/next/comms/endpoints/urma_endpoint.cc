...
HcclResult UrmaEndpoint::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if(this->endpointDesc_.loc.locType == ENDPOINT_LOC_TYPE_HOST) {
        ...
        s32 deviceLogicId;
        CHK_RET(hrtGetDevice(&deviceLogicId));
        int ret = halMemRegUbSegment(deviceLogicId, static_cast<uint64_t>(mem.addr), mem.size);
        if(ret) {
...
HcclResult UrmaEndpoint::UnregisterMemory(void* memHandle)
{
    ...
    if(this->endpointDesc_.loc.locType == ENDPOINT_LOC_TYPE_HOST) {
        ...
        s32 deviceLogicId;
        CHK_RET(hrtGetDevice(&deviceLogicId));
        int ret = halMemUnRegUbSegment(deviceLogicId, static_cast<uint64_t>(bufferInfo.first), bufferInfo.second);
        if(ret) {
