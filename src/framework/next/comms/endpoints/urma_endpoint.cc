...
HcclResult UrmaEndpoint::Init()
{
    ...
    HCCL_INFO("[UrmaEndpoint][%s] Hccl::HccpHdcManager::GetInstance().Init deviceLogicId[%d]",__func__,deviceLogicId);
    EXECEPTION_CATCH(Hccl::HccpHdcManager::GetInstance().Init(deviceLogicId), return HCCL_E_INTERNAL);
    HCCL_INFO("[UrmaEndpoint][%s] Hccl::HccpHdcManager::GetInstance().Init finish",__func__);
    u32 devPhyId;
    ...
    HCCL_INFO("[UrmaEndpoint][%s] start get rdmaHandleMgr",__func__);

    auto &rdmaHandleMgr = Hccl::RdmaHandleManager::GetInstance();
    HCCL_INFO("[UrmaEndpoint][%s] end get rdmaHandleMgr[%p]",__func__,&rdmaHandleMgr);
    ...
    HCCL_INFO("[UrmaEndpoint][%s] end get ctxHandle_[%p]",__func__,ctxHandle_);
    HCCL_INFO("[UrmaEndpoint][%s] endpointDesc_.loc.device.devPhyId[%u], ipAddr[%s], ctxHandle[%p]",
        __func__, endpointDesc_.loc.device.devPhyId, ipAddr.Describe().c_str(), ctxHandle_);
    CHK_PTR_NULL(ctxHandle_);
    HCCL_INFO("[UrmaEndpoint][%s] success, endpointDesc_.loc.device.devPhyId[%u], ipAddr[%s], ctxHandle[%p]",
        __func__, endpointDesc_.loc.device.devPhyId, ipAddr.Describe().c_str(), ctxHandle_);

    CHK_RET(ServerSocketListen(DEFAULT_LISTENING_PORT));
...
HcclResult UrmaEndpoint::ServerSocketListen(const uint16_t port)
{
    if (endpointDesc_.loc.locType != ENDPOINT_LOC_TYPE_DEVICE){
    ...
    EXECEPTION_CATCH(serverSocket = std::make_unique<Hccl::Socket>(socketHandle, ipaddr, port, ipaddr, "server", Hccl::SocketRole::SERVER, Hccl::NicType::DEVICE_NIC_TYPE), return HCCL_E_PARA); //端口号可能冲突，需要SE做决定
