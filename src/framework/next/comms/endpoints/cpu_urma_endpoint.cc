...
HcclResult CpuUrmaEndpoint::ServerSocketListen()
{
    Hccl::IpAddress ipAddr{};
    CHK_RET(CommAddrToIpAddress(endpointDesc_.commAddr, ipAddr));

    auto &serverSocketMap = CpuUrmaEndpoint::GetServerSocketMap();
    s32 devId = 0;
    CHK_RET(hrtGetDevice(&devId));
    u32 devPhyId = 0;
    CHK_RET(hrtGetDevicePhyIdByIndex(devId, devPhyId));
    ...
    if (serverSocketMap.find(ipAddr) != serverSocketMap.end()) {
        HCCL_INFO("[CpuUrmaEndpoint::%s] reuse serverSocket", __func__);
        return HCCL_SUCCESS;
    }
    ...
    HCCL_INFO("[CpuUrmaEndpoint::%s] socketHandle[%p] devicePhyId[%u] ipAddress[%s]",
    ...
    HCCL_INFO("[CpuUrmaEndpoint::%s] listen_socket_info[%s]", __func__, serverSocket->Describe().c_str());
...
std::unordered_map<Hccl::IpAddress, std::shared_ptr<Hccl::Socket>> &CpuUrmaEndpoint::GetServerSocketMap()
{
...
CcuChannelCtxPool *CpuUrmaEndpoint::GetCcuChannelCtxPool()
{
