...
HcclResult CpuRoceEndpoint::GetCapabilities(Capabilities &caps)
{
    ...
    if (!isCapabilitiesAvailable_) {
        ...
        uint32_t ret = RaGetLbMax(rdmaHandle_, &(Capabilities_.lbMax));
        CHK_PRT_RET(ret != 0,
            HCCL_ERROR("[HostCpuRoceChannel::BuildConnection][GetLbMax]errNo[0x%016llx] RaGetLbMax fail. "
            "return[%d], params: rdmaHandle[%p], lbMax[%u]",
            HCCL_ERROR_CODE(HCCL_E_NETWORK), ret, rdmaHandle_, Capabilities_.lbMax),
            HCCL_E_NETWORK);
