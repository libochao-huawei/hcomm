...
HcclResult CpuRoceEndpoint::ServerSocketListen(const uint16_t port)
{
    HCCL_INFO("[CpuRoceEndpoint::%s] START. port[%u].", __func__, port);
    ...
    EXECEPTION_CATCH(
        serverSocket = std::make_shared<Hccl::Socket>(socketHandle, ipAddr, port, ipAddr, "server",  // 端口号可能冲突，需要SE做决定
...
std::unordered_map<Hccl::IpAddress, std::unordered_map<uint16_t, std::shared_ptr<Hccl::Socket>>> &CpuRoceEndpoint::GetServerSocketMap()
{
    // 确保 HostSocketHandleManager 在 serverSocketMap 之前构造，
    // 以确保 HostSocketHandleManager 在 serverSocketMap 之后析构。
    Hccl::HostSocketHandleManager::GetInstance();
    static std::unordered_map<Hccl::IpAddress, std::unordered_map<uint16_t, std::shared_ptr<Hccl::Socket>>, std::hash<Hccl::IpAddress>>
        serverSocketMap;
