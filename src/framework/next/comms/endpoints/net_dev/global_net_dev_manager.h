...
private:
    ...
    HcclResult ServerSocketListen(const HcclNetDevCtx netDevCtx, const uint32_t port);
    HcclResult ServerSocketStop(const uint32_t port);
...
private:
    ...
    std::unique_ptr<hccl::HcclSocket> serverSocket_;
    u32 serverPort_;
