...
void GlobalNetDevMgr::UnInit()
{
    ...
    for (auto& pair : netDevCtxMap_) {
        if (pair.second.first == NicType::DEVICE_NIC_TYPE) {
            (void)ServerSocketStop(pair.first.listenPort);
        }
...
HcclResult GlobalNetDevMgr::ServerInit(const HcclNetDevCtx netDevCtx, u32 port)
{
    ...
    if (netDevCtxTmp->GetNicType() == NicType::DEVICE_NIC_TYPE) {
        HcclResult ret = ServerSocketListen(port);
        if (ret != HCCL_SUCCESS) {
...
HcclResult GlobalNetDevMgr::ServerSocketListen(const uint32_t port)
{
    ...
    HCCL_INFO("[GlobalNetDevMgr][%s] listen on port[%u] success", __func__, listenPort);
    return HCCL_SUCCESS;
}

HcclResult GlobalNetDevMgr::ServerSocketStop(const uint32_t port)
{
    uint32_t listenPort = port;
    auto socketIt = serverSocketMap_.find(port);
    if (socketIt == serverSocketMap_.end() || it->second == nullptr) {
        HCCL_INFO("[GlobalNetDevMgr::%s] reuse serverSocket for port[%u]", __func__, port);
        return HCCL_SUCCESS;
    }

    socketIt->second->StopListen();
...
{
    socketTag = tagServerIp.GetReadableIP() + std::string(":") +
        std::to_string(tagPort) + ":" + tagClientIp.GetReadableIP();
}

HcclResult GlobalNetDevMgr::ConnectToServer(const HcclNetDevCtx netDevCtx, hccl::HcclIpAddress remoteIp,
    uint32_t remotePort, std::make_shared<hccl::HcclSocket> &socket)
{
    ...
    uint32_t localPort = netDevCtxPtr->GetLocalPort();

    std::string socketTag;
    MakeSocketTag(remoteIp, remotePort, localIpAddr, socketTag);
    ...
    std::make_shared<hccl::HcclSocket> socketTemp = nullptr;
...
HcclResult GlobalNetDevMgr::AcceptClient(const HcclNetDevCtx netDevCtx, hccl::HcclIpAddress remoteIp,
   std::make_shared<hccl::HcclSocket> &socket)
{
    ...
    std::string socketTag;
    MakeSocketTag(localIpAddr, localPort, remoteIp, socketTag);
    HCCL_INFO("[GlobalNetDevMgr]AcceptClient localIp[%s] localPort[%u] remoteIp[%s] remotePort[%u] socketTag[%s]",
        localIpAddr.GetReadableIP(), localPort, remoteIp.GetReadableIP(), remotePort, socketTag.c_str());
...
void GlobalNetDevMgr::CloseSocket(std::make_shared<hccl::HcclSocket> &socket)
{
    if (socket != nullptr) {
        socket->Close();
        socket = nullptr;
