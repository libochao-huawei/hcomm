    ...
    HcclNetDevCtx &netDevCtx)
{
    ...
    if (nicType == NicType::DEVICE_NIC_TYPE && port != 0) {
        HcclResult ret = ServerSocketListen(tempNetDevCtx, port);
        if (ret != HCCL_SUCCESS) {
...
HcclResult GlobalNetDevMgr::ServerInit(const HcclNetDevCtx netDevCtx, u32 port)
{
    ...
    if (netDevCtxTmp->GetNicType() == NicType::DEVICE_NIC_TYPE) {
        HcclResult ret = ServerSocketListen(netDevCtx, port);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
    }
...
HcclResult GlobalNetDevMgr::ServerSocketListen(const HcclNetDevCtx netDevCtx, const uint32_t port)
{
