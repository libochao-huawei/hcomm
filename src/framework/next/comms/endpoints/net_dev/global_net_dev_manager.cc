...
HcclResult GlobalNetDevMgr::Init(u32 devicePhyId, u32 deviceLogicId)
{
    ...
    if (netDevMgrInstance[deviceLogicId].isInited_) {
        HCCL_INFO(
            "[GlobalNetDevMgr][Init]Has been inited. devicePhyId[%u], deviceLogicId[%d]", devicePhyId, deviceLogicId);
        return HCCL_SUCCESS;
    }
    ...
    HCCL_INFO("[GlobalNetDevMgr][Init]Init success, devicePhyId[%u], deviceLogicId[%d]", devicePhyId, deviceLogicId);
    return HCCL_SUCCESS;
}
...
HcclResult GlobalNetDevMgr::GetDeviceVnicIP(u32 devicePhyId, u32 superDeviceId, hccl::HcclIpAddress& vnicIP)
{
    ...
    HCCL_DEBUG(
        "[GlobalNetDevMgr][%s]GetDeviceVnicIP, deviceLogicId[%u], devicePhyId[%u], "
        "nicDeploy[%d], hasBackup[%d], tempDevicePhyId[%u]",
        __func__, localDeviceLogicId, devicePhyId, NICDeployment::NIC_DEPLOYMENT_DEVICE, false, tempDevicePhyId);
    CHK_RET(
...
GlobalNetDevMgr::RefNetDevCtx(NicType nicType, const HcclIpAddress& ipAddr, u32 port, HcclNetDevCtx& netDevCtx)
{
    ...
    if (netDevCtxMap_.find(portInfo) != netDevCtxMap_.end()) {
        ...
        HCCL_INFO(
            "[GlobalNetDevMgr][RefNetDevCtx] nicType[%d] ip[%s] has been Ref.", nicType, ipAddr.GetReadableAddress());
        return HCCL_SUCCESS;
    }
...
HcclResult GlobalNetDevMgr::UnRefNetDevCtx(NicType nicType, const HcclIpAddress& ipAddr, u32 port)
{
    ...
    if (netDevCtxMap_.find(portInfo) != netDevCtxMap_.end()) {
        ...
        netDevCtxRef.Unref();
        HCCL_INFO(
            "[GlobalNetDevMgr][UnRefNetDevCtx] nicType[%d] ip[%s] has been UnRef.", nicType,
        ...
        if (netDevCtxRef.Count() == 0) {
            ...
            HCCL_INFO(
                "[GlobalNetDevMgr][UnRefNetDevCtx] nicType[%d] ip[%s] has been Deinit.", nicType,
                ipAddr.GetReadableAddress());
        }
    }
...
HcclResult GlobalNetDevMgr::ServerInit(u32 port)
{
    ...
    EXECEPTION_CATCH((tempSocket = std::make_shared<HcclSocket>(netDevCtx_, port)), return HCCL_E_PTR);
    ...
    u64 id)
{
    socketTag = tagServerIp.GetReadableIP() + std::string(":") + std::to_string(tagServerPort) + std::string(":")
                + tagClientIp.GetReadableIP() + std::string(":") + std::to_string(id);
}
