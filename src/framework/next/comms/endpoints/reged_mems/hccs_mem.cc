...
HccsRegedMemMgr::HccsRegedMemMgr(HcclNetDevCtx netDevCtx_)
{
    netDevCtx_ = netDevCtx_;
}
    ...
    EndpointDesc &endpointDesc, std::string &ipcRmaBufferDesc)
{
    ...
    if (memcpy_s(&endpointDesc, sizeof(EndpointDesc), description, sizeof(EndpointDesc)) != EOK) {
