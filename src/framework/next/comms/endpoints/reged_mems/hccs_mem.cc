    ...
    void **memDesc, uint32_t *descLen)
{
    ...
    if(memcpy_s(ipcRmaBufferDesc.c_str() + (ipcRmaBufferDesc.length() - sizeof(EndpointDesc)), sizeof(EndpointDesc),
    ...
    *memDesc = const_cast<void *>(ipcRmaBufferDesc.c_str());
    return HCCL_SUCCESS;
}
...
/*
    ...
    std::string ipcRmaBufferDesc2;
    ipcRmaBufferDesc2.resize(*memDescLen);
    CHK_RET(DeSerializeFromMemDesc(*memDesc, *memDescLen, endpointDesc2, ipcRmaBufferDesc2));
    HCCL_INFO("[%s] ipcRmaBufferDesc2.len[%u]", __FUNCTION__, ipcRmaBufferDesc2.length());
    ...
    CHK_RET(remoteIpcRmaBuffer->Deserialize(ipcRmaBufferDesc2));
*/
...
HcclResult HccsRegedMemMgr::MemoryGrant(void* memHandle, const HcommMemGrantInfo *remoteGrantInfo)
{
    HCCL_INFO("[%s] Begin", __FUNCTION__);
    CHK_PTR_NULL(memHandle);
