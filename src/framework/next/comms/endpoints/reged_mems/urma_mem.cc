...
HcclResult UbRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        // 精确重复（用户传入的 addr/size 和已有 buffer 一致）：复用
        ...
        localUbRmaBufferMgr_->AddWithoutCheck(realHwKey, existingBuffer);  // ref++

        auto virtBuffer = std::make_shared<Hccl::VirtualLocalUbRmaBuffer>(existingBuffer, reqAddr, mem.size);
