...
HcclResult UbRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    if (!resultPair) {
        HCCL_INFO(
            "[UbRegedMemMgr][[UnregisterMemory] Memory reference count is larger than 0"
...
HcclResult UbRegedMemMgr::MemoryImport(const void* memDesc, uint32_t descLen, HcommMem* outMem)
{
    ...
    EXECEPTION_CATCH(
        remoteUbRmaBuffer = std::make_shared<Hccl::RemoteUbRmaBuffer>(this->rdmaHandle_, dto), return HCCL_E_PTR;);
    ...
    if (remoteUbRmaBufferMgrs_.find(endpointDesc) == remoteUbRmaBufferMgrs_.end()) {
        ...
        EXECEPTION_CATCH((remoteUbRmaBufferMgr = std::make_unique<RemoteUbRmaBufferMgr>()), return HCCL_E_PTR);
...
HcclResult UbRegedMemMgr::MemoryUnimport(const void* memDesc, uint32_t descLen)
{
    ...
    if (!resultPair) {
        HCCL_INFO(
            "[UrmaRegedMemMgr][[MemoryUnimport] Memory reference count is larger than 0"
