...
HcclResult UbRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        if (existingBuffer->GetAddr() == reqAddr &&
            ...
            return RegExactDup(*localUbRmaBufferMgr_, existingHwKey, existingBuffer, memHandle);
        }
        return RegSubset(*localUbRmaBufferMgr_, existingBuffer, allRegisteredBuffers_,
