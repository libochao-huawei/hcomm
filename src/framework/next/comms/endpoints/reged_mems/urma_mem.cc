...
#include "../../../../../../legacy/unified_platform/resource/buffer/virtual_local_rma_buffer.h"
...
HcclResult UbRegedMemMgr::RegNew(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    *memHandle = static_cast<void *>(localBuffer.get());
    ...
    if (resultPair.second) {
        HCCL_INFO("[UbRegedMemMgr][RegNew] new HW registration success! "
        ...
        HCCL_INFO("[UbRegedMemMgr][RegNew] HW range already registered (unexpected), ref++");
        return HCCL_SUCCESS;
    }
...
HcclResult UbRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    auto findPair = localUbRmaBufferMgr_->Find(origKey);
    if (!findPair.first) {
        findPair = localUbRmaBufferMgr_->Find(
            hccl::BufferKey<uintptr_t, u64>(hwRange.first, hwRange.second));
    }
    ...
    if (findPair.first) {
        ...
        return RegSubset<LocalUbRmaBufferMgr, Hccl::LocalUbRmaBuffer, Hccl::VirtualLocalUbRmaBuffer>(
            *localUbRmaBufferMgr_, existingBuffer, allRegisteredBuffers_,
            existingHwKey, memHandle, reqAddr, mem.size, memTag);
    }

    return RegNew(mem, memTag, memHandle);
}
