...
HcclResult UbRegedMemMgr::RegExactDup(HcommMem mem,
                                       ...
                                       void **memHandle)
{
    ...
    *memHandle = static_cast<void *>(existingBuffer.get());
    HCCL_INFO("[UbRegedMemMgr][RegExactDup] exact duplicate, ref++, key {%p, %llu}", mem.addr, mem.size);
    return HCCL_SUCCESS;
}

HcclResult UbRegedMemMgr::RegSubset(HcommMem mem, const char *memTag,
                                     std::shared_ptr<Hccl::LocalUbRmaBuffer> &existingBuffer,
                                     void **memHandle)
{
    ...
    *memHandle = static_cast<void *>(virtBuffer.get());
    HCCL_INFO("[UbRegedMemMgr][RegSubset] virtual UB buffer for subset {%p, %llu} "
...
HcclResult UbRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (!findPair.first) {
        findPair = localUbRmaBufferMgr_->DirectFind(
