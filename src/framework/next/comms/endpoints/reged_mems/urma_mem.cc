...
HcclResult UbRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        AddRefToParent(*localUbRmaBufferMgr_, localUbRmaBuffer);

        Hccl::HrtRaUbLocalMemRegOutParam memRegOutParam = localUbRmaBuffer->GetMemRegOutParam();
        ...
        (void)localUbRmaBufferMgr_->AddWithoutCheck(actualRegKey, localUbRmaBuffer);
    }
...
HcclResult UbRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    //   - 子buffer (IsAlias()=true):  自己的key不在tree中 → 通过handle找父key做Del
