...
HcclResult UbRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    RegisterOrAlias(*localUbRmaBufferMgr_, findPair, localUbRmaBuffer,
        [&](auto& parent) {
            return std::make_shared<Hccl::LocalUbRmaBuffer>(localBufferPtr, this->rdmaHandle_,
                parent->GetMemRegOutParam());
        },
        [&]() {
            return std::make_shared<Hccl::LocalUbRmaBuffer>(localBufferPtr, this->rdmaHandle_);
        });
...
HcclResult UbRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    if (buffer->IsAlias()) {
        refBuffer = ResolveAliasParent(this->localUbRmaBufferMgr_, ownKey, buffer, allRegisteredBuffers_,
            [](auto* b) { return b->GetMemRegOutParam().handle; });
    }
