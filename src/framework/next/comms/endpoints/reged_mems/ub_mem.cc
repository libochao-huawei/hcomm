...
HcclResult UbMemRegedMemMgr::RegisterMemory(HcommMem mem, const char* memTag, void** memHandle)
{
    ...
    if (findPair.first) {
        ...
        EXECEPTION_CATCH(
            ...
            return HCCL_E_PTR);
        EXECEPTION_CATCH(
            (localIpcRmaBuffer = std::make_shared<Hccl::LocalIpcRmaBuffer>(localBufferPtr)), return HCCL_E_PTR);
    }
    ...
    if (resultPair.first == localIpcRmaBufferMgr_->End()) {
        // 注册内存有交叉
        HCCL_ERROR(
            "[%s] The memory {addr[%p], size[%llu]} overlaps with the memory that has been registered.", __func__,
            mem.addr, mem.size);
    ...
    *memHandle = static_cast<void*>(localBuffer.get());
    ...
    if (!resultPair.second) {
        HCCL_INFO(
            "[%s] Memory {addr[%p], size[%llu]} is already registered, just increase the reference count.", __func__,
            mem.addr, mem.size);
        return HCCL_SUCCESS;
    }
