...
HcclResult UbMemRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        if (existingBuffer->GetAddr() == addr && existingBuffer->GetSize() == size) {
            ...
            auto resultPair = localIpcRmaBufferMgr_->AddWithoutCheck(tempKey, localIpcRmaBuffer);
            ...
            *memHandle = static_cast<void *>(localBuffer.get());
            ...
            HCCL_INFO("[%s] alias created for subset {addr[%p], size[%llu]} within parent {addr[%p], size[%llu]}",
                __func__, mem.addr, mem.size, reinterpret_cast<void *>(existingBuffer->GetAddr()),
...
HcclResult UbMemRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    if (buffer->IsAlias()) {
        ...
        bool dummy = false;
        EXECEPTION_CATCH(dummy = localIpcRmaBufferMgr_->Del(aliasKey), return HCCL_E_NOT_FOUND);
