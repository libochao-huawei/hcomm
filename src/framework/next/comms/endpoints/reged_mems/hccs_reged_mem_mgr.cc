...
HcclResult HccsRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        hccl::BufferKey<uintptr_t, u64> actualRegKey(reinterpret_cast<uintptr_t>(parentBuffer->GetAddr()),
            static_cast<uint64_t>(parentBuffer->GetSize()));
        (void)localIpcRmaBufferMgr->AddWithoutCheck(actualRegKey, parentBuffer);
        ...
        hccl::BufferKey<uintptr_t, u64> actualRegKey(reinterpret_cast<uintptr_t>(localIpcRmaBuffer->GetAddr()),
            static_cast<uint64_t>(localIpcRmaBuffer->GetSize()));
        (void)localIpcRmaBufferMgr->AddWithoutCheck(actualRegKey, localIpcRmaBuffer);
