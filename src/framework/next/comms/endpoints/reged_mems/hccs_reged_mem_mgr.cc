...
HcclResult HccsRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    HCCL_INFO(
        "[%s] addr[%p] size[%u] memHandle[%p] allRegisteredBuffers_.size[%d] Not Found", __FUNCTION__, addr, size,
        memHandle, allRegisteredBuffers_.size());
    ...
    std::string& ipcRmaBufferDesc, std::shared_ptr<hccl::RemoteIpcRmaBuffer>& remoteIpcRmaBuffer)
{
    ...
    EXECEPTION_CATCH(remoteIpcRmaBuffer = std::make_shared<hccl::RemoteIpcRmaBuffer>(netDevCtx_), return HCCL_E_PTR;);
