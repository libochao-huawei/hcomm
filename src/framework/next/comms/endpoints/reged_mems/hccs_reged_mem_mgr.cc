...
HcclResult HccsRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        return RegSubset<LocalIpcRmaBufferMgr, hccl::LocalIpcRmaBuffer, hccl::VirtualLocalIpcRmaBuffer>(
            *localIpcRmaBufferMgr, existingBuffer, allRegisteredBuffers_,
            existingHwKey, memHandle, mem.addr, static_cast<u64>(mem.size), memTag);
    }
...
HcclResult HccsRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    hccl::BufferKey<uintptr_t, u64> hwKey(reinterpret_cast<uintptr_t>(localIpcRmaBuffer->GetAlignedAddr()),
