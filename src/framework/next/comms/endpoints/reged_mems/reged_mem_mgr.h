...
protected:
    ...
    static HcclResult AddRefToParent(Mgr& mgr, const BufferPtr& buffer)
    {
        ...
        MakeAlias&& makeAlias, MakeNew&& makeNew)
    {
        if (findPair.first) {
            ...
            CHK_RET(AddRefToParent(mgr, parentBuffer));
        } else {
            EXECEPTION_CATCH((buffer = makeNew()), return HCCL_E_PTR);
            CHK_RET(AddRefToParent(mgr, buffer));
        }
        return HCCL_SUCCESS;
    }
    ...
    template <typename Mgr, typename RmaBuffer, typename HwHandleFn>
    static HcclResult UnregisterMemoryImpl(void* memHandle, Mgr& mgr,
        std::vector<std::shared_ptr<RmaBuffer>>& allBuffers, HwHandleFn&& hwHandleGetter)
    {
        CHK_PTR_NULL(memHandle);
