...
HcclResult RoceRegedMemMgr::RegExactDup(HcommMem mem,
                                         ...
                                         void **memHandle)
{
    ...
    if (resultPair.first == localRdmaRmaBufferMgr_->End()) {
        HCCL_ERROR("[RoceRegedMemMgr][RegExactDup] overlap error.");
    ...
    *memHandle = static_cast<void *>(localBuffer.get());
    HCCL_INFO("[RoceRegedMemMgr][RegExactDup] exact duplicate, ref++, key {%p, %llu}", mem.addr, mem.size);
    return HCCL_SUCCESS;
}

HcclResult RoceRegedMemMgr::RegSubset(HcommMem mem, const char *memTag,
                                       std::shared_ptr<Hccl::LocalRdmaRmaBuffer> &existingBuffer,
                                       void **memHandle)
{
    ...
    *memHandle = static_cast<void *>(virtBuffer.get());
    HCCL_INFO("[RoceRegedMemMgr][RegSubset] virtual RDMA buffer for subset {%p, %llu} "
