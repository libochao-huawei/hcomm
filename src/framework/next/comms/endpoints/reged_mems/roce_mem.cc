...
HcclResult RoceRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    RegisterOrAlias(*localRdmaRmaBufferMgr_, findPair, localRdmaRmaBuffer,
        [&](auto& parent) {
            return std::make_shared<Hccl::LocalRdmaRmaBuffer>(localBufferPtr, this->rdmaHandle_,
...
HcclResult RoceRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    if (buffer->IsAlias()) {
        refBuffer = ResolveAliasParent(this->localRdmaRmaBufferMgr_, ownKey, buffer, allRegisteredBuffers_,
