...
HcclResult AicpuTsRoceRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        HCCL_INFO("[AicpuTsRoceRegedMemMgr][RegisterMemory] Find hit, reuse buffer mgr entry key {%p, %llu}", mem.addr, mem.size);
    } else {
...
HcclResult AicpuTsRoceRegedMemMgr::GetAllRoceMemDetails(std::vector<RoceMemDetails> &localOut,
