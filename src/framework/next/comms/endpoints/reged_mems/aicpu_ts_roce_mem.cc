...
HcclResult AicpuTsRoceRegedMemMgr::RegExactDup(HcommMem mem,
                                                hccl::BufferKey<uintptr_t, u64> &tempKey,
                                                std::shared_ptr<hccl::LocalRdmaRmaBuffer> &existingBuffer,
                                                void **memHandle)
{
    auto resultPair = localRdmaRmaBufferMgr_->Add(tempKey, existingBuffer);
    if (resultPair.first == localRdmaRmaBufferMgr_->End()) {
        HCCL_ERROR("[AicpuTsRoceRegedMemMgr][RegExactDup] overlap error");
    ...
    *memHandle = static_cast<void *>(localBuffer.get());
    ...
    HCCL_INFO("[AicpuTsRoceRegedMemMgr][RegExactDup] exact duplicate, ref++, key {%p, %llu}", mem.addr, mem.size);
    return HCCL_SUCCESS;
}

HcclResult AicpuTsRoceRegedMemMgr::RegSubset(HcommMem mem, const char *memTag,
                                              std::shared_ptr<hccl::LocalRdmaRmaBuffer> &existingBuffer,
                                              void **memHandle)
{
    hccl::BufferKey<uintptr_t, u64> realHwKey(existingBuffer->GetAlignedAddr(),
        existingBuffer->GetAlignedSize());
    localRdmaRmaBufferMgr_->AddToTree(realHwKey, existingBuffer);
    ...
    *memHandle = static_cast<void *>(virtBuffer.get());
    HCCL_INFO("[AicpuTsRoceRegedMemMgr][RegSubset] virtual buffer for subset {%p, %llu} "
...
HcclResult AicpuTsRoceRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        return RegExactDup(mem, tempKey, existingBuffer, memHandle);
    }
