...
void AicpuTsRoceRegedMemMgr::TrackRegisteredBuffer(const std::shared_ptr<hccl::LocalRdmaRmaBuffer>& localBuffer)
{
    ...
    const bool alreadyListed = std::any_of(
        ...
        [handlePtr](const std::shared_ptr<hccl::LocalRdmaRmaBuffer>& p) {
            return static_cast<void*>(p.get()) == handlePtr;
        });
    if (alreadyListed) {
        return;
    }
...
HcclResult AicpuTsRoceRegedMemMgr::GetOrCreateLocalRdmaRmaBuffer(
    hccl::NetDevContext* netDevCtx, HcommMem mem, hccl::BufferKey<uintptr_t, u64> tempKey,
    std::shared_ptr<hccl::LocalRdmaRmaBuffer>& localRdmaRmaBuffer)
{
    ...
    if (findPair.first) {
        ...
        HCCL_INFO(
            "[AicpuTsRoceRegedMemMgr][RegisterMemory] Find hit, reuse buffer mgr entry key {%p, %llu}", mem.addr,
            mem.size);
        return HCCL_SUCCESS;
    }
    HCCL_INFO(
        "[AicpuTsRoceRegedMemMgr][RegisterMemory] Find miss, construct LocalRdmaRmaBuffer key {%p, %llu} type[%u]",
...
HcclResult AicpuTsRoceRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    allRegisteredBuffers_.erase(
        std::remove_if(
            allRegisteredBuffers_.begin(), allRegisteredBuffers_.end(),
            [memHandle](const std::shared_ptr<hccl::LocalRdmaRmaBuffer>& p) {
                return p.get() == memHandle;
            }),
