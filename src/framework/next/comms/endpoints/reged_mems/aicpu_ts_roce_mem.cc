...
HcclResult AicpuTsRoceRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        hccl::BufferKey<uintptr_t, u64> actualRegKey(
            reinterpret_cast<uintptr_t>(parentBuffer->GetAddr()),
            static_cast<u64>(parentBuffer->GetSize()));
        (void)localRdmaRmaBufferMgr_->AddWithoutCheck(actualRegKey, parentBuffer);
        ...
        hccl::BufferKey<uintptr_t, u64> actualRegKey(
            reinterpret_cast<uintptr_t>(localRdmaRmaBuffer->GetAddr()),
...
HcclResult AicpuTsRoceRegedMemMgr::UnregisterMemory(void *memHandle)
{
    ...
    if (buffer->IsAlias()) {
        auto hwHandle = buffer->GetKey();
        auto findResult = localRdmaRmaBufferMgr_->Find(ownKey);
        if (findResult.first && findResult.second->GetKey() == hwHandle) {
            refBuffer = findResult.second.get();
        } else {
            for (auto& ptr : allRegisteredBuffers_) {
                if (ptr->GetKey() == hwHandle) {
                    refBuffer = ptr.get();
                    break;
                }
            }
