...
HcclResult AicpuTsRoceRegedMemMgr::RegisterMemory(HcommMem mem, const char *memTag, void **memHandle)
{
    ...
    if (findPair.first) {
        ...
        EXECEPTION_CATCH((localRdmaRmaBuffer = std::make_shared<hccl::LocalRdmaRmaBuffer>(netDevCtx, mem.addr, size, memType)),
            return HCCL_E_PTR);
    }
...
HcclResult AicpuTsRoceRegedMemMgr::MemoryImport(const void *memDesc, uint32_t descLen, HcommMem *outMem)
{
    ...
    outMem->type = HCCL_MEM_TYPE_DEVICE;
    return HCCL_SUCCESS;
}
...
HcclResult AicpuTsRoceRegedMemMgr::GetAllMemDetails(std::vector<MemDetails> &localOut,
    std::vector<MemDetails> &remoteOut) const
{
    localOut.clear();
    remoteOut.clear();
    ...
    for (const auto &buf : allRegisteredBuffers_) {
        ...
        MemDetails d{};
        d.addr = reinterpret_cast<u64>(buf->GetAddr());
        d.size = buf->GetSize();
        d.key = buf->GetKey();
        localOut.push_back(d);
    }
    for (const auto &epMgr : remoteRdmaRmaBufferMgrs_) {
        ...
        mgr->ForEach([&remoteOut](const hccl::BufferKey<uintptr_t, u64> &, const std::shared_ptr<hccl::RemoteRdmaRmaBuffer> &rb) {
            ...
            MemDetails d{};
            d.addr = reinterpret_cast<u64>(rb->GetAddr());
            d.size = rb->GetSize();
            d.key = rb->GetKey();
            remoteOut.push_back(d);
        });
    }
    return HCCL_SUCCESS;
}
