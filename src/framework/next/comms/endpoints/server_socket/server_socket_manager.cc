...
HcclResult ServerSocketManager::ServerSocketStartListen(
    const Hccl::PortData &localPort, const Hccl::NicType nicType, const uint32_t devPhyId, const uint32_t port)
{
...
HcclResult ServerSocketManager::HostSocketListen(
    const Hccl::PortData &localPort, const uint32_t devPhyId, const uint32_t port)
{
    ...
    EXECEPTION_CATCH(socketHandle = Hccl::HostSocketHandleManager::GetInstance().Create(devPhyId, localPort.GetAddr()),
        return HCCL_E_PARA);

    std::unique_ptr<Hccl::Socket> serverSocket{};
    EXECEPTION_CATCH(serverSocket = std::make_unique<Hccl::Socket>(socketHandle, localPort.GetAddr(), port,
                         localPort.GetAddr(), "server", Hccl::SocketRole::SERVER, Hccl::NicType::HOST_NIC_TYPE),
...
HcclResult ServerSocketManager::DeviceSocketListen(
    const Hccl::PortData &localPort, const uint32_t devPhyId, const uint32_t port)
{
    ...
    std::unique_ptr<Hccl::Socket> serverSocket;
    EXECEPTION_CATCH(serverSocket = std::make_unique<Hccl::Socket>(socketHandle, localPort.GetAddr(), port,
                         localPort.GetAddr(), "server", Hccl::SocketRole::SERVER, Hccl::NicType::DEVICE_NIC_TYPE),
        return HCCL_E_PARA); // 端口号可能冲突，需要SE做决定
    HCCL_INFO("[ServerSocketManager][%s] listen_socket_info[%s]", __func__, serverSocket->Describe().c_str());
    if (!isListen) {
        EXECEPTION_CATCH(serverSocket->Listen(), return HCCL_E_INTERNAL);
...
HcclResult ServerSocketManager::DeviceSocketStopListen(const Hccl::PortData &localPort, const uint32_t port)
{
    ...
    if (deviceServerSocketMap_.find(localPort) != deviceServerSocketMap_.end()
        ...
        if (deviceServerSocketMap_[localPort][port].second == 0) {
            ...
            bool isListen = socketMgrCompat_->CheckServerPortListening(localPort);
            if (isListen) {
                Hccl::PortData portDataCopy(localPort.GetRankId(), localPort.GetType(), localPort.GetProto(),
                    localPort.GetId(), localPort.GetAddr());
