...
namespace hcomm {
HcclResult ServerSocketManager::ServerSocketStartListen(const Hccl::PortData& localPort, const Hccl::NicType nicType, const uint32_t devPhyId, const uint32_t port, uint32_t *actualPort)
{
    uint32_t dynamicPort = port;
    if (nicType == Hccl::NicType::HOST_NIC_TYPE) {
        CHK_RET(HostSocketListen(localPort, devPhyId, &dynamicPort));
    } else if (nicType == Hccl::NicType::DEVICE_NIC_TYPE) {  
        CHK_RET(DeviceSocketListen(localPort, devPhyId, &dynamicPort));
    } else {
...
HcclResult ServerSocketManager::HostSocketListen(const Hccl::PortData& localPort, const uint32_t devPhyId, uint32_t *port)
{
    ...
    if (requestedPort == 0) {
        EXECEPTION_CATCH(serverSocket->Listen(actualPort), return HCCL_E_INTERNAL);
...
{
    if (port == nullptr) {
        HCCL_ERROR("[ServerSocketManager][%s] port is nullptr", __func__);
        return HCCL_E_PARA;
    }
    *port = 0;
    if (nicType == Hccl::NicType::HOST_NIC_TYPE) {
