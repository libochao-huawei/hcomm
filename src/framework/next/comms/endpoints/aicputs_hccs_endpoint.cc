...
AicpuTsHccsEndPoint::~AicpuTsHccsEndPoint()
{
    if (!devIpAddr_.IsInvalid()) {
        hccl::GlobalNetDevMgr::GetInstance(localEp_.loc.device.devPhyId).UnRefNetDevCtx(
...
HcclResult AicpuTsHccsEndPoint::Init()
{
    ...
    CHK_RET(GlobalNetDevMgr::GetInstance(localEp_.loc.device.devPhyId).InitNic());
    ...
    CHK_RET(GlobalNetDevMgr::GetInstance(localEp_.loc.device.devPhyId).GetDeviceIP(devPhyId, deviceIp));
    ...
    CHK_RET(hccl::GlobalNetDevMgr::GetInstance(localEp_.loc.device.devPhyId).RefNetDevCtx(
...
{
    CHK_RET(hccl::GlobalNetDevMgr::GetInstance(localEp_.loc.device.devPhyId).ServerInit(netDevCtx_, port));
    return HCCL_SUCCESS;
}

HcclResult AicpuTsHccsEndPoint::ServerSocketStopListen(const uint32_t port)
{
    CHK_RET(hccl::GlobalNetDevMgr::GetInstance(localEp_.loc.device.devPhyId).ServerDeInit(netDevCtx_, port));
    return HCCL_SUCCESS;
}
