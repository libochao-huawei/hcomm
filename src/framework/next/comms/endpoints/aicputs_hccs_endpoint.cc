...
HcclResult AicpuTsHccsEndPoint::EnableMemAccess(const HcommMemGrantInfo *remoteGrantInfo)
{
    CHK_RET(this->regedMemMgr_->EnableMemAccess(remoteGrantInfo));
    return HCCL_SUCCESS;
}

HcclResult AicpuTsHccsEndPoint::DisableMemAccess(const HcommMemGrantInfo *remoteGrantInfo)
{
    CHK_RET(this->regedMemMgr_->DisableMemAccess(remoteGrantInfo));
    return HCCL_SUCCESS;
}
}