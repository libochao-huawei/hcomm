...
HcclResult AicpuTsHccsEndPoint::Init()
{
    ...
    CHK_RET(GlobalNetDevMgr::GetInstance(endpointDesc_.loc.device.devPhyId).InitNic());
...
{
    CHK_RET(hccl::GlobalNetDevMgr::GetInstance(endpointDesc_.loc.device.devPhyId).ServerInit(netDevCtx_, port));
    return HCCL_SUCCESS;
}

HcclResult AicpuTsHccsEndPoint::ServerSocketStopListen(const uint32_t port)
{
    CHK_RET(hccl::GlobalNetDevMgr::GetInstance(endpointDesc_.loc.device.devPhyId).ServerDeInit(netDevCtx_, port));
    return HCCL_SUCCESS;
}
