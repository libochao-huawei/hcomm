...
HcclResult AivUbMemChannel::GetRemoteMem(HcclMem **remoteMem, uint32_t *memNum)
{
    return transport_->GetRemoteMem(remoteMem, memNum);
}

HcclResult AivUbMemChannel::GetUserRemoteMem(CommMem **remoteMem, uint32_t *memNum)
{
    return transport_->GetUserRemoteMem(remoteMem, memNum);
}
