...
HcclResult hcomm::HostCpuUrmaChannel::UrmaPostJettySendWr(urma_opcode_t opcode, void *dst, const void *src, uint64_t len)
{
    HCCL_INFO("[HostCpuUrmaChannel::%s] start. opcode[%u] dst[%p], src[%p], len[%llu] fenceFlag_[%u].",
    ...
    fenceFlag_ = false;
    wqeNum_++;
    HCCL_INFO("[HostCpuUrmaChannel::%s] opencode[%u] fenceFlag_[%u] wqeNum_[%u] splitNum[%llu] SUCCESS.", __func__, opcode, fenceFlag_, wqeNum_, splitNum);
    return HCCL_SUCCESS;
}
