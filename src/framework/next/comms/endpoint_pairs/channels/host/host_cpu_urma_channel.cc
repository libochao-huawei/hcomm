...
HostCpuUrmaChannel::HostCpuUrmaChannel(EndpointHandle endpointHandle, HcommChannelDesc channelDesc)
...
HostCpuUrmaChannel::~HostCpuUrmaChannel() {
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[HostCpuUrmaChannel::~HostCpuUrmaChannel] exception occurred, HcclResult=[%d]", ret);
    }
}
...
HcclResult HostCpuUrmaChannel::ParseInputParam()
{
    ...
    for (uint32_t i = 0; i < channelDesc_.memHandleNum; ++i) {
        ...
        Hccl::LocalUbRmaBuffer* localRdmaBuffer = static_cast<Hccl::LocalUbRmaBuffer *>(channelDesc_.memHandles[i]);
...
HcclResult HostCpuUrmaChannel::BuildConnection()
{
    ...
    EXECEPTION_CATCH(
        conn = std::make_unique<HostUrmaConnection>(socket_, rdmaHandle_),
...
ChannelStatus HostCpuUrmaChannel::GetStatus()
{
    ...
    if (ret != HCCL_SUCCESS && ret != HCCL_E_AGAIN) {
        HCCL_ERROR("[HostCpuUrmaChannel::GetStatus] get status exception occurred, HcclResult=[%d]", ret);
        return ChannelStatus::FAILED;
    }
    return status;
}

HcclResult HostCpuUrmaChannel::GetStatus(ChannelStatus &status) {
    switch (rdmaStatus_) {
...
HcclResult HostCpuUrmaChannel::CheckSocketStatus() {
    CHK_PTR_NULL(socket_);
    Hccl::SocketStatus socketStatus = socket_->GetStatus(); // socket状态机
    HCCL_DEBUG("[HostCpuUrmaChannel::CheckSocketStatus] socket status = %s", socketStatus.Describe().c_str());
...
// 准备资源（创建QP）
HcclResult HostCpuUrmaChannel::CreateQp() {
    for (auto &conn : connections_) {
        Hccl::CHECK_NULLPTR(conn,
            Hccl::StringFormat("[HostCpuUrmaChannel::%s] failed, connection pointer is nullptr", __func__));
    ...
    HCCL_INFO("[HostCpuUrmaChannel::IsResReady] all connections resources connected.");
    return HCCL_SUCCESS;
}

// 交换数据
HcclResult HostCpuUrmaChannel::ExchangeData()
{
    HCCL_INFO("[HostCpuUrmaChannel::%s] Start to SendExchangeData, notifyNum=%u, bufferNum=%u, connNum=%u",
    ...
    // 同步接受数据数据
    HCCL_INFO("[HostCpuUrmaChannel::%s] Start to Receive Exchange Data", __func__);
    std::vector<char> recvData{};
    recvData.resize(exchangeDataSize);
    socket_->Recv(reinterpret_cast<void *>(recvData.data()), exchangeDataSize);
    HCCL_INFO("[HostCpuUrmaChannel::%s] Receive Exchange Data success, size=%llu, exchangeDataSize=%u", __func__,
    ...
    HCCL_INFO("[HostCpuUrmaChannel::%s] Unpack exchange Data success. ", __func__);    
    return HCCL_SUCCESS;
}

 
void HostCpuUrmaChannel::NotifyVecPack(Hccl::BinaryStream &binaryStream)
{
...
HcclResult HostCpuUrmaChannel::BufferVecPack(Hccl::BinaryStream &binaryStream)
{
    binaryStream << bufferNum_;
    HCCL_INFO("[HostCpuUrmaChannel::%s] start to pack RmaBuffers", __func__);
    u32 pos = 0;
    ...
    HCCL_INFO("[HostCpuUrmaChannel::%s] pack RmaBuffers finish", __func__);
    return HCCL_SUCCESS;
}
 
HcclResult HostCpuUrmaChannel::ConnVecPack(Hccl::BinaryStream &binaryStream)
{
    binaryStream << connNum_;
    HCCL_INFO("[HostCpuUrmaChannel::%s] start to pack connections", __func__);
    u32 pos = 0;
    ...
    HCCL_INFO("[HostCpuUrmaChannel::%s] pack connections finish", __func__);
    return HCCL_SUCCESS;
}

HcclResult HostCpuUrmaChannel::RmtBufferVecUnpackProc(Hccl::BinaryStream &binaryStream)
{
    u32 rmtNum;
    binaryStream >> rmtNum;
...
HcclResult HostCpuUrmaChannel::NotifyVecUnpack(Hccl::BinaryStream &binaryStream)
{
    uint32_t notifySize = 0;
    binaryStream >> notifySize;
    if (notifySize != notifyNum_) {
        HCCL_ERROR("[HostCpuUrmaChannel::NotifyVecUnpack] rmtNum=%u is not equal to localNum=%u", notifySize, notifyNum_);
        return HCCL_E_ROCE_CONNECT;
    }
    ...
    HCCL_INFO("[HostCpuUrmaChannel::NotifyVecUnpack] unpack dpuNotify");
    return HCCL_SUCCESS;
}

HcclResult HostCpuUrmaChannel::ConnVecUnpackProc(Hccl::BinaryStream &binaryStream)
{
    u32 rmtConnNum;
...
HcclResult HostCpuUrmaChannel::ModifyQp() {
    for (auto &conn : connections_) {
        Hccl::CHECK_NULLPTR(conn,
            Hccl::StringFormat("[HostCpuUrmaChannel::%s] failed, connection pointer is nullptr", __func__));
...
HcclResult HostCpuUrmaChannel::GetRemoteMem(HcclMem **remoteMem, uint32_t *memNum, char** memTags)
{
    ...
    *memNum = 0;
    ...
    for (uint32_t i = 0; i < totalCount; i++) {
        ...
        HCCL_INFO("[HostCpuUrmaChannel::%s] rmtBuf[addr[%p], size[%lu]]", 
...
std::vector<Hccl::QpInfo> HostCpuUrmaChannel::GetQpInfos() const
{
...
std::string HostCpuUrmaChannel::Describe() const
{
    std::string msg = "HostCpuUrmaChannel{";
...
// TODO: 可能需要错开地址
HcclResult HostCpuUrmaChannel::IbvPostRecv() const {
    std::vector<Hccl::QpInfo> qpInfo = GetQpInfos();
    CHK_PRT_RET(qpInfo.empty(), HCCL_ERROR("[HostCpuUrmaChannel::%s] qpInfos is Empty", __func__), HCCL_E_ROCE_CONNECT);
    CHK_PRT_RET(localRmaBuffers_.empty(), HCCL_ERROR("[HostCpuUrmaChannel::%s] localRmaBuffer is Empty", __func__),
                HCCL_E_ROCE_CONNECT);
    CHK_PRT_RET(rmtRmaBuffers_.empty(), HCCL_ERROR("[HostCpuUrmaChannel::%s] rmtRmaBuffers is Empty", __func__),
                HCCL_E_ROCE_CONNECT);
    ...
    CHK_PRT_RET(ret == ENOMEM,
                HCCL_WARNING("[HostCpuUrmaChannel][%s] post recv wqe overflow. ret:%d, "
                             "badWr->wr_id[%llu], badWr->sg_list->addr[%llu]",
                             __func__, ret, recvbadWr->wr_id, recvbadWr->sg_list->addr),
                HCCL_E_AGAIN);

    CHK_PRT_RET(ret != 0,
                HCCL_ERROR("[HostCpuUrmaChannel][%s] ibv_post_recv failed. ret:%d, "
                           "badWr->wr_id[%llu], badWr->sg_list->addr[%llu]",
                           __func__, ret, recvbadWr->wr_id, recvbadWr->sg_list->addr),
                HCCL_E_NETWORK);
...
HcclResult HostCpuUrmaChannel::PrepareNotifyWrResource(
    const uint64_t len, const uint32_t remoteNotifyIdx, struct ibv_send_wr &notifyRecordWr) const
{
    if (remoteNotifyIdx >= remoteDpuNotifyIds_.size()) {
        HCCL_ERROR("[HostCpuUrmaChannel::%s] remoteNotifyIdx[%u] out of the range of remoteDpuNotifyIds_[%u].",
...
HcclResult HostCpuUrmaChannel::NotifyRecord(const uint32_t remoteNotifyIdx) const
{
    ...
    // 3.调用ibv_post_send
    HCCL_INFO("[HostCpuUrmaChannel::%s] call ibv_post_send, qp_state = [%u]", __func__, qpInfo[0].qp->state);
    int32_t ret = ibv_post_send(qpInfo[0].qp, &notifyRecordWr, &sendbadWr);
    if (ret != 0 && sendbadWr == nullptr) {
        HCCL_ERROR("[HostCpuUrmaChannel::%s] ibv_post_send failed while badWr is nullptr", __func__);
        return HCCL_E_INTERNAL;
    }
    CHK_PRT_RET(ret == ENOMEM,
        HCCL_WARNING("[HostCpuUrmaChannel][%s] post send wqe overflow. ret:%d, badWr->wr_id[%llu], "
    ...
    CHK_PRT_RET(ret != 0,
        HCCL_ERROR("[HostCpuUrmaChannel][%s] ibv_post_send failed. ret:%d, badWr->wr_id[%llu], "
                   "badWr->sg_list->addr[%llu], badWr->wr.rdma.remote_addr[%llu], badWr->wr.ud.remote_qpn[%u]",
            __func__, ret, sendbadWr->wr_id, sendbadWr->sg_list->addr, sendbadWr->wr.rdma.remote_addr, sendbadWr->wr.ud.remote_qpn),
        HCCL_E_NETWORK);
    HCCL_INFO("[HostCpuUrmaChannel::NotifyRecord] NotifyRecord end");
    return HCCL_SUCCESS;
}

HcclResult HostCpuUrmaChannel::NotifyWait(const uint32_t localNotifyIdx, const uint32_t timeout)
{
    HCCL_INFO("[HostCpuUrmaChannel::NotifyWait] NotifyWait start");

    if (localNotifyIdx >= localDpuNotifyIds_.size()) {
        HCCL_ERROR("[HostCpuUrmaChannel::%s] localNotifyIdx[%u] out of the range of localDpuNotifyIds_[%u].",
    ...
    std::vector<Hccl::QpInfo> qpInfo = GetQpInfos();
    if (qpInfo.empty()) {
        HCCL_ERROR("[HostCpuUrmaChannel::%s] qpInfos is Empty", __func__);
        return HCCL_E_ROCE_CONNECT;
    }
    if (qpInfo[0].recvCq == nullptr) {
        HCCL_INFO("[HostCpuUrmaChannel::NotifyWait] recvCq is null");
        return HCCL_E_INTERNAL;
    }
    if (qpInfo[0].recvCq->context == nullptr) {
        HCCL_INFO("[HostCpuUrmaChannel::NotifyWait] recvCq->context is null");
        return HCCL_E_INTERNAL;
    }

    HCCL_INFO("[HostCpuUrmaChannel::NotifyWait] poll recvCq = %p, localNotifyIdx = %u, notifyId = %u.",
    ...
    while (true) {
        HCCL_INFO("[HostCpuUrmaChannel::NotifyWait] start to poll cq");
        
        HCCL_INFO("qp_state = [%u]", qpInfo[0].qp->state);
        auto actualNum = ibv_poll_cq(qpInfo[0].recvCq, 1, &wc);
        CHK_PRT_RET(wc.status != IBV_WC_SUCCESS,
            HCCL_ERROR("[HostCpuUrmaChannel][%s] ibv_poll_cq return wc.status is [%d].",
            __func__, wc.status), HCCL_E_NETWORK);

        HCCL_INFO("[HostCpuUrmaChannel::NotifyWait] actualNum = %d; imm_data = %u", actualNum, wc.imm_data);

        if (actualNum > 0 && wc.imm_data == dpuNotifyId) {
            HCCL_INFO("[HostCpuUrmaChannel::NotifyWait] poll cq success");
            break;
        }

        if ((std::chrono::steady_clock::now() - startTime) >= waitTime) {
            HCCL_ERROR("[HostCpuUrmaChannel][%s] call ibv_poll_cq timeout.", __func__);
...
HcclResult HostCpuUrmaChannel::PrepareWriteWrResource(const void *dst, const void *src, const uint64_t len,
...
HcclResult HostCpuUrmaChannel::WriteWithNotify(
    void *dst, const void *src, const uint64_t len, const uint32_t remoteNotifyIdx) const
{
    CHK_PTR_NULL(src);
    CHK_PTR_NULL(dst);
    HCCL_INFO("[HostCpuUrmaChannel::WriteWithNotify] WriteWithNotify start");
    ...
    CHK_PRT_RET(ret != 0,
        HCCL_ERROR("[HostCpuUrmaChannel][%s] ibv_post_send failed. ret:%d, badWr->wr_id[%llu], "
            ...
            __func__, ret, badWr->wr_id, badWr->sg_list->addr, badWr->wr.rdma.remote_addr, badWr->wr.ud.remote_qpn),
        HCCL_E_NETWORK);
    HCCL_INFO("[HostCpuUrmaChannel::WriteWithNotify] WriteWithNotify end");
    return HCCL_SUCCESS;
}

HcclResult HostCpuUrmaChannel::Write(void *dst, const void *src, const uint64_t len) const
{
    return HCCL_E_NOT_SUPPORT;
}

HcclResult HostCpuUrmaChannel::Read(void *dst, const void *src, const uint64_t len) const 
{
    return HCCL_E_NOT_SUPPORT;
}

HcclResult HostCpuUrmaChannel::ChannelFence() const
{
    struct ibv_wc wc{};
    int wcNum = 2;
    std::vector<Hccl::QpInfo> qpInfo = GetQpInfos();
...
HcclResult HostCpuUrmaChannel::GetHcclBuffer(void*& addr, uint64_t& size)
{
    if (rmtRmaBuffers_.empty()) {
        HCCL_ERROR("[HostCpuUrmaChannel::%s] remote buffer is empty, please check if channel complete exchange data",
                   __func__);
        return HCCL_E_INTERNAL;
    }
