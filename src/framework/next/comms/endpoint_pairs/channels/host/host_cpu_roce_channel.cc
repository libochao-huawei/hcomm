...
HcclResult HostCpuRoceChannel::PreparePostRecvWr(utin32_t qpIdx, struct ibv_recv_wr &recvWr)
{
...
HcclResult HostCpuRoceChannel::NotifyWait(const uint32_t localNotifyIdx, const uint32_t timeout)
{
    ...
    for (uint32_t i = 0; i < qpInfo.size(); i++) {
        ...
        CHK_PRT_RET(NotifyWaitIbvPollCq(qpInfo[i].recvCq, dpuNotifyId, startTime, waitTime));
    }
    ...
    void *dst, const void *src, const uint64_t len, const uint32_t remoteNotifyIdx)
{
    ...
    CHK_PRT_RET(CalcQpTile(tailLen, qpInfo.size(), useQpNum, tileLen, tileLenTail));
