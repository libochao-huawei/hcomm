...
{
    CHK_PRT_RET(remoteMem == nullptr, HCCL_ERROR("[GetRemoteMem] remoteMem is nullptr"), HCCL_E_PARA);
    CHK_PRT_RET(memNum == nullptr, HCCL_ERROR("[GetRemoteMem] memNum is nullptr"), HCCL_E_PARA);
    ...
    *memNum = 0;
    ...
    for (uint32_t i = 0; i < totalCount; i++) {
        ...
        remoteMem[i] = new HcclMem();
        remoteMem[i]->type = rmtRmaBuffer->GetMemType();
        remoteMem[i]->addr = reinterpret_cast<void *>(rmtRmaBuffer->GetAddr());
        remoteMem[i]->size = rmtRmaBuffer->GetSize();
...
// TODO: 测试中发现会导致poll cq 返回wc.status=5，待定位
...
HcclResult HostCpuRoceChannel::NotifyRecord(const uint32_t remoteNotifyIdx) const
{
    ...
    // 补充rq中消耗的rqe
    // 1. 准备recv_WR
    struct ibv_recv_wr notifyWaitWr{};
    struct ibv_recv_wr *recvbadWr = nullptr;
    struct ibv_sge recvsgList{};
    recvsgList.addr = localRmaBuffers_[0]->GetBufferInfo().first; // 本端起始地址
    recvsgList.length = MEM_BLOCK_SIZE; //bufferBlockSize / 2; // 取的本端长度
    recvsgList.lkey = localRmaBuffers_[0]->GetLkey(); // 本端的访问秘钥
    notifyWaitWr.wr_id = 0;
    notifyWaitWr.sg_list = &recvsgList;
    notifyWaitWr.next = nullptr;
    notifyWaitWr.num_sge = 1;
    // 3.补充rq中消耗的rqe
    HCCL_INFO("[DpuRoceChannelImpl::%s] call ibv_post_recv", __func__);
    HCCL_INFO("qp_state = [%u]", qpInfo[0].qp->state);
    s32 ret = ibv_post_recv(qpInfo[0].qp, &notifyWaitWr, &recvbadWr);
    CHK_PRT_RET(ret == ENOMEM,
        HCCL_WARNING("[DpuRoceChannelImpl][%s] post recv wqe overflow. ret:%d, "
        "badWr->wr_id[%llu], badWr->sg_list->addr[%llu]", recvbadWr->wr_id, recvbadWr->sg_list->addr, __func__, ret),
        HCCL_E_AGAIN);

    CHK_PRT_RET(ret != 0,
        HCCL_ERROR("[DpuRoceChannelImpl][%s] ibv_post_recv failed. ret:%d, "
        "badWr->wr_id[%llu], badWr->sg_list->addr[%llu]", recvbadWr->wr_id, recvbadWr->sg_list->addr, __func__, ret),
        HCCL_E_NETWORK);
    ...
    ret = ibv_post_send(qpInfo[0].qp, &notifyRecordWr, &sendbadWr);
    ...
    void *dst, const void *src, const uint64_t len, const uint32_t remoteNotifyIdx) const
{
    ...
    // 2. 调用ibv_post_send
    ret = ibv_post_send(qpInfo[0].qp, &writeWithNotifyWr, &badWr);
    CHK_PRT_RET(ret == ENOMEM,
        HCCL_WARNING("[HostCpuRoceChannel][%s] post send wqe overflow. ret:%d, badWr->wr_id[%llu], "
                     "badWr->sg_list->addr[%llu], badWr->wr.rdma.remote_addr[%llu], badWr->wr.ud.remote_qpn[%u]",
            __func__,
