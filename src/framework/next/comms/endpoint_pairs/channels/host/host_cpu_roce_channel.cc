...
HcclResult HostCpuRoceChannel::NotifyWait(const uint32_t localNotifyIdx, const uint32_t timeout)
{
    ...
    CHK_PRT_RET(qpInfo[0].recvCq == nullptr, HCCL_INFO("[HostCpuRoceChannel::NotifyWait] recvCq is null"), HCCL_E_INTERNAL);
    CHK_PRT_RET(qpInfo[0].qp == nullptr, HCCL_INFO("[HostCpuRoceChannel::NotifyWait] qp is null"), HCCL_E_INTERNAL);
...
HcclResult HostCpuRoceChannel::ChannelFence()
{
    ...
    if (wqeNum_ == 0) {
        HCCL_INFO("[HostCpuRoceChannel::%s] no need to fence since no wqe has been posted.", __func__);
        return HCCL_SUCCESS;
    }
    ...
    CHK_PRT_RET(qpInfo[0].recvCq == nullptr, HCCL_INFO("[HostCpuRoceChannel::%s] recvCq is null", __func__), HCCL_E_INTERNAL);
    CHK_PRT_RET(qpInfo[0].qp == nullptr, HCCL_INFO("[HostCpuRoceChannel::%s] qp is null", __func__), HCCL_E_INTERNAL);
    if (qpInfo[0].recvCq->context == nullptr) {
        HCCL_INFO("[HostCpuRoceChannel::%s] recvCq->context is null", __func__);
        return HCCL_E_INTERNAL;
    }
    ...
    while (true) {
        ...
        if (actualNum32 > 0 && actualNum32 <= wqeNum_) {
            HCCL_INFO("[HostCpuRoceChannel::%s] ibv_poll_cq polled %d completions.", __func__, actualNum32, wqeNum_);
            for (uint32_t i = 0; i < actualNum32; i++) {
                if (wc[i].status != IBV_WC_SUCCESS) {
                    HCCL_ERROR("[HostCpuRoceChannel::%s] ibv_poll_cq polled a completion with error. wc[%u] status: %d.",
