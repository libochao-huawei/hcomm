#ifndef NDA_RDMA_1825_OPS_H
#define NDA_RDMA_1825_OPS_H
...
protected:
    ...
    {
        HCCL_INFO("[Nda1825Ops::%s] start BuildDoorbell", __func__);
        ...
        dbAddr  = sqContext_->dbVa + ((pi >> 8) & 0xff);
        dbValue = dbEntry.dw0.value;

        HCCL_INFO("[Nda1825Ops::%s] Hcomm UB Build Doorbell OK", __func__);
        return HCCL_SUCCESS;
    }
...
private:
    ...
    int BuildWriteWqe(const RmaBufSliceLite *loc, const RmtRmaBufSliceLite *rmt, RoceWqeEntry *wqe, uint32_t opCode)
    {
        ...
        HCCL_INFO("[Nda1825Ops::%s] Hcomm UB BuildWrite wqe OK", __func__);
        return HCCL_SUCCESS;
    }
};
