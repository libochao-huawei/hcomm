    ...
    ChannelHandle &outHandle)
{
    ...
    auto localBase = static_cast<const MemDetails *>(res->localMem);
    ...
    auto remoteBase = static_cast<const MemDetails *>(res->remoteMem);
    ...
    ibd.localMemDetailsList = std::move(localMd);
    ibd.remoteMemDetailsList = std::move(remoteMd);
    ibd.useMemDetailsMgr = !ibd.localMemDetailsList.empty() && !ibd.remoteMemDetailsList.empty();

    u32 devId = INVALID_UINT;
    CHK_RET(hrtDrvGetLocalDevIDByHostDevID(deviceInfo.devicePhyId, &devId));
    CHK_PRT_RET(devId == INVALID_UINT,
    ...
    int nc = snprintf_s(commId, sizeof(commId), sizeof(commId) - 1U, "hcomm_ts_roce_%d_%llu", deviceInfo.deviceLogicId,
    ...
    CHK_RET(CreateDispatcherCtx(&dctxPtr, devId, commId));
