...
HcclResult AicpuTsUboeChannel::Makebufs(
...
HcclResult AicpuTsUboeChannel::ParseInputParam()
{
    ...
    if (channelDesc_.exchangeAllMems) {
        ...
        for (uint32_t i = 0; i < memHandleNum; ++i) {
            ...
            HCCL_INFO(
                "[AicpuTsUboeChannel][%s] Got memHandle No.%u: addr[0x%llx], size[0x%llx], memTag[%s].", __func__, i,
...
HcclResult AicpuTsUboeChannel::BuildConnection()
{
    ...
    HCCL_INFO(
        ...
        protocol.Describe().c_str(), locIpv4Addr.Describe().c_str(), rmtIpv4Addr.Describe().c_str());
    HCCL_INFO(
        "[RmaConnManager::%s] locAddr_[%s], rmtAddr_[%s]", __func__, locAddr_.Describe().c_str(),
    ...
    std::unique_ptr<Hccl::DevUbConnection> ubConn = std::make_unique<Hccl::DevUbUboeConnection>(
        rdmaHandle_, locAddr_, rmtAddr_, opMode, devUsed, Hccl::HrtUbJfcMode::STARS_POLL, locIpv4Addr, rmtIpv4Addr);
...
HcclResult AicpuTsUboeChannel::FillTagVec(
    std::vector<Hccl::LocalRmaBuffer*>& bufferVec, std::vector<std::array<char, HCCL_RES_TAG_MAX_LEN>>& localUserMemTag)
{
...
HcclResult AicpuTsUboeChannel::GetUserRemoteMem(CommMem** remoteMem, char*** memTag, uint32_t* memNum)
{
    ...
    Hccl::RemoteMemCtx<std::unique_ptr<Hccl::RemoteUbRmaBuffer>> remoteMemCtx{
        userMemCount,    cacheValid_, rmtBufferVec_, remoteUserMemTag_,
        remoteUserMems_, tagCopies_,  tagPointers_,  cacheBuilder,
        remoteMem,       memTag,      memNum};
    ...
    for (auto& it : commonRes_.connVec) {
        if (it == nullptr) {
            Hccl::THROW<Hccl::InternalException>(
                "[AicpuTsUboeChannel::%s] failed, connection pointer is nullptr", __func__);
        }
        Hccl::RmaConnType connType = it->GetRmaConnType();
        if (connType != Hccl::RmaConnType::UB) {
            Hccl::THROW<Hccl::InternalException>(
                "[AicpuTsUboeChannel::%s] connection type[%s] is not ub", __func__, connType.Describe().c_str());
        }
...
void AicpuTsUboeChannel::NotifyVecPack(Hccl::BinaryStream& binaryStream)
{
    ...
    for (auto& it : commonRes_.notifyVec) {
        ...
        HCCL_INFO("[AicpuTsUboeChannel::%s] pack notify pos=%u, dto %s", __func__, pos, dto->Describe().c_str());
        pos++;
    }
}

void AicpuTsUboeChannel::BufferVecPack(
    ...
    std::vector<std::array<char, HCCL_RES_TAG_MAX_LEN>>& tagVec)
{
    ...
    for (auto& it : bufferVec) {
        ...
        if (it != nullptr) { // 非空的buffer，从buffer中获取 dto
            std::unique_ptr<Hccl::Serializable> dto = it->GetExchangeDto();
            dto->Serialize(binaryStream);
            HCCL_INFO("[AicpuTsUboeChannel::%s] pack buffer pos=%u dto %s", __func__, pos, dto->Describe().c_str());
        } else { // 空的buffer，dto所有字段为0(size=0)
            Hccl::ExchangeUbBufferDto exchangeDto;
            exchangeDto.Serialize(binaryStream);
            HCCL_INFO(
                "[AicpuTsUboeChannel::%s] pack buffer pos=%u, dto is null %s", __func__, pos,
...
void AicpuTsUboeChannel::ConnVecPack(Hccl::BinaryStream& binaryStream)
{
    ...
    for (auto& it : commonRes_.connVec) {
        ...
        std::unique_ptr<Hccl::Serializable> dto = it->GetExchangeDto();
        dto->Serialize(binaryStream);
        HCCL_INFO("[AicpuTsUboeChannel::%s] pack connection pos=%u, dto %s", __func__, pos, dto->Describe().c_str());
        pos++;
    }
}
...
void AicpuTsUboeChannel::SendDataSize()
{
    ...
    HCCL_INFO(
        "[AicpuTsUboeChannel::%s] Send size[%u] of data success. [%zu] bytes sent.", __func__, sendSize,
        sizeof(sendSize));
}
...
void AicpuTsUboeChannel::RmtBufferVecUnpackProc(
    u32 locNum, Hccl::BinaryStream& binaryStream, RemoteBufferVec& bufferVec, UboeRmtBufType type)
{
    ...
    if (type == UboeRmtBufType::BUFFER && rmtNum > MAX_BUFFER_NUM) {
        MACRO_THROW(
            ...
            Hccl::StringFormat(
                "[AicpuTsUboeChannel][RmtBufferVecUnpackProc] rmtNum[%u] exceeds limit[%u]", rmtNum, MAX_BUFFER_NUM));
    }
...
{
    auto SetState = [&](UboeStatus next, ChannelStatus ch) {
        uboeStatus = next;
        channelStatus = ch;
    };
...
HcclResult AicpuTsUboeChannel::H2DResPack(std::vector<char>& buffer)
{
    ...
    HCCL_INFO(
        "[AicpuTsUboeChannel][%s] Pack Buffer data[%p], Pack Buffer size[%zu].", __func__, buffer.data(),
        buffer.size());
    return HCCL_SUCCESS;
}
