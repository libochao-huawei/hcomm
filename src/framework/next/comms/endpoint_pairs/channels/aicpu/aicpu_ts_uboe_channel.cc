...
HcclResult AicpuTsUboeChannel::BuildAttr()
{
...
HcclResult AicpuTsUboeChannel::BuildUbMemTransport()
{
    ...
    EXECEPTION_CATCH(
        memTransport_ = std::make_unique<Hccl::AicpuTsUboeChannel>(
...
bool AicpuTsUboeChannel::IsResReady()
{
    for (auto &it : commonRes_.connVec) {
        CHECK_NULLPTR(it,
            StringFormat("[AicpuTsUboeChannel::%s] failed, connection pointer is nullptr", __func__));

        Hccl::RmaConnType connType = it->GetRmaConnType();
        if (connType != Hccl::RmaConnType::UB) {
            THROW<InternalException>("[AicpuTsUboeChannel::%s] connection type[%s] is not ub",
...
void AicpuTsUboeChannel::HandshakeMsgPack(Hccl::BinaryStream &binaryStream)
{
...
void AicpuTsUboeChannel::BufferVecPack(Hccl::BinaryStream &binaryStream, std::vector<LocalRmaBuffer *> &bufferVec,
...
{
    notifyNum_    = commonRes_.notifyVec.size(); // 需要交换的notify数量
    ...
    HCCL_INFO("notifyNum=%u, bufferNum=%u, connNum=%u, cntNotifyNum=%u, cntNotifyDescSize=%u",
              notifyNum_, bufferNum_, connNum_, cntNotifyNum_, cntNotifyDescSize_);
...
{
    // 接收数据包尺寸
    socket->RecvAsync(reinterpret_cast<u8 *>(&recvDataSize_), sizeof(recvDataSize_));
    HCCL_INFO("[AicpuTsUboeChannel::%s] Receive Data Size", __func__);
}

void AicpuTsUboeChannel::SendExchangeData()
{
    socket->SendAsync(reinterpret_cast<u8 *>(sendData_.data()), sendData_.size());
...
void AicpuTsUboeChannel::RecvExchangeData()
{
    ...
    socket->RecvAsync(reinterpret_cast<u8 *>(recvData_.data()), recvData_.size());
...
bool AicpuTsUboeChannel::RecvDataProcess()
{
    ...
    RmtBufferVecUnpackProc(notifyNum, binaryStream, rmtNotifyVec, UboeRmtBufType::NOTIFY);
    RmtBufferVecUnpackProc(bufferNum, binaryStream, rmtBufferVec, UboeRmtBufType::BUFFER);
    return ConnVecUnpackProc(binaryStream);
}

void AicpuTsUboeChannel::RmtBufferVecUnpackProc(u32 locNum, BinaryStream &binaryStream, RemoteBufferVec &bufferVec,
                                            UboeRmtBufType type)
{
    u32 rmtNum;
    binaryStream >> rmtNum;
    if (type == UboeRmtBufType::BUFFER && rmtNum > MAX_BUFFER_NUM) {
        MACRO_THROW(InvalidParamsException,
            StringFormat("[AicpuTsUboeChannel][RmtBufferVecUnpackProc] rmtNum[%u] exceeds limit[%u]",
