...
bool AicpuTsUboeChannel::RecvDataProcess()
{
    ...
    RmtBufferVecUnpackProc(notifyNum, binaryStream_, rmtNotifyVec, UboeRmtBufType::NOTIFY);
    RmtBufferVecUnpackProc(bufferNum, binaryStream_, rmtBufferVec, UboeRmtBufType::BUFFER);
                                            ...
                                            UboeRmtBufType type)
{
    ...
    for (u32 i = 0; i < rmtNum; i++) {
        ...
        if (dto.size == 0) { // size为0，则为 remote 空buffer
            ...
            bufferVec.push_back(make_unique<Hccl::RemoteUbRmaBuffer>(rdmaHandle_, dto));
...
bool AicpuTsUboeChannel::ConnVecUnpackProc(Hccl::BinaryStream &binaryStream)
{
    ...
    if (connNum_ != rmtConnNum) {
        MACRO_THROW(Hccl::InvalidParamsException,
                    Hccl::StringFormat("connNum=%u is not equal to rmtConnNum=%u", connNum, rmtConnNum));
    }
...
void AicpuTsUboeChannel::SendFinish()
{
    ...
    socket->SendAsync(reinterpret_cast<u8 *>(sendFinishMsg_.data()), FINISH_MSG_SIZE);
...
void AicpuTsUboeChannel::RecvFinish()
{
    ...
    socket->RecvAsync(reinterpret_cast<u8 *>(recvFinishMsg_.data()), FINISH_MSG_SIZE);
