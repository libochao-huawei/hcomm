...
HcclResult AicpuTsRoceChannelV2::BuildAndGetRmtBufInfo(RegedBufferEntity** bufferEntityPtr)
{
    ...
    for (uint32_t i = 0; i < bufferNum_; i++) {
        ...
        bufferEntity.bufferInfo.rmaBuffer.address = static_cast<uint64_t>(rmtRmaBuffer->GetAddr());
        bufferEntity.bufferInfo.rmaBuffer.size = rmtRmaBuffer->GetSize();
        bufferEntity.bufferInfo.rmaBuffer.protectionInfo.type = PROTECTION_TYPE_RDMA;
        bufferEntity.bufferInfo.rmaBuffer.protectionInfo.memInfo.rdmaMemInfo.rkey = rmtRmaBuffer->GetRkey();
        rmtBufEntityList_.emplace_back(bufferEntity);
        HCCL_INFO("[AicpuTsRoceChannelV2::%s] rmtBuf[addr[%p], size[%lu]]", 
            __func__, bufferEntity.bufferInfo.rmaBuffer.address, bufferEntity.bufferInfo.rmaBuffer.size);
    }
...
HcclResult AicpuTsRoceChannelV2::BuildAndGetLocBufInfo(RegedBufferEntity** bufferEntityPtr)
{
    ...
    for (uint32_t i = 0; i < bufferNum_; i++) {
        ...
        RegedBufferEntity bufferEntity;
        bufferEntity.type = REGED_BUFFER_RMA;
        bufferEntity.bufferInfo.rmaBuffer.address = static_cast<uint64_t>(locRmaBuffer->GetAddr());
        bufferEntity.bufferInfo.rmaBuffer.size = locRmaBuffer->GetSize();
        bufferEntity.bufferInfo.rmaBuffer.protectionInfo.type = PROTECTION_TYPE_RDMA;
        bufferEntity.bufferInfo.rmaBuffer.protectionInfo.memInfo.rdmaMemInfo.lkey = locRmaBuffer->GetLkey();
