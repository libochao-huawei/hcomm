    ...
    HcclResult WaitClientSocketLinkEstablished(const std::shared_ptr<hccl::HcclSocket>& socket, s32 timeoutSec)
    {
        ...
        while (true) {
            if ((std::chrono::steady_clock::now() - startTime) >= timeout) {
                HCCL_ERROR(
                    "[AicpuTsRoceChannel][client][WaitLink] wait socket establish timeout, timeout[%lld s]",
            ...
            if (status == hccl::HcclSocketStatus::SOCKET_OK) {
                HCCL_DEBUG(
                    "[AicpuTsRoceChannel][client][WaitLink] socket established. localIp[%s], remoteIp[%s]",
...
HcclResult AicpuTsRoceChannel::BuildSocketTagName(std::string& outTag) const
{
    ...
    CHK_SAFETY_FUNC_RET(memcpy_s(
        &socketTagIdx, sizeof(socketTagIdx), channelDesc_.raws + sizeof(channelDesc_.raws) - sizeof(uint32_t),
        sizeof(uint32_t)));
...
HcclResult AicpuTsRoceChannel::CreateAndInitTransport(HcclDispatcher dispatcher)
{
    EXECEPTION_CATCH(
        transport_ = std::make_unique<hccl::Transport>(
            hccl::TransportType::TRANS_TYPE_IBV_EXP, transportPara_, dispatcher, notifyPoolHolder_, machinePara_),
        return HCCL_E_PTR);
...
void AicpuTsRoceChannel::InitSerializeRoceChannelRes(
    HcommRoceChannelRes& res, size_t localMemCount, size_t remoteMemCount, void* localMem, void* remoteMem,
    const std::vector<HcclQpInfoV2>& aiQpInfos, u32 qpNum) const noexcept
{
