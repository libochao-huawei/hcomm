...
#include "../host/dpu_notify/dpu_notify_manager.h"
...
HcclResult AicpuTsRoceChannel::BuildBuffer()
{
    if (channelDesc_.exchangeAllMems) {
        ...
        // 3. 从 channelDesc 的 memHandle，获得 localRmaBuffers_
...
HcclResult AicpuTsRoceChannel::CreateQp() {
    ...
    HCCL_INFO("[AicpuTsRoceChannel::IsResReady] all connections resources connected.");
    return HCCL_SUCCESS;
}
...
HcclResult AicpuTsRoceChannel::ExchangeData()
{
    ...
    HCCL_INFO("[AicpuTsRoceChannel::%s] Receive Exchange Data success, size=%llu, exchangeDataSize=%u", __func__,
...
HcclResult AicpuTsRoceChannel::RmtBufferVecUnpackProc(Hccl::BinaryStream &binaryStream)
{
    ...
    HCCL_INFO("unpack, bufferNum_=%u, rmtNum=%u", bufferNum_, rmtNum);
    ...
    for (u32 i = 0; i < rmtNum; i++) {
        ...
        if (rmtRmaBuffers_.size() > pos) { // ljy:什么场景
            // 对于之前已经加过的资源，无需追加
            continue;
        }
...
HcclResult AicpuTsRoceChannel::BuildAndGetRmtBufInfo(ProtectionInfo** protectionInfoPtr)
{
    ...
    for (uint32_t i = 0; i < bufferNum_; i++) {
        ...
        if (rmtRmaBuffer != nullptr) {
            ...
            protectionInfo.type = 0; // 0-RDMA 1-URMA
            protectionInfo.addr = 0;
            protectionInfo.length = 0;
            protectionInfo.RdmaMemProtectionInfo.rkey = 0;
        }
...
HcclResult AicpuTsRoceChannel::BuildAndGetLocBufInfo(ProtectionInfo** protectionInfoPtr)
{
    ...
    for (uint32_t i = 0; i < bufferNum_; i++) {
        ...
        if (locRmaBuffer != nullptr) {
            ...
            protectionInfo.addr = 0;
            protectionInfo.length = 0;
            protectionInfo.RdmaMemProtectionInfo.lkey = 0;
        }
...
HcclResult AicpuTsRoceChannel::Clean()
{
    ...
    localNotifies_.clear();
    notifyValueMem_.reset();
    notifyValueBuffer_.reset();
    remoteNotifies_.clear();
    rmtRmaBuffers_.clear();
    remoteMems.clear();
    return HCCL_SUCCESS;
}
