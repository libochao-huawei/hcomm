...
HcclResult AicpuTsRoceChannel::ParseInputParam()
{
    ...
    HCCL_INFO("[AicpuTsRoceChannel][%s] Start. endpointHandle[0x%llx]", __func__, reinterpret_cast<uint64_t>(endpointHandle_));
...
HcclResult AicpuTsRoceChannel::BuildAndGetDevChannelEntity(void** devChannelEntityPtr)
{
    ...
    CHK_PRT_RET(entityDevPtr == nullptr,
        HCCL_ERROR("[AicpuTsRoceChannel::%s] HrtMalloc for ChannelEntity failed", __func__), HCCL_E_MEMORY);
    ...
    *devChannelEntityPtr = entityDevPtr;
    HCCL_INFO("[AicpuTsRoceChannel::%s] Success, devPtr=%p", __func__, entityDevPtr);
    return HCCL_SUCCESS;
}

template<typename T>
HcclResult AicpuTsRoceChannel::CopyArrayToDevice(const T* hostArray, uint32_t arrayNum,
    T** deviceArrayPtr, const char* arrayName)
{
    ...
    CHK_PRT_RET(arrayDevPtr == nullptr,
        HCCL_ERROR("[AicpuTsRoceChannel::%s] HrtMalloc for %s failed, size=%zu", __func__, arrayName, arraySize),
        HCCL_E_MEMORY);
    ...
    *deviceArrayPtr = reinterpret_cast<T*>(arrayDevPtr);
    HCCL_INFO("[AicpuTsRoceChannel::%s] %s: host=%p -> dev=%p, num=%u, size=%zu",
              __func__, arrayName, hostArray, arrayDevPtr, arrayNum, arraySize);
    return HCCL_SUCCESS;
}

void AicpuTsRoceChannel::FreeDeviceMemories()
{
...
HcclResult AicpuTsRoceChannel::GetRemoteMem(HcclMem **remoteMem, uint32_t *memNum, char** memTags)
{
    ...
    *memNum = 0;
    ...
    for (uint32_t i = 0; i < totalCount; i++) {
        ...
        HCCL_INFO("[AicpuTsRoceChannel::%s] rmtBuf[addr[%p], size[%llu]]",
...
HcclResult AicpuTsRoceChannel::GetUserRemoteMem(CommMem **remoteMem, char ***memTags, uint32_t *memNum)
{
    std::lock_guard<std::mutex> lock(remoteMemsMutex_);
    CHK_PRT_RET(remoteMem == nullptr, HCCL_ERROR("[AicpuTsRoceChannel::%s] remoteMem is nullptr", __func__), HCCL_E_PTR);
