...
#include "../../../endpoints/aicputs_roce_endpoint.h"
...
#include "aicpu_ts_roce_res_pack.h"
...
/** Owns res / local MemDetails / remote MemDetails as separate device allocations for AICPU kernel blob. */
...
HcclResult AicpuTsRoceChannel::BuildDispatcherAndTransport()
{
    ...
    machinePara_.localUserrank = static_cast<u32>(localEp_.loc.device.devPhyId);
    machinePara_.remoteUserrank = static_cast<u32>(remoteEp_.loc.device.devPhyId);
    machinePara_.localWorldRank = machinePara_.localUserrank;
    machinePara_.remoteWorldRank = machinePara_.remoteUserrank;
    ...
    machinePara_.userDeviceMem.clear();
    for (const auto &m : regMems_) {
        machinePara_.userDeviceMem.emplace_back(
            hccl::DeviceMem::create(reinterpret_cast<void *>(m.addr), m.size));
    }
...
HcclResult AicpuTsRoceChannel::PrepareAicpuKernelDeviceBlob(std::shared_ptr<hccl::DeviceMem> &out)
{
    ...
    std::vector<MemDetails> localMd;
    std::vector<MemDetails> remoteMd;
    ...
    CHK_PRT_RET(nL > 0U && nL > (SIZE_MAX / sizeof(MemDetails)),
        HCCL_ERROR("[AicpuTsRoceChannel][PrepareAicpuKernelDeviceBlob] localMem count overflow"), HCCL_E_PARA);
    CHK_PRT_RET(nR > 0U && nR > (SIZE_MAX / sizeof(MemDetails)),
        HCCL_ERROR("[AicpuTsRoceChannel][PrepareAicpuKernelDeviceBlob] remoteMem count overflow"), HCCL_E_PARA);
    const u64 localBytes = static_cast<u64>(nL * sizeof(MemDetails));
    const u64 remoteBytes = static_cast<u64>(nR * sizeof(MemDetails));
