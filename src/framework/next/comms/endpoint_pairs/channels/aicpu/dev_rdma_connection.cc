...
HcclResult DevRdmaConnection::CreateQp()
{
    ...
    CHK_RET(Hccl::HrtRaNdaCqCreate(rdmaHandle_, &ndaOps_, dmaMode_, &cqInfo_, &cqHandle_));

    CHK_RET(Hccl::HrtRaNdaQpCreate(rdmaHandle_, &ndaOps_, dmaMode_, &qpInfo_, &qpHandle_));
...
HcclResult DevRdmaConnection::BuildSqContext(SqContext* context)
{
    ...
    context->RdmaSqContext.wqeSize = qpInfo_.sqInfo.qBuf.entrySize;
    context->RdmaSqContext.depth = qpInfo_.sqInfo.qBuf.entryCnt;
    context->RdmaSqContext.headAddr = reinterpret_cast<uint64_t >(qpInfo_.sqInfo.dbrPiVa.iov_base);
    context->RdmaSqContext.tailAddr = reinterpret_cast<uint64_t >(qpInfo_.sqInfo.dbrCiVa.iov_base);
    context->RdmaSqContext.dbVa = reinterpret_cast<uint64_t >(qpInfo_.sqInfo.dbHwVa.iov_base);
    context->RdmaSqContext.sl = SL_TEMP; // ljy:改为环境变量获取
...
HcclResult DevRdmaConnection::BuildCqContext(CqContext* context)
{
    ...
    context->RdmaCqContext.cqVa = cqInfo_.cqInfo.qBuf.base;
    context->RdmaCqContext.cqeSize = cqInfo_.cqInfo.qBuf.entrySize;
    context->RdmaCqContext.cqDepth = cqInfo_.cqInfo.qBuf.entryCnt;
    context->RdmaCqContext.headAddr = reinterpret_cast<uint64_t >(cqInfo_.cqInfo.dbrPiVa.iov_base);
    context->RdmaCqContext.tailAddr = reinterpret_cast<uint64_t >(cqInfo_.cqInfo.dbrCiVa.iov_base);
    context->RdmaCqContext.dbVa = reinterpret_cast<uint64_t >(cqInfo_.cqInfo.dbHwVa.iov_base);
...
std::vector<char> DevRdmaConnection::GetSqUniqueId() const
{
    ...
    binaryStream << qpInfo_.sqInfo.qBuf.base; // sqVa
    binaryStream << qpInfo_.sqInfo.qBuf.entrySize; // wqeSize
    binaryStream << qpInfo_.sqInfo.qBuf.entryCnt; // depth
    binaryStream << reinterpret_cast<uint64_t >(qpInfo_.sqInfo.dbrPiVa.iov_base); // headAddr
    binaryStream << reinterpret_cast<uint64_t >(qpInfo_.sqInfo.dbrCiVa.iov_base); // tailAddr
    binaryStream << reinterpret_cast<uint64_t >(qpInfo_.sqInfo.dbHwVa.iov_base); // dbVa
    binaryStream << SL_TEMP; // ljy:改为环境变量获取
...
std::vector<char> DevRdmaConnection::GetCqUniqueId() const
{
    ...
    Hccl::BinaryStream binaryStream;
    binaryStream << 0; // cqn
    binaryStream << cqInfo_.cqInfo.qBuf.base; // cqVa
    binaryStream << cqInfo_.cqInfo.qBuf.entrySize; // cqeSize
    binaryStream << cqInfo_.cqInfo.qBuf.entryCnt; // cqDepth
    binaryStream << reinterpret_cast<uint64_t >(cqInfo_.cqInfo.dbrPiVa.iov_base); // headAddr
    binaryStream << reinterpret_cast<uint64_t >(cqInfo_.cqInfo.dbrCiVa.iov_base); // tailAddr
    binaryStream << reinterpret_cast<uint64_t >(cqInfo_.cqInfo.dbHwVa.iov_base); // dbVa
    binaryStream << 0; // 0-hw 1-sw
