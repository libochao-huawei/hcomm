...
HcclResult DevRdmaConnection::BuildSqContext(SqContext* context)
{
    ...
    if (dmaMode_ == QBUF_DMA_MODE_DEFAULT) {
        context->RdmaSqContext.headAddr = reinterpret_cast<uint64_t >(SqPi_);
        context->RdmaSqContext.tailAddr = reinterpret_cast<uint64_t >(SqCi_);
    } else {
        context->RdmaSqContext.headAddr = reinterpret_cast<uint64_t >(ndaQpInfo_.sqInfo.dbrPiVa.iovBase);
        context->RdmaSqContext.tailAddr = reinterpret_cast<uint64_t >(ndaQpInfo_.sqInfo.dbrCiVa.iovBase);
    }
    context->RdmaSqContext.dbVa = reinterpret_cast<uint64_t >(ndaQpInfo_.sqInfo.dbHwVa.iovBase);
...
HcclResult DevRdmaConnection::BuildCqContext(CqContext* context)
{
    ...
    if (dmaMode_ == QBUF_DMA_MODE_DEFAULT) {
        context->RdmaCqContext.headAddr = reinterpret_cast<uint64_t >(CqPi_);
        context->RdmaCqContext.tailAddr = reinterpret_cast<uint64_t >(CqCi_);
    } else {
        context->RdmaCqContext.headAddr = reinterpret_cast<uint64_t >(ndaCqInfo_.cqInfo.dbrPiVa.iovBase);
        context->RdmaCqContext.tailAddr = reinterpret_cast<uint64_t >(ndaCqInfo_.cqInfo.dbrCiVa.iovBase);
    }
    context->RdmaCqContext.dbVa = reinterpret_cast<uint64_t >(ndaCqInfo_.cqInfo.dbHwVa.iovBase);
