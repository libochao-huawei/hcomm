...
HcclResult AicpuTsHccsChannel::SetMachinePara(hccl::MachinePara &machinePara)
{
    ...
    HccsExchangeInfo *exchangeInfo = reinterpret_cast<HccsExchangeInfo *>(machinePara.exchangeInfo.data());
    SalGetBareTgid(&exchangeInfo->grantInfo.pid);
    exchangeInfo->->grantInfo.sdid = localEp_.loc.device.superDevId;
...
HcclResult EnablePeerAccess()
{
    AicpuTsHccsEndPoint* localEpPtr{nullptr};
    CHK_RET(HcommEndpointGet(endpointHandle_, (void **)&localEpPtr));
    CHK_PTR_NULL(localEpPtr);

    HccsExchangeInfo *exchangeInfo = reinterpret_cast<HccsExchangeInfo *>(transport_->GetExchangeInfo().data());

    return localEpPtr->EnableMemAccess(&exchangeInfo->grantInfo);
}

HcclResult DisablePeerAccess()
{
    ...
    return localEpPtr->DisableMemAccess(&exchangeInfo->grantInfo);
}
