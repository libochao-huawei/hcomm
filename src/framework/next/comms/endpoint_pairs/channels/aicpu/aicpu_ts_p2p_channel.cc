...
HcclResult AicpuTsP2pChannel::Makebufs(void **memHandles, uint32_t memHandleNum,
    std::vector<std::shared_ptr<Hccl::Buffer>> &bufs)
{
    bufs.clear();
    for (uint32_t i = 0; i < memHandleNum; ++i) {
        auto locMemInfo = reinterpret_cast<hccl::CommMemHandle *>(memHandles[i]);
        HCCL_INFO("[AicpuTsP2pChannel][%s] tag[%s]", __func__, locMemInfo->memTag.c_str());
    ...
    if (channelDesc_.exchangeAllMems) {
        ...
        for (uint32_t i = 0; i < memHandleNum; ++i) {
            ...
            HCCL_INFO("[AicpuTsP2pChannel][%s] Got memHandle No.%u: addr[0x%llx], size[0x%llx], memTag[%s].",
                __func__, i, localIpcRmaBuffer->GetBufferInfo().first, localIpcRmaBuffer->GetBufferInfo().second, 
                localIpcRmaBuffer->Describe().c_str());
            bufs_.emplace_back(std::move(std::make_shared<Hccl::Buffer>(
                localIpcRmaBuffer->GetBufferInfo().first, localIpcRmaBuffer->GetBufferInfo().second, 
                localIpcRmaBuffer->Describe().c_str())
            ));
        }
    } else {
...
HcclResult AicpuTsP2pChannel::BuildConnection()
{
    ...
    EXECEPTION_CATCH(
        p2pConn = std::make_unique<Hccl::P2PConnection>(socket_, "p2p_channel"),
        return HCCL_E_PTR
    );
...
HcclResult AicpuTsP2pChannel::BuildP2PTransport()
{
...
HcclResult AicpuTsP2pChannel::PackOpData(std::vector<char> &data)
{
    ...
    binaryStream << memTransport_->GetUniqueId();
...
HcclResult AicpuTsP2pChannel::UpdateMemInfo(void **memHandles, uint32_t memHandleNum)
{
