...
{
    Stream localStream = Stream(StreamType::STREAM_TYPE_ONLINE);
...
HcclResult ChannelProcess::SaveChannels(ChannelHandle* targetChannels, ChannelHandle* userChannels, 
    uint32_t channelNum, CommEngine engine, aclrtBinHandle binHandle, bool needProfiling) 
{
    if (engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) {
        ...
        for (int64_t i = 0; i < channelNum; i++) {
