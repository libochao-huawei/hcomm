...
HcclResult ChannelProcess::WithChannelByHandleLocked(ChannelHandle inHandle, Func&& func)
{
    ...
    if (itC == g_ChannelMap.end() || !itC->second) {
        HCCL_ERROR(
            "[%s] channel not found in g_ChannelMap, deviceId[%d], inHandle[0x%llx], mappedHandle[0x%llx].", __func__,
...
HcclResult ChannelProcess::ChannelGetStatus(const ChannelHandle* channelList, uint32_t listNum, int32_t* statusList)
{
    ...
    for (uint32_t i = 0; i < listNum; ++i) {
        ...
        CHK_PRT_RET(
            status == ChannelStatus::SOCKET_TIMEOUT, HCCL_ERROR("[%s] TIMEOUT, status[%d]", __func__, status),
            HCCL_E_TIMEOUT);
...
HcclResult ChannelProcess::SaveChannels(
...
HcclResult
ChannelProcess::ChannelGetRemoteMem(ChannelHandle channelHandle, CommMem** remoteMem, uint32_t* memNum, char** memTags)
{
