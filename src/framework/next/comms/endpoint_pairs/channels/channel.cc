    ...
    HcommChannelDesc channelDesc, std::unique_ptr<Channel>& channelPtr)
{
    ...
    HcclResult initRet = channelPtr->Init();
    if (UNLIKELY(initRet != HCCL_SUCCESS)) {
        channelPtr.reset();
        if (initRet == HCCL_E_AGAIN) {
            HCCL_WARNING("[%s]call trace: hcclRet -> %d", __func__, initRet);
        } else {
            HCCL_ERROR("[%s]call trace: hcclRet -> %d", __func__, initRet);
        }
        return initRet;
    }
    return HCCL_SUCCESS;
