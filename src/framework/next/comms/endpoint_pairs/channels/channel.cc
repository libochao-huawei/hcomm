    ...
    switch (engine) {
        ...
        case COMM_ENGINE_AICPU_TS:
            if (channelDesc.remoteEndpoint.protocol == COMM_PROTOCOL_UBOE) {
                ...
                channelPtr.reset(new (std::nothrow) AicpuTsRoceChannel(endpointHandle, channelDesc, engine));
            } else {
        ...
        case COMM_ENGINE_AIV:
            if (channelDesc.remoteEndpoint.protocol == COMM_PROTOCOL_ROCE) {
                EXECEPTION_CATCH(channelPtr = std::make_unique<AicpuTsRoceChannel>(endpointHandle, channelDesc, engine),
                    return HCCL_E_PARA);
            } else {
                channelPtr.reset(
