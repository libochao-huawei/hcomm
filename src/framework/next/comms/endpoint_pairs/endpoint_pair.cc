...
HcclResult EndpointPair::GetSocket(const std::string &socketTag, Hccl::Socket*& socket, const uint32_t listenPort)
{
...
HcclResult EndpointPair::GetSocket(const uint32_t myRank, const uint32_t rmtRank,
    const std::string &socketTag, Hccl::Socket*& socket, const uint32_t listenPort)
{
    ...
    if (localEndpointDesc_.loc.locType == EndpointLocType::ENDPOINT_LOC_TYPE_HOST) {
        CHK_RET(this->GetSocket(socketTag, socket, listenPort));
        return HCCL_SUCCESS;
    }
