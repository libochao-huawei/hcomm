...
HcclResult SocketMgr::PutSocket(Hccl::SocketConfig*& socketConfig, Hccl::Socket*& socket)
{
    CHK_PTR_NULL(socket);
...
HcclResult SocketMgr::UpdateSocketConfig(Hccl::SocketConfig*& socketConfig, Hccl::Socket*& socket)
{
    for (auto it = socketMap_.begin(); it != socketMap_.end(); ++it) {
        if (it->second.get() == socket) {
