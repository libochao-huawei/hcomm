...
HcclResult SocketMgr::GetSocket(const Hccl::SocketConfig &socketConfig, Hccl::Socket*& socket)
{
    ...
    HCCL_INFO("[SocketMgr][%s] socketConfig tag is %s", __func__, socketConfig.tag.c_str());
    ...
    for (; it != socketMap_.end(); ++it) {
        ...
        HCCL_INFO("[SocketMgr][%s] socketConfig link.GetLocalPort().GetAddr() is %s, map link.GetLocalPort().GetAddr() is %s", __func__, socketConfig.link.GetLocalPort().GetAddr().Describe(), it->first.link.GetLocalPort().GetAddr().Describe().c_str());
        HCCL_INFO("[SocketMgr][%s] socketConfig link.GetRemotePort().GetAddr() is %s, map link.GetRemotePort().GetAddr() is %s", __func__, socketConfig.link.GetRemotePort().GetAddr().Describe(), it->first.link.GetRemotePort().GetAddr().Describe().c_str());
        ...
        if (it->first.tag == socketConfig.tag) {
            break;
        }
    }
