    ...
    HcclDataType dataType, HcclReduceOp op, HcclComm comm, aclrtStream stream)
{
    ...
    CHK_PRT_RET(
        inputCount == 0,
        HCCL_WARNING(
            "The inputCount is 0, this ReduceScatter v has no task to execute, "
            "returning success."),
        HCCL_SUCCESS);
    RPT_INPUT_ERR(
    ...
    HcclDataType dataType, HcclComm comm, aclrtStream stream)
{
    ...
    CHK_PRT_RET(
        outputCount == 0,
        HCCL_WARNING(
            "The outputCount is 0, this AllGatherV has no task to execute, "
            "returning success."),
        HCCL_SUCCESS);
    RPT_INPUT_ERR(
        recvBuf == nullptr, "EI0003", std::vector<std::string>({"ccl_op", "value", "parameter", "expect"}),
