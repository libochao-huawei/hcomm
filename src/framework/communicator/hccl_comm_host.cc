...
HcclResult hcclComm::GetEndpointDesc(
...
HcclResult hcclComm::InitCollComm(void *commV2, void *rankGraph, uint32_t userRank, HcclMem cclBuffer,
    const std::string &commName, HcclCommConfig *config)
{
    ...
    EXECEPTION_CATCH(collComm_ = std::make_unique<CollComm>(commV2, userRank, commName, callbacks), return HCCL_E_PTR);
...
HcclResult hcclComm::KernelLaunchAicpuCommInit()
{
    ...
    u16 timeOut = NOTIFY_DEFAULT_WAIT_TIME > std::numeric_limits<uint16_t>::max() ? std::numeric_limits<uint16_t>::max()
                                                                                  : NOTIFY_DEFAULT_WAIT_TIME;
    CHK_RET(AicpuAclKernelLaunch(localStream.ptr(), reinterpret_cast<void *>(&commAicpuParam_), sizeof(commAicpuParam_),
