...
HcclResult SymmetricMemory::Init()
{
    ...
    size_t totalHeapSize
        = static_cast<size_t>(stride_ * rankSize_); // 每个rank都预留一个总大小为 totalHeapSize 的VA空间。
    ...
    HCCL_INFO(
        "[SymmetricMemory][Init] SymmetricMemory initialized. Rank[%u], Local Heap Base: %p, Stride: %llu, RankSize: "
        "%u.",
    ...
    void* ptr, size_t size, void** baseUserVa, size_t* baseVaSize, aclrtDrvMemHandle* paHandle)
{
    ...
    HCCL_INFO(
        "[SymmetricMemory][GetMemoryInfo] Retained paHandle[%p] for baseUserVa[%p],  baseVaSize[%zu]. Total Stride: "
        "%zu",
