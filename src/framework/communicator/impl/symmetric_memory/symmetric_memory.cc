...
HcclResult SymmetricMemory::RegisterInternal(aclrtDrvMemHandle &paHandle, size_t offset, size_t mapSize)
{
    ...
    if(paMappingMap_[paHandle]->refCount == 1) {
        if (aclrtMemExportToShareableHandleV2(paHandle, 0, ACL_MEM_SHARE_HANDLE_TYPE_FABRIC, (void*)&shareableHandle) != ACL_SUCCESS) {
            HCCL_ERROR("[SymmetricMemory][RegisterInternal] Failed to export shareable handle. offset: %zu, size: %zu", offset, mapSize);
            return HCCL_E_DRV;
        }
