...
constexpr u32 SYMMETRIC_MEMORY_MAX_RANK_SIZE = 640;
...
struct SymmetricPeerChannelInfo {
    u32 peerRank;
    u32 channelOffset;
    u32 channelNum;
    u32 reserved;
...
struct SymmetricUrmaInfo {
    SymmetricPeerChannelInfo* peerChannels;
    u32 peerChannelNum;
    u32 reserved;
    ChannelHandle* channelHandles;
    u32 channelHandleNum;
};

struct SymmetricChannelResource {
    std::vector<SymmetricPeerChannelInfo> peerChannels;
    std::vector<ChannelHandle> channelHandles;
    std::string memTag;
    void* memHandle{nullptr};
};

using SymmetricChannelCreateCallback = std::function<HcclResult(void*, size_t, SymmetricChannelResource&)>;
using SymmetricChannelDestroyCallback = std::function<void(const SymmetricChannelResource&)>;
...
public:
    ...
    void SetChannelCallbacks(SymmetricChannelCreateCallback createCallback,
...
private:
    ...
    HcclResult RegisterChannelMode(void* ptr, size_t size, void** devWin);
    HcclResult DeregisterChannelMode(void* devWin);
    HcclResult GetMemoryRange(void* ptr, size_t size, void** baseUserVa, size_t* baseVaSize);
    HcclResult BuildWindowUrmaInfo(std::shared_ptr<SymmetricWindow> &win,
        const SymmetricChannelResource &channelResource);
    void ReleaseWindowUrmaInfo(std::shared_ptr<SymmetricWindow> &win);
...
private:
    ...
    u32 rank_{0};
    u32 rankSize_{0};
    SymmetricMemoryMode mode_{SymmetricMemoryMode::VMM};
    ...
    SymmetricChannelCreateCallback channelCreateCallback_{nullptr};
    SymmetricChannelDestroyCallback channelDestroyCallback_{nullptr};
    std::unordered_map<void*, SymmetricChannelResource> channelResourceMap_;
