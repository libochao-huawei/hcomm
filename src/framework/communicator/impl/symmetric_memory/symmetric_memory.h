...
public:
    ...
    HcclResult UpdatePeerPointers(u32 peerRank, const CommMem *remoteMems, char **memTags, u32 memNum);
...
private:
    ...
    HcclResult EnsurePeerBasePtrs(std::shared_ptr<SymmetricWindow> &win);
    HcclResult SyncWindowToDevice(std::shared_ptr<SymmetricWindow> &win);
    void ReleaseWindowPeerPointers(std::shared_ptr<SymmetricWindow> &win);
...
private:
    ...
    std::unordered_map<void*, std::vector<void*>> peerBasePtrsHostMap_;
