        ...
        if (rankTable.rankList[i].rankId == userRank_) {
            ...
            if (superDeviceId_ != static_cast<u32>(drvSuperDeviceID)) {
                RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({ "error_reason" }),
                    std::vector<std::string>({ "Value [0x" + std::to_string(superDeviceId_) + "] for rankTable variable [super_device_id] is invalid, expected value 0x" + std::to_string(drvSuperDeviceID) + "." }));
