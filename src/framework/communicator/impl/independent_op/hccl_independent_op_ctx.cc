...
HcclResult HcclEngineCtxCreate(HcclComm comm, const char* ctxTag, CommEngine engine, uint64_t size, void** ctx)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[%s] ctxTag length exceeds maximum length, ctxTag length[%zu], max length[%d]", __func__,
            strlen(ctxTagTmp), HCCL_RES_TAG_MAX_LEN),
        HCCL_E_PARA);
    ...
    HCCLV2_FUNC_RUN([&]() -> HcclResult {
        ...
        CHK_PRT_RET(
            ...
            HCCL_ERROR(
                ...
                ctxTagTmp, GetEnumToString(COMMENGINE_STATUS_STR_MAP, engine).c_str(), size, ret),
            ret);
        HCCL_RUN_INFO(
    ...
    HcclComm comm, CommEngine engine, const char* ctxTag, const void* srcCtx, uint64_t size, uint64_t dstCtxOffset)
{
    ...
    HCCLV2_FUNC_RUN([&]() -> HcclResult {
        ...
        CHK_PRT_RET(
            ...
            HCCL_WARNING(
                ...
                " ret[%d]",
                __func__, ctxTagTmp, GetEnumToString(COMMENGINE_STATUS_STR_MAP, engine).c_str(), size, dstCtxOffset,
                ret),
            ret);
        HCCL_RUN_INFO(
            "[%s] success, ctxTag[%s], engine[%s], srcCtx[%p], size[%llu], dstCtxOffset[%llu], group[%s]", __func__,
            ctxTagTmp, GetEnumToString(COMMENGINE_STATUS_STR_MAP, engine).c_str(), srcCtx, size, dstCtxOffset,
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "[%s] Failed to copy CommEngineCtx with ctxTag[%s], engine[%s], size[%llu], dstCtxOffset[%llu],"
            " ret[%d]",
            __func__, ctxTagTmp, GetEnumToString(COMMENGINE_STATUS_STR_MAP, engine).c_str(), size, dstCtxOffset, ret);
        return ret;
    }
...
HcclResult HcclEngineCtxDestroy(HcclComm comm, const char* ctxTag, CommEngine engine)
{
    ...
    HCCLV2_FUNC_RUN([&]() -> HcclResult {
        ...
        HCCL_RUN_INFO(
            ...
            GetEnumToString(COMMENGINE_STATUS_STR_MAP, engine).c_str(), hcclComm->GetIdentifier().c_str());
        return HCCL_SUCCESS;
    }());
#endif
