...
HcclResult HcclEngineCtxDestroy(HcclComm comm, cosnt char *ctxTag, CommEngine engine)
{
    CHK_PTR_NULL(comm);
    CHK_PTR_NULL(ctxTag);
    ...
    HCCLV2_FUNC_RUN(
        [&]() -> HcclResult {
            ...
            CHK_PTR_NULL(myRank);
            EngineCtxs* engineCtxs = myRank->GetEngineCtx();
            HcclResult ret = HCCL_SUCCESS;
...
if (hcclComm->IsCommunicatorV2()) {
    ...
    ContextManager* contextMgr = collComm->GetContextManager();
    CHK_PTR_NULL(contextMgr);
    ret = contextMgr->DestroyCommEngineCtx(ctxTag, engine);
}