...
HcclResult ThreadMgr::SupplementNotify(CommEngine engine, uint32_t threadNum, uint32_t notifyNumPerThread)
{
    ...
    for (uint32_t i = 0; i < threads.size() && i < threadNum; ++i) {
        ...
        if (notifyNumPerThread > notifyNum) {
            CHK_RET(CheckNotifyNum(engine, 1, (notifyNumPerThread - notifyNum)));
            ret = threads[i]->SupplementNotify(notifyNumPerThread - notifyNum);
            if (ret != HCCL_SUCCESS) {
                HCCL_ERROR("[ThreadMgr][SupplementNotify] Failed to supplement notify[%u] on thread index %u",
                    (notifyNumPerThread - notifyNum), i);
                return ret;
            }
    ...
    if ((engine == COMM_ENGINE_AICPU || engine == COMM_ENGINE_AICPU_TS) && needSupplementThread.size() > 0) {
        ...
        EXECEPTION_CATCH(threadHandle = std::make_unique<ThreadHandle[]>(threads.size()),
            return HCCL_E_PTR);
        ...
        ret = AicpuLaunchMgr::SupplementNotifyKernelLaunch(threads, commId_, threadHandle, binHandle_);
