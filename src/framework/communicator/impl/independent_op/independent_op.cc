    ...
    HDCommunicateParams &kfcStatusTransferD2HParams, CCLBufferManager &bufferManager)
{
    ...
    // Aicpu通信域初始化参数
    snprintf_s(commAicpuParam_.hcomId, HCOMID_MAX_SIZE, HCOMID_MAX_SIZE, "%s", commId_.c_str());
    ...
    char *debugLevel = getenv("HCCL_DEBUG_LEVEL");
    HCCL_INFO("[IndependentOp][%s] Hcom[%s] threadNum[%u], notifyPerThread[%u], cclBufferSize[%llu], deviceLogicId[%u], "
        "devicePhyId[%u], deviceType[%u], userRank[%u], userRankSize[%u], debugLevel[%s]",
        ...
        commAicpuParam_.deviceType, commAicpuParam_.userRank, commAicpuParam_.userRankSize, debugLevel);
    return HCCL_SUCCESS;
}
