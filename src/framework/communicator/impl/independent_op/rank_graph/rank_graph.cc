...
bool RankGraphV1::IsRoceInSameServer(uint32_t netLayer, const RankInfo_t& srcInfo, const RankInfo_t& dstInfo)
{
    ...
    HCCL_INFO(
        "[%s] netLayer[%u], devType[%u], srcPhyId[%u], dstPhyId[%u], isStandardCard[%d], isDiffDeviceModule[%d] "
        "IntraRoceSwitch[%u] \n",
...
RankGraphV1::GetLinks(uint32_t netLayer, uint32_t srcRank, uint32_t dstRank, CommLink** linkList, uint32_t* listSize)
{
    ...
    if (netLayer > HCCL_NETLAYER_2) {
        HCCL_ERROR(
            "[RankGraphV1][%s] srcRank[%u] and dstRank[%u] are do not have netLayer[%u]", __func__, srcRank, dstRank,
            netLayer);
        return HCCL_E_PARA;
    }
    ...
    HCCL_INFO(
        "[RankGraphV1][%s] links, netLayer[%u] srcRank[%u] dstRank[%u] protocol[%u]", __func__, netLayer, srcRank,
        dstRank, protocol);
...
HcclResult RankGraphV1::InitSuperPodRankInfo()
{
    ...
    for (u32 index = 0; index < rankInfoList.size(); index++) {
        ...
        HCCL_DEBUG(
            "[RankGraphV1][%s] superPodIdx[%d],superPodId[%s]", __func__, rankInfoList[index].superPodIdx,
    ...
    if (superPodToRank_.find(rankData_.superPodIdx) != superPodToRank_.end()) {
        ...
        HCCL_INFO(
            "[RankGraphV1][%s] curRank[%d], curSuperPod[%s] superPodToRanklist[%s]", __func__, rankData_.userRank,
