...
void RankGraph::PrintLinksInfo(CommLink &link)
{
    ...
    // 打印【链路属性】linkAttr 信息
    HCCL_INFO("[RankGraph][%s] linkProtocol[%d] hop[%u]", __func__, link.linkAttr.linkProtocol, link.linkAttr.hop);
}
    ...
    *listSize = static_cast<uint32_t>(links.size());
    if (links.empty()) {
        ...
        *linkList = links.data(); // 连续数组首地址
        HCCL_INFO("[RankGraph][%s] srcRank[%u] dstRank[%u] linkList[%p] linkNum[%u]", __func__, srcRank, dstRank, *linkList, *listSize);
    }
