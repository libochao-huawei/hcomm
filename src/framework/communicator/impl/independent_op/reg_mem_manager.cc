...
HcclResult RegMemMgr::RegisterMemory(CommMem mem, const char *memTag, HcommMemHandle *memHandle)
{
    ...
    *memHandle = static_cast<HcommMemHandle>(rawHandle);
    ...
    info.mem = mem;
    if (memTag != nullptr) {
        size_t tagLen = strnlen(memTag, HCCL_RES_TAG_MAX_LEN - 1);
        if (tagLen > 0) {
            CHK_SAFETY_FUNC_RET(memcpy_s(info.memTag.data(), info.memTag.size(), memTag, tagLen));
        }
    }
    memInfoVec_.emplace_back(*memHandle, info);
    HCCL_INFO("[RegMemMgr][RegisterMemory] registered handle[%p] for addr[%p] size[%llu] tag[%s]",
        *memHandle, mem.addr, mem.size, info.memTag.data());
    return HCCL_SUCCESS;
}
