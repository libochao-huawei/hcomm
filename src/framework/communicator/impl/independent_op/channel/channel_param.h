...
struct HcommRoceChannelRes {
    void *localMem = nullptr;            // device 地址，指向 localMemCount 个 MemDetails 连续数组；可与本结构体不在同一段 device 内存
    void *remoteMem = nullptr;         // device 地址，指向 remoteMemCount 个 MemDetails 连续数组；可与本结构体不在同一段 device 内存
    u32 localMemCount = 0;             // 本端内存数量
    u32 remoteMemCount = 0;            // 远端内存数量
    s64 chipId{LLONG_MAX};
    HcclQpInfoV2 QpInfo[RDMA_QP_MAX_NUM];
    u32 qpsPerConnection{1};
};
