    ...
    ThreadType type, const ThreadCongif *config, ThreadHandle *threads)
{
    ...
    if (hcclComm->IsCommunicatorV2()) {
        ...
        ret = HcclThreadAcquire(comm, engine, threadNum, notifyNumPerThread, threads);
    }
    if (ret != HCCL_SUCCESS) {
    ...
    uint32_t notifyNumPerThread, ThreadHandle *threads)
{
    ...
    if (hcclComm->IsCommunicatorV2()) {
        ...
        ret = engineResMgr->HcclThreadAcquireV2(engine, threadNum, config.get(), threads, threadId);
        if (ret != HCCL_SUCCESS) {
