    ...
    HcclComm comm, CommEngine engine, uint32_t threadNum, uint32_t notifyNumPerThread, ThreadHandle *threads)
{
    ...
    auto *hcclComm = static_cast<hccl::hcclComm *>(comm);
    std::string commId = hcclComm->GetIdentifier();
    HCCL_RUN_INFO("Entry-%s:comm[%s] engine[%u] reqThreadNum[%u] notifyNumPerThread[%u]", __func__, commId.c_str(),
    ...
    if (hcclComm->IsCommunicatorV2()) {
        ...
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[%s] failed to create threads for engine[%d],threadsNum[%u], notifyNumperThread[%u]", __func__,
                engine, threadNum, notifyNumPerThread);
            return ret;
        }
        ...
        if (engine == CommEngine::COMM_ENGINE_AICPU_TS || engine == CommEngine::COMM_ENGINE_AICPU) {
            ...
            for (u32 num = 0; num < threadNum; ++num) {
                ...
                if (hert != HCCL_SUCCESS) {
                    HCCL_ERROR("[HcclThreadAciqure] ReportThreadAciqureKernel HcommThreadRegisterDfx failed "
                               "hert:[%d],num:[%d]",
                        hert, num);
                    return HCCL_E_PTR;
                }
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[%s] Failed to create threads for engine[%d], threadNum[%u], notifyNumPerThread[%u]", __func__,
            engine, threadNum, notifyNumPerThread);
        return ret;
    }

    HCCL_INFO("[%s] Allocated %u threads for engine[%d], notifyPerThread[%u]", __func__, threadNum, engine,
        notifyNumPerThread);
    return HCCL_SUCCESS;
}
    ...
    HcclComm comm, CommEngine engine, aclrtStream stream, uint32_t notifyNum, ThreadHandle *thread)
{
    ...
    std::string commId = hcclComm->GetIdentifier();
    HCCL_RUN_INFO(
        "Entry-%s:comm[%s] engine[%u] notifyNum[%u] stream[%p]", __func__, commId.c_str(), engine, notifyNum, stream);
    HcclResult ret = HCCL_SUCCESS;
    if (hcclComm->IsCommunicatorV2()) {
        hccl::CollComm *collComm = hcclComm->GetCollComm();
        CHK_PTR_NULL(collComm);
    ...
    HCCL_INFO("[HcclThreadAcquireWithStream] Allocated thread for engine[%d], stream[%p], notifyNum[%u]", engine,
        stream, notifyNum);
    return HCCL_SUCCESS;
}
    ...
    HcclComm comm, CommEngine commEngine, ::NotifyType notifyType, uint32_t notifyNum, NotifyHandle **notifyHandleList)
{
    ...
    CHK_PRT_RET(!IsValidCommEngine(commEngine), HCCL_ERROR("[%s] commEngine[%u] is invalid", __func__, commEngine),
        HCCL_E_PARA);
    CHK_PRT_RET(
    ...
    auto *hcclComm = static_cast<hccl::hcclComm *>(comm);
    std::string commId = hcclComm->GetIdentifier();
    HCCL_RUN_INFO("Entry-%s:comm[%s] commEngine[%u] notifyType[%u] notifyNum[%p]", __func__, commId.c_str(), commEngine,
        notifyType, notifyNum);
    HcclResult ret = HCCL_SUCCESS;
    if (hcclComm->IsCommunicatorV2()) {
        hccl::CollComm *collComm = hcclComm->GetCollComm();
        CHK_PTR_NULL(collComm);
    ...
    HCCL_RUN_INFO("[%s] Allocated notify for commEngine[%d], notifyType[%p], notifyNum[%u]", __func__, commEngine,
        notifyType, notifyNum);
    return HCCL_SUCCESS;
}
    ...
    CommEngine dstCommEngine, ThreadHandle *exportedThreads)
{
    ...
    std::string commId = hcclComm->GetIdentifier();
    HCCL_INFO("Entry-[%s]:comm[%s], threadNum[%u], commEngine[%d], threadsPtr[%p], exportedThreadsPtr[%p]", __func__,
        commId.c_str(), threadNum, dstCommEngine, threads, exportedThreads);
    HcclResult ret;
    if (hcclComm->IsCommunicatorV2()) {
        hccl::CollComm *collComm = hcclComm->GetCollComm();
        CHK_PTR_NULL(collComm);
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[%s] Thread export failed. Export threadNum[%u], commEngine[%d], threadsPtr[%p], exportedThreadsPtr[%p]",
            __func__, threadNum, dstCommEngine, threads, exportedThreads),
        ret);
...
HcclResult HcclThreadResGetInfo(
    HcclComm comm, ThreadHandle thread, ThreadResType resType, uint32_t infoLen, void **info)
{
    ...
    hccl::hcclComm *hcclComm = static_cast<hccl::hcclComm *>(comm);
    std::string commId = hcclComm->GetIdentifier();
    HCCL_INFO("Entry-[%s]:comm[%s], thread[0x%llx], resType[%d], infoLen[%u], info[%p]", __func__, commId.c_str(),
        thread, static_cast<int32_t>(resType), infoLen, info);
    HcclResult ret = HCCL_SUCCESS;
    if (hcclComm->IsCommunicatorV2()) {
        hccl::CollComm *collComm = hcclComm->GetCollComm();
        CHK_PTR_NULL(collComm);
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[%s] thread resource get info failed. thread[0x%llx], resType[%d], infoLen[%u], info[%p]", __func__,
            thread, static_cast<int32_t>(resType), infoLen, info),
        ret);
