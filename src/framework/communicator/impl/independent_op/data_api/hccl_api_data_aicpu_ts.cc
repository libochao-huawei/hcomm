...
int32_t HcommThreadJoin(ThreadHandle thread, uint32_t timeout)
{
    ...
    if (threadPtr->IsDeviceA5()) {
        ...
        HCCL_INFO("[%s] aicpu stream sqid[%d] tail[%u]", __func__, sqId, tail);
        ...
        do {
            ...
            if (curUsec - startUsec > NANOSECOND_TO_SECOND * timeout) {
                HCCL_ERROR("[%s] timeout %lus. curhead:%u, curtall:%u, sqId:%d",
        ...
        HCCL_INFO("[%s] SUCCESS. RTSQ's head[%d] == tail[%d].", __func__, head, tail);
        return HCCL_SUCCESS;
    }
