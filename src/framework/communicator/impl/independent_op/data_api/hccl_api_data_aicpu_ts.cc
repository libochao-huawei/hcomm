...
static int32_t HcommBatchTransferOnThreadA5(ThreadHandle thread, ChannelHandle channel,
    const HcommBatchTransferDesc *transferDescs, uint32_t transferDescNum)
{
    ...
    EXECEPTION_CATCH(ubTransportLitePtr->BatchTransfer(locSlices, rmtSlices, transferOps, notifyWaitInfos, *streamLitePtr), return HCCL_E_INTERNAL);
    return HCCL_SUCCESS;
}
    ...
    const HcommBatchTransferDesc *transferDescs, uint32_t transferDescNum)
{
    ...
    AddThread(thread);
    if (threadPtr->IsDeviceA5()) {
        return HcommBatchTransferOnThreadA5(thread, channel, transferDescs, transferDescNum);
    }
    Stream *stream = GetStream(thread);
    CHK_PTR_NULL(stream);
