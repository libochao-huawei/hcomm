    ...
    uint64_t *lenList, uint32_t *rw, HcommDataType *dataType, HcommReduceOp *reduceOp, uint32_t count)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        Hccl::RmaBufferLite locRmaBuf;
        HcclResult ret = ubTransportLitePtr->BuildLocRmaBufferLite(reinterpret_cast<uintptr_t>(srcList[i]), lenList[i], locRmaBuf);
        CHK_PRT_RET(ret != HCCL_SUCCESS,
        ...
        const Hccl::Buffer rmtBuf{reinterpret_cast<uintptr_t>(dstList[i]), lenList[i]};
