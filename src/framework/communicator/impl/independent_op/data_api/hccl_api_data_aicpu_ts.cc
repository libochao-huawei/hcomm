    ...
    uint64_t len, uint32_t remoteNotifyIdx, u64 aicpuTaskId)
{
    ...
    HCCL_DEBUG("[%s] thread[0x%llx], channel[0x%llx], dst[0x%llx], src[0x%llx], len[%llu], remoteNotifyIdxIdx[%u], dpuDevId[%d], aicpuTaskId[%lu].",
...
int32_t HcommChannelFenceOnThread(ThreadHandle thread, ChannelHandle channel, uint64_t aicpuTaskId)
{
    HCCL_DEBUG("[%s] thread[0x%llx], channel[0x%llx], aicpuTaskId[0x%llx].", __func__, thread, channel, aicpuTaskId);
    Thread *const threadPtr = reinterpret_cast<Thread *>(thread);
    CHK_PTR_NULL(threadPtr);
    if (threadPtr->IsDeviceA5()) {
        auto *const ubTransportLitePtr = reinterpret_cast<Hccl::UbTransportLiteImpl *>(channel);
