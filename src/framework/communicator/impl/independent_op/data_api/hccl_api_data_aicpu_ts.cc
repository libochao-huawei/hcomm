...
static const std::unordered_map<HcommDataType, uint32_t> HCOMM_DATA_TYPE_SIZE_MAP = {
    ...
    const HcommBatchTransferDesc *transferDescs, uint32_t transferDescNum)
{
    ...
    for (uint32_t i = 0; i < transferDescNum; i++) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[%s] FAIL at ParseData for index %u. thread[0x%llx], channel[0x%llx].",
            __func__, i, thread, channel), ret);
        CHK_PTR_NULL(rmt);
        CHK_PTR_NULL(loc);

        ret = ubTransportLitePtr->BuildLocRmaBufferLite(reinterpret_cast<uintptr_t>(loc), len, locRmaBuf);
        CHK_PRT_RET(ret != HCCL_SUCCESS,
        ...
        HCCL_DEBUG("[%s] Prepared transfer op for index %u. thread[0x%llx], channel[0x%llx], rmt[0x%llx], loc[0x%llx], len[0x%llx], tfType[%u], dataType[%d], reduceOp[%d].",
