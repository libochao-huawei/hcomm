...
static bool ShouldSkipAicpuDfx()
{
    ...
    if (!(l0State || l1State || taskExceptionEnable)) {
        HCCL_INFO(
            "[%s] l0State[%d], l1State[%d], taskExceptionEnable[%d], skip aicpu dfx", __func__, l0State, l1State,
            taskExceptionEnable);
        return true;
    }
    return false;
...
{
    HCCL_INFO(
        "[%s] START. thread[0x%llx], channel[0x%llx], transferDescNum[%u].", __func__, thread, channel,
        transferDescNum);
