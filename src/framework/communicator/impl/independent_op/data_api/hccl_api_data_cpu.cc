...
int32_t HcommChannelFence(ChannelHandle channel) { return HcommChannelFenceOnThread(0, channel); }
...
static HcclResult RegAicpuTaskException(HcclDfxOpInfo* dfxOpInfo, hccl::CollComm* collComm)
{
    ...
    CHK_PRT_RET(
        !notify,
        HCCL_ERROR(
            "[%s]GetNotify null, thread[%llu], notifyIdx[%u]", __func__, dfxOpInfo->cpuTsThread,
            dfxOpInfo->cpuWaitAicpuNotifyIdx),
        HCCL_E_PTR);
