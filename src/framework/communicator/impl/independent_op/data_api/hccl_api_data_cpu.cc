...
int32_t HcommWriteWithNotifyNbiOnThread(ThreadHandle thread, ChannelHandle channel, void *dst, const void *src,
    uint64_t len, uint32_t remoteNotifyIdx, u64 aicpuTaskId)
{
    s32 dpuDevId;
    CHK_RET(hrtGetDevice(&dpuDevId));
    HCCL_INFO("[%s] START. thread[0x%llx], channel[0x%llx], dst[0x%llx], src[0x%llx], len[%llu], remoteNotifyIdxIdx[%u], dpuDevId[%d], aicpuTaskId[%lu].",
        __func__, thread, channel, dst, src, len, remoteNotifyIdx, dpuDevId, aicpuTaskId);
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[%s] FAIL. thread[0x%llx], channel[0x%llx], dst[0x%llx], src[0x%llx], len[%llu], remoteNotifyIdxIdx[%u], dpuDevId[%u], aicpuTaskId[%lu].",
            __func__, thread, channel, dst, src, len, remoteNotifyIdx, dpuDevId, aicpuTaskId), ret);
    HCCL_INFO("[%s] SUCCESS.", __func__);
    return HCCL_SUCCESS;
}
...
int32_t HcommWriteWithNotifyNbi(ChannelHandle channel, void *dst, const void *src,
    uint64_t len, uint32_t remoteNotifyIdx)
{
    return HcommWriteWithNotifyNbiOnThread(0, channel, dst, src, len, remoteNotifyIdx, 0);
}
    ...
    u64 aicpuTaskId)
{
    s32 dpuDevId;
    CHK_RET(hrtGetDevice(&dpuDevId));
    HCCL_INFO("[%s] START. thread[0x%llx], channel[0x%llx], remoteNotifyIdx[%u], dpuDevId[%d], aicpuTaskId[%lu].",
        __func__, thread, channel, remoteNotifyIdx, dpuDevId, aicpuTaskId);
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[%s] FAIL. thread[0x%llx], channel[0x%llx], remoteNotifyIdx[%u], dpuDevId[%u], aicpuTaskId[%lu].",
            __func__, thread, channel, remoteNotifyIdx, dpuDevId, aicpuTaskId), ret);
    HCCL_INFO("[%s] SUCCESS.", __func__);
    return HCCL_SUCCESS;
}
...
int32_t HcommChannelNotifyRecord(ChannelHandle channel, uint32_t remoteNotifyIdx)
{
    ...
    return HcommChannelNotifyRecordOnThread(0, channel, remoteNotifyIdx, 0);
}
    ...
    u64 aicpuTaskId)
{
    s32 dpuDevId;
    CHK_RET(hrtGetDevice(&dpuDevId));
    HCCL_INFO("[%s] START. thread[0x%llx], channel[0x%llx], localNotifyIdx[%u], timeOut[%u], dpuDevId[%d], aicpuTaskId[%lu].",
        __func__, thread, channel, localNotifyIdx, timeOut, dpuDevId, aicpuTaskId);
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[%s] FAIL. thread[0x%llx], channel[0x%llx], localNotifyIdx[%u], timeOut[%u], dpuDevId[%u], aicpuTaskId[%lu].",
            __func__, thread, channel, localNotifyIdx, timeOut, dpuDevId, aicpuTaskId), ret);
    HCCL_INFO("[%s] SUCCESS.", __func__);
    return HCCL_SUCCESS;
}
...
int32_t HcommChannelNotifyWait(ChannelHandle channel, uint32_t localNotifyIdx, uint32_t timeOut)
{
    ...
    return HcommChannelNotifyWaitOnThread(0, channel, localNotifyIdx, timeOut, 0);
}
