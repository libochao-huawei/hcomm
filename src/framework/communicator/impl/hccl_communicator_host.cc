...
namespace hccl
{
    ...
    */
    ...
    HcclResult HcclCommunicator::ExecOp(HcclCMDType opType, OpParam &opParam, bool isCustom)
    {
        ...
        // 根据是否是LocalGather/LocalScatter之间的算子，调整CCL中转buffer使用前半段还是全部
        auto& algRes = resMap_[newTag];
        if (useHalfCclBuffer_) {
            algRes.cclInputMem = cclBufferManager_.GetInCCLbuffer().range(0,
                cclBufferManager_.GetInCCLbufferSize() / 2);
            algRes.cclOutputMem = cclBufferManager_.GetOutCCLbuffer().range(0,
                cclBufferManager_.GetOutCCLbufferSize() / 2);
        } else {
            algRes.cclInputMem = cclBufferManager_.GetInCCLbuffer();
            algRes.cclOutputMem = cclBufferManager_.GetOutCCLbuffer();
        }
                                              ...
                                              void* gatheredBuf, u32 splitNum, HcclDataType dataType, HcclRtStream stream)
    {
        ...
        if (GetIfProfile()) {
            ...
            CHK_RET_AND_PRINT_IDE(ProfilingManagerPub::CallMsprofReportHostApi(HcclCMDType::HCCL_CMD_ALLGATHER,
                beginTime, totalCount, dataType, algType, groupName, numBlocks), tag.c_str());
        }
        return HCCL_SUCCESS;
    }
