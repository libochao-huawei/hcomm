...
namespace hccl
{
    ...
    */
                                                             ...
                                                             bool isCustom)
    {
        ...
        if (opType == HcclCMDType::HCCL_CMD_ALLREDUCE &&
            ...
            for (u32 i = 0; i < itemNum; i++) {
                ...
                CHK_RET(TryAsyncUnfold(opParam)); // 对当前aicpu算子尝试异步展开
                isInplacePreSync_ = false;
            }
    ...
    HcclResult HcclCommunicator::TryAsyncUnfold(const OpParam &opParam)
    {
        // TODOSSY: 检查约束时, 校验itemNum和rankSize
        // TODOSSY: 异步展开下发前, 校验dynamicSize与分配空间的大小, 如果大于分配空间, 则跳过 (不进行异步展开)
        ...
        if (asyncUnfoldEnable_) {
            if (dynamicDataSize > ASYNC_UNFOLD_MAX_DYNAMIC_SIZE) {
                ...
                HcclResult ret = AicpuOpAsyncUnfoldInfoPut();
                if (ret != HCCL_SUCCESS) {
