...
HcclResult HcclCommunicator::BuildZeroCopyParam()
{
    ...
    CHK_RET(
        ZeroCopyMemoryAgent::GetRingBufferAddr(
...
HcclResult HcclCommunicator::InitZeroCopyMemoryAgent()
{
    ...
    zeroCopyMemoryAgent_.reset(
        static_cast<ZeroCopyMemoryAgent*>(new (std::nothrow) ZeroCopyMemoryAgent(
...
HcclResult HcclCommunicator::GetGroupRanksInfo(const std::vector<u32>& groupRanks, std::vector<RankInfo>& ranksInfo)
{
    ...
    for (u32 index = 0; index < ranksInfo.size(); ++index) {
        if (index != ranksInfo[index].userRank) {
            HCCL_ERROR(
                "[Get][GroupRanksInfo]errNo[0x%016llx] index[%u] !=  user rank[%u]", HCCL_ERROR_CODE(HCCL_E_PARA),
                index, ranksInfo[index].userRank);
            return HCCL_E_PARA;
        }
    }
    return HCCL_SUCCESS;
...
HcclResult HcclCommunicator::InitNicHostDeploy()
{
    ...
    CHK_PRT_RET(
        (hostIp_.IsInvalid()),
        HCCL_ERROR(
            "[Init][Nic] host ip is invalid when NIC "
            "deployment is host. "),
        HCCL_E_PARA);
    attrCollector_.GenUsedRdmaLevel0();
    isUsedRdmaLevel0_ = attrCollector_.GetUsedRdmaLevel0();
...
HcclResult HcclCommunicator::DeinitNicHostDeploy()
{
    ...
    CHK_PRT_RET(
        (hostIp_.IsInvalid()),
        HCCL_ERROR(
            "[DeInit][Nic] host ip is invalid when NIC "
            "deployment is host. "),
        HCCL_E_PARA);
...
HcclResult HcclCommunicator::ExecOpAlltoAll(HcclCMDType opType, OpParam& opParam, bool isCustom)
{
    ...
    auto isSupportAicpuAlg = [](const std::string& algName) {
        static const std::set<std::string> aicpuAlgs
            = {"RunAlltoAllVFullMesh", "RunAlltoAllDirectFullmesh", "RunAlltoAllVTwoLevelPipeline",
               "RunAlltoAllFullMeshSymmetricMemory", "RunAlltoAllVContinuousPipeline"};
...
HcclResult HcclCommunicator::RecordOpPara(HcclCMDType opType, const OpParam& opParam)
{
    ...
    switch (opType) {
        ...
        case HcclCMDType::HCCL_CMD_BROADCAST:
            CHK_RET(
                RankConsistentcyChecker::GetInstance().RecordOpPara(
                    opType, opParam.tag, opParam.DataDes.count, opParam.DataDes.dataType, opParam.reduceType,
                    opParam.root, cclBufferManager_.GetInCCLbufferSize(), cclBufferManager_.GetOutCCLbufferSize(),
        ...
        case HcclCMDType::HCCL_CMD_BATCH_SEND_RECV:
            CHK_RET(
                RankConsistentcyChecker::GetInstance().RecordOpPara(
                    opType, opParam.tag, cclBufferManager_.GetInCCLbufferSize(),
                    cclBufferManager_.GetOutCCLbufferSize(), identifier_.c_str(), ranktableCrc_));
            break;
        case HcclCMDType::HCCL_CMD_SEND:
            CHK_RET(
                RankConsistentcyChecker::GetInstance().RecordOpPara(
                    opType, opParam.tag, opParam.DataDes.count, opParam.DataDes.dataType, opParam.dstRank,
                    opParam.srTag, opParam.localGroupRank, cclBufferManager_.GetInCCLbufferSize(),
                    cclBufferManager_.GetOutCCLbufferSize(), identifier_.c_str(), ranktableCrc_));
            break;
        case HcclCMDType::HCCL_CMD_RECEIVE:
            CHK_RET(
                RankConsistentcyChecker::GetInstance().RecordOpPara(
                    opType, opParam.tag, opParam.DataDes.count, opParam.DataDes.dataType, opParam.srcRank,
                    opParam.srTag, opParam.localGroupRank, cclBufferManager_.GetInCCLbufferSize(),
                    cclBufferManager_.GetOutCCLbufferSize(), identifier_.c_str(), ranktableCrc_));
            break;
...
HcclResult HcclCommunicator::CaptureSlaveStreams(rtStream_t mainStream, vector<Stream>& slaveStreams)
{
    if ((deviceType_ != DevType::DEV_TYPE_910_93) && (deviceType_ != DevType::DEV_TYPE_310P3)) {
        HCCL_INFO(
            "[HcclCommunicator][%s]Only 310P3 or A3 device in host expand mode need to capture slave streams.",
            __func__);
        return HCCL_SUCCESS;
    }
    aclmdlRI rtModel = nullptr;
...
HcclResult HcclCommunicator::GetReportHcclMC2Info(const Stream& kfcStream, const std::vector<Stream>& aicpuStreams)
{
    ...
    for (uint32_t streamIndex = 0, reportId = 0; streamIndex < aicpuStreams.size(); streamIndex++) {
        ...
        if (reportId == ONCE_REPORT_STREAM_NUM_MAX) {
            ...
            CHK_RET(
                ProfilingManagerPub::CallMsprofReportMc2CommInfo(
    ...
    const rtStream_t& aicpuStream, const HcclCMDType opType, bool isCustom)
{
    ...
    if (IsEnableCustom()) {
        ...
        CHK_RET(
            ProfilingManagerPub::CallMsprofReportNodeInfo(
...
HcclResult HcclCommunicator::InitAndCheckAicpuOrderNotify(u8& orderLaunchMode)
{
    if (orderLaunchMode == AICPU_ORDERLAUNCH_INVALID_HCOM_MODE) {
        HCCL_INFO(
            "[HcclCommunicator][InitAndCheckAicpuOrderNotify] orderLaunchMode is invalid in hcom "
...
HcclResult HcclCommunicator::CreateDeviceCommContext(u64 size, DeviceMem& buffer) const
{
    CHK_PRT_RET(
        !size,
        HCCL_INFO(
            "[Create][DeviceCommContext]device commContext size is zero. "
...
HcclResult
HcclCommunicator::GetAllReduceScratchSize(const u32 count, const HcclDataType dataType, u64& scratchSize) const
{
    CHK_SMART_PTR_NULL(implAlg_);
...
HcclResult HcclCommunicator::RegisterToSnapshot()
{
    ...
    CHK_RET(
        SnapshotControl::GetInstance(deviceLogicId_)
