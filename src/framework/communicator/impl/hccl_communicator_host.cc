...
HcclResult HcclCommunicator::InitOneSidedServiceNetDevCtx(u32 remoteRankId)
{
    if (nicDeployment_ != NICDeployment::NIC_DEPLOYMENT_DEVICE) {
        ...
        HCCL_ERROR("[%s]nicDeployment_[%d], userRankSize_[%u], do not support oneSidedService.", __func__,
            nicDeployment_, userRankSize_);
        return HCCL_E_INTERNAL;
    }
...
bool HcclCommunicator::IsSupportSymmetricMemory(HcclCMDType opType, OpParam &opParam)
{
    ...
    CHK_PRT_RET(opType != HcclCMDType::HCCL_CMD_ALLGATHER && opType != HcclCMDType::HCCL_CMD_ALLREDUCE
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS || opParam.outputSymWindow == nullptr,
        HCCL_INFO("[%s] output[%p] size[%llu] is not support symmetric memory", __func__, opParam.outputPtr,
            opParam.outputSize),
        false);
...
bool HcclCommunicator::IsSupportZeroCopy(const OpParam &opParam)
{
    ...
    CHK_PRT_RET(deviceNumPerAggregation_ == 1 || multiModuleDiffDeviceNumMode_,
        HCCL_INFO("[%s] deviceNumPerAggregation[%u], multiModuleDiffDeviceNumMode_[%d] not support zero copy", __func__,
...
HcclResult HcclCommunicator::UpdateZeroCopy(const OpParam &opParam, const AlgResourceResponse &algResource)
{
    ...
    for (auto &singleSubCommTransport : algResource.opTransportResponse[COMM_LEVEL0]) {
        for (u64 i = 0; i < singleSubCommTransport.links.size(); ++i) {
            ...
            CHK_PRT_RET(zeroCopyIpcPtrs_[remoteRank % deviceNumPerAggregation_] != nullptr
                            && zeroCopyIpcPtrs_[remoteRank % deviceNumPerAggregation_] != remotePtr,
                HCCL_ERROR("[BuildZeroCopyParam] zeroCopyIpcPtrs_[%u] is [%p] not equal to %p", remoteRank,
                    zeroCopyIpcPtrs_[remoteRank % deviceNumPerAggregation_], remotePtr),
                HCCL_E_PARA);
    ...
    if (serverNum_ == 1 && moduleNum_ == HCCL_MODULE_NUM_TWO && GetExternalInputIntraRoceSwitch() == 0
        ...
        for (size_t i = 0; i < devIdList0.size(); i++) {
            if (devIdList0[i] % DEVICE_PER_MODULE != devIdList1[i] % DEVICE_PER_MODULE) {
                ...
                RPT_INPUT_ERR(
                    true, "EI0010", std::vector<std::string>({"reason"}), std::vector<std::string>({errormessage}));
                HCCL_ERROR("[%s][%s]%s", LOG_KEYWORDS_INIT_CHANNEL.c_str(), LOG_KEYWORDS_TIMEOUT.c_str(),
                    errormessage.c_str());
                return HCCL_E_NOT_SUPPORT;
            }
        }
    ...
    if (needReduce) {
        if (Is310P3Common(isHaveCpuRank_, deviceType_)) {
            if ((dataType == HCCL_DATA_TYPE_INT64) || (dataType == HCCL_DATA_TYPE_BFP16)) {
                RPT_INPUT_ERR(true, "EI0003", infoTitle,
                    vector<string>({"CheckDataType", GetDataTypeEnumStr(dataType), "dataType",
...
HcclResult HcclCommunicator::InitZeroCopyMemoryAgent()
{
    ...
    zeroCopyMemoryAgent_.reset(
        static_cast<ZeroCopyMemoryAgent *>(new (std::nothrow) ZeroCopyMemoryAgent(socketManager_, devicePhyId_,
    ...
    if ((deviceType_ == DevType::DEV_TYPE_910B) || (deviceType_ == DevType::DEV_TYPE_910_93)) {
        if ((op == HCCL_REDUCE_PROD) && ((dataType == HCCL_DATA_TYPE_INT16) || (dataType == HCCL_DATA_TYPE_BFP16))) {
            RPT_INPUT_ERR(true, "EI0003", std::vector<std::string>({"ccl_op", "value", "parameter", "expect"}),
                std::vector<std::string>({"CheckReduceDataType", GetDataTypeEnumStr(dataType), "dataType",
...
HcclResult HcclCommunicator::InitPara()
{
    ...
    CHK_SMART_PTR_NULL(implAlg_);
    CHK_RET(implAlg_->Init(static_cast<const void *>(&transportResInfo_), sizeof(transportResInfo_), workSpaceRes_,
        notifyPool_, netDevCtxMap_, queueNotifyManager_, algoAttr, topoAttr, false));
    return HCCL_SUCCESS;
}
...
HcclResult HcclCommunicator::InitPreResource(const RankTable_t &rankTable)
{
    ...
    if (deviceType_ == DevType::DEV_TYPE_910_93) {
        ...
        CHK_PRT_RET(ret != RT_ERROR_NONE,
            HCCL_ERROR("[Init][PreResource]rtGetServerIDBySDID failed sdid[0x%08x], serverID[%u], ret[%u]",
    ...
    HCCL_INFO("[Init][PreResource]Current deviceType[%d], isStandardCard[%s]", deviceType_,
...
HcclResult HcclCommunicator::CreateMyRank(HcclCommParams &params, const RankTable_t &rankTable, HcclTopoAttr &topoAttr)
{
    ...
    CHK_PRT_RET(
        myRank_ != nullptr, HCCL_ERROR("[HcclCommunicator][CreateMyRank]myRank already initialized"), HCCL_E_INTERNAL);
...
HcclResult HcclCommunicator::InitMyRank()
{
    ...
    HcclMem cclBuffer
        = {HCCL_MEM_TYPE_DEVICE, cclBufferManager_.GetInCCLbuffer().ptr(), cclBufferManager_.GetInCCLbuffer().size()};
...
HcclResult HcclCommunicator::InitRaResource()
{
    ...
    HCCL_INFO("isUsedRdmaLevel0_[%u] nicNum[%u] hostIP[%s], nicDeployment[%d].", isUsedRdmaLevel0_, devIpAddr_.size(),
    ...
    const std::string &algName, u64 &streamNum, u64 dataSize, bool ifAiv, HcclCMDType opType)
{
    ...
    HCCL_INFO("[GetWorkspaceSubStreamNum] ifAiv[%d], workflowMode[%d], ifHcomWithAiv[%d]", ifAiv, GetWorkflowMode(),
        ifHcomWithAiv);
    if (ifHcomWithAiv && (deviceType_ == DevType::DEV_TYPE_910_93 || deviceType_ == DevType::DEV_TYPE_910B)) {
        ...
        HCCL_INFO("[GetWorkspaceSubStreamNum] Hcom AIV enabled on DeviceType[%d], the streamNum is [%llu]", deviceType_,
            streamNum);
        return HCCL_SUCCESS;
    }
    ...
    } else if (FullmeshPairwiseSatisfyHighPerfAlltoallMeshCondition(deviceType_, meshAggregationRankSize_,
                   useSuperPodMode_, commConfig_.GetConfigHcclAlgo(HcclCMDType::HCCL_CMD_ALLTOALL))) {
...
{
    HCCL_INFO("[HcclCommunicator][HcclSelectAlg] start to run with opType[%d], count[%llu], dataType[%d], "
              "reduceOp[%d], aivCoreLimit[%d]",
...
HcclResult HcclCommunicator::HcclGetAlgExecParam(const std::string &tag, HcclCMDType opType, u64 count, void *inputPtr,
    void *outputPtr, bool clearEnable, HcclDataType dataType, HcclReduceOp op, void *&commContext, u64 &len,
    u32 aivCoreLimit)
{
    ...
    HCCL_INFO("SPK, Tag %llu  aivCoreLimit %u, numBlocks %llu.", aivSuperKernelArgs.tag, aivCoreLimit,
    ...
    HcclResult hcclRet = hrtMemSyncCopy(sendAlgParamMemPtr, sizeof(AivSuperKernelArgs), &aivSuperKernelArgs,
        sizeof(AivSuperKernelArgs), HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_HOST_TO_DEVICE);
...
HcclResult HcclCommunicator::GetGroupRanksInfo(const std::vector<u32> &groupRanks, std::vector<RankInfo> &ranksInfo)
{
    ...
    for (u32 index = 0; index < groupRanks.size(); index++) {
        ...
        HCCL_DEBUG("index: %d userRank: %dhost ip: %s host port: %u dev phy id: %d serverIdx:%d", index,
    ...
    std::vector<HcclIpAddress> &localIpList, bool &isOneSidedTaskAndBackupInitA3)
{
    ...
    if (deviceType != DevType::DEV_TYPE_910_93) {
        isOneSidedTaskAndBackupInitA3 = false;
        HCCL_INFO("[HcclCommunicator::CheckOneSidedBackupAndSetDevId] DeviceType[%d] is not 910_93, one sided backup "
                  "not support",
    ...
    std::copy_if(chipDeviceIPs[ipIdex].begin(), chipDeviceIPs[ipIdex].end(), std::back_inserter(backupIpList),
        [](const HcclIpAddress &ip) {
            return !ip.IsIPv6();
        });
    HCCL_INFO("devicePhysicID[%u], backupDeviceId[%d], backupDeviceIP[0]:[%s], devIpAddr_[%s], ", devicePhyId_,
    ...
    u32 &backupDevLogicId, std::vector<HcclIpAddress> &localIpList)
{
    ...
    HCCL_INFO("[HcclCommunicator::OneSidedBackupInitNetResource] OpenDev with backupDevPhyId[%d], "
              "backupDevLogicId[%d], localIpList[%s], backupIp[%s]",
...
HcclResult HcclCommunicator::RegisterToHeartBeat()
{
    ...
    return Heartbeat::GetInstance(deviceLogicId_)
        .RegisterToHeartBeat(userRank_, deviceType_, rankInfoList_, localPort, isNeedInitNic_, identifier_,
...
HcclResult HcclCommunicator::RegisterToHeartBeat(u32 peerRankId, string &tag)
{
    u32 localPort = commPortConfig_.devPortSwitchOn ? HCCL_INVALID_PORT : GetLocalNicPort(NicType::DEVICE_NIC_TYPE);
    return Heartbeat::GetInstance(deviceLogicId_)
        .RegisterToHeartBeat(userRank_, deviceType_, rankInfoList_, localPort, isNeedInitNic_, peerRankId, identifier_,
            tag, useSuperPodMode_, isUsedRdmaLevel0_, retryEnable_, IsEnableBackupLink());
}
    ...
    if (UNLIKELY(GetDebugConfig() & HCCL_ALG)) {
        for (u32 i = 0; i < userRankSize_; i++) {
            HCCL_CONFIG_DEBUG(HCCL_ALG,
                "[HcclCommunicator][AllGatherV]userRank_[%u], rankIdx[%u], recvCounts[%llu], rdispls[%llu]", userRank_,
                i, counts[i], static_cast<const u64 *>(rdispls)[i]);
        }
    }
    ...
    HcclDataType dataType, HcclReduceOp op, HcclRtStream stream, HcclCMDType cmdType)
{
    ...
    if (!IsExistCommRes(identifier_)) {
        HCCL_INFO("[AicpuUnfold] tag[%s] count[%llu] dataType[%s] op[%s].", identifier_.c_str(), count,
...
HcclResult HcclCommunicator::AllGatherVOutPlace(const std::string &tag, void *inputPtr, void *outputPtr, u64 inputCount,
...
HcclResult HcclCommunicator::AllReduce(const std::string &tag, void *inputPtr, void *outputPtr, u64 count,
    HcclDataType dataType, HcclReduceOp op, HcclRtStream stream, SyncMode syncMode, const HcomCollOpInfo *opInfo)
{
    CHK_RET(CheckSuspendingStatus());
    bool aicpuUnfoldMode = false;
...
HcclResult HcclCommunicator::AlltoAll(const void *sendBuf, u64 sendCount, HcclDataType sendType, const void *recvBuf,
    ...
    HcclDataType dataType, u32 root, HcclRtStream stream)
{
    CHK_RET(CheckSuspendingStatus());
    if (Is310P3Common(isHaveCpuRank_, deviceType_)) {
        HCCL_ERROR("[%s][%s]ScatterOutPlace Not Supported Yet", LOG_KEYWORDS_TASK_EXEC.c_str(),
...
HcclResult HcclCommunicator::ReduceScatterOutPlace(const std::string &tag, void *inputPtr, void *outputPtr, u64 count,
    HcclDataType dataType, HcclReduceOp op, HcclRtStream stream)
{
    CHK_RET(CheckSuspendingStatus());
    if (userRankSize_ > 1) {
...
HcclResult HcclCommunicator::ReduceScatterV(const std::string &tag, void *inputPtr, const void *inputCounts,
    const void *inputDispls, void *outputPtr, u64 outputCount, HcclDataType dataType, HcclReduceOp op,
...
HcclResult HcclCommunicator::ReduceScatterVOutPlace(const std::string &tag, void *inputPtr, void *outputPtr,
    const void *inputCounts, const void *inputDispls, u64 outputCount, HcclDataType dataType, HcclReduceOp op,
    HcclRtStream stream)
{
    CHK_RET(CheckSuspendingStatus());
    if (userRankSize_ == 1) {
    ...
    const std::string &tag, void *inputPtr, u64 count, HcclDataType dataType, u32 destRank, rtStream_t stream)
{
    ...
    if (Is310P3Common(isHaveCpuRank_, deviceType_)) {
        HCCL_ERROR("[%s][%s]SendOutPlace is not supported", LOG_KEYWORDS_TASK_EXEC.c_str(),
            LOG_KEYWORDS_NOT_SUPPORTED.c_str());
        return HCCL_E_NOT_SUPPORT;
    }
    if (!IsAtomicInit()) {
    ...
    const std::string &tag, void *outputPtr, u64 count, HcclDataType dataType, u32 srcRank, rtStream_t stream)
{
    ...
    if (Is310P3Common(isHaveCpuRank_, deviceType_)) {
        HCCL_ERROR("[%s][%s]ReceiveOutPlace is not supported", LOG_KEYWORDS_TASK_EXEC.c_str(),
            LOG_KEYWORDS_NOT_SUPPORTED.c_str());
        return HCCL_E_NOT_SUPPORT;
    }
    if (!IsAtomicInit()) {
...
HcclResult HcclCommunicator::ExecOpCache(HcclCMDType opType, OpParam &opParam, HcclCacheInfo &cacheInfo)
{
    ...
    if (opType == HcclCMDType::HCCL_CMD_REDUCE_SCATTER_V || opType == HcclCMDType::HCCL_CMD_ALLGATHER_V
        ...
        ret = ExecuteKernelLaunch(cacheInfo.opArgs, cacheInfo.topoArgs, cacheInfo.resourceArgs, cacheInfo.algArgs,
            cacheInfo.extraArgs, cacheInfo.profilingInfo);
    } else {
    ...
    std::vector<HcclSendRecvItem> &hostSendRecvInfo, std::vector<HcclSendRecvItem> &aicpuSendRecvInfo)
{
    ...
    HCCL_INFO("[HcclCommunicator][SplitBsrData] itemNum %u hostItemNum %zu aicpuItemNum %zu", itemNum,
...
HcclResult HcclCommunicator::ExecOp(HcclCMDType opType, OpParam &opParam, bool isCustom)
{
    ...
    if (retryEnable_ && needWarnAboutReduceProdInt64_ && IsReduceWithInt64OrProd(opType, opParam)) {
        HCCL_RUN_WARNING("[HcclCommunicator][%s]comm[%s], opType[%d], reduceType[%d]. Reduce operators with prod "
                         "operation or int64 data type. This operator type unsupportd for AICPU mode, retry disabled",
    ...
    if (selectAivAlg) {
        ...
        HCCL_INFO("[HcclCommunicator][ExecOp] tag[%s] userRank[%u] cur aiv tag [%d]", identifier_.c_str(), userRank_,
            opParam.aivTag);
        opParam.aicpuUnfoldMode = false;
        opParam.aicpuCacheEnable = 0;
...
HcclResult HcclCommunicator::HandleExistAlgResource(const std::string &newTag, const std::string &algName,
    HcclCMDType opType, const OpParam &opParam, std::unique_ptr<CollAlgOperator> &algOperator, bool selectAivAlg,
    bool aicpuUnfoldModeFor910B, bool needRecreateAlltoallComm)
{
...
HcclResult HcclCommunicator::ExecOpAlltoAll(HcclCMDType opType, OpParam &opParam, bool isCustom)
{
    ...
    if (resMap_.find(newTag) == resMap_.end()) {
        ...
        CHK_RET(HandleExistAlgResource(newTag, algName, opType, opParam, algOperator, selectAivAlg,
            aicpuUnfoldModeFor910B, needRecreateAlltoallComm));
    }
    ...
    if (hcclNslbDp::GetInstance().GetGlobalCommTaskId() != 0 && hcclNslbDp::GetInstance().GetInitNetCoFlag() == true) {
        ...
        HCCL_INFO("[NSLBDP-WEN]-nslbAdjInfosize[%u]-algName[%s]-rankSize[%u]-commDesc[%s]..", nslbAdjInfo.dstRankNum,
    ...
    if (selectAivAlg) {
        ...
        GetAivTag(algDesc.aivTagNum, opParam.isCapture, opParam.aivTag);
        HCCL_INFO("[HcclCommunicator][ExecOpAlltoAll] tag[%s] userRank[%u] cur aiv tag [%d].", identifier_.c_str(),
            userRank_, opParam.aivTag);
        opParam.aicpuUnfoldMode = false;
        opParam.aicpuCacheEnable = 0;
        CHK_RET(algOperator->SetNumBlocks(aivCoreLimit));
    }
    ...
    auto isSupportAicpuAlg = [](const std::string &algName) {
        static const std::set<std::string> aicpuAlgs = {"RunAlltoAllVFullMesh", "RunAlltoAllDirectFullmesh",
            "RunAlltoAllVTwoLevelPipeline", "RunAlltoAllFullMeshSymmetricMemory", "RunAlltoAllVContinuousPipeline"};
...
HcclResult HcclCommunicator::RecordOpPara(HcclCMDType opType, const OpParam &opParam)
{
    ...
    switch (opType) {
        ...
        case HcclCMDType::HCCL_CMD_BROADCAST:
            CHK_RET(RankConsistentcyChecker::GetInstance().RecordOpPara(opType, opParam.tag, opParam.DataDes.count,
                opParam.DataDes.dataType, opParam.reduceType, opParam.root, cclBufferManager_.GetInCCLbufferSize(),
        ...
        case HcclCMDType::HCCL_CMD_ALLGATHER_V:
            CHK_RET(RankConsistentcyChecker::GetInstance().RecordOpPara(opType, opParam.tag, opParam.VDataDes.counts,
                opParam.VDataDes.displs, userRankSize_, opParam.VDataDes.dataType, opParam.reduceType,
        ...
        case HcclCMDType::HCCL_CMD_BATCH_SEND_RECV:
            CHK_RET(RankConsistentcyChecker::GetInstance().RecordOpPara(opType, opParam.tag,
                cclBufferManager_.GetInCCLbufferSize(), cclBufferManager_.GetOutCCLbufferSize(), identifier_.c_str(),
                ranktableCrc_));
            break;
        case HcclCMDType::HCCL_CMD_SEND:
            CHK_RET(RankConsistentcyChecker::GetInstance().RecordOpPara(opType, opParam.tag, opParam.DataDes.count,
                opParam.DataDes.dataType, opParam.dstRank, opParam.srTag, opParam.localGroupRank,
                cclBufferManager_.GetInCCLbufferSize(), cclBufferManager_.GetOutCCLbufferSize(), identifier_.c_str(),
                ranktableCrc_));
            break;
        case HcclCMDType::HCCL_CMD_RECEIVE:
            CHK_RET(RankConsistentcyChecker::GetInstance().RecordOpPara(opType, opParam.tag, opParam.DataDes.count,
                opParam.DataDes.dataType, opParam.srcRank, opParam.srTag, opParam.localGroupRank,
                cclBufferManager_.GetInCCLbufferSize(), cclBufferManager_.GetOutCCLbufferSize(), identifier_.c_str(),
                ranktableCrc_));
            break;
        case HcclCMDType::HCCL_CMD_ALLTOALL:
            CHK_RET(RankConsistentcyChecker::GetInstance().RecordOpPara(opType, opParam.tag,
                opParam.All2AllDataDes.sendCount, opParam.All2AllDataDes.sendType, opParam.reduceType, opParam.root,
                cclBufferManager_.GetInCCLbufferSize(), cclBufferManager_.GetOutCCLbufferSize(), identifier_.c_str(),
                ranktableCrc_, aivCoreLimit));
            break;
        case HcclCMDType::HCCL_CMD_ALLTOALLV:
        case HcclCMDType::HCCL_CMD_ALLTOALLVC:
            CHK_RET(RankConsistentcyChecker::GetInstance().RecordOpPara(opType, opParam.tag, 0, HCCL_DATA_TYPE_RESERVED,
...
HcclResult HcclCommunicator::CaptureSlaveStreams(rtStream_t mainStream, vector<Stream> &slaveStreams)
{
    if ((deviceType_ != DevType::DEV_TYPE_910_93) && (deviceType_ != DevType::DEV_TYPE_310P3)) {
        HCCL_INFO("[HcclCommunicator][%s]Only 310P3 or A3 device in host expand mode need to capture slave streams.",
            __func__);
        return HCCL_SUCCESS;
    }
    aclmdlRI rtModel = nullptr;
    ...
    const AlgResourceResponse &algResource, const std::string &newTag, LocalResInfoV2 *localResHostPtr)
{
    if (algResource.scratchMem.size() > 0) {
        ...
        // 3、将节点插入链表头
        ListCommonAddHead(&tagLocalResDevicePtr->nextTagRes, &tagLocalResHostPtr->nextTagRes,
            &localResHostPtr->nextTagRes, &opResDeviceParaPtr_->localRes.nextTagRes);
...
HcclResult HcclCommunicator::BuildOpLocalResParam(const AlgResourceResponse &algResource, const std::string &newTag)
{
    ...
    CHK_RET(AllocAndGetStreamContextBuff(opMainStream_.id(), localResHostPtr->mainStreamParam.sqCqContextAddr,
    ...
    CHK_RET(AllocAndGetStreamContextBuff(opResPara_.aicpuOrderStreamParam.streamInfo.streamIds,
        opResPara_.aicpuOrderStreamParam.sqCqContextAddr, opResPara_.aicpuOrderStreamParam.sqCqContextSize));
...
u32 HcclCommunicator::UpdateOpIndex(const OpParam &opParam)
{
    ...
    HCCL_DEBUG("%s tag:%s opType:%u commIndex:%u opIndex:%u", __func__, opParam.tag.c_str(), opParam.opType, commIndex,
        opIndex);
    return opIndex;
}
...
HcclResult HcclCommunicator::BuildAicpuCustomParam()
{
    ...
    HCCL_INFO("%s success, aicpuCustomParamAddr:0x%llx, aicpuCustomParamSize:%llu", __func__,
        opResPara_.aicpuCustomParamAddr, opResPara_.aicpuCustomParamSize);
    return HCCL_SUCCESS;
}
...
HcclResult HcclCommunicator::BuildAicpuOrderLaunchNotify()
{
    ...
    HCCL_INFO("%s success, aicpuOrderNotifyAddr:0x%llx, aicpuOrderNotifySize:%llu", __func__,
        opResPara_.aicpuOrderNotifyAddr, opResPara_.aicpuOrderNotifySize);
    return HCCL_SUCCESS;
}
    ...
    const std::string &newTag, const std::string &algName, const HcclCMDType opType)
{
    ...
    CHK_PRT_RET(commTransport.size() <= 0,
        HCCL_ERROR("[%s] no LevelComm resource, please create comm first. "
    ...
    CHK_PRT_RET(links.size() <= 0,
        HCCL_ERROR("[%s] no transport resource, please create links first. "
                   "tag[%s], curRankId[%u] rankNum[%u]",
            __func__, newTag.c_str(), aiRMAInfoPtr->curRankId, aiRMAInfoPtr->rankNum),
        HCCL_E_INTERNAL);

    LevelNSubCommTransport &tmpCommTransport = resMap_[newTag].opTransportResponse[COMM_MESH_L1];
    CHK_PRT_RET(tmpCommTransport.size() <= 0,
        HCCL_ERROR("[%s] no LevelComm resource, please create comm first. "
                   "tag[%s], curRankId[%u] rankNum[%u]",
            __func__, newTag.c_str(), aiRMAInfoPtr->curRankId, aiRMAInfoPtr->rankNum),
        HCCL_E_INTERNAL);
    std::vector<LINK> &tmpLinks = tmpCommTransport[0].links;
    CHK_PRT_RET(tmpLinks.size() <= 0,
        HCCL_ERROR("[%s] no transport resource, please create links first. "
                   "tag[%s], curRankId[%u] rankNum[%u]",
            __func__, newTag.c_str(), aiRMAInfoPtr->curRankId, aiRMAInfoPtr->rankNum),
        HCCL_E_INTERNAL);
    ...
    const std::vector<std::vector<std::vector<u32>>> &inputVectorInfo, DeviceMem &dstTlvDeviceMem, u64 &tlvLen)
{
    vector<u32> tlv;
    CommonTlv commonTlv;
    HCCL_DEBUG("[HcclCommunicator][BuildOpTopoResTlvParam] input vector size[%lu], group[%s].", inputVectorInfo.size(),
        identifier_.c_str());
...
HcclResult HcclCommunicator::BuildPairLinkCounter(const std::string &algName)
{
    ...
    if (pairLinkCounterDevice_.ptr() == nullptr) {
        ...
        HCCL_DEBUG("[HcclCommunicator][BuildPairLinkCounter] pairLinkCounter size[%lu], group[%s]", pairLinkCounterSize,
            identifier_.c_str());
...
HcclResult HcclCommunicator::BuildIsUsedRdmaRank(const std::string &algName)
{
    ...
    if (isUsedRdmaRankPairDevice_.ptr() == nullptr) {
        ...
        HCCL_DEBUG("[HcclCommunicator][BuildIsUsedRdmaRank] is used Rdma rank size[%lu], group[%s]", isUsedRdmaMapSize,
            identifier_.c_str());
...
HcclResult HcclCommunicator::BuildBridgeRank(const std::string &algName)
{
    if (bridgeRankDevice_.ptr() == nullptr) {
        ...
        HCCL_DEBUG("[HcclCommunicator][BuildBridgeRank] Bridge size[%lu], group[%s]", isBridgeVector.size(),
            identifier_.c_str());
...
HcclResult HcclCommunicator::BuildHierarchicalAlgOption(u32 *ahcConfInfo)
{
    ...
    HCCL_DEBUG("[HcclCommunicator][BuildHierarchicalAlgOption] host hierarchicalAlgOption.size() [%u]",
    ...
    for (auto it = hierarchicalAlgOption.begin(); it != hierarchicalAlgOption.end(); ++it) {
        HCCL_DEBUG("[HcclCommunicator][BuildHierarchicalAlgOption] host Level [%u], ConcType[%u] AHCOpType[%u], "
                   "TemplateType [%u]",
    ...
    const LINK &link, HccltagRemoteResV3 &tagRemoteRes, TransportLinkType linkType)
{
    ...
    if (linkp2p->localIpcSignal[0].resId != INVALID_U64) {
        HCCL_INFO("[%s]the linkP2p is existed, no need to refresh transport resource, resId[%llu]", __func__,
    ...
    const LINK &link, HccltagRemoteResV3 &tagRemoteRes, bool isBackup, bool isRetry, bool isSecondBuild)
{
    ...
    HCCL_DEBUG("[%s] finish set localnotify & remotenotify info, notifyNum[%llu], linkNotifyNum[%llu]", __func__,
        notifyNum, signalInfos.size());
...
template <typename T> HcclResult HcclCommunicator::CreateListNode(T **resHostPtr, T **resDevicePtr)
{
    hostMemVec_.resize(hostMemVec_.size() + 1);
    ...
    const std::string &newTag, const ListCommon *headHostList, const u64 size)
{
    ...
    while (nextHostList != headHostList) {
        HCCL_INFO("[HcclCommunicator][CopyHostListResToDeviceParam] remote resource, tag[%s], head Host List[%p], next "
...
HcclResult HcclCommunicator::BuildOpResParam(const std::string &algName, const AlgResourceResponse &algResource,
    const std::string &newTag, const HcclCMDType opType, const rtStream_t aicpuStream)
{
    ...
    HCCL_RUN_INFO("[%s]build aicpu unfold resource success, tag[%s] rWinStart[%u] rWinOffset[%u] opEntry[%d]", __func__,
    ...
    bool isCustom, bool needIncreLink, bool needRecreateAlltoallComm)
{
    ...
    if (!isContextLaunched_) {
        ...
        CHK_PRT_RET(guard.IsLockFailed(), HCCL_ERROR("[HcclCommunicator][OrchestrateAicp] hostDeviceLock lock failed"),
            HCCL_E_INTERNAL);
        CHK_RET(AicpuResourceRefresh(algResource, newTag, opType));
    }
    HCCL_DEBUG("%s isContextLaunched[%u], needRecreateAlltoallComm[%u]", __func__, isContextLaunched_,
        needRecreateAlltoallComm);
        ...
        for (u32 ringIndex = 0; ringIndex < opTransportReq[levelIndex].size(); ringIndex++) {
            ...
            for (u32 rankIndex = 0; rankIndex < reqSingleSubComm.transportRequests.size(); rankIndex++) {
                ...
                CHK_PRT_RET(rankIndex >= respSingleSubComm.links.size(),
                    HCCL_ERROR("[CleanTransportLinks] The remote rank_id[%u] is larger than the existent "
                               "respSingleSubComm map "
                               "size[%u]",
                ...
                if (respSingleSubComm.links[rankIndex] != nullptr
                    ...
                    HCCL_INFO("[CleanTransportLinks] The link to remote userRank[%u] has existed",
                        transportRequest.remoteUserRank);
                    continue;
                }
...
HcclResult HcclCommunicator::AllocAlgNotifys(const std::string &tag, const NotifyLoadType notifyLoadType,
    ...
    AlgResourceRequest &resRequest, AlgResourceResponse &algResResponse, bool selectAivAlg)
{
    ...
    TransportIOMem transMem{algResResponse.cclInputMem, algResResponse.cclOutputMem, algResResponse.paramInputMem,
        algResResponse.paramOutputMem, algResResponse.scratchMem, algResResponse.aivInputMem,
        algResResponse.aivOutputMem, expMem, DeviceMem()};
    HCCL_DEBUG(
    ...
    if (!GetExternalInputHcclEnableFfts() && GetWorkflowMode() == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE) {
        ...
        for (u32 ringIndex = 0; ringIndex < slaveNum; ringIndex++) {
            algResResponse.threadManage[ringIndex].reset(
                new (std::nothrow) ThreadManage(deviceLogicId_, userRank_, dispatcher_));
    ...
    if (isUserMemRegisted_) {
        ...
        ret = transportManager_->Alloc(opParam.tag, transMem, algResResponse.opTransportResponse,
            opParam.aicpuUnfoldMode, false, opParam.isZeroCopy, opParam.opType, opParam.isCapture, false,
            opParam.isNpuDirectRoce, &opParam);
        CHK_PRT_RET(
        ...
        for (auto &singleSubCommTransport : levelNSubCommTransport) {
            ...
            for (auto &transportRequest : singleSubCommTransport.transportRequests) {
                ...
                CHK_PRT_RET((remoteId >= MAX_RANK_NUM_A3),
                    HCCL_ERROR("[%s]Invalid remoteId, valid range is [0, %u), remoteId[%u]", __func__, MAX_RANK_NUM_A3,
                        remoteId),
                    HCCL_E_PARA);
    ...
    AlgResourceRequest &resRequest, AlgResourceResponse &algResResponse)
{
        ...
        algResResponse.aivOutputMem, expMem, DeviceMem()};
    {
        ...
        CHK_RET(transportManager_->IncreAlloc(opParam.tag, transMem, resRequest.opTransport,
            algResResponse.opTransportResponse, opParam.aicpuUnfoldMode, false, opParam.isCapture, opParam.opType));
    }
    if (retryEnable_) {
        // 获取当前rdma相连的所有对端rankList
        std::vector<u32> rankList;
        CHK_RET(transportManager_->GetIncreRemoteRankList(
            resRequest.opTransport, algResResponse.opTransportResponse, rankList, TransportType::TRANS_TYPE_IBV_EXP));
        std::string rankListStr = "";
        for (auto remoteRank : rankList) {
            rankListStr += (std::to_string(remoteRank) + ";");
        }
    ...
    if (!GetExternalInputHcclIsTcpMode() && (Is310PDevice() || isHostUseDevNic_)) {
        ...
        if (!isHostUseDevNic_) {
            ...
            CHK_RET(mrManager_->GetKey(
                memBlocksManager_->GetMemAddr(), memBlocksManager_->GetMemSize(), transportResInfo_.lkey));
        }
...
HcclResult HcclCommunicator::CreateWorkSpace(u64 size, DeviceMem &buffer) const
{
    ...
    CHK_PRT_RET((size > ULONG_MAX), HCCL_ERROR("[Create][WorkSpace]work space size is greater than %llu", ULONG_MAX),
        HCCL_E_PARA);
    ...
    const HcclCMDType commType, OpParam &opParam, const uint64_t count, void *pCount, void *pDispls)
{
    ...
    } else if (commType == HcclCMDType::HCCL_CMD_ALLTOALLV || commType == HcclCMDType::HCCL_CMD_ALLTOALL
...
HcclResult HcclCommunicator::AllocComResource(const string &newTag, const string &algName, const HcclCMDType commType,
    const OpParam &opParam, rtStream_t stream, bool isNeedHostSlaveStream)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("%s GetWorkSpace fail, size[%llu] space[%llu]", __func__, opResPara_.mc2WorkSpace.workSpaceSize,
    ...
    const std::string &tag, rtStream_t aiCpuStream, bool isOpbaseMode, void **commContext, const std::string &algConfig)
{
    ...
    HCCL_INFO("[HcclCommunicator][CommResource]newTag[%s] aicpu stream[%p] isOpbaseMode[%u]", newTag.c_str(),
        aiCpuStream, isOpbaseMode);
    ...
    rtStream_t aiCpuStream, bool isOpbaseMode, void **commContext, const string &tag)
{
    ...
    CHK_PRT_RET(result != HCCL_SUCCESS,
        HCCL_ERROR("[HcclCommunicator][CommResource]errNo[0x%016llx] size[%llu] space[%llu]", HCCL_ERROR_CODE(result),
    ...
    if (isA2MC2MultiServer_) {
        ...
        CHK_RET(hrtMemAsyncCopy(ibverbsDataBuffer_.ptr(), ibverbsDataBuffer_.size(), transDevIbverbsDataMem_->ptr(),
            ibverbsDataSize, HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_HOST_TO_DEVICE, aiCpuStream));
        ...
        CHK_RET(hrtMemAsyncCopy(combinedCapabilityBuffer_.ptr(), combinedCapabilityBuffer_.size(),
            combinedCapabilityMem_->ptr(), capabilitySize, HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_HOST_TO_DEVICE,
            aiCpuStream));
    ...
    const std::shared_ptr<LocalNotify> &localNotify, HcclSignalInfo &notifyInfo)
{
    ...
    HCCL_INFO("[HcclCommunicator][GetAiCpuNotifyData]resId[%lld], addr[%lld], devId[%u], tsId[%u].", notifyInfo.resId,
...
HcclResult HcclCommunicator::Mc2AiCpuStreamAllocAndGet(u32 streamMode, rtStream_t &aiCpuStream)
{
    ...
    aiCpuStream = opStream_.ptr();
    HCCL_RUN_INFO("%s alloc success, group:%s, stream id:%u, mainStreamMode:%u, aicpuStreamMode:%u", __func__,
...
HcclResult HcclCommunicator::AicpuResourceInit(const std::string &algName, const AlgResourceResponse &algResource,
    const std::string &newTag, const rtStream_t &aicpuStream, const HcclCMDType opType, bool isCustom)
{
    HCCL_RUN_INFO("[%s] start to init group[%s] aicpu resources newTag[%s] local rankId[%u]", __func__,
    ...
    if (IsEnableCustom()) {
        ...
        CHK_RET(AicpuAclKernelLaunch(aicpuStream, reinterpret_cast<void *>(&customInitTask), sizeof(customInitTask),
            binCustomHandle_, kernelName, true, timeOut));
    ...
    const DeviceMem &deviceContext, const std::string &kernelName, const AicpuOpTiling opTilingInfo)
{
    ...
    CHK_RET(AicpuUnfoldKernelLaunch(opParam.inputPtr, opParam.outputPtr, kfcOpStream,
        reinterpret_cast<u64>(deviceContext.ptr()), &tilingDate, sizeof(HcclKFCTilingData), kernelName, mode,
        opParam.tag));
...
HcclResult HcclCommunicator::AicpuKfcClearOpResLaunch(const std::unordered_set<std::string> &tags)
{
    ...
    while (it != tags.end()) {
        ...
        HcclResult ret = AicpuAclKernelLaunchV2(opStream_.ptr(), reinterpret_cast<void *>(&initTask), sizeof(initTask),
            binHandle_, "RunAicpuKfcClearOpRes", true, timeOut, nullptr, 0, identifier_);
        if (ret != HCCL_SUCCESS) {
    ...
    if (opParam.aicpuCacheEnable != 0 && GetWorkflowMode() == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OPS_KERNEL_INFO_LIB
        ...
        CHK_PRT_RET(opTilingData->aicpuCacheEnable <= FORCE_OP_BASE_DELTA,
            HCCL_ERROR("[HcclCommunicator][AicpuInitOpTilingDataBuf] enforce opbase mode: "
                       "opTilingData->aicpuCacheEnable[%u] <= %u",
                opTilingData->aicpuCacheEnable, FORCE_OP_BASE_DELTA),
            HCCL_E_INTERNAL);
    }
    ...
    const std::string &kernelName, const AicpuOpTiling opTilingInfo, u64 dynamicDataSize)
{
    ...
    CHK_SAFETY_FUNC_RET(memcpy_s(opTilingData->newTag, sizeof(opTilingData->newTag), opTilingInfo.newTag.c_str(),
...
HcclResult HcclCommunicator::InitAndCheckAicpuOrderNotify(u8 &orderLaunchMode)
{
    ...
    CHK_RET(
        hrtMemSyncCopy(static_cast<char *>(aicpuOrderNotifyAddr_.ptr()) + (sizeof(HcclSignalInfo) * orderLaunchMode),
    ...
    const std::string &kernelName, const AicpuOpTiling opTilingInfo, u64 opTilingDataSize, bool isCustom)
{
    ...
    if (opParam.isCapture) { // 非主流下发时，acl graph场景，capture从流
        ...
        CHK_RET(GetModelId(rtModel, modelId));
        HCCL_INFO("[HcclCommunicator][%s]tag[%s], add stream[%d] to modelId[%llu] success.", __func__,
            opParam.tag.c_str(), streamId, modelId);
    }
    ...
    CHK_RET(KernelLaunchChooseAicpuOrCustom(opParam.inputPtr, opParam.outputPtr, kfcOpStream.ptr(),
        reinterpret_cast<u64>(deviceContext.ptr()), opTilingDataMem.ptr(), opTilingDataSize, kernelName, mode,
        opParam.tag, isCustom));
    if (opParam.isCapture) {
...
HcclResult HcclCommunicator::AicpuKfcTilingDataLaunchExt(const OpParam &opParam, const HcclCMDType &opType,
    const DeviceMem &deviceContext, const std::string &kernelName, const AicpuOpTiling opTilingInfo, bool isCustom)
{
    ...
    const std::string &tag)
{
    ...
    CHK_PRT(AicpuAclKernelLaunch(stm, reinterpret_cast<void *>(&apiParam), sizeof(apiParam), binHandle_, kernelName,
        false, timeOut, tilingDataPtr, tilingDataSize));
    ...
    const std::string &tag, bool isCustom)
{
    ...
    HcclResult ret = AicpuAclKernelLaunchV2(stm, reinterpret_cast<void *>(&context), sizeof(context), binHandle,
        kernelName, false, timeOut, tilingDataPtr, tilingDataSize, identifier_);
    CHK_PRT_RET(ret != HCCL_SUCCESS,
...
HcclResult HcclCommunicator::InitCombinOpara()
{
    ...
    for (u32 i = 0;
        i < sizeof(combinOparaPtr->signalInfo.noIpcNotifys) / sizeof(combinOparaPtr->signalInfo.noIpcNotifys[0]); i++) {
...
{
    if (Is310P3Common(isHaveCpuRank_, deviceType_)) {
        HCCL_ERROR("[%s][%s]GetAlltoAllStagedWorkSpaceMemSize Not Supported!", LOG_KEYWORDS_TASK_EXEC.c_str(),
            LOG_KEYWORDS_NOT_SUPPORTED.c_str());
        return HCCL_E_NOT_SUPPORT;
    }
    CHK_SMART_PTR_NULL(implAlg_);
    ...
    vector<u32> &nicRanksPort, vector<u32> &vnicRanksPort)
{
    for (auto &ipInfo : phyIdNicInfoMap) {
        for (auto &devInfo : ipInfo.second) {
            ...
            HCCL_DEBUG("phyIdNicInfoMap print hostIp[%s] devId[%u] devIp[%s]", ipInfo.first.c_str(), devInfo.first,
...
{
    HcclResult ret = HCCL_SUCCESS;
    CHK_PRT_RET(!IsEnableBackupLink(),
        HCCL_RUN_WARNING(
            ...
            IsEnableRoce()),
        HCCL_SUCCESS);
    CHK_PRT_RET(resMap_.empty(),
        HCCL_ERROR("[HcclCommunicator][%s] "
    ...
    u8 *distRemoteRankNicStatus, u32 &distNicStatusNum, bool &needCheckDefaultNic, bool &needCheckBackupNic)
{
    ...
    CHK_PRT_RET(ret != EOK,
        HCCL_ERROR("[HcclCommunicator][GetSwitchRanks] mem copy switch rank list fail, ret[%u].", ret),
        HCCL_E_INTERNAL);
    ret = memcpy_s(
        distSwitchUseBackup, sizeof(bool) * AICPU_MAX_RANK_NUM, switchUseBackup_, sizeof(bool) * switchRanksNum_);
    CHK_PRT_RET(ret != EOK,
        HCCL_ERROR("[HcclCommunicator][GetSwitchRanks] mem copy switch use backup fail, ret[%u].", ret),
        HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(ret != EOK,
        HCCL_ERROR("[HcclCommunicator][GetSwitchRanks] mem copy remote rank nic status fail, "
                   "ret[%u].",
            ret),
        HCCL_E_INTERNAL);
...
HcclResult GetCannPath(const char *binPath, std::string &cannPath)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[GetCannPath]cannot found version file in %s.", libraryPath.c_str()),
        HCCL_E_PARA);
    tmpPath += binPath;
    cannPath = tmpPath;
...
HcclResult HcclCommunicator::DeregisterCommUserMem(void *handle)
{
    if (deviceType_ != DevType::DEV_TYPE_910_93 || superPodNum_ > 1) {
        HCCL_ERROR("[HcclCommunicator][%s]Unsupported on the device type[%d] or superPodNum[%u]", __func__, deviceType_,
            superPodNum_);
        return HCCL_E_NOT_SUPPORT;
    }
...
HcclResult HcclCommunicator::ExchangeCommUserMem(void *handle, std::vector<u32> &peerRanks)
{
    ...
    HcclResult ret = HCCL_SUCCESS;
    {
        ...
        HCCL_RUN_INFO("[%s]Alloc transport, level size[%u], trans request size[%u], mem ptr[%p], mem size[%llu]",
            __func__, opCommTransport.size(), commTransport.transportRequests.size(), userMem.ptr(), userMem.size());
...
HcclResult HcclCommunicator::GetLocalCCLBuf(void **addr, uint64_t *size)
{
    ...
    *size = cclbufSize;
    HCCL_INFO("[%s] GetlocalCCLBuf success, addr[%p], size[%u]", identifier_.c_str(),
...
{
    CHK_PRT_RET((remoteRank >= AICPU_MAX_RANK_NUM), HCCL_ERROR("[%s] invalid remoteRank[%d]", __func__, remoteRank),
        HCCL_E_PARA);
    // 仅sdma场景
    uint64_t cclbufSize = cclBufferManager_.GetInCCLbufferSize() + cclBufferManager_.GetOutCCLbufferSize()
                          + cclBufferManager_.GetExpBufferSize();
    ...
    *size = cclbufSize;
    HCCL_INFO("[%s] GetRemoteCCLBuf success, remoteRank[%u], addr[%p], size[%u]", identifier_.c_str(), remoteRank,
    ...
    const std::string &tag, OpCommTransport &opCommTransport, TransportIOMem &transMem, bool isAicpuModeEn)
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[%s] Failed to alloc transport, tag[%s], isAicpuModeEn[%d], ret[%d]", __func__, tag.c_str(),
            isAicpuModeEn, ret);
        return ret;
    }
...
HcclResult HcclCommunicator::GetRanksByTopoInst(
    uint32_t netLayer, uint32_t topoInstId, uint32_t **ranks, uint32_t *rankNum)
{
...
HcclResult HcclCommunicator::SnapshotCheckPostProcess()
{
    ...
    CHK_PRT_RET(errorFlag,
        HCCL_ERROR("[HcclCommunicator][SnapshotCheckPostProcess] comm[%s], rank[%u], "
...
HcclResult HcclCommunicator::InitSymmetricMemory()
{
    ...
    symmetricMemoryAgent_ = std::make_shared<SymmetricMemoryAgent>(socketManager_, devicePhyId_, deviceLogicId_,
        localVnicIp_, rankInfoList_, realUserRank_, useSuperPodMode_, identifier_);
...
{
    CHK_PRT_RET(superPodNum_ > 1, HCCL_ERROR("[RegisterWindow] Cross-SuperNode not support symmetric memory"),
        HCCL_E_NOT_SUPPORT);
...
HcclResult HcclCommunicator::GetDevMemWorkSpace(
    const std::string &memTag, uint64_t *size, void **addr, bool *newCreated)
{
