...
namespace hccl
{
    ...
    */
        ...
        const std::string &tag, bool isCustom)
    {
        ...
        // TODOSSY: debug
        struct OpTilingData *opTilingDataPtr = reinterpret_cast<struct OpTilingData *>(tilingDataPtr);
        struct OpTilingAlltoallvcDataDes *dynamicDataPtr =
            reinterpret_cast<struct OpTilingAlltoallvcDataDes *>(reinterpret_cast<uint8_t *>(tilingDataPtr) + sizeof(struct OpTilingData));
        HCCL_DEBUG("[HcclCommunicator][AicpuUnfoldKernelLaunchV2] FORDEBUG tilingDataSize[%llu] sizeof(struct OpTilingData)[%llu] "
            "tilingDataPtr[%p] dynamicLength[%llu] dynamicDataPtr->sendCountMatrix[%p] dynamicDataPtr->sendCountMatrix[0][%llu]",
            tilingDataSize, sizeof(struct OpTilingData), tilingDataPtr, opTilingDataPtr->length, dynamicDataPtr->sendCountMatrix, dynamicDataPtr->sendCountMatrix[0]);
