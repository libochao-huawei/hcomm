...
namespace hccl
{
    ...
    HcclResult HcclCommunicator::InitPreResource(const RankTable_t &rankTable)
    {
        ...
        for (u32 i = 0; i < iterServ->second.size(); i++) {
            HCCL_INFO("TEST3 size[%u], servRankInfo(serverid[%s], rankId[%u]), i[%u]",
                iterServ->second.size(), serverId_.c_str(), iterServ->second[i].rankId, i);
            if (iterServ->second[i].deviceInfo.devicePhyId != HOST_DEVICE_ID) {
                ...
                HCCL_INFO("[Init][PreResource]In the current server[%s], userRank[%u]-localuserRank[%u]-localDevicePhyId[%u]-rankidlocalDevicePhyId[%u] needs to enableP2P " \
                    "with remoteRankId[%u]-remoteDevicePhyId[%u]",
                    serverId_.c_str(), userRank_, iterServ->second[userRank_].rankId, iterServ->second[userRank_].deviceInfo.devicePhyId,
                    iterServ->second[iterServ->second[userRank_].rankId].deviceInfo.devicePhyId,
