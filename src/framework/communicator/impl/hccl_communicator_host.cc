    ...
    HcclResult HcclCommunicator::InitPreResource(const RankTable_t &rankTable)
    {
        ...
        if (deviceType_ == DevType::DEV_TYPE_910_93) {
            ...
            for (size_t index = 0; index < rankInfoList_.size(); ++index)
            {
                ...
                HCCL_INFO("TEST1 userRank[%u], worldRank[%u], localRank[%u], index[%u], devPhyId[%u]",
                    rankInfo.userRank, rankInfo.worldRank, rankInfo.localRank, index, rankInfo.devicePhyId);
        ...
        for (u32 i = 0; i < iterServ->second.size(); i++) {
            if (iterServ->second[i].deviceInfo.devicePhyId != HOST_DEVICE_ID) {
                ...
                HCCL_INFO("[Init][PreResource]In the current server[%s], localRankId[%u]-localDevicePhyId[%u] needs to enableP2P " \
                    "with remoteRankId[%u]-remoteDevicePhyId[%u]",
                    serverId_.c_str(), iterServ->second[realUserRank_].rankId, iterServ->second[realUserRank_].deviceInfo.devicePhyId,
