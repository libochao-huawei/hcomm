...
namespace hccl
{
    ...
    HcclResult HcclCommunicator::CreateMyRank(HcclCommParams &params, const RankTable_t &rankTable, HcclTopoAttr &topoAttr)
    {
        ...
        std::unordered_map<u32, std::unordered_map<Hccl::IpAddress, u32>> rankListenPortMap{};
    
        EXECEPTION_CATCH((myRank_ = std::make_unique<MyRank>(binHandle, rankId, commConfig, callbacks, &rankGraph_, rankListenPortMap)),
            return HCCL_E_PTR);
