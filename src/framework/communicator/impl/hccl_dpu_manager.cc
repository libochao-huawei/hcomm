...
HcclResult DpuManager::LaunchDpuKernel(aclrtFuncHandle& funcHandle)
{
    ...
    HCCL_INFO("[DpuManager::%s] DpuKernelLaunchParam{commId:%s; memorySize:%u; deviceMem:%p; hostMem:%p}",
        __func__, g_hostArgsTemp.commId.c_str(), g_hostArgsTemp.memorySize, g_hostArgsTemp.deviceMem,
