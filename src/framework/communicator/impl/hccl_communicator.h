...
public:
    ...
    virtual HcclResult AllGatherVOutPlace(const std::string &tag, void *inputPtr, void *outputPtr, u64 inputCount,
    ...
    virtual HcclResult AlltoAll(const void *sendBuf, u64 sendCount, HcclDataType sendType, const void *recvBuf,
    ...
    virtual HcclResult Reduce(const std::string &tag, void *inputPtr, void *outputPtr, u64 count, HcclDataType dataType,
        HcclReduceOp op, u32 root, HcclRtStream stream);
    ...
    virtual HcclResult ReduceScatterV(const std::string &tag, void *inputPtr, const void *inputCounts,
    ...
    virtual HcclResult Send(const std::string &tag, void *inputPtr, u64 count, HcclDataType dataType, u32 destRank,
        rtStream_t stream, u32 srTag = 0, u32 localGroupRank = 0);
    ...
    virtual HcclResult Receive(const std::string &tag, void *outputPtr, u64 count, HcclDataType dataType, u32 srcRank,
        rtStream_t stream, u32 srTag = 0, u32 localGroupRank = 0);
    ...
    HcclResult HcclGetAlgExecParam(const std::string &tag, HcclCMDType opType, u64 count, void *inputPtr,
    ...
    HcclResult GetWorkspaceSubStreamNum(u64 count, HcclDataType dataType, HcclReduceOp op, const std::string &algName,
        u64 &streamNum, u64 dataSize, bool ifAiv, HcclCMDType opType);
    HcclResult GetWorkspaceMemSize(const std::string &opType, u64 count, HcclDataType dataType, u32 &rankSize,
        u64 &memSize, DevType &deviceType) const;
    ...
    HcclResult SetWorldGroupInfo(std::unordered_map<std::string, std::map<u32, HcclIpAddress>> phyIdNicInfoMap,
    ...
    HcclResult AllocComResource(const std::string &newTag, const std::string &algName, const HcclCMDType commType,
        const OpParam &opParam, rtStream_t stream, bool isNeedHostSlaveStream = true);
    ...
    HcclResult SetTransportStatus(const HcclOpIdentifier &opId, bool statusStop,
        const std::map<u32, bool> &remoteRankPortMap, const std::map<u32, bool> &isChangeLinkMap,
        bool isChangeLinkFlag);
    ...
    HcclResult GetCacheMap(std::unique_ptr<CollAlgOperator> &algOperator, OpParam &opParam, AlgType &algType,
        bool selectAivAlg, std::string &newTag);
...
private:
    ...
    HcclResult CheckOneSidedBackupAndSetDevId(u32 &backupDevPhyId, u32 &backupDevLogicId,
        std::vector<HcclIpAddress> &localIpList, bool &isOneSidedTaskAndBackupInitA3);
    HcclResult OneSidedBackupInitNetResource(HcclNetDevCtx &nicPortBackUpCtx, u32 &backupDevPhyId,
    ...
    HcclResult InitDeviceBackupNic(u32 backupDevPhyId, u32 backupDevLogicId, std::vector<HcclIpAddress> &localIpList,
    ...
    HcclResult SetCommResource(u64 commBufferSize, void *commInPtr, void *commOutPtr, void *commExpPtr, CommBase *comm,
    ...
    HcclResult AicpuKfcTilingDataLaunch(const OpParam &opParam, const HcclCMDType &opType,
    ...
    HcclResult RegisterDfxInfo(const OpParam &param, AlgType algType, const std::vector<Stream> &slaveStreams,
        bool isAiv = false, const std::string &tag = "");
    ...
    HcclResult SetTransportStatusImpl(OpCommTransport &opCommTransport, bool statusStop, const HcclOpIdentifier &opId,
    ...
    HcclResult ResumeTransportsImplForChange(OpCommTransport &opCommTransport,
        const std::map<u32, bool> &remoteRankPortMap, const std::map<u32, bool> &isChangeLinkMap, bool isUseDefault);
    HcclResult ResumeTransportsImpl(OpCommTransport &opCommTransport, const std::map<u32, bool> &remoteRankPortMap,
        bool isUseDefault, bool statusStop);
    HcclResult SetBsrTransportStatusImplforchange(OpCommTransport &opCommTransport, const HcclOpIdentifier &opId,
        u32 remoteRank, const std::map<u32, bool> &remoteRankPortMap, bool isUseDefault,
        const std::map<u32, bool> &isChangeLinkMap, bool isCurTag);
    ...
    HcclResult SwitchNic(uint32_t nRanks, uint32_t *ranks, bool *useBackup, std::shared_ptr<HDCommunicate> &controlH2D,
    ...
    HcclResult ReAllocScratchMemForAlltoall(HcclCMDType opType, const OpParam &opParam, AlgResourceRequest &resRequest,
        AlgResourceResponse &algResResponse);

    HcclResult HandleExistAlgResource(const std::string &newTag, const std::string &algName, HcclCMDType opType,
        const OpParam &opParam, std::unique_ptr<CollAlgOperator> &algOperator, bool selectAivAlg,
...
private:
    ...
    HcclResult AllocOpBaseModeScratchMem(HcclCMDType opType, const OpParam &opParam, AlgResourceRequest &resRequest,
        AlgResourceResponse &algResResponse);
    ...
    HcclResult IncreAllocLink(const std::string &newTag, const OpParam &opParam, AlgResourceRequest &resRequest,
        AlgResourceResponse &algResResponse);
    ...
    HcclResult AicpuUnfold(const std::string &tag, void *inputPtr, void *outputPtr, u64 count, HcclDataType dataType,
        HcclReduceOp op, HcclRtStream stream, HcclCMDType cmdType);
    ...
    HcclResult AicpuResourceInit(const std::string &algName, const AlgResourceResponse &algResource,
        const std::string &newTag, const rtStream_t &aicpuStream, const HcclCMDType opType, bool isCustom = false);
    ...
    template <typename T> HcclResult CreateListNode(T **resHostPtr, T **resDevicePtr);
