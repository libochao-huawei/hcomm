...
HcclResult HcclOneSidedConn::Connect(const std::string &commIdentifier, s32 timeoutSec)
{
    ...
    if (isStandardCard_ && !useRdma_) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[HcclOneSidedConn][Connect]Enable P2P Failed, deviceLogicId[%d], ret[%u]", deviceLogicId_, ret), ret);
    }
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[HcclOneSidedConn][Connect]Wait Enable P2P Failed, src devicePhyId[%d], dst devicePhyId[%d], ret[%u]",
