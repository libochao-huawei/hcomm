...
HcclResult HcclCommunicator::InitTransportManager()
{
    ...
    transportManager_.reset(
        static_cast<TransportManager*>(new (std::nothrow) TransportManager(
            ...
            sizeof(transportResInfo_), isUseRankPort_, isUsedRdmaLevel0_, nicRanksPorts, vnicRanksPorts,
            useSuperPodMode_, devIpAddr_, hostIp_, localVnicIp_, netDevCtxMap_)));
    ...
    CHK_PTR_NULL(ctx);
    indptOpTransportManager_.reset(
        static_cast<TransportManager*>(new (std::nothrow) TransportManager(
...
HcclResult
HcclCommunicator::InitRankInfoSubGroup(const std::vector<RankInfo>& rankList, WorldGroupInfo& groupCommonData)
{
    SetAttrs();
    // inline reduce 开关
...
HcclResult HcclCommunicator::ReleasePreemptSocket()
{
    ...
    if (commPortConfig_.backupDevNicListen.first) {
        ...
        if (hrtGetDeviceIndexByPhyId(deviceBackUpPhyId_, deviceBackUpLogicId_) == HCCL_SUCCESS) {
            CHK_RET(
                PreemptPortManager::GetInstance(deviceBackUpLogicId_)
