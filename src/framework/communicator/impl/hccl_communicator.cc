...
unordered_map<std::string, std::string> ALGCFG_TO_NAME = {{"AllGather=level0:ring", "AllGatherRingFor91093Executor"},
    ...
    {"AlltoAll=level1:hierarchy", "RunAlltoAllAivDirect"}, {"BatchWrite=level0:fullmesh", "BatchWriteBySdma"},
...
HcclResult HcclCommunicator::InitRankInfo(const RankTable_t &rankTable)
{
    ...
    SetRetryEnable(deviceType_, superPodNum_, serverNum_, deviceNumPerAggregation_, isDiffDeviceType_, isAivMode,
        serverIp, localIp, retryEnable_, commConfig_.GetConfigInterServerRetryEnable(),
...
HcclResult HcclCommunicator::InitDispatcher()
{
    ...
    HCCL_INFO("[%s] RegisterLoadTaskCallBack Dispatcher = [%p], Ctx_tmp = [%p]", __func__, Ctx_tmp->GetDispatcher(),
...
HcclResult HcclCommunicator::InitTransportManager()
{
    ...
    transportManager_.reset(static_cast<TransportManager *>(
        new (std::nothrow) TransportManager(cclBufferManager_, socketManager_, dispatcher_, notifyPool_, rankInfoList_,
            userRank_, identifier_, deviceLogicId_, nicDeployment_, isHaveCpuRank_,
            static_cast<const void *>(&transportResInfo_), sizeof(transportResInfo_), isUseRankPort_, isUsedRdmaLevel0_,
            nicRanksPorts, vnicRanksPorts, useSuperPodMode_, devIpAddr_, hostIp_, localVnicIp_, netDevCtxMap_)));
    ...
    CHK_PTR_NULL(ctx);
    indptOpTransportManager_.reset(static_cast<TransportManager *>(
        new (std::nothrow) TransportManager(cclBufferManager_, socketManager_, ctx->GetDispatcher(), notifyPool_,
            rankInfoList_, userRank_, identifier_, deviceLogicId_, nicDeployment_, isHaveCpuRank_,
            static_cast<const void *>(&transportResInfo_), sizeof(transportResInfo_), isUseRankPort_, isUsedRdmaLevel0_,
            nicRanksPorts, vnicRanksPorts, useSuperPodMode_, devIpAddr_, hostIp_, localVnicIp_, netDevCtxMap_)));
...
{
    HCCL_INFO("%s aicpuUnfold[%d], deviceType_[%d], isHaveCpuRank_[%d]", __func__, GetAicpuUnfoldConfig(), deviceType_,
        isHaveCpuRank_);
    if (deviceType_ == DevType::DEV_TYPE_910B && GetAicpuUnfoldConfig() == false && IsOneSidedIdentifier(identifier_)) {
...
HcclResult HcclCommunicator::InitHDCommunicate()
{
    if (GetSupportHDCommunicate()) {
        ...
        EXECEPTION_CATCH((kfcControlTransferH2D_ = std::make_shared<hccl::HDCommunicate>(
                              deviceLogicId_, HCCL_HDC_TYPE_H2D, sizeof(KfcExecControl))),
            return HCCL_E_PTR);
        CHK_RET(kfcControlTransferH2D_->InitHost());

        EXECEPTION_CATCH((kfcStatusTransferD2H_ = std::make_shared<hccl::HDCommunicate>(
                              deviceLogicId_, HCCL_HDC_TYPE_D2H, sizeof(KfcExecStatus))),
            return HCCL_E_PTR);
        CHK_RET(kfcStatusTransferD2H_->InitHost());

        if (IsEnableCustom()) {
            // 初始化custom进程host-device共享内存
            EXECEPTION_CATCH((customControlTransferH2D_ = std::make_shared<hccl::HDCommunicate>(
                                  deviceLogicId_, HCCL_HDC_TYPE_H2D, sizeof(KfcExecControl))),
                return HCCL_E_PTR);
            CHK_RET(customControlTransferH2D_->InitHost());

            EXECEPTION_CATCH((customStatusTransferD2H_ = std::make_shared<hccl::HDCommunicate>(
                                  deviceLogicId_, HCCL_HDC_TYPE_D2H, sizeof(KfcExecStatus))),
                return HCCL_E_PTR);
...
u32 HcclCommunicator::CalcStreamNumForReduceOrderPreservation()
{
    ...
    HCCL_INFO("[%s]level0StreamNum[%u], level1StreamNum[%u], streamNum[%u]", __func__, level0StreamNum, level1StreamNum,
        streamNum);
    return streamNum;
}
...
ErrorMessageReport HcclCommunicator::GetAicpuTaskException()
{
    ...
    if (kfcStatusTransferD2H_ != nullptr) {
        CHK_PRT_RET(isInvalidComm_,
            ...
            errorMessage);
        ret = kfcStatusTransferD2H_->Get(sizeof(HcclOpIdentifier) + sizeof(ExecStatusDef), sizeof(errorMessage),
    ...
    if (deviceType_ == DevType::DEV_TYPE_910 || (deviceType_ == DevType::DEV_TYPE_910B && !GetAicpuUnfoldFlag())) {
        if (GetMC2EnvFlag()) {
            if (controlH2D != nullptr) {
                ...
                CHK_RET(controlH2D->Put(sizeof(KfcCommand), sizeof(BackgroundCommand),
...
HcclResult HcclCommunicator::SetInfoToDevice(const OpParam &opParam,
    const std::unique_ptr<PreProcessMetaInfo> &preMetaInfo, const HcclWorkflowMode &mode, Stream &stream)
{
    ...
    HcclCMDType opType, OpParam &opParam, AlgType nslbAlgType, AdjInfo &nslbAdjInfo)
{
    ...
    HCCL_INFO("NSLBDP-SHEN opType:[%u],srcLocalRankId[%u],rootRank[%u],algValue[%u],rankSize[%u]-commDesc[%s].", opType,
    ...
    if (GetAicpuUnfoldFlag() || GetAicpuCommEngine()) {
        ...
        while (true) {
            ...
            if (opInfo.execStatus.kfcStatus == KfcStatus::kStoplaunch) {
                ...
                if ((std::chrono::steady_clock::now() - startTime) >= waitStopExecCmdTimeout) {
                    HCCL_ERROR("[NsRecovery]Wait suspend response status timeout[%u ms] and get the opExecState is "
                               "[%u] and opId[%u].",
        ...
        if (opInfo.execStatus.kfcStatus == KfcStatus::kStoplaunch) {
            ...
            while (true) {
                ...
                if (opInfo.execStatus.kfcStatus == KfcStatus::kStopExec) {
                    ...
                    if ((std::chrono::steady_clock::now() - startTime) >= waitStopExecCmdTimeout) {
                        HCCL_ERROR("[NsRecovery]Wait stopExec response status timeout[%u ms] and get the opExecState "
                                   "is [%u] and opId[%u].",
                            waitStopExecCmdTimeoutMs, opInfo.execStatus.kfcStatus, opInfo.opId.index);
                        return HCCL_E_INTERNAL;
                    }
        ...
        if (opInfo.execStatus.kfcStatus == KfcStatus::kStopExec || opInfo.execStatus.kfcStatus == KfcStatus::kEnd) {
            ...
            while (true) {
                ...
                if (opInfo.execStatus.kfcStatus == KfcStatus::kClear) {
                    ...
                    if ((std::chrono::steady_clock::now() - startTime) >= waitStopExecCmdTimeout) {
                        HCCL_ERROR("[NsRecovery]Wait clean response status timeout[%u ms] and get the opExecState is "
                                   "[%u] and opId[%u].",
                            waitStopExecCmdTimeoutMs, opInfo.execStatus.kfcStatus, opInfo.opId.index);
                        return HCCL_E_INTERNAL;
                    }
...
HcclResult HcclCommunicator::CheckNotifyOrQPMaxNum(u64 &existNum, const u64 &MaxNum, const bool &isNotifyRes)
{
    ...
    if (existNum + 1 > MaxNum) {
        HCCL_ERROR("[%s]%s resources are insufficient, existNum[%llu], MaxNum is [%llu]", __func__, resType.c_str(),
            existNum, MaxNum);
        return HCCL_E_INTERNAL;
    }
    HCCL_DEBUG("[%s]%s resources are sufficient, existNum[%llu], MaxNum is [%llu]", __func__, resType.c_str(), existNum,
        MaxNum);
    return HCCL_SUCCESS;
}
...
HcclResult HcclCommunicator::GetAlgInfo(const std::string &algConfig, const std::string &tag, HcclCMDType commType,
    std::string &algName, std::string &newTag)
{
    ...
    HCCL_INFO("[%s] tag=[%s], algName=[%s], newTag=[%s], topoType=[%d]", __func__, tag.c_str(), algName.c_str(),
    ...
    const OpParam &opParam, HostMem &dynamicDataMem, const std::string &algName)
{
    ...
    HCCL_DEBUG("[SetDynamicTilingDataAlltoallv] set dynamic tiling data for AllToAllV success, alltoallvDataPtr[%p]",
        alltoallvDataPtr);
    return HCCL_SUCCESS;
}
...
HcclResult HcclCommunicator::KernelLaunchChooseAicpuOrCustom(void *inputPtr, void *outputPtr, const rtStream_t stm,
...
HcclResult HcclCommunicator::SetTransportStatusImplForChange(OpCommTransport &opCommTransport,
    const HcclOpIdentifier &opId, u32 remoteRank, const std::map<u32, bool> &remoteRankPortMap, bool isUseDefault,
    const std::map<u32, bool> &isChangeLinkMap, bool isCurTag)
{
    bool isSendRecv = opId.isSendRecv;
    ...
    if (statusStop) {
        ...
        if (isChangeLinkFlag) {
            ...
            for (auto &resMapIt : resMap_) {
                ...
                CHK_RET(SetTransportStatusImplForChange(resMapIt.second.opTransportResponseBackUp, opId, remoteRank,
                    remoteRankPortMap, false, isChangeLinkMap, isCurTag));
...
HcclResult HcclCommunicator::SetTransportResumeStatus(const std::map<u32, bool> &remoteRankPortMap,
    const std::map<u32, bool> &isChangeLinkMap, bool isChangeLinkFlag, bool statusStop)
{
    HCCL_INFO("[SetTransportResumeStatus]isChangeLinkFlag[%d], rank[%u], group[%s]", isChangeLinkFlag, userRank_,
        identifier_.c_str());
...
HcclResult HcclCommunicator::ResumeTransportsImplForChange(OpCommTransport &opCommTransport,
    const std::map<u32, bool> &remoteRankPortMap, const std::map<u32, bool> &isChangeLinkMap, bool isUseDefault)
{
    // 借轨的resume阶段
    for (auto &levelNSubCommTransport : opCommTransport) {
        for (auto &singleSubCommTransport : levelNSubCommTransport) {
...
HcclResult HcclCommunicator::HostMC2EnvResume()
{
    if (GetAicpuUnfoldFlag() || GetAicpuCommEngine()) {
        ...
        if (IsEnableCustom()) {
            CHK_RET(
                customControlTransferH2D_->Put(0, sizeof(KfcExecControl), reinterpret_cast<uint8_t *>(&controlCmd)));
        }
    }
    return HCCL_SUCCESS;
}
        ...
        for (auto &transports : opCommTransport) {
            for (u32 i = 0; i < transports.transportRequests.size(); i++) {
                if (transports.links[i] != nullptr
                    ...
                    LinkInfo linkInfo(identifier_, userRank_, localServerId, localDevicePhyId, remoteRank,
                        remoteServerId, remoteDevicePhyId);
...
HcclResult HcclCommunicator::GetAIVNormalQPInfo(CommBase *comm, const std::string &tag)
{
    ...
    CHK_PRT_RET(aiRMAInfoPtr->qpNum <= 0,
        HCCL_ERROR("[%s] invalid qpNum. tag[%s] curRankId[%u] rankNum[%u] qpNum[%u]", __func__, tag.c_str(),
    ...
    u32 rankid, const std::shared_ptr<Transport> &transport, const std::string &tag, T *aiRMAInfoPtr)
{
    ...
    CHK_PRT_RET(aiQpVec.size() != aiRMAInfoPtr->qpNum,
        HCCL_ERROR("[%s] different qpNum. tag[%s] curRankId[%u] rankNum[%u] qpNum[%u] qpVecNum[%u]", __func__,
            tag.c_str(), aiRMAInfoPtr->curRankId, aiRMAInfoPtr->rankNum, aiRMAInfoPtr->qpNum, aiQpVec.size()),
        HCCL_E_INTERNAL);
