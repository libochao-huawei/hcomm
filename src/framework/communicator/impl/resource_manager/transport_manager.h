...
private:
    ...
    HcclResult CreateBatchSendRecvLinks(const std::string &tag, const TransportIOMem &transMem,
        struct LinkPoolPara &linkPoolPara, u32 threadIndex, bool isAicpuModeEn, bool isBackup, u32 subCommIndex,
        bool isCapture = false, const HcclCMDType &opType = HcclCMDType::HCCL_CMD_INVALID, bool isIndOp = false);
    ...
    std::mutex createSocketMutex;    // BatchSendRecv建链调用CreateDestSockets时，保护socketTagVec_等资源
};
}  // namespace hccl
