...
struct TransportData {
    ...
    TransportData(LinkMode linkMode, const std::vector<HcclIpAddress> &remoteIpAddr, u32 remoteUserrank,
        u32 remoteWorldRank, s32 remoteDeviceId, DevType deviceType, const DeviceMem &inputMem,
        const DeviceMem &outputMem, bool supportDataReceivedAck, u32 remoteSocketPort)
        : linkMode(linkMode),
...
struct SubCommLinkPara {
    ...
    SubCommLinkPara(struct SingleSubCommTransport &singleSubCommTransport,
        std::vector<std::pair<u32, u32>> &remoteRankMap, u32 remoteRankIdStartIndex, u32 remoteRankIdNum)
...
struct LinkPoolPara {
    ...
    LinkPoolPara(struct SingleSubCommTransport &transport, const std::string &name,
...
public:
    TransportManager(CCLBufferManager &cclBufferManager, const std::unique_ptr<HcclSocketManager> &socketManager,
    ...
    HcclResult GetIncreRemoteRankList(OpCommTransport &opTransportReq, OpCommTransport &opTransportResponse,
...
private:
    HcclResult GetIOMem(const TransportIOMem &transMem, const TransportMemType inputMemType,
    ...
    HcclResult TransportInit(const u32 dstRank, MachinePara &machinePara, std::shared_ptr<Transport> &link,
        bool useOneDoorbell, bool isUsedRdma, TransportType type);
    ...
    HcclResult GetTransNewTag(const std::string &tag, std::string &newTag, RankId remoteRank, bool &isInterRdma,
        u32 subCommIndex, TransportLinkType linkType, HcclRankLinkInfo remoteLink, uint32_t mode);
    HcclResult CreateBatchSendRecvLinks(const std::string &tag, const TransportIOMem &transMem,
        struct LinkPoolPara &linkPoolPara, bool isAicpuModeEn, bool isBackup, u32 subCommIndex, bool isCapture = false,
    ...
    HcclResult AllocBatchSendRecvLinks(HcclSendRecvItem *sendRecvItemsPtr, u32 itemNum, const std::string &tag,
        const TransportIOMem &transMem, struct SingleSubCommTransport &singleSubCommTransport, bool isAicpuModeEn,
        bool isBackup, u32 subCommIndex, bool isCapture = false,
        const HcclCMDType &opType = HcclCMDType::HCCL_CMD_INVALID, bool isIndOp = false);
    HcclResult PrepareTaskLists(HcclSendRecvItem *sendRecvItemsPtr, u32 itemNum,
