...
HcclResult TransportManager::CreateVirturalTransport(SingleSubCommTransport& singleSubCommTransport)
{
    ...
    for (u32 i = 0; i < singleSubCommTransport.transportRequests.size(); i++) {
        ...
        CHK_PRT_RET(
            ...
            HCCL_ERROR(
                "[CreateVirturalTransport]In create link,"
                "new link failed"),
            HCCL_E_PTR);
    }
    ...
    const HcclCMDType& opType, bool isNpuDirectRoce)
{
    ...
    CHK_RET(
        RankConsistentcyChecker::GetInstance().GetCheckFrame(
    ...
    TransportType type)
{
    ...
    if (recvData.size() != 0) {
        CHK_PRT_RET(
            ...
            HCCL_E_INTERNAL);
        CHK_RET(
            RankConsistentcyChecker::GetInstance().CheckFrameRecv(
