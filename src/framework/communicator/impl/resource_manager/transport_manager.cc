    ...
    bool isIndOp, const IndOpMem indOpMemd, const HcclCMDType &opType, bool isNpuDirectRoce)
{
    ...
    if (ret == HCCL_SUCCESS) {
        ...
        HCCL_ERROR("[TransportManager][%s]nicType[%d], logicSuperPodId[%s], phySuperPodId[%lld].Cannot assign ranks from different physical nodes to the same logical node.", nicType, logicSuperPodId, phySuperPodId);
        // 临时硬编码出包验证
        return HCCL_E_PARA;
    }
