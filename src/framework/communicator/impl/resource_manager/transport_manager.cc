...
HcclResult TransportManager::PrintErrorInfo(NicType nicType)
{
    ...
    if (logicSuperPodId.empty()) {
        HCCL_ERROR("[TransportManager][%s]nicType[%s], logicSuperPodId is not set, phySuperPodId[%lld].", 
            __func__, nicTypeStr.c_str(), phySuperPodId);
    } else {
        HCCL_ERROR("[TransportManager][%s]nicType[%s], logicSuperPodId[%s], phySuperPodId[%lld]. Maybe ranks from "
            "swparate physical super-nodes are assigned to one logical super-node.", 
