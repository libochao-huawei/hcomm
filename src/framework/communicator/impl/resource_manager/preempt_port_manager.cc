    ...
    const std::vector<HcclSocketPortRange>& portRange, u32& usePort)
{
    ...
    HCCL_ERROR(
        "[PreemptPortManager][PreemptPortInRange] All ports in socket port range are bound already. "
        "no available port to listen. Please check the ports status, or change the port range to listen on.");
    HCCL_ERROR(
        "NOTICE: Users need to make sure ports in HCCL_HOST_SOCKET_PORT_RANGE and HCCL_NPU_SOCKET_PORT_RANGE "
    ...
    HCCL_ERROR(
        "NOTICE: The host/npu port range size is not suggested to be smaller than the process number"
        " on current rank.");
