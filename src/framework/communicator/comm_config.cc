...
HcclResult CommConfig::Load(const HcclCommConfig *userConfig)
{
    ...
    HCCL_RUN_INFO("[Load] comm config of [%s]: bufferSize[%llu], deterministic[%u], trafficClass[%u], serviceLevel[%u]"
        ", execTimeOut[%u], bufferName[%s], symmetricMemoryStride[%llu]",
...
HcclResult CommConfig::SetConfigByVersion(const CommConfigHandle &config)
{
    ...
    if (config.info.version >= CommConfigVersion::COMM_CONFIG_VERSION_TEN) {
        // 版本大于等于10，支持配置对称内存预留VA大小
        CHK_RET(SetConfigSymmetricMemoryStride(config));
    }
...
HcclResult CommConfig::SetConfigSymmetricMemoryStride(const CommConfigHandle &config)
{
    RPT_INPUT_ERR(config.symmetricMemoryStride < COMM_SYMMETRIC_MEMORY_MIN_STRIDE,
        "EI0003",
        std::vector<std::string>({"ccl_op", "parameter", "value", "tips"}),
        std::vector<std::string>({"HcclCommInitRootInfoConfig",
            "symmetricMemoryStride",
            std::to_string(config.symmetricMemoryStride),
            "symmetricMemoryStride Value is invalid. support >= 1."}));
    CHK_PRT_RET(config.symmetricMemoryStride < COMM_SYMMETRIC_MEMORY_MIN_STRIDE,
        HCCL_ERROR("[CommConfig][SetConfigByVersion] symmetricMemoryStride value=[%u] invalid. support >= 1",
            config.symmetricMemoryStride),
        HCCL_E_PARA);
