...
HcclResult HcclMemAlloc(void** ptr, size_t size)
{
    ...
    HCCL_INFO(
        "[HcclMemAlloc] deviceId[%d], granularity[%llu], size[%llu], allocSize[%llu].", deviceId, granularity, size,
        allocSize);
    ...
    CHK_PRT_RET(
        ret != ACL_SUCCESS,
        HCCL_ERROR(
            "[HcclMemAlloc] ReserveMemAddress failed, "
            "virPtr[%p] size[%llu], ret[%d]",
            ptr, allocSize, ret),
        HCCL_E_RUNTIME);
    ...
    if (ret != ACL_SUCCESS) {
        HCCL_ERROR(
            "[HcclMemAlloc] MapMem virPtr[%p] size[%llu] handle[%p] failed, ret[%d]", virPtr, allocSize, handle, ret);
...
HcclResult HcclMemFree(void* ptr)
{
    ...
    CHK_PRT_RET(
        ret != ACL_SUCCESS, HCCL_ERROR("[HcclMemFree] RetainAllocationHandle virPtr[%p] failed, ret[%d]", ptr, ret),
        HCCL_E_RUNTIME);
    HCCL_INFO("[HcclMemFree]Start to UnmapMem virPtr[%p], handle[%p]", ptr, handle);
    ret = aclrtUnmapMem(ptr);
    CHK_PRT_RET(
        ret != ACL_SUCCESS, HCCL_ERROR("[HcclMemFree] UnmapMem virPtr[%p] failed, ret[%d]", ptr, ret), HCCL_E_RUNTIME);
    ret = aclrtFreePhysical(handle);
    CHK_PRT_RET(
        ret != ACL_SUCCESS, HCCL_ERROR("[HcclMemFree] FreePhysical handle[%p] failed, ret[%d]", handle, ret),
        HCCL_E_RUNTIME);
    ret = aclrtReleaseMemAddress(ptr);
    CHK_PRT_RET(
        ret != ACL_SUCCESS, HCCL_ERROR("[HcclMemFree] ReleaseMemAddress virPtr[%p] failed, ret[%d]", ptr, ret),
        HCCL_E_RUNTIME);
    return HCCL_SUCCESS;
}
#ifdef __cplusplus
