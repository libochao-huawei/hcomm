...
private:
    ...
    void EnsureOrderStreamForGroup(std::string &group, Stream &orderStream);
    u64 GetCurrentContext();

    std::mutex streamMutex_;
    // std::unordered_map<std::string, u64> groupCtxMap_; // 记录已经初始化过的group, 并在其申请流时记录aclrtContext
    std::unordered_map<u64, std::unordered_set<std::string>> contextGroupsMap_; // 记录每个context对应的group
    std::unordered_map<std::string, OrderLaunchResMgr> orderLaunchGroupCtxResMgrMap_; // key:group, value:OrderLaunchResMgr, 管理申请的资源
