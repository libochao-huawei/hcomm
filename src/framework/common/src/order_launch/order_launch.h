...
{
    u64 context;
    std::unique_ptr<Stream> opbaseStream; // 单算子模式使用
    std::unique_ptr<Stream> aclgraphStream; // aclgraph模式使用
    bool resValid;
    bool contextInitialized; // 标记context是否已初始化，一旦初始化就不会变回INVALID_U64）

    OrderLaunchResMgr() : context(INVALID_U64), opbaseStream(nullptr), aclgraphStream(nullptr), resValid(false), contextInitialized(false) {}
...
private:
    ...
    HcclResult EnsureOrderStreamForGroup(std::string &group, std::unique_ptr<Stream> &orderStream);
