...
HcclResult OrderLaunch::EnsureOrderStreamForGroup(std::string &group, std::unique_ptr<Stream> &orderStream) {
    ...
    if (!groupCtxRes.IsCtxMatch(currentContext) || orderStream == nullptr || orderStream->ptr() == nullptr) {
        ...
        // 在UpdateContext之前，先从旧的context中删除旧的group
        if (contextGroupsMap_.find(groupCtxRes.context) != contextGroupsMap_.end()) {
            contextGroupsMap_[groupCtxRes.context].erase(group);
        }
        ...
        if (!groupCtxRes.contextInitialized) {
            groupCtxRes.MarkContextInitialized(currentContext);
        } else {
        ...
        HCCL_INFO("[OrderLaunch] Added group[%s] to context [0x%llx] with order stream id[%d]",
            group.c_str(), currentContext, orderStream->id());
    }
