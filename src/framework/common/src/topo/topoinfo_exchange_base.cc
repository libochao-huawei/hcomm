...
void TopoInfoExchangeBase::PrintRecvFailReasons(std::shared_ptr<HcclSocket> socket, HcclResult ret)
{
    ...
    HCCL_ERROR(
        ...
        socket->GetRemoteIp().GetReadableIP());
    HCCL_ERROR(
        "2. in large-scale cluster scenarios, occasional connection failures may occur due to the maximum connection "
        "limit in the system configuration. ");
    HCCL_ERROR(
        "   these issues can be resolved by modifying the system configuration in all node: `sysctl -w "
...
HcclResult TopoInfoExchangeBase::Json2Struct(const nlohmann::json& jClusterJson, RankTable_t& clusterInfo) const
{
    ...
    for (auto& rankInfoJson : jClusterJson[PROP_RANK_LIST]) {
        ...
        CHK_PRT_RET(
            ...
            HCCL_ERROR(
                "[Json2Struct] Fail to find port infos in rank info json. "
        ...
        if (rankInfoJson[PROP_DEV_INFO].find(PROP_BACKUP_DEV_IP) == rankInfoJson[PROP_DEV_INFO].end()) {
            HCCL_RUN_WARNING(
                "[Json2Struct] Fail to find backup device ip in rank info json. "
