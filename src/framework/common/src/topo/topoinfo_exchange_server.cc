...
HcclResult TopoInfoExchangeServer::StopSocketListen(
    const std::vector<HcclIpAddress>& whitelist, HcclIpAddress& hostIP, u32 hostPort)
{
    if (listenSocket_) {
...
HcclResult
TopoInfoExchangeServer::StopNetwork(const std::vector<HcclIpAddress>& whitelist, HcclIpAddress& hostIP, u32 hostPort)
{
    std::unique_lock<std::mutex> lock(lock_);
...
HcclResult TopoInfoExchangeServer::RecvRemoteRankNum(std::shared_ptr<HcclSocket> socket, u32& remoteRankNum)
{
    ...
    CHK_PRT_RET(
        (remoteRankNum == 0),
        HCCL_ERROR(
            "[Recv][RemoteRankNum]GetRemoteRankNum receive rank num "
    ...
    const std::map<std::string, std::shared_ptr<HcclSocket>>& connectSockets, RankTable_t& rankTable)
{
    ...
    for (auto& handle : connectSockets) {
        ...
        CHK_PRT_RET(
            ...
            HCCL_ERROR(
                "[Get][RanksTransInfo]received rank list "
                "is empty."),
            HCCL_E_INTERNAL);
...
HcclResult TopoInfoExchangeServer::GetRankBasicInfo(std::shared_ptr<HcclSocket> socket, RankTable_t& rankTable)
{
    ...
    CHK_PRT_RET(
        tmpRankTable.rankList.size() == 0,
        HCCL_ERROR(
            "[Get][RankBasicInfo]received rank list is "
            "empty."),
        HCCL_E_INTERNAL);
    CHK_PRT_RET(
        tmpRankTable.serverList.size() == 0,
        HCCL_ERROR(
            "[Get][RankBasicInfo]received server list "
            "is empty."),
        HCCL_E_INTERNAL);
