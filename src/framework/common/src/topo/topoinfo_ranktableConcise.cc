...
HcclResult TopoinfoRanktableConcise::ParserClusterInfo(hccl::HcclCommParams &params, hccl::RankTable_t &rankTable)
{
    ...
    RPT_ENV_ERR(phyId != static_cast<u32>(devId), "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
        std::vector<std::string>(
            {"[" + std::to_string(phyId) + "]", "[devicePhyId]", std::to_string(static_cast<u32>(devId))}));
    CHK_PRT_RET(phyId != static_cast<u32>(devId),
        HCCL_ERROR("[%s][%s]ranktable config devId[%d],but local devId[%u]", LOG_KEYWORDS_INIT_GROUP.c_str(),
...
HcclResult TopoinfoRanktableConcise::GetServerList(const nlohmann::json &obj, RankTable_t &clusterInfo)
{
    ...
    if (ret == HCCL_SUCCESS) {
        ...
        if (serverList.size() != clusterInfo.serverNum) {
            RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
                std::vector<std::string>(
                    {"[" + serverCount + "]", "[server_count]", std::to_string(serverList.size())}));
    ...
    if (clusterInfo.serverNum == 0) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
            std::vector<std::string>(
                {"[" + std::to_string(clusterInfo.serverNum) + "]", "[server_count]", "is a positive integer"}));
    ...
    const nlohmann::json &serverListObj, u32 objIndex, RankTable_t &clusterInfo)
{
    ...
    if (serverId.empty()) {
        HCCL_ERROR("[Get][JsonArrayMemberProperty]errNo[0x%016llx] serverId[%s] is empty", HCOM_ERROR_CODE(HCCL_E_PARA),
            serverId.c_str());
        return HCCL_E_PARA;
    }
    ...
    RankTable_t &clusterInfo, std::string &serverId, u32 &serverIdx, HcclIpAddress &hostIp)
{
    ...
    if (version.compare(SUPERPOD_CLUSTER_VERSION) != 0) {
        ...
        HCCL_DEBUG("[%s.json]->rankId[%u], serverId[%s], devicePhyId[%d]", fileName_.c_str(), rankinfo.rankId,
    ...
    const nlohmann::json &deviceListObj, u32 objIndex, RankInfo_t &rankinfo)
{
    ...
    if (backupDeviceIp != "") {
        ...
        CHK_PRT_RET(strBackupDeviceIp.size() == 0,
            HCCL_ERROR("[Get][SingleBackupDeviceIp]in device: deviceip size is zero"), HCCL_E_PARA);
    ...
    if (ret != HCCL_E_NOT_FOUND) {
        ...
        CHK_PRT_RET(rankinfo.hostPort == HCCL_SOCKET_PORT_RANGE_AUTO,
            HCCL_ERROR("[Get][SingleDeviceHostPort] deviceIndex[%u], please do not use the reserved port number[%u]",
                objIndex, HCCL_SOCKET_PORT_RANGE_AUTO),
            HCCL_E_PARA);
        CHK_PRT_RET(rankinfo.hostPort > MAX_PORT_NUMBER,
            HCCL_ERROR("[Get][SingleDeviceHostPort] deviceIndex[%u], port number[%u] exceed max port number[%u]",
                objIndex, rankinfo.hostPort, MAX_PORT_NUMBER),
            HCCL_E_PARA);
    ...
    const nlohmann::json &deviceListObj, u32 objIndex, RankTable_t &clusterInfo, RankInfo_t &rankinfo)
{
    ...
    CHK_PRT_RET(ret == HCCL_E_NOT_FOUND, HCCL_WARNING("[Get][SingleSuperDeviceId]'super_device_id' is not found"),
        HCCL_SUCCESS);
    ...
    if (ret != HCCL_SUCCESS) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
            std::vector<std::string>(
                {"[" + strSuperDeviceId + "]", "[super_device_id]", "is a valid non-negative integer"}));
        HCCL_ERROR("[%s][%s]errNo[0x%016llx] super_device_id[%s] is invalid", LOG_KEYWORDS_INIT_GROUP.c_str(),
    ...
    const nlohmann::json &superPodList, u32 objIndex, RankTable_t &clusterInfo)
{
    ...
    if (ret != HCCL_SUCCESS || superPodId.empty()) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
            std::vector<std::string>({"[" + superPodId + "]", "[super_pod_id]", "is a valid non-empty string."}));
    ...
    const nlohmann::json &superPodServerList, u32 objIndex, RankTable_t &clusterInfo, std::string superPodId)
{
    ...
    if (ret != HCCL_SUCCESS) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
            std::vector<std::string>({"[" + serverId + "]", "[server_id]", "is a valid non-empty string."}));
        return ret;
    }
    ...
    HCCL_DEBUG("[%s.json]super_pod_list -> server_id[%s], super_pod_id[%s]", fileName_.c_str(), serverId.c_str(),
        superPodId.c_str());
