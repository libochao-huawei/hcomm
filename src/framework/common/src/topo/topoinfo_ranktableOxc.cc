...
HcclResult ParseU32FromJsonValue(const nlohmann::json &value, const char *propName, u32 &result)
{
    if (value.is_number_unsigned()) {
        ...
        CHK_PRT_RET(tmp > std::numeric_limits<u32>::max(),
            HCCL_ERROR("[Parse][%s] value[%llu] exceeds u32 range.", propName, tmp), HCCL_E_PARA);
    ...
    if (value.is_number_integer()) {
        ...
        CHK_PRT_RET(tmp < 0 || tmp > static_cast<s64>(std::numeric_limits<u32>::max()),
            HCCL_ERROR("[Parse][%s] value[%lld] exceeds u32 range.", propName, tmp), HCCL_E_PARA);
        result = static_cast<u32>(tmp);
        return HCCL_SUCCESS;
    }
    ...
    HCCL_ERROR("[Parse][%s] value type is invalid.", propName);
    return HCCL_E_PARA;
}
...
{
    auto iter = obj.find(propName);
    CHK_PRT_RET(iter == obj.end(), HCCL_ERROR("[Parse][%s] field is missing.", propName), HCCL_E_NOT_FOUND);
...
HcclResult GetRequiredStringField(const nlohmann::json &obj, const char *propName, std::string &value)
{
    auto iter = obj.find(propName);
    CHK_PRT_RET(iter == obj.end(), HCCL_ERROR("[Parse][%s] field is missing.", propName), HCCL_E_NOT_FOUND);
    CHK_PRT_RET(!iter->is_string(), HCCL_ERROR("[Parse][%s] field is not string.", propName), HCCL_E_PARA);
    value = *iter;
    return HCCL_SUCCESS;
}
...
HcclResult TopoinfoRanktableOxc::ParserClusterInfo(HcclCommParams &params, RankTable_t &rankTable)
{
    ...
    CHK_PRT_RET(IsAllDigit(identify_.c_str()) != HCCL_SUCCESS ||
        ...
        HCCL_ERROR("[ParserClusterInfo] rank_id[%s] is invalid.", identify_.c_str()), HCCL_E_PARA);
    CHK_PRT_RET(rankId >= rankTable.rankList.size(),
        HCCL_ERROR("[ParserClusterInfo] rank_id[%u] exceeds rank size[%zu].", rankId, rankTable.rankList.size()),
        HCCL_E_PARA);
    ...
    CHK_PRT_RET(devicePhyId != static_cast<u32>(rankTable.rankList[rankId].deviceInfo.devicePhyId),
        HCCL_ERROR("[ParserClusterInfo] ranktable devId[%d] mismatch local devId[%u].",
...
HcclResult TopoinfoRanktableOxc::ParseTaskId(RankTable_t &rankTable)
{
    ...
    CHK_PRT_RET(!iter->is_string(), HCCL_ERROR("[Parse][task_id] field is not string."), HCCL_E_PARA);
...
HcclResult TopoinfoRanktableOxc::ParseRankList(RankTable_t &rankTable)
{
    ...
    CHK_PRT_RET(rankListIter == fileContent_.end(), HCCL_ERROR("[Parse][rank_list] field is missing."),
        HCCL_E_NOT_FOUND);
    CHK_PRT_RET(!rankListIter->is_array(), HCCL_ERROR("[Parse][rank_list] field is not array."), HCCL_E_PARA);
    ...
    for (const auto &rankObj : *rankListIter) {
        CHK_PRT_RET(!rankObj.is_object(), HCCL_ERROR("[Parse][rank_list] rank item is not object."), HCCL_E_PARA);
        RankInfo_t rankInfo;
    ...
    CHK_PRT_RET(rankTable.rankList.size() != rankCount,
        HCCL_ERROR("[Parse][rank_count] expect[%u] actual[%zu] mismatch.", rankCount, rankTable.rankList.size()),
        HCCL_E_PARA);
...
HcclResult TopoinfoRanktableOxc::ParseLevelList(const nlohmann::json &rankObj, RankInfo_t &rankInfo)
{
    ...
    CHK_PRT_RET(iter == rankObj.end(), HCCL_ERROR("[Parse][level_list] field is missing."), HCCL_E_NOT_FOUND);
    CHK_PRT_RET(!iter->is_array() || iter->empty(), HCCL_ERROR("[Parse][level_list] field is invalid."),
        HCCL_E_PARA);
    ...
    for (const auto &levelObj : *iter) {
        CHK_PRT_RET(!levelObj.is_object(), HCCL_ERROR("[Parse][level_list] level item is not object."), HCCL_E_PARA);
        ...
        if (!GetStringField(levelObj, PROP_NET_INSTANCE_ID.c_str(), levelInfo.netInstanceId, true) &&
            ...
            HCCL_ERROR("[Parse][net_instance_id] field is missing.");
            return HCCL_E_NOT_FOUND;
        }
        ...
        if (!GetStringField(levelObj, PROP_RANK_ADDR_TYPE, defaultAddrType, true) &&
            ...
            HCCL_ERROR("[Parse][rank_addr_type] field is invalid.");
            return HCCL_E_PARA;
        }
    ...
    const std::string &defaultAddrType)
{
    ...
    CHK_PRT_RET(addrIter == levelObj.end(), HCCL_ERROR("[Parse][rank_addr_list] field is missing."),
        HCCL_E_NOT_FOUND);
    CHK_PRT_RET(!addrIter->is_array(), HCCL_ERROR("[Parse][rank_addr_list] field is not array."), HCCL_E_PARA);

    levelInfo.rankAddrList.clear();
    levelInfo.rankAddrList.reserve(addrIter->size());
    for (const auto &addrObj : *addrIter) {
        CHK_PRT_RET(!addrObj.is_object(), HCCL_ERROR("[Parse][rank_addr_list] addr item is not object."),
            HCCL_E_PARA);
        ...
        if (!GetStringField(addrObj, PROP_ADDR_TYPE.c_str(), addrInfo.addrType, true)) {
            CHK_PRT_RET(defaultAddrType.empty(), HCCL_ERROR("[Parse][addr_type] field is missing."), HCCL_E_NOT_FOUND);
        ...
        CHK_PRT_RET(portsIter == addrObj.end(), HCCL_ERROR("[Parse][ports] field is missing."), HCCL_E_NOT_FOUND);
        CHK_PRT_RET(!portsIter->is_array(), HCCL_ERROR("[Parse][ports] field is not array."), HCCL_E_PARA);
        for (const auto &portItem : *portsIter) {
            CHK_PRT_RET(!portItem.is_string(), HCCL_ERROR("[Parse][ports] port item is not string."), HCCL_E_PARA);
