...
HcclResult TopoInfoExchangeAgent::Setup()
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[TopoInfoExchangeAgent][Setup]TopoExchangeAgent: "
                   "connect server[%s : %u] failed",
            serverIP_.GetReadableAddress(), serverPort_),
        ret);
    HCCL_INFO("TopoExchangeAgent: client connect with server ip[%s] port[%u] success.", serverIP_.GetReadableAddress(),
        serverPort_);
    ...
    if (!isByMasterInfo_ && localRankInfo_.rankSize > TOPO_HIERARCHICAL_ENABLE_THRESHOLD) {
        ...
        if (ret != HCCL_SUCCESS) {
            ...
            HCCL_ERROR("[TopoInfoExchangeAgent][Setup]VerifyCluseterInfo failed, g_broadcastStage[%d]",
...
HcclResult TopoInfoExchangeAgent::SetupMember()
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[TopoInfoExchangeAgent][Setup]SetupGroupMember: "
                   "connect server[%s : %u] failed",
            serverIP_.GetReadableAddress(), serverPort_),
        ret);
    HCCL_INFO("SetupGroupMember: client connect with server ip[%s] port[%u] success.", serverIP_.GetReadableAddress(),
        serverPort_);
...
HcclResult TopoInfoExchangeAgent::SetSuperPodIdx(RankTable_t &clusterInfo) const
{
    ...
    for (u32 i = 0; i < clusterInfo.rankList.size(); ++i) {
        ...
        HCCL_INFO("SetSuperPodIdx rankList[%u]: rankId[%u], superPodId[%s], superPodIdx[%u], sdid[%u]", i,
            rankInfo.rankId, rankInfo.superPodId.c_str(), rankInfo.superPodIdx, rankInfo.superDeviceId);
    }
    return HCCL_SUCCESS;
}
...
HcclResult TopoInfoExchangeAgent::ConnectWithRetry(
    HcclIpAddress &serverIp, u32 port, std::shared_ptr<HcclSocket> &socket)
{
    ...
    while (retryTime <= AGENT_MAX_RETRY_TIME) {
        std::string tag = TOPO_DETECT_TAG + "_" + identifier_ + "_" + std::to_string(port);
        EXECEPTION_CATCH((socket = std::make_shared<HcclSocket>(
                              tag, netDevCtx_, serverIp, port, HcclSocketRole::SOCKET_ROLE_CLIENT)),
            return HCCL_E_PTR);
        CHK_SMART_PTR_NULL(socket);
        CHK_RET(socket->Init());
        CHK_RET(socket->Connect());
    ...
    HcclIpAddress &serverIp, u32 port, std::shared_ptr<HcclSocket> &socket)
{
    ...
    HCCL_ERROR("1. local host ip is [%s], server host ip and port is [%s:%u], Please check the network connectivity. "
               "If it is not connected, modify the network configuration or use HCCL_SOCKET_IFNAME and "
               "HCCL_IF_BASE_PORT to specify ifname and server port.",
    ...
    while (true) {
        ...
        if (status == HcclSocketStatus::SOCKET_CONNECTING) {
            ...
            CHK_PRT_RET(ret != HCCL_SUCCESS,
                ...
                ret);
            HCCL_INFO("local rank[%u] get socket connection with server[%s] port[%u] success.", localRankInfo_.rank,
...
HcclResult TopoInfoExchangeAgent::SendGroupLeaderPortInfo(
    std::shared_ptr<HcclSocket> socket, HcclRankHandle &rankHandle)
{
    CHK_RET(GetConnection(socket));
    HcclResult ret = socket->Send(&rankHandle, sizeof(rankHandle));
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[TopoInfoExchangeAgent][SendGroupLeaderPortInfo]errNo[0x%016llx] "
...
{
    CHK_PRT_RET(clusterInfo.rankList.size() <= localRankInfo_.rank,
        HCCL_ERROR("[Set][TransportInfo]rank list is "
...
HcclResult TopoInfoExchangeAgent::VerifyClusterInfo(RankTable_t &clusterInfo)
{
    ...
    for (auto &server : serverMap) {
        CHK_PRT_RET((server.second.size() == 0),
            HCCL_ERROR("[%s][%s]server ip[%s] has %u device.", LOG_KEYWORDS_INIT_GROUP.c_str(),
                LOG_KEYWORDS_RANKTABLE_DETECT.c_str(), server.first.c_str(), server.second.size()),
            HCCL_E_PARA);

        if (deviceNumInServer != 0) {
            HCCL_WARNING("[%s][%s]server ip[%s] has %u devices, other server has %u.", LOG_KEYWORDS_INIT_GROUP.c_str(),
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[%s][%s]server id[%s] verify device physic id failed.", LOG_KEYWORDS_INIT_GROUP.c_str(),
    ...
    for (u32 i = 0; i < (clusterInfo.rankList.size() - 1); i++) {
        for (u32 j = (i + 1); j < clusterInfo.rankList.size(); j++) {
            ...
            if (isErr) {
                ...
                HCCL_ERROR("[%s][%s]%s", LOG_KEYWORDS_INIT_GROUP.c_str(), LOG_KEYWORDS_RANKTABLE_DETECT.c_str(),
                    errormessage.c_str());
                return HCCL_E_PARA;
            }
        }
        ...
        for (auto &backupDevIp : rankInfo.deviceInfo.backupDeviceIp) {
            ...
            if (linkType != LinkTypeInServer::SIO_TYPE) {
                RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
                    std::vector<std::string>({std::to_string(backupDevPhyId),
                        " \"backup_device_ip of "
                        "rank "
...
HcclResult TopoInfoExchangeAgent::VerifyClusterRankID(const RankTable_t &clusterInfo) const
{
    ...
    for (u32 i = 0; i < (clusterInfo.rankList.size() - 1); i++) {
        for (u32 j = (i + 1); j < clusterInfo.rankList.size(); j++) {
            ...
            if (isErr) {
                std::string errormessage = "Rank ID" + std::to_string(clusterInfo.rankList[i].rankId) + " of device ID "
                                           + std::to_string(clusterInfo.rankList[i].deviceInfo.devicePhyId)
                                           + " on node " + clusterInfo.rankList[i].serverId
                                           + " is the same as that of device ID "
                                           + std::to_string(clusterInfo.rankList[j].deviceInfo.devicePhyId)
...
HcclResult TopoInfoExchangeAgent::VerifyClusterSuperPodInfo(const std::vector<RankInfo_t> &rankInfo) const
{
    ...
    bool useSuperPodMode = false;
    CHK_RET(IsSuperPodMode(useSuperPodMode));
    CHK_PRT_RET(useSuperPodMode == false, HCCL_DEBUG("[Verify][SuperPodInfo] does not need verify superPod info"),
        HCCL_SUCCESS);
    ...
    for (u32 i = 0; i < rankInfo.size(); i++) {
        ...
        if ((rankInfo[i].superPodId.empty() || rankInfo[i].superDeviceId == INVALID_UINT)
            ...
            HCCL_ERROR("[%s][%s]%s", LOG_KEYWORDS_INIT_GROUP.c_str(), LOG_KEYWORDS_RANKTABLE_CHECK.c_str(),
                errormessage.c_str());
            return HCCL_E_PARA;
        }
        ...
        if (it == superPodSdidMap.end()) {
            ...
            if (it->second.find(rankInfo[i].superDeviceId) != it->second.end()) {
                RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
                    std::vector<std::string>({std::to_string(rankInfo[i].superDeviceId),
...
void TopoInfoExchangeAgent::ReportTlsConfigurationError(const std::string &tlsInconsistentTlsType,
    const std::string &tlsInconsistentEnableStr, const std::string &tlsInconsistentDisableStr,
    const std::string &tlsUnknownRankStr)
{
    std::string expectStr = StringFormat("All ranks are consistent. Current status: rankList for enabled tls: %s; "
                                         "rankList for disabled tls: %s rankList for query failure tls: %s.",
        tlsInconsistentEnableStr.c_str(), tlsInconsistentDisableStr.c_str(), tlsUnknownRankStr.c_str());
    std::string errormessage = StringFormat(
        "Value %s for config \"tls\" is invalid. Expected: \"%s\"", tlsInconsistentTlsType.c_str(), expectStr.c_str());
    RPT_INPUT_ERR(true, "EI0016", std::vector<std::string>({"value", "variable", "expect"}),
        std::vector<std::string>({tlsInconsistentTlsType, " \"tls\" ", " \"" + expectStr + "\" "}));
    HCCL_ERROR(
