    ...
    const vector<HcclIpAddress>& whitelist, const HcclIpAddress& hostIP, u32& bindPort)
{
    ...
    if (bindPort == HCCL_INVALID_PORT) {
        ...
        CHK_SMART_PTR_NULL(listenSocket_);
        CHK_RET(listenSocket_->Init());
        HcclResult ret = PreemptPortManager::GetInstance(deviceLogicID_)
                             .ListenPreempt(listenSocket_, GetExternalInputHostSocketPortRange(), bindPort);
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[TopoInfoDetect][StartRootNetwork] devPhyId[%u], devLogicId[%u], host ip[%s], "
...
HcclResult TopoInfoDetect::GetDeviceBackupNicInfo(HcclBasicRankInfo& localRankInfo)
{
    ...
    if (chipDeviceIPs.size() != 2U) {
        ...
        HCCL_RUN_WARNING(
            "[TopoInfoDetect][GetDeviceBackupNicInfo]Fail to load backup device ip!"
...
HcclResult TopoInfoDetect::GetLocalRankInfo(HcclBasicRankInfo& rankInfo)
{
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[Get][LocalRankInfo]GetLocalRankInfo failed, topo "
