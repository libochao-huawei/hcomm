...
HcclResult TopoInfoRanktableParser::ReadFile(const std::string &readFile)
{
    ...
    if (!infile) {
        RPT_INPUT_ERR(true,
                ...
                std::vector<std::string>({readFile.c_str(), RANKTABLE_PATH_NOT_EXIST}));
        ...
        try {
            ...
            RPT_INPUT_ERR(true,
                "EI0004",
                std::vector<std::string>({"ranktable_path", "error_reason"}),
                std::vector<std::string>({readFile.c_str(), RANKTABLE_JSON_FORMAT_ERROR}));
...
HcclResult TopoInfoRanktableParser::LoadString(const std::string &string)
{
    ...
    if (string.empty()) {
        ...
        "EI0004",
        std::vector<std::string>({"ranktable_path", "error_reason"}),
        std::vector<std::string>({string, RANKTABLE_CONTENT_ERROR}));
    ...
    RPT_INPUT_ERR(ret != HCCL_SUCCESS,
        "EI0004",
        std::vector<std::string>({"ranktable_path", "error_reason"}),
        std::vector<std::string>({string, RANKTABLE_JSON_FORMAT_ERROR}));
    CHK_PRT_RET(ret != HCCL_SUCCESS,
...
HcclResult TopoInfoRanktableParser::LoadFileInit(std::string &rankTableM)
{
    ...
    RPT_INPUT_ERR(ret != HCCL_SUCCESS,
        "EI0004",
        std::vector<std::string>({"ranktable_path", "error_reason"}),
        std::vector<std::string>({rankTableFile_, RANKTABLE_CONTENT_ERROR}));
    CHK_PRT_RET(ret != HCCL_SUCCESS,
