...
HcclResult ParseDFSConfig()
{
    ...
    if (!topoDetectThreshold.empty()) {
        ...
        HCCL_RUN_INFO("TOPO_HIERARCHICAL_THRESHOLD set by environment to [%llu]", g_envConfig.topoHierarchicalThreshold);
    } else {
        HCCL_RUN_WARNING("[ParseDFSConfig] HCCL_DFS_CONFIG-topo_hierarchical_threshold was configed to [%llu], please configed to"\
