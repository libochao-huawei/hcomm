...
HcclResult hcclH2dTlv::InitHccpChannel(u32 devicePhyId)
{
    ...
    nslb_hccp.phyId = phyID;
    nslb_hccp.nic_posion = NSLBDP_HCCP_NICPOSION;

    u32 tlvBuffersize = 0;
    void *tlvHandle;
    HCCL_INFO("H2DTlvInit HCCL InitHccpChannel version:[%u]-phy_id:[%u]-nic_posion:[%u] .",
        nslb_hccp.version, nslb_hccp.phyId, nslb_hccp.nic_posion);

    HcclResult ret = H2DTlvInit(reinterpret_cast<TlvInitInfo *>(&nslb_hccp), &tlvBuffersize, &tlvHandle);
    if (ret != HCCL_SUCCESS) {
        return ret;
    }
    if (tlvBuffersize == 0) {
        HCCL_INFO("HCCL could not InitHccp nslb_hccp .tlvBuffersize:[%u].", tlvBuffersize);
...
void hcclH2dTlv::DeinitHccpChannel()
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("HCCL ndlbdp InitHccp is failed.");
        return;
    }
    hcclH2dTlvBuffsize_ = 0;
