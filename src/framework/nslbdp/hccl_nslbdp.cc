...
HcclResult hcclNslbDp::SendRankTable(NslbDpCommConfigInfo tab_f)
{
	...
	HCCL_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_TBL_COMM_INFO to hccp. ret(%d)\n", ret);
    return HCCL_SUCCESS;
}
...
HcclResult hcclNslbDp::SendRankTableOpAndAdj(NslbDpOperatorInfo &tab_f)
{
	...
	HCCL_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_TBL_OPER to hccp. ret(%d)\n", ret);
...
HcclResult hcclNslbDp::SendRankTableAlgorithmInfo(NslbDpAlgorithmTlv &tab_f)
{
    ...
    HCCL_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_TBL_ADJ to hccp. ret(%d)\n", ret);
    return HCCL_SUCCESS;
}
...
HcclResult hcclNslbDp::SendRankTableGlobalRank(NslbDpGlobalRankInfo &tab_f)
{
    ...
    HCCL_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_TBL_RANK to hccp. ret(%d)\n", ret);
...
HcclResult hcclNslbDp::SendRankTableGlobalDisRankVal(NslbDpGlobalDisRankVal &tab_f)
{
    ...
    HCCL_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_TBL_RANK_DIST to hccp. ret(%d)\n", ret);
    return HCCL_SUCCESS;
}
...
HcclResult hcclNslbDp::SendRankTableRootRank(NslbDpRootRank &tab_f)
{
    ...
    HCCL_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_TBL_ROOT_RANK to hccp. ret(%d)\n", ret);
    return HCCL_SUCCESS;
}
...
HcclResult hcclNslbDp::InitNetCo()
{
    if (hcclH2dTlv::GetInstance().GetH2dTlvInitFlag() != true) {
        HCCL_INFO("hcclNslbDp GetH2dTlvInitFlag not init");
        return HCCL_SUCCESS;
    }
    /* 避免二次初始化 */
    ...
    if (hcclH2dTlv::GetInstance().GetH2dTlvBufferSize() == 0) {
        /* 异常场景处理 */
        HCCL_INFO("HCCL could not InitHccp nslb_hccp .nslb_buffersize:0.");
        ClearInitNetCoFlag();
    ...
    u32 nslb_buffersize = hcclH2dTlv::GetInstance().GetH2dTlvBufferSize();
    void *tlv_handle = hcclH2dTlv::GetInstance().GetH2dTlvHandle();
    nslb_msg sendMsg;
    nslb_msg recvMsg;
    ...
    tlvData.push_back(0);
    sendMsg.data.assign(tlvData.begin(), tlvData.end());
    s32 ret = H2DTlvRequest(tlv_handle, MODULE_TYPE_NSLB,
        reinterpret_cast<TlvMsg*>(&sendMsg), reinterpret_cast<TlvMsg*>(&recvMsg));
    HCCL_RUN_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_INIT_NETCO to hccp. ret(%d)\n", ret);
    /* 数据转存 */
    SetH2DTlvInitInfo(nslb_buffersize, tlv_handle);
...
void hcclNslbDp::DeinitNetCo()
{
    ...
    s32 ret = H2DTlvRequest(nslbdp_handle_, MODULE_TYPE_NSLB,
        reinterpret_cast<TlvMsg*>(&sendMsg), reinterpret_cast<TlvMsg*>(&recvMsg));
    HCCL_RUN_INFO("[NSLBDP-SENDTABLE] hccl send Table NSLBDP_TYPE_DEINIT_NETCO to hccp. ret(%d)\n", ret);
    ClearInitNetCoFlag();
    return;
}
