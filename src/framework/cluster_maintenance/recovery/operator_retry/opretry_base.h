...
public:
    ...
    uint32_t partialOpRetryWaitCnt = 0; // 当前errorOp局部重执行等待慢卡的次数
    bool errorOpPartialOpRetryFlag = false; // 当前errorOp局部重执行是否使能的flag (要求errorOp同步卡全部使能局部重执行)
    uint32_t partialOpRetryErrorRank = INVALID_UINT; // 上报当前errorOp且rankId最小的卡为故障卡
