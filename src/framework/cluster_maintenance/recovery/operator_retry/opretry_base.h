...
private:
    ...
    HcclResult CheckOpName(RetryContext &retryCtx, const uint32_t agent1, const RetryInfo &retryinfo1,
        const uint32_t agent2, const RetryInfo &retryinfo2); // 校验算子一致
...
public:
    ...
    //     OpRetryWaitResp: 如果使能局部重执行, 收到故障/非故障卡的KfcStatus::kStoplaunch时, 如果是同步卡, 则更新flag
