...
HcclResult OpRetryAgentWaitCmd::ProcessEvent(RetryContext* retryCtx)
{
    ...
    while (true) {
        ...
        if (ret == HCCL_SUCCESS) {
            ...
            CHK_RET(ParseCommandWithOpId(retryCtx, commandinfo, nextState));
    ...
    RetryState &nextState)
{
    ...
    switch (commandinfo.command) {
        ...
        case RETRY_CMD_CAN_PARTIAL_RETRY:
            ...
            if (curState == RETRY_STATE_WAIT_CMD_CAN_RETRY) {
                ...
                if (UNLIKELY(retryCtx->recvCmdCanPartialRetry_)) {
                    HCCL_RUN_INFO("[OpRetry][Agent] curState[%s] recv RETRY_CMD_CAN_PARTIAL_RETRY again! "
                        "keep nextState[%s] to wait for RETRY_CMD_CAN_RETRY",
                        GetReadableState(curState), GetReadableState(nextState));
                    break;
                }

                // 避免重复处理RETRY_CMD_CAN_PARTIAL_RETRY
                retryCtx->recvCmdCanPartialRetry_ = true;
                HCCL_RUN_INFO("[OpRetry][Agent] recv RETRY_CMD_CAN_PARTIAL_RETRY");
        ...
        case RETRY_CMD_CAN_RETRY:
            if (curState == RETRY_STATE_WAIT_CMD_CAN_RETRY) {
                // 校验局部重执行信息
                HCCL_RUN_INFO("[OpRetry][Agent] recv RETRY_CMD_CAN_RETRY; recvCmdCanPartialRetry_[%d]",
                    retryCtx->recvCmdCanPartialRetry_);
                if (retryCtx->recvCmdCanPartialRetry_) { // 接收过RETRY_CMD_CAN_PARTIAL_RETRY命令, 即使能局部重执行
                    // 局部重执行信息的使能flag一定为true
                    CHK_PRT_RET(!retryCtx->partialRetryInfo_.partialOpRetryFlag,
                        HCCL_ERROR("[OpRetry][Agent] partialOpRetryFlag[%d] is not enabled",
                            retryCtx->partialRetryInfo_.partialOpRetryFlag),
                        HCCL_E_INTERNAL);
                    
                    // 注意: 如果使能局部重执行, 相关信息已经在OpRetryAgentWaitPartialRetryInfo中通过H2D下发aicpu kfc thread
                    // 因此这里无需再次下发
                } else { // 正常重执行
