        ...
        if (!isFoundSendRecv) {
            ...
            if (!retryCtx->partialOpRetryFlag_ || !isWait) {
                if (retryCtx->partialOpRetryFlag_) { // 使能局部重执行
                    ...
                    std::map<u32, HcclAgentRetryInfo>::const_iterator tmpIter =
                        retryCtx->serverSockets_.find(retryCtx->partialOpRetryFirstErrorAgent_);
                    CHK_PRT_RET(tmpIter == retryCtx->serverSockets_.end(),
                    ...
                    const std::string curTag = std::string(reinterpret_cast<const char*>(
                        tmpIter->second.retryInfo.opInfo.opId.tag));
                    HCCL_RUN_INFO("[OpRetry][Server] begin to exec partial retry of op tag[%s]/index[%u] "
                        ...
                        retryCtx->partialOpRetryFirstErrorAgent_, tmpIter->second.retryInfo.rankId);
                    
                    // 更新errorRankList_, 只清理errorOp慢卡/同步卡
                    for (std::map<u32, HcclOpIdentifier>::const_iterator iter = retryCtx->errorRankList_.cbegin();
                         iter != retryCtx->errorRankList_.cend();) {
                        // 备份next iterator
                        std::map<u32, HcclOpIdentifier>::const_iterator nextIter = std::next(iter);

                        // 如果为errorOp慢卡/同步卡, 则从errorRankList_中移除
                        const uint32_t curOpIdx = iter->second.index;
                        if (curOpIdx <= retryCtx->partialOpRetryErrorOpIdx_) {
                            retryCtx->errorRankList_.erase(iter);
                        }
                ...
                if (dstState == RETRY_STATE_RESP_AICPU_STOPED && retryCtx->partialOpRetryFlag_) {
                    ...
                    CHK_PRT_RET(retryCtx->partialOpRetryFirstErrorAgent_ == INVALID_UINT ||
                        ...
                        HCCL_ERROR("[OpRetryServerWaitResp][ProcessEvent] "
                            ...
                            GetReadableState(dstState), rank, agentRetryInfo.retryInfo.rankId),
                        HCCL_E_INTERNAL);
                    
                    // TODO: 获取当前agent对应的opIdx
                    // 注意: opIdx一定 >= errorOpIdx (OpRetryHandleError时已经判断过不存在比errorOp还小的慢卡)

                    // TODO: 如果opIdx > errorOpIdx, 更新partialOpRetryErrorAgentFlagMap_ (isSameAsErrorOp = false)

                    // TODO: 如果opIdx == errorOpIdx, 更新partialOpRetryErrorAgentFlagMap_ (isSameAsErrorOp = true)
                    //     同时, 根据isEnablePartialOpRetry更新partialOpRetryFlag_
                }
