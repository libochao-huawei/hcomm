...
HcclResult OpRetryServerIssueCmd::ProcessEvent(RetryContext* retryCtx)
{
    ...
    HCCL_INFO("[OpRetry][Server]OpRetryServerIssueCmd curState[%s], command[%s], nextState[%s]",
        GetReadableState(curState), GetReadableCmd(command), GetReadableState(nextState));
    ...
    while (recvVaild.size() < retryCtx->needRetryServerRanks_.size()) {
        ...
        for (auto rank : retryCtx->needRetryServerRanks_) {
            ...
            if (ret == HCCL_SUCCESS && dstState == expectagentState) { // 接收到对端信息且状态有效
                ...
                } else if (dstState == RETRY_STATE_WAIT_CAN_PARTIAL_RETRY ||
                    dstState == RETRY_STATE_WAIT_ISSUE_PARTIAL_RETRY_INFO) {
...
HcclResult OpRetryServerIssuePartialRetryInfo::ProcessEvent(RetryContext* retryCtx)
{
    ...
    for (auto agent : retryCtx->needRetryServerRanks_) {
        // 打印发送日志
        HCCL_INFO("[OpRetryServerIssuePartialRetryInfo][ProcessEvent] fastRankNum[%u] target agent[%u]/rank[%u]",
