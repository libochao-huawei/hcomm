...
HcclResult OpRetryServerIssueCmd::ProcessEvent(RetryContext* retryCtx)
{
    ...
    CHK_PRT_RET(nextState == RETRY_STATE_RESERVED,
        HCCL_ERROR("[OpRetryServerIssueCmd][ProcessEvent] invalid nextState[%d]", GetReadableState(nextState)),
        HCCL_E_INTERNAL);

    // 发送命令
    RetryCommand command = RETRY_CMD_RESERVED;
    auto itCommandPartial = RETRY_SERVER_STATE_TO_CMD_LABEL_PARTIAL.find(curState);
    if (retryCtx->partialOpRetryFlag_ && itCommandPartial != RETRY_SERVER_STATE_TO_CMD_LABEL_PARTIAL.end()) {
        // 局部重执行对应的命令
        command = itCommandPartial->second;
    } else {
        // 正常重执行对应的命令
    ...
    CHK_PRT_RET(command == RETRY_CMD_RESERVED,
        HCCL_ERROR("[OpRetryServerIssueCmd][ProcessEvent] invalid command[%d]", GetReadableCmd(command)),
        HCCL_E_INTERNAL);
...
HcclResult OpRetryServerWaitResp::ProcessEvent(RetryContext* retryCtx)
{
    ...
    // TODO: 处理curState = RETRY_STATE_WAIT_CAN_PARTIAL_RETRY的情况
