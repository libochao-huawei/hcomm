...
HcclResult OpRetryServerBase::ServerResetPartialOpRetryInfo(RetryContext* retryCtx)
{
    HCCL_INFO("[OpRetryServerBase][ServerResetPartialOpRetryInfo] reset for partial op retry");
    ...
    const HcclOpIdentifier &opId)
{
    ...
    HCCL_DEBUG("[OpRetry][Server]set needRetryServerRanks_.size[%u] partialRetryInfo.fastRankNum[%u] success",
...
HcclResult OpRetryServerWaitResp::ProcessEvent(RetryContext* retryCtx)
{
    ...
    // 因此, curState为RETRY_STATE_WAIT_ISSUE_PARTIAL_RETRY_INFO时, 不需要重置局部重执行信息, 统一在触发重执行后进行清理

    // 成功触发重执行, 进入RUNNING前, 重置局部重执行信息
    if (curState == RETRY_STATE_WAIT_CAN_RETRY) {
        // 后续状态一定回到重执行状态机的起始状态
        CHK_PRT_RET(expectNextState != RETRY_STATE_SERVER_RUNNING,
            HCCL_ERROR("[OpRetryServerWaitResp][ProcessEvent] expectNextState[%s] != RETRY_STATE_SERVER_RUNNING for "
                "curState[%s]", GetReadableState(expectNextState), GetReadableState(curState)),
            HCCL_E_INTERNAL);
