    ...
    if (UNLIKELY(nonFastRankCnt <= 1)) {
        ...
        for (std::map<u32, HcclAgentRetryInfo>::const_iterator constIter = retryCtx->serverSockets_.cbegin();
            ...
            HCCL_ERROR("[OpRetryServerHandleError][UpdatePartialOpRetryInfoForAllErrors] "
                ...
                constIter->second.retryInfo.execStatus.retryInfo.isEnablePartialOpRetry
            );
        }
...
HcclResult OpRetryServerHandleError::SetNeedRetryServerRank(RetryContext* retryCtx, constcclOpIdentifier &opId)
{
    if (opId.isSendRecv) {
        ...
        for (auto rank : retryCtx->needRetryServerRanks_) {
            ...
            if (ret == HCCL_SUCCESS && dstState == expectagentState) { // 接收到对端信息且状态有效
                ...
                if (dstState == RETRY_STATE_RESP_AICPU_STOPED && retryCtx->partialOpRetryFlag_) {
                    // 如果使能局部重执行, partialOpRetryFirstErrorRank_在OpRetryServerHandleError时一定初始化过
                    CHK_PRT_RET(retryCtx->partialOpRetryFirstErrorRank_ == INVALID_UINT,
                        HCCL_ERROR("[OpRetryServerWaitResp][ProcessEvent] "
                            "invalid partialOpRetryFirstErrorRank_[%u] when recv state[%s] from agent[%u] rank[%u]",
                            retryCtx->partialOpRetryFirstErrorRank_, GetReadableState(dstState), rank,
