    ...
    RetryContext* retryCtx, const u32 rankId, HcclAgentRetryInfo& agentInfo)
{
    ...
    while (true) {
        ...
        CHK_PRT_RET(
            elapsed > timeout,
            HCCL_ERROR("[SwitchNic][Server] timeout in recv agent RetryInfo, waitime[%u>%u]", elapsed, timeout),
            HCCL_E_TIMEOUT);
...
HcclResult ResumeServerCheckAllLink::CheckAllLink(RetryContext* retryCtx, RetryState& nextState)
{
    ...
    for (auto it : retryCtx->serverSockets_) {
        ...
        for (u32 i = 0; i < linkPortStatus.rankSize; i++) {
            ...
            HCCL_RUN_INFO(
                "[OpRetry][Server][Resume]CheckAllLink remoteRank[%d], changeLinkInfo remoteRankList[%d], "
                "linkPortStatus rankList[%d]",
