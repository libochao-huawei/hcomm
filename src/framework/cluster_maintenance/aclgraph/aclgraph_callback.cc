...
HcclResult AclgraphCallback::CleanCaptureRes(u64 modelId)
{
    ...
    for (auto commIt : modelIt->second) {
        for (auto newTag : commIt.second) {
            ...
            HCCL_DEBUG("[%s] modelID[%llu] tag[%s] resource release finish", __func__, modelId, tag.c_str());
        }
    }
...
void AclgraphCallback::CleanCaptureRes(const HcclCommunicator *communicator)
{
    ...
    for (auto modelIt : captureResMap_) {
        if (modelIt.find(communicator) != modelIt.end()) {
            modelIt.erase(communicator);
        }
    }
...
HcclResult AclgraphCallback::InsertNewTagToCaptureResMap(const HcclCommunicator *communicator,
