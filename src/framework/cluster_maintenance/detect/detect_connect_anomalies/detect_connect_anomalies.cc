...
HcclResult DetectConnectionAnomalies::ProcessDetectionResults()
{
    ...
    if (recvErrorInfoMap_.size() >= 1) {
        ...
        errMsg = "This node (server "", device ID -1 ) detects no exception event. The possible cause is that the behaviors of different ranks are inconsistent. The possible causes are as follows:";
