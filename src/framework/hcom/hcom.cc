    ...
    const char* group, rtStream_t stream)
{
    ...
    CHK_PRT_RET(
        hcomInfo.pComm == nullptr,
        HCCL_ERROR(
            "[Reduce][Result]hcomInfo.pComm is null, "
    ...
    const char* group, rtStream_t stream)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS, HCCL_ERROR(
                                 ...
                                 HCOM_ERROR_CODE(ret), tag, inputPtr, outputPtr, count,
    ...
    HcclDataType dataType, HcclReduceOp op, const char* group, rtStream_t stream)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS, HCCL_ERROR(
                                 ...
                                 "error, tag[%s], input_ptr[%p], output_ptr[%p], count[%llu], data_type[%s], op[%s]",
                                 HCOM_ERROR_CODE(ret), tag, sendBuf, recvBuf, recvCount,
                                 GetDataTypeEnumStr(dataType).c_str(), GetReduceOpEnumStr(op).c_str());
        , ret);
...
HcclResult
HcclCommGraphGetAllReduceScratchSize(s64 opBaseHcom, const u32 count, const HcclDataType dataType, u64& outScratchSize)
{
    hccl::hcclComm* hcclComm = reinterpret_cast<hccl::hcclComm*>(opBaseHcom);
    CHK_PTR_NULL(hcclComm);
...
HcclResult HcomGetLocalRankSize(const char* group, u32* localRankSize)
{
    ...
    CHK_PRT_RET(
        hcomInfo.pComm == nullptr,
        HCCL_ERROR(
            "[Get][LocalRankSize]hcomInfo.pComm is null, "
            "please check if the initialize process is called."),
        HCCL_E_PTR);
...
HcclResult HcomGetLocalRankId(const char* group, u32* localRankId)
{
    ...
    CHK_PRT_RET(
        hcomInfo.pComm == nullptr,
        HCCL_ERROR(
            "[Get][LocalRankId]hcomInfo.pComm is null, "
            "please check if the initialize process is called."),
        HCCL_E_PTR);
...
HcclResult HcomAbortGroup(const char* group)
{
    ...
    CHK_PRT_RET(
        hcomInfo.pComm == nullptr,
        HCCL_ERROR(
            "[Destroy][Group]hcomInfo.pComm is null, "
            "please check if the initialize process is called."),
        HCCL_E_PTR);
...
HcclResult
HcomGetAllReduceScratchSize(const char* group, const u32 count, const HcclDataType dataType, u64& outScratchSize)
{
    std::shared_ptr<hccl::hcclComm> hcclComm;
    HcomInfo& hcomInfo = HcomGetCtxHomInfo();
    if (hcomInfo.pComm != nullptr) {
...
HcclResult HcomGetAlgorithm(u32 level, char** algo)
{
    ...
    CHK_PRT_RET(
        hcomInfo.pComm == nullptr,
        HCCL_ERROR(
            "[Get][Algorithm]hcomInfo.pComm is null, "
            "please check if the initialize process is called."),
        HCCL_E_PTR);
    AlgType algType;
...
HcclResult HcomGetBandWidthPerNPU(u32 level, float* bandWidth)
{
    ...
    CHK_PRT_RET(
        hcomInfo.pComm == nullptr,
        HCCL_ERROR(
            "[Get][BandWidth]hcomInfo.pComm is null, "
            "please check if the initialize process is called."),
        HCCL_E_PTR);
...
HcclResult HcomUnloadTask(const char* group, const char* tag)
{
    ...
    if (HcomGetCommByGroup(group, hcclComm) == HCCL_SUCCESS) {
        CHK_PRT_RET(
            hcclComm == nullptr,
            HCCL_WARNING(
                "[UnloadAllTask]hcclComm is null, "
                "please check if the initialize process is called."),
            HCCL_SUCCESS);
...
HcclResult HcomGetServerNumAndDeviceNumPerServer(u32* serverNum, u32* deviceNumPerServer, u32* deviceNumPerAggregation)
{
    ...
    CHK_PRT_RET(
        hcomInfo.pComm == nullptr,
        HCCL_ERROR(
            "[GetServerNumAndDeviceNumPerServer]hcomInfo.pComm is null, "
            "please check if the initialize process is called."),
        HCCL_E_INTERNAL);
    u32 totalDevNum;
...
HcclResult HcclCommGraphUnloadTask(s64 opBaseHcom, const char* tag)
{
    ...
    CHK_PRT_RET(
        hcclComm == nullptr,
        HCCL_WARNING(
            "[HcclCommGraphUnloadTask]hcclComm is null, "
            "please check if the initialize process is called."),
        HCCL_SUCCESS);
    CHK_RET(SetWorkflowMode(HcclWorkflowMode::HCCL_WORKFLOW_MODE_OPS_KERNEL_INFO_LIB));
...
HcclResult GetClusterInfoAndDeviceNum(const std::string rankTableString, RankTable_t& clusterInfo, s32& deviceNum)
{
    ...
    CHK_PRT_RET(
        deviceNum == 0, HCCL_ERROR("[GetStremNumOfflineByDev]cur device num per server is 0,\
