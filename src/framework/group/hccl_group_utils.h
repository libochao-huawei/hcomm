...
struct hcclOpInfo {/*用于保存算子的入参。所有算子用同个结构体保存info */
    ...
    hcclOpInfo() : coll(), sendbuff(nullptr), recvbuff(nullptr), sendCount(0), recvCount(0),
        sendCounts(nullptr), recvCounts(nullptr), sdispls(nullptr), rdispls(nullptr),
        sendType(), recvType(), op(), root(0), comm(nullptr), stream(nullptr) {}
};
