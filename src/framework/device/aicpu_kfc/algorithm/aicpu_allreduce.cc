    ...
    std::vector<u32>& ringOrder, HcclDataType dataType) const
{
    ...
    for (size_t rankOffset = 0; rankOffset < ringSize - 1; rankOffset++) {
        ...
        CHK_RET(
            TaskOrchestrator::IpcCpyWin2WinP2P(
        ...
        if (rankOffset < ringSize - 2) { // ringSize - 2： 最后一轮不进行本地搬运
            ...
            CHK_RET(
                TaskOrchestrator::SelfCpySnd2Win(
                    sendBuffer, localSlice->size, localSlice->offset, localSlice->offset, HCCL_REDUCE_RESERVED,
                    dataType));
        }
    ...
    for (size_t rankOffset = 0; rankOffset < ringSize - 1; rankOffset++) {
        ...
        if (rankOffset < ringSize - 2) { // < RingSize - 2: 不是最后一轮，搬到 window 上，让下游读
            CHK_RET(
                TaskOrchestrator::IpcCpyWin2WinP2P(
                    prevRank, remoteSlice->size, remoteSlice->offset, remoteSlice->offset, HCCL_REDUCE_RESERVED,
                    dataType));
        } else { // 最后一轮，dma 消减，直接从对端读入 rcv
            CHK_RET(
                TaskOrchestrator::IpcCpyWin2RcvP2P(
                    recvBuffer, prevRank, remoteSlice->size, remoteSlice->offset, remoteSlice->offset,
