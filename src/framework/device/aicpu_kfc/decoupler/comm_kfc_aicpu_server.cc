...
void FormatOpData(const HcclMsg &msg, const HcclMsgExt &extMsg, u32 rankNum, u32 repeat, HcclOpData &data)
{
    if (repeat == 0U) {
        ...
        for (u32 i = 0U; i < rankNum; ++i) {
            ...
            HCCL_INFO("Formatted alltoallv info: rank id %u, send offset %llu, recv offset %llu.", i,
                      static_cast<u64 *>(data.all2AllVDataDes.sendCounts)[i],
                      static_cast<u64 *>(data.all2AllVDataDes.recvCounts)[i]);
        }
    }
