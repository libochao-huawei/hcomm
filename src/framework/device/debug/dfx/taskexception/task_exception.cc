...
void TaskException::PrintFinishedTaskQue(u32 sqTail, SqeRingBuffer *sqeContextBuffer, u32 sqeNum)
{
    ...
    HCCL_RUN_INFO("%s start, will print last %u finished tasks from sqIdx:%u to sqIdx:%u",
        __func__, sqeNum, (sqTail - sqeNum + HCCL_SQE_MAX_CNT) % HCCL_SQE_MAX_CNT, lastSqIdx);
    ...
    for (u32 i = 0; i < sqeNum; i++) {
        ...
        if (newOpIdx != opIndex || i == sqeNum - 1) { // 不同一个算子，或已经到打印的最后一个位置
            PrintTaskExceptionOpInfo(lastOpInfo);
