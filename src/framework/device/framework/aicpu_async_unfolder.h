...
struct MainSlaveStreamInfo {
    ...
    uint64_t opBaseOpId = 0; // 异步展开后续算子前, 备份主从stream信息时, 对应的当前算子索引
...
public:
    ...
    HcclResult BackupStreamInfo(const uint64_t curOpBaseOpIdx, Stream &mainStream, std::vector<Stream> &slaveStreams);

    // ns快恢/重执行前, 恢复各stream异步展开备份的tailSqeTaskId和flipNum (if any)
    HcclResult RestoreStreamInfo(const uint64_t curOpBaseOpIdx, Stream &mainStream, std::vector<Stream> &slaveStreams);
private:
