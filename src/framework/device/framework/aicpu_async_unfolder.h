...
public:
    // 获取异步展开的key
    // static HcclResult GetAsyncUnfoldKey(const OpParam &param, AsyncUnfoldKey& asyncUnfoldKey);
public:
    AicpuAsyncUnfolder();
    ~AicpuAsyncUnfolder();
    ...
    HcclResult ClearAsyncUnfoldInfo(const uint64_t opBaseOpIdx, Stream &mainStream, std::vector<Stream> &slaveStreams,
        void* dispatcherPtr);
private:
    ...
    // ns快恢/重执行前, 清理异步展开信息
    HcclResult RestoreStreamInfo_(const uint64_t opBaseOpIdx, Stream &mainStream, std::vector<Stream> &slaveStreams);
