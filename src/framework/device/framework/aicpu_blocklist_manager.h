...
public:
    ...
    struct StreamPartialOpRetryInfo {
        ...
        void ResetAndBackup(const uint32_t sqTail);
    };
    ...
    // 正常展开条件下, 检查局部重执行约束
    HcclResult CheckConstraints(const size_t opUnfoldIdx, const std::string& algName, const OpParam &param,
        const HcclTopoInfo& topoinfo, const bool isDeviceMode);
    
    // 故障算子重执行条件下, 标记当前为重执行展开流程
    HcclResult MarkIsRetry(const size_t opUnfoldIdx, const std::string& algName, const OpParam &param);
    ...
    // Part 6: 正常算子展开/故障算子重执行结束后阶段 (所有SQE均正常执行完毕)

    // 重置与备份各stream的局部重执行相关信息
    // 注意: isEnablePartialOpRetry_可能一开始约束检查时为true, 但是后续展开时违法totalSqeCnt约束导致变成false,
    //     但此时AicpuBlocklistManager状态已经变更, 因此即使isEnablePartialOpRetry_为false也仍然需要重置
    // 注意: 即使当前算子不使能局部重执行 (即isEnablePartialOpRetry_为false), 但不确定下一算子是否使能,
    //     因此仍然需要备份rtsq tail
    HcclResult ResetAndBackupAtEnd(const size_t opUnfoldIdx, const std::string& algName, const OpParam &param,
        const uint32_t devId, Stream& mainStream, std::vector<Stream>& slaveStreams);
private:
    // Part 2: 当前算子展开前阶段
    ...
    // Part 6: 正常算子展开/故障算子重执行结束后阶段

    // 重置与备份给定stream的局部重执行相关信息
    HcclResult ResetAndBackupForStream_(const uint32_t devId, Stream& stream);
