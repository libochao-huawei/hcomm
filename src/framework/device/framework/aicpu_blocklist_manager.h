...
public:
    ...
    struct StreamPartialOpRetryInfo {
        ...
        uint64_t opRetryNonFlipSqeCount = 0; // 故障算子在给定stream已展开的非flip placeholder SQE的个数
    ...
    // Rule 1: 拷贝类SQE, 如果故障前已经执行过 (即opRetryNonFlipSqeCount < execNonFlipSqeCount), 则过滤; 否则正常下发;
