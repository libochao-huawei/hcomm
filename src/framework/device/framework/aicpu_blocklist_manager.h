...
public:
    ...
    HcclResult InitBlocklistManager(void *dispatcherPtr, Stream& mainStream, std::vector<Stream>& slaveStreams);

    // 当前算子展开前, 备份aicpu main/slave stream SQE信息 (故障后与各stream CQE信息比较可以推导各stream成功执行的SQE count)
    HcclResult BackupSqeCounts();

    // 当前算子展开过程中, 更新aicpu main/slave stream SQE信息 (用于下一算子展开前备份)
    HcclResult UpdateSqeCounts(const int32_t streamId, const uint64_t sqeCount);
private:
