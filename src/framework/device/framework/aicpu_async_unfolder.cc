...
namespace hccl {
    ...
    HcclResult AicpuAsyncUnfolder::GetOpAsyncUnfoldInfo(bool& isValid, OpAsyncUnfoldInfo& opAsyncUnfoldInfo) {
        ...
        // 校验tail
        CHK_PRT_RET(tail >= opH2DRingBuffer_.size(),
            HCCL_ERROR("[AicpuAsyncUnfolder][GetOpAsyncUnfoldInfo] tail[%llu] should < opH2DRingBuffer_.size[%u]",
                tail, opH2DRingBuffer_.size()),
            HCCL_E_INTERNAL);
        
        // 校验head
        CHK_PRT_RET(head_ >= opH2DRingBuffer_.size(),
            HCCL_ERROR("[AicpuAsyncUnfolder][GetOpAsyncUnfoldInfo] head_[%llu] should < opH2DRingBuffer_.size[%u]",
                head_, opH2DRingBuffer_.size()),
            HCCL_E_INTERNAL);
        
        HCCL_DEBUG("[AicpuAsyncUnfolder][GetOpAsyncUnfoldInfo] tail[%llu] head[%llu] opH2DRingBuffer_.size[%u]",
            tail, head_, opH2DRingBuffer_.size());
