...
namespace hccl {
    ...
    {
        // 判断exceedSqDepth_, 如果为true (即某条流下发的SQE count超过sqDepth), 则一定不会使用局部重执行, 没有必要再更新局部重执行相关信息
        if (exceedSqDepth_) {
            HCCL_INFO("[AicpuBlocklistManager][UpdateTotalSqeCount] exceedSqDepth_[%u]: no need to "
        ...
        const int64_t curFlipPlaceholderSqIdx)
    {
        // 判断exceedSqDepth_, 如果为true (即某条流下发的SQE count超过sqDepth), 则一定不会使用局部重执行, 没有必要再更新局部重执行相关信息
        if (exceedSqDepth_) {
            HCCL_INFO("[AicpuBlocklistManager][UpdateFlipPlaceholderSqIdx] exceedSqDepth_[%u]: no need to "
                "update flipPlaceholderSqIdx for streamId[%d] curFlipPlaceholderSqIdx[%lld]",
                exceedSqDepth_, streamId, curFlipPlaceholderSqIdx);
            return HCCL_SUCCESS;
        }
    ...
    {
        // 判断exceedSqDepth_, 如果为true (即某条流下发的SQE count超过sqDepth), 则一定不会使用局部重执行, 没有必要再计算
        if (exceedSqDepth_) {
            HCCL_INFO("[AicpuBlocklistManager][CalcExecSqeCount] exceedSqDepth_[%u]: no need to "
                "calculate non-flip executed SQE count", exceedSqDepth_);
            return HCCL_SUCCESS;
        }
    ...
    HcclResult AicpuBlocklistManager::EnablePartialOpretry(const std::string& algName, const OpParam &param,
        ...
        // Part 1: 基于当前算子各stream下发的SQE数量, 不考虑超过RTSQ size的情况 (否则RTSQ head/tail无法推导SQE执行情况)
        if (exceedSqDepth_) {
        ...
        // Part 3: 白名单控制影响范围 (暂时只支持alltoall类DirectFullmesh算法, 其他算子算法后续再考虑)
