...
namespace hccl {
        ...
        Stream& mainStream, std::vector<Stream>& slaveStreams, const bool isRetry)
    {
        ...
        if (!isRetry) { // 非故障算子正常展开
            ...
            HCCL_INFO("[AicpuBlocklistManager][ResetAndBackupBeforeUnfold] algName[%s] opType[%u] "
                "isDeviceMode[%d] devId[%u] isRetry[%d] isEnablePartialOpRetry_[%d]",
                algName.c_str(), param.opType, isDeviceMode, devId, isRetry, isEnablePartialOpRetry_);
        } else { // 故障算子重执行
            ...
            HCCL_RUN_INFO("[AicpuBlocklistManager][ResetAndBackupBeforeUnfold] algName[%s] opType[%u] "
                "isDeviceMode[%d] devId[%u] isRetry[%d] isEnablePartialOpRetry_[%d] (before SyncPartialOpRetry)",
                algName.c_str(), param.opType, isDeviceMode, devId, isRetry, isEnablePartialOpRetry_);
        }
        ...
        if (!isRetry) { // 非故障算子正常展开
            ...
            // 故障算子重执行使用HCCL_RUN_INFO (非性能瓶颈)
            HCCL_RUN_INFO("[AicpuBlocklistManager][ResetAndBackupBeforeUnfold] waitNotifyIdBlockSet_.size[%u] "
                "recordSignalAddrBlockSet_.size[%u] (before CalcBlockSet)",
