...
namespace hccl {
    ...
    AicpuBlocklistManager::StreamDispatchInfo::StreamDispatchInfo(const uint32_t sqDepth)
        : totalSqeCount_(0), beginSqTail_(0), sqDepth_(sqDepth) {}
    
    AicpuBlocklistManager::StreamDispatchInfo::StreamDispatchInfo(const StreamDispatchInfo& other)
        : totalSqeCount_(other.totalSqeCount_), beginSqTail_(other.beginSqTail_), sqDepth_(other.sqDepth_) {}
    ...
    HcclResult AicpuBlocklistManager::EnablePartialOpretry(const std::string& algName, const OpParam &param,
        ...
        for (std::unordered_map<int32_t, StreamDispatchInfo>::const_iterator mapIter = perStreamDispatchInfoMap_.cbegin();
            mapIter != perStreamDispatchInfoMap_.cend(); mapIter++) {
            const StreamDispatchInfo& streamDispatchInfo = mapIter->second;
            if (streamDispatchInfo.totalSqeCount_ > streamDispatchInfo.sqDepth_) {
                isEnablePartialOpretry = false;
                HCCL_INFO("[AicpuBlocklistManager][EnablePartialOpretry] streamId[%d] totalSqeCount[%llu] > sqDepth[%u], "
                    "partial opretry is disabled",
                    mapIter->first, streamDispatchInfo.totalSqeCount_, streamDispatchInfo.sqDepth_);
                return HCCL_SUCCESS;
            }
        }
        ...
        Stream& mainStream, std::vector<Stream>& slaveStreams)
    {
        ...
        mainIter->second.totalSqeCount = 0;

        // 备份当前算子展开前, aicpu main stream的RTSQ tail
        CHK_RET(QuerySqStatusByType(devId, mainStream.sqId(), DRV_SQCQ_PROP_SQ_TAIL, mainIter->second.beginSqTail));

        HCCL_INFO("[AicpuBlocklistManager][ResetAndBackupBeforeUnfold] mainStreamId[%d] totalSqeCount[%llu] beginSqTail[%u]",
            mainStreamId, mainIter->second.totalSqeCount, mainIter->second.beginSqTail);

        // Part 2: 针对aicpu slave streams
        for (size_t i = 0; i < slaveStreams.size(); i++) {
            ...
            slaveIter->second.totalSqeCount = 0;

            // 备份当前算子展开前的各stream的RTSQ tail
            CHK_RET(QuerySqStatusByType(devId, slaveStream.sqId(), DRV_SQCQ_PROP_SQ_TAIL, slaveIter->second.beginSqTail));

            HCCL_INFO("[AicpuBlocklistManager][ResetAndBackupBeforeUnfold] slaveStreamId[%d] totalSqeCount[%llu] beginSqTail[%u]",
                slaveStreamId, slaveIter->second.totalSqeCount, slaveIter->second.beginSqTail);
        }

        // Part 3: 重置firstFlipPlaceholderSqIdx_
        firstFlipPlaceholderSqIdx_ = -1;
    ...
    HcclResult AicpuBlocklistManager::UpdateTotalSqeCount(const int32_t streamId, const uint64_t sqeCount)
    {
        ...
        if (UNLIKELY(mapIter->second.totalSqeCount_ + sqeCount < mapIter->second.totalSqeCount_)) {
            HCCL_ERROR("[AicpuBlocklistManager][UpdateTotalSqeCount] existing sqeCount[%llu] + current sqeCount[%llu]"
                "overflow, streamId[%d]", mapIter->second, sqeCount, streamId);
            return HCCL_E_INTERNAL;
        }

        // 更新totalSqeCount
        mapIter->second.totalSqeCount_ += sqeCount;
