...
namespace hccl {
    ...
    HcclResult AicpuBlocklistManager::InitBlocklistManager(void *dispatcherPtr, Stream& mainStream,
        std::vector<Stream>& slaveStreams)
    {
        streamIdSqeCountMap_.clear();
        streamIdSqeCountMapBackup_.clear();
        CHK_RET((reinterpret_cast<DispatcherAiCpu *>(dispatcherPtr))->SetSqeMonitorContext(mainStream_, slaveStreams_));
        return HCCL_SUCCESS;
    }
    ...
    HcclResult AicpuBlocklistManager::UpdateSqeCounts(const int32_t streamId, const uint64_t sqeCount)
    {
        std::unordered_map<int32_t, uint64_t>::iterator mapIter = streamIdSqeCountMap_.find(streamId);
        if (mapIter == streamIdSqeCountMap_.end()) {
            streamIdSqeCountMap_[streamId] = sqeCount;
        } else {
            if (UNLIKELY(mapIter->second + sqeCount < mapIter->second) {
                HCCL_ERROR("[AicpuBlocklistManager][UpdateSqeCounts] existing sqeCount[%llu] + current sqeCount[%llu]"
                    "overflow, streamId[%d]", mapIter->second, sqeCount, streamId);
                return HCCL_E_INTERNAL;
            }
            mapIter->second += sqeCount;
        }
        return HCCL_SUCCESS;
    }
