...
namespace hccl {
    ...
    void AicpuBlocklistManager::StreamPartialOpRetryInfo::ResetAndBackup(const uint32_t sqTail, const bool isRetry)
    {
        ...
        if (!isRetry) { // 非故障算子正常展开
            ...
            // 注意: execNonPlaceholderSqeCount在故障算子正常展开触发故障停流时已经设置过, 重执行时需要沿用之前设置的值
            // 如果故障算子重执行过程中, 又触发了故障停流, execNonPlaceholderSqeCount会被CalcExecSqeCount重新计算
        }
        ...
        const bool isFlip)
    {
        ...
        if (isFlip) { // flip placeholder SQE
            ...
            alltoallvPlaceholderSqIdxVec.push_back(curPlaceholderSqIdx);
            HCCL_INFO("[AicpuBlocklistManager][UpdatePlaceholderSqIdx] add placeholderSqIdx[%lld] "
                ...
                curPlaceholderSqIdx, alltoallvPlaceholderSqIdxVec.size(), streamId);
        }
        ...
        std::vector<Stream>& slaveStreams)
    {
        ...
        HCCL_INFO("[AicpuBlocklistManager][CalcExecSqeCount] calc non-placeholder executed SQE count "
            "for main stream[%d]", mainStream.GetHcclStreamInfo().actualStreamId);
    ...
    HcclResult AicpuBlocklistManager::SyncPartialOpRetry(const bool partialOpRetryFlag)
    {
        if (partialOpRetryFlag) {
            ...
            HCCL_INFO("[AicpuBlocklistManager][SyncPartialOpRetry] set isEnablePartialOpRetry_[%d] to false",
                isEnablePartialOpRetry_);
            isEnablePartialOpRetry_ = false;
        }
    ...
    HcclResult AicpuBlocklistManager::CalcBlockSet(const AlgResourceResponse& algResource, const HcclTopoInfo& topoinfo,
        ...
        if (!isEnablePartialOpRetry_) {
            HCCL_INFO("[AicpuBlocklistManager][CalcBlockSet] isEnablePartialOpRetry_[%d]: no need to "
        ...
        HCCL_INFO("[AicpuBlocklistManager][CalcBlockSet] links.size[%u] fastRankNum[%u] rankSize[%u] curRank[%u]",
        ...
        HCCL_INFO("[AicpuBlocklistManager][CalcBlockSet] add mainWaitParamStreamNotifyId[%u] into waitNotifyIdBlockSet_",
            mainWaitParamStreamNotifyId);
        
        HCCL_INFO("[AicpuBlocklistManager][CalcBlockSet] waitNotifyIdBlockSet_.size[%u] recordSignalAddrBlockSet_.size[%u]",
    ...
    HcclResult AicpuBlocklistManager::ApplyBlocklist(const bool isCopySqe, Stream& stream, const bool isWaitSqe,
        const uint32_t notifyId, const uint64_t signalAddr, bool& isFilter) {
        // 如果isEnablePartialOpRetry_为false, 则没有必要应用黑名单信息
        if (!isEnablePartialOpRetry_) {
            HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] isEnablePartialOpRetry_[%d]: no need to "
                "apply blacklist", isEnablePartialOpRetry_);
            return HCCL_SUCCESS;
        }

        const int32_t streamId = stream.GetHcclStreamInfo().actualStreamId;
        HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] isCopySqe[%d] stream[%d] isWaitSqe[%d] notifyId[%u] "
        ...
        if (isCopySqe) { // 拷贝类SQE
            // 判断当前拷贝类SQE故障前是否执行过
            if (mapIter->second.opRetryNonFlipSqeCount < mapIter->second.execNonFlipSqeCount) { // 执行过, 需要过滤
                HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] execNonFlipSqeCount[%llu] > "
                    "opRetryNonFlipSqeCount[%llu] for streamId[%d] -> filter current SQE",
                    mapIter->second.execNonFlipSqeCount, mapIter->second.opRetryNonFlipSqeCount, streamId);
                isFilter = true;
            } else { // 未执行过, 需要下发
                HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] execNonFlipSqeCount[%llu] <= "
                    "opRetryNonFlipSqeCount[%llu] for streamId[%d] -> not filter current SQE",
                    mapIter->second.execNonFlipSqeCount, mapIter->second.opRetryNonFlipSqeCount, streamId);
                isFilter = false;
            }
        } else { // 同步类SQE
            if (isWaitSqe) { // 同步等待类SQE
                if (waitNotifyIdBlockSet_.find(notifyId) != waitNotifyIdBlockSet_.end()) { // 需要过滤
                    HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] notifyId[%u] is in waitNotifyIdBlockSet_ -> "
                        "filter current SQE", notifyId);
                    isFilter = true;
                } else { // 需要下发
                    HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] notifyId[%u] is not in waitNotifyIdBlockSet_ -> "
                        "not filter current SQE", notifyId);
                    isFilter = false;
                }
            } else { // 同步通知类SQE
                if (recordSignalAddrBlockSet_.find(signalAddr) != recordSignalAddrBlockSet_.end()) { // 需要过滤
                    HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] signalAddr[0x%016llx] is in "
                        "recordSignalAddrBlockSet_ -> filter current SQE", signalAddr);
                    isFilter = true;
                } else { // 需要下发
                    HCCL_INFO("[AicpuBlocklistManager][ApplyBlocklist] signalAddr[0x%016llx] is not in "
...
// #endif
    ...
    HcclResult AicpuBlocklistManager::CalcWaitNotifyIdBlockSet_(const LINK& tmpLink)
    {
        ...
        HCCL_INFO("[AicpuBlocklistManager][CalcWaitNotifyIdBlockSet_] "
    ...
    HcclResult AicpuBlocklistManager::CalcRecordSignalAddrBlockSet_(const LINK& tmpLink)
    {
        ...
        HCCL_INFO("[AicpuBlocklistManager][CalcRecordSignalAddrBlockSet_] "
