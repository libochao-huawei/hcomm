...
namespace hccl {
    ...
    {
        // 如果isEnablePartialOpRetry_为false, 则没有必要再计算non-flip executed SQE count (黑名单-拷贝类信息)
        if (!isEnablePartialOpRetry_) {
    ...
    HcclResult AicpuBlocklistManager::ApplyBlocklist(const bool isCopySqe, Stream& stream, const bool isWaitSqe,
        ...
        // 更新重执行故障算子时, 在当前stream已展开的非flip placeholder SQE数量
        mapIter->second.opRetryNonFlipSqeCount += 1;
