    ...
    uint32_t &endSqePos, const AsyncUnfoldStage asyncUnfoldStage)
{
    ...
    // 注意: 异步展开生成阶段, 不会改变RTSQ tail, 无需备份tail位置 (应用阶段下发RTSQ前备份即可)
    HcclResult ret = HCCL_SUCCESS;
    ...
    // 正常展开 or 异步展开生成阶段:
    // (i) 缓存命中: LaunchNewTask根据异步展开上下文, 判断将刷新的SQE直接下发到RTSQ, 还是保存到AsyncUnfoldCache
    // (ii) 不使能aicpu cache: LaunchTask根据异步展开上下文, 判断将生成的SQE直接下发到RTSQ, 还是保存到AsyncUnfoldCache
    // (iii) 缓存不命中: LaunchTask根据异步展开上下文, 判断将生成的SQE直接下发到RTSQ, 还是保存到AsyncUnfoldCache;
    ...
    if (asyncUnfoldStage != AsyncUnfoldStage::ASYNC_UNFOLD_APPLY) { // 正常展开 或者 异步展开生成阶段
        if (asyncUnfoldStage == AsyncUnfoldStage::ASYNC_UNFOLD_GENERATE) { // 异步展开生成阶段
            // TODO: 构造AsyncUnfoldKey
            AsyncUnfoldKey asyncUnfoldKey;

            // TODO: 获取AsyncUnfoldCacheEntry
            ...
            CHK_RET((reinterpret_cast<DispatcherAiCpu *>(dispatcher_))->SetLaunchAsyncContext(
                asyncUnfoldCacheEntryPtr, asyncUnfoldStage, profL1Enable));
        }
    ...
    if (asyncUnfoldStage != AsyncUnfoldStage::ASYNC_UNFOLD_GENERATE) {
        ...
        // 注意: 异步展开生成阶段, 不会改变RTSQ tail, 无需备份tail位置 (应用阶段下发RTSQ后备份即可)
        if (retryForBatchSndRcv) {
    ...
    HCCL_INFO("hccl aicpu launch hccl op task success. stream sqid:%u begin:%u end:%u. asyncUnfoldStage:%d",
        mainStream_.sqId(), beginSqePos, endSqePos, asyncUnfoldStage);
    return HCCL_SUCCESS;
}
