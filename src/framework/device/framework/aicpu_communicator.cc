...
// 获取指定索引的算子信息
const AicpuOpInfo* HcclCommAicpu::GetOpInfoFromSqIdx(u32 sqIdx, SqeRingBuffer *sqeContextBuffer) const
{
    return aicpuShareData_.GetAicpuOpInfo(sqeContextBuffer->rtsDfxInfo[sqIdx].opRingBufferIdx);
}

// 判断是否需要打印当前行
bool HcclCommAicpu::ShouldPrintCurrentLine(const std::vector<std::string>& currentOpTasks, 
                                           u32 maxTasksPerLine, u32 currentOpIndex, 
                                           u32 newOpIndex, const std::string& currentOpTag,
                                           const std::string& newOpTag) const
{
    // 当遇到不同算子，或当前行已满时，需要打印
    return (newOpIndex != currentOpIndex || newOpTag != currentOpTag ||
            currentOpTasks.size() >= (maxTasksPerLine + 1));
}

// 打印算子数据信息
void HcclCommAicpu::PrintOpDataInfo(u32 sqIdx, SqeRingBuffer *sqeContextBuffer, bool isMonitor) const
{
...
void HcclCommAicpu::PrintTaskExceptionTaskQue(u32 sqIdx, SqeRingBuffer *sqeContextBuffer, bool isMonitor)
{
    ...
    for (u32 i = 0; i < sqeNum; i++) {
        ...
        // 判断是否需要打印
        if (ShouldPrintCurrentLine(currentOpTasks, maxTasksPerLine, opIndex,
                                 newOpInfo->opIndex, opTag, newOpInfo->tagBuff)) {
            // 打印算子信息（仅第一次）
            if (lineCount == 0) {
