    ...
    KfcError &errorCode, uint32_t &beginSqePos, uint32_t &endSqePos, uint32_t retryCnt)
{
    ...
    CHK_RET(aicpuBlocklistManager_.ResetAndBackupBeforeUnfold(algName, param, topoInfo_,
    ...
    KfcError &errorCode, uint32_t &retryCnt, uint32_t &beginSqePos, uint32_t &endSqePos)
{
    ...
    if (partialRetryInfo.partialOpRetryFlag) {
        // TODO: 校验本地使能flag
        CHK_PRT_RET(!aicpuBlocklistManager_.IsEnablePartialOpRetry,
            HCCL_ERROR(""))

        // TODO: 根据快卡信息和通信主流信息, 构建黑名单-同步类信息
    }
