    ...
    KfcError &errorCode, uint32_t &beginSqePos, uint32_t &endSqePos, uint32_t retryCnt)
{
    ...
    resetCount++;
    HCCL_WARNING("[HcclCommAicpu][HcclOpExecFsmLaunchProcess] avg resetUs[%f] w/ %u resets",
        resetTotalUs / resetCount, resetCount);
#endif
    ...
    const std::string &algName, OpParam &param, uint32_t beginSqePos, uint32_t endSqePos)
{
    ...
    if (sqHead == endSqePos) {
        ...
        // TODO: 在aicpu kfc上报kStopExec KfcStatus给hccl host agent之前, 通过D2H上报局部重执行使能情况
