    ...
    std::string &tag, const uint32_t &beginSqePos, OpParam &param)
{
    ...
    do {
        ...
        if (aicpuAsyncUnfolder_.IsAsyncUnfoldEnable()) {
            if (hasSubsequentOp && (isAsyncUnfoldCacheFull || !isExpecctedAdjacentOp)) { // 存在后续算子信息
                ...
                if (isValid) { // 存在有效 (过时/后续) 算子信息
                    // TODO: 判断获取的OpAsyncUnfoldInfo与当前kernel相比是否为后续算子信息 (即opIdx > curIdx)
                    // TODO: 更新hasSubsequentOp
