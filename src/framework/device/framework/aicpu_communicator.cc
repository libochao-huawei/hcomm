...
HcclResult HcclCommAicpu::LookupOpUnfoldCache(const OpParam &param, const AlgResourceResponse &algResource, bool& needExecute, bool& isCacheMiss)
{
    ...
    // if (opUnfoldCachePtr_ != nullptr && !param.supportSymmetricMemory) {
    if (opUnfoldCachePtr_ != nullptr) {
                ...
                if (curReq.isValid) {
                    ...
                    if (!isZeroCopy_) {
                        if (curReq.inputMemType == TransportMemType::PARAM_INPUT) {
                            ...
                            HCCL_INFO("liliguo [HcclCommAicpu][PrepareUserMemRanges] non-zero copy prepare user input of remoteRank[%u] for graph mode; baseAddr[0x%016llx]", curReq.remoteUserRank, remoteUserInputBaseAddr);
                        ...
                        if (curReq.outputMemType == TransportMemType::PARAM_OUTPUT) {
                            ...
                            HCCL_INFO("liliguo[HcclCommAicpu][PrepareUserMemRanges] non-zero copy prepare user output of remoteRank[%u] for graph mode; baseAddr[0x%016llx]", curReq.remoteUserRank, remoteUserOutputBaseAddr);
                            ...
                            HCCL_INFO("liliguo [HcclCommAicpu][PrepareUserMemRanges] zero copy prepare user input of remoteRank[%u] for graph mode; baseAddr[0x%016llx]", curReq.remoteUserRank, remoteUserInputBaseAddr);
                            ...
                            HCCL_INFO("liliguo [HcclCommAicpu][PrepareUserMemRanges] zero copy prepare user output of remoteRank[%u] for graph mode; baseAddr[0x%016llx]", curReq.remoteUserRank, remoteUserOutputBaseAddr);
    ...
    const HcclOpResParam *commParam, AlgResourceRequest &resRequest, AlgResourceResponse &algResResponse)
{
    ...
    HCCL_INFO("[HcclCommAicpu][AllocAlgResource] alloc resource start tag[%s], scratchMemSize[%llu], streamNum[%u], notifyNum[%u]",
        newTag.c_str(), resRequest.scratchMemSize, resRequest.streamNum, resRequest.notifyNum);
    ...
    std::unique_ptr<CollExecutorBase> &executor, AlgResourceRequest &resourceRequest)
{
    if (executor.get() == nullptr) {
        ...
        if (param.opType == HcclCMDType::HCCL_CMD_REDUCE_SCATTER) {
            ...
            if (GetWorkflowMode() != HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE || param.supportSymmetricMemory) {
...
HcclResult HcclCommAicpu::PrepareSymmetricMemory(const OpParam &param, OpCommTransport &opTransportResponse)
{
    ...
    const std::unordered_set<LinkType> supportedLinkTypes = {LinkType::LINK_HCCS, LinkType::LINK_SIO, LinkType::LINK_HCCS_SW};
    for (u32 level = 0; level < opTransportResponse.size(); ++level) {
        for (auto &singleSubCommTransport : opTransportResponse[level]) {
            for (u64 i = 0; i < singleSubCommTransport.links.size(); ++i) {
                ...
                if (!isZeroCopy_) {
                    HCCL_INFO("liliguo [HcclCommAicpu][PrepareSymmetricMemory] non-zeroCopy remoteAddr prepare for remoteRank[%d]", peerRank);
                    if (singleSubCommTransport.transportRequests[i].inputMemType == PARAM_INPUT) {
                        void *remoteIn = nullptr;
                        CHK_RET(HcommSymWinGetPeerPointer(param.inputSymWindow, param.inputOffset, peerRank, &remoteIn));
                        HCCL_INFO("liliguo [HcclCommAicpu][PrepareSymmetricMemory] non-zeroCopy remoteIn[%p] for remoteRank[%d]", remoteIn, peerRank);
                        CHK_RET(link->UpdateRemoteInAddr(remoteIn));
                        CHK_PRT_RET(remoteIn == nullptr,
                            HCCL_ERROR("[HcclCommAicpu][PrepareSymmetricMemory] remoteRank[%d] in[%p] is invalid", peerRank, remoteIn),
                            HCCL_E_INTERNAL);    
                    }

                    if (singleSubCommTransport.transportRequests[i].outputMemType == PARAM_OUTPUT) {
                        void *remoteOut = nullptr;
                        CHK_RET(HcommSymWinGetPeerPointer(param.outputSymWindow, param.outputOffset, peerRank, &remoteOut));
                        HCCL_INFO("liliguo [HcclCommAicpu][PrepareSymmetricMemory] non-zeroCopy remoteOut[%p] for remoteRank[%d]", remoteOut, peerRank);
                        CHK_RET(link->UpdateRemoteOutAddr(remoteOut));
                        CHK_PRT_RET(remoteOut == nullptr,
                            HCCL_ERROR("[HcclCommAicpu][PrepareSymmetricMemory] remoteRank[%d] out[%p] is invalid", peerRank, remoteOut),
                            HCCL_E_INTERNAL);
                    }
                } else {
                    HCCL_INFO("liliguo [HcclCommAicpu][PrepareSymmetricMemory] zeroCopy remoteAddr prepare for remoteRank[%d]", peerRank);
                    void *remoteIn = nullptr;
                    CHK_RET(HcommSymWinGetPeerPointer(param.inputSymWindow, param.inputOffset, peerRank, &remoteIn));
                    CHK_RET(link->UpdateRemoteInAddr(remoteIn));
                    CHK_PRT_RET(remoteIn == nullptr,
                ...
                void *remoteInPtr = nullptr;
                void *remoteOutPtr = nullptr;
                link->GetRemoteMem(UserMemType::INPUT_MEM, &remoteInPtr);
                link->GetRemoteMem(UserMemType::OUTPUT_MEM, &remoteOutPtr);
                HCCL_INFO("liliguo [HcclCommAicpu][PrepareSymmetricMemory] remoteRank[%d] inAddr[%p] outAddr[%p]", peerRank,
                    remoteInPtr, remoteOutPtr);
                                            ...
                                            OpParam &opParam, const HcclOpResParam *commParam)
{
    ...
    } else if (!isZeroCopy_ && isSymmetricMemory_) {
        HCCL_INFO("liliguo [HcclCommAicpu][ExecOp] non-zeroCopy prepare symmetric memory");
        CHK_RET(PrepareSymmetricMemory(opParam, algResResponse->opTransportResponse));
    }
