...
HcclResult HcclCommAicpu::RegisterDispatcherCallback()
{
    ...
    auto updateSqeCountsCallback = [this](const int32_t streamID, const uint64_t sqeCount) {
        return this->aicpuBlocklistManager_.UpdateSqeCounts(streamID, sqeCount);
    };

    auto updateFirstFlipPlaceholderSqIdxCallback = [this](const int32_t streamID, const int64_t firstFlipPlaceholderSqIdx) {
        return this->aicpuBlocklistManager_.UpdateFirstFlipPlaceholderSqIdx(streamID, firstFlipPlaceholderSqIdx);
    };

    return HcclSetDispatcherAicpuCallback(dispatcher_, checkOpExecStatusCallback, updateSqeCountsCallback,
        updateFirstFlipPlaceholderSqIdxCallback);
}
