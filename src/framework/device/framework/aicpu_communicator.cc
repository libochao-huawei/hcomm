    ...
    while (true) {
        switch (state) {
            ...
            case HcclOpExecFSM::HCCL_OP_EXEC_FSM_RETRY:
                ...
                // 重执行前恢复异步展开备份的主从流信息 (if any)
                CHK_RET(aicpuAsyncUnfolder_.RestoreStreamInfo(param.opBaseOpIdx, mainStream_, algResource.slaveStreams));
    ...
    uint32_t &endSqePos, const AsyncUnfoldStage asyncUnfoldStage)
{
    ...
    HcclResult ret = HCCL_SUCCESS;
    if (asyncUnfoldStage != ASYNC_UNFOLD_GENERATE) {
    ...
    if (asyncUnfoldStage != ASYNC_UNFOLD_APPLY) {
    ...
    if (asyncUnfoldStage != ASYNC_UNFOLD_APPLY) { // 正常展开 或者 异步展开生成阶段
        ...
        CHK_RET(aicpuCacheManager_.LookupOpUnfoldCache(algName, param, algResource, needExecute, isCacheMiss,
            ...
            ZeroCopyExchanger_, GetWorkflowMode(), tinySendRecvMem_, setProfStartCallback, asyncUnfoldStage));

        // 根据needExecute有条件的执行算子展开
        // 需要算子执行的场景: (i) expansion mode为AI_CPU_NO_CACHE; (ii) uncacheable算子/场景; (iii) cache miss
        if (needExecute) {
            // TODO: asyncUnfoldStage传入DispatcherAicpu, 判断展开的SQE是否需要保存到AsyncUnfoldCache
            // 注意: 如果是cache miss, 展开的SQE还需要保存到aicpu cache (与AsyncUnfoldCache独立)
