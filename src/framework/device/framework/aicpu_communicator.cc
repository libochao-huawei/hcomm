...
HcclResult HcclCommAicpu::OrchestrateHcclOp(const std::string &algName, OpParam &param,
    ...
    uint32_t &endSqePos, const AsyncUnfoldStage asyncUnfoldStage, AsyncUnfoldCacheEntry* asyncUnfoldCacheEntryPtr)
{
    if (asyncUnfoldStage != AsyncUnfoldStage::NORMAL_UNFOLD) { // 异步展开生成/应用阶段
    ...
    if (asyncUnfoldStage != AsyncUnfoldStage::ASYNC_UNFOLD_APPLY) { // 正常展开 或者 异步展开生成阶段
        ...
        // 异步展开生成阶段: 捕捉生成/刷新SQE到AsyncUnfoldCache后
        if (asyncUnfoldStage == AsyncUnfoldStage::ASYNC_UNFOLD_GENERATE) {
                    ...
                    if (!hasSubsequentOp) { // 过时算子信息
                        ...
                        if (isAsyncUnfoldCacheFull) { // Cache容量已满
                            ...
                            if (isExpectedAdjacentOp) {
                                ...
                                // TODO: 根据后续算子信息, 构造AsyncUnfoldKey
                                AsyncUnfoldKey asyncUnfoldKey;
                                // TODO: 创建AsyncUnfoldCacheEntry (创建前一定不存在)
                                AsyncUnfoldCacheEntry* asyncUnfoldCacheEntryPtr = nullptr;

                                // TODO: 触发异步展开, 将展开的SQE保存为AsyncUnfoldCache中的new entry
