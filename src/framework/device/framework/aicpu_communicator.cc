    ...
    const HcclOpResParam *commParam)
{
    ...
    // (3) 初始化预期的相邻算子为, 当前正在执行算子的下一个算子 (expectedAdjacentOp = current opBaseOpIdx + 1)
    uint64_t expectedAdjacentOpIdx = param.opBaseOpIdx + 1;
    bool isExpectedAdjacentOp = true; // 后续算子是否为预期的相邻算子
    // (4) 从OpH2DRingBuffer中获取的算子信息
    ...
    do {
        { MY_TIMER("WaitFinishWhileLoop_step3"); // TODOSSY:性能打点
