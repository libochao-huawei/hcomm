...
HcclResult HcclCommAicpu::RecordHostOrder(const HcclOpResParam *commParam, const std::string& tag, const std::string& newTag, u8 orderLaunchMode)
{
    ...
    if (orderLaunchMode == orderLaunchInvalidInHcom) {
        HCCL_ERROR("[%s] attachedStreams_[%d] is invalid in graph mode", __func__, orderStream_.id());
        return HCCL_SUCCESS;
    }
    if (orderNotifies_[orderLaunchMode] == nullptr) {
        ...
        HCCL_ERROR("[%s] attachedStreams_[%d] aicpuOrderNotify resId[%llu], addr[0x%llx], flag[%u], devId[%u], tsId[%u], rankId[%u]",
        ...
        HCCL_ERROR("%s success, group[%s], resId[%llu]", __func__, identifier_.c_str(), aicpuOrderNotify->resId);
    }

    HCCL_ERROR("%s group[%s] tag[%s] newTag[%s] isDeviceMode[%d] orderLaunchMode[%u] mode[%d] streamId[%d] notifyId[%u]",
