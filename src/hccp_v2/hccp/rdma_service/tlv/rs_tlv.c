    ...
    bool *isSendFinish)
{
    ...
    CHK_PRT_RETURN(head->sendBytes > MAX_TLV_MSG_DATA_LEN, hccp_err("[recv][rs_tlv]send_bytes(%u) > data size(%u),"
        " phy_id(%u)", head->sendBytes, MAX_TLV_MSG_DATA_LEN, head->phyId), -EINVAL);
