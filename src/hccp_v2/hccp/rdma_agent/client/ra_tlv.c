...
HCCP_ATTRI_VISI_DEF int RaTlvInit(struct TlvInitInfo *initInfo, unsigned int moduleType, unsigned int *bufferSize, void **tlvHandle)
{
    ...
    CHK_PRT_RETURN(moduleType >= TLV_MODULE_TYPE_MAX,
        hccp_err("[init][ra_tlv]moduleType(%u) invalid, must smaller than TLV_MODULE_TYPE_MAX(%u)",
    ...
    CHK_PRT_RETURN(initInfo->phyId >= RA_MAX_PHY_ID_NUM,
        hccp_err("[init][ra_tlv]phyId(%u) must smaller than %u", initInfo->phyId, RA_MAX_PHY_ID_NUM),
        ConverReturnCode(HCCP_INIT, -EINVAL));
    ...
    hccp_run_info("Input parameters: phyId[%u], nicPosition[%u] moduleType[%u]",
        initInfo->phyId, initInfo->nicPosition, moduleType);
...
HCCP_ATTRI_VISI_DEF int RaTlvDeinit(void *tlvHandle)
{
    ...
    hccp_run_info("Input parameters: phyId[%u], nic_position[%u]  moduleType[%u]", tlvHandleTmp->initInfo.phyId,
