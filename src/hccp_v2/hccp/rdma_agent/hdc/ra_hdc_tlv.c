...
int RaHdcTlvInit(struct RaTlvHandle *tlvHandle)
{
    ...
    CHK_PRT_RETURN(ret == -ENOTSUPP, hccp_warn("[init][ra_hdc_tlv]ra hdc message process unsuccessful ret(%d) phyId(%u)",
        ret, phyId), ret);
    CHK_PRT_RETURN(ret != 0, hccp_err("[init][ra_hdc_tlv]ra hdc message process failed ret(%d) phyId(%u)",
        ret, phyId), ret);
...
int RaHdcTlvDeinit(struct RaTlvHandle *tlvHandle)
{
    ...
    CHK_PRT_RETURN(ret != 0, hccp_err("[deinit][ra_hdc_tlv]ra hdc message process failed ret(%d) phyId(%u)",
        ret, phyId), ret);

    return 0;
}

STATIC void RaHdcTlvRequestHeadInit(struct RaTlvHandle *tlvHandle,
    ...
    struct TlvRequestMsgHead *head, struct TlvMsg *recvMsg)
{
    ...
    CHK_PRT_RETURN(ret != 0, hccp_err("[request][ra_hdc_tlv]hdc message process failed ret(%d) phyId(%u)",
        ret, phyId), ret);
...
int RaHdcTlvRequest(struct RaTlvHandle *tlvHandle, struct TlvMsg *sendMsg, struct TlvMsg *recvMsg)
{
    ...
    while (head.offset < sendMsg->length) {
        ...
        CHK_PRT_RETURN(ret != 0, hccp_err("[request][ra_hdc_tlv]memcpy_s data failed ret(%d) phyId(%u) send_bytes(%u)",
