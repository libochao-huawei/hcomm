...
/// 底层未回 bit17（slBitmap 扩展不可用）时：固定选 list[0]，jetty priority 用 2
static constexpr uint32_t kLegacyMappedJettyPriority = 2U;

static bool IsTpAttrSlBitmapExtSupported(const uint32_t tpAttrBitmap)
{
    return (tpAttrBitmap & kTpAttrSlBitmapExtMask) != 0U;
}
...
/// UBOE 8-TP：hcclQos 高→slBitmap 低档（高优先级），低→高档；3:2:3 对应 qos [7–5]/[4–3]/[2–0]。
...
HcclResult TpMgr::HandleCompletedRequest(RequestCtx reqCtx, const GetTpInfoParam &param, TpInfo &tpInfo)
{
    ...
    const bool slBitmapExtSupported = IsTpAttrSlBitmapExtSupported(reqCtx.tpAttrBitmap);
    HCCL_INFO("[TpMgr][%s] after get_tp_attr: slBitmapExtSupported[%d] slBitmap[0x%x] dscp[%u] dscpConfigMode[%u] "
              "tpAttrBitmap[0x%x] param[%s].",
        __func__, static_cast<int>(slBitmapExtSupported), static_cast<unsigned>(reqCtx.tpAttr.slBitmap),
    ...
    TpInfo tmpTpInfo{};
    if (!slBitmapExtSupported) {
        FillLegacyTpInfoWithoutSlExt(baseInfoPtr, tmpTpInfo);
        HCCL_INFO("[TpMgr][%s] slBitmap ext not supported (tpAttrBitmap bit17=0): use first tpHandle[%llu] "
                  "mappedJettyPriority[%u] param[%s].",
        ...
        HCCL_INFO("[TpMgr][%s] slBitmap ext supported: slMask[0x%04x] slAvailableCnt[%u].",
            __func__, static_cast<unsigned>(slMask), slAvailableCnt);
        if (slAvailableCnt == 0U) {
            HCCL_ERROR("[TpMgr][%s] slBitmap ext supported but sl_available mask empty, param[%s].", __func__,
                param.Describe().c_str());
            return HcclResult::HCCL_E_INTERNAL;
        }
