    ...
    unsigned long long wrId)
{
    ...
    hccp_info("[send][ra_hdc_wr] liteWr: wr_id=%llu num_sge=%d opcode=%u send_flags=%u remote_addr=0x%llx rkey=%u imm_data=%u",
    ...
    for (i = 0; i < liteWr.num_sge; i++) {
        hccp_info("[send][ra_hdc_wr] liteWr.sg_list[%d]: addr=0x%llx length=%u lkey=%u",
