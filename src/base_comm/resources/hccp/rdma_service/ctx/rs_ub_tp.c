    ...
    unsigned int *num)
{
    ...
    hccp_info("[rs_ub_ctx][RsUbGetTpInfoList] before rs_urma_get_tp_list: tpCnt[%u] flag.value[0x%x] "
        "ctp[%u] rtp[%u] utp[%u] uboe[%u] pre_defined[%u] dynamic_defined[%u] udp[%u] group_id[%u] "
        ...
        udmaCfg.flag.bs.udp, udmaCfg.flag.bs.group_id, (unsigned int)udmaCfg.trans_mode,
        (unsigned long long)be64toh(udmaCfg.local_eid.in6.subnetPrefix),
        (unsigned long long)be64toh(udmaCfg.local_eid.in6.interfaceId),
        (unsigned long long)be64toh(udmaCfg.peer_eid.in6.subnetPrefix),
        (unsigned long long)be64toh(udmaCfg.peer_eid.in6.interfaceId));
