...
HcclResult HostCpuRoceChannel::CreateQp() {
    for (uint32_t i = 0; i < connections_.size(); i++) {
        ...
        Hccl::QpInfo& qpInfo = conn->GetQpInfo();
        HCCL_INFO("[HostCpuRoceChannel::%s] qpInfo[%u] created, qp_num=%u, qp_state=%u",
            __func__, i, qpInfo.qp->qp_num, qpInfo.qp->state);
    }
...
HcclResult HostCpuRoceChannel::ModifyQp() {
    for (uint32_t i = 0; i < connections_.size(); i++) {
        ...
        qpInfo.retryCnt = channelDesc_.roceAttr.retryCnt;
        qpInfo.retryInterval = channelDesc_.roceAttr.retryInterval;
        HCCL_DEBUG("[HostCpuRoceChannel::ModifyQp] qpInfo[%u] qp_num[%u], serviceLevel[%u], trafficClass[%u], retryCnt[%u], retryInterval[%u].",
            i, qpInfo.qp->qp_num, qpInfo.serviceLevel, qpInfo.trafficClass, qpInfo.retryCnt, qpInfo.retryInterval);
...
HcclResult HostCpuRoceChannel::IbvPostRecv() const {
    ...
    for (uint32_t i = 0; i < qpInfo.size(); i++) {
        HCCL_DEBUG("[HostCpuRoceChannel::%s] qpInfo[%u], qp_state = [%u], qp_num = [%u]",
                   __func__, i, qpInfo[i].qp->state, qpInfo[i].qp->qp_num);
        ...
        CHK_PRT_RET(ret == ENOMEM,
                    HCCL_WARNING("[HostCpuRoceChannel][%s] post recv wqe overflow. ret:%d, qp_num = [%u] "
                                "badWr->wr_id[%llu], badWr->sg_list->addr[%llu]",
                                __func__, ret, qpInfo[i].qp->qp_num, recvbadWr->wr_id, recvbadWr->sg_list->addr),
                    HCCL_E_AGAIN);
        CHK_PRT_RET(ret != 0,
                    HCCL_ERROR("[HostCpuRoceChannel][%s] ibv_post_recv failed. ret:%d, qp_num = [%u]"
                            "badWr->wr_id[%llu], badWr->sg_list->addr[%llu]",
                            __func__, ret, qpInfo[i].qp->qp_num, recvbadWr->wr_id, recvbadWr->sg_list->addr),
                    HCCL_E_NETWORK);
    }
...
HcclResult HostCpuRoceChannel::NotifyRecord(const uint32_t remoteNotifyIdx)
{
    ...
    // 3.调用ibv_post_send
    for (uint32_t i = 0; i < qpInfo.size(); i++) {
        HCCL_DEBUG("[HostCpuRoceChannel::%s] qpInfo[%u] call ibv_post_send, qp_state = [%u], qp_num = [%u]",
                   __func__, i, qpInfo[i].qp->state, qpInfo[i].qp->qp_num);
        if (isHybridMode_) {
        ...
        CHK_PRT_RET(ret == ENOMEM,
            HCCL_WARNING("[HostCpuRoceChannel][%s] post send wqe overflow. ret:%d, qpInfo[%u], qpInfo.qp->qp_num[%u], "
                         "badWr->wr_id[%llu], badWr->sg_list->addr[%llu], badWr->wr.rdma.remote_addr[%llu], "
                         "badWr->wr.ud.remote_qpn[%u]",
                        __func__, ret, i, qpInfo[i].qp->qp_num, sendbadWr->wr_id, sendbadWr->sg_list->addr,
