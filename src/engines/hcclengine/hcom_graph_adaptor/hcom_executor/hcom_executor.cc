...
HcclResult HcomExecutor::Initialize()
{
    ...
    // 执行器创建stream
    rtStream_t rtStream = nullptr; // StreamType::STREAM_TYPE_ONLINE
    ret = hrtStreamCreateWithFlags(&rtStream, HCCL_STREAM_PRIORITY_HIGH, RT_STREAM_FAST_LAUNCH | RT_STREAM_FAST_SYNC);
    if (ret == HCCL_SUCCESS) {
        HCCL_DEBUG("rtStreamCreate ok, streamType STREAM_TYPE_ONLINE");
        parralMap_[MsgQueueType::OPBASE_QUEUE].stream = const_cast<void *>(rtStream);
        HCCL_INFO("Construct stream success, ptr[%p]", rtStream);
    } else {
        ...
        HCCL_ERROR("[Stream]Construct stream failed, errNo[0x%016llx] rtStreamCreate error, ret[%d]",
...
HcclResult HcomExecutor::ExecuteAlltoAll(const HcomAllToAllVParams& opInfo, bool isGatherAlltoAll)
{
    ...
    if (!isGatherAlltoAll) {
        ...
        CHK_RET(HcclAlltoAllV(opInfo.sendbuf, sendCountsPtr.get(), sdisplsPtr.get(), opInfo.sendtype,
                              opInfo.recvbuf, recvCountsPtr.get(), rdisplsPtr.get(), opInfo.recvtype,
...
HcclResult HcomExecutor::ExecuteAlltoAllVC(const HcomAllToAllVCParams& opInfo)
{
    ...
    CHK_RET(hrtMemSyncCopy(sendCountMatrixPtr.get(), rankSize * rankSize * sizeof(u64),
        opInfo.sendcountmatrix, rankSize * rankSize * sizeof(u64),
        HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_DEVICE_TO_HOST));

    CHK_RET(HcclAlltoAllVC(opInfo.sendbuf, sendCountMatrixPtr.get(), opInfo.sendtype,
...
HcclResult HcomExecutor::CallbackNotifyThreadLoop(MsgQueueType queType)
{
    ...
    while (!parralMap_[queType].shutDown) {
        ...
        HCOM_EXECUTOR_ERR_BREAK(status,
            HCCL_ERROR("[Callback][NotifyThreadLoop]event execute failed, event destroy ret[%d], is timeout[%d s]",
    ...
    std::function<void(HcclResult status)> callback)
{
    ...
    CHK_SMART_PTR_NULL(parralMap_[MsgQueueType::OPBASE_QUEUE].stream);
    if (parralMap_[MsgQueueType::OPBASE_QUEUE].shutDown) {
        HCCL_ERROR("[Exec][EnqueueGatherAlltoAllV]Thread has been shut down, Enqueue AllToAll failed");
        return HCCL_E_INTERNAL;
    }
                                   ...
                                   u64 *sendCounts, u64 *sdispls, void *sendDevBuf, s32 addrLength)
{
    ...
    CHK_PRT_RET(memset_s(sendCounts, rankSize * sizeof(u64), 0, rankSize * sizeof(u64)) != EOK,
        HCCL_ERROR("[Exec][EnqueueGatherAlltoAllV] mem set failed,count[%lld]",
        rankSize * sizeof(u64)), HCCL_E_SYSCALL);
    u64 prevNum = 0;
    u64 nextNum = 0;
