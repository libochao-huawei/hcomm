...
{
    HCCL_DEBUG("[Communication][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    return PARA_FLOAT_ONE;
}

float Communication::CalculateXferPercentage(int gpuNum, string algorithm, vector<int> &slices) const
{
    HCCL_DEBUG("[Communication][CalculateXferPercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    return PARA_FLOAT_ONE;
}
...
{
    HCCL_DEBUG("[Communication][CalculateXferBubbles] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    return 0.0;
}
...
{
    HCCL_DEBUG("[Communication][CalculateComputePercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    return 0.0;
}
...
{
    HCCL_DEBUG("[Scatter][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float frequency = 0.0;
    if (algorithm == "ring") {
        frequency = (PARA_FLOAT_ONE + (gpuNum / PARA_FLOAT_TWO) - PARA_FLOAT_ONE) *
            (gpuNum / PARA_FLOAT_TWO) / PARA_FLOAT_TWO * PARA_FLOAT_TWO;
    } else {
...
{
    HCCL_DEBUG("[Scatter][CalculateXferPercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    ...
    if (algorithm == "ring") {
        percentage = ((PARA_FLOAT_ONE + (gpuNum / PARA_FLOAT_TWO) - PARA_FLOAT_ONE) *
            (gpuNum / PARA_FLOAT_TWO) / PARA_FLOAT_TWO) / gpuNum * PARA_FLOAT_TWO;
...
{
    HCCL_DEBUG("[Gather][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float frequency = 0.0;
    if (algorithm == "ring") {
...
{
    HCCL_DEBUG("[Gather][CalculateXferPercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float percentage = PARA_FLOAT_ONE;
    if (algorithm == "ring") {
        percentage = ((PARA_FLOAT_ONE + (static_cast<float>(gpuNum / PARA_FLOAT_TWO)) - PARA_FLOAT_ONE) *
            (static_cast<float>(gpuNum / PARA_FLOAT_TWO)) / PARA_FLOAT_TWO) / gpuNum * PARA_FLOAT_TWO;
        if (gpuNum % PARA_INT_TWO == 0) {  //lint !e587
...
{
    HCCL_DEBUG("[Allgather][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float frequency = 0.0;
    float logValue;
    int temp;
...
float Allreduce::CalculateXferFrequency(int gpuNum, string algorithm, vector<int> &slices) const
{
    ...
    for (size_t i = 0; i < opList.size(); i++) {
        frequency +=
            opList[i]->CalculateXferFrequency(gpuNum, algorithmImpl, slicesImpl);
    }
...
float Allreduce::CalculateXferPercentage(int gpuNum, string algorithm, vector<int> &slices) const
{
    ...
    for (size_t i = 0; i < opList.size(); i++) {
        percentage +=
            opList[i]->CalculateXferPercentage(gpuNum, algorithmImpl, slicesImpl) * initialPecentages[i];
    }
...
float Allreduce::CalculateComputePercentage(int gpuNum, string algorithm, vector<int> &slices) const
{
    ...
    for (size_t i = 0; i < opList.size(); i++) {
        percentage +=
            opList[i]->CalculateComputePercentage(gpuNum, algorithmImpl, slicesImpl) * initialPecentages[i];
    }
...
{
    HCCL_DEBUG("[Broadcast][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float frequency = 0.0;
    float logValue;
    int temp;
...
{
    HCCL_DEBUG("[Broadcast][CalculateXferPercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float percentage = PARA_FLOAT_ONE;
...
{
    HCCL_DEBUG("[Broadcast][CalculateXferBubbles] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float bublleNum = PARA_FLOAT_ONE;
...
{
    HCCL_DEBUG("[Reduce][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float frequency = 0.0;
...
{
    HCCL_DEBUG("[Reduce][CalculateXferPercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
...
{
    HCCL_DEBUG("[Reduce][CalculateXferBubbles] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float bublleNum = 0.0;
...
{
    HCCL_DEBUG("[Reduce][CalculateComputePercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float percentage = 0.0;
...
{
    HCCL_DEBUG("[Reducescatter][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
...
{
    HCCL_DEBUG("[Reducescatter][CalculateXferPercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
...
{
    HCCL_DEBUG("[Reducescatter][CalculateComputePercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float percentage = static_cast<float>(static_cast<float>(gpuNum - PARA_FLOAT_ONE) / gpuNum);
...
{
    HCCL_DEBUG("[AlltoAll][CalculateXferFrequency] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float frequency = 0;
...
{
    HCCL_DEBUG("[AlltoAll][CalculateXferPercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    float percentage = PARA_FLOAT_ONE;
    if (gpuNum == 0) {
...
{
    HCCL_DEBUG("[AlltoAll][CalculateComputePercentage] gpuNum[%d], algorithm[%s], slices_size[%u]",
        gpuNum, algorithm.c_str(), slices.size());
    if (gpuNum == 0) {
        HCCL_WARNING("para error,gpuNum is zero");
        return PARA_FLOAT_ONE;
