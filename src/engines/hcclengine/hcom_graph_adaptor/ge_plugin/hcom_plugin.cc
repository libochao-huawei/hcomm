...
#include "hccl/hcom.h"
#include "hccl/hccl.h"
#include "hccl/hccl_ctrl_plane.h"
...
HcomPlugin::HcomPlugin()
    : initializeCount_(0), hcomOpsKernelInfoStoreInfoPtr_(nullptr), \
...
ge::Status HcomPlugin::Initialize(const std::map<string, string> &options)
{
    ...
    CHK_PRT_RET(ConfigHcclAlgo(options, &comConfig),
        HCCL_ERROR("[Initialize][Plugin]ConfigHcclAlgo failed."), ge::INTERNAL_ERROR);
    ...
    EXECEPTION_CATCH((hcomFusionOptimizerPtr_ = std::make_shared<hccl::HcomFusionOptimizer>()),
        return ge::INTERNAL_ERROR);
    EXECEPTION_CATCH((hcomGraphOptimizerPtr_ = std::make_shared<hccl::HcomGraphOptimizer>()),
        return ge::INTERNAL_ERROR);
#endif
...
void HcomPlugin::GetGraphOptimizerObjs(map<string, GraphOptimizerPtr> &graphOptimizers)
{
    ...
    if (hcomFusionOptimizerPtr_ != nullptr) {
        graphOptimizers.insert(std::make_pair(HCCL_FUSION_OPTIMIZER_NAME,
            hcomFusionOptimizerPtr_));
    } else {
        ...
        if (iter->second == "1") {
            ...
            if (iter != options.end()) {
                ...
                if (iter->second.find("task_trace") != iter->second.npos) {
                    ...
                    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[Init][HcomPlugin]errNo[0x%016llx] Initialize: enable"
                        "profiling mode failed.", HCOM_ERROR_CODE(ret)), ge::INTERNAL_ERROR);
...
ge::Status HcomPlugin::InitializeHcom(const std::map<string, string>& options, HcomInitConfig *comConfig)
{
    ...
    CHK_PRT_RET(geRet != ge::SUCCESS, HCCL_ERROR("[Init][HcomPlugin]errNo[0x%016llx] Initialize: ProfilingModeParser "\
        "failed.", HCOM_ERROR_CODE(geRet)), ge::INTERNAL_ERROR);
    ...
    if (iter == options.end()) {
        ...
        if (masterInfoConfiged && rankInfoConfiged) {
            ret = HcomInitByMasterInfo(masterIp.c_str(), masterPort.c_str(), masterDeviceId.c_str(),
                rankSize.c_str(), rankIp.c_str(), comConfig);
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[GetRanktable] rankTablePath[%s]"
            "load rankTable error.", rankTable.c_str()), ge::INTERNAL_ERROR);
        ret = HcomInitByString(rankTableM.c_str(), identify.c_str(), WorkMode::HCCL_MODE_NORMAL, comConfig);
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[Init][HcomPlugin]errNo[0x%016llx] Initialize: HcomInitByString failed.",
                HCOM_ERROR_CODE(ret)), ge::INTERNAL_ERROR);
        }
    return ge::SUCCESS;
}
...
HcclResult HcomPlugin::ConfigHcclDeterministic(const std::map<string, string> &options, HcomInitConfig *comConfig)
{
    ...
    if (iter != options.end()) {
        ...
        if (hcclDeterministicEnv == "EmptyString") {
            ...
            HCCL_WARNING("ParserHcclDeterministicDesc: key[ge.deterministic] has been set by"\
...
HcclResult HcomPlugin::HcomSetGroupToTopoInfo(const char *group, uint32_t rankSize)
{
    ...
    u32 gRankSize;
    u32 topoType;
    ret = CommGetInstSizeByNetLayer(commHandle, static_cast<uint32_t>(HcclTopoLevel::HCCL_TOPO_L0), &gRankSize);
    if (ret != HCCL_SUCCESS) return ret;
    ret = CommGetInstTopoTypeByNetLayer(commHandle, static_cast<uint32_t>(HcclTopoLevel::HCCL_TOPO_L0), &topoType);
    if (ret != HCCL_SUCCESS) return ret;
    topoInfo.topo_level_descs[static_cast<uint32_t>(ge::HcomTopoInfo::TopoLevel::L0)].comm_sets = topoType;
    topoInfo.topo_level_descs[static_cast<uint32_t>(ge::HcomTopoInfo::TopoLevel::L0)].rank_size = gRankSize;

    ret = CommGetInstSizeByNetLayer(commHandle, static_cast<uint32_t>(HcclTopoLevel::HCCL_TOPO_L1), &gRankSize);
    if (ret != HCCL_SUCCESS) return ret;
    ret = CommGetInstTopoTypeByNetLayer(commHandle, static_cast<uint32_t>(HcclTopoLevel::HCCL_TOPO_L1), &topoType);
    if (ret != HCCL_SUCCESS) return ret;
    topoInfo.topo_level_descs[static_cast<uint32_t>(ge::HcomTopoInfo::TopoLevel::L1)].comm_sets = topoType;
    topoInfo.topo_level_descs[static_cast<uint32_t>(ge::HcomTopoInfo::TopoLevel::L1)].rank_size = gRankSize;
