...
HcclResult OpFusionBase::RunFusionOps(ge::ComputeGraph& graph, std::vector<ge::NodePtr>& fusionOps, \
    ...
    std::vector<FusionAnchorInfo_t>& anchorInfos)
{
    ...
    for (u32 segIndex = 0; segIndex < fusedNodes.size(); segIndex++) {
        ...
        for (u32 anchorIndex = 0; anchorIndex < anchorInfosPerSeg.peerOutDataAnchor.size(); anchorIndex++) {
            gRet = ge::GraphUtils::AddEdge(anchorInfosPerSeg.peerOutDataAnchor[anchorIndex], \
                node->GetInDataAnchor(anchorIndex));
        ...
        for (u32 anchorIndex = 0; anchorIndex < anchorInfosPerSeg.peerOutControlAnchor.size(); anchorIndex++) {
            gRet = ge::GraphUtils::AddEdge(anchorInfosPerSeg.peerOutControlAnchor[anchorIndex], \
        ...
        for (u32 anchorIndex = 0; anchorIndex < anchorInfosPerSeg.peerOutDataToInControl.size(); anchorIndex++) {
            gRet = ge::GraphUtils::AddEdge(anchorInfosPerSeg.peerOutDataToInControl[anchorIndex], \
                node->GetInControlAnchor());
            bErr = (gRet != ge::GRAPH_SUCCESS);
            CHK_PRT_RET(bErr, HCCL_ERROR("[Restore][OpsEdges]add edge from out data to incontrol failed."),
                HCCL_E_INTERNAL);
        }
        ...
        for (u32 anchorIndex = 0; anchorIndex < anchorInfosPerSeg.peerInControlFromOutData.size(); anchorIndex++) {
            ...
            bErr = (gRet != ge::GRAPH_SUCCESS);
            CHK_PRT_RET(bErr, HCCL_ERROR("[Restore][OpsEdges]add edge from out data to in control failed."),
                HCCL_E_INTERNAL);
        }
        // Link the outputControlAnchor
        for (u32 anchorIndex = 0; anchorIndex < anchorInfosPerSeg.peerInControlAnchor.size(); anchorIndex++) {
            gRet = ge::GraphUtils::AddEdge(node->GetOutControlAnchor(), \
                anchorInfosPerSeg.peerInControlAnchor[anchorIndex]);
            bErr = (gRet != ge::GRAPH_SUCCESS);
...
HcclResult OpFusionBase::RemoveOpsEdges(ge::ComputeGraph& graph, std::vector<ge::NodePtr>& fusionOps, \
    u32 segmentNum, const std::vector<u32>& segmentIndex, std::vector<FusionAnchorInfo_t>& anchorInfos, \
    std::vector<ge::OpDescPtr>& fusedOps)
{
    ...
    for (uint32_t segmentIdx = 0; segmentIdx < segmentNum; segmentIdx++) {
        ...
        for (uint32_t idx = start; idx <= end; idx++) {
            ...
            CHK_RET(GetPeerOutDataToInData(anchorPtrSet, anchorInfosPreSeg.peerOutDataAnchor, fusionOps[idx],
                newOpDesc, duplication));
            CHK_RET(GetPeerOutDataToInControl(anchorPtrSet, anchorInfosPreSeg.peerOutDataToInControl, fusionOps[idx]));
            CHK_RET(GetPeerOutControlToInControl(anchorPtrSet, anchorInfosPreSeg.peerOutControlAnchor,
                fusionOps[idx]));
            CHK_RET(GetPeerAnchorFromOutData(anchorPtrSet, anchorInfosPreSeg.peerInDataAnchor, \
                anchorInfosPreSeg.peerInControlFromOutData, fusionOps[idx], newOpDesc, outDataAnchorIndex,
                duplication));
            CHK_RET(GetPeerInControlFromOutControl(anchorPtrSet, anchorInfosPreSeg.peerInControlAnchor,
                fusionOps[idx]));
            // remove the node after keeping all anchors
            ge::graphStatus geRet = graph.RemoveNode(fusionOps[idx]);
            CHK_PRT_RET((geRet != ge::GRAPH_SUCCESS), HCCL_ERROR("[Remove][OpsEdges]remove node[%s] failed. ret[%u]", \
                opName.c_str(), geRet), HCCL_E_INTERNAL);
            HCCL_DEBUG("fusionOps idx[%u] name[%s]", idx, opName.c_str());
        }
        anchorInfos.push_back(anchorInfosPreSeg);
        fusedOps.push_back(newOpDesc);
        HCCL_INFO("FusedOp[%s]:index[%u],peerOutDataAnchor[%u],peerOutDataToInControl[%u],peerOutControlAnchor[%u], "\
            "peerInDataAnchor[%u],peerInControlFromOutData[%u],peerInControlAnchor[%u], "\
            "inDescSize[%zu],outDescSize[%zu]", newOpDesc->GetName().c_str(), segmentIdx,
            anchorInfosPreSeg.peerOutDataAnchor.size(), anchorInfosPreSeg.peerOutDataToInControl.size(),
            anchorInfosPreSeg.peerOutControlAnchor.size(), anchorInfosPreSeg.peerInDataAnchor.size(),
            anchorInfosPreSeg.peerInControlFromOutData.size(), anchorInfosPreSeg.peerInControlAnchor.size(),
            newOpDesc->GetAllInputName().size(), newOpDesc->GetAllOutputsDescSize());
        start = end + 1;
    }
    return HCCL_SUCCESS;
}
    ...
    std::vector<FusionAnchorInfo_t>& anchorInfos, std::vector<ge::OpDescPtr>& fusedOps)
{
    ...
    for (uint32_t idx = start; idx < end; idx++) {
        ...
        CHK_RET(GetPeerAnchorFromOutData(anchorPtrSet, anchorInfosPreSeg.peerInDataAnchor, \
            anchorInfosPreSeg.peerInControlFromOutData, fusionOps[idx], newOpDesc, outDataAnchorIndex, duplication));
        ...
        CHK_PRT_RET((gRet != ge::GRAPH_SUCCESS), HCCL_ERROR("[Remove][OpsEdges]remove node[%s] failed. ret[%u]",
            opName.c_str(), gRet), HCCL_E_INTERNAL);
        HCCL_DEBUG("fusionOps idx[%u] name[%s]", idx, opName.c_str());
    }
    anchorInfos.push_back(anchorInfosPreSeg);
    fusedOps.push_back(newOpDesc);

    HCCL_INFO("FusedOp[%s]:peerOutDataAnchor[%u],peerOutDataToInControl[%u],peerOutControlAnchor[%u], "\
        "peerInDataAnchor[%u],peerInControlFromOutData[%u],peerInControlAnchor[%u],inDescSize[%zu], "\
        "outDescSize[%zu]", newOpDesc->GetName().c_str(), anchorInfosPreSeg.peerOutDataAnchor.size(),
...
HcclResult OpFusionBase::GetPeerOutDataToInData(std::unordered_set<uintptr_t>& anchorSet,
    std::vector<ge::OutDataAnchorPtr>& peerOutDataAnchorVec, ge::NodePtr& srcNodePtr, ge::OpDescPtr& dstOpDescPtr,
    std::vector<std::pair<int, int>> &duplication)
{
        ...
        if (anchorSet.count((uintptr_t)peerOutDataAnchor.get()) == 0) {
            ...
            if (dstOpName != srcOpName) {
                CHK_PRT_RET((dstOpDescPtr->AddInputDesc(inTensor) != ge::GRAPH_SUCCESS), \
                    ...
                    "failed.", inDataAnchor->GetIdx(), srcOpName.c_str(), dstOpName.c_str()), HCCL_E_INTERNAL);
            }
        } else {
...
HcclResult OpFusionBase::GetPeerAnchorFromOutData(std::unordered_set<uintptr_t>& anchorSet,
    ...
    ge::NodePtr& srcNodePtr, ge::OpDescPtr& dstOpDescPtr, int& index, std::vector<std::pair<int, int>> &duplication)
{
    HcclResult ret;
    ge::graphStatus gRet;
    CHK_SMART_PTR_NULL(srcNodePtr);
    ...
    for (auto outDataAnchor : srcNodePtr->GetAllOutDataAnchors()) {
        ...
        if (dstOpName != srcOpName) {
            ...
            if ((duplicationPosition < 0) || (outDataAnchor->GetPeerInControlAnchors().size() > 0)) {
                gRet = dstOpDescPtr->AddOutputDesc(outDataAnchor->GetOwnerNode()->GetOpDesc()->
                    GetOutputDesc(outDataAnchor->GetIdx()));
                if (gRet != ge::GRAPH_SUCCESS) {
                    HCCL_ERROR("[Get][PeerAnchor]GetPeerAnchorFromOutData: add outputDesc[%d] of srcOp[%s] to"
                        "destOp[%s] failed.", outDataAnchor->GetIdx(), srcOpName.c_str(), dstOpName.c_str());
                    return HCCL_E_INTERNAL;
                }
                index++;
            }
        ...
        CHK_RET(GetPeerInControlAnchorFromOutData(anchorSet, peerInControlFromOutDataVec, outDataAnchor, srcNodePtr,
            index));
    }
    return HCCL_SUCCESS;
}

HcclResult OpFusionBase::GetPeerInDataAnchorFromOutData(std::unordered_set<uintptr_t>& anchorSet,
    std::vector<std::pair<u32, ge::InDataAnchorPtr>>& peerInDataFromOutDataVec,
...
HcclResult OpFusionBase::GetPeerInControlAnchorFromOutData(std::unordered_set<uintptr_t>& anchorSet,
    std::vector<std::pair<u32, ge::InControlAnchorPtr>>& peerInControlFromOutDataVec,
    ge::OutDataAnchorPtr outDataAnchor, ge::NodePtr& srcNodePtr, int index)
{
    ge::graphStatus gRet;
    ...
    for (auto peerInControlAnchorFromData : outDataAnchor->GetPeerInControlAnchors()) {
        ...
        if (anchorSet.count((uintptr_t)peerInControlAnchorFromData.get()) == 0) {
            ...
            HCCL_INFO("[Get][PeerInControlAnchor]GetPeerAnchorFromOutData: remove edge between outDataAnchor[%d] "\
                ...
                outDataAnchor->GetIdx(), srcNodePtr->GetOpDesc()->GetName().c_str(),
                peerInControlAnchorFromData->GetIdx(),
...
HcclResult OpFusionBase::GetGroupFromOpDescPtr(const ge::NodePtr& nodePtr, std::string& group)
{
    auto opDescPtr = nodePtr->GetOpDesc();
    bool bErr = false;
    CHK_PRT_RET(!(ge::AttrUtils::HasAttr(opDescPtr, "group")), HCCL_ERROR("[Get][FusionOption]errNo[0x%016llx] \
        node[%s] has no attr \"group\".", HCOM_ERROR_CODE(HCCL_E_PARA), nodePtr->GetName().c_str()), HCCL_E_PARA);
    ...
    CHK_PRT_RET(!bErr, HCCL_ERROR("errNo[0x%016llx] get attr \"group\" failed. ", HCOM_ERROR_CODE(HCCL_E_PARA)), \
        HCCL_E_PARA);
    CHK_PRT_RET(group.empty(), HCCL_ERROR("errNo[0x%016llx] group from opDesc is empty.", \
