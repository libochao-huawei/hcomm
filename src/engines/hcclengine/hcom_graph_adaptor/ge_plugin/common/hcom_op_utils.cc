...
HcclResult HcomOpUtils::GetAivCoreLimit(const ge::OpDescPtr &op, const std::string &sCollectiveType, u32 &aivCoreLimit)
{
    ...
    if (ge::AttrUtils::HasAttr(op, "_op_vectorcore_num")) { // 核数优先取算子级配置
        ...
        if (bRet && aivNumStr != "") {
            ...
            HCCL_INFO("[HcomOpUtils][GetAivCoreLimit]node [%s] optype [%s] opVectorCoreNum [%u]",
                op->GetName().c_str(), sCollectiveType.c_str(), aivNum);
        ...
        if (ge::GetContext().GetOption("ge.hardwareInfo", hardwareInfoStr) == ge::GRAPH_SUCCESS) {
            ...
            if (pos != string::npos) {
                ...
                if (aivNumStr != "") {
                    s32 aivNum = 0;
                    CHK_RET(SalStrToInt(aivNumStr, HCCL_BASE_DECIMAL, aivNum));
                    HCCL_INFO("[HcomOpUtils][GetAivCoreLimit]node [%s] optype [%s] ge hardwareInfo [%u]",
                        op->GetName().c_str(), sCollectiveType.c_str(), aivNum);
                    aivCoreLimit = aivNum > 0 ? aivNum : aivCoreLimit;
                }
            }
    ...
    HCCL_INFO("[HcomOpUtils][GetAivCoreLimit] op[%s] get aivCoreLimit[%u] success.",
        sCollectiveType.c_str(), aivCoreLimit);
    return HCCL_SUCCESS;
}
