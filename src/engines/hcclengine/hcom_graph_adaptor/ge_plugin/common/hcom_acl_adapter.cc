    ...
    HcclRtMemcpyKind kind, rtStream_t stream)
{
    ...
    if (rtKind == ACL_MEMCPY_INNER_DEVICE_TO_DEVICE) {
        // 使用二级指针方式进行片内 D2D 拷贝
        // 1. 获取 MemcpyDesc 描述符占用内存大小
        size_t descSize;
        ret = aclrtGetMemcpyDescSize(rtKind, &descSize);
        HCCL_DEBUG("[hrtMemAsyncCopy] Call aclrtGetMemcpyDescSize, return value[%d], para: rtKind[%d], descSize[%llu]",
            ret, rtKind, descSize);
        // 2. 申请 Device 内存，用于存放 MemcpyDesc 描述符
        void *desc = nullptr;
        ret = aclrtMalloc(&desc, descSize, ACL_MEM_MALLOC_NORMAL_ONLY_P2P);
        HCCL_DEBUG("[hrtMemAsyncCopy] Call aclrtMalloc, return value[%d]", ret);
        // 3. 设置 MemcpyDesc 描述符，将源地址、目的地址、内存复制长度记录到内存复制描述符中
        ret = aclrtSetMemcpyDesc(desc, rtKind, const_cast<void *>(src), dst, count, nullptr);
        HCCL_DEBUG("[hrtMemAsyncCopy] Call aclrtSetMemcpyDesc, return value[%d]", ret);
        // 4. 执行片内 D2D 拷贝
        ret = aclrtMemcpyAsyncWithDesc(desc, rtKind, stream);
        HCCL_DEBUG("[hrtMemAsyncCopy] Call aclrtMemcpyAsyncWithDesc, return value[%d]", ret);
    } else {
        ret = aclrtMemcpyAsync(dst, destMax, src, count, rtKind, stream);
    }
    ...
    CHK_PRT_RET(ret != ACL_SUCCESS, HCCL_ERROR("[AsyncCopy][Mem] rt memory async copy failed, "\
        ...
        ret, dst, destMax, src, count, rtKind, stream), HCCL_E_RUNTIME);
...
HcclResult hrtMemSyncCopy(void *dst, uint64_t destMax, const void *src, uint64_t count, HcclRtMemcpyKind kind)
{
    ...
    CHK_PRT_RET(ret != ACL_SUCCESS, HCCL_ERROR("[SyncCopy][Mem] aclrtMemcpy failed, "\
        "return[%d], para: dstAddr[%p], destMax[%llu], srcAddr[%p], count[%llu], rtKind[%d].",\
        ret, dst, destMax, src, count, rtKind), HCCL_E_RUNTIME);
    return HCCL_SUCCESS;
#else
...
HcclResult hrtStreamGetMode(rtStream_t const stream, uint64_t *const stmMode)
{
    ...
    CHK_PRT_RET(ret != ACL_SUCCESS, HCCL_ERROR("[Stream][GetMode] aclrtGetStreamAttribute error, "
        "rtRet[%d]", ret), HCCL_E_RUNTIME);
...
HcclResult hrtFree(void *devPtr)
{
    ...
    CHK_PRT_RET((ret != ACL_SUCCESS), HCCL_ERROR("[Free][Mem] aclrtFree failed, "\
        "return[%d], para: devPtrAddr[%p].", ret, devPtr), HCCL_E_RUNTIME);
    return HCCL_SUCCESS;
#else
    CHK_PTR_NULL(devPtr);
    free(devPtr);
...
HcclResult hrtGetDeviceRefresh(s32 *deviceLogicId)
{
    ...
    CHK_PRT_RET(ret != ACL_SUCCESS, HCCL_ERROR("[Get][DeviceRefresh] rtGet device fail, "\
...
HcclResult hrtSetDevice(s32 deviceLogicId)
{
    ...
    HCCL_DEBUG("Call aclrtSetDevice, return value[%d], para: device_id[%d].",
        ret, deviceLogicId);
...
HcclResult hrtMalloc(void **devPtr, u64 size, bool level2Address)
{
    ...
    if (ret != ACL_SUCCESS) {
        ...
        HCCL_ERROR("[Malloc][Mem] rtMalloc failed, "\
            "return[%d], para: devPtrAddr[%p], size[%llu Byte], isTsMem[%d].", ret, *devPtr, size, isTsMem);
        return HCCL_E_RUNTIME;
    }

    s32 deviceId = 0;
    CHK_RET(hrtGetDeviceRefresh(&deviceId));
    HCCL_DEBUG("Malloc DevMem para: deviceId[%d] policy[0x%x] devPtr[%p] size[%llu Byte] "
...
// 后续替换aclrtMemcpyAsyncWithOffset
...
HcclResult hrtMallocHost(void **hostPtr, u64 size)
{
    ...
    if (ret != ACL_SUCCESS) {
        ...
        HCCL_ERROR("[Malloc][Host]errNo rt malloc host fail. return[%d], "\
            "para: hostPtr[%p], size[%llu Byte].", ret, *hostPtr, size);
        return HCCL_E_RUNTIME;
    }
...
HcclResult hrtFreeHost(void *hostPtr)
{
    ...
    if (ret != ACL_SUCCESS) {
        HCCL_ERROR("[Free][Host] rt free host fail. return[%d], "\
            "para: hostPtr[%p].", ret, hostPtr);
        return HCCL_E_RUNTIME;
    }
...
HcclResult hcclStreamSynchronize(void *stream)
{
    ...
    CHK_PRT_RET(ret != ACL_SUCCESS, HCCL_ERROR("[Synchronize][Stream]errNo[0x%016llx] rt "\
        "aclrtSynchronizeStream fail. return[%d].", HCCL_ERROR_CODE(HCCL_E_RUNTIME), ret), HCCL_E_RUNTIME);
    return HCCL_SUCCESS;
#else
...
HcclResult hrtEventCreate(aclrtEvent *event)
{
    ...
    if (ret != ACL_SUCCESS) {
        ...
        HCCL_ERROR("[Create][Event]errNo[0x%016llx] rt event create, return[%d], "\
            "event[%p]", HCCL_ERROR_CODE(HCCL_E_RUNTIME), ret, event);
        return HCCL_E_RUNTIME;
    }
