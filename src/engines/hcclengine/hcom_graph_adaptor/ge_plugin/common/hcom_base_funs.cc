...
HcclResult GetCCLBufferAvailableSize(u64 &size)
{
    ...
    size = static_cast<u64>(cclBufferSize * HCCL_CCL_COMM_FIXED_CALC_BUFFER_SIZE) - CCL_COMM_INBUFFER_UNALIGNED_RESERVE_SIZE;
    return HCCL_SUCCESS;
}
...
HcclResult HcomLoadRanktableFile(const std::string &rankTablePath, std::string &rankTableM)
{
    ...
    if (!CheckFilePath(rankTablePath, fileType)) {
        HCCL_ERROR("[Load][File] path %s is not a valid %s file",
            rankTablePath.c_str(), fileType.c_str());
        return HCCL_E_PARA;
    }
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[Load][File] get file[%s] real path error",
        rankTablePath.c_str()), HCCL_E_PARA);
    ...
    do {
        if ((std::chrono::steady_clock::now() - startTime) >= TIMEOUT) {
            HCCL_ERROR("[Load][File] Load ranktable file[%s] timeout[%lld]s",
                realFilePath.c_str(), TIMEOUT);
            return HCCL_E_TIMEOUT;
        }
        // 读取文件内容
        ret = ReadFile(realFilePath, fileContent);
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[Load][File]read file[%s] error", realFilePath.c_str()), HCCL_E_PARA);
