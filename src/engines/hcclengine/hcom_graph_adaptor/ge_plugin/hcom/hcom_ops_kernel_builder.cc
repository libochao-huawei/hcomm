...
ge::Status HcomOpsKernelBuilder::CalcOpRunningParam(ge::Node& node)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[Calc][OpRunningParam]errNo[0x%016llx] GetSuperKernelFromDesc failed.",
    ...
    if (superKernel!="") { 
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[Calc][OpRunningParam]errNo[0x%016llx] GetRootGraphID failed.",
                HCOM_ERROR_CODE(ret)), ge::INTERNAL_ERROR);
...
HcclResult HcomOpsKernelBuilder::GetSuperKernelFromDesc(const ge::OpDescPtr &op, std::string &superKernel)
{
    ...
    if (ge::AttrUtils::HasAttr(op, "_super_kernel_scope")) {
        if (ge::AttrUtils::GetStr(op, "_super_kernel_scope", superKernel) == false) {
            HCCL_ERROR("[Get][superKernel]errNo[0x%016llx]: get superKernel failed. get \"superKernel\" from"
                "opDesc failed", HCOM_ERROR_CODE(HCCL_E_PARA));
            return HCCL_E_PARA;
        }
    }
    ...
    std::vector<domi::TaskDef> &taskDefList)
{
    ...
    HCCL_RUN_INFO("GenerateTask: graph[%u], node[%s]-hash[%zu], opType[%s], opID[%d], comm[%lld], group[%s], "
        "srcRank[%u], dstRank[%u], srTag[%u], dataType[%s], aivCoreLimit[%u].", privateDefBuf.graphId, nodeName.c_str(),
        ...
        GetDataTypeEnumStr(privateDefBuf.dataType).c_str(), privateDefBuf.aivCoreLimit);
...
HcclResult HcomOpsKernelBuilder::HcomCalcOpRunningParam(ge::Node& node)
{
    ...
    if (ge::AttrUtils::SetInt(node.GetOpDesc(), "_hccl_task_num", taskNum) == false) {
        HCCL_ERROR("[HcomCalc][OpRunningParam]errNo[0x%016llx] op[%s]: set _hccl_task_num to OpDesc failed.",
                    HCCL_E_PARA, hcomOpParam.opType);
        return HCCL_E_PARA;
    }
    ...
    HCCL_KERNEL_INFO_PRIVATE_DEF &privateDefBuf, domi::TaskDef &taskDef)
{
    ...
    ret = memcpy_s(static_cast<int64_t *>(static_cast<void *>(static_cast<s8 *>(privateDefPtr) +\
