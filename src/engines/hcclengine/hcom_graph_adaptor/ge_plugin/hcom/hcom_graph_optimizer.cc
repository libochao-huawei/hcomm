...
std::map<HcclCMDType, std::pair<std::string, std::string>> AivSuperKernelMap = {
    {HcclCMDType::HCCL_CMD_ALLTOALL, {"/hccl_a2a_superkernel",  "sk_alltoall"}},
    {HcclCMDType::HCCL_CMD_ALLGATHER, {"/hccl_ag_superkernel",  "sk_allgather"}},
    {HcclCMDType::HCCL_CMD_REDUCE_SCATTER, {"/hccl_rs_superkernel", "sk_reduce_scatter"}},
    {HcclCMDType::HCCL_CMD_ALLREDUCE, {"/hccl_ar_superkernel", "sk_allreduce"}},
};
...
HcclResult HcomGraphOptimizer::SetUnknownShapeAttr(ge::ComputeGraph &graph, bool uknownShapeGraph)
{
    ...
    for (auto nodePtr : graph.GetAllNodes()) {
        ...
        CHK_RET(SetHcomOpParam(*nodePtr, &hcomOpParam, sCollectiveType, sGroup, socVersion,
            sendCountMatrix, sendCounts, sendDispls, recvCounts, recvDispls,
            curRanks, rankTableStr, rankTableM));
...
HcclResult HcomGraphOptimizer::SetSuperKernelScopeAttr(ge::ComputeGraph &graph)
{
    ...
    for (auto nodePtr : graph.GetAllNodes()) {
        ...
        bool isSupportOP = HCCL_SUPER_KERNEL_OP_TYPE_MAP.find(opType) != HCCL_SUPER_KERNEL_OP_TYPE_MAP.end();
        u8 deterministic = DETERMINISTIC_DISABLE;
        CHK_RET(GetDeterministic(deterministic));
        if (!isSupportOP || optionFeatureBaseRefreshable_ == 1 || deterministic != DETERMINISTIC_DISABLE) {
            HCCL_WARNING("super kernel not support opType[%d] optionFeatureBaseRefreshable_[%d]", opType, optionFeatureBaseRefreshable_);
            opDescPtr->DelAttr("_super_kernel_scope");
            continue;
        }

        ret = HcomOpUtils::ConversionOpDataType(opDescPtr, sCollectiveType, dataType);
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[Get][SetSuperKernelScopeAttr]op[%s]: get data type failed. ret[%d]",
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[Get][SetSuperKernelScopeAttr]op[%s]: get count failed. ret[%d]",
            sCollectiveType.c_str(), ret), ret);
        ...
        CHK_RET(HcomSelectAlg(comm, group.c_str(), count, dataType, reduction, opType, &ifAiv, algName, true));
        ...
        if (!ifAiv) {
            ...
            auto it = AivSuperKernelMap.find(opType);
            if (it != AivSuperKernelMap.end()) {
                // 写入bin路径和算法名称
            ...
            CHK_PRT_RET(aivCoreLimit < rankSize, HCCL_ERROR("[Get][SetSuperKernelScopeAttr]aivCoreLimit[%u] is less than rankSize[%u]",
                aivCoreLimit, rankSize), HCCL_E_PARA);
            
            // 将BlockDim包装成Hcom层接口，层层下发参数，到Hccl communictor里计算结果并返回到这一层。
            u32 originBlockDim;
            CHK_RET(HcomCalcAivCoreNum(group.c_str(), opType, count, dataType, algName, &originBlockDim));

            u32 blockDim = min(aivCoreLimit, originBlockDim);
            if (aivCoreLimit < originBlockDim) {
                blockDim = aivCoreLimit / rankSize * rankSize;
            }
            HCCL_INFO("[HcomGraphOptimizer][SetSuperKernelScopeAttr] rankSize[%u] aivCoreLimit[%u] blockDim[%u]",
                rankSize, aivCoreLimit, blockDim);

            if (opDescPtr->GetType() == HCCL_KERNEL_OP_TYPE_ALLREDUCE) { // allreduce现在不支持多核，固定ranksize核
                ge::AttrUtils::SetInt(opDescPtr, "hcom_block_dim", rankSize);
            } else if (opDescPtr->GetType() == HCCL_KERNEL_OP_TYPE_REDUCESCATTER) {
                CHK_PRT_RET(aivCoreLimit < originBlockDim, HCCL_ERROR("[Get][SetSuperKernelScopeAttr]aivCoreLimit[%u] is less than need[%u]",
                    aivCoreLimit,  originBlockDim), HCCL_E_PARA);
                ge::AttrUtils::SetInt(opDescPtr, "hcom_block_dim", originBlockDim);
            } else {
    ...
    if (sCollectiveType == HCCL_KERNEL_OP_TYPE_RECEIVE) {
        ...
        for (u64 i  = 0; i < op->GetInputsSize(); i++) {
            ...
            if (sCollectiveType == HCCL_KERNEL_OP_TYPE_REDUCESCATTERV) {
                ...
                CHK_PRT_RET((shapeSize > INVALID_U64 / dataTypeSize), HCCL_ERROR("[Get][Count]op[%s] "
                    "shape size[%llu] is overflow.", sCollectiveType.c_str(), shapeSize), HCCL_E_PARA);
...
{
    HCCL_DEBUG("[HcomGraphOptimizer][CalcHCCLOutputMemSize] sCollectiveType[%s] memSize[%lld]",
...
HcclResult HcomGraphOptimizer::SetOpMemAttr(ge::Node& node, const std::string &sCollectiveType, const u64 &opMemSize)
{
    ...
    bool withoutImplCompile = IsOfflineCompilation() ||
    ...
    // 板卡推理不需要设置申请内存为p2p
    u32 memType = 0;
    // u32 p2pMemType = static_cast<int>(ACL_MEM_TYPE_LOW_BAND_WIDTH) | static_cast<int>(ACL_MEM_MALLOC_NORMAL_ONLY_P2P);
    ...
    if (memType == p2pMemType) {
        ...
        CHK_PRT_RET(!ret,
            ...
            HCCL_E_PARA, sCollectiveType.c_str()), HCCL_E_PARA);
...
HcclResult HcomGraphOptimizer::HcomCalcOpRunningParam(ge::Node& node, bool uknownShapeGraph)
{
    ...
    CHK_RET(SetHcomOpParam(node, &hcomOpParam, sCollectiveType, sGroup, socVersion,
        sendCountMatrix, sendCounts, sendDispls, recvCounts, recvDispls,
        curRanks, rankTableStr, rankTableM));
    ...
    if (ge::AttrUtils::SetInt(node.GetOpDesc(), "used_stream_num", hcomResResponse.streamNum) == false) {
        HCCL_ERROR("[Calc][OpRunningParam]errNo[0x%016llx] op[%s]: set stream number[%llu] to OpDesc failed.",
                   HCCL_E_PARA, hcomOpParam.opType, hcomResResponse.streamNum);
        return HCCL_E_INTERNAL;
    }
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[Calc][OpRunningParam]errNo[0x%016llx] set op[%s] output size failed.",
            ret,  hcomOpParam.opType), HCCL_E_INTERNAL);
    ...
    HCCL_INFO("[Calc][OpRunningParam] node[%s] calculate hccl runing parameters completed. stream num:[%llu], workspace size:[%llu]bytes",
...
HcclResult HcomGraphOptimizer::SetHcomOpParam(const ge::Node& node, HcomOpParam *hcomOpParam,
    std::string &sCollectiveType, std::string &sGroup, std::string &socVersion,
    std::vector<int64_t> &sendCountMatrix, std::vector<int64_t> &sendCounts, std::vector<int64_t> &sendDispls,
    ...
    std::vector<u32> &curRanks, std::string &rankTableStr, std::string &rankTableM)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[Calc][OpRunningParam]errNo[0x%016llx] op type[%s] is not supported.",
        ret, sCollectiveType.c_str()), HCCL_E_NOT_SUPPORT);
    ...
    if (sCollectiveType == HCCL_KERNEL_OP_TYPE_REDUCESCATTERV) {
        std::vector<int64_t> sendCnts;
        std::vector<int64_t> sDispls;
        std::vector<int64_t> recvCnt;
        CHK_RET(HcomOpUtils::GetReduceScatterVCountsDispl(const_cast<ge::Node&>(node), sendCnts, sDispls, recvCnt));
        if (!sendCnts.empty()){
            auto maxIt = std::max_element(sendCnts.begin(), sendCnts.end());
            u64 maxCount = *maxIt;
            hcomOpParam->count = maxCount;
        }
    }
    ...
    if ((sCollectiveType == HCCL_KERNEL_OP_TYPE_REDUCESCATTER) || (sCollectiveType == HCCL_KERNEL_OP_TYPE_ALLGATHER)) {
        CHK_PRT_RET((!ge::AttrUtils::GetInt(node.GetOpDesc(), HCOM_ATTR_RANK_SIZE, rankSize)),
                        HCCL_ERROR("[Get][OpWorkspaceMemSize]op[%s] get  attr[%s] failed.",
                        sCollectiveType.c_str(), HCOM_ATTR_RANK_SIZE.c_str()), HCCL_E_PARA);
    ...
    if (sCollectiveType == HCCL_KERNEL_OP_TYPE_ALLTOALLV) {
        ...
        if (ge::AttrUtils::HasAttr(op, "send_counts")) {
            ...
            CHK_RET(HcomOpUtils::GetAlltoAllCountsDispl(const_cast<ge::Node&>(node), sendCounts, sendDispls, recvCounts, recvDispls));
        }

        if (sendCounts.size() < rankSize) {
            HCCL_ERROR("[sendCounts] size[%u] is invalid, expect size: %llu",
                sendCounts.size(), rankSize);
            return HCCL_E_PARA;
        }
    ...
    if ((ge::GetThreadLocalContext().GetOption(ge::OPTION_EXEC_RANK_TABLE, rankTableStr) ==
            ge::GRAPH_SUCCESS) && !rankTableStr.empty()) {
    ...
    if ((ge::GetThreadLocalContext().GetOption(ge::OPTION_EXEC_RANK_TABLE_FILE, rankTablePath) ==
            ge::GRAPH_SUCCESS) && !rankTablePath.empty()) {
        ret = HcomLoadRanktableFile(rankTablePath, rankTableM);
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[GetRanktable] rankTablePath[%s]"
            "load rankTable error.", rankTablePath.c_str()), HCCL_E_INTERNAL);
        hcomOpParam->rankTable = const_cast<char*>(rankTableM.c_str());
    } else {
        HCCL_INFO("get rankTablePath failed");
    }

    HCCL_INFO("[SetHcomOpParam]HcomOpParam opType:[%s] dataType:[%s] count:[%llu] group:[%s] reduceOp:[%s] "
