...
HcclResult HcomReduceScatterFusion::Run(ge::ComputeGraph &graph)
{
    ...
    for (auto iterFusionInfos = fusionInfosTemp.begin(); iterFusionInfos != fusionInfosTemp.end(); iterFusionInfos++) {
        HCCL_INFO("graph[%s] fusionlabel[%s]: there are %zu HcomReduceScatter ops to be fused.",
    ...
    switch (fusionOption.fusionAttr) {
        ...
        default:
            ...
            REPORT_PREDEFINED_ERR_MSG("EI0003", std::vector<const char*>({"ccl_op", "parameter", "value", "tips"}), \
                ...
                ({"HcomReduceScatterFusion", "fusion", fusionValue.c_str(), "please check fusion setting"}));
    ...
    for (auto peerOutDataToInControl : inControlAnchor->GetPeerOutDataAnchors()) {
        ...
        if (gRet != ge::GRAPH_SUCCESS) {
            HCCL_ERROR("[Get][PeerOutData]GetPeerOutDataToInControl: remove edge between peer outDataAnchor[%d] of"
                ...
                peerOutDataToInControl->GetIdx(),
                peerOutDataToInControl->GetOwnerNode()->GetOpDesc()->GetName().c_str(), inControlAnchor->GetIdx(),
                srcNodePtr->GetOpDesc()->GetName().c_str(), gRet);
...
HcclResult HcomReduceScatterFusion::GetPeerAnchorFromOutData(std::vector<ge::InDataAnchorPtr> &peerInDataFromOutDataVec,
...
HcclResult HcomReduceScatterFusion::GetPeerInDataAnchorFromOutData(
    vector<ge::InDataAnchorPtr> &peerInDataFromOutDataVec, ge::OutDataAnchorPtr outDataAnchor, ge::NodePtr &srcNodePtr)
{
    ge::graphStatus gRet;
    ...
    ge::NodePtr &srcNodePtr)
{
    ...
    CHK_PRT_RET(!bErr,
        HCCL_ERROR("[Get][ReduceScatterOpInfo]errNo[0x%016llx] get attr rankSize failed.",
    ...
    HCCL_DEBUG("[Get][ReduceScatterOpInfo]node[%s], rankSize[%d], group[%s]", nodeName.c_str(), rankSize,
        group.c_str());
    return HCCL_SUCCESS;
}

HcclResult HcomReduceScatterFusion::AddFusionNode(ge::ComputeGraph &graph,
    std::vector<CommonNodeInfo> &nodeInfos, ReduceScatterFusionNodesInfo &fusionNodesInfo,
    ge::OpDescPtr &fusedOp)
{
...
HcclResult HcomReduceScatterFusion::AddSendDataSplitV(ge::ComputeGraph &graph,
    std::vector<CommonNodeInfo> &nodeInfos, ReduceScatterFusionNodesInfo &fusionNodesInfo)
{
    for (uint32_t idx = 0; idx < nodeInfos.size(); idx++) {
        ...
        CHK_PRT_RET((ret != HCCL_SUCCESS),
            HCCL_ERROR("[Add][SendDataSplitV]create node[%s] failed.", splitDimName.c_str()), HCCL_E_INTERNAL);
    ...
    ReduceScatterFusionNodesInfo &fusionNodesInfo)
{
    ...
    for (u32 nodeIndex = 0; nodeIndex < nodeInfos.size(); nodeIndex++) {
        for (ge::InDataAnchorPtr &peerIndataAnchor : nodeInfos[nodeIndex].peerInDataAnchor) {
            CHK_RET(
                AddOpsEdge(fusionNodesInfo.reducescatterFusionNodePtr->GetOutDataAnchor(static_cast<s32>(nodeIndex)),
                peerIndataAnchor));
        }
    ...
    std::vector<int32_t> nodeValue, std::vector<int64_t> dim, ge::ComputeGraph &graph)
{
    ...
    EXECEPTION_CATCH((tensorPtr = std::make_shared<ge::GeTensor>(tensorDesc,
        reinterpret_cast<uint8_t *>(nodeValue.data()), sizeof(int32_t) * nodeValue.size())), return HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(!nodePtr,
        HCCL_ERROR("[Create][Const]graph[%s] add node[%s] failed", graph.GetName().c_str(),
...
HcclResult HcomReduceScatterFusion::CreateSplitVNode(SplitVNodeInfo &splitvNodInfo, ge::ComputeGraph &graph)
{
    ...
    CHK_PRT_RET(!bErr,
        HCCL_ERROR("[Create][SplitV]node[%s] set attr: dynamicOutput[%s] failed", splitVOpDescPtr->GetName().c_str(),
        dynamicOutputName.c_str()),
        HCCL_E_INTERNAL);
