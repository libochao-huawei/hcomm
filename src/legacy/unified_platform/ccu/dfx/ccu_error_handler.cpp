...
CcuMissionContext CcuErrorHandler::GetCcuMissionContext(int32_t deviceId, uint32_t dieId, uint32_t missionId)
{
    ...
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuMissionContext] tlvHandle is nullptr, deviceId[%d].", deviceId);
        return {};
    }
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuMissionContext] failed, ret[%d], deviceId[%d], dieId[%u], missionId[%u].",
...
CcuLoopContext CcuErrorHandler::GetCcuLoopContext(int32_t deviceId, uint32_t dieId, uint32_t loopCtxId)
{
    auto tlvHandle = HccpTlvHdcManager::GetInstance().GetTlvHandle(deviceId);
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuLoopContext] tlvHandle is nullptr, deviceId[%d].", deviceId);
        return {};
    }
    struct CustomChannelInfoIn  inBuff;
    struct CustomChannelInfoOut outBuff;
    ...
    auto ret = HrtRaTlvRequestForCustomChannel(tlvHandle, &inBuff, &outBuff);
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuLoopContext] failed, ret[%d], deviceId[%d], dieId[%u], loopCtxId[%u].",
...
uint64_t CcuErrorHandler::GetCcuXnValue(int32_t deviceId, uint32_t dieId, uint32_t xnId)
{
    auto tlvHandle = HccpTlvHdcManager::GetInstance().GetTlvHandle(deviceId);
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuXnValue] tlvHandle is nullptr, deviceId[%d].", deviceId);
        return 0;
    }
    struct CustomChannelInfoIn  inBuff;
    struct CustomChannelInfoOut outBuff;
    ...
    auto ret = HrtRaTlvRequestForCustomChannel(tlvHandle, &inBuff, &outBuff);
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuXnValue] failed, ret[%d], deviceId[%d], dieId[%u], xnId[%u].",
...
uint64_t CcuErrorHandler::GetCcuGSAValue(int32_t deviceId, uint32_t dieId, uint32_t gsaId)
{
    auto tlvHandle = HccpTlvHdcManager::GetInstance().GetTlvHandle(deviceId);
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuGSAValue] tlvHandle is nullptr, deviceId[%d].", deviceId);
        return 0;
    }
    struct CustomChannelInfoIn  inBuff;
    struct CustomChannelInfoOut outBuff;
    ...
    auto ret = HrtRaTlvRequestForCustomChannel(tlvHandle, &inBuff, &outBuff);
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuGSAValue] failed, ret[%d], deviceId[%d], dieId[%u], gsaId[%u].",
...
uint16_t CcuErrorHandler::GetCcuCKEValue(int32_t deviceId, uint32_t dieId, uint32_t ckeId)
{
    auto tlvHandle = HccpTlvHdcManager::GetInstance().GetTlvHandle(deviceId);
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuCKEValue] tlvHandle is nullptr, deviceId[%d].", deviceId);
        return 0;
    }
    struct CustomChannelInfoIn  inBuff;
    struct CustomChannelInfoOut outBuff;
    ...
    auto ret = HrtRaTlvRequestForCustomChannel(tlvHandle, &inBuff, &outBuff);
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuErrorHandler][GetCcuCKEValue] failed, ret[%d], deviceId[%d], dieId[%u], ckeId[%u].",
