...
void CtxMgrImp::LoadInstruction(CcuRep::CcuInstrInfo &instrInfo, uint32_t dieId)
{
    ...
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CtxMgrImp][LoadInstruction] tlvHandle is nullptr, deviceLogicId[%d].", deviceLogicId_);
        return;
    }
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CtxMgrImp][LoadInstruction] failed, ret[%d], deviceLogicId[%d], dieId[%u], startInstrId[%u].",
