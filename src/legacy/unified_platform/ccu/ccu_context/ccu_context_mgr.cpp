...
{
    TRY_CATCH_RETURN(
        HCCL_INFO("[GetTaskParam] Input params: deviceLogicId[%d], executorId[%u], taskParam size[%u]", deviceLogicId, executorId, taskParam.size());
        // 入参校验拦截
        CHK_PRT_RET((deviceLogicId < 0 || static_cast<u32>(deviceLogicId) >= MAX_MODULE_DEVICE_NUM),
...
{
    TRY_CATCH_RETURN(
        HCCL_INFO("[GetProfilingInfo] Input params: deviceLogicId[%d], entityId[%u], profilingInfo size[%u]", deviceLogicId, entityId, profilingInfo.size());
        // 入参校验拦截
        CHK_PRT_RET((deviceLogicId < 0 || static_cast<u32>(deviceLogicId) >= MAX_MODULE_DEVICE_NUM),
