...
{
    TRY_CATCH_RETURN(
        HCCL_INFO("[GetTaskParam] Input params: deviceLogicId[%d], executorId[%u]", deviceLogicId, executorId);
        // 入参校验拦截
        CHK_PRT_RET((deviceLogicId < 0 || static_cast<u32>(deviceLogicId) >= MAX_MODULE_DEVICE_NUM),
...
{
    TRY_CATCH_RETURN(
        HCCL_INFO("[GetProfilingInfo] Input params: deviceLogicId[%d], entityId[%u]", deviceLogicId, entityId);
        // 入参校验拦截
        CHK_PRT_RET((deviceLogicId < 0 || static_cast<u32>(deviceLogicId) >= MAX_MODULE_DEVICE_NUM),
