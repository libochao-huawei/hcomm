...
{
    HCCL_INFO("[CcuGetChannelSpecNum] Input params: deviceLogicId[%u], dieId[%u], channelNum[%u]", deviceLogicId, dieId, channelNum);
    // 入参校验拦截
    CHK_PRT_RET((deviceLogicId < 0 || static_cast<u32>(deviceLogicId) >= MAX_MODULE_DEVICE_NUM),
...
{
    HCCL_INFO("[CcuSetTaskKill] Input params: deviceLogicId[%u]", deviceLogicId);
    // 入参校验拦截
    CHK_PRT_RET((deviceLogicId < 0 || static_cast<u32>(deviceLogicId) >= MAX_MODULE_DEVICE_NUM),
...
{
    HCCL_INFO("[CcuSetTaskKillDone] Input params: deviceLogicId[%u]", deviceLogicId);
    // 入参校验拦截
    CHK_PRT_RET((deviceLogicId < 0 || static_cast<u32>(deviceLogicId) >= MAX_MODULE_DEVICE_NUM),
