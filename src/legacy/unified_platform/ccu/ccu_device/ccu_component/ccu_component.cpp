...
void CcuComponent::ConfigMsIdToken()
{
    ...
    for (uint8_t dieId = 0; dieId < MAX_CCU_IODIE_NUM; dieId++) {
        ...
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[CcuComponent][ConfigMsIdToken] failed, ret[%d], devLogicId[%d], dieId[%u].", ret, devLogicId, dieId);
            continue;
        }
...
HcclResult CcuComponent::CleanDieCkes(const uint8_t dieId) const
{
    ...
    for (uint32_t startIdx = 0; startIdx < ckeNum; startIdx += MAX_CKE_DATA_ARRAY_SIZE) {
        ...
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[CcuComponent][CleanDieCkes] failed, ret[%d], devLogicId[%d], dieId[%u], startIdx[%u].", ret, devLogicId, dieId, startIdx);
        }
    }
...
void CcuComponent::SetProcess(CcuOpcodeType opCode) const
{
    ...
    for (uint8_t dieId = 0; dieId < MAX_CCU_IODIE_NUM; dieId++) {
        ...
        auto ret = HrtRaTlvRequestForCustomChannel(tlvHandle, static_cast<void *>(&inBuff), static_cast<void *>(&outBuff));
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[CcuComponent][SetProcess] failed, ret[%d], devLogicId[%d], dieId[%u].", ret, devLogicId, dieId);
        }
    }
}
