...
static bool CheckDieEnable(const int32_t devLogicId, const uint8_t dieId)
{
    ...
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CcuResSpecifications][CheckDieEnable] tlvHandle is nullptr, devLogicId[%d].", devLogicId);
        return false;
    }
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuResSpecifications][CheckDieEnable] failed, ret[%d], devLogicId[%d], dieId[%u].", ret, devLogicId, dieId);
        return false;
    }
    ...
    const CcuVersion ccuVersion)
{
    auto tlvHandle = HccpTlvHdcManager::GetInstance().GetTlvHandle(devLogicId);
    if (tlvHandle == nullptr) {
        HCCL_ERROR("[CcuResSpecifications][CheckResSpecifications] tlvHandle is nullptr, devLogicId[%d].", devLogicId);
        return {};
    }
    struct CustomChannelInfoIn  inBuff;
    struct CustomChannelInfoOut outBuff;
    ...
    auto ret = HrtRaTlvRequestForCustomChannel(tlvHandle, reinterpret_cast<void *>(&inBuff), reinterpret_cast<void *>(&outBuff));
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[CcuResSpecifications][CheckResSpecifications] failed, ret[%d], devLogicId[%d], dieId[%u].", ret, devLogicId, dieId);
        return {};
    }
