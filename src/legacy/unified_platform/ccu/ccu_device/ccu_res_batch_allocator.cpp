...
uint32_t CcuResBatchAllocator::GetPreAllocatedMaxBlockNum(const uint8_t dieId) const
{
    ...
    HCCL_INFO(
        "[CcuResBatchAllocator][%s] batch allocator will alloc [%u] "
        "blocks ccu resoures, devLogicId[%d] dieId[%u].",
        __func__, minBlockNum, devLogicId, dieId);
    return minBlockNum;
}
        ...
        for (auto& resReq : blockResReqs) {
            ...
            if (reqNum == 0) {
                HCCL_WARNING(
                    ...
                    __func__, devLogicId, i, resReq.first.Describe().c_str());
                continue;
            }
    ...
    const uintptr_t handleKey, const CcuResReq& resReq, std::unique_ptr<CcuResRepository>& resRepoPtr)
{
    ...
    for (uint8_t i = 0; i < MAX_CCU_IODIE_NUM; i++) {
        ...
        std::array<ResTypeReqNumBlockNumFunc, BLOCK_RES_TYPE_NUM> blockReqParas = {
            std::make_tuple(
                ResType::LOOP, resReq.blockLoopEngineReq[i], resStrategys[i].loopNum,
                std::ref(resRepoPtr->blockLoopEngine[i])),
            std::make_tuple(ResType::MS, resReq.blockMsReq[i], resStrategys[i].msNum, std::ref(resRepoPtr->blockMs[i])),
            std::make_tuple(
                ResType::CKE, resReq.blockCkeReq[i], resStrategys[i].ckeNum, std::ref(resRepoPtr->blockCke[i]))};
...
CcuResBatchAllocator::AllocConsecutiveRes(const CcuResReq& resReq, std::unique_ptr<CcuResRepository>& resRepoPtr) const
{
    ...
    for (uint8_t i = 0; i < MAX_CCU_IODIE_NUM; i++) {
        ...
        std::array<ResTypeReqNumResInfoTuple, CONS_RES_TYPE_NUM> reqParas
            = {std::make_tuple(ResType::XN, resReq.continuousXnReq[i], std::ref(resRepoPtr->continuousXn[i]))};
