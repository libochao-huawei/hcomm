...
bool UrmaDirectTransport::ConnVecUnpackProc(BinaryStream &binaryStream)
{
    ...
    HCCL_INFO("UrmaDirectTransport start unpack conn %s connNum=%u, rmtConnNum=%u", GetLinkDescInfo().c_str(), connNum,
        rmtConnNum);
    if (connNum != rmtConnNum) {
        MACRO_THROW(InvalidParamsException,
            StringFormat("UrmaDirectTransport connNum=%u is not equal to rmtConnNum=%u", connNum, rmtConnNum));
    }
    ...
    for (u32 i = 0; i < rmtConnNum; i++) {
        ...
        HCCL_INFO("UrmaDirectTransport unpack connection pos=%u dto %s", pos, rmtDto.Describe().c_str());
        if (commonLocRes.connVec[i]->GetStatus() != RmaConnStatus::READY) {
            HCCL_INFO("UrmaDirectTransport parse and import pos=%u, rmt dto to connection[%s]", pos,
