...
HcclResult HcclMemRegV2(HcclNetDev netDev, const HcclMem *mem, HcclBuf *buf)
{
    if (netDev == nullptr || mem == nullptr || buf == nullptr) {
        HCCL_ERROR("[%s] netDev[%p] or mem[%p] or buf[%p] is null", __FUNCTION__, netDev, mem, buf);
        return HCCL_E_PTR;
    }
    HCCL_INFO("[%s] Begin, addr[%p], size[%llu], type[%d]", __FUNCTION__, mem->addr, mem->size, mem->type);
    // 仅支持UB类型
    ...
    HCCL_INFO("[%s]End, addr[%p], size[%llu], handle[%p]", __FUNCTION__, buf->addr, buf->len, buf->handle);
    return HCCL_SUCCESS;
}
...
{
    if (buf == nullptr) {
        HCCL_ERROR("[%s]buf[%p] is null", __FUNCTION__, buf);
        return HCCL_E_PTR;
    }
    HCCL_INFO("[%s] Begin, addr[%p], size[%llu], handle[%p]", __FUNCTION__, buf->addr, buf->len, buf->handle);
...
{
    if (buf == nullptr || outDesc == nullptr || outDescLen == nullptr) {
        HCCL_ERROR("[%s] buf[%p] or outDesc[%p] or outDescLen[%p] is null", __FUNCTION__, buf, outDesc, outDescLen);
        return HCCL_E_PTR;
    }
    HCCL_INFO("[%s] Begin, addr[%p], size[%llu], handle[%p]", __FUNCTION__, buf->addr, buf->len, buf->handle);
    // 获取序列化信息
    ...
    *outDescLen = tempLocalMemDesc.size();
    ...
    HCCL_INFO("[%s]End, outDescLen[%llu]", __FUNCTION__, *outDescLen);
    return HCCL_SUCCESS;
}
...
HcclResult HcclMemImportV2(const char *description, uint64_t descLen, bool isRemote, HcclBuf *outBuf, HcclNetDev netDev)
{
    if (description == nullptr || outBuf == nullptr || netDev == nullptr) {
        HCCL_ERROR("[%s] description[%p] or outBuf[%p] or netDev[%p] is null", __FUNCTION__,
            description, outBuf, netDev);
        return HCCL_E_PTR;
    }
    (void)(isRemote);
    HCCL_INFO("[%s] Begin,  descLen[%llu]", __FUNCTION__, descLen);
    // 仅支持UB类型
    HcclNetDevice *hcclNetDevice = static_cast<HcclNetDevice *>(netDev);
    if (!hcclNetDevice->IsUB()) {
        HCCL_ERROR("[%s] only support UB", __FUNCTION__);
    ...
    HCCL_INFO("[%s]End, addr[%p], size[%llu], handle[%p]", __FUNCTION__, outBuf->addr, outBuf->len, outBuf->handle);
    return HCCL_SUCCESS;
}

HcclResult HcclMemCloseV2(HcclBuf *buf)
{
    if (buf == nullptr || buf->handle) {
        HCCL_ERROR("[%s] buf[%p] or buf->handle[%p] is null", __FUNCTION__,  buf, buf->handle);
        return HCCL_E_PTR;
    }
    HCCL_INFO("[%s] Begin, addr[%p], size[%llu], handle[%p]", __FUNCTION__, buf->addr, buf->len, buf->handle);
    // 仅支持UB类型
