...
LocalUbRmaBuffer::LocalUbRmaBuffer(std::shared_ptr<Buffer> buf, RdmaHandle rdmaHandle, HrtRaUbLocalMemRegOutParam memRegOutParam)
    : LocalRmaBuffer(buf, RmaType::UB, true), rdmaHandle(rdmaHandle), reqReg(memRegOutParam)
{
    if (rdmaHandle == nullptr) {
        THROW<NullPtrException>("LocalUbRmaBuffer's rdmaHandle is NULL");
    }
...
{
    return StringFormat("LocalUbRmaBuffer[rdmaHandle=%p, buf=%s, reqReg.handle=%p]",
                        rdmaHandle, buf->Describe().c_str(), reqReg.handle);
}
...
{
    if (rdmaHandle != nullptr && reqReg.handle != 0 && isAlias_ == false) {
