...
public:
    VirtualLocalRdmaRmaBuffer(std::shared_ptr<LocalRdmaRmaBuffer> real,
        ...
        : LocalRdmaRmaBuffer(
              std::make_shared<Buffer>(addr, size, real->GetBuf()->GetMemType(), real->GetBuf()->GetMemTag().c_str()),
              real->GetRdmaHandle(),
              real->GetLkey(),
              real->GetRkey()),
          real_(std::move(real)),
    ...
    std::unique_ptr<Serializable> GetExchangeDto() override
    {
        auto dto = std::make_unique<ExchangeRdmaBufferDto>(
            vAddr_, vSize_, real_->GetRkey(), real_->GetBuf()->GetMemTag().c_str());
        return std::unique_ptr<Serializable>(dto.release());
    }

    std::string Describe() const override
    {
        return StringFormat("VirtualLocalRdmaRmaBuffer[addr=0x%llx, size=%llu, real=%s]",
...
public:
    ...
    std::string Describe() const override
    {
        return StringFormat("VirtualLocalIpcRmaBuffer[addr=0x%llx, size=%llu, real=%s]",
                            static_cast<unsigned long long>(vAddr_),
                            static_cast<unsigned long long>(vSize_),
                            real_->Describe().c_str());
    }
...
public:
    ...
    std::string Describe() const override
    {
        return StringFormat("VirtualLocalUbRmaBuffer[addr=0x%llx, size=%llu, real=%s]",
                            static_cast<unsigned long long>(vAddr_),
                            static_cast<unsigned long long>(vSize_),
                            real_->Describe().c_str());
    }
