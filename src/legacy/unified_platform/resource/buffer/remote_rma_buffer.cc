...
RemoteIpcRmaBuffer::RemoteIpcRmaBuffer(const Serializable& rmtDto) : RemoteRmaBuffer(RmaType::IPC), isOpened(false)
{
    ...
    HCCL_INFO(
        "[RemoteIpcRmaBuffer][RemoteIpcRmaBuffer]ipcAddr[%llu] ipcOffset[%llu] ipcName[%s] memTag[%s]", ipcAddr,
        ipcOffset, ipcName, memTag.c_str());
      ...
      isOpened(true)
{
    ...
    (void)memcpy_s(ipcName, RTS_IPC_MEM_NAME_LEN, dto.name, RTS_IPC_MEM_NAME_LEN);
    HCCL_INFO(
        "[RemoteIpcRmaBuffer][RemoteIpcRmaBuffer] tag[%s] ipcAddr[%llu] ipcOffset[%llu] ipcName[%s] memTag[%s]",
        tag.c_str(), ipcAddr, ipcOffset, ipcName, memTag.c_str());
    ipcPtr = HrtIpcOpenMemory(ipcName);
    addr = reinterpret_cast<uintptr_t>(ipcPtr) + ipcOffset;
    isOpened = true;
}
...
{
    return StringFormat(
        "RemoteUbRmaBuffer[rdmaHandle=%p, addr=0x%llx, size=0x%llx, memHandle=%p segVa=%llu]", rdmaHandle, addr, size,
        memHandle, segVa);
}
