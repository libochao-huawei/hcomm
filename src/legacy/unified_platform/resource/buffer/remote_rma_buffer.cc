...
static std::vector<HrtRaUbRemMemImportParam> remoteMemImports;
...
HcclResult RemoteUbRmaBuffer::BatchMemReg(RdmaHandle rdmaHandle, std::vector<std::unique_ptr<RemoteRmaBuffer *>> &rmtBufs)
{
    CHK_PRT_RET(rmtBufs.size() != remoteMemImports.size(),
        ...
        HCCL_E_INTERNAL);
    std::vector<HrtRaUbRemMemImportOutParam> reqRegs;
