...
public:
    ...
    ExchangeUbBufferDto(u64 addr, u64 size, HcclMemType memType, u32 tokenValue, u32 tokenId, u32 keySize)
        : addr(addr), size(size), memType(memType), tokenValue(tokenValue), tokenId(tokenId), keySize(keySize)
    {
    }

    void Serialize(Hccl::BinaryStream &stream) override
    {
        stream << addr << size << memType << tokenValue << tokenId << keySize << key << segVa;
    }
    ...
    {
        return StringFormat(
            "ExchangeUbBufferDto[addr=0x%llx, size=0x%llx keySize=%u]", addr, size, keySize);
    }
