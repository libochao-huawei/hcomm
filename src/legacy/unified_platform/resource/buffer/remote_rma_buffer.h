...
public:
    ...
    virtual ~RemoteUbRmaBufferBase() = default;
...
public:
    explicit RemoteUbAggregatedRmaBuffer(const std::vector<RdmaHandle> &handles);

    RemoteUbAggregatedRmaBuffer(const std::vector<RdmaHandle> &handles, const Serializable &rmtDto);
