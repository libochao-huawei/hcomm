...
{
    HCCL_INFO("[P2PTransport::%s]does not support, p2p dst addr[%llu], stream[%p]", __func__, p2pMemcpy.GetDstAddr(), stream.GetPtr());
}

static void SubmitTask(const TaskSdmaReduce &sdmaReduce, const Stream &stream)
{
    HCCL_INFO("[P2PTransport::%s]does not support, sdmaReduce dst addr[%llu], stream[%p]", __func__, sdmaReduce.GetDstAddr(), stream.GetPtr());
}
