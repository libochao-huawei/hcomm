                                            ...
                                            UbRmtBufType type)
{
    ...
    for (u32 i = 0; i < rmtNum; i++) {
        ...
        if (dto.size == 0) { // size为0，则为 remote 空buffer
            ...
            importParams.push_back(
                dynamic_cast<RemoteUbRmaBuffer*>(bufferVec.back().get())->GetImportParam());
    ...
    HcclResult ret = Hccl::RemoteUbRmaBuffer::BatchMemImport(rdmaHandle, rmtUbRmaBuffer, importParams);
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[RemoteUbRmaBuffer][RmtBufferVecUnpackProc] BatchMemImport failed ret[%d]", ret);
        MACRO_THROW(InternalException,
            StringFormat("[RmtBufferVecUnpackProc] BatchMemImport failed ret[%d]", ret));
    }
