      ...
      isRecvFirst_(isRecvFirst)
{
    ...
    CHK_RET_THROW(
        InternalException, StringFormat("[UbMemTransport][UbMemTransport] failed to construct UbMemTransport."),
        result);
}
...
HcclResult UbMemTransport::FillTagVec(
    std::vector<LocalRmaBuffer*>& bufferVec, std::vector<std::array<char, HCCL_RES_TAG_MAX_LEN>>& tagVec)
{
...
void UbMemTransport::SendDataSize()
{
    ...
    HCCL_INFO(
        "notifyNum=%u, bufferNum=%u, connNum=%u, cntNotifyNum=%u, cntNotifyDescSize=%u", notifyNum, bufferNum, connNum,
        cntNotifyNum, cntNotifyDescSize);
...
{
    HCCL_INFO(
        "RecvDataProcess: link=%s, size=%llu, exchangeDataSize=%u", GetLinkDescInfo().c_str(), recvData.size(),
        exchangeDataSize);
...
HcclResult UbMemTransport::GetUserRemoteMem(CommMem** remoteMem, char*** memTags, uint32_t* memNum)
{
    ...
    RemoteMemCtx<std::unique_ptr<RemoteUbRmaBuffer>> remoteMemCtx{
        userMemCount, cacheValid_,  rmtBufferVec, remoteUserMemTag_, remoteUserMems_, tagCopies_,
        tagPointers_, cacheBuilder, remoteMem,    memTags,           memNum};
        ...
        if ((std::chrono::steady_clock::now() - startTime) >= timeout || socketStatus == Hccl::SocketStatus::TIMEOUT) {
            auto elapsed
                = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - startTime)
                      .count();
            HCCL_ERROR(
                "[UbMemTransport][%s] channel connect timeout after %lld sec, elapsed[%lld]ms, retryCount[%u]",
...
HcclResult UbMemTransport::UpdateMemInfo(std::vector<LocalRmaBuffer*>& bufferVecTemp)
{
    ...
    TRY_CATCH_RETURN([&]() -> void {
        ...
        socket->SendAsync(reinterpret_cast<u8*>(&sendSize), sizeof(sendSize));
        HCCL_INFO(
            "[UbMemTransport][UpdateMemInfo] Send size[%u] of data success. [%zu] bytes sent.", __func__, sendSize,
            sizeof(sendSize));
