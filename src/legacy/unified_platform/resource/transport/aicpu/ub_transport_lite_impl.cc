...
RmtRmaBufSliceLite UbTransportLiteImpl::GetRmtRmaBufSliceLite(const Buffer &rmtBuf)
{
    for (auto &it : rmtBufferVec) {
        ...
        HCCL_INFO("TESTZJN --- UbTransportLiteImpl::GetRmtRmaBufSliceLite rmtBuf before:%s", rmtBuf.Describe().c_str());
        if (buf.Contains(rmtBuf.GetAddr(), rmtBuf.GetSize())) {
            // ub conn lite 不关心rkey , rkey 设定为0
            HCCL_INFO("TESTZJN --- UbTransportLiteImpl::GetRmtRmaBufSliceLite rmtBuf end:%s", rmtBuf.Describe().c_str());
