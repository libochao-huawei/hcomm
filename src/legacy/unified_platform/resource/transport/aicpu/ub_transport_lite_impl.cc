...
static int32_t ParseData(const HcommBatchTransferDesc &transferDesc, void* &rmt, void* &loc,
                        uint64_t &len, Hccl::TransferType &tfType, HcommDataType &dataType, HcommReduceOp &reduceOp, Hccl::NotifyWaitInfo &notifyWaitInfo)
{
    ...
    const HcommBatchTransferDesc *transferDescs, uint32_t transferDescNum)
{
    ...
    std::vector<Hccl::NotifyWaitInfo> notifyWaitInfos;
    ...
    for (uint32_t i = 0; i < transferDescNum; i++) {
        ...
        Hccl::NotifyWaitInfo notifyWaitInfo{};
        ParseData(transferDescs[i], rmt, loc, len, tfType, dataType, reduceOp, notifyWaitInfo);
        CHK_PTR_NULL(rmt);
        CHK_PTR_NULL(loc);
