...
void UbTransportLiteImpl::Init(std::vector<char> &uniqueId)
{
    ...
    ParseLocBufferVec(locBufferUniqueIds, RmaUbBufType::BUFFER);
...
void UbTransportLiteImpl::ParseLocBufferVec(std::vector<char> &data, RmaUbBufType rmtType)
{
    u32 num = 0;
    if (rmtType == RmaUbBufType::NOTIFY) {
        num = notifyNum;
...
HcclResult UbTransportLiteImpl::BuildLocRmaBufferLite(const uintptr_t addr, const size_t size, RmaBufferLite &rmaBufferLite)
{
    ...
    if (!isAddrInRange) {
        HCCL_ERROR("[UbTransportLiteImpl::%s] addr[0x%llx], size[0x%llx] not in any range of locBufferVec. plase check",
            __func__, addr, size);
