...
static HcclResult CheckReduceHcommDataTypeAndHcommReduceOp(HcommDataType dataType, HcommReduceOp reduceOp)
{
    ...
    if (dataType == HcommDataType::HCOMM_DATA_TYPE_RESERVED) { // reduce操作中，dataType不能是Reserved
        HCCL_ERROR("[%s] op[%u] is not supported.", __func__, reduceOp);
        return HCCL_E_PARA;
    }

    if (reduceOp == HcommReduceOp::HCOMM_REDUCE_RESERVED) {   // reduce操作中，reduceOp不能是Reserved
        HCCL_ERROR("[%s] type[%u] is not supported.", __func__, dataType);
        return HCCL_E_PARA;
    }
...
static HcclResult ParseData(const HcommBatchTransferDesc &transferDesc, void* &rmt, void* &loc,
                        uint64_t &len, Hccl::TransferType &tfType, HcommDataType &dataType, HcommReduceOp &reduceOp, u32 &notifyIdx)
{
    if (transferDesc.transType == HCOMM_TRANSFER_TYPE_WRITE) {
        ...
        notifyWaitInfo.index_ = transferDesc.transferInfo.writeWithNotify.notifyIdx;
        ...
        reduceOp = transferDesc.transferInfo.reduwriteReduceWithNotifyce.reduceOp;
        notifyWaitInfo.index_ = transferDesc.transferInfo.reduwriteReduceWithNotifyce.notifyIdx;
        tfType = Hccl::TransferType::WRITE_REDUCE_WITH_NOTIFY;
        CHK_RET(CheckReduceHcommDataTypeAndHcommReduceOp(dataType, reduceOp));
    } else if (transferDesc.transType == HCOMM_TRANSFER_TYPE_NOTIFY_RECORD) {
        notifyWaitInfo.index_ = transferDesc.transferInfo.notifyRecord.notifyIdx;
        tfType = Hccl::TransferType::NOTIFY_RECORD;
    // } else if (transferDesc.transType == HCOMM_TRANSFER_TYPE_NOTIFY_WAIT) {
    //     notifyWaitInfo.index_ = transferDesc.transferInfo.notifyWait.notifyIdx;
    //     notifyWaitInfo.timeout = transferDesc.transferInfo.notifyWait.timeout;
    //     tfType = Hccl::TransferType::NOTIFY_WAIT;
    // } else if (transferDesc.transType == HCOMM_TRANSFER_TYPE_NOTIFY_WAIT_WITH_DEFAULT_TIMEOUT) {
    //     notifyWaitInfo.index_ = transferDesc.transferInfo.reduwriteReduceWithNotifyce.notifyIdx;
    //     tfType = Hccl::TransferType::NOTIFY_WAIT_WITH_DEFAULT_TIMEOUT;
    } else {
        HCCL_ERROR("[%s] unsupported transType[%d]", __func__, transferDesc.transType);
        return HCCL_E_PARA;
    }
    auto ret = CheckHcommDataTypeAndHcommReduceOp(dataType, reduceOp); // 统一校验是否在枚举中
...
constexpr u32       NOTIFYIDX_INVALID_VALUE  = 0xFFFFFFFF; // NOTIFY idex非法值
    ...
    const HcommBatchTransferDesc *transferDescs, uint32_t transferDescNum)
{
    ...
    for (uint32_t i = 0; i < transferDescNum; i++) {
        ...
        u32 notifyIdx = NOTIFYIDX_INVALID_VALUE;
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[%s] FAIL at CheckDataTypeAndReduceOp for index %u. rmt[0x%llx], loc[0x%llx], len[0x%llx], tfType[%u], dataType[%d], reduceOp[%d].",
            __func__, i, rmt, loc, len, tfType, dataType, reduceOp), ret);
        ...
        HCCL_INFO("[%s] Prepared transfer op for index %u. rmt[0x%llx], loc[0x%llx], len[0x%llx], tfType[%u], dataType[%d], reduceOp[%d].",
            __func__, i, rmt, loc, len, tfType, dataType, reduceOp);
    }
    EXECEPTION_CATCH(BatchTransferAll(locSlices, rmtSlices, transferOps, notifyIdxs, *streamLitePtr), return HCCL_E_INTERNAL);
    return HCCL_SUCCESS;
}
...
void UbTransportLiteImpl::BatchTransferAll(const std::vector<RmaBufferLite> &loc, const std::vector<Buffer> &rmt,
    const std::vector<BaseTransportLiteImpl::TransferOp> &transferOp, const std::vector<u32> &notifyIdxs, const StreamLite &stream)
{
    if (UNLIKELY(loc.empty())) {
        return;
    }
    ...
    for (u32 i = 0; i < insNum; i++) {
        ...
        auto localBuffer  = GetRmaBufSlicelite(loc[i]);
        auto remoteBuffer = GetRmtRmaBufSliceLite(rmt[i]);
        auto rmtNotifySliceLite = GetRmtNotifySliceLite(notifyIdxs[i]);
        auto rmtBuffSliceLite = GetRmtNotifySliceLite(notifyIdxs[i]);
