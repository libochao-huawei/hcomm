...
static HcclResult CheckReduceHcommDataTypeAndHcommReduceOp(HcommDataType dataType, HcommReduceOp reduceOp)
{
    ...
    if (reduceOpIt == mapHcommReduceOpA5.end() || reduceOpIt->first == HcommDataType::HCOMM_DATA_TYPE_RESERVED) {
                                    ...
                                    HcommDataType &dataType, HcommReduceOp &reduceOp)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("FAIL at CheckHcommDataTypeAndHcommReduceOp dataType[%d], reduceOp[%d].", dataType, reduceOp), ret);
    return HCCL_SUCCESS;
}
