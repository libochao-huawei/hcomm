...
void UbTransportLiteImpl::Init(std::vector<char> &uniqueId)
{
    ...
    ParseLocBufferVec(locBufferUniqueIds, locBufferVec, locBufferMap, RmaUbBufType::BUFFER);
...
void UbTransportLiteImpl::ParseLocBufferVec(std::vector<char> &data, LocUbBufLite &vec, LocUbBufLiteMap &map, RmaUbBufType rmtType) const
{
    u32 num = 0;
    if (rmtType == RmaUbBufType::NOTIFY) {
        num = notifyNum;
    ...
    for (u32 idx = 0; idx < num; idx++) {
        ...
        HCCL_INFO("idx=%u, %s %s", idx, rmtType.Describe().c_str(), ubBufLite.Describe().c_str());
        vec.push_back(ubBufLite);
        map[static_cast<uintptr_t>(ubBufLite.addr)] = ubBufLite;
    }
}
