...
static HcclResult ParseData(const HcommBatchTransferDesc &transferDesc, void* &rmt, void* &loc,
                        uint64_t &len, Hccl::TransferType &tfType, HcommDataType &dataType, HcommReduceOp &reduceOp, NotifyWaitInfo &notifyWaitInfo)
{
    ...
    const HcommBatchTransferDesc *transferDescs, uint32_t transferDescNum)
{
    ...
    std::vector<NotifyWaitInfo> notifyWaitInfos;
    ...
    for (uint32_t i = 0; i < transferDescNum; i++) {
        ...
        NotifyWaitInfo notifyWaitInfo{};
        CHK_RET(ParseData(transferDescs[i], rmt, loc, len, tfType, dataType, reduceOp, notifyWaitInfo));
        CHK_PTR_NULL(rmt);
        CHK_PTR_NULL(loc);
        notifyWaitInfos.push_back(notifyWaitInfo);
    ...
    EXECEPTION_CATCH(BatchTransferAll(locSlices, rmtSlices, transferOps, notifyWaitInfos, *streamLitePtr), return HCCL_E_INTERNAL);
    return HCCL_SUCCESS;
}
