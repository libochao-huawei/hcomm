    ...
    RmtRmaBufferSlice& remoteRmaBufferSlice)
{
    ...
    HCCL_INFO(
        "[TransportUrmaMem][FillRmaBufferSlice] Local [%p], buff[%lu], offset[%u], after mapping is [%llu], Datasize "
        "is [%llu].",
        localAddr, localBuffer.second->GetBuf()->GetAddr(), localDataOffSet, localRmaBufferSlice.addr, byteSize);

    HCCL_INFO(
        "[TransportUrmaMem][FillRmaBufferSlice] rmt [%p], buff[%lu], offset[%u], after mapping is [%llu], Datasize is "
        "[%llu].",
