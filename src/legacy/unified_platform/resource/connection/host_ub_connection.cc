...
void HostUbConnection::SetWqInfo(HcclAiRMAWQ &wq)
{
    ...
    wq.tp_id = tpn;
    CHK_SAFETY_FUNC_RET(memcpy_s(wq.rmtEid, sizeof(wq.rmtEid), rmtEid.raw, sizeof(wq.rmtEid)));
}
...
std::unique_ptr<Serializable> HostUbConnection::GetExchangeDto()
{
    ...
    CHK_SAFETY_FUNC_RET(memcpy_s(dto->qpKey, HRT_UB_QP_KEY_MAX_LEN, repJetty_.key, HRT_UB_QP_KEY_MAX_LEN));
...
void HostUbConnection::ParseRmtExchangeDto(const Serializable &rmtDto)
{
    ...
    CHK_SAFETY_FUNC_RET(memcpy_s(remoteQpKey, HRT_UB_QP_KEY_MAX_LEN, dto.qpKey, HRT_UB_QP_KEY_MAX_LEN));
