...
HcclResult Ipv4ToTpArray(const char *ipv4Str, uint8_t ipArr[16U])
{
    if (ipv4Str == NULL || ipArr == NULL) {
        HCCL_WARNING("[Ipv4ToTpArray] ipv4Str or ipArr is null");
        return HCCL_E_PARA;
    }
    ...
    if (ret != 1) {
        HCCL_WARNING("[%s] Failed to convert the ipv4Str[%s] to ipArr.", __func__, ipv4Str);
        return HCCL_E_PARA;
    }
...
HcclResult DevUbConnection::SetTpAttrAsync()
{
    ...
    */
    ...
    CHK_RET(Ipv4ToTpArray(localIp, tpAttr.sip));
    ...
    CHK_RET(Ipv4ToTpArray(rmtIp, tpAttr.dip));
