...
bool DevUbConnection::GetTpInfo()
{
    ...
    auto ret = TpManager::GetInstance(devLogicId).GetTpInfo({locAddr, rmtAddr, tpProtocol}, tpInfo);
...
void DevUbConnection::ReleaseTp()
{
    if (tpInfo.tpHandle != 0) {
        (void)TpManager::GetInstance(devLogicId).ReleaseTpInfo({locAddr, rmtAddr, tpProtocol}, tpInfo);
        tpInfo.tpHandle = 0;
    }
}
...
HcclResult DevUbConnection::SetTpAttrAsync()
{
    ...
    /*  bitmap 至少配置为1FC，转2进制: 0011 1111
       1000(前两位retry_times_init+at不用配置、后三位at_times+sl+ttl不用配置)，转10进制:508 0-retry_times_init: 3 bit
       1-at: 5 bit             2-sip: 128 bit 3-dip: 128 bit              4-sma: 48 bit           5-dma: 48 bit
    ...
    */
    ...
    HCCL_INFO(
        "[DevUbConnection::%s] localIpv4Str[%s], sip[%u:%u:%u:%u]", __func__, localIp, tpAttr.sip[12], tpAttr.sip[13],
        tpAttr.sip[14], tpAttr.sip[15]);

    // 填充对端IP
    const char* rmtIp = rmtIpv4Addr.GetIpStr().c_str();
    CHK_RET(Ipv4ToIpArray(rmtIp, tpAttr.dip));
    HCCL_INFO(
        "[DevUbConnection::%s] rmtIpv4Str[%s], dip[%u:%u:%u:%u]", __func__, rmtIp, tpAttr.dip[12], tpAttr.dip[13],
        tpAttr.dip[14], tpAttr.dip[15]);
...
HcclResult DevUbConnection::GetTpAttrAsync()
{
    ...
    HCCL_INFO(
        "[DevUbConnection::%s] locIpv4Addr[%s], rmtIpv4Addr[%s], locAddr[%s], rmtAddr[%s]", __func__,
        locIpv4Addr.Describe().c_str(), rmtIpv4Addr.Describe().c_str(), locAddr.Describe().c_str(),
        rmtAddr.Describe().c_str());

    HCCL_INFO(
        "[DevUbConnection::%s] attrBitmap[%u], "
        "sip[%u:%u:%u:%u], dip[%u:%u:%u:%u], "
        "sma[%#x:%#x:%#x:%#x:%#x:%#x], dma[%#x:%#x:%#x:%#x:%#x:%#x]",
        __func__, attrBitmap, tpAttr.sip[12], tpAttr.sip[13], tpAttr.sip[14], tpAttr.sip[15], tpAttr.dip[12],
        tpAttr.dip[13], tpAttr.dip[14], tpAttr.dip[15], tpAttr.sma[0], tpAttr.sma[1], tpAttr.sma[2], tpAttr.sma[3],
        tpAttr.sma[4], tpAttr.sma[5], tpAttr.dma[0], tpAttr.dma[1], tpAttr.dma[2], tpAttr.dma[3], tpAttr.dma[4],
        tpAttr.dma[5]);
    return HCCL_SUCCESS;
}
