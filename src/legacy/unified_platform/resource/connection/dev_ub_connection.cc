...
HcclResult DevUbConnection::CalcTotalTimeout(uint32_t &outTotalTimeoutMs)
{
    ...
    HCCL_INFO("%s TP timeout calc success: "
        ...
        __func__, rawAtGear, singleAtTimeoutMs, rawRetryTimes, totalTimeoutMs);
...
static uint8_t FindMinTaHwValue(uint32_t tpTotalTimeoutMs)
{
    ...
    HCCL_INFO("%s tp_total_timeout[%ums], __func__, selected_gear_hw_value[%u], corresponding_timeout[%ums]",
...
void DevUbConnection::GetTimeOut() // 直接基于环境变量控制
{
    ...
    if (envTimeOut < tpTimeOut) {
        ...
        timeOut = FindMinTaHwValue(tpTimeOut);
    ...
    HCCL_INFO("%s final TA Timeout [%u] (%ums).", __func__, timeOut);
}
