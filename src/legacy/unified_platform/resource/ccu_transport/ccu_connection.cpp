...
BatchDeleteJettyInfo CcuConnection::Clean()
{
    ...
    for(auto remoteJettyHandle : remoteDelJettyList) {
        ...
        batchDeleteJettyInfo.unimportJettyList.push_back(rdmaHandle, jettyInfo.deleteJetty);
    }
    ...
    for (auto &ccuJetty : ccuJettys_) {
        ...
        if (jettyInfo.isValid) {
            batchDeleteJettyInfo.deleteJettyList.push_back(rdmaHandle, jettyInfo.deleteJetty);
        }
    }
    return batchDeleteJettyInfo;
}
