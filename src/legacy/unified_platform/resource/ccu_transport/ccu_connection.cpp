...
{
    TRY_CATCH_RETURN(
        devLogicId = HrtGetDevice(); uint32_t devPhyId = HrtGetDevicePhyIdByIndex(devLogicId);

        auto& rdmaHandleMgr = RdmaHandleManager::GetInstance(); rdmaHandle = rdmaHandleMgr.GetByIp(devPhyId, locAddr_);
...
HcclResult CcuConnection::StartImportJettyRequest(uint32_t jettyIndex, RequestHandle& reqHandle)
{
    ...
    TRY_CATCH_RETURN(
        reqHandle = RaUbTpImportJettyAsync(
