...
public:
    ...
    HcclResult FindEntry(const AicpuTaskCacheKey& key, AicpuTaskCacheEntry** entryPtrPtr) const;
    HcclResult AddEntry(const AicpuTaskCacheKey& key, const AicpuTaskMemRange& localUserInputMemRange,
        const AicpuTaskMemRange& localUserOutputMemRange, AicpuTaskCacheEntry** entryPtrPtr);
    HcclResult ClearEntry(const AicpuTaskCacheKey& key);

private:
    using CacheHashMap = std::unordered_map<AicpuTaskCacheKey, AicpuTaskCacheEntry*>;
