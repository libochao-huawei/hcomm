...
struct AddrRefreshInfo {
    ...
    static constexpr uint8_t FIXED_MEMTYPE = 0;
    static constexpr uint8_t USER_INPUT_MEMTYPE = 1;
    static constexpr uint8_t USER_OUTPUT_MEMTYPE = 2;

    uint32_t rankId = 0; // 当前只考虑单算子场景, 即远端只访问ccl buffer且地址不变 (无需刷新), 因此rankId一定为localRank (此字段暂不适用)
    uint8_t memType = FIXED_MEMTYPE; // 0: fixed memory (例如硬件地址, ccl buffer); 1: user input; 2: user output
};
...
private:
    ...
    HcclResult RefreshTaskAddr_(uint32_t& addrLow, uint32_t& addrHigh, AddrRefreshInfo& addrRefreshInfo,
