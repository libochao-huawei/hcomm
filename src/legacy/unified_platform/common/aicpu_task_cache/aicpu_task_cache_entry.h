...
private:
    ...
    // TODOSSY: === END HERE===
    HcclResult RefreshTaskAddr(uint64_t& sqeAddr, const uint32_t rankId,
    ...
    std::vector<AddrRefreshInfo> wqeSrcInfoArrays_; // 每段每个WQE中srcAddr (if any)对应的刷新信息
    std::vector<AddrRefreshInfo> wqeDstInfoArrays_; // 每段每个WQE中dstAddr (if any)对应的刷新信息
