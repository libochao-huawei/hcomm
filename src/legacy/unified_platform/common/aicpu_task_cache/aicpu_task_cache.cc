...
HcclResult AicpuTaskCache::FindEntry(const AicpuTaskCacheKey& key, AicpuTaskCacheEntry** entryPtrPtr) const
{
    ...
    if (constIter != cacheHashMap_.cend()) {
        *entryPtrPtr = constIter->second;
        ...
        HCCL_INFO("[AicpuTaskCache][FindEntry] find cache entry for key[%s]", key.GetKeyString().c_str());
    } else {
        *entryPtrPtr = nullptr;
        HCCL_INFO("[AicpuTaskCache][FindEntry] not find cache entry for key[%s]", key.GetKeyString().c_str());
    }

    return HCCL_SUCCESS;
}

HcclResult AicpuTaskCache::AddEntry(const AicpuTaskCacheKey& key, const AicpuTaskMemRange& localUserInputMemRange,
    const AicpuTaskMemRange& localUserOutputMemRange, AicpuTaskCacheEntry** entryPtrPtr)
{
    ...
    CHK_PRT_RET(iter != cacheHashMap_.end(),
        HCCL_ERROR("[AicpuTaskCache][AddEntry] cache entry for key[%s] exists", key.GetKeyString().c_str()),
        HCCL_E_INTERNAL);
    ...
    if (UNLIKELY(!(insertResult.second))) {
        HCCL_ERROR("[AicpuTaskCache][AddEntry] fail to insert a new cache entry for key[%s]",
        ...
        HCCL_INFO("[AicpuTaskCache][AddEntry] add a new cache entry for key[%s] cacheHashMap_.size[%u]",
...
HcclResult AicpuTaskCache::ClearEntry(const AicpuTaskCacheKey& key)
{
    CacheHashMap::iterator iter = cacheHashMap_.find(key);
    if (iter != cacheHashMap_.end()) {
        HCCL_INFO("[AicpuTaskCache][ClearEntry] clear cache entry for key[%s]", key.GetKeyString().c_str());
        ...
        HCCL_INFO("[AicpuTaskCache][ClearEntry] not find cache entry for key [%s]", key.GetKeyString().c_str());
    }
