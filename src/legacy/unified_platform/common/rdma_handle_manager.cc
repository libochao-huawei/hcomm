...
{
    bool uboeFlagValid;
    if (HrtGetUboeFlagEnable(devPhyId, uboeFlagValid) != HCCL_SUCCESS) {
        HCCL_ERROR("[RdmaHandleManager][%s] HrtGetUboeFlagEnable failed, devPhyId[%u]", __func__, devPhyId);
        return;
    }
    if (uboeFlagValid == false) {
        HCCL_ERROR("[RdmaHandleManager][%s] Uboe flag is not enabled, devPhyId[%u].", __func__, devPhyId);
        return;
    }
