...
void RdmaHandleManager::UboeIpv4ToEid(const IpAddress& ipV4Address, IpAddress& eidAddress)
{
    ...
    HCCL_INFO("[RdmaHandleManager::%s] Get EidInfo success, logicDeviceId=%d, eidInfo size=%u",
        __func__, logicDeviceId, eidInfoList.size());

    HrtDevEidInfo eidInfo = eidInfoList.front;
    // 2.根据ipV4Address初始化rdmaHandle
    HrtRaUbCtxInitParam in(HrtNetworkMode::HDC, HrtGetDevicePhyIdByIndex(deviceLogicId), ipV4Address);
