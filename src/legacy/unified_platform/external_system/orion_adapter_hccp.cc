...
HcclResult HrtRaCtxQpDestoryBatch(const RdmaHandle handle, const std::vector<JettyHandle> &jettyHandles, std::vector<JettyHandle> &failJettyHandles)
{
    ...
    unsigned int delNum = min(qp_handle.size(), MAX_DELETE_JETTY_NUMS);
    std::vector<void*> del_qp_handle;
    while (true) {
        void *raReqHandle = nullptr;
        delNum = min(qp_handle.size(), MAX_DELETE_JETTY_NUMS);
        ...
        while (true) {
            if ((std::chrono::steady_clock::now() - startTime) >= waitPollTimeOutMs) {
                HCCL_INFO("[%s]poll timeout, originalJettyCount[%u], undeleteJettyCount[%u].", __func__, jettyHandles.size(), failJettyHandles.size());
