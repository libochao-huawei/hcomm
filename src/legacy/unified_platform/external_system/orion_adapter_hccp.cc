...
SocketHandle HrtRaSocketInit(HrtNetworkMode  netMode, RaInterface &in)
{
    ...
    HCCL_INFO("[HrtRaSocketInit] Input params: mode=%u, ip=%u, ipv6_32=%u device id=%u, family=%u",
        mode, rdevInfo.localIp.addr.s_addr, rdevInfo.localIp.addr6.s6_addr32, rdevInfo.phyId, rdevInfo.family);
...
HrtRaUbJettyCreatedOutParam HrtRaUbCreateJetty(RdmaHandle handle, const HrtRaUbCreateJettyParam &in)
{
    ...
    HCCL_INFO("Create jetty success, handle[%llu] key[%s] jettyVa[%llu] uasid[%u] id[%u] keySize[%u] dbVa[%llu] sqBuffVa[%llu]",
        out.handle, out.key, out.jettyVa, out.uasid, out.id, out.keySize, out.dbVa, out.sqBuffVa);
    return out;
}
    ...
    u32 tokenValue, const JettyImportCfg &jettyImportCfg)
{
    ...
    HCCL_INFO("[RaUbTpImportJetty] Input params: handle=%p, key=%s, keyLen=%u tokenValue=[%u]",
        handle, key, keyLen, tokenValue);
