...
void HrtRaSocketConnectOne(RaSocketConnectParam &in)
{
    ...
    int sret = strcpy_s(connInfo.tag, sizeof(connInfo.tag) - 1, in.tag.c_str());
    if (sret != 0) {
        string msg
            = StringFormat("[HrtRaSocketConnectOne] copy tag[%s] to hccp tag failed, in.tag=%d, connInfo.tag=%d, ret=%d", in.tag.c_str(), sizeof(in.tag.c_str()), sizeof(connInfo.tag), sret);
...
RaSocketFdHandleParam HrtRaBlockGetOneSocket(u32 role, RaSocketGetParam &param)
{
    ...
    int sret = strcpy_s(socketInfo.tag, sizeof(socketInfo.tag) - 1, param.tag.c_str());
    if (sret != 0) {
        MACRO_THROW(NetworkApiException, StringFormat("[HrtRaBlockGetOneSocket] copy tag[%s] to hccp failed, ret=%d, role=%u,socketHandle=%p, fdHandle=%p, remoteIp=%s, socketInfo.tag=%d, param.tag=%d",
...
void HrtRaSocketBlockRecv(const FdHandle fdHandle, void *data, u32 size)
{
    ...
    while (true) {
        ...
        if ((rtRet == 0) && (recvSize > 0)) { // 接收完成，也有可能要多次接收
            ...
            MACRO_THROW(NetworkApiException, StringFormat("[Recv][RaSocket]recv fail, fdHandle=%p data=%p bufLen=%u, recLen=%lld, ret=%d",
...
void HrtRaSocketSetWhiteListStatus(u32 enable)
{
    ...
    if (ret != 0) {
        MACRO_THROW(NetworkApiException, StringFormat("[Set][WhiteListStatus]errNo[0x%016llx] ra socekt set whilte list fail, call RaSocketSetWhiteListStatus failed, params: enable[%u], return: ret[%d]",
...
void HrtRaSocketWhiteListAdd(SocketHandle socketHandle, vector<RaSocketWhitelist> &wlists)
{
    ...
    while (wlistNum > 0) {
        ...
        for (size_t idx = startIdx; idx < addListNum + startIdx; idx++) {
            ...
            int sret = strcpy_s(wlistInfo.tag, sizeof(wlistInfo.tag) - 1, wlists[idx].tag.c_str());
            if (sret != EOK) {
                MACRO_THROW(InternalException, StringFormat("[Add][RaSocketWhiteList]errNo[0x%016llx]errName[HCCL_E_MEMORY] memory copy failed. params: socketHandle[%p], return: ret[%d], wlistInfo.tag=%d,  wlists[%d].tag=%d",
...
void HrtRaSocketWhiteListDel(SocketHandle socketHandle, vector<RaSocketWhitelist> &wlists)
{
    ...
    while (wlistNum > 0) {
        ...
        for (size_t idx = startIdx; idx < delListNum + startIdx; idx++) {
            ...
            int sret = strcpy_s(wlistInfo.tag, sizeof(wlistInfo.tag) - 1, wlists[idx].tag.c_str());
            if (sret != EOK) {
                auto msg = StringFormat("[Del][RaSocketWhiteList]errNo[0x%016llx] memory copy failed. ret[%d], wlistInfo.tag[%d], wlists[%d].tag[%d]",
...
QpHandle HrtRaQpCreate(RdmaHandle rdmaHandle, int flag, int qpMode)
{
    ...
    if (ret != 0 || connHandle == nullptr) {
        MACRO_THROW(NetworkApiException, StringFormat("[Create][RaQp]errNo[0x%016llx] ra qp create fail. call RaGetNotifyBaseAddr, params: rdmaHandle[%p], flag[%d], qpMode[%d]. return: ret[%d]",
...
void HrtRaUbCtxDestroy(RdmaHandle handle)
{
    ...
    HCCL_INFO("[HrtRaUbCtxDestroy] rdmaHandle[%p].", handle);
...
JfcHandle HrtRaUbCreateJfc(RdmaHandle handle, HrtUbJfcMode mode)
{
    ...
    HCCL_INFO("[HrtRaUbCreateJfc] Input params: handle=%p", handle);
...
HrtRaUbJettyCreatedOutParam HrtRaUbCreateJetty(RdmaHandle handle, const HrtRaUbCreateJettyParam &in)
{
    ...
    if (sRet != EOK) {
        MACRO_THROW(InternalException, StringFormat("HrtRaUbCreateJetty memcpy_s failed. sRet[%d]", sRet));
    }
    ...
    u32 tokenValue, jetty_import_exp_cfg cfg, jetty_import_mode mode, TpProtocol protocol = TpProtocol::INVALID)
{
    ...
    HCCL_INFO("[ImportJetty] Input params: handle=%p, key=%d, keyLen=%u", handle, *key, keyLen);
...
std::pair<uint32_t, uint32_t> HraGetDieAndFuncId(RdmaHandle handle)
{
    ...
    if (ret != 0) {
        MACRO_THROW(NetworkApiException, StringFormat("call ra_get_dev_base_attr failed, error code =%d.", ret));
    }
...
RequestHandle RaSocketConnectOneAsync(RaSocketConnectParam &in)
{
    ...
    if (sret != 0) {
        MACRO_THROW(NetworkApiException, StringFormat(
            "[%s] copy tag[%s] to hccp tag failed, ret=%d, connInfo.tag=%d, in.tag=%d",
...
RaSocketFdHandleParam RaGetOneSocket(u32 role, RaSocketGetParam &param)
{
    ...
    int sret = strcpy_s(socketInfo.tag, sizeof(socketInfo.tag) - 1, param.tag.c_str());
    if (sret != 0) {
        MACRO_THROW(NetworkApiException, StringFormat("[%s] failed, copy tag[%s] to hccp failed, ret=%d, socketInfo.tag=%d, param.tag=%d",
...
inline hccp_eid IpAddressToHccpEid(const IpAddress &ipAddr)
{
    ...
    if (sRet != EOK) {
        MACRO_THROW(InternalException, StringFormat("[IpAddressToHccpEid]memcpy_s failed. sRet[%d]", sRet));
    }
    ...
    TpProtocol protocol = TpProtocol::INVALID)
{
    ...
    CHECK_NULLPTR(remQpHandle, "[ImportJettyAsync] remQpHandle is nullptr!");
    ...
    vector<char_t> &out, void *&remQpHandle)
{
    ...
    CHECK_NULLPTR(remQpHandle, "[RaUbImportJettyAsync] remQpHandle is nullptr!");
    ...
    vector<char_t> &out, void *&remQpHandle)
{
    ...
    CHECK_NULLPTR(remQpHandle, "[RaUbTpImportJettyAsync] remQpHandle is nullptr!");
...
{
    CHECK_NULLPTR(rdmaHandle, "[HrtRaCreateQpWithCq] rdmaHandle is nullptr!");
    CHECK_NULLPTR(sendChannel, "[HrtRaCreateQpWithCq] sendChannel is nullptr!");
    CHECK_NULLPTR(recvChannel, "[HrtRaCreateQpWithCq] recvChannel is nullptr!");
    HCCL_INFO("[HrtRaCreateQpWithCq] Input params: rdmaHandle=%p, sendChannel=%p, recvChannel=%p", rdmaHandle, sendChannel, recvChannel);
...
{
    CHECK_NULLPTR(rdmaHandle, "[HrtRaCreateCq] rdmaHandle is nullptr!");
...
{
    CHECK_NULLPTR(rdmaHandle, "[HrtRaDestroyCq] rdmaHandle is nullptr!");
...
{
    CHECK_NULLPTR(rdmaHandle, "[HrtRaNormalQpCreate] rdmaHandle is nullptr!");
