        ...
        if (!ret) {
            ...
            if (bTimeout) {
                MACRO_THROW(
                    NetworkApiException,
                    StringFormat(
                        "[Init][Ra]errNo[0x%016llx], ra init timeout[%lld], phy_id=%u, nic_position=%u, ret=%d",
        ...
        if (!ret) {
            ...
            if (bTimeout) {
                MACRO_THROW(
                    NetworkApiException,
                    StringFormat(
                        "[DeInit][Ra]errNo[0x%016llx] ra deinit timeout[%lld], phy_id=%u, nic_position=%u, ret=%d",
                        HCCL_ERROR_CODE(HcclResult::HCCL_E_NETWORK), timeout, config.phyId, config.nicPosition, ret));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
            ...
            if (bTimeout) {
                MACRO_THROW(
                    NetworkApiException, StringFormat(
                                             ...
                                             "timeout[%lld]. return[%d], params: num[%u]",
    ...
    while (true) {
        ...
        if (!ret) {
            ...
            if (bTimeout) {
                MACRO_THROW(
                    NetworkApiException, StringFormat(
                                             "[BatchClose][RaSocket]errNo[0x%016llx]  ra socket batch close, "
                                             "timeout[%d], return[%d], params: num[%u]",
                                             HCCL_ERROR_CODE(HcclResult::HCCL_E_TCP_CONNECT), timeout, ret, num));
            }
            SaluSleep(ONE_MILLISECOND_OF_USLEEP);
        } else {
        ...
        if (ret == 0) {
            ...
            if (bTimeout) {
                MACRO_THROW(
                    NetworkApiException,
                    StringFormat(
                        "[ListenStart][RaSocket]errNo[0x%016llx] ra socket listen start, timeout[%d], return[%d], "
                        "params: num[%u]",
...
static bool RaSocketTryListenStart(struct SocketListenInfoT conn[], u32 num)
{
    ...
    if (ret == 0) {
        ...
        HCCL_INFO(
            ...
            __func__, (num > 0 ? conn[0].port : HCCL_INVALID_PORT));
        return false;
...
{
    HCCL_INFO(
        "[GetOneSocket][RaSocket] Input params: role=%u,socketHandle=%p, fdHandle=%p, remoteIp=%s", role,
        param.socketHandle, param.fdHandle, param.remoteIp.Describe().c_str());
            ...
            if (sret != EOK) {
                MACRO_THROW(
                    ...
                    StringFormat(
                        ...
                        "socketHandle[%p], return: ret[%d], wlistInfo.tag size=%d,  wlists[%d].tag size=%d",
        ...
        if (ret != 0) {
            MACRO_THROW(
                ...
                StringFormat(
                    ...
                    "RaSocketWhiteListAdd failed, socketHandle[%p], num=%llu, return[%d].",
        ...
        for (size_t idx = startIdx; idx < delListNum + startIdx; idx++) {
            ...
            if (sret != EOK) {
                auto msg = StringFormat(
                    "[Del][RaSocketWhiteList]errNo[0x%016llx] memory copy failed. ret[%d], wlistInfo.tag size[%d], "
                    "wlists[%d].tag size[%d]",
        ...
        if (ret != 0) {
            MACRO_THROW(
                NetworkApiException, StringFormat(
                                         ...
                                         "RaSocketWhiteListDel failed, num=%llu, return[%d].",
...
QpHandle HrtRaQpCreate(RdmaHandle rdmaHandle, int flag, int qpMode)
{
    ...
    if (ret != 0 || connHandle == nullptr) {
        RPT_INPUT_ERR(
            ...
            std::vector<std::string>({"memory_size"}), // A3是当ROCE_ENOMEM_RET才上报EI0011
    ...
    while (true) {
        ...
        if (ret == 0) {
            ...
            if (bTimeout != 0) {
                MACRO_THROW(
                    NetworkApiException, StringFormat(
                                             "[Destroy][RaQp]errNo[0x%016llx] ra qp destroy timeout[%d]. "
                                             "qpHandle[%p], return[%d].",
...
static void HrtRaSendWr(QpHandle qpHandle, struct SendWr* wr, struct SendWrRsp* opRsp)
{
    ...
    while (true) {
        ...
        if (ret == 0) {
            ...
            if (bTimeout) {
                HCCL_ERROR(
                    "[Send][RaWr]errNo[0x%016llx] ra get send async timeout[%d]. "
...
RequestHandle RaSocketConnectOneAsync(RaSocketConnectParam& in)
{
    ...
    if (sret != 0) {
        MACRO_THROW(
            NetworkApiException,
            StringFormat(
                "[%s] copy tag[%s] to hccp tag failed, ret=%d, connInfo.tag size=%d, in.tag size=%d", __func__,
...
RaSocketFdHandleParam RaGetOneSocket(u32 role, RaSocketGetParam& param)
{
    ...
    if (sret != 0) {
        MACRO_THROW(
            NetworkApiException,
            StringFormat(
                "[%s] failed, copy tag[%s] to hccp failed, ret=%d, socketInfo.tag size=%d, param.tag size=%d", __func__,
                param.tag.c_str(), sret, sizeof(socketInfo.tag), sizeof(param.tag.c_str())));
    }
...
{
    HCCL_INFO(
        "[HrtRaWaitEventHandle] Input params: event_handle=%d, timeout=%d, maxevents=%u, events_num=%u", event_handle,
    ...
    std::vector<JettyHandle>& failJettyHandles)
{
    ...
    while (true) {
        ...
        while (true) {
            if ((std::chrono::steady_clock::now() - startTime) >= waitPollTimeOutMs) {
                HCCL_ERROR(
                    "[%s]poll timeout, originalJettyCount[%u], undeleteJettyCount[%u].", __func__, jettyHandles.size(),
        ...
        if (delNum > del_qp_handle.size()) {
            HCCL_ERROR(
                "[%s] run RaCtxQpDestroyBatchAsync error, del jetty num[%u] greater than all jetty num[%u].", __func__,
    ...
    HCCL_INFO(
        "[%s] run success, originalJettyCount[%u], undeleteJettyCount[%u].", __func__, jettyHandles.size(),
        failJettyHandles.size());
    return HCCL_SUCCESS;
}
