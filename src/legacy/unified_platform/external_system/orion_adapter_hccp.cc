...
std::vector<HrtDevEidInfo> HrtRaGetDevEidInfoList(const HRaInfo &raInfo)
{
    ...
    for (u32 i = 0; i < num; i++) {
        ...
        HCCL_INFO("[%s] HrtDevEidInfo[%d]: name[%s], ipAddress[%s], type[%u], eidIndex[%u], dieId[%u], chipId[%u], funcId[%u], devFeature[%u]",
        __func__, i,
        hrtDevEidInfo[i].name.c_str(), hrtDevEidInfo[i].ipAddress.Describe().c_str(), hrtDevEidInfo[i].type,
        hrtDevEidInfo[i].eidIndex, hrtDevEidInfo[i].dieId, hrtDevEidInfo[i].chipId, hrtDevEidInfo[i].funcId, hrtDevEidInfo[i].devFeature);
    }
