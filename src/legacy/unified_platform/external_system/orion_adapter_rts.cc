...
aclrtStream HrtStreamCreateWithFlags(int32_t priority, uint32_t flags)
{
    ...
    aclError ret = aclrtCreateStreamWithConfig(&ptr, priority, flags);
    ...
    if (ret != ACL_SUCCESS) {
        ...
        throw RuntimeApiException(
                StringFormat("call aclrtCreateStreamWithConfig failed, priority=%p, flags=%u", priority, flags));
    }
...
// 兜底extern, 正式适配待todo
void HrtKernelLaunchWithFlagV2(aclrtFuncHandle funcHandle, uint32_t numBlocks, aclrtStream stream,
                               aclrtLaunchKernelCfg *cfg, void *hostArgs, size_t argsSize,
                               aclrtPlaceHolderInfo *placeHolderArray, size_t placeHolderNum)
{
    HCCL_INFO("[hrtKernelLaunchWithFlagV2]blockdim: [%u]", numBlocks);
...
// 兜底extern形式
void HrtAicpuLaunchKernelWithHostArgs(aclrtFuncHandle funcHandle, uint32_t blockDim, aclrtStream stream,
                                      aclrtLaunchKernelCfg *cfg, void *hostArgs, size_t argsSize,
                                      aclrtPlaceHolderInfo *placeHolderArray, size_t placeHolderNum)
{
    rtError_t ret = aclrtLaunchKernelWithHostArgs(funcHandle, blockDim, stream, cfg, hostArgs, argsSize,
                                                  placeHolderArray, placeHolderNum);
    if (ret != RT_ERROR_NONE) {
