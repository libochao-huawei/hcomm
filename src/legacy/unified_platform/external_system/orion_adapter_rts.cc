...
void HrtNotifyWaitWithTimeOut(RtNotify_t notifyPtr, aclrtStream streamPtr, uint32_t timeOut)
{
    ...
    HCCL_INFO(
        "[HrtNotifyWaitWithTimeOut] notifyPtr[%p], streamPtr[%p], timeOut[%u], ret[%d].", notifyPtr, streamPtr, timeOut,
        ret);
    if (ret != ACL_SUCCESS) {
        string msg = StringFormat(
            "call aclrtWaitAndResetNotify failed. notifyPtr=%p, streamPtr=%p, timeout=%u, return=%d.", notifyPtr,
            streamPtr, timeOut, ret);
        MACRO_THROW(RuntimeApiException, msg);
    }
}
