...
const std::unordered_map<std::string, DevType> SOC_VER_CONVERT{{"Ascend310P1", DevType::DEV_TYPE_V51_310_P1},
    {"Ascend310P3", DevType::DEV_TYPE_V51_310_P3}, {"Ascend910", DevType::DEV_TYPE_910A},
    {"Ascend910A", DevType::DEV_TYPE_910A}, {"Ascend910B", DevType::DEV_TYPE_910A},
    {"Ascend910ProA", DevType::DEV_TYPE_910A}, {"Ascend910ProB", DevType::DEV_TYPE_910A},
    {"Ascend910PremiumA", DevType::DEV_TYPE_910A}, {"Ascend910B1", DevType::DEV_TYPE_910A2},
    {"Ascend910B2", DevType::DEV_TYPE_910A2}, {"Ascend910B3", DevType::DEV_TYPE_910A2},
    {"Ascend910B4", DevType::DEV_TYPE_910A2}, {"Ascend910B4-1", DevType::DEV_TYPE_910A2},
    {"Ascend910_939", DevType::DEV_TYPE_910A3}, {"Ascend910_938", DevType::DEV_TYPE_910A3},
    {"Ascend910_937", DevType::DEV_TYPE_910A3}, {"nosoc", DevType::DEV_TYPE_NOSOC}};
...
HcclResult HrtGetMainboardId(uint32_t deviceLogicId, HcclMainboardId &hcclMainboardId)
{
    ...
    HCCL_INFO("[HrtGetMainboardId] deviceLogicId[%u] mainboardId[%llu] hcclMainboardId[%s].", deviceLogicId,
...
void HrtNotifyWaitWithTimeOut(RtNotify_t notifyPtr, aclrtStream streamPtr, uint32_t timeOut)
{
    ...
    HCCL_INFO("[HrtNotifyWaitWithTimeOut] notifyPtr[%p], streamPtr[%p], timeOut[%u], ret[%d].", notifyPtr, streamPtr,
        timeOut, ret);
    if (ret != ACL_SUCCESS) {
        string msg
            = StringFormat("call aclrtWaitAndResetNotify failed. notifyPtr=%p, streamPtr=%p, timeout=%u, return=%d.",
    ...
    aclDataType type, aclrtStream streamPtr)
{
    ...
    if (ret != ACL_SUCCESS) {
        string msg = StringFormat("[rt][ReduceAsync]errNo[0x%016llx] rt reduce async fail. "
                                  "return[%d], para: dst[%p], destMax[%llu], src[%p], count[%llu], rtReduceOp[%d], "
                                  "rtDataType[%d], streamPtr[%p].",
...
void HrtRDMASend(u32 qpn, u32 wqeIndex, aclrtStream streamPtr)
{
    ...
    HCCL_INFO("Call rtRDMASend, return value[%d]. Params: qpn[%u], wqeIndex[%u], streamPtr[%p].", ret, qpn, wqeIndex,
        streamPtr);
    if (ret != RT_ERROR_NONE) {
    ...
    const aclrtCntNotify inCntNotify, const aclrtStream streamPtr, HrtCntNotifyRecordMode mode, u32 value)
{
    ...
    HCCL_INFO("[HrtCntNotifyRecord] inCntNotify[%p], streamPtr[%p], mode[%d], value[%u], ret[%d].", inCntNotify,
        streamPtr, mode, value, ret);
    if (ret != ACL_SUCCESS) {
...
const std::map<HrtDevResType, rtDevResType_t> HRT_DEV_RES_TYPE_MAP
    = {{HrtDevResType::RES_TYPE_STARS_NOTIFY_RECORD, RT_RES_TYPE_STARS_NOTIFY_RECORD},
        {HrtDevResType::RES_TYPE_CCU_CKE, RT_RES_TYPE_CCU_CKE}, {HrtDevResType::RES_TYPE_CCU_XN, RT_RES_TYPE_CCU_XN},
...
HcclResult HrtThreadExchangeCaptureMode(aclmdlRICaptureMode *mode)
{
    ...
    if (ret == ACL_ERROR_RT_FEATURE_NOT_SUPPORT) {
        ...
        CHK_PRT_RET(ret != ACL_SUCCESS,
            HCCL_ERROR("[HrtThreadExchangeCaptureMode]rtThreadExchangeCaptureMode "
...
{
    CHK_PRT_RET(aclrtMemP2PMap == nullptr,
...
HcclResult HrtEnableP2P(u32 deviceLogicId, u32 devicePhyId)
{
    ...
    CHK_PRT_RET(ret != RT_ERROR_NONE,
        HCCL_ERROR("[Enable][P2P]errNo[0x%016llx] rt enableP2P deviceLogicId[%u] and "
...
HcclResult HrtDisableP2P(u32 deviceLogicId, u32 devicePhyId)
{
    ...
    CHK_PRT_RET(ret != RT_ERROR_NONE,
        HCCL_ERROR("[Disable][P2P]errNo[0x%016llx] rt disableP2P deviceLogicId[%u] and "
                   "devicePhyId[%u] fail[%d]",
            HCCL_ERROR_CODE(HCCL_E_RUNTIME), deviceLogicId, devicePhyId, ret),
        HCCL_E_RUNTIME);
    return HCCL_SUCCESS;
...
HcclResult HrtGetP2PStatus(u32 deviceLogicId, u32 devicePhyId, uint32_t *status)
{
    ...
    HCCL_DEBUG("rt getp2pstatus deviceLogicId[%u] and devicePhyId[%u] fail[%d], status[%u]", deviceLogicId, devicePhyId,
        ret, *status);
    CHK_PRT_RET(ret != RT_ERROR_NONE,
        HCCL_ERROR("[Get][P2PStatus]errNo[0x%016llx]Call rtGetP2PStatus failed, "
