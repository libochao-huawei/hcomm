...
{
    static auto funcPtr = static_cast<rtError_t(*)(uint32_t, const uint32_t)>(g_dlRts.Handle<RT_RESET_XPU_DEVICE>());
...
{
    static auto funcPtr = static_cast<rtError_t(*)(uint32_t, const uint32_t)>(g_dlRts.Handle<RT_SET_XPU_DEVICE>());
    CHK_PTR_NULL(funcPtr);
    rtError_t ret = funcPtr(devType, devId);
    if (ret != RT_ERROR_NONE) {
