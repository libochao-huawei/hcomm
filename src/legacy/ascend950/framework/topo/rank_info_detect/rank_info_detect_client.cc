...
{
    TRY_CATCH_RETURN(clientSocket_->Connect());
    CHK_RET(CheckStatus());
    return HCCL_SUCCESS;
}
...
HcclResult RankInfoDetectClient::TearDown()
{
    ...
    // close socket
    TRY_CATCH_RETURN(clientSocket_->Close());
...
RankInfoDetectClient::~RankInfoDetectClient()
{
    ...
    CHK_PRT_CONT(ret != HCCL_SUCCESS, HCCL_ERROR("[RankInfoDetectClient::%s] TearDown failed, ret[%d]", __func__, ret));
}
