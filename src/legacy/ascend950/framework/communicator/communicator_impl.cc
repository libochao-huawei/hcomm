...
void CommunicatorImpl::InitDataBufferManager()
{
    ...
    if (rankSize > 1) {
        ...
        cclBuffer_ = DevBuffer::Create(reinterpret_cast<uintptr_t>(reinterpret_cast<u8 *>(cclBuffer->GetAddr())), scratchBufSize/2);
        HCCL_RUN_INFO(
...
HcclResult CommunicatorImpl::CreateCommCclBuf()
{
    ...
    size_t inOutCclBufferSize = cclBufferSize / 2;
