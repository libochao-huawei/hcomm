...
{
    printf("[Socket][%s] socketStatus =  %u",  __func__, static_cast<unsigned int>(socketStatus));
    switch (socketStatus) {
...
void Socket::GetOneSocket()
{
    ...
    printf("[Socket][%s] fdHandleParam.status = %u",  __func__, static_cast<unsigned int>(fdHandleParam.status));
    // socket status:0 not connected 1:connected 2:connect timeout 3:connecting
