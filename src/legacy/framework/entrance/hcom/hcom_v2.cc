    ...
    void *recvCounts, void *rdispls, HcclDataType dataType, const char *group, rtStream_t stream)
{
    ...
    if (rankSize == 1) {
        ...
        if (sendCount == 0) {
            ...
            CHK_PRT_RET(sendBuf == nullptr, HCCL_ERROR("[AllGatherV] sendBuf is null when single rank"),
                        HCCL_E_PTR);
            CHK_PRT_RET(recvBuf == nullptr, HCCL_ERROR("[AllGatherV] recvBuf is null when single rank"),
                        HCCL_E_PTR);
        }
    ...
    u64 recvCount, HcclDataType dataType, HcclReduceOp op, const char *group, rtStream_t stream)
{
    ...
    if (rankSize == 1) {
        ...
        if (recvCount == 0) {
            ...
            CHK_PRT_RET(sendBuf == nullptr, HCCL_ERROR("[ReduceScatterV] sendBuf is null when single rank"),
                        HCCL_E_PTR);
            CHK_PRT_RET(recvBuf == nullptr, HCCL_ERROR("[ReduceScatterV] recvBuf is null when single rank"),
                        HCCL_E_PTR);
        }
