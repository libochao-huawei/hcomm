...
HcclResult HcclSetOpSrcDataTypeV2(void *opArgs, uint8_t srcDataType)
{
    ...
    if (srcDataType >= (sizeof(MC2_DATA_TYPE) / sizeof(MC2_DATA_TYPE[0])) || srcDataType < HCCL_DATA_TYPE_INT8) {
...
HcclResult HcclSetOpDstDataTypeV2(void *opArgs, uint8_t dstDataType)
{
    ...
    HcclOpArgs *opArgsPtr = static_cast<HcclOpArgs *>(opArgs);
    if (dstDataType >= (sizeof(MC2_DATA_TYPE) / sizeof(MC2_DATA_TYPE[0])) || dstDataType < HCCL_DATA_TYPE_INT8) {
...
HcclResult HcclSetOpReduceTypeV2(void *opArgs, uint32_t reduceType)
{
    ...
    HcclOpArgs *opArgsPtr = static_cast<HcclOpArgs *>(opArgs);
    if (reduceType >= (sizeof(MC2_REDUCE_TYPE) / sizeof(MC2_REDUCE_TYPE[0])) || reduceType < HCCL_REDUCE_SUM) {
        HCCL_ERROR("HcclSetOpReduceType set reduceType[%u] error, it's invalid.", dstDataType)
        return HCCL_E_PARA;
    }
