...
HcclResult CreateCommConfig(uint32_t rank, HcclCommConfig* config, HcclComm* comm, std::string& ranktableM)
{
    ...
    if (errorFlag) {
        HCCL_ERROR(
            "[Init][CreateCommConfig]CreateCommConfig failed,  rank[%u],"
            "return[0x%016llx]",
            rank, HCCL_ERROR_CODE(ret));
    ...
    HcclComm* comm)
{
    ...
    if (errorFlag) {
        HCCL_ERROR("[Init][%s]CreateCommConfigRootInfo failed return[0x%016llx]", __func__, HCCL_ERROR_CODE(ret));
        (void)HcclCommDestroyV2(opbasedCommInfoV2.pComm.get());
        *comm = nullptr;
        return ret;
    }
...
HcclResult HcclCommInitClusterInfoV2(const char* clusterInfo, uint32_t rank, HcclComm* comm)
{
    ...
    if (errorFlag) {
        HCCL_ERROR(
            ...
            "return[0x%016llx]",
            __func__, clusterInfo, rank, deviceLogicId, devPhyId, HCCL_ERROR_CODE(ret));
        (void)HcclCommDestroyV2(opbasedCommInfoV2.pComm.get());
        *comm = nullptr;
        return ret;
    }
...
HcclResult HcclTaskRegisterV2(HcclComm comm, const char* msgTag, Callback cb)
{
    ...
    if (g_taskServiceMap.find(commId) == g_taskServiceMap.end()) {
        HCCL_ERROR(
            "[HcclTaskRegisterV2] TaskService of CommId[%s] Not Found, g_taskServiceMap size[%zu]", commId.c_str(),
            g_taskServiceMap.size());
...
HcclResult HcclTaskRegisterProfV2(HcclComm comm, ProfCallbackTemplate profCallback)
{
    ...
    if (g_taskServiceMap.find(commId) == g_taskServiceMap.end()) {
        HCCL_ERROR(
            "[HcclTaskRegisterProfV2] TaskService of CommId[%s] Not Found, g_taskServiceMap size[%zu]", commId.c_str(),
            g_taskServiceMap.size());
        return HCCL_E_NOT_FOUND;
    }
...
HcclResult HcclTaskUnRegisterV2(HcclComm comm, const char* msgTag)
{
    ...
    if (g_taskServiceMap.find(commId) == g_taskServiceMap.end()) {
        HCCL_ERROR(
            "[HcclTaskUnRegisterV2] TaskService of CommId[%s] Not Found, g_taskServiceMap size[%zu]", commId.c_str(),
            g_taskServiceMap.size());
        return HCCL_E_NOT_FOUND;
    }
...
HcclResult HcclGetRootInfoV2(HcclRootInfo* rootInfo)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[%s] hccl root info overflow. max length: %u, actual:%zu, identifier[%s]", __func__, HCCL_ROOT_INFO_BYTES,
            rootHandleLen, rootHandle.identifier),
        HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(
        sRet != EOK,
        HCCL_ERROR(
            "[%s] memcpy root info fail. errorno[%d] params:destMaxSize[%u],"
            " count[%u]",
    ...
    s32 devPhyId = HrtGetDevicePhyIdByIndex(deviceLogicId);
    HCCL_RUN_INFO(
        "HcclGetRootInfoV2 success, take time [%lld]us, rootinfo: host ip[%s] port[%u] netMode[%s] "
...
HcclResult HcclCommDestroyV2(HcclComm comm)
{
    ...
    s32 devPhyId = HrtGetDevicePhyIdByIndex(deviceLogicId);
    HCCL_RUN_INFO(
        "HcclCommDestroy V950 comm[%s] success, take time [%lld]us, deviceLogicId[%d], devPhyId[%d].", commId.c_str(),
        DURATION_US(TIME_NOW() - startut), deviceLogicId, devPhyId);
    return HCCL_SUCCESS;
}
    ...
    HcclCommConfig* config, HcclComm* subComm)
{
    ...
    if (errorFlag) {
        HCCL_ERROR(
            "[Init][%s]HcclCreateSubCommConfigV2 failed, deviceLogicId[%d], devPhyId[%d],"
            "return[0x%016llx]",
            __func__, logicDevId, devPhyId, HCCL_ERROR_CODE(ret));
...
HcclResult HcclAllocComResourceByTilingV2(HcclComm comm, void* stream, void* mc2Tiling, void** commContext)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS, HCCL_ERROR("[HcclAllocComResourceByTilingV2]AllocCommResource fail, errNo[%d]", ret),
        HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(
        totalServerNum > MAX_CCU_MC2_SERVER_NUM,
        HCCL_ERROR(
            "[%s] the number of operators is %u, "
            "which exceeds the maximum operator specification of %u supported by CCU MC2.",
            __func__, totalServerNum, MAX_CCU_MC2_SERVER_NUM),
        HCCL_E_INTERNAL);
...
HcclResult RootInfoDetect(
    std::shared_ptr<RankInfoDetect> rankInfoDetectAgent, const u32 nRanks, u32 rank, const HcclRootHandleV2& rootHandle,
    RankTableInfo& rankTable)
{
    s32 deviceLogicId = HcclGetThreadDeviceId();
    s32 devPhyId = HrtGetDevicePhyIdByIndex(deviceLogicId);
    HCCL_RUN_INFO(
        "[%s] nRanks[%u], rank[%u] entry flat topo detect, rootinfo: host ip[%s] port[%u] netMode[%s] "
        "identifier[%s], deviceLogicId[%d], devPhyId[%d]",
    ...
    uint32_t nRanks, const HcclRootInfo* rootInfo, uint32_t rank, HcclComm* comm, std::string& identifier)
{
    ...
    /* 关键状态记录 */
    HCCL_RUN_INFO(
        "HcclCommInitRootInfoV2 success, take time [%lld]us, rankNum[%u], rank[%u]", DURATION_US(TIME_NOW() - startut),
        nRanks, rank);
    return HCCL_SUCCESS;
}
    ...
    uint32_t nRanks, const HcclRootInfo* rootInfo, uint32_t rank, const HcclCommConfig* config, HcclComm* comm)
{
    ...
    CHK_PTR_NULL(config);
    HCCL_RUN_INFO(
        "Entry-HcclCommInitRootInfoConfig V950: nRanks[%u], rank[%u], commEngine[%u]", nRanks, rank,
    ...
    CHK_PRT_RET(
        ret, HCCL_ERROR("[%s]errNo[0x%016llx] and create comm failed.", __func__, HCCL_ERROR_CODE(ret)),
        static_cast<HcclResult>(ret));

    /* 关键状态记录 */
    HCCL_RUN_INFO(
        "HcclCommInitRootInfoConfigV2 success, take time [%lld]us, rankNum[%u], rank[%u]",
...
HcclResult HcclGetCcuTaskInfoLegacy(HcclComm comm, void* tilingData, void* ccuTaskGroup)
{
    ...
    if (EnvConfig::GetInstance().GetLogConfig().GetEntryLogEnable()) {
        HCCL_RUN_INFO(
            "Entry-HcclGetCcuTaskInfo V950, commId[%s], tilingData[%p], ccuTaskGroup[%p]", communicator->GetId(),
...
HcclGetInstSizeListByNetLayerV2(HcclComm comm, uint32_t netLayer, uint32_t** instSizeList, uint32_t* listSize)
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "HcclGetInstSizeListByNetLayerV2 get netInstance size from communicator failed at netLayer[%u]", netLayer);
        return HCCL_E_NOT_FOUND;
    }
    /* 关键状态记录 */
    ...
    HcclComm comm, uint32_t netLayer, uint32_t srcRank, uint32_t dstRank, CommLink** linkList, uint32_t* listSize)
{
    ...
    /* 关键状态记录 */
    HCCL_INFO(
        "HcclGetLinks success, netLayer[%u], srcRank = %u, dstRank=%u, listSize=%u", netLayer, srcRank, dstRank,
        *listSize);
    return HCCL_SUCCESS;
}
...
HcclResult HcclRankGraphGetEndpointNumV2(HcclComm comm, uint32_t layer, uint32_t topoInstId, uint32_t* num)
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "HcclRankGraphGetEndpointNum get endpoint num from communicator failed at netLayer[%u] with topoInstId[%u]",
            layer, topoInstId);
        return HCCL_E_NOT_FOUND;
    }
    return HCCL_SUCCESS;
    ...
    void* info)
{
    ...
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR(
            "HcclRankGraphGetEndpointInfo get info from communicator failed with endpointAttr [%d]",
