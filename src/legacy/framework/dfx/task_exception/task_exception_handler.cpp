...
void TaskExceptionHandler::ProcessAivException(rtExceptionInfo_t* exceptionInfo, const TaskInfo& taskInfo)
{
    ...
    aclRet = aclrtMemcpy(
        ...
        taskInfo.taskParam_.taskPara.Aiv.flagMemSize, ACL_MEMCPY_DEVICE_TO_HOST);
    if (aclRet != ACL_SUCCESS) {
        HCCL_ERROR("[TaskExceptionHandler] [%s] error[%d].", __func__, aclRet);
...
void TaskExceptionHandler::PrintTaskContextInfo(uint32_t deviceId, uint32_t streamId, uint32_t taskId)
{
    ...
    HCCL_ERROR(
        "[TaskExceptionHandler]Task run failed, context sequence before error task is "
...
TaskExceptionHandler::GetCcuErrorIpInfo(uint32_t deviceId, uint16_t status, const TaskInfo& taskInfo)
{
