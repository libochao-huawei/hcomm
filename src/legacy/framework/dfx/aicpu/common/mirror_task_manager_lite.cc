...
MirrorTaskManagerLite::MirrorTaskManagerLite() { currDfxOpInfo_ = std::make_shared<Hccl::DfxOpInfo>(); }
...
void MirrorTaskManagerLite::AddTaskInfo(std::shared_ptr<TaskInfo> taskInfo)
{
    ...
    HCCL_INFO(
        "[MirrorTaskManagerLite][AddTaskInfo]add streamId(sqId)[%u] taskId(sqeId)[%u] queueMapsize[%u]",
