...
HcclResult GlobalMirrorTasks::FindTaskInfo(u32 devId, u32 streamId, u32 taskId, TaskInfo* taskInfo) const
{
    ...
    if (streamIterator == devMap.end()) { // rts回调时不会判断异常task是否HCCL task，索引不到可能是其他组件task，此处不打印ERROR日志
        HCCL_RUN_WARNING("[%s]devId[%u] streamId(sqId)[%u] do not found", __func__, devId, streamId);
    ...
    auto task = *queue->Find(FindTask);
    if (task == *queue->End()) {
        HCCL_RUN_WARNING("[%s]devId[%u] streamId(sqId)[%u] taskId(sqeId)[%u] do not found",
