...
public:
    ...
    std::shared_ptr<typename Queue<T>::Iterator> Find(std::function<bool(const T &)> cond) override
    {
        ...
        if (it != elems_.end()) {
            return std::make_shared<Iterator>(it, this, static_cast<u32>(it - elems_.begin()));
        }
        return std::make_shared<Iterator>(elems_.end(), this, static_cast<u32>(it - elems_.begin()));
    }
    ...
    {
        return std::make_shared<Iterator>(elems_.begin() + elems_.size() - 1, this, static_cast<u32>(elems_.size() - 1));
    }

    std::shared_ptr<typename Queue<T>::Iterator> End() override
    {
        return std::make_shared<Iterator>(elems_.end(), this, static_cast<u32>(elems_.size()));
    }
};
