...
ProfilingHandler::ProfilingHandler() { Init(); }
...
void ProfilingHandler::Init()
{
    ...
    if (ret != 0) {
        THROW<InternalException>(
            "[ProfilingHandler][Init]errNo[0x%016llx] Prof Register CtrlCallback"
            " fail, return[%d]",
    ...
    TaskParamType taskType, uint64_t beginTime, uint64_t endTime, bool isMasterStream, bool cachedReq, bool ignoreLevel)
{
    ...
    HCCL_INFO(
        "[ProfilingHandler]ReportHcclTaskApi, reporterData data is: level[%u], type[%u], threadId[%u], "
...
void ProfilingHandler::ReportHcclTaskDetails(const TaskInfo& taskInfo, bool cachedReq)
{
    ...
    CHECK_NULLPTR(
        taskInfo.dfxOpInfo_->comm_, "[ProfilingHandler::ReportHcclTaskDetails] taskInfo.dfxOpInfo_->comm_ is nullptr!");
    ...
    if (taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_INLINE_WRITE
        ...
        HCCL_INFO(
            "[ProfilingHandler]ReportHcclTaskDetails Report DPU info taskId[%llu].", hcclReportData.dpuProfInfo.taskId);
        CallAddtionInfo(
            hcclReportData, &hcclReportData.dpuProfInfo, sizeof(hcclReportData.dpuProfInfo),
            ProfTaskType::TASK_DPU_HCCL_INFO);
    } else {
        HCCL_INFO("[ProfilingHandler]ReportHcclTaskDetails Report HCCL info .");
        CallAddtionInfo(
            hcclReportData, &hcclReportData.profInfo, sizeof(hcclReportData.profInfo), ProfTaskType::TASK_HCCL_INFO);
    }
...
void ProfilingHandler::CallAddtionInfo(HCCLReportData& hcclReportData, void* data, u32 len, ProfTaskType type) const
{
    ...
    HCCL_INFO(
        "[ProfilingHandler]ReportHcclTaskDetails data is: level[%u], type[%u], threadId[%u],  dataLen[%u], "
        "timeStamp[%llu]",
...
void ProfilingHandler::GetHCCLReportData(const TaskInfo& taskInfo, HCCLReportData& hcclReportData) const
{
    ...
    hcclReportData.ts
        = taskInfo.taskParam_.endTime; // 该处时间需要保证在ReportHcclTaskApi内的begin和end时间之间。目前在end之外
    ...
    if (taskInfo.taskParam_.taskType == TaskParamType::TASK_SDMA
        ...
        taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_INLINE_WRITE
        || taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_WRITE_WITH_NOTIFY
        || taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_NOTIFY_WAIT
        || taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_CHANNEL_FENCE) {
        // 公共字段赋值
        ...
        hcclReportData.dpuProfInfo.dpuDevId = (1U << 12); // 高4位=1，低12位=0
        hcclReportData.dpuProfInfo.linkType = static_cast<uint16_t>(taskInfo.taskParam_.taskPara.DMA.linkType);
        ...
        HCCL_INFO(
            "[GetHCCLReportData]taskId[%llu], streamId[%lu], npuDevId[%lu], dpuDevId[%lu], starttime[%llu], "
            "endtime[%llu], aicputaskId[%llu].",
            hcclReportData.dpuProfInfo.taskId, hcclReportData.dpuProfInfo.streamId, hcclReportData.dpuProfInfo.npuDevId,
            hcclReportData.dpuProfInfo.dpuDevId, hcclReportData.dpuProfInfo.timeStamp, hcclReportData.ts,
            hcclReportData.dpuProfInfo.aicpu_task_id);
        // 按task类型填充特定字段
        if (taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_INLINE_WRITE
            || taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_WRITE_WITH_NOTIFY) {
            ...
            hcclReportData.dpuProfInfo.dstAddr
                = static_cast<u64>(reinterpret_cast<uintptr_t>(taskInfo.taskParam_.taskPara.DMA.dst));
            hcclReportData.dpuProfInfo.dataSize = static_cast<u32>(taskInfo.taskParam_.taskPara.DMA.size);
            hcclReportData.dpuProfInfo.notifyID = taskInfo.taskParam_.taskPara.DMA.notifyID;
        } else if (
            taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_NOTIFY_WAIT
            || taskInfo.taskParam_.taskType == TaskParamType::TASK_DPU_CHANNEL_FENCE) {
            hcclReportData.dpuProfInfo.notifyID = taskInfo.taskParam_.taskPara.Notify.notifyID;
        }
    }
...
void ProfilingHandler::DumpHCCLReportData(const TaskInfo& taskInfo, const HCCLReportData& hcclReportData) const
{
    ...
    HCCL_INFO(
        ...
        "hcclReportData.profInfo.dataType[%s], hcclReportData.profInfo.localRank[%u], "
        "hcclReportData.profInfo.remoteRank[%u]",
        ...
        DataTypeToSerialString(hcclReportData.profInfo.dataType).c_str(), hcclReportData.profInfo.localRank,
        hcclReportData.profInfo.remoteRank);

    HCCL_INFO(
        "HCCLReportData is: hcclReportData.ts[%llu], hcclReportData.dpuProfInfo.itemId[%llu], "
        "hcclReportData.dpuProfInfo.cclTag[%llu], hcclReportData.dpuProfInfo.groupName[%llu],  "
        "hcclReportData.dpuProfInfo.rankSize[%u], hcclReportData.dpuProfInfo.workFlowMode [%u], "
        "hcclReportData.dpuProfInfo.stage[%u], hcclReportData.dpuProfInfo.role[%u], "
        "hcclReportData.dpuProfInfo.durationEstimated[%f], taskInfo.taskParam_.taskType[%d]",
        hcclReportData.ts, hcclReportData.dpuProfInfo.itemId, hcclReportData.dpuProfInfo.cclTag,
        hcclReportData.dpuProfInfo.groupName, hcclReportData.dpuProfInfo.rankSize,
        hcclReportData.dpuProfInfo.workFlowMode, hcclReportData.dpuProfInfo.stage, hcclReportData.dpuProfInfo.role,
        hcclReportData.dpuProfInfo.durationEstimated, taskInfo.taskParam_.taskType);
    HCCL_INFO(
        "HCCLReportData other data is: hcclReportData.dpuProfInfo.srcAddr[%llu], "
        "hcclReportData.dpuProfInfo.dstAddr[%llu], "
        ...
        "hcclReportData.dpuProfInfo.dataType[%s], hcclReportData.dpuProfInfo.localRank[%u], "
        "hcclReportData.dpuProfInfo.remoteRank[%u]",
        ...
        DataTypeToSerialString(hcclReportData.dpuProfInfo.dataType).c_str(), hcclReportData.dpuProfInfo.localRank,
        hcclReportData.dpuProfInfo.remoteRank);
}
...
void ProfilingHandler::GetCcuTaskInfo(const TaskInfo& taskInfo, const CcuProfilingInfo& info) const
{
    ...
    HCCL_INFO(
        ...
        ccuTaskInfo.version, ccuTaskInfo.workFlowMode, ccuTaskInfo.itemId, ccuTaskInfo.groupName, ccuTaskInfo.rankId,
        ccuTaskInfo.ranksize, ccuTaskInfo.streamId, ccuTaskInfo.taskId, ccuTaskInfo.dieId, ccuTaskInfo.missionId,
        ccuTaskInfo.instrId);
...
void ProfilingHandler::DumpCcuGroupInfo(const MsprofCcuGroupInfo& ccuGroupInfo) const
{
    HCCL_INFO(
        ...
        ccuGroupInfo.version, ccuGroupInfo.itemId, ccuGroupInfo.groupName, ccuGroupInfo.rankId, ccuGroupInfo.ranksize,
        ccuGroupInfo.workFlowMode, ccuGroupInfo.streamId, ccuGroupInfo.taskId, ccuGroupInfo.dieId, ccuGroupInfo.instrId,
    ...
    for (auto i = 0; i < CCU_MAX_CHANNEL_NUM; i++) {
        if (ccuGroupInfo.channelId[i] != INVALID_VALUE_CHANNELID && ccuGroupInfo.remoteRankId[i] != INVALID_RANKID) {
            HCCL_INFO(
...
void ProfilingHandler::GetCcuWaitSignalInfo(const TaskInfo& taskInfo, const CcuProfilingInfo& info) const
{
    ...
    HCCL_INFO(
        ...
        "ckeId[%u],mask[%u]",
        waitSignalInfo.version, waitSignalInfo.itemId, waitSignalInfo.groupName, waitSignalInfo.rankId,
        waitSignalInfo.ranksize, waitSignalInfo.workFlowMode, waitSignalInfo.streamId, waitSignalInfo.taskId,
        waitSignalInfo.dieId, waitSignalInfo.instrId, waitSignalInfo.missionId, waitSignalInfo.ckeId,
        waitSignalInfo.mask);
    for (auto i = 0; i < CCU_MAX_CHANNEL_NUM; i++) {
        if (waitSignalInfo.channelId[i] != INVALID_VALUE_CHANNELID
            && waitSignalInfo.remoteRankId[i] != INVALID_RANKID) {
            HCCL_INFO(
                "[ProfilingHandler]GetCcuWaitSignalInfo, waitSignalInfo data is: channelId[%d] =  %u, remoteRankId[%d] "
                "= %u",
    ...
    uint32_t cmdType, uint64_t beginTime, uint64_t endTime, uint64_t cmdItemId, uint32_t threadId) const
{
    ...
    HCCL_INFO(
        "[ProfilingHandler][ReportAclApi], reporterData data is: level[%u], type[%u], threadId[%u], beginTime "
...
bool ProfilingHandler::GetHcclL2State() const { return enableHcclL2_; }
    ...
    RankId rankInParentComm)
{
    ...
    for (uint32_t streamIndex = 0; streamIndex < aicpuStreamsId.size(); streamIndex++) {
        HCCL_INFO(
            "streamIndex:[%u], reportId:[%d], streamId:[%u] id [%s] hcclMC2Info.groupName:[%lu]", streamIndex, reportId,
            aicpuStreamsId[streamIndex], id.c_str(), hcclMc2Info.groupName);
