...
void CcuTransportMgr::Clean()
{
    ...
    for (auto &linkTransPair : ccuLink2TransportMap) {
        ...
        for (auto& tmp : partInfo.deleteJettyList) {
            ...
            delList.insert(tmp.second.begin(), tmp.second.end());
        }
        for (auto& tmp : partInfo.unimportJettyList) {
            auto& unimportList = batchDeleteJettyInfo.unimportJettyList[tmp.first];
            unimportList.insert(tmp.second.begin(), tmp.second.end());
        }
    }
