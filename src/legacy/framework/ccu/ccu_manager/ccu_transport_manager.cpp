...
void CcuTransportMgr::Clean()
{
    ...
    for (auto &linkTransPair : ccuLink2TransportMap) {
        ...
        linkTransPair.second->GetDeleteJettyInfo(partInfo);
        for (auto& tmp : partInfo.deleteJettyList) {
            batchDeleteJettyInfo.deleteJettyList[tmp.first].insert(tmp.second.begin(), tmp.second.end());
        }
        for (auto& tmp : partInfo.unimportJettyList) {
            batchDeleteJettyInfo.unimportJettyList[tmp.first].insert(tmp.second.begin(), tmp.second.end());
        }
    }
