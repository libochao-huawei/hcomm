...
const CcuTransport::CcuConnectionInfo GeneratorCcuConnInfo(LinkProtocol linkProtocol)
{
...
const CcuTransport::CclBufferInfo GeneratorLocCclBufInfo(const CommunicatorImpl *comm)
{
...
HcclResult CcuTransportMgr::CreateTransportByLink(const LinkData &link, CcuTransport *&transport)
{
    ...
    auto connectionInfo = GeneratorCcuConnInfo(link.GetLinkProtocol());
    auto locCclBufInfo = GeneratorLocCclBufInfo(comm);
    // 当前不支持创建非UBC协议的链路
