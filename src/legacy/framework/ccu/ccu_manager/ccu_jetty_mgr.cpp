...
HcclResult CcuJettyMgr::GetAvailableBatch(const BatchKey &batchKey, ResourceBatch *&batchPtr, uint32_t sqSize)
{
    ...
    const u8 jettyQos = comm_ != nullptr ? comm_->GetCommQos()
                                          : static_cast<u8>(HCCL_COMM_QOS_CONFIG_DEFAULT_UB);
    HCCL_INFO("CcuJettyMgr::GetAvailableBatch jettyQos = %u", jettyQos);
    ...
    HCCL_INFO("[CcuJettyMgr][%s] try to alloc ccu channels with channelPara[channelNum=%u, jettyNum=%u, sqSize=%u], "
        "jettyQos[%u], locAddr[%s], devLogicId[%d].", __func__, channelPara.channelNum,
        channelPara.jettyNum, channelPara.sqSize, jettyQos, batchKey.Describe().c_str(), devLogicId_);
