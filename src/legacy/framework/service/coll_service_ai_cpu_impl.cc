...
void CollServiceAiCpuImpl::AicpuKernelLaunch(HcclKernelLaunchParam &param, Stream &stream, OpMode opMode)
{
    ...
    if (ccacheFunctionHandle.find(param.kernelName) != ccacheFunctionHandle.end()) {
        funcHandle = ccacheFunctionHandle[param.kernelName];
    } else {
        std::string jsonPath;
        ...
        ccacheFunctionHandle[param.kernelName] = funcHandle;
    }
