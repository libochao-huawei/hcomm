...
namespace {
    ...
    void PrintDeviceData(const char* label, void* devicePtr, size_t size, u32 rank, u32 tag, size_t maxPrintBytes = 64) {
        ...
        // 从device拷贝到host
        HcclResult ret = HrtMemcpy(hostData.data(), printSize, devicePtr, printSize, RT_MEMCPY_DEVICE_TO_HOST);
        if (ret != HCCL_SUCCESS) {
            HCCL_WARNING("[DEBUG][%s] rank[%u] tag[%u] Failed to copy %s from device to host, ret[%d]", 
                        __func__, rank, tag, label, ret);
            return;
        }
