...
void CollServiceBase::RegisterCclLocRmaBuffer() const // 注册CCL buffer
{
    ...
    for (const auto &pair : interfacesMap) {
        ...
        for (const auto &connIface : ifaceVec) {
            ...
            // TODO 确认是否可用使用第一个protocol
            rmaBufManager.Reg(comm->GetId(), BufferType::SCRATCH, comm->GetCclBuffer(), portData, *(protocols.begain()));
        }
    }
}
    ...
    for (const auto &pair : interfacesMap) {
        ...
        for (const auto &connIface : ifaceVec) {
            ...
            for (auto &devBuf : devBuffers) {
                ...
                // TODO 确认是否可用使用第一个protocol
                localRmaBufManager.Reg(opTag, devBuf.first, devBuf.second, portData, *(protocols.begain()));
            }
        }
    }
}
