...
SocketHandle HostSocketHandleManager::Create(DevId devicePhyId, const IpAddress &hostIp)
{
    ...
    HcclResult ret = HrtRaSocketInit(HrtNetworkMode::PEER, intf, socketHandle);
    if (ret != HCCL_SUCCESS) {
...
void HostSocketHandleManager::DestroyAll()
{
    ...
    for (u32 i = 0; i < hostSocketHandleMap.size(); ++i) {
        for (const auto &innerMap : hostSocketHandleMap[i]) {
            ...
            HcclResult ret = HrtRaSocketDeInit(innerMap.second.first);
            if (ret != HCCL_SUCCESS) {
...
void HostSocketHandleManager::Destroy(DevId devicePhyId, const IpAddress &hostIp)
{
    ...
    // 若引用计数为0则deinit
    if (count == 0) {
        HcclResult ret = HrtRaSocketDeInit(socketHandleRef.first);
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("HrtRaSocketDeInit failed, ret=%d", ret);
        }
