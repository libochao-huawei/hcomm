...
void SocketManager::ServerInit(PortData &localPort)
{
    ...
    u32 serverListenPort  = GetDeviceListenPort(localPort.GetRankId(), ipAddress);
...
Socket *SocketManager::CreateConnectedSocket(SocketConfig &socketConfig)
{
    ...
    string     hccpSocketTag   = socketConfig.GetHccpTag();
    u32        serverListenPort = GetDeviceListenPort(remotePort.GetRankId(), remoteIpAddress);
    NicType nicType = localPort.GetType() == PortDeploymentType::P2P ? NicType::DEVICE_VNIC_TYPE : NicType::DEVICE_NIC_TYPE;
...
std::unordered_map<u32, std::unordered_map<IpAddress, u32>> SocketManager::GetSubCommDeviceServerListenPortMap(const std::vector<u32> &rankIds)
{
    ...
    for (u32 subRankId = 0; subRankId < rankIds.size(); ++subRankId) {
        ...
        if (rankListenPortMap_.find(rankId) == rankListenPortMap_.end()) {
            ...
            subRankListenPortMap.insert(std::make_pair(subRankId, rankListenPortMap_[rankId]));
        }
    }
    return subRankListenPortMap;
}

u32 SocketManager::GetDeviceListenPort(const u32 &rankId, const IpAddress &ipAddress) const
{
    u32 listenPort = DEFAULT_VALUE_DEVICEPORT;
    if (rankListenPortMap_.find(rankId) == rankListenPortMap_.end() 
        || rankListenPortMap_[rankId].find(ipAddress) == rankListenPortMap_[rankId].end()) {
        return listenPort;
    }
    return rankListenPortMap_[rankId][ipAddress];
}
