...
void SocketManager::ServerInitOne(RankTableInfo &localRankTable)
{
    ...
    SocketHandle socketHandle = SocketHandleManager::GetInstance().Create(RankTableInfo.ranks[0].deviceId, localPort);
    ...
    PreemptPortManager::GetInstance(localRankTable.ranks[0].localId).ListenPreempt(serverSocket, listenPortRanges_, serverListenPort);
