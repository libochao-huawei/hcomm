...
void SocketManager::ServerInitOne(RankTableInfo &localRankTable)
{
    ...
    SocketHandleManager::GetInstance().Destroy(localRankTable.ranks[0].deviceId, localPort);
}
...
void SocketManager::SetDeviceServerListenPortMap(const std::unordered_map<u32, u32> &rankListenPortMap)
{
    ...
    for (auto &pair : rankListenPortMap) {
        ...
        HCCL_DEBUG("[SocketManager::%s] update device %u port %u", __func__, pair.first, pair.second);
    }
    return;
}
