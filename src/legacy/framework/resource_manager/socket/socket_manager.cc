...
void SocketManager::ServerInit(PortData &localPort)
{
    ...
    auto iter = rankListenPortMap.find(ipAddress);
...
Socket *SocketManager::CreateConnectedSocket(SocketConfig &socketConfig)
{
    ...
    string     hccpSocketTag   = socketConfig.GetHccpTag();
    u32 serverListenPort = DEFAULT_VALUE_DEVICEPORT;
    auto iter = rankListenPortMap.find(remoteIpAddress);
    if (iter != rankListenPortMap.end()) {
        serverListenPort = iter->second;
    }
    NicType nicType = localPort.GetType() == PortDeploymentType::P2P ? NicType::DEVICE_VNIC_TYPE : NicType::DEVICE_NIC_TYPE;
...
void SocketManager::SetDeviceServerListenPortMap(const std::unordered_map<IpAddress, u32> &rankListenPortMap)
{
    std::lock_guard<std::mutex> lock(socketLock);
    std::unordered_map<IpAddress, u32> &staticMap = SocketManager::GetDeviceServerListenPortMap();
    staticMap.clear();
...
std::unordered_map<IpAddress, u32> &SocketManager::GetDeviceServerListenPortMap()
{
    static std::unordered_map<IpAddress, u32> rankListenPortMap;
