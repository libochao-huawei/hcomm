...
{
    for (auto &i : rankListenPortMap) {
        rankListenPortMap_.insert(std::make_pair(i.first, i.second));
        for (auto &j : i.second) {
            HCCL_DEBUG("[SocketManager::%s] rankId[%u], addr[%s], listen port[%u]", __func__, i.first, j.first.Describe().c_str(), j.second);
        }
    }
}
...
{
    HCCL_DEBUG("[SocketManager::%s] rankListenPortMap_ size %u", __func__, rankListenPortMap_.size());
    for (auto &i : rankListenPortMap_) {
        for (auto &j : i.second) {
            HCCL_DEBUG("[SocketManager::%s] rankId[%u], addr[%s], listen port[%u]", __func__, i.first, j.first.Describe().c_str(), j.second);
        }
    }
    if (rankListenPortMap_.find(rankId) == rankListenPortMap_.end()
        || rankListenPortMap_.at(rankId).find(ipAddress) == rankListenPortMap_.at(rankId).end()) {
        HCCL_WARNING("[SocketManager::%s] Can't find rankId[%u], addr[%s] listen port, use default", __func__, rankId, ipAddress.Describe().c_str());
        return DEFAULT_VALUE_DEVICEPORT;
    }
    return rankListenPortMap_.at(rankId).at(ipAddress);
}
