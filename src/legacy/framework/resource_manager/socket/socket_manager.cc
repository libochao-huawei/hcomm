...
void SocketManager::ServerInitAll(NewRankInfo &rankInfo)
{
    ...
    HCCL_DEBUG("[SocketManager::%s] rankId[%u], localId[%u], devicePhyId[%u]", __func__, rankId, localId, devicePhyId);
        ...
        for (auto &link : links) {
            ...
            for (auto &protocol : protoclos) {
                ...
                for (auto &rankAddr : rankLevelInfo.rankAddrs) {
                    HCCL_DEBUG("[SocketManager::%s] rankAddr %s", __func__, rankAddr.Describe().c_str());
                    // topo查得网口使用则打开建链
