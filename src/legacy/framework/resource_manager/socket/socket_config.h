...
public:
        ...
        : link(link), listeningPort(port), tag(tag)
    {
        ...
        if (role == SocketRole::SERVER) { // server: tag_local_remote_port
            hccpTag = tag + "_" +
                      link.GetLocalAddr().GetIpStr() + "_" + link.GetRemoteAddr().GetIpStr() + "_" + to_string(port);
        } else { // client: tag_remote_local_port
            hccpTag = tag + "_" +
                      link.GetRemoteAddr().GetIpStr() + "_" + link.GetLocalAddr().GetIpStr() + "_" + to_string(port);
        }
    }
