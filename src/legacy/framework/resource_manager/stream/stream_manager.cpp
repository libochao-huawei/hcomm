...
{
    HCCL_INFO("[StreamManager][%s] masterStream[%u] slaveStream[%u]", __func__,
    ...
    if (opMode == OpMode::OPBASE) {
        ...
        HCCL_INFO("[StreamManager][%s] isCapture[%u] rtModel[%p]", __func__,
                  isCapture, rtModel);
        if (ret != HCCL_SUCCESS) {
        ...
        if (isCapture) {
            ...
            HCCL_INFO("[StreamManager][%s] get model id[%u]", __func__, modelId);
            if (ret != HCCL_SUCCESS) {
            ...
            HCCL_INFO("[StreamManager::%s] Add slaveStream[%u] to model[%u] success, masterStream[%u]",
