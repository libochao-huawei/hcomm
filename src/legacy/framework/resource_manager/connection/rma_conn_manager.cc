...
void RmaConnManager::GetDeleteJettys(std::vector<JettyHandle> &remoteJettyList, std::vector<JettyHandle> &jettyList)
{
    std::vector<TpInfo> tpList;
    for (auto *conn : rmaDelConnList) {
        auto& rdmaHandle = conn->GetRdmaHandle();
        if (recordRdmaHandle == nullptr) {
            recordRdmaHandle = rdmaHandle;
        } else if(rdmaHandle != recordRdmaHandle){
            HCCL_WARNING("[%s]rdmaHandle[%p] not equal recordRdmaHandle[%p]", __func__, rdmaHandle, recordRdmaHandle);
        }
        ...
        auto& tpInfo = conn->GetRdmaHandle();
...
void RmaConnManager::BatchDeleteJettys()
{
    ...
    GetDeleteJettys(remoteJettyList, jettyList);
