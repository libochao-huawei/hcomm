...
/**
  ...
  * @param pciePath1 第一个PCIe路径
  * @param pciePath2 第二个PCIe路径 
  * @return 1表示在同一个PCIE Switch，0表示不在同一个PCIE Switch
*/
...
#define PCIE_HOST_BRIDGE_PATH_LEN (33)
    size_t common_prefix_len = getCommonPrefixLen(pciePath1, pciePath2);
    if (common_prefix_len >= strlen(pciePath1) - SAME_PCIE_SW_BDF_DIFF_LEN
...
int IsNpuAndNicInSamePcieSwitch(NPU *npu, HCA *nic)
{
    ...
    return IsSamePcieSwitch(abs_path, nic->pciePath);
}
...
int ReadIntFromFile(const char* path, int *value)
{
    ...
    fclose(fp);
    static const int maxNumaCount = 8;
    if (temp >= 0 && temp < maxNumaCount) {
        *value = temp;
        return 0;
    }
    return -1;
