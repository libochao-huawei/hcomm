...
static int ProcessLayerMesh(int npu_id, NetLayer *layer, dcmi_urma_eid_info_t *eid_list, size_t eid_cnt)
{
    ...
    hal_sprintf_s(net_instance_id, sizeof(net_instance_id), "%s_%d", server_id, (npu_id / CARD_4P_MESH_NUM));
    ...
    for (size_t i = 0; i < eid_cnt; i++) {
        ...
        hal_sprintf_s(port, MAX_PORT_LEN, "%d/%d", dieId, portId);
        hal_sprintf_s(planeId, MAX_PORT_LEN, "plane_%d", dieId);
