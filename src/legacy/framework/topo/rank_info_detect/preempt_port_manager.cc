...
{
    CHK_PRT_RET_NULL(
        !initialized, HCCL_ERROR("[PreemptPortManager::%s] preempt port manager has already been release.", __func__));
    ...
    u32& usePort)
{
    ...
    if (portRef.find(ipAddr) != portRef.end()) {
        ...
        HCCL_INFO(
            "[PreemptPortManager::%s] socket has already been listened, ref count[%u].", __func__,
    ...
    HCCL_ERROR(
        ...
        envName.c_str());
    HCCL_ERROR(
        "NOTICE: The host port range size is not suggested to be smaller than the process number"
        " on current rank.");
    ...
    IpPortRef& portRef, const std::shared_ptr<Socket>& listenSocket, HrtNetworkMode netMode)
{
    ...
    CHK_PRT_THROW(
        ref.Count() <= 0,
        HCCL_ERROR(
            "[PreemptPortManager::%s] ref[%u], ip[%s] port[%u] has already been released.", __func__, ref.Count(),
            ipAddr.c_str(), port),
    ...
    CHK_PRT_RET_NULL(
        count > 0, HCCL_INFO(
                       "[PreemptPortManager::%s] release a socket on ip[%s], port[%u], ref[%u].", __func__,
