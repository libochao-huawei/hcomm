...
void RankTableInfo::Check()
{
    if (version != "2.0") {
        ...
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with version is not 2.0."}));
    ...
    if (rankCount > MAX_RANKCOUNT) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with rankCount, exceeds maximum limit."}));
    ...
    if (rankCount == 0) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with rankCount, exceeds minimum limit."}));
        THROW<InvalidParamsException>(StringFormat(
    ...
    if (rankCount != ranks.size()) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with rankCount, rankCount is not equal to rank_list size."}));
    ...
    for (auto &rank : ranks) {
        if (static_cast<u32>(rank.rankId) >= rankCount) {
            RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with rank_id is out of range."}));
        ...
        if (rankIdSet.count(rank.rankId) > 0) {
            RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with rank_id is repeat."}));
            THROW<InvalidParamsException>(StringFormat("[Parse][ClusterInfo][RankTableInfo::%s] failed with rank_id is "
        ...
        if (rank.localId != BACKUP_LOCAL_ID && rank.localId != rank.replacedLocalId) {
            RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with replacedLocalId not equal to locaId."}));
            ...
            if (recordedReplaceLocalId == UNDEFIEND_LOCAL_ID) {
                ...
                RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"multiple replaced rank is configured."}));
                THROW<InvalidParamsException>(StringFormat("[Parse][ClusterInfo][RankTableInfo::Check] "
    ...
    for (u32 rankRange = 0; rankRange < rankCount; rankRange++) {
        if (rankIdSet.find(rankRange) == rankIdSet.end()) {
            RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"mfailed with rank_id is not continuous."}));
            THROW<InvalidParamsException>(StringFormat("[Parse][ClusterInfo][RankTableInfo::%s] failed with rank_id is "
    ...
    if(localIdSet.find(recordedReplaceLocalId) != localIdSet.end()) {
        RPT_INPUT_ERR(true, "EI0014", std::vector<std::string>({"error_reason"}),
                            std::vector<std::string>({"failed with configuring same local_id with replaced one simutaneously."}));
