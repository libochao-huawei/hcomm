...
void NetInstance::Peer::SetPortPortAddrMapLayer0(std::unordered_map<std::string, IpAddress> portAddrMap)
{
 ...
 const std::unordered_map<u32, std::shared_ptr<TopoInstance>>&  NetInstance::GetTopoInsts() const
 {
    return topoInsts_;
 }

void NetInstance::AddTopoInst(std::unordered_map<u32, std::shared_ptr<TopoInstance>>&& topoInsts)
{
