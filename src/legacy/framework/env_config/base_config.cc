...
void EnvLogConfig::Parse()
{
    ...
    dfsConfig.Parse();
    HCCL_RUN_INFO("[HCCL_ENV] HCCL_DFS_CONFIG set by %s to [taskException=%d, cluster_heartbeat=%d]",
        dfsConfig.GetSource(), GetDfsConfig().taskExceptionEnable, GetDfsConfig().clusterHeartBeatEnable);
}
