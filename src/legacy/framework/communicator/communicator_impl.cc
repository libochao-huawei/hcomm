...
{
    int32_t trueLogicDevId = 0; // devLogicId是用户逻辑DeviceID，需要转换为真实逻辑DeviceID
    aclError aclRet = aclrtGetLogicDevIdByUserDevId(devLogicId, &trueLogicDevId);
    if (aclRet != ACL_SUCCESS) {
        HCCL_ERROR("[%s]aclrtGetLogicDevIdByUserDevId failed, devLogicId: %d, ret: %d", __func__, devLogicId, aclRet);
        return HCCL_E_RUNTIME;
    }
...
HcclResult CommunicatorImpl::DestroyKFCWorkSpaceVA()
{
    ...
    if (accessVA_ != nullptr) {
        drvError_t drvRet = halHostUnregister(accessVA_, trueLogicDevId);
