...
void CommunicatorImpl::CalcA2ASendRecvMem(const CollOpParams &opParams, u64 &sendSize, u64 &recvSize) const
{
    ...
    if (opParams.opType == OpType::ALLTOALLV) {
        for (u32 i = 0; i < rankSize; i++) {
            if (opParams.all2AllVDataDes.sendCounts == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.sendCounts is nullptr");
                THROW<NullPtrException>(msg);
            }
            if ((static_cast<const u64 *>(opParams.all2AllVDataDes.sendCounts) + i) == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.sendCounts[%d] is nullptr", i);
                THROW<NullPtrException>(msg);
            }
            if (opParams.all2AllVDataDes.sdispls == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.sdispls is nullptr");
                THROW<NullPtrException>(msg);
            }
            if ((static_cast<const u64 *>(opParams.all2AllVDataDes.sdispls) + i) == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.sdispls[%d] is nullptr", i);
                THROW<NullPtrException>(msg);
            }
            if (opParams.all2AllVDataDes.recvCounts == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.recvCounts is nullptr");
                THROW<NullPtrException>(msg);
            }
            if ((static_cast<const u64 *>(opParams.all2AllVDataDes.recvCounts) + i) == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.recvCounts[%d] is nullptr", i);
                THROW<NullPtrException>(msg);
            }
            if (opParams.all2AllVDataDes.rdispls == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.rdispls is nullptr");
                THROW<NullPtrException>(msg);
            }
            if ((static_cast<const u64 *>(opParams.all2AllVDataDes.rdispls) + i) == nullptr) {
                std::string msg = StringFormat("opParams.all2AllVDataDes.rdispls[%d] is nullptr", i);
                THROW<NullPtrException>(msg);
            }
