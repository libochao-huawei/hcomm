...
void CommunicatorImpl::CheckRankGraphAddrs() const
{
    ...
    std::shared_ptr<NetInstance::Peer> peer = rankGraph->GetPeer(myRank);
    std::vector<std::shared_ptr<NetInstance::ConnInterface>> interfaces = peer->GetIfaces();
    for(auto &interface : interfaces) {
        if (interface->GetPos() == AddrPosition::DEVICE) {
            RPT_INPUT_ERR(localEidSet.count(interface->GetAddr().GetEid()) == 0, 
                "EI0014", 
