    ...
    DevId inputDevLogicId)
{
    if (!initFlag) {
        ...
        TRY_CATCH_RETURN(
            ...
            DlProfFunction::GetInstance().DlProfFunctionInit(); InitMirrorTaskManager(); InitProfilingReporter();
            InitTaskExceptionHandler(); RegisterKernel(); InitDpuKernel(); SetCommStatus(CommStatus::COMM_READY);
...
static void FastCcuLaunchSaveDfxTaskInfo(
    const CommunicatorImpl& comm, const TaskParam& taskParam, bool isMaster, const RankId remoteRankId = INVALID_RANKID)
{
    u32 taskId;
    u32 streamId;
...
void CommunicatorImpl::TryInitCcuFeature()
{
    ...
    if (commExecuteConfig.accState != AcceleratorState::CCU_MS
        && commExecuteConfig.accState != AcceleratorState::CCU_SCHED) {
        HCCL_RUN_INFO(
            "[CommunicatorImpl][%s] communicator accstate[%s] doesn't use ccu, init steps passed.", __func__,
            commExecuteConfig.accState.Describe().c_str());
        return;
    }
...
void CommunicatorImpl::InitCcuSuperFastLoad()
{
    ...
    HCCL_INFO(
        "taskExceptionEnv[%d], enableProfilingEnv: hostApiState[%d] nodeState[%d] l0State[%d] l1State[%d] l2State[%d]",
...
void CommunicatorImpl::DeInitPreResource() { (void)P2PEnableManager::GetInstance().DisableP2P(enableP2PDevices_); }
...
void CommunicatorImpl::InitNotifyFixedValue() { notifyFixedValue = std::make_unique<NotifyFixedValue>(); }
...
NotifyFixedValue* CommunicatorImpl::GetNotifyFixedValue() const { return notifyFixedValue.get(); }
...
{
    bool isAiv
        = (GetCommExecuteConfig().accState == AcceleratorState::AIV
           || GetCommExecuteConfig().accState == AcceleratorState::AIV_ONLY);
    if (!GetCommCcuFeatureFlag() && !isAiv) { // 通信域粒度
        HCCL_ERROR(
            "CommunicatorImpl::AllocCommResource: Comm accelerator is [%s] not support AllocCommResource",
...
HcclResult CommunicatorImpl::LaunchDpuKernel(aclrtFuncHandle& funcHandle)
{
    ...
    HCCL_INFO(
        "[CommunicatorImpl::%s] DpuKernelLaunchParam{commId:%s; memorySize:%u; shareHBM:%p; hostMem:%p}", __func__,
        hostArgsTemp.commId.c_str(), hostArgsTemp.memorySize, hostArgsTemp.shareHBM, hostArgsTemp.hostMem);
    ...
    if (aclrtLaunchKernelWithHostArgs(
            funcHandle, numBlocks, dpuStream, &cfg, &hostArgsTemp, argsSize, &placeHolderArrays, placeHolderNum)
        != ACL_SUCCESS) {
...
u32 CommunicatorImpl::GetRankInParentComm() { return static_cast<u32>(rankInParentComm); }
