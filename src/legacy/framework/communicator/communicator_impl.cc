...
HcclResult CheckDataTypeRes(OpType opType,HcclResult dataTypeChkRes, std::string operationName, CommStatus &status)
{
    if (dataTypeChkRes != HcclResult::HCCL_SUCCESS) { // 上报入参校验失败EI0003
        RPT_INPUT_ERR(true, "EI0003", std::vector<std::string>({"ccl_op", "value", "parameter", "reason"}),
            std::vector<std::string>({operationName, OpTypeToString(opType), "opDataType",
            "please check DataType that is not found"}));
        HCCL_ERROR("[CommunicatorImpl][%s] DataType[%s] check fail.", operationName, OpTypeToString(opType));
        status = CommStatus::COMM_READY;
    }
...
HcclResult CommunicatorImpl::LoadOpbasedCollOp(const CollOpParams &opParams, void *stream)
{
    try {
        ...
        CHK_RET(CheckDataTypeRes(opParams.opType, dataTypeChkRes, std::string(__func__), status));
...
HcclResult CommunicatorImpl::ReLoadOffloadOp()
{
    ...
    HcclResult dataTypeChkRes = OpParamsChecker::CheckOpDataTypeOffload(curOpParams, GetOpCcuFeatureFlag(),
                                                                        GetOpAiCpuTSFeatureFlag(), isAiv); // 算子粒度
    CHK_RET(CheckDataTypeRes(curOpParams.opType, dataTypeChkRes, std::string(__func__), status));
