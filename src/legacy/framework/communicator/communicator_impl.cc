...
HcclResult CommunicatorImpl::DestroyDpuKernelResource()
{
    ...
    (void)WaitDpuKernelThreadTerminate();

    // 切换回 dpu ctx
    aclError ret = aclrtSetCurrentContext(dpuContext);
    if (ACL_SUCCESS != ret) {
        HCCL_ERROR("set dpu Ctx Failed, aclReturn[%d]", ret);
        return HCCL_E_RUNTIME;
    }
    // 销毁局部流
    ret = aclrtDestroyStreamForce(dpuStream);
    if (ACL_SUCCESS != ret) {
        HCCL_ERROR("Destroy Stream Failed, aclReturn[%d]", ret);
        CHK_PRT_RET(ret = aclrtSetCurrentContext(npuContext), HCCL_ERROR("set npu Ctx Failed, aclReturn[%d]", ret), HCCL_E_RUNTIME);
        return HCCL_E_RUNTIME;
    }
    // reset DPU kernel 线程
    ret = HrtResetXpuDevice(TEMP_DEV_TYPE_DPU, 0);
    if (HCCL_SUCCESS != ret) {
    ...
    // 切回 npu ctx
    ret = aclrtSetCurrentContext(npuContext);
    if (ACL_SUCCESS != ret) {
        HCCL_ERROR("set npu Ctx Failed, aclReturn[%d]", ret);
        return HCCL_E_RUNTIME;
    }
...
HcclResult CommunicatorImpl::WaitDpuKernelThreadTerminate()
{
    ...
    HcclUs        startTime                   = std::chrono::steady_clock::now();
    constexpr u32 waitTransportReadyTimeoutMs = 500 * 1000; // 定义最大等待500秒
