...
AicpuBinaryHolder::~AicpuBinaryHolder()
{
    Unload();
}
...
void AicpuBinaryHolder::Load()
{
    ...
    //register base Func
    for (const auto& kernelName : KernelFunction_)
    {
        if (kernelName == nullptr || strlen(kernelName) == 0) {
            HCCL_ERROR("[AicpuBinaryHolder::%s] invalid kernel name", __func__);
            THROW<InvalidParamsException>("kernel name is null or empty");
        }
