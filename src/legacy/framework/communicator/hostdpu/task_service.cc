...
{
    aclError ret
        = aclrtMemcpy(flagPtr, sizeof(newFlag), &newFlag, sizeof(newFlag), aclrtMemcpyKind::ACL_MEMCPY_HOST_TO_DEVICE);
    if (ret != ACL_SUCCESS) {
...
HcclResult TaskService::ReadTaskType(
    uint8_t *ctrlHdr, uint64_t hdrLen, uint8_t *srcTaskTypePtr, std::string &taskTypeStr) const
{
    CHK_PTR_NULL(srcTaskTypePtr);
    // 读 taskType
    char *taskType = new char[TASKTYPE_ADDR_LENGTH];
    CHK_PTR_NULL(taskType);
    int ret = memcpy_s(taskType, (sizeof(char) * TASKTYPE_ADDR_LENGTH), ctrlHdr + CTRL_HDR_FLAG_LENGTH,
...
HcclResult TaskService::SynchronizeControlInfo(uint8_t *ctrlHdr, uint64_t hdrLen)
{
    ...
    *ctrlHdr = 1;
    ...
    aclError ret = aclrtMemcpy(
        dpu2npuMem_, controlDataSize, ctrlHdr, controlDataSize, aclrtMemcpyKind::ACL_MEMCPY_HOST_TO_DEVICE);
    if (ret != ACL_SUCCESS) {
