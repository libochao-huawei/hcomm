...
HcclResult TaskService::TaskRun()
{
    ...
    if (dataSize_ == 0) {
        HCCL_ERROR("[TaskService::%s] dataSize[%zu] illegal", __func__, (size_t)dataSize_);
        return HCCL_E_INTERNAL;
    }
    if (dataSize_ > hostMemSize_) {
        HCCL_ERROR("[TaskService::%s] hostMemSize[%zu] less than dataSize[%zu]", __func__, (size_t)hostMemSize_, (size_t)dataSize_);
        return HCCL_E_INTERNAL;
    }
    uint8_t flag{0};
