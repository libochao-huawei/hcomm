...
HcclResult TaskService::ExecuteTask(uint8_t *srcPtr, std::string taskTypeStr)
{
    ...
    if (dataSize == 0) {
        HCCL_WARNING("[TaskService::TaskRun] dataSize is 0, no data to copy.");
    ...
    if (dataSize > static_cast<uint32_t>(leftSize_) || dataSize > static_cast<uint32_t>(hostMemSize_)) {
        HCCL_ERROR("[TaskService::%s] dataSize[%u] larger than leftSize[%d] or hostMemSize[%d]", __func__, dataSize,
