...
HcclResult TaskService::ExecuteTask(uint8_t *ctrlHdr, uint64_t hdrLen, uint8_t *srcPtr, std::string taskTypeStr)
{
    ...
    if (dataLen > static_cast<uint32_t>(leftSize_) || dataLen > static_cast<uint32_t>(hostMemSize_)) {
        HCCL_ERROR("[TaskService::%s] dataLen[%u] larger than leftSize[%d] or hostMemSize[%d]", __func__, dataLen,
