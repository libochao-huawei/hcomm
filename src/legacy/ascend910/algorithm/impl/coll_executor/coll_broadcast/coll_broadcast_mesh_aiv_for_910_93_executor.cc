...
HcclResult CollBroadcastMeshAivFor91093Executor::CalNumBlocks(u32& numBlocks, u32 rankSize, u64 dataSize, HcclCMDType cmdType)
{
    ...
    if (numBlocks_ < NUM_BLOCKS_FACTOR_TWO) {
        HCCL_ERROR("[CollBroadcastMeshAivFor91093Executor][CalNumBlocks] aivCore[%u] is invalid, at least need 2", numBlocks_);
        return HCCL_E_PARA;
    }
    ...
    if (numBlocks_ < minNumBlock) {
        HCCL_ERROR("[CollBroadcastMeshAivFor91093Executor][CalNumBlocks] limit[%u], at least[%u]", numBlocks_, minNumBlock);
        return HCCL_E_PARA;
    }

    // 记录日志
    HCCL_INFO("[CollBroadcastMeshAivFor91093Executor][CalNumBlocks] numBlock is set to [%u], limit[%u], best[%u]",
        numBlocks, numBlocks_, bestNumBlock);
    return HCCL_SUCCESS;
}
...
HcclResult CollBroadcastMeshAivFor91093Executor::Orchestrate(OpParam& param, AlgResourceResponse& algRes)
{
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollBroadcastMeshAivFor91093Executor][Orchestrate]errNo[0x%016llx] tag[%s] excutor kernel run failed",
