     ...
     int32_t serverNum, uint64_t rmaInfo, uint64_t len)
{
    ...
    for(uint32_t i = 0; i < rankPerSever; i++) {
        // 等待数据到达
        uint32_t sourceRank = (rank_ + rankSize_ - (blockIdx_ % serverNum + i * serverNum)) % rankSize_; // 从哪个卡收数据
        if(sourceRank == rank_) {
            continue;
        }
    ...
    for (uint32_t i = 0; i < rankPerSever; i++) {
        ...
        __gm__ int32_t *ctrlPostFlagGM = (__gm__ int32_t *)(GM_OUT_RDMA[rank_] + pingpongOffset + flagOffsetBase + ranksizeOffset + localFlagOffset + FLAG_SIZE); // flag标志位
        SetSignalValue(ctrlPostFlagGM, localSetTensor, tag);
        PipeBarrier<PIPE_ALL>();
        AIVRDMAPostSend((GM_ADDR)((uint64_t)ctrlPostFlagGM), (GM_ADDR)((uint64_t)(GM_OUT_RDMA[sourceRank] + pingpongOffset + flagOffsetBase + ranksizeOffset + flagOffset)),
    ...
    for (uint32_t i = 0; i < rankPerSever; i++) {
        ...
        uint64_t localFlagOffset = 2 * targetRank * FLAG_SIZE;
        __gm__ int32_t *ctrlPostFlagGM = (__gm__ int32_t *)(GM_OUT_RDMA[rank_] + pingpongOffset + flagOffsetBase + ranksizeOffset + localFlagOffset);
