...
constexpr uint32_t ASYNC_UNFOLD_MAX_BSR_ITEM_NUM = 128;
constexpr uint32_t ASYNC_UNFOLD_MAX_RANK_SIZE = 128;
constexpr uint32_t ASYNC_UNFOLD_MAX_DYNAMIC_SIZE = 5 * 1024;

// 定义结构体封装算子信息, 用于aicpu异步展开单算子 (< 1 + 129 = 130 KiB)
// Part 1: static part of tiling data (~576 B < 1 KiB)
// Part 2: dynamic part of tiling data (<= 132 + 4 KiB < 5 KiB)
...
//     <= 4 + 128 * 32 + 128 * 1 = 132 + 4 KiB < 5 KiB
...
//     (4) allgatherv/reducescatterv: sizeof(struct OpTilingVDataDes) + rankSize * 2 * sizeof(u64)
//     <= 9 + 128 * 2 * 8 = 9 + 2 KiB < 3 KiB
//     (5) other operators: sizeof(struct OpTilingDataDes) = 9 B
// 注意: 避免HDC开销过大, 对于dynamic part过大的算子, 不进行异步展开
//     (1) alltoallvc: sizeof(struct OpTilingAlltoallvcDataDes) + rankSize * rankSize * sizeof(u64)
//     <= 2 + 128 * 128 * 8 = 2 + 128 KiB < 129 KiB
//     (2) batchsendrcv: bsrItemNum > ASYNC_UNFOLD_MAX_BSR_ITEM_NUM
//     (3) alltoallv: rankSize > ASYNC_UNFOLD_MAX_RANK_SIZE
