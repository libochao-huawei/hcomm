...
namespace hccl {
    ...
    HcclResult AicpuAsyncUnfolder::GetAsyncUnfoldKey(const OpParam &param, AsyncUnfoldKey& asyncUnfoldKey)
    {
        // TODOSSY: 性能打点
        FUNCTION_TRACE;
        ...
        if (opType == HcclCMDType::HCCL_CMD_ALLTOALL || opType == HcclCMDType::HCCL_CMD_ALLTOALLV ||
            opType == HcclCMDType::HCCL_CMD_ALLTOALLVC) { // alltoall类算子
            dataType = param.All2AllDataDes.sendType;
        } else { // 普通算子
            dataType = param.DataDes.dataType;
        }
        CHK_RET(asyncUnfoldKey.Init(param.opBaseOpIdx, opType, dataType));
        return HCCL_SUCCESS;
    }
    ...
    {
        // TODOSSY: 性能打点
        HCCL_ERROR("[AicpuAsyncUnfolder::~AicpuAsyncUnfolder]");
        #ifdef ENABLE_ASYNC_UNFOLD_BREAKDOWN
        HCCL_ERROR("[AicpuAsyncUnfolder::~AicpuAsyncUnfolder] dump timer logs");
    ...
    HcclResult AicpuAsyncUnfolder::Init(const hccl::HDCommunicateParams& tailH2DParams,
        ...
        if (tailH2DParams.buffLen == 0) {
            ...
            EXCEPTION_CATCH((tailH2D_ = std::make_shared<hccl::HDCommunicate>()), return HCCL_E_PTR);
        ...
        if (headD2HParams.buffLen == 0) {
            ...
            EXCEPTION_CATCH((headD2H_ = std::make_shared<hccl::HDCommunicate>()), return HCCL_E_PTR);
        ...
        for (size_t i = 0; i < opH2DRingBufferSize; ++i) {
            ...
            if (opH2DParams.buffLen == 0) {
                ...
                EXCEPTION_CATCH((opH2DRingBuffer_[i] = std::make_shared<hccl::HDCommunicate>()), return HCCL_E_PTR);
    ...
    HcclResult AicpuAsyncUnfolder::UpdateAsyncUnfoldFlag(const HcclWorkflowMode workflowMode, const OpParam &param,
        const bool isUseRdma, const bool isDeviceMode, const bool isZeroCopy, const bool isSymmetricMemory)
    {
        // TODOSSY: 性能打点
        FUNCTION_TRACE;
        ...
        if (isDeviceMode) {
            asyncUnfoldEnable_ = false;
            HCCL_RUN_INFO("[AicpuAsyncUnfolder][UpdateAsyncUnfoldFlag] isDeviceMode[%d]: disable async unfold",
                isDeviceMode);
            return HCCL_SUCCESS;
        }
        ...
        if (isZeroCopy) {
            asyncUnfoldEnable_ = false;
            HCCL_RUN_INFO("[AicpuAsyncUnfolder][UpdateAsyncUnfoldFlag] isZeroCopy[%d]: disable async unfold",
                isZeroCopy);
            return HCCL_SUCCESS;
        }

        // 检查对称内存
        if (isSymmetricMemory) {
            asyncUnfoldEnable_ = false;
            HCCL_RUN_INFO("[AicpuAsyncUnfolder][UpdateAsyncUnfoldFlag] isSymmetricMemory[%d]: disable async unfold",
                isSymmetricMemory);
            return HCCL_SUCCESS;
        }
        ...
        if (isUseRdma) {
            asyncUnfoldEnable_ = false;
            HCCL_RUN_INFO("[AicpuAsyncUnfolder][UpdateAsyncUnfoldFlag] isUseRdma[%d]: disable async unfold", isUseRdma);
            return HCCL_SUCCESS;
        }
    ...
    HcclResult AicpuAsyncUnfolder::GetOpAsyncUnfoldInfo(bool& isValid, OpAsyncUnfoldInfo& opAsyncUnfoldInfo)
    {
        ...
        // 校验获取的OpAsyncUnfoldInfo
        OpTilingData* tilingDataPtr = reinterpret_cast<OpTilingData*>(&opAsyncUnfoldInfo);
        CHK_PTR_NULL(tilingDataPtr);
        CHK_PRT_RET(static_cast<HcclWorkflowMode>(tilingDataPtr->workflowMode) != HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE ||
            tilingDataPtr->isZeroCopy || tilingDataPtr->isSymmetricMemory || tilingDataPtr->isCapture,
            HCCL_ERROR("[AicpuAsyncUnfolder][GetOpAsyncUnfoldInfo] invalid tilingData for async unfold: "
                "workflowMode[%d] isZeroCopy[%d] isSymmetricMemory[%d] isCapture[%d]",
                tilingDataPtr->workflowMode, tilingDataPtr->isZeroCopy, tilingDataPtr->isSymmetricMemory,
                tilingDataPtr->isCapture),
            HCCL_E_INTERNAL);
    ...
    HcclResult AicpuAsyncUnfolder::BackupStreamInfo(const uint64_t opBaseOpIdx,
        Stream &mainStream, std::vector<Stream> &slaveStreams)
    {
        // TODOSSY: 性能打点
        FUNCTION_TRACE;

        // 异步展开op(n)之前, 备份op(n-1)的算子索引
        mainSlaveStreamInfo_.isBackup = true;
        mainSlaveStreamInfo_.opBaseOpIdx = opBaseOpIdx;
        ...
        mainSlaveStreamInfo_.mainFlipNum = mainSqeContextPtr->buffer.filpNum;
        ...
        for (size_t i = 0; i < slaveStreamCnt; ++i) {
            ...
            mainSlaveStreamInfo_.slaveFlipNums[i] = slaveSqeContextPtr->buffer.filpNum;
        }

        HCCL_INFO("[AicpuAsyncUnfolder][BackupStreamInfo] isBackup[%d] opBaseOpIdx[%llu] mainTailSqeTaskId[%u] "
            "mainFlipNum[%u] slaveStreamCnt[%u]",
            mainSlaveStreamInfo_.isBackup, opBaseOpIdx, mainSlaveStreamInfo_.mainTailSqeTaskId,
            mainSlaveStreamInfo_.mainFlipNum, slaveStreamCnt);

        return HCCL_SUCCESS;
    }

    HcclResult AicpuAsyncUnfolder::ClearAsyncUnfoldInfo(const uint64_t opBaseOpIdx, Stream &mainStream,
        std::vector<Stream> &slaveStreams, void* dispatcherPtr)
    {
        HCCL_INFO("[AicpuAsyncUnfolder][ClearAsyncUnfoldInfo] opBaseOpIdx[%llu]", opBaseOpIdx);

        // (1) 恢复各stream异步展开备份的tailSqeTaskId和flipNum (if any)
        CHK_RET(RestoreStreamInfo_(opBaseOpIdx, mainStream, slaveStreams));

        // (2) 清理DispatcherAicpu异步展开的上下文
        CHK_PTR_NULL(dispatcherPtr);
        CHK_RET((reinterpret_cast<DispatcherAiCpu *>(dispatcherPtr))->ClearLaunchAsyncContext());

        // (3) 清空AsyncUnfoldCache
        CHK_RET(asyncUnfoldCache_.ClearAll());

        return HCCL_SUCCESS;
    }

    HcclResult AicpuAsyncUnfolder::RestoreStreamInfo_(const uint64_t opBaseOpIdx,
        Stream &mainStream, std::vector<Stream> &slaveStreams) {
        // 检查是否需要恢复备份信息
        if (!mainSlaveStreamInfo_.isBackup || mainSlaveStreamInfo_.opBaseOpIdx != opBaseOpIdx) {
            HCCL_INFO("[AicpuAsyncUnfolder][RestoreStreamInfo_] isBackup[%d] or "
                "mainSlaveStreamInfo_.opBaseOpIdx[%llu] != opBaseOpIdx[%llu] -> no need to restore stream info",
                mainSlaveStreamInfo_.isBackup, mainSlaveStreamInfo_.opBaseOpIdx, opBaseOpIdx);
            return HCCL_SUCCESS;
        }
        ...
        mainSqeContextPtr->buffer.filpNum = mainSlaveStreamInfo_.mainFlipNum;

        // 恢复从流信息
        const size_t slaveStreamCnt = slaveStreams.size();
        CHK_PRT_RET(mainSlaveStreamInfo_.slaveTailSqeTaskIds.size() != slaveStreamCnt,
            HCCL_ERROR("[AicpuAsyncUnfolder][RestoreStreamInfo_] slaveTailSqeTaskIds.size[%u] != slaveStreamCnt[%u]",
        ...
        CHK_PRT_RET(mainSlaveStreamInfo_.slaveFlipNums.size() != slaveStreamCnt,
            HCCL_ERROR("[AicpuAsyncUnfolder][RestoreStreamInfo_] slaveFlipNums.size[%u] != slaveStreamCnt[%u]",
        ...
        for (size_t i = 0; i < slaveStreamCnt; ++i) {
            ...
            slaveSqeContextPtr->buffer.filpNum = mainSlaveStreamInfo_.slaveFlipNums[i];
        }

        HCCL_INFO("[AicpuAsyncUnfolder][RestoreStreamInfo_] isBackup[%d] opBaseOpIdx[%llu] mainTailSqeTaskId[%u] "
            "mainFlipNum[%u] slaveStreamCnt[%u]",
            mainSlaveStreamInfo_.isBackup, opBaseOpIdx, mainSlaveStreamInfo_.mainTailSqeTaskId,
            mainSlaveStreamInfo_.mainFlipNum, slaveStreamCnt);
