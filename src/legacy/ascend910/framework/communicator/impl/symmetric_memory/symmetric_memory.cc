...
HcclResult SymmetricMemory::AddUrmaSymmetricWindow(std::shared_ptr<SymmetricWindow> &win)
{
    ...
    CHK_PRT_RET(newEnd < newStart,
        HCCL_ERROR("[SymmetricMemory][AddUrmaSymmetricWindow] window address overflow, userVa[%p], size[%zu].",
    ...
    if (insertIt != sortedWindows_.begin()) {
        ...
        CHK_PRT_RET(newStart < prevEnd,
            HCCL_ERROR("[SymmetricMemory][AddUrmaSymmetricWindow] window overlaps previous, userVa[%p], size[%zu], "
    ...
    if (insertIt != sortedWindows_.end()) {
        ...
        CHK_PRT_RET(newEnd > nextStart,
            HCCL_ERROR("[SymmetricMemory][AddUrmaSymmetricWindow] window overlaps next, userVa[%p], size[%zu], "
...
HcclResult SymmetricMemory::RegisterUrmaMode(void* ptr, size_t size, void** devWin)
{
    CHK_PTR_NULL(devWin);
    CHK_PTR_NULL(ptr);
    CHK_PRT_RET(size == 0, HCCL_ERROR("[SymmetricMemory][RegisterUrmaMode] Invalid size: 0."), HCCL_E_PARA);
    CHK_PRT_RET(reinterpret_cast<uintptr_t>(ptr) + size < reinterpret_cast<uintptr_t>(ptr),
        HCCL_ERROR("[SymmetricMemory][RegisterUrmaMode] address overflow, ptr[%p], size[%zu].", ptr, size),
        HCCL_E_PARA);
    if (isSingleRank_) {
    ...
    ret = AddUrmaSymmetricWindow(pWin);
    if (ret != HCCL_SUCCESS) {
        HCCL_ERROR("[SymmetricMemory][RegisterUrmaMode] AddUrmaSymmetricWindow failed, ret[%d].", ret);
        CHK_PRT(hrtFree(devRemoteMems));
        return ret;
    }
    ...
    uint32_t memNum)
{
    ...
    for (uint32_t memIdx = 0; memIdx < memNum; ++memIdx) {
        ...
        for (const auto &resourceItem : memoryResourceMap_) {
            ...
            CHK_PRT_RET(remoteMems[memIdx].addr == nullptr || remoteMems[memIdx].size == 0,
                HCCL_ERROR("[SymmetricMemory][UpdateRemoteMem] invalid remote symmetric mem, tag[%s], "
                    "remoteRank[%u], addr[%p], size[%llu].", resource.memTag.c_str(), remoteRank,
                    remoteMems[memIdx].addr, remoteMems[memIdx].size), HCCL_E_PARA);
            CHK_PRT_RET(remoteMems[memIdx].size != static_cast<uint64_t>(win->userSize),
                HCCL_ERROR("[SymmetricMemory][UpdateRemoteMem] remote symmetric mem size mismatch, tag[%s], "
                    ...
                    win->userSize, remoteMems[memIdx].size), HCCL_E_PARA);
