...
HcclResult HcclDedicatedThreadAcquire(HcclComm comm, uint8_t useType, uint32_t notifyNumPerThread, ThreadHandle *thread)
{
    CHK_PRT_RET(comm == nullptr, HCCL_ERROR("[%s] comm is null", __func__), HCCL_E_PTR);
    ...
    std::string commId = hcclComm->GetIdentifier();
    HCCL_RUN_INFO(
        "Entry-%s:comm[%s] useType[%u] notifyNumPerThread[%u]", __func__, commId.c_str(), useType, notifyNumPerThread);

    {
        ...
        if (hcclComm->HasDedicatedThread(useType)) {
            *thread = hcclComm->GetDedicatedThread(useType);
            HCCL_RUN_INFO("[%s] reuse dedicated thread, useType[%u], thread[0x%llx]", __func__, useType, *thread);
            return HCCL_SUCCESS;
        }
    }
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[%s] Failed to acquire thread for useType[%u], engine[%d], ret[%d]", __func__, useType,
            static_cast<int32_t>(engine), ret),
        ret);

    {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[%s] Failed to cache dedicated thread, useType[%u], ret[%d]", __func__, useType, ret), ret);
    }

    HCCL_RUN_INFO("[%s] success, useType[%u], thread[0x%llx], notifyNumPerThread[%u], group[%s]", __func__, useType,
        *thread, notifyNumPerThread, commId.c_str());
    return HCCL_SUCCESS;
}
