...
HcclResult HcclThreadAcquireWithConfigDfx(hccl::CollComm* collComm, std::string commId, CommEngine engine,
    ...
    ThreadType type, const ThreadConfig *config, ThreadHandle *threads)
{
    ...
    if (hcclComm->IsCommunicatorV2()) {
        ...
        CHK_RET(HcclThreadAcquireWithConfigDfx(collComm, commId, engine, threadNum, threads, threadId));
        return HCCL_SUCCESS;
