...
{
    // 上报故障码EI0005
    RPT_INPUT_ERR(true, "EI0005", std::vector<std::string>({"paraName", "localPara", "remotePara", "tips"}),
                            std::vector<std::string>({paraName, localPara, remotePara, "Please check whether is the same."}));
...
void ReportOpCheckFailed(const std::string &paraName, uint32_t localPara, uint32_t remotePara)
{
    // 上报故障码EI0005
    RPT_INPUT_ERR(true, "EI0005", std::vector<std::string>({"paraName", "localPara", "remotePara", "tips"}),
                            std::vector<std::string>({paraName, std::to_string(localPara), std::to_string(remotePara), "Please check whether is the same."}));
    THROW<InvalidParamsException>(StringFormat(
        "[RankConsistentImpl][CompareFrame][%s]op information %s check fail. "
