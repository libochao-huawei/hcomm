                    ...
                    ResLinks &tempResLinks, std::vector<InsQuePtr> &tempInsQues)
{
    ...
    if (root_ == u32(myRank_)) {
        ...
        CHK_PRT_RET(tempInsQues.size() < requiredSize, HCCL_ERROR("[InsTempScatterMesh1D] tempInsQues size is insufficient: %zu, required: %u"),
                HcclResult::HCCL_E_INTERNAL, tempInsQues.size(), requiredSize);
    } else {
        CHK_PRT_RET(tempInsQues.empty(), HCCL_ERROR("[InsTempScatterMesh1D] tempInsQues is empty for non-root"),
                HcclResult::HCCL_E_INTERNAL);
    }
