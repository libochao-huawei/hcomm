        ...
        for (u32 step = 0; step < nSteps; ++step) {
            ...
            for (u32 j = 0; j < linkNum; j++)
            {
                ...
                for (u32 i = 0; i < stepInfo.nSlices; ++i) {
                    ...
                    txSrcSlices.emplace_back(CalcDataSliceForLinks(txSrcSliceAllLink, dataSplitRate, j));
                    txDstSlices.emplace_back(CalcDataSliceForLinks(txDstSliceAllLink, dataSplitRate, j));
                    rxSrcSlices.emplace_back(CalcDataSliceForLinks(rxSrcSliceAllLink, dataSplitRate, j));
                    rxDstSlices.emplace_back(CalcDataSliceForLinks(rxDstSliceAllLink, dataSplitRate, j));
                }
...
DataSlice InsTempAllGatherNHR::CalcDataSliceForLinks(const DataSlice& recvSrcSliceAllLinks, std::vector<float> dataSplitRate, u32 j) const
{
