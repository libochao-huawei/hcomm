...
HcclResult InsTempAllGatherNHR::LocalDataCopy(std::vector<InsQuePtr> &tempInsQues)
{
    ...
    u64 sliceSize = ((algRankIdx == tempRankSize_ - 1) && (tempAlgParams_.tailSize != 0)) ? tempAlgParams_.tailSize
...
HcclResult InsTempAllGatherNHR::PostLocalCopy(std::vector<InsQuePtr> &tempInsQues)
{
    ...
    for (u64 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        for (u32 algIdx = 0; algIdx < tempRankSize_; ++algIdx) {
            ...
            // 如果是最后一张卡需要用尾部数据计算
            u64 sliceSize = ((algIdx == tempRankSize_ - 1) && (tempAlgParams_.tailSize != 0))
        ...
        for (u32 j = 0; j < linkNum; j++) {
            ...
            for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
                ...
                for (u32 i = 0; i < stepInfo.nSlices; ++i) {
                    ...
                    u64 sliceSize = ((myAlgRank == tempRankSize_ - 1) && (tempAlgParams_.tailSize != 0))
                                        ? tempAlgParams_.tailSize
                                        : tempAlgParams_.sliceSize;
