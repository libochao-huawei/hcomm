    ...
    const TemplateDataParams &tempAlgParams)
{
    ...
    for (u32 rankIdx = 0; rankIdx < tempRankSize_ - 1; rankIdx++) {
        ...
        HCCL_DEBUG("[InsTempReduceScatterMesh1D][RunReduceScatter] myRank[%d], toRank[%d], fromRank[%d], rmAlgRank[%d]",
            myRank_, remoteRank, remoteRank, rmAlgRank);
        CHK_PRT_RET(tempLinks.at(remoteRank).empty(),
            HCCL_ERROR("[InsTempReduceScatterMesh1D][RunReduceScatter] Rank [%d], remoteRank[%d] required links Error.", myRank_, remoteRank),
            HcclResult::HCCL_E_INTERNAL);
        ...
        for (u32 linkIdx = 0; linkIdx < linkNum; linkIdx++) {
            ...
            SendRecvInfo sendRecvInfo{{neighborLinkData, neighborLinkData},
