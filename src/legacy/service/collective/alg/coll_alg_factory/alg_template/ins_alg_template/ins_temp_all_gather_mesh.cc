...
void InsTempAllGatherMesh1D::CalcTxRxSlices(const u32 myAlgRank, u32 connectedAlgRank, std::vector<float> dataSplitRate,
                                            u64 tailSize, u64 sendSliceSize, u64 recvSliceSize, u32 idx,
    ...
    for (u32 rpt = 0; rpt < tempAlgParams_.repeatNum; ++rpt) {
        ...
        txSrcSlices.push_back(CalcDataSliceForLinks(txSrcSlice, dataSplitRate, idx, dataType_));
        txDstSlices.push_back(CalcDataSliceForLinks(txDstSlice, dataSplitRate, idx, dataType_));
        rxSrcSlices.push_back(CalcDataSliceForLinks(rxSrcSlice, dataSplitRate, idx, dataType_));
        rxDstSlices.push_back(CalcDataSliceForLinks(rxDstSlice, dataSplitRate, idx, dataType_));
    }
}
    ...
    const u32 myAlgRank, const std::vector<RankId> &vTopo, std::vector<InsQuePtr> &tempInsQues)
{
    ...
    for (u32 rankIdx = 0; rankIdx < vTopo.size() - 1; rankIdx++) {
        ...
        u64 sendSliceSize = (myAlgRank == tempRankSize_ - 1) ? tailSize : tempAlgParams_.sliceSize;
        u64 recvSliceSize = (connectedAlgRank == tempRankSize_ - 1) ? tailSize : tempAlgParams_.sliceSize;
        ...
        for (u32 j = 0; j < linkNum; j++) {
            CHK_PRT_RET(queIdx >= tempInsQues.size(),
                HCCL_ERROR("[InsTempAllGatherMesh1D] queIdx=%u, tempInsQues.size=%u", queIdx, tempInsQues.size()),
                HcclResult::HCCL_E_INTERNAL);
            ...
            CalcTxRxSlices(myAlgRank, connectedAlgRank, dataSplitRate, tailSize, sendSliceSize, recvSliceSize, 
                            j, txSrcSlices, txDstSlices, rxSrcSlices, rxDstSlices);
            ...
            CHK_PRT_RET(SendRecv(sendRecvInfo, currQue, 0, true, DmaMode::GET),
                HCCL_ERROR("[InsTempAllGatherMesh1D] sendrecv failed (nbr=%d, queIdx=%u)", connectedRank, queIdx),
                HcclResult::HCCL_E_INTERNAL);
        }
    }
    return HcclResult::HCCL_SUCCESS;
