    ...
    const MultiDataLinksDmaModeInfo& dmaModeInfo, std::vector<InsQuePtr>& syncQues, bool& hasDiffDmaMode)
{
    ...
    for (; dataInfoIter != dataInfo.end(); dataInfoIter++, queIter++) {
        ...
        if (tmpLinkType == PortDeploymentType::DEV_NET) {
            CHK_PRT_RET(
                ...
                HCCL_ERROR(
                    "[InsCollAlgFactory] [AlgDataTrans] Send/RecvThruMultiLinks: portType of first link "
    ...
    CHK_PRT_RET(
        netLinkNum > 1,
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] Send/RecvThruMultiLinks: more than one net links, use "
    ...
    const MultiDataLinksDmaModeInfo& dmaModeInfo, std::vector<InsQuePtr>& syncQues, bool& hasDiffDmaMode)
{
    ...
    for (; dataInfoIter != dataInfo.end(); dataInfoIter++, queIter++) {
        ...
        if (tmpLinkType == PortDeploymentType::DEV_NET) {
            CHK_PRT_RET(
                linkType == PortDeploymentType::P2P,
                HCCL_ERROR(
                    "[InsCollAlgFactory] [AlgDataTrans] Send/RecvReduceThruMultiLinks: portType of first link "
                    "should be DEV_NET when there exists NET links."),
                HcclResult::HCCL_E_INTERNAL);
            netLinkNum++;
        }
    ...
    CHK_PRT_RET(
        netLinkNum > 1,
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] Send/RecvReduceThruMultiLinks: more than one net links, "
