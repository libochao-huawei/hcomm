    ...
    std::vector<InsQuePtr> &tempInsQues, TemplateDataParams &templateDataParams, u32 repeat) const
{
    ...
    for (u32 j = 0; j < linkNum; j++) {
        ...
        for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
            ...
            u64 srcDstOffset
                = repeat * ((tempRankSize_ - 1) * templateDataParams.sliceSize + templateDataParams.tailSize)
        ...
        DataInfo sendData(linkSend[j], txSlicesList);
    ...
    std::vector<InsQuePtr> &tempInsQues, TemplateDataParams &templateDataParams, u32 repeat) const
{
    ...
    for (u32 j = 0; j < linkNum; j++) {
        ...
        DataInfo recvData(linkRecv[j], rxSlicesList);
    ...
    std::vector<InsQuePtr> &tempInsQues, TemplateDataParams &templateDataParams, u32 repeat) const
{
    ...
    for (u32 j = 0; j < linkNum; j++) {
        ...
        for (u32 i = 0; i < stepInfo.txSliceIdxs.size(); i++) {
            const u32 txId = stepInfo.txSliceIdxs[i];
            u64 srcDstOffset
                = repeat * ((tempRankSize_ - 1) * templateDataParams.sliceSize + templateDataParams.tailSize)
                  + buffInfo_.scratchBuffBaseOff + txId * templateDataParams.sliceSize;
            u64 sliceSize = templateDataParams.sliceSize;
        ...
        for (u32 i = 0; i < stepInfo.rxSliceIdxs.size(); i++) {
            const u32 rxId = stepInfo.rxSliceIdxs[i];
            u64 srcDstOffset
                = repeat * ((tempRankSize_ - 1) * templateDataParams.sliceSize + templateDataParams.tailSize)
                  + buffInfo_.scratchBuffBaseOff + rxId * templateDataParams.sliceSize;
            u64 sliceSize = templateDataParams.sliceSize;
        ...
        SendRecvInfo sendRecvInfo(sendRecvLinks, txRxSlicesList);
