...
HcclResult MultiSendCounter(const MultiDataInfo& sendInfo, std::vector<InsQuePtr>& queues, u32 topicId, DmaMode dmaMode)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] MultiSendCounter: inter-rank CounterNotify is "
...
HcclResult MultiRecvCounter(const MultiDataInfo& recvInfo, std::vector<InsQuePtr>& queues, u32 topicId, DmaMode dmaMode)
{
    ...
    CHK_PRT_RET(
        !DevCapability::GetInstance().IsSupportStarsPollNetCq(),
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] MultiRecvCounter: inter-rank CounterNotify is "
            "supported only when device supports StarsPollNetCq."),
        HcclResult::HCCL_E_INTERNAL);
    ...
    const MultiSendRecvInfo& sendRecvInfo, std::vector<InsQuePtr>& queues, u32 topicId, DmaMode dmaMode)
{
    ...
    CHK_PRT_RET(
        !DevCapability::GetInstance().IsSupportStarsPollNetCq(),
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] MultiSendRecvCounter: inter-rank CounterNotify is "
            "supported only when device supports StarsPollNetCq."),
        HcclResult::HCCL_E_INTERNAL);
    ...
    const MultiDataReduceInfo& sendInfo, std::vector<InsQuePtr>& queues, u32 topicId, DmaMode dmaMode)
{
    ...
    CHK_PRT_RET(
        !DevCapability::GetInstance().IsSupportStarsPollNetCq(),
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] MultiSendReduceCounter: inter-rank CounterNotify is  "
            "supported only when device supports StarsPollNetCq."),
        HcclResult::HCCL_E_INTERNAL);
    ...
    const MultiDataReduceInfo& recvInfo, std::vector<InsQuePtr>& queues, u32 topicId, DmaMode dmaMode)
{
    ...
    CHK_PRT_RET(
        !DevCapability::GetInstance().IsSupportStarsPollNetCq(),
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] MultiRecvReduceCounter: inter-rank CounterNotify is "
            "supported only when device supports StarsPollNetCq."),
        HcclResult::HCCL_E_INTERNAL);
    ...
    const MultiSendRecvReduceInfo& sendRecvInfo, std::vector<InsQuePtr>& queues, u32 topicId, DmaMode dmaMode)
{
    ...
    CHK_PRT_RET(
        !DevCapability::GetInstance().IsSupportStarsPollNetCq(),
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] MultiSendRecvReduceCounter: inter-rank CounterNotify is "
            "supported only when device supports StarsPollNetCq."),
        HcclResult::HCCL_E_INTERNAL);
    ...
    DmaMode dmaMode)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] SendRecvThruMultiLinks: multi net links is not "
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[InsCollAlgFactory] [AlgDataTrans] SendRecvThruMultiLinks: first link must be NET when there "
            "exists NET links."),
        HcclResult::HCCL_E_INTERNAL);
