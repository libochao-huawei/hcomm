...
std::vector<uint64_t> CcuContextAllToAllVMesh1D::GeneArgs(const CcuTaskArg &arg)
{
    ...
    for (uint64_t i = 0; i < rankSize; i++) {
        ...
        HCCL_INFO("[CcuContextAllToAllVMesh1D][GeneArgs] CalGoSize size[%llu]", tailGoSize);
...
void CcuContextAllToAllVMesh1D::RefreshArgs(CollOpParams opParams, u32 rankSize, std::vector<uint64_t> &args) 
{
    ...
    (void) memcpy_s(&outputAddr, sizeof(outputAddr), &opParams.sendBuf, sizeof(outputAddr));
    ...
    uint64_t xnMaxTransportGoSize = CcuContext::CalGoSizeStatic(xnMaxTransportSize, moConfig);
    for (auto val : xnMaxTransportGoSize) {
        args.push_back(val);
    }
    ...
    for (u32 i = 0; i < rankSize; i++) {
        ...
        u64 sendLenth = curSendCounts * DataTypeSizeGet(opParams.all2AllVDataDes.sendType);
