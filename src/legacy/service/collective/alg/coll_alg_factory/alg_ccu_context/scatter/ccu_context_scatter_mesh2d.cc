    ...
    std::vector<CcuRep::Memory>& relaySrc, std::vector<CcuRep::Memory>& relayDst, uint64_t j)
{
    ...
    for (uint64_t i = 0; i < dimSize_[axisId_]; i++) {
        if (i != dimId_[axisId_]) {
            ...
            HCCL_INFO(
                "[CcuContextScatterMesh2D][PrepareRootSendInfo] src globalId[%u], curRank[%u], j[%llu], i[%llu] ",
    ...
    HCCL_INFO(
        "[CcuContextScatterMesh2D][PrepareRootSendInfo] relaySrcSize[%llu], relayDstSize_[%zu] ", relaySrc.size(),
        relayDst.size());
    ...
    std::vector<CcuRep::Memory>& relaySrc, std::vector<CcuRep::Memory>& relayDst)
{
    ...
    HCCL_INFO(
        "[CcuContextScatterMesh2D][PrepareRootSendInfo] axisId[%llu], dimSize_[%u] ", axisId_, dimSize_[1 - axisId_]);
...
void CcuContextScatterMesh2D::RelaySend(std::vector<CcuRep::Memory>& relaySrc, std::vector<CcuRep::Memory>& relayDst)
{
    ...
    for (uint64_t i = 0; i < dimSize_[axisId_]; i++) {
        if (i != dimId_[axisId_]) {
            ...
            globalId = CoordinateToGlobalId(i, dimId_[1 - axisId_]);
            HCCL_INFO(
                "[CcuContextScatterMesh2D][PrepareRelaySendInfo] src globalId[%u], curRank[%u], axisId[%u], "
                "i:[%u], localId[%u]",
...
{
    HCCL_INFO(
        "[CcuContextScatterMesh2D][NonDirectRecvAlgorithm] start, dimIdX_[%u], dimIdY_[%u]", dimId_[0], dimId_[1]);
    PrepareVariables();
    LoadArgs();
    PreSync();       // 前同步
    Sync(CKE_IDX_0); // 后同步
