...
public:
    ...
    void Init(uint32_t rankId, uint64_t inputAddr, uint64_t outputAddr, uint64_t offset, uint64_t token, 
        CollAlgOperator &op, std::vector<std::vector<RankId>> &tempVTopo, uint64_t iterNum, uint64_t tailOffset, 
        uint64_t tailSize, uint64_t singleTransportSize, uint64_t detourPathNum, uint64_t pathNumPerPeer, 
