    ...
    : CcuContextAlgBase(arg, transports, group)
{
    ...
    HCCL_INFO("[CcuContextReduceMeshMem2Mem1D] CtxArg: rankId[%u] rankSize[%u]", rankId_, rankSize_);
    ...
    for (uint64_t peerId = 0; peerId < rankSize_; peerId++) {
        if (peerId == rankId_) {
            ...
            CHK_PRT_RET(
                ...
                HCCL_ERROR(
                    "[CcuContextReduceMeshMem2Mem1D] Algorithm transport ptr is null or transportIdx is out of "
                    "bounds"), );
            input_.push_back(
