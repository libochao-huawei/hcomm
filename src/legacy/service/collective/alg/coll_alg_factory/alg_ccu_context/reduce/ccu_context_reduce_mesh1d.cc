    ...
    : CcuContextAlgBase(arg, transports, group)
{
    ...
    HCCL_INFO("[CcuContextReduceMesh1D] CtxArg: rankId[%u] rankSize[%u]", rankId_, rankSize_);

    reduceOp_ = ctxArg->op_.reduceOp;
    rootId_ = ctxArg->rootId_;
    HCCL_INFO(
        "[CcuContextReduceMesh1D] init end, ctxArg->dimSize size[%u] rankSize[%llu]", ctxArg->dimSize_.size(),
        rankSize_);
}
