                                                                                  ...
                                                                                  CollOffloadOpResReq &resReq)
{
    ...
    if (enableDetour_) {
        HCCL_DEBUG("Scatter_[InsCollAlgFactory], CalcRes with detouring enabled.");
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Scatter_[InsCollAlgFactory], CalcRes with detouring disabled.");
                                                                           ...
                                                                           CollAlgResReq     &algResReq)
{
    ...
    AlgTempResReq tempResReq;
    if (enableDetour_) {
        HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring enabled.", myRank_);
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring disabled.", myRank_);
        CHK_RET(tempAlg.CalcRes(tempResReq));
    }
    ...
    HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], requiredQueNum [%u].", myRank_, algResReq.primQueueNum);
                                                                              ...
                                                                              InsQuePtr              insQue)
{
    ...
    HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], template [%s], requiredQue Num [%u].", myRank_,
    ...
    if (opMode_ == OpMode::OFFLOAD) {
        HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], Generating Instruction Queues in OFFLOAD Mode for HOST.", myRank_);
        CHK_RET(GenInsQues4Offload(tempAlg));
    } else { // OPBASE
        HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], Generating Instruction Queues in OPBASE Mode for HOST.", myRank_);
...
HcclResult InsScatterSoleExecutor<AlgTopoMatch, InsAlgTemplate>::Orchestrate(const AlgTopoInfo     &topoInfo,
                                                                                 ...
                                                                                 InsQuePtr              insQue)
{
    HCCL_INFO("Scatter_[InsCollAlgFactory] [InsScatterSoleExecutor] AiCpu Orchestrate begins.");
    // 参数校验和初始化
    CHK_RET(Init(op, params, insQue));
    ...
    AlgTempResReq tempResReq;
    if (enableDetour_) {
        HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring enabled for Orchestrate.", myRank_);
        CHK_RET(tempAlg.CalcResDetour(linkMgr, tempResReq));
    } else {
        HCCL_DEBUG("Scatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring disabled for Orchestrate.", myRank_);
        CHK_RET(tempAlg.CalcRes(tempResReq));
    }
