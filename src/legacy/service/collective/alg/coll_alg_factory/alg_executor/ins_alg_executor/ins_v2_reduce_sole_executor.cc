...
{
    if (enableDetour_) {
        HCCL_DEBUG("V2Reduce_[%s] Rank[%d], CalcRes with detouring enabled.", __func__, myRank_);
        CHK_RET(algTemplate->CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("V2Reduce_[%s] Rank[%d], CalcRes with detouring disabled.", __func__, myRank_);
