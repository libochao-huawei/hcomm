...
{
    if (enableDetour_) {
        HCCL_DEBUG("V2Broadcast_[%s] Rank[%d], CalcRes with detouring enabled.", __func__, myRank_);
        CHK_RET(algTemplate->CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("V2Broadcast_[%s] Rank[%d], CalcRes with detouring disabled.", __func__, myRank_);
    ...
    const CollAlgOperator &op, const CollAlgParams   &params, InsQuePtr              insQue)
{
    ...
    CHK_PRT_RET(dataTypeSize_ == 0,
            HCCL_ERROR("V2Broadcast_[CollAlgFactory] Rank [%d], Invalid dataTypeSize_ [%u].", myRank_, dataTypeSize_),
            HcclResult::HCCL_E_INTERNAL);
