    ...
    std::shared_ptr<InsAlgTemplate> algTemplate)
{
    ...
    for (u64 loop = 0; loop < loopTimes; loop++) {
        ...
        HCCL_WARNING("[myinfo][%s]currDataCount:%llu, dataTypeSize_:%llu, sliceSize:%llu, outputSliceStride:%llu",
         __func__, currDataCount, dataTypeSize_, tempAlgParams.sliceSize, tempAlgParams.outputSliceStride);
