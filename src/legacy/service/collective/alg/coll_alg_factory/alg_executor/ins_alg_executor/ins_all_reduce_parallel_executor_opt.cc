    ...
    u64 &memBlockSize, float &SplitRate, u32 &multipleIntraRS, u32 &multipleInterRS, u32 &multipleIntraAG, u32 &multipleInterAG)
{
    ...
    if (nonZeroCount == 0) {
        ...
        if (multipleInterAG == 0 || multipleIntraAG == 0) {
            ...
            u64 memBlockSizeAG = (multipleInterAG == 0) 
            ? std::min(static_cast<u64>(UB_MAX_DATA_SIZE), maxTmpMemSize_ / multipleIntraAG) * templateNum
            : std::min(static_cast<u64>(UB_MAX_DATA_SIZE), maxTmpMemSize_ / multipleInterAG) * templateNum;
