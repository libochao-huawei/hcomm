...
InsBroadcastParallelAiCpuExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::
    InsBroadcastParallelAiCpuExecutor()
    : InsCollAlgBase()
{
}
    ...
    InsAlgTemplate3>::CalcRes(const RankGraph* rankGraph, CollAlgResReq& algResReq)
{
    ...
    if (enableDetour_) {
        ...
        CHK_RET(intraScatterTempAlg.CalcResDetour(rankGraph, resReqIntraScatter));
        CHK_RET(intraAllReduceTempAlg.CalcResDetour(rankGraph, resReqIntraAllReduce));
    } else {
        HCCL_DEBUG("[%s] Rank[%d], CalcRes with detouring disabled.", __func__, myRank_);
        CHK_RET(intraScatterTempAlg.CalcRes(resReqIntraScatter));
        CHK_RET(intraAllReduceTempAlg.CalcRes(resReqIntraAllReduce));
    }
    CHK_RET(interScatterTempAlg.CalcRes(resReqInterScatter));
    CHK_RET(interAllReduceTempAlg.CalcRes(resReqInterAllReduce));
        ...
        InsAlgTemplate2& intraAllReduceTempAlg, InsAlgTemplate3& interAllReduceTempAlg)
{
    ...
    if (enableDetour_) {
        HCCL_DEBUG("[%s] Rank[%d], CalcRes with detouring enabled.", __func__, myRank_);
        CHK_RET(intraScatterTempAlg.CalcResDetour(linkMgr, resReqIntraScatter));
        CHK_RET(intraAllReduceTempAlg.CalcResDetour(linkMgr, resReqIntraAllReduce));
    } else {
        HCCL_DEBUG("[%s] Rank[%d], CalcRes with detouring disabled.", __func__, myRank_);
        CHK_RET(intraScatterTempAlg.CalcRes(resReqIntraScatter));
        CHK_RET(intraAllReduceTempAlg.CalcRes(resReqIntraAllReduce));
        ...
        InsAlgTemplate3& interAllReduceTempAlg)
{
    ...
    scratchMultiple.intraAllReduce = intraAllReduceTempAlg.CalcScratchMultiple(BufferType::INPUT, BufferType::INPUT);
    scratchMultiple.interAllReduce = interAllReduceTempAlg.CalcScratchMultiple(BufferType::INPUT, BufferType::INPUT);
        ...
        InsAlgTemplate2& intraAllReduceTempAlg, InsAlgTemplate3& interAllReduceTempAlg)
{
    ...
    std::vector<StageProcAlgPara> stageProcAlgParaVec = {
        ...
        {[&](auto&... args) { return intraAllReduceTempAlg.GenExtIns(args...); }, scratchOffset.intraAllReduceStage2,
         intraLinks_, intraQue_, // stage2 part0
         [&](auto&... args) { return interAllReduceTempAlg.GenExtIns(args...); }, scratchOffset.interAllReduceStage2,
         interLinks_, interQue_}, // stage2 part1
        {[&](auto&... args) { return interAllReduceTempAlg.GenExtIns(args...); }, scratchOffset.interAllReduceStage3,
         interLinks_, interQue_, // stage3 part0
         [&](auto&... args) { return intraAllReduceTempAlg.GenExtIns(args...); }, scratchOffset.intraAllReduceStage3,
         intraLinks_, intraQue_}, // stage3 part1
    };
