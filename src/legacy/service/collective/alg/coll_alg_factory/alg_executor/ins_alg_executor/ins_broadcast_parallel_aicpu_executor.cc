...
InsBroadcastParallelAiCpuExecutor<AlgTopoMatch, InsAlgTemplate0, InsAlgTemplate1, InsAlgTemplate2, InsAlgTemplate3>::
    CalcSingleAlgRes(
        ...
        InsAlgTemplate3& interAllReduce, T* type, AlgTempResReq& resReqIntraScatter, AlgTempResReq& resReqInterScatter,
        AlgTempResReq& resReqIntraAllReduce, AlgTempResReq& resReqInterAllReduce)
{
    if (enableDetour_) {
        ...
        CHK_RET(intraAllReduce.CalcResDetour(type, resReqIntraAllReduce));
    } else {
    ...
    InsAlgTemplate3>::CalcRes(const RankGraph* rankGraph, CollAlgResReq& algResReq)
{
    ...
    CHK_RET(PreCalcRes(rankGraph, resReqIntraScatter, resReqInterScatter, resReqIntraAllReduce, resReqInterAllReduce));
        ...
        AlgTempResReq& resReqIntraAllReduce, AlgTempResReq& resReqInterAllReduce)
{
    ...
    CHK_RET(WrapPrepResLinks(type, resReqIntraAllReduce.links, intraLinks_));
    CHK_RET(WrapPrepResLinks(type, resReqInterAllReduce.links, interLinks_));
    HCCL_INFO(
        ...
        InsAlgTemplate2& intraAllReduceTempAlg, InsAlgTemplate3& interAllReduceTempAlg)
{
    ...
    CHK_RET(CalcSingleAlgRes(
        intraScatterTempAlg, interScatterTempAlg, intraAllReduceTempAlg, interAllReduceTempAlg, rankGraph,
        resReqIntraScatter, resReqInterScatter, resReqIntraAllReduce, resReqInterAllReduce));
    CHK_RET(PrepareRes(rankGraph, resReqIntraScatter, resReqInterScatter, resReqIntraAllReduce, resReqInterAllReduce));
        ...
        InsAlgTemplate2& intraAllReduceTempAlg, InsAlgTemplate3& interAllReduceTempAlg)
{
    ...
    CHK_RET(CalcSingleAlgRes(
        ...
        resReqIntraScatter, resReqInterScatter, resReqIntraAllReduce, resReqInterAllReduce));
    CHK_RET(PrepareRes(linkMgr, resReqIntraScatter, resReqInterScatter, resReqIntraAllReduce, resReqInterAllReduce));
