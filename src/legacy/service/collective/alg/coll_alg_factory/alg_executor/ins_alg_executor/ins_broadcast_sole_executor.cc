                                                                                      ...
                                                                                      CollOffloadOpResReq  &resReq)
{
    ...
    if (enableDetour_) {
        HCCL_DEBUG("Broadcast_[InsCollAlgFactory], CalcRes with detouring enabled.");
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Broadcast_[InsCollAlgFactory], CalcRes with detouring disabled.");
    ...
    CollAlgResReq        &algResReq)
{
    ...
    AlgTempResReq tempResReq;
    if (enableDetour_) {
        HCCL_DEBUG("Broadcast_[InsCollAlgFactory] Rank[%d], CalcRes with detouring enabled.", myRank_);
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Broadcast_[InsCollAlgFactory] Rank[%d], CalcRes with detouring disabled.", myRank_);
        CHK_RET(tempAlg.CalcRes(tempResReq));
    }
    ...
    HCCL_DEBUG("Broadcast_[InsCollAlgFactory] Rank[%d], requiredQueNum [%u].", myRank_, algResReq.primQueueNum);
    ...
    const CollAlgOperator &op, const CollAlgParams   &params, InsQuePtr              insQue)
{
    ...
    AlgTempResReq tempResReq;
    if (enableDetour_) {
        HCCL_DEBUG("Broadcast_[InsCollAlgFactory] Rank[%d], CalcRes with detouring enabled for Orchestrate.", myRank_);
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Broadcast_[InsCollAlgFactory] Rank[%d], CalcRes with detouring disabled for Orchestrate.", myRank_);
        CHK_RET(tempAlg.CalcRes(tempResReq));
    }
    // 申请算法模板所需资源
...
HcclResult InsBroadcastSoleExecutor<AlgTopoMatch, InsAlgTemplate>::Orchestrate(const AlgTopoInfo     &topoInfo,
    ...
    InsQuePtr              insQue)
{
    HCCL_INFO("Broadcast_[InsCollAlgFactory] [InsReduceSoleExecutor] AiCpu Orchestrate begins.");
    // 参数校验和初始化
    CHK_RET(Init(op, params, insQue));
...
HcclResult InsBroadcastSoleExecutor<AlgTopoMatch, InsAlgTemplate>::OrchestrateOpbase(InsAlgTemplate &tempAlg)
{
    ...
    for (u32 loop = 0; loop < loopTimes; loop++) {
        ...
        HCCL_DEBUG("Broadcast_[InsCollAlgFactory] Rank[%d], done generating instruction queues, currSize[%llu], currOffset[%llu].",
