                                                                                      ...
                                                                                      CollOffloadOpResReq  &resReq)
{
    ...
    if (enableDetour_) {
        HCCL_DEBUG("Reducescatter_[InsCollAlgFactory], CalcRes with detouring enabled.");
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Reducescatter_[InsCollAlgFactory], CalcRes with detouring disabled.");
                                                                               ...
                                                                               CollAlgResReq        &algResReq)
{
    ...
    AlgTempResReq tempResReq;
    if (enableDetour_) {
        HCCL_DEBUG("Reducescatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring enabled.", myRank_);
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Reducescatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring disabled.", myRank_);
        CHK_RET(tempAlg.CalcRes(tempResReq));
    }
    ...
    HCCL_DEBUG("Reducescatter_[InsCollAlgFactory] Rank[%d], requiredQueNum [%u].", myRank_, algResReq.primQueueNum);
                                                                                   ...
                                                                                   InsQuePtr              insQue)
{
    ...
    AlgTempResReq tempResReq;
    if (enableDetour_) {
        HCCL_DEBUG("Reducescatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring enabled for Orchestrate.", myRank_);
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Reducescatter_[InsCollAlgFactory] Rank[%d], CalcRes with detouring disabled for Orchestrate.", myRank_);
        CHK_RET(tempAlg.CalcRes(tempResReq));
    }
    // 申请算法模板所需资源
    ...
    HCCL_DEBUG("Reducescatter_[InsCollAlgFactory] Rank[%d], Generating Instruction Queues in OFFLOAD Mode for HOST.", myRank_);
