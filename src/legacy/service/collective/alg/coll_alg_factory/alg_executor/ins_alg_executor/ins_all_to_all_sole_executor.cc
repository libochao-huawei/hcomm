                                                                                  ...
                                                                                  CollOffloadOpResReq &resReq)
{
    ...
    if (enableDetour_) {
        HCCL_DEBUG("Alltoall_[InsCollAlgFactory], CalcRes with detouring enabled.");
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Alltoall_[InsCollAlgFactory], CalcRes with detouring disabled.");
                                                                              ...
                                                                              InsQuePtr              insQue)
{
    ...
    AlgTempResReq tempResReq;
    if (enableDetour_) {
        HCCL_DEBUG("Alltoall_[InsCollAlgFactory] Rank[%d], CalcRes with detouring enabled.", myRank_);
        CHK_RET(tempAlg.CalcResDetour(rankGraph, tempResReq));
    } else {
        HCCL_DEBUG("Alltoall_[InsCollAlgFactory] Rank[%d], CalcRes with detouring disabled.", myRank_);
        CHK_RET(tempAlg.CalcRes(tempResReq));
    }
