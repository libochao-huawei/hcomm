...
HcclResult HcclP2pSchedulerGenerate(HcclComm comm, u32 rankSize)
{
    ...
    if (collComm->plannerV2 == nullptr) {
        HCCL_ERROR("[HcclP2pSchedulerGenerate] plannerV2 is null, comm[%p]", comm);
        return HCCL_E_PTR;
    }
    ...
    HcclComm comm, std::vector<HcclP2pTask> &sortedSendQue, std::vector<HcclP2pTask> &sortedRecvQue)
{
    ...
    if (collComm->plannerV2 == nullptr) {
        HCCL_ERROR("[HcclP2pTaskSchedule] plannerV2 is null, comm[%p]", comm);
        return HCCL_E_PTR;
    }
...
HcclResult initGroupPlanner(HcclComm comm) {
    ...
    if (collComm->plannerV2 == nullptr) {
        HCCL_ERROR("[initGroupPlanner] plannerV2 is null, comm[%p]", comm);
        return HCCL_E_PTR;
    }
    
    std::shared_ptr<hcclKernelPlannerV2> planner = collComm->plannerV2;
    planner->rankSize = collComm->GetRankSize(); 
    HCCL_DEBUG("[initGroupPlanner] ranksize: %d", planner->rankSize);
...
HcclResult HcclGroupAddP2pTaskV2(HcclComm comm, const HcclP2pTask& task, const HcclOpP2pDesc& p2pDesc)
{
    ...
    if (collComm->plannerV2 == nullptr) {
        HCCL_ERROR("[HcclGroupAddP2pTaskV2] plannerV2 is null, comm[%p]", comm);
        return HCCL_E_PTR;
    }
    
    std::shared_ptr<hcclKernelPlannerV2> planner = collComm->plannerV2;

    HCCL_INFO("[HcclGroupAddP2pTaskV2] init comm %d ", planner->nTasksP2p);
    if(planner->nTasksP2p == -1){
        HCCL_INFO("[HcclGroupAddP2pTaskV2] init comm %d ", planner->nTasksP2p);
        ret = initGroupPlanner(comm);
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[HcclGroupAddP2pTaskV2] initGroupPlanner failed, ret[%d]", ret);
            return ret;
        }
        
        ret = HcclP2pSchedulerGenerate(comm, planner->rankSize);
        if (ret != HCCL_SUCCESS) {
            HCCL_ERROR("[HcclGroupAddP2pTaskV2] HcclP2pSchedulerGenerate failed, ret[%d]", ret);
            return ret;
        }
    ...
    if (itComm == hcclGroupCommListV2.end()) {
        HCCL_INFO("[HcclGroupAddP2pTaskV2] add comm");
...
HcclResult HcclAicpuKernelLaunch(HcclComm comm, HcclOpDesc opInfo, HcclKernelFuncInfo funcInfo,
    void *args, uint32_t argSize, ThreadHandle aicpuThreadHandle, aclrtStream userStream)
{
    HCCL_INFO("[HcclAicpuKernelLaunch] start");
    CHK_PTR_NULL(comm);
...
{
    HCCL_INFO("[LaunchP2pExec] to the start");
    aclrtFuncHandle funcHandle;
    aclrtArgsHandle argsHandle;
    // 1. 获取 function handle
    hccl::hcclComm* hcclComm = static_cast<hccl::hcclComm *>(comm);
...
static HcclResult groupLaunchA5()
{
    ...
    for (HcclComm comm : hcclGroupCommListV2) {
        HCCL_INFO("[hcclGroupCommListV2] comm");
        hccl::hcclComm *hcclComm = static_cast<hccl::hcclComm *>(comm);
        CollComm *collComm = hcclComm->GetCollComm();
        CHK_PTR_NULL(collComm);
        /*新建send/recv流*/
        ...
        // std::string sortSendList = rankQueToString(sortedSendQue);
        // HCCL_DEBUG("send dst rank list:[%s]", sortSendList.c_str());

        // std::string sortRecvList = rankQueToString(sortedRecvQue);
        // HCCL_DEBUG("recv dst rank list:[%s]", sortRecvList.c_str());
        ...
        if (!sortedSendQue.empty()) {
            ...
            collComm->plannerV2->usrStream = sortedSendQue[0].usrStream;
        } else if (!sortedRecvQue.empty()) {
            unfoldStream = sortedRecvQue[0].stream;
            collComm->plannerV2->usrStream = sortedRecvQue[0].usrStream;
        } else {
            continue;
        }
        CHK_RET(HcclThreadAcquireWithStream(comm, COMM_ENGINE_CPU_TS, unfoldStream, 1, &cpuTsThread));
        ...
        // 下发wait kernel
        CHK_RET(LaunchNotifyWaitToThread(comm, unfoldStream, aicpuSendThread, 0, "RunAicpuNotifyWait"));
        CHK_RET(LaunchNotifyWaitToThread(comm, unfoldStream, aicpuRecvThread, 0, "RunAicpuNotifyWait"));
        // // Send/Recv交替执行以避免死锁
        // size_t sendIdx = 0;
        // size_t recvIdx = 0;
        // while (sendIdx < sortedSendQue.size() || recvIdx < sortedRecvQue.size()) {
        //     if (sendIdx < sortedSendQue.size()) {
        //         CHK_RET(LaunchP2pExec(comm, sortedSendQue[sendIdx].stream, 
        //             sortedSendQue[sendIdx].funcInfo, sortedSendQue[sendIdx].args, 
        //             aicpuSendThread, "HcclP2pLaunchGroupAicpuKernel"));
        //         sendIdx++;
        //     }
        //     if (recvIdx < sortedRecvQue.size()) {
        //         CHK_RET(LaunchP2pExec(comm, sortedRecvQue[recvIdx].stream,
        //             sortedRecvQue[recvIdx].funcInfo, sortedRecvQue[recvIdx].args,
        //             aicpuRecvThread, "HcclP2pLaunchGroupAicpuKernel"));
        //         recvIdx++;
        //     }
        // }
        ...
        for (const auto &task : sortedSendQue) {
            CHK_RET(LaunchP2pExec(
                comm, task.stream, task.funcInfo, task.args, aicpuSendThread, "HcclP2pLaunchGroupAicpuKernel"));
        }

        for (const auto &task : sortedRecvQue) {
            CHK_RET(LaunchP2pExec(
                comm, task.stream, task.funcInfo, task.args, aicpuRecvThread, "HcclP2pLaunchGroupAicpuKernel"));
        }
        
        // 下发record kernel
        CHK_RET(LaunchNotifyRecordToThread(
            comm, unfoldStream, aicpuSendThread, exportedAicpuTsThread, 0, "RunAicpuNotifyRecord"));
        CHK_RET(LaunchNotifyRecordToThread(
            comm, unfoldStream, aicpuRecvThread, exportedAicpuTsThread, 0, "RunAicpuNotifyRecord"));
