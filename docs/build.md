...
> 本文提到的编译态和运行态含义如下：
>
> - 编译态：针对仅编译本项目不运行的场景，需要CPU环境并安装CANN Toolkit开发套件包。
> - 运行态：针对运行本项目的场景（编译运行或纯运行），需要NPU环境并安装驱动固件、CANN Toolkit开发套件包与CANN ops算子包。
...
> [!NOTE]说明
> 上板测试前，请确保满足运行态环境要求，即已安装驱动固件、CANN Toolkit开发套件包与CANN ops算子包。
