...
sequenceDiagram
    ...
    participant Panic as PrintPanicLogInfo
...
| `CcuMissionContext` raw | custom channel 查询 | 供 `GetCcuErrorMsg` 提取 `currIns` / `startIns` / `endIns` |
| `CcuLoopContext` raw | custom channel 查询 | 供 `GenErrorInfoLoop` 提取 `currentCnt` / `addrStride` / `currentIns` |
...
5. 随后在 `GetCcuErrorMsg` / `GenErrorInfoLoop` 中查询 mission/loop raw，并通过 ops 提取最小标量，完成 errorInfo 组装；
...
6. `GetCcuErrorMsg` 只调用 `ops->getMissionInfo(&missionContext, &missionInfo)`，用 `missionInfo.currentIns/startIns/endIns` 参与 REP 查询与日志输出；
7. `GenErrorInfoLoop` 只调用 `ops->getLoopInfo(&loopContext, &loopInfo)`，并直接检查 `HcclResult`，用 `loopInfo.currentCnt/addrStride/currentIns` 组装 loop errorInfo；
...
- `CcuLoopContext`：版本私有 getter，仅抽 `currentIns` / `currentCnt` / `addrStride`；
...
| loop 标量提取 | `getLoopInfo` 提取 `currentCnt/addrStride/currentIns` |
...
sequenceDiagram
    ...
    alt PrintPanicLogInfo
        ...
        Caller->>Impl: ops->getMissionInfo(&missionContext, &missionInfo)
        Impl-->>Caller: currentIns / startIns / endIns
        Caller->>Caller: 用 currentIns 查 REP，组装 errorInfo
    else GenErrorInfoLoop
        Caller->>Impl: ops->getLoopInfo(&loopContext, &loopInfo)
        Impl-->>Caller: currentIns / currentCnt / addrStride
...
| `ccumDfxInfoV2` | `ccuTaskException.cc` | next V2 raw（位域 + `valid_bits` union） |
...
| `struct CcuLoopInfo` | loop 最小标量：`currentIns`、`currentCnt`、`addrStride` |
...
- `GetCcuLoopInfoV1/V2` 都可以直接复用 raw `GetCurrentIns()` / `GetCurrentCnt()` / `GetAddrStride()`。

另外，当前 `GetCcuMissionContext()` / `GetCcuLoopContext()` 仍保持“返回 raw struct”接口：

- custom channel 查询失败时，函数直接返回**零初始化 context**；
- 后续由 `getMissionInfo` / `getLoopInfo` 继续做最小标量抽取；
- 文档中的错误传播描述，均以这一当前实现现状为准，而不是假定 raw 查询 helper 已经改成 `HcclResult + out param`。
...
HcclResult CcuTaskException::GenErrorInfoLoop(...)
{
    ...
    CHK_RET(ops->getLoopInfo(&loopContext, &loopInfo));
...
6. `panicLog == nullptr`、ops 获取失败都必须先行返回并打印明确日志；当前 mission/loop raw 查询 helper 仍保留“失败返回零初始化 struct”的实现；
...
| 功能-UT | V1 `getMissionInfo` | 高 | 构造 `CcuMissionContextV1` | `currentIns` / `startIns` / `endIns` 正确 | 现有 UT 可复用扩展 |
| 功能-UT | V2 `getMissionInfo` | 高 | 构造 `CcuMissionContextV2` | `currentIns` / `startIns` / `endIns` 正确 | `Ut_CcuMissionContextV2_*` |
| 功能-UT | V1 `getLoopInfo` | 高 | 构造 `CcuLoopContextV1` | 返回 `HCCL_SUCCESS`，且 `currentIns` / `currentCnt` / `addrStride` 正确 | 现有 UT 可复用扩展 |
| 功能-UT | V2 `getLoopInfo` | 高 | 构造 `CcuLoopContextV2` | 返回 `HCCL_SUCCESS`，且 `currentIns` / `currentCnt` / `addrStride` 正确 | `Ut_CcuLoopContextV2_*` |
| 功能-UT | raw context 查询失败保护 | 高 | `hrtGetDevicePhyIdByIndex` 失败 | 返回零初始化 `CcuMissionContext` / `CcuLoopContext` | `Ut_GetCcuMissionContext_WhenGetDevicePhyIdFail_ExpectZeroInitializedContext`、`Ut_GetCcuLoopContext_WhenGetDevicePhyIdFail_ExpectZeroInitializedContext` |
...
| 功能-UT | getter 入参 nullptr 校验 | 高 | rawData 或 out 为 nullptr | 返回错误码并 `HCCL_ERROR` | 待补（当前内部静态函数未直接暴露） |
...
| 性能-UT | 单次异常路径判型次数 | 中 | 一次异常路径内统计判型函数调用 | 调用次数与 `GetCcuOps()` 实际调用次数一致 | 待补 |
