...
│  ├─ 默认路径 ${ASCEND_HOME_PATH}/hcomm_plugin/                │
...
HCOMM 库加载时，自动扫描 `${ASCEND_HOME_PATH}/hcomm_plugin/` 目录，对该目录下所有 `*.so` 文件执行 `dlopen` → `dlsym` 三个导出符号 → 注册到插件表。

其中 `ASCEND_HOME_PATH` 为 CANN 工具包的安装根路径。用户只需将编译好的插件 `.so` 放置到 `<ascend_home>/hcomm_plugin/` 目录下，重启进程即可生效，无需额外配置。
...
两种方式的加载优先级：优先使用默认路径；仅当 `ASCEND_HOME_PATH` 未设置或为空时，才处理 `HCOMM_NIC_PLUGIN_SO`（如有设置）。若同一协议号在同一次加载中被重复注册，后加载的覆盖先加载的并记录 WARNING 日志。
...
1. 获取 `ASCEND_HOME_PATH` 环境变量，若非空则拼接默认目录 `<ascend_home>/hcomm_plugin/`
2. `opendir` / `readdir` 扫描默认目录下所有 `*.so` 文件，对每个调用 `LoadOneSo()`
3. 若 `ASCEND_HOME_PATH` 未设置或为空，则读取 `HCOMM_NIC_PLUGIN_SO` 环境变量（备选方案），按 `:` 分隔后逐路径调用 `LoadOneSo()`
...
- **部署策略**：将插件 .so 放入 `<ascend_home>/hcomm_plugin/` 目录（生产环境）或通过 `HCOMM_NIC_PLUGIN_SO` 环境变量指定（调试环境），单节点独立控制不受集群中其他节点影响
