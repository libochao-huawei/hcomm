...
struct CondExpr {
    ...
    uint64_t imm;       // 比较的立即数
    CcuConditionType cond; // CCU_CONDITION_EQ 或 CCU_CONDITION_NE
};
} // namespace ccu
} // namespace AscendC
```
...
```cpp
ccu::Variable v;
v == 0;   // 产生 CondExpr{&v, 0, CCU_CONDITION_EQ}
v != 10;  // 产生 CondExpr{&v, 10, CCU_CONDITION_NE}
```
...
CcuResult MyKernel(CcuKernelArg arg) {
    Variable flag;
    LoadArg(flag, 0);   // 从 taskArgs[0] 加载运行期标志
