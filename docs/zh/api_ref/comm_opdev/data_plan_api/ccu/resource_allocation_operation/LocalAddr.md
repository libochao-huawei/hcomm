...
C++ 构造只申请虚拟句柄：在 kernel 注册阶段内调用时恒成功；若不在 kernel 注册阶段调用，则构造时抛出异常（携带错误码 `CCU_E_PTR`）。GSA/XN 物理资源不足时，在 `HcommCcuKernelRegister` 阶段返回 `CCU_E_UNAVAIL`，不是在构造时抛出。

> [!CAUTION]注意
> `LocalAddr` 类有 copy / move 构造函数，它们只拷贝 `handle` / `addr.handle` / `token.handle` 三个字段、不申请新 GSA/XN——`LocalAddr l2 = l1;` 后 `l1` 和 `l2` 指向同一组寄存器。但 `operator=(const LocalAddr& other)`（赋值，不是构造）不是 handle 拷贝：它会执行 `this->addr = other.addr; this->token = other.token;`，即发出两条 device 端寄存器赋值指令（参见 `Address::operator=` / `Variable::operator=`），操作的是两组独立寄存器之间的值搬移。两者语义不对称，使用时需特别留意。
