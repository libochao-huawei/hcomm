...
> [!CAUTION]注意
> `Address`类有copy/move构造函数，它们只拷贝`handle`字段、不申请新的GSA——`Address a2 = a1;` 后`a1`和`a2`指向同一个地址寄存器。如需独立的 Address，必须显式`Address a2;`默认构造。`operator=(const Address&)`则会发出device端寄存器赋值指令，操作的是两个独立寄存器之间的值搬移。
