...
等待其他Thread发送的同步信号，主要用于多Thread之间的同步等待场景。**使用通过[HcommSetNotifyWaitTimeOut](./HcommSetNotifyWaitTimeOut.md)设置的默认超时时间**，无需手动传入超时参数。
...
| 参数名 | 输入/输出 | 描述 |
| --- | --- | --- |
| thread | 输入 | 本地通信线程句柄，为通过[HcclThreadAcquire](./HclThreadAcquire.md)接口获取到的threads。<br>ThreadHandle类型的定义请参见[ThreadHandle](../datatype_definition/ThreadHandle.md)。 |
| notifyIdx | 输入 | 本地Notify索引。<br>取值范围为：[0, [HcommThreadAcquire](./HcommThreadAcquire.md)接口传入的notifyNumPerThread参数的值)。 |
...
- [HcommSetNotifyWaitTimeOut](./HcommSetNotifyTimeOut.md) - 设置默认超时时间
