...
| 重载2 | N 个本端MS Buffer → `buffers[0]`（2 ≤ N ≤ 8） | `buffers[0] = reduce(buffers[0..N-1], opType)`，硬件一次性归约N 路MS 到 `buffers[0]`。输入数据类型可以为（`HCCL_DATA_TYPE_UINT8`/`HCCL_DATA_TYPE_INT16`/`HCCL_DATA_TYPE_INT32`/`HCCL_DATA_TYPE_FP16`/`HCCL_DATA_TYPE_BF16`/`HCCL_DATA_TYPE_FP32`），经过Reduce计算后输出数据类型与输入类型一致。 |
