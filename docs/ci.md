...
- UT_Test 门禁所执行的命令为：
    ...
    > 注意：执行时会指定 `cann_3rd_lib_path` 参数，会直接使用环境中的三方件编译缓存，因此在执行时不会重新编译三方件
...
1. 运行日志下载
    ...
    <p><img src="./figures/ci/ci_ut_1.png" width="100%" /></p>

2. 覆盖率报告下载

    在 PR “下载链接”位置下载覆盖率报告，文件名为：`ut_cov.tar.gz`

    <p><img src="./figures/ci/ci_ut_2.png" width="80%" /></p>
...
3. 排查覆盖率问题
    ...
    查看 `ut_cov.tar.gz` 中的 `inc_cov/result/index.html`，增量行覆盖率是否满足门禁要求

    > 注意：当前门禁要求行覆盖率 >= 50%，未来将逐步提高至 80%

    <p><img src="./figures/ci/ci_ut_3.png" width="100%" /></p>

### 5. UT_Test 门禁任务失败 FAQ

#### 5.1 覆盖率报告中 inc_cov 文件夹为空
...
> 注意：申请屏蔽需找 maintainer （@yanyefeng、@yanzhenghang）加分才能合入
