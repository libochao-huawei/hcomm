...
- 多线程调用通信操作API时（例如HcclAllReduce等），用户需确保不同线程中调用通信操作API的前后时间差不超过环境变量[HCCL_CONNECT_TIMEOUT](https://gitcode.com/cann/hccl/tree/master/docs/user_guide/hccl_env/HCCL_CONNECT_TIMEOUT.md)的时间，避免建链超时。
- 不支持一张卡同时调用多个通信操作API。
