...
typedef enum {
    ...
    HCOMM_TRANSFER_TYPE_NOTIFY_WAIT = 7,                    /* 等待通知事件（指定超时时间），对应transferInfo.notifyWait */
    HCOMM_TRANSFER_TYPE_NOTIFY_WAIT_WITH_DEFAULT_TIMEOUT = 8 /* 等待通知事件（默认超时时间），对应transferInfo.notifyWaitWithDefaultTimeout */
} HcommTransferType;
```
