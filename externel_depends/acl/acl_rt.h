...
typedef enum {
    ...
    ACL_DEV_ATTR_CUBE_CORE_NUM   = 102,  // Cube Core数量

    ACL_DEV_ATTR_VECTOR_CORE_NUM = 201,  // Vector Core数量
    ACL_DEV_ATTR_WARP_SIZE       = 202,  // 一个Warp里的线程数
    ACL_DEV_ATTR_MAX_THREAD_PER_VECTOR_CORE = 203,    // 每个Vector Core上可同时驻留的最大线程数
    ACL_DEV_ATTR_LOCAL_MEM_PER_VECTOR_CORE  = 204,    // 每个Vector Core上可以使用的最大本地内存，单位Byte

    ACL_DEV_ATTR_TOTAL_GLOBAL_MEM_SIZE = 301,    // Device上的可用总内存，单位Byte
    ACL_DEV_ATTR_L2_CACHE_SIZE         = 302,    // L2 Cache（二级缓存）大小，单位Byte
    ...
    ACL_DEV_ATTR_CUST_OP_PRIVILEGE = 406U,      // indecates whether the custom operator privilege is enabled
 ...
 * @param destMax [in]       array of destination address memory max length
 ...
 * @retval OtherValues Failure
 */
ACL_FUNC_VISIBILITY aclError aclrtIpcMemGetExportKey(void *devPtr, size_t size, char *key, size_t len, uint64_t flag);
 ...
 * @retval OtherValues Failure
 */
ACL_FUNC_VISIBILITY aclError aclrtIpcMemImportByKey(void **devPtr, const char *key, uint64_t flag);
 ...
 * @retval OtherValues Failure
 */
ACL_FUNC_VISIBILITY aclError aclrtNotifyGetExportKey(aclrtNotify notify, char *key, size_t len, uint64_t flag);
 ...
 * @retval OtherValues Failure
 */
ACL_FUNC_VISIBILITY aclError aclrtNotifyImportByKey(aclrtNotify *notify, const char *key, uint64_t flag);
