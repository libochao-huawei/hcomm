...
ACL_FUNC_VISIBILITY aclError aclrtMemcpyBatchImpl(void **dsts, size_t *destMax, void **srcs, size_t *sizes,
    size_t numBatches, aclrtMemcpyBatchAttr *attrs, size_t *attrsIndexes, size_t numAttrs, size_t *failIndex);

ACL_FUNC_VISIBILITY aclError aclrtIpcMemGetExportKeyImpl(void *devPtr, size_t size, char *key, size_t len, uint64_t flag);

ACL_FUNC_VISIBILITY aclError aclrtIpcMemCloseImpl(const char *key);

ACL_FUNC_VISIBILITY aclError aclrtIpcMemImportByKeyImpl(void **devPtr, const char *key, uint64_t flag);
...
ACL_FUNC_VISIBILITY aclError aclrtNotifyGetExportKeyImpl(aclrtNotify notify, char *key, size_t len, uint64_t flag);

ACL_FUNC_VISIBILITY aclError aclrtNotifyImportByKeyImpl(aclrtNotify *notify, const char *key, uint64_t flag);
