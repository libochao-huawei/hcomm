...
function make_ut_gov() {
  if [[ "X$ENABLE_UT" = "Xon" && "X$ENABLE_GCOV" = "Xon" ]]; then
    ...
    lcov -c -d ${BUILD_DIR}/test/ut/ -d ${BUILD_DIR}/test/legacy/ut/ -o cov/coverage.info -rc geninfo_unexecuted_block=1 \
    --ignore-errors mismatch,empty,inconsistent,source --ignore-errors source
